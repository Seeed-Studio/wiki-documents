...
Type-Cケーブルを使用してGrove Vision AI V2をコンピューターに接続し、**Connect**ボタンをクリックしてください。
...
モデルが正常にアップロードされると、左側のPreviewでGrove Vision AI V2カメラからのライブフィードを確認できます。
...
- **IoU:** IoUは、予測されたバウンディングボックスと真のバウンディングボックスの精度を評価するために使用されます。
...
- `bool begin(TwoWire *wire = &Wire, uint16_t address = I2C_ADDRESS, uint32_t wait_delay = 2, uint32_t clock = 400000)` —— Grove Vision AI V2を初期化します。
  ...
  **入力パラメータ:**
    ...
    - `uint16_t address` —— これはI2Cバスに接続された特定のデバイスを識別するI2Cデバイスのアドレスです。
    - `uint32_t wait_delay` —— コマンドを送信する前に応答を待つ遅延（ミリ秒）。
    - `uint32_t clock` —— これはI2Cバスのクロックレート（Hz）です。

  **戻り値:** `True`または`False`。初期化成功はtrue、初期化失敗はfalse。
