...
ESP32C3のピン数の制限により、エンジニアはXIAO ESP32C3が他のXIAO シリーズと同じ数のGPIOを持つことを保証するため、バッテリーの電圧測定に割り当てる追加のピンがありませんでした。
...
幸い、各チップの校正済み補正値がヒューズエリアに書き込まれており、`analogReadMilliVolts()`関数を使用することで、特別なことをしなくても補正された電圧値を読み取ることができます。
