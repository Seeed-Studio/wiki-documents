---
description: ReSpeaker XVF3800 USB 4-Mic Array は、AEC、ビームフォーミング、ノイズ抑制、360°音声キャプチャを備えたプロフェッショナルな円形マイクロフォンアレイです。XIAO ESP32S3 と組み合わせることで、スマートデバイス、ロボティクス、IoT アプリケーション向けの高度な音声制御を実現します。シームレスな統合とデュアルモードの柔軟性をご体験ください。

title: reSpeaker XVF3800 USB Mic Array と XIAO ESP32S3 による HTTP オーディオストリーミング
...
slug: /ja/respeaker_xvf3800_xiao_http_audio_stream
...
このガイドでは、ESP32-S3（XIAO ESP32S3）を使用して XVF3800 マイクロフォンアレイからオーディオをキャプチャし、16 kHz ステレオ 32 ビット PCM オーディオを 5 秒間録音し、大きなオーディオバッファを処理するためにデータを PSRAM に保存する方法について説明します。録音されたオーディオは Wi-Fi 経由で Python ベースの Flask HTTP サーバーにアップロードされ、そこで受信されて標準的な WAV ファイルとして保存されます。このセットアップは、音声キャプチャ、リモートオーディオログ、音声テキスト変換（STT）や機械学習ベースのオーディオ分析を含む音声処理パイプラインなどのアプリケーションに適しています。
...
## Python サーバーコード（Flask）

**環境に flask をインストールして実行する必要があります** `pip install flask`
