...
このキットはXIAO nRF52840とWio-SX1262を組み合わせ、BLE5.0、Thread、Zigbee、LoRa（862-930MHz）をサポートし、低消費電力で動作します。[Meshtastic®](https://meshtastic.org/)、センサーネットワーク、環境監視、資産追跡、教育、イノベーションなど、様々な分野に応用できます。
...
- **多機能マイクロコントローラー**: FPU搭載のNordic nRF52840チップを搭載し、最大64MHzで動作、複数の開発ポートを実装、Arduino / CircuitPython / Micropythonをサポート
- **強力なLoRa性能**: Semtech SX1262による長距離LoRa、868/915 MHzバンドをサポート。
- **デュアルコア通信**: BluetoothとLoRaの両方のデュアルモードをサポートし、クロスドメイン相互接続を可能にします。
- **8mm × 22mm × 23mm**のコンパクトサイズで、統合アンテナ設計を特徴としています。
...
XIAO nRF52840 & Wio-SX1262 キットには、Meshtasticファームウェアがプリフラッシュされており、すぐに開始できます。
...
[Meshtastic Flasher](https://flasher.meshtastic.org/)にアクセスして`Serial Monitor`を開き、`nodenum`を検索して対応するノードIDを見つけます。
...
:::info
**EU_868**は、1時間ごとのデューティサイクル制限10%を遵守する必要があり、1時間のローリングベースで毎分計算されます。制限に達すると、再び許可されるまでデバイスは送信を停止します。
:::
