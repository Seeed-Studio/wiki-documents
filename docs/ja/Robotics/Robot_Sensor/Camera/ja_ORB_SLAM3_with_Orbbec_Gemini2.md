---
description: このwikiでは、高度なビジュアルSLAMアプリケーション向けに、Orbbec Gemini2 RGB-Dカメラを使用してreComputer Jetson上でORB-SLAM3アルゴリズムをセットアップして実行するための詳細な手順を提供します。
...
```
**ステップ 3.** キャリブレーションチェッカーボードをダウンロードする

キャリブレーションチェッカーボードを以下からダウンロードします
