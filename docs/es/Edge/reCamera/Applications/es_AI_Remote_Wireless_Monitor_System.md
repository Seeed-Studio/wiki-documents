...
También se integra con Node-RED y hace que el proceso de desarrollo sea muy conveniente y eficiente. Si quieres rtsp para empujar stream o http para enviar datos, solo necesitas arrastrar el nodo para implementarlo, y no necesitas construir una gran cantidad de programas desde cero. Consulta el enlace para más detalles: [Tutorial de Node-RED en reCamera](https://wiki.seeedstudio.com/es/recamera_develop_with_node-red/)

Aquí hay un ejemplo de desarrollo de Node-RED: simplemente arrastra 3 nodos para construir un flujo de trabajo de visión IA:
...
La estructura del sistema es muy simple. Los dos módulos HaLow usan cable Ethernet para conectar la reCamera y reTerminal respectivamente. Después de eso, los dos dispositivos transmiten inalámbricamente a través de Wifi HaLow.
...
Una conexión es reCamera a la PC para suministrar energía, y las otras dos se usan para suministrar energía al módulo Wifi Halow.

- **Teclado/Ratón**: Conectar reTerminal para depuración.

- **Adaptador de Energía USB-C 5V3A** Raspberry Pi 4/5 Misma Fuente de Alimentación para Alimentar reTerminal. Enlace de Compra: [Fuente de Alimentación USB-C 5V3A](https://www.seeedstudio.com/Wall-Adapter-Power-Supply-5VDC-3A-Type-C-p-4103.html)
...
La idea general de todo el sistema es permitir que la reCamera forme una red con el equipo terminal a través de Wifi HaloW. Primero, configurar IP estática en el equipo terminal (reTerminal) y reCamera, porque las aplicaciones al aire libre o móviles no pueden tener routers, no hay servicio DHCP, y la IP necesita ser asignada manualmente. Después de que la IP es asignada, estos dispositivos forman una "red de área local", y pueden acceder entre sí a través de IP. En este momento, crear nodos de stream y nodos http post en la reCamera a través de Node-RED, enviar el flujo de video en forma de push stream rtsp, y enviar el resultado de detección yolo en forma de http post. Finalmente, el flujo de video y los resultados de detección AI son accedidos en el dispositivo terminal, como VLC Media Player en Windows/Linux. Este tutorial demostrará cómo obtener push stream rtsp a través de ffplay en Linux.

### Fase 1: Configurar reCamera

#### Paso 1.1 Iniciar Sesión en reCamera

Conecta la reCamera al ordenador con un cable USB-C. Toma algo de tiempo iniciar el dispositivo justo después de encenderlo. Después de esperar 1-2 minutos, abre cualquier navegador web e ingresa **192.68.42.1** para entrar al sistema reCamera.
...
Haz doble clic en el nodo de cámara para entrar a la página de configuración. Para asegurar fluidez, configuramos el nodo de cámara como **480P, 5 fotogramas**. Puedes probar una resolución o tasa de fotogramas más alta, pero la latencia puede ser mayor.
...
Como se mencionó anteriormente, necesitamos configurar push stream rtsp en Node-RED. En la lista de nodos a la izquierda, desliza la rueda del ratón hacia abajo hasta el final para encontrar el nodo Stream.
...
#### Paso 1.4 Configurar el nodo http

Para mostrar los resultados detectados por yolo en el equipo terminal, necesitamos enviar los resultados de detección en forma de http post. Para detalles, por favor consulta [Configuración HTTP Post de Node-RED](https://wiki.seeedstudio.com/es/getting_started_for_home_assistant_with_recamera/#after-entering-the-workspace-configure-the-nodes). Nota que cuando configures la URL del http post aquí, necesita ser configurada como http://192.168.nnn.nnn:8123/recamera_detection 192.168.nnnn.nnn es la dirección IP estática del reTerminal o tu propio otro equipo terminal, no la dirección IP estática de reCamera mencionada arriba. 192.168.10.3 se usa aquí para demostración. El resultado final es el siguiente:
...
Repite el paso 1.5 para regresar a la Configuración de reCamera y verificar si la IP estática está configurada exitosamente:
...
ffmpeg se usa para obtener la transmisión rtsp, nmap se usa para escuchar http post. ejecuta en la Terminal del reTerminal:
...
#### Paso 4.4 reproduce la transmisión de video rtsp
...
En este momento aparecerá una nueva ventana, la transmisión de video rtsp se mostrará dentro, hacer doble clic en la ventana puede reproducir en pantalla completa.

#### Paso 4.5 Monitorear Resultados de Prueba Yolo

Presiona **Ctrl + Alt + T** para abrir una nueva terminal. Ejecuta el comando ncat para monitorear los resultados de detección yolo del reCamera (este comando pertenece a nmap):

```
ncat -l -p 8123
```

En esta terminal, puedes ver el número de personas que reCamera detectó debajo de la imagen muestra el efecto final, la transmisión de video y los resultados de detección AI se muestran en el reTerminal.

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/Applications/Wifi_HaLow/t14.png" /></div>

**¡Felicidades! Has implementado tu propio sistema de detección AI remoto.**

## Explora lo que es Posible

Como se mencionó al principio, este proyecto es solo un ejemplo simple de detección AI remota. Puedes expandirlo a escenarios más complejos. Introduciremos algunos ejemplos aquí. Todos estos ejemplos se actualizarán a Github más tarde.
