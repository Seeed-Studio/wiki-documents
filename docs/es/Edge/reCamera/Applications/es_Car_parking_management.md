---
title: Demo de Monitoreo de Plazas de Estacionamiento con IA usando reCamera
description: Este wiki presenta un demo de monitoreo de plazas de estacionamiento basado en IA usando reCamera, mostrando detección y visualización de disponibilidad de estacionamiento en tiempo real.
keywords:
    - Detección de Plazas de Estacionamiento
    - reCamera
    - Visión IA Edge
...
- **Detección de Plazas de Estacionamiento**: Detecta el estado de ocupación de cada plaza de estacionamiento en la vista de la cámara.
- **Anti-vibración / Estabilización**: Reduce las vibraciones visuales y fluctuaciones de detección a corto plazo para hacer los resultados más estables.
- **Conteo y Resumen**: Resume automáticamente el estado actual del estacionamiento, como el número de plazas disponibles.
- **Visualización en Pantalla**: Muestra los resultados de detección y estado de las plazas directamente en la interfaz de vista previa para verificación rápida y demostración.
...
Una vez que el flujo de trabajo se haya subido y desplegado exitosamente, reCamera comenzará automáticamente a ejecutar el demo de monitoreo de plazas de estacionamiento en segundo plano. Este flujo de trabajo está diseñado como un **pipeline de monitoreo de plazas de estacionamiento de extremo a extremo**, ejecutándose completamente en reCamera. La lógica de alto nivel es la siguiente:
...
3. **Asociación de Plazas y Estabilización**  
   ...
   - Cada plaza entra en un *estado estable* solo después de ser detectada consistentemente por un número fijo de fotogramas.
   - Se toleran fallos a corto plazo para prevenir cambios de estado falsos.

4. **Gestión del Pool de Plazas**  
   - Cada plaza de estacionamiento se almacena en un pool de plazas con su posición, historial de estado y contador de estabilidad.
   - Las plazas que desaparecen por demasiado tiempo se eliminan automáticamente.
...
- **Cajas Delimitadoras**  
  Cada plaza de estacionamiento está asociada con una región detectada. El sistema clasifica cada región como `free` o `car` basado en la salida del modelo de IA.

- **Marcador Central (Círculo)**  
  Se dibuja un círculo en el centro de cada plaza de estacionamiento *estable*.  
  - **Círculo verde** indica una plaza libre  
  - **Círculo rojo** indica una plaza ocupada  

- **Etiquetas y Coordenadas de Plazas**  
  Cada plaza está etiquetada (ej., `Slot1`, `Slot2`, `Slot3`) junto con sus coordenadas centrales `(x, y)`.  
  Estas etiquetas se mapean desde la lista de nombres de plazas que proporcionas vía Node-RED.

- **Panel de Estado (Esquina Superior Izquierda)**  
  El panel de superposición resume el estado general:
  - **Monitoring Slots**: Todas las plazas actualmente siendo rastreadas
  - **Free Slots**: Plazas que están confirmadas como libres después de validación multi-fotograma
...
:::note
La lógica actual de detección de plazas de estacionamiento está específicamente diseñada para **tres plazas de estacionamiento adyacentes dispuestas lado a lado**. En este demo, **reCamera está instalada frente a los espacios de estacionamiento**, mirando directamente a los vehículos, en lugar de usar una vista desde arriba (vista de pájaro).  

Como resultado, la asociación de plazas, posicionamiento de puntos centrales y lógica de estabilidad están optimizados para una **perspectiva frontal**. Si planeas usar una cámara aérea o un diseño de estacionamiento diferente, el mapeo de plazas y la lógica de detección pueden necesitar ajustarse en consecuencia.
:::
