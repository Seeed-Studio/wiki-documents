...
<svg viewBox="0 0 880 480" width="100%" height="auto" style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '10px', fontFamily: 'sans-serif' }}>
  ...
  {/* Title */}
  <text x="440" y="30" textAnchor="middle" fontSize="20" fontWeight="bold" fill="#333333">Arquitectura de Depuración Embebida</text>

  {/* 1. HOST PC AREA (Expanded Size) */}
  <rect x="20" y="60" width="420" height="390" rx="10" fill="#f0f4f8" stroke="#cbd5e0" strokeWidth="2" />
  <text x="40" y="90" fontSize="16" fontWeight="bold" fill="#555555">PC Host (Software)</text>

  {/* GDB Client */}
  <rect x="50" y="120" width="140" height="60" rx="5" fill="#fff3cd" stroke="#ffecb5" strokeWidth="2" />
  <text x="120" y="145" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#856404">Cliente GDB</text>
  ...
  <text x="120" y="325" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#0c5460">Cliente Telnet</text>
  <text x="120" y="345" textAnchor="middle" fontSize="10" fill="#0c5460">(Humano / Script)</text>
  ...
  <text x="350" y="180" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#155724">Servidor de Depuración</text>
  ...
  <text x="350" y="275" textAnchor="middle" fontSize="11" fill="#155724" fontStyle="italic">Analiza GDB/Telnet</text>
  <text x="350" y="290" textAnchor="middle" fontSize="11" fill="#155724" fontStyle="italic">Traduce a USB</text>
  ...
  <text x="235" y="135" textAnchor="middle" fontSize="10" fill="#666666" fontWeight="bold">Puerto TCP 3333</text>
  <text x="235" y="147" textAnchor="middle" fontSize="10" fill="#666666">(Protocolo RSP)</text>
  ...
  <text x="235" y="345" textAnchor="middle" fontSize="10" fill="#666666" fontWeight="bold">Puerto TCP 4444</text>
  <text x="235" y="357" textAnchor="middle" fontSize="10" fill="#666666">(Comandos CLI)</text>
  ...
  <text x="580" y="210" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#383d41">Sonda de Depuración</text>
  <text x="580" y="230" textAnchor="middle" fontSize="10" fill="#555555">(Dongle de Hardware)</text>
  ...
  <text x="580" y="295" textAnchor="middle" fontSize="9" fill="#555555">Maneja USB &lt;-&gt; SWD</text>

  {/* 5. TARGET MCU */}
  <rect x="720" y="190" width="130" height="130" rx="5" fill="#ffeeba" stroke="#ffdf7e" strokeWidth="2" />
  <text x="785" y="220" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#856404">MCU Objetivo</text>
  <rect x="735" y="240" width="100" height="60" rx="2" fill="#ffffff" stroke="#856404" strokeWidth="1" />
  <text x="785" y="265" textAnchor="middle" fontSize="11" fill="#333333">Núcleo</text>
  ...
  <text x="510" y="380" fontSize="12" fontWeight="bold" fill="#333333">Conceptos Clave:</text>
  <circle cx="520" cy="400" r="3" fill="#155724" />
  <text x="530" y="404" fontSize="11" fill="#555555">OpenOCD/PyOCD: Traduce comandos GDB a USB.</text>
  <circle cx="520" cy="420" r="3" fill="#007bff" />
  <text x="530" y="424" fontSize="11" fill="#555555">CMSIS-DAP: El protocolo USB estándar.</text>
  <circle cx="520" cy="440" r="3" fill="#d63384" />
  <text x="530" y="444" fontSize="11" fill="#555555">DAPLink: El firmware ejecutándose en la sonda.</text>
...
El XIAO Debug Mate está diseñado para todo el ecosistema XIAO, pero la función de depuración DAPLink es específica para núcleos ARM Cortex-M. Las funciones de UART y monitoreo de energía, sin embargo, son universalmente compatibles ya que son independientes de la arquitectura. Por favor, consulta la tabla a continuación para información detallada de compatibilidad.
...
1.  Descarga Zadig desde [su sitio web oficial](https://zadig.akeo.ie/).
2.  Conecta el XIAO Debug Mate a tu PC.
3.  Ejecuta Zadig. Ve a `Options` y asegúrate de que `List All Devices` esté marcado.
...
4.  De la lista desplegable, encuentra la interfaz del dispositivo llamada **`CMSIS-DAP`**.
...
5.  Selecciona el controlador **`WinUSB`** en la caja a la derecha de la flecha verde.
6.  Haz clic en `Replace Driver`. Después de que la instalación esté completa, tu sistema estará listo.
...
Ve a la [página de lanzamientos de xPack OpenOCD](https://github.com/xpack-dev-tools/openocd-xpack/releases). Desplázate hacia abajo hasta el último lanzamiento y encuentra la versión de Windows (por ejemplo, `xpack-openocd-...-win32-x64.zip`). Descarga este archivo.
...
Si la instalación fue exitosa, verás la información de versión impresa (por ejemplo, `xPack Open On-Chip Debugger 0.12.0+dev-02228`).
...
Ve a la [página de lanzamientos de xPack OpenOCD](https://github.com/xpack-dev-tools/openocd-xpack/releases). Encuentra el último lanzamiento y descarga el archivo correcto para tu Mac (por ejemplo, `...-darwin-x64.tar.gz` para Intel o `...-darwin-arm64.tar.gz` para Apple Silicon).
...
El cambio de `PATH` que hiciste no está activo en tu sesión de terminal actual hasta que el archivo de configuración sea recargado.
...
Ve a la [página de lanzamientos de xPack OpenOCD](https://github.com/xpack-dev-tools/openocd-xpack/releases). Encuentra el último lanzamiento y descarga el archivo de Linux (`...-linux-x64.tar.gz`).
...
El cambio de `PATH` que realizaste no está activo en tu sesión de terminal actual hasta que se recargue el archivo de configuración.
...
Deberías ver la información correcta de la versión de xPack.
...
1.  **Prerrequisitos**
    Asegúrate de tener **Python** instalado en tu sistema. Si no lo tienes, descárgalo desde el sitio web oficial [python.org](python.org) y asegúrate de marcar la casilla **"Add Python to PATH"** durante la instalación.
...
El XIAO Debug Mate está diseñado para una experiencia perfecta de plug-and-play con todas las placas Seeed Studio XIAO.
...
3.  Conecta los cables desde los conectores de expansión del XIAO Debug Mate a tu placa objetivo de la siguiente manera:
...
| `VCC`                   | `VTref` o `3V3`          | Referencia de Voltaje del Objetivo (Importante para el cambio de nivel) |
...
<TabItem value="Linux" label="Linux">
La mayoría de las distribuciones de Linux vienen con un cliente Telnet. Si no está presente, puedes instalarlo fácilmente usando tu gestor de paquetes. Para sistemas basados en Debian/Ubuntu, abre una terminal y ejecuta:
```bash
sudo apt-get update
...
Encuentra tu placa XIAO en la lista a continuación y sigue las instrucciones específicas para tu sistema operativo para iniciar el servidor OpenOCD. Este proceso del servidor esperará una conexión desde un depurador como GDB.
...
*   **Carpeta de Destino:** Copia este archivo en la carpeta de scripts `target` de tu instalación xPack OpenOCD.
...
*   `-c "..."`: La bandera `-c` pasa un **c**omando a OpenOCD después de que se inicia.
    *   `adapter speed 5000`: (Para RP2040/RP2350) Establece la velocidad del reloj SWD a 5000 kHz para una comunicación estable.
...
*   `4444` es el número de puerto predeterminado que OpenOCD usa para conexiones Telnet.
...
| `mdw [address] [count]` | **Mostrar Memoria Palabra.** Lee y muestra el contenido de la memoria. Por ejemplo, `mdw 0x20000000 10` mostrará 10 palabras (valores de 32 bits) comenzando desde la dirección de memoria `0x20000000`. |
...
Esta tabla resume qué placas XIAO y tipos de archivos son compatibles para el flasheo por línea de comandos con las herramientas proporcionadas.
...
2. Archivos de bootloader: Solo se proporcionan aquí archivos de bootloader XIAO y comandos de flasheo. Si este modelo XIAO no viene con archivos de bootloader, se marcará con ❌.
...
3.  **Terminal/Símbolo del Sistema:** Necesitarás estar cómodo abriendo y usando una interfaz de línea de comandos (por ejemplo, Command Prompt, PowerShell, o Terminal).
...
Simplemente reemplaza la ruta de ejemplo (por ejemplo, `examples/Blink_RP2040.elf`) con la ruta a tu propio archivo de firmware.
...
*   **.hex (Intel HEX):** Una representación de texto ASCII del código binario. Incluye direcciones de memoria, checksums y metadatos, haciéndolo un formato más robusto que `.bin`.
...
*   **PlatformIO:** Después de una compilación exitosa, los archivos de firmware (`firmware.bin`, `firmware.hex`, `firmware.elf`) se encuentran en el directorio `.pio/build/<nombre_entorno>/` de tu proyecto.
...
Cuando uses OpenOCD para flashear el XIAO RP2040, se recomienda encarecidamente usar archivos `.elf` en lugar de archivos `.bin`.

Esto es porque los archivos `.elf` ya contienen la información de direcciones de memoria del programa (Dirección del Enlazador) internamente. OpenOCD automáticamente lo escribirá en la ubicación correcta (usualmente `0x10000000`), por lo que no necesitas calcular manualmente o adivinar la dirección de desplazamiento.
...
Cuando uses OpenOCD para flashear el XIAO RP2350, se recomienda encarecidamente usar archivos `.elf` en lugar de archivos `.bin`.
...
Esto es porque los Bootloaders de chips de la serie Nordic (como nRF52832) necesitan verificar información de validez sobre el firmware de la aplicación en una dirección de memoria específica (usualmente llamada "Bootloader Settings Page") durante el arranque. Esta "página de configuraciones" contiene el checksum CRC del firmware, longitud y banderas de validez.

Cuando usas herramientas de programación de propósito general como OpenOCD para grabar directamente archivos .bin, solo escriben los datos binarios del firmware en la dirección de inicio de aplicación especificada, y no generan ni escriben automáticamente la "página de configuración" requerida por el Bootloader. Por lo tanto, cuando el dispositivo se reinicia, el Bootloader no puede detectar una página de configuración válida y determina que no hay una aplicación legítima, rechazando así iniciar el firmware y usualmente entrando en modo DFU (Device Firmware Update), esperando actualizaciones vía OTA o puerto serie.
:::
...
3.  **Configurar `platformio.ini`**: Este es el paso más crítico. Para habilitar la depuración con el XIAO Debug Mate y reconocer correctamente la placa, debes modificar el archivo `platformio.ini` ubicado en la raíz de tu proyecto.
    ...
    **Agregando Soporte para Placas de Seeed Studio:**
    ...
    El archivo de configuración le dice a PlatformIO que descargue las definiciones de placa desde este repositorio personalizado, y especifica qué herramientas usar para cargar y depurar. El XIAO Debug Mate funciona como un depurador **CMSIS-DAP** estándar.
...
Ten en cuenta que el RA4M1 es un caso especial. Usa `pyocd` para cargar pero aún usa `cmsis-dap` para depurar.
...
3.  **Controles de Depuración**: PlatformIO compilará tu código, lo cargará al XIAO vía el Debug Mate, y pausará la ejecución en la primera línea de `main()` o `setup()`. Una barra de herramientas de depuración aparecerá en la parte superior del editor, permitiéndote controlar el flujo del programa:
...
Si quieres que el programa se pause en una ubicación específica, los puntos de interrupción son la herramienta para el trabajo.
...
2.  **Ejecutar hasta el Punto de Interrupción**: Inicia una sesión de depuración (`F5`). El programa se ejecutará a toda velocidad y se pausará automáticamente cuando alcance la línea con el punto de interrupción.
...
2.  **Observar Variables**: Si quieres monitorear variables o expresiones específicas, ve a la sección "WATCH", haz clic en el icono `+`, e ingresa el nombre de la variable (ej., `myVariable` o `&myVariable` para ver su dirección de memoria).
...
Puedes intentar volver a flashear el bootloader para restaurar el dispositivo. Para el modelo XIAO RA4M1, consulta **[este método](#para-seeed-studio-xiao-ra4m1-2)** para la recuperación del bootloader.

Al restaurar el Bootloader, si ocurre un error, intenta conectar el XIAO RA4M1 al XIAO Debug Mate, que está conectado a tu computadora. Mantén presionado el botón B en el XIAO RA4M1 mientras simultáneamente haces clic en el botón R una vez, luego suelta tanto R como B. Procede a flashear el Bootloader nuevamente.

Después de flashear el Bootloader, intenta volver a actualizar el firmware usando software como Arduino. Esto usualmente restaura la funcionalidad. Si no tiene éxito, el dispositivo puede estar permanentemente dañado e irreparable. Cualquier acción que involucre flashear firmware con XIAO Debug Mate puede requerir que asumas toda la responsabilidad.


### P2: ¿Cómo puedes saber si un dispositivo ha sido dañado?

La forma más directa de determinar si el dispositivo XIAO está dañado es que no muestre conexión de puerto serie cuando se conecta a una computadora. Si el dispositivo aún no muestra conexión de puerto serie después de mantener presionado el botón B y luego conectarlo a la computadora, el dispositivo está dañado.


### P3: ¿Se puede reparar el equipo dañado? ¿Puedo contactar al soporte técnico para solicitar un reemplazo?

El daño a los dispositivos XIAO típicamente es causado por problemas con el particionado o el archivo Bootloader. Esto generalmente se puede resolver reflasheando el archivo Bootloader. Si esto falla, actualmente no hay soluciones alternativas. Los usuarios deben asumir la responsabilidad completa por realizar esta operación ellos mismos, así que por favor asegúrense de que los comandos de flasheo y el firmware estén verificados y sean válidos.
