...
5. Download and install can-utils to use essential tools such as candump and cansend.
...
6. While monitoring can0, send frames from can1. If you can see the message, it means the path is normal.
...
7. Conversely, you can monitor can1 while sending frames from can0. If you can see the message, it means the path is normal.
...
8. If you want to make subsequent startups simpler, you can write the startup command into a script. You only need to execute it once at startup to automatically pull the interface.
