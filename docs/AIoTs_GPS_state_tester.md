...
The project is based on the Arduino platform which means Arduino IDE and various Arduino libraries are required. If this is your first time using the Wio terminal, we recommand a guide to quickly [Get Started with Wio Terminal](https://wiki.seeedstudio.com/Wio-Terminal-Getting-Started/).
...
- **Step 4**. Once the firmware is set up, click "connect using WebUSB" to connect the Wio Terminal to the website. And it is time to start training the action.
...
- **Step 10**. Download the [code](https://github.com/0hotpotman0/AIoTs_GPS_state_tester) file from github and open it with Arduino IDE. Change the third-party library of your own and run the code.
