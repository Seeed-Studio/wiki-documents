...
```
每次新打开一个终端，都应先执行上面的激活命令。
当终端提示符前出现 (lerobot)（如下所示）时，说明环境切换成功：

(lerobot) user@host:~$

此时执行的所有 Python 相关命令，都会基于该环境运行。
...
5. 使用 miniconda 时，在环境中安装 ffmpeg：
...
7. (电脑端可跳过这一步) 对于 Jetson Jetpack 6.0+ 设备（请确保在执行此步骤前按照[此链接教程](https://github.com/Seeed-Projects/reComputer-Jetson-for-Beginners/tree/main/3-Basic-Tools-and-Getting-Started/3.5-Pytorch)的第 5 步安装了 Pytorch-gpu 和 Torchvision）：
...
由于通过 pip 安装 lerobot 环境时会卸载原有的 Pytorch 和 Torchvision 并安装 CPU 版本，因此需要进行检查。
...
SO101的舵机校准初始化与SO100方法和代码一致，只是需要注意SO101的Leader机械臂的前三个关节减速比与SO100不同，所以需要仔细区分并校准。

方便起见，建议在每个电机上做好标记，分别注明每个舵机是属于 Follower（用 F 表示）还是 Leader（用 L 表示），以及它们对应的编号从 1 到 6（例如 F1...F6 和 L1...L6）。从底座开始是1号，往上为2、3...，直到夹爪是6号。
占位符：一张舵机查看减速比的图片

后续我们将用F1-F6来代表Follower机械臂的1到6的关节舵机，L1-L6来代表Leader机械臂从1到6的关节舵机,对应的舵机型号关节及减速比信息如下。
...
查找机械臂对应的 USB 端口
将Leader驱动板和Follower的驱动板线缆都插到电脑或边缘计算设备上（可以通过拓展坞）


为了找到每个机械臂正确的端口，请运行实用脚本：
```bash
lerobot-find-port
```
...
这时候拔出你想要知道端口的USB线缆（例如：拔下 Foller 的控制线），按下键盘上的 Enter 键
...
```
这时候，重新插上线缆，出现这个输出意味着刚刚拔下的线缆对应的USB端口是/dev/ttyACM0

:::tip
这里的端口号是虚拟的，而不是物理意义上的USB口。端口号的分配与USB设备插入的顺序有关，例如，先插入Follower,它将可能会成为/dev/ttyACM0。/dev/ttyACM0 也可能被其他设备（例如鼠标）占据。因此，每次插拔线缆，都应重新进行端口的识别。
:::
...
故障排除：在 Linux 上，你可能需要通过运行以下命令来赋予 USB 端口访问权限，每次插拔USB线缆都应该重新授予权限：
...
```
这里的/dev/ttyACM0是上一步得到的 Follower 臂 USB 端口号，再次提醒：Linux需要对端口授予权限。您需要确定电源电压与 Follower 臂相符。
...
依照指示，将驱动板单独连接夹爪的舵机（也就是6号舵机）。请确保它现在是唯一连接到舵机驱动板的舵机【电脑---USB线---驱动板---三针线---6号舵机】，并且舵机本身没有连接到任何其他舵机。当您按下 **[Enter]** 键后，脚本将自动设置该舵机的 ID 和波特率。
...
这条消息说明电脑准备设置手腕舵机（也就是5号舵机）了，您需要拔下6号舵机，连接5号舵机。与之前的舵机一样，请确保它是唯一连接到驱动板的舵机，并且舵机本身没有连接到任何其他舵机。然后按下Enter。
...
```    
同样的，这里的/ttyACM0是上一步得到的领导臂 USB 控制线的端口号，如果不确定，您可以再运行一次端口识别步骤。
...
:::tip
`fourcc: "MJPG"`格式图像是经过压缩后的图像，你可以尝试更高分辨率，当然你可以尝试`YUYV`格式图像，但是这会导致图像的分辨率和FPS降低导致机械臂运行卡顿。目前`MJPG`格式下可支持`3`个摄像头`1920*1080`分辨率并且保持`30FPS`。
不推荐2个摄像头通过同一个USB HUB接入电脑
同时，如果摄像头接在 USB2.0 的接口，也可能会有无法读取的问题
:::
...
:::
Tips:
假设你正在执行“将红色方块抓取并放入盒子”的任务。如果在操作过程中不慎将方块弄掉，或出现任何可能导致该剧集数据质量较差的情况，可以先将机械臂操控至休息状态（即弯曲并折叠回初始位置），然后按下左箭头键，此时系统将返回到环境准备阶段，刚刚录制的动作数据会被直接舍弃。
如果本次任务完成得较快，机械臂已经成功完成操作并回到休息状态，而你不希望等待剩余时间结束，也可以按下左箭头键，以跳过剩余等待时间，直接进入下一个剧集前的环境准备阶段。在录制过程中合理使用方向键，有助于避免失败动作污染数据集，并有效提升整体录制效率。
如果你想要了解如何删除或者改动已录制数据集，请参阅
...
请注意，**bf16 需要硬件支持**，并非所有 GPU 都可以使用，一般来说，只有较新的 GPU 支持。
...
如果你希望模型“学习速度”保持和单卡相近，通常需要按 GPU 数量等比例增大学习率。
- 新的学习率 = 单卡学习率 × GPU 数量
例如：
...
```
注意：
这些调整并不是强制的规则，而是常见的经验做法。如果你不确定如何调整，也可以先：
