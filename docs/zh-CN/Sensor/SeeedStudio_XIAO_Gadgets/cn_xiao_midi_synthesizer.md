...
- **🎵1**（长按 2 秒）：音量增加
- **🎵2**（长按 2 秒）：音量减少
- **🎵3**（长按 2 秒）：切换到下一个模式（循环：声音测试 → 节拍保持 → 多轨播放 → ...）
...
此实现代表了使用 XIAO MIDI 合成器平台构建更复杂音乐应用程序的基础，无论是用于教育目的、音乐原型制作还是嵌入式音频项目。
...
:::tip
对于那些希望将 MIDI 文件转换为 Arduino 代码的用户，您可以尝试使用 [Midi2ArduinoTone](https://tawsiftorabi.github.io/Midi2ArduinoTone/) 等工具。但是，请注意此类工具生成的音符值可能需要修改才能与 XIAO MIDI 合成器配合使用，因为它们使用不同的音符映射。这些工具仍然可以作为将 MIDI 文件转换为您可以为此合成器调整的格式的起点。
