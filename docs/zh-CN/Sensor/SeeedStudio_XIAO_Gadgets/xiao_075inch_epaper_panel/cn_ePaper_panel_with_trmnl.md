...
一旦您拥有材料并购买了 TRMNL 访问权限，您就可以继续将电子纸面板连接到 TRMNL 平台。如果您遇到任何问题或有疑问，可以直接联系 TRMNL 团队：team@usetrmnl.com。
...
有时我们更新代码后需要向 TRMNL 提交 PR 并在显示之前进行审查，如果您想第一时间使用最新的固件版本，您也可以使用 Seeed 项目下的 TRMNL 仓库。
...
要将您的 XIAO 7.5" 电子纸面板与 TRMNL 配合使用并发送播放列表，您首先需要将设备连接到您的 Wi-Fi 网络。此过程称为"WiFi 配对模式"，通过简单的强制门户完成。请按照以下步骤操作：
...
3. **添加新设备**  
   - 点击 **添加新设备** 按钮，该按钮通常位于右上角或设备页面的中央。
...
5. **注册 XIAO 7.5" 设备 MAC 地址**
   ...
   - 然后您可以为您的 ePaper Panel 设置自定义名称并根据需要调整其他设置。
...
始终保持两个选项都禁用，以确保您的 XIAO ePaper Panel 稳定运行。
:::
...
TRMNL 的插件系统是让您的 ePaper Panel 真正强大和可定制的关键。插件是模块化的应用程序或小部件，您可以添加到设备中以显示各种信息和内容。
...
在插件页面的顶部，您将看到已连接到您的 TRMNL 账户的所有插件。这些插件已准备好添加到您的播放列表并在您的 ePaper Panel 上显示。示例包括：
...
特别感谢整个 **TRMNL 团队** 对这个项目的大力支持和宝贵帮助。特别是，我们要感谢 **Bogdan**、**Ryan Kulp**、**Fr3d**、**Schappi** 以及所有其他团队成员在整个开发和文档编写过程中的奉献和协助。

您的专业知识和承诺使这种集成成为可能，并大大改善了 XIAO 7.5" ePaper Panel 社区的用户体验。

## 故障排除

### 如何重新配置网络？

按下 XIAO 上的复位按钮，松开它，然后按住启动按钮五秒钟。设备将返回到初始 TRMNL 界面并激活 AP。
