...
这个指南将为您提供关于如何根据提供的BSP（板支持包）快速添加、删除和修改程序的OpenAI演示的组织信息

## 先决条件

-一个 [SenseCAP Indicator](https://www.seeedstudio.com/SenseCAP-Indicator-D1-p-5643.html)
- 在您的计算机上安装了 [ESP-IDF](https://github.com/espressif/esp-idf) 工具链

:::提示
如果您想学习如何更改用户界面（UI），您可以参考指南：[如何创建自己的UI](/SenseCAP_Indicator_How_to_Create_your_own_UI)

如果您还没有安装ESP-IDF工具链，可以按照指南中的说明操作：[如何刷新默认固件](/SenseCAP_Indicator_How_To_Flash_The_Default_Firmware)
:::
...
SenseCAP Indicator x ChatGPT x DALL·E应用是基于MVC（Model-View-Controller）架构开发的。该项目的工作流程表明了它对MVC架构的依赖。

在这个架构中，组件的结构如下：

- 视图: 视图处理各种触发事件的信号，并使用LVGL（轻巧多功能图形库）驱动显示。

:::提示
要快速构建用户界面（UI），您可以使用 [SquareLine Studio](https://squareline.io/), 我们的项目也使用了这个工具。建议使用SquareLine Studio版本1.3.0，以确保在按照本指南操作时获得流畅的体验。
:::

- M模型：模型包含 indicator_openai.c 文件，其中包含 indicator_openai_init() 函数。当在模型入口执行此函数时，该函数会向OpenAI发送请求，接收响应，并解析响应以通过视图显示在屏幕上

以下是模型的主要功能和工作流程( 一旦[API 密钥](/Sensor/SenseCAP/SenseCAP_Indicator/Set_An_API_Key) 保存后):
...
为了利用OpenAI服务，我们需要实现能够向OpenAI发送请求、接收响应并解析JSON响应的功能。以下代码片段说明了必要的代码结构：
...
```
 ...
 `chat_request(...)` 中的 `indicator_openai.c` 函数负责向 OpenAI API 发送请求、接收响应并解析 JSON 响应。它生成一个封装了用户提供数据的 HTTP 请求，并使用 `mbedtls_send_then_recv(...)`与服务器进行通信

要在您的应用程序中添加提示，可以修改 `data_buf` 函数中的  `chat_request(...)`  变量，如下所示：
...
> 原理：当请求成功时，它会返回一个URL地址。通过从这个URL链接下载并解码图像，就可以显示图像。


与 `ChatGPT Code`, 相同，在初始请求中，我们将使用提示来获取图像的URL。在获取到URL后，我们将尝试使用获取到的URL将图像下载到本地缓冲区
...
有关详细和最新的代码，请参阅[SenseCAP Indicator OpenAI](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32/tree/main/examples/indicator_openai).
...
3. **Demo SDK**: SenseCAP Indicator的演示软件开发工具包可在 [GitHub](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32).
4. **用户指南**: 用户指南提供了有关SenseCAP Indicator板的软件和硬件的详细信息。您可以在 [这里](/Sensor/SenseCAP/SenseCAP_Indicator/Get_started_with_SenseCAP_Indicator).阅读
5. **Chat completions OpenAI Guide**: 指南: 如果您是Chat API的新手，这个指南将帮助您上手。您可以在[这里](https://platform.openai.com/docs/guides/chat/chat-completions-beta)找到。
6. **`indicator_openai.c` File**: 文件: 该文件包含了 `ChatGPT` 和 `DALL·E` 集成的主要功能。您可以在[这里](https://raw.githubusercontent.com/Seeed-Solution/SenseCAP_Indicator_ESP32/main/examples/indicator_openai/main/model/indicator_openai.c)查看。
7. **ESP-IDF入门指南:**: 该指南提供了配置和使用ESP-IDF构建项目的完整步骤。您可以在[这里](https://docs.espressif.com/projects/esp-idf/en/latest/get-started/index.html).访问


## 技术支持

**在使用SenseCAP Indicator时需要帮助吗？我们将提供协助！**

如果您在按照本教程操作时遇到任何问题或有任何疑问，请随时向我们的技术支持寻求帮助。我们随时为您提供支持！

访问我们的 [ Seeed官方Discord频道 ](https://discord.com/invite/QqMgVwHT3X) 提问，或在 [GitHub讨论区](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32/discussions)享您的需求!
