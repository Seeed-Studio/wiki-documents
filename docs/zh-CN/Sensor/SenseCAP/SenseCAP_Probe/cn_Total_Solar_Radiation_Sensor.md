...
IP68 RS485 总太阳辐射传感器测量300到1100纳米范围内的实时太阳总辐射。它非常适合环境监测和农业应用。将其与SenseCAP传感器集线器4G数据记录器或SenseCAP S2100 LoRaWAN®数据记录器配对，可实现无缝数据收集和管理。

## 可升级为工业传感器

使用SenseCAP S2100[数据记录器](https://www.seeedstudio.com/SenseCAP-S2100-LoRaWAN-Data-Logger-p-5361.html?queryID=ec697c44483ad32db968bd7daaf7839d&objectID=5361&indexName=bazaar_retailer_products)，您可以轻松将此传感器转换为LoRaWAN®传感器。Seeed不仅帮助您进行原型设计，还为您提供了使用SenseCAP系列坚固[工业传感器](https://www.seeedstudio.com/catalogsearch/result/?q=sensecap&categories=SenseCAP&application=Temperature%2FHumidity~Soil~Gas~Light~Weather~Water~Automation~Positioning~Machine%20Learning~Voice%20Recognition&compatibility=SenseCAP)扩展项目的可能性。
...
**数据记录器**：这是一个LoRaWAN转换器，将MODBUS RS485/模拟/GPIO传感器转换为支持LoRa的传感器，因此通过LoRaWAN协议传输数据。

**总太阳辐射传感器**：总太阳辐射传感器利用硅光电探测器，当光照射时产生与入射光强度成正比的电流信号，可直接输出以W/㎡为单位的测量值。

**十字螺丝刀**：用于组装传感器探头。

**网关**：LoRaWAN传感器将数据传输到LoRaWAN网关，然后网关将数据传输到云服务器。如果您没有网关，有两个选择：

- 选项1：传感器在Helium网络覆盖的区域工作（在[Helium Explorer](https://explorer.helium.com/)上查看）。无论helium网络在哪里可用，您都可以使用传感器传输数据而无需购买网关。
...
总太阳辐射传感器的接线顺序，默认包含防水连接器，为了与S2100连接，您需要切断连接器：
...
**步骤 2：** 按住按钮 **3** 秒，绿色LED将以1秒频率闪烁。然后点击 **扫描** 开始扫描数据记录器的蓝牙。
...
**步骤 3：** 配置LoRaWAN和传感器参数（包括RS485 Modbus-RTU命令）。
...
**步骤 1：** 请点击以下链接并选择您的浏览器下载。[总太阳辐射传感器](https://files.seeedstudio.com/wiki/Total_Solar_Radiation_Sensor/Total%20Solar%20Radiation%20Sensor.seeed)
