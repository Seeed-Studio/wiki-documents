...
将 `watcher_service_x.x.x_arm64.deb` 替换为下载的 `.deb` 文件的实际名称。安装过程将开始。您可能需要输入系统密码来授权安装。等待安装完成。终端将显示进度和任何其他信息。
...
我们将继续在应用程序目录中添加关于在本地化部署后使用 **[HTTP Message Block](https://wiki.seeedstudio.com/cn/integrate_watcher_to_ha/#step-6-place-the-task-and-configure-the-http-message-block)** 进行消息推送的教程，敬请关注！
