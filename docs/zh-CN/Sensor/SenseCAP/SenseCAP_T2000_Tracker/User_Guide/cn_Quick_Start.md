...
为了适应不同的应用场景，SenseCAP T2000 tracker 提供了几种不同的工作模式，您可以根据需要进行选择。

| **工作模式** | **描述** | **场景** |
| - | - | - |
| 待机模式 | <ul><li>仅上传心跳包，只包含电池信息。</li><li>可以使用 LoRa 下行命令获取位置。</li></ul> | 如果您需要长时间定位设备，并且设备可以在充电前长时间运行，云平台可以发出位置请求命令来定位设备。 |
| 周期模式 | 设置一个间隔，设备定期上传位置、3 轴加速度计数据和电池信息。 | 推荐用于大多数场景<ul><li>资产跟踪</li><li>牲畜监控</li></ul> |
| 事件模式 | 基于 3 轴加速度计传感器调整上传间隔，包括运动事件、静止超时和冲击事件。每当触发事件时，设备将上传位置、3 轴加速度计数据和电池信息。 | 当您需要监控跟踪对象的运动状态时推荐使用。 |
...
设备首先上传实时位置数据。一旦平台为该上行链路返回 ACK，跟踪器开始上传本地存储的缓存数据，从最旧的条目开始，以避免覆盖较新的数据。
...
将磁铁靠近传感器区域并快速敲击 4 次以开机，然后跟踪器将自动进入蓝牙配对（如果跟踪器已开机，快速敲击 2 次以手动进入蓝牙配对），并通过 SN 选择设备。
...
Tracker 在退出蓝牙配对模式后将尝试加入 LoRaWAN 网络，尝试加入网络时绿色呼吸灯闪烁，如果网络成功加入则快速闪烁 5 次。

### 高级配置

- 打开 APP 并在 `User` 页面中点击 `Device Bluetooth Configuration`。然后选择 `SenseCAP Asset Tracker T2000` 进入 `Setup` 来配置跟踪器。
- 按照上述步骤进入蓝牙配对模式。
- 通过 S/N（S/N 在设备标签上）选择设备并选择 `Advanced Configuration`。然后，进入后将显示跟踪器的基本信息。
...
在 `Information` - `Basic` 页面中点击 `Measure`，然后您将获得传感器值：
...
<table align="center">
  <tbody>
    ...
    <td>默认平台。
必须与 SenseCAP Gateway 配合使用。SenseCAP 构建了一个专有的 TTN 服务器，使传感器在与 SenseCAP 网关配对时可以开箱即用。<br />
...
<tr><td>上行间隔 – 非事件（分钟）</td><td>这是没有事件触发时的上传间隔。</td><td><p>默认 60 分钟。</p><p>范围：1~10080 分钟。</p></td></tr>
<tr><td rowspan="2">冲击事件</td><td>当启用冲击事件时，追踪器的冲击将触发数据报告，包括冲击事件、位置和传感器数据。</td><td>默认关闭。</td></tr>
<tr><td>三轴运动阈值（mg）</td><td>默认为 300。当加速度超过 300mg 时，触发冲击事件。</td></tr>
...
<tr><td><b>Group UUID (Hex)</b></td><td>允许跟踪器仅扫描和报告 UUID 与指定模式匹配的蓝牙信标，有助于过滤掉无关的信标。</td><td><p>设置 UUID 过滤器，最多 16 字节。例如，如果设置为 '01 02 03 04'，它将过滤具有模式 '01 02 03 04 xx xx ...' 的信标</p></td></tr>
...
拆卸报警会在 APP 的 `Device` 和 `Details` 页面上显示，同时还有通知窗口，帮助用户快速检查设备状态。
