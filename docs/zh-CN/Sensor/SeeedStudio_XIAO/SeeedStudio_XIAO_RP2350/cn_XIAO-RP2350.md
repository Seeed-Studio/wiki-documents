...
由 RP2350 驱动的 XIAO RP2350 支持 MicroPython 和 Raspberry Pi 提供的 C/C++ SDK。这种灵活性允许开发者为原型制作和开发选择他们首选的编程语言和环境。
...
3. **释放 BOOT 按钮**：  
   板子连接到您的计算机后，您可以释放 BOOT 按钮。XIAO RP2350 现在应该处于 BOOTSEL 模式，您的计算机将识别它为可移动存储设备。
