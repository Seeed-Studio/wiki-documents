...
- **强大的 CPU：** ESP32-C5，32 位 RISC-V 单核处理器，工作频率高达 240 MHz
...
上传程序后，你将看到 L 指示灯以 1 秒的间隔闪烁，同时串口监视器会打印 LED ON 和 LED OFF。
...
如果你能在 XIAO 进入深度睡眠之前快速打开串口监视器，那么你可以看到如下所示的消息输出。这意味着 XIAO 现在处于**睡眠**状态。然后你也可以通过按下按钮来查看并激活它。你还可以观察 **L LED** 的开关状态来检查设备是否已被唤醒。一旦被唤醒，它将呈现闪烁效果。
...
:::tip
进入深度睡眠模式后，XIAO 的端口会消失，你需要唤醒它才能再次看到其端口号！
:::

:::caution
目前 XIAO ESP32-C5 仅支持 GPIO 唤醒，支持唤醒的引脚只有 D0~D1。此程序可能在其他引脚上无法工作。
:::

## 电池使用

XIAO ESP32-C5 能够使用 3.7V 锂电池作为电源输入。你可以参考以下图表了解接线方法。
...
1. 请使用符合规格的合格电池。
2. 使用电池时，XIAO 可以通过数据线连接到你的计算机设备，请放心，XIAO 内置电路保护芯片，是安全的。
3. 当 XIAO ESP32-C5 由电池供电时，**C LED** 会亮起。你可以以此作为判断是否进行了充电管理的依据。

### 检查电池电压

XIAO ESP32-C5 配备了 SGM40567 电池充电芯片和 TPS22916CYFPR 电池电压采集芯片。你可以启用 **BAT_VOLT_PIN_EN** 来激活电池电压采集功能，并通过 **BAT_VOLT_PIN** 读取电池电压。
...
:::tip
根据数据手册，ESP32-C5 的有效测量范围覆盖 0~3300 mV。因此，XIAO ESP32-C5 的内置电池电压采集电路设计了两个 100K 电阻进行分压，能够准确读取数值。
:::
