---
description: |
  来自 Seeed Studio 的多功能驱动板，专为控制机器人应用中的串行总线舵机而设计。它具有一个关键的跳线设置，必须根据连接方式（USB 或直接 UART）进行调整。
title: 总线舵机驱动板
...
- 另一方面，**XIAO 总线舵机适配器**包含 XIAO ESP32-C3 作为主控制器，并配有 3D 打印外壳。使用此版本，您可以直接使用板载 XIAO 控制总线舵机，使其成为机器人项目更集成和即用的解决方案。
...
总线舵机驱动板 / XIAO 总线舵机适配器是 Seeed Studio 推出的紧凑而强大的硬件解决方案，专为机器人和自动化项目中驱动串行总线舵机而设计。通过支持 UART 通信，它能够精确控制多个 ST/SC 系列舵机并获得反馈，包括飞特 SCS 系列（参见[飞特 SCS/STS/TTL 系列官方网站](https://www.feetechrc.com/en/scs_ttl_Servo.html)）。这使其非常适合机械臂、六足机器人、人形机器人和需要舵机角度和负载反馈的轮式机器人等应用。

本指南重点介绍硬件设置、物理连接、关键规格和**关键跳线设置**，以帮助用户有效地将该板集成到他们的项目中。
