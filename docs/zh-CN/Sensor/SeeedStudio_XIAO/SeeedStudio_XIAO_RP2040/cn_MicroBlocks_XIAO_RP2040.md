...
该开发板有11个 GPIO 引脚（引脚0..10）。引脚0..3可以作为模拟输入。
它还有三个小LED：红色（引脚11，用户LED）、绿色（引脚12）和蓝色（引脚13）。
...
要进入引导加载程序模式，按住B（引导）按钮的同时点击R（复位）按钮。
所有三个LED将点亮，并出现一个名为 **RPI-RP2** 的虚拟USB驱动器。

从 [vm文件夹](https://microblocks.fun/downloads/latest/vm) 下载 [固件文件](https://microblocks.fun/downloads/latest/vm/vm_xiao_rp2040.uf2)，
然后将文件拖到虚拟USB驱动器上。
