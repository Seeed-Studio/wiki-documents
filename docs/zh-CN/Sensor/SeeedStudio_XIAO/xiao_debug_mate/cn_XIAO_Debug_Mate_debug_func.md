...
欢迎使用 XIAO Debug Mate 的 DAPLink 功能官方用户手册。这份综合指南旨在帮助初学者和高级用户充分发挥 XIAO Debug Mate 在专业调试任务中的全部潜力。在这里，您将学习如何连接、配置和操作 DAPLink 调试器，了解基本的软件工具，并遵循最佳实践，确保与各种 XIAO 系列开发板的调试体验顺畅且高效。无论您是在排查复杂的固件问题还是优化开发工作流程，本文档都提供了清晰的分步说明、技巧和深入的技术解释，为您的项目从开始到完成提供全程支持。
...
**[GDB](https://www.sourceware.org/gdb/)**，GNU 项目调试器，是调试过程的核心引擎。它是一个运行在您的主机计算机（PC）上的强大命令行工具。GDB 负责控制目标微控制器上程序的执行。它允许您：
*   启动和停止您的程序。
*   设置断点以在特定代码行暂停执行。
*   逐行单步执行您的代码。
*   检查变量的值和内存寄存器的内容。

大多数现代集成开发环境（IDE），如 VS Code（配合 PlatformIO）或 Eclipse，都提供了一个图形用户界面，作为 GDB 的前端，将您的点击操作转换为 GDB 命令。

### OpenOCD/PyOCD

**[OpenOCD](https://openocd.org/)**（开放式片上调试器）和 **[PyOCD](https://pyocd.io/)** 是运行在您的主机计算机上的重要"服务器"应用程序。它们充当 GDB 的高级命令与物理调试探针（在本例中为 XIAO Debug Mate）所需的低级信号之间的桥梁或转换器。

当 GDB 发出诸如"读取变量 `x` 的值"这样的命令时，OpenOCD 接收此命令，将其转换为调试探针能够理解的指令序列，并通过 USB 将这些指令发送给探针。

### DAPLink

**[DAPLink](https://github.com/ARMmbed/DAPLink)** 是运行在 XIAO Debug Mate 的 ESP32-S3 微控制器上的开源固件。该固件将 Debug Mate 转换为功能齐全的调试探针。其主要工作是实现 CMSIS-DAP 协议，使其能够通过 USB 接收来自主机计算机的调试命令，并与目标芯片的调试引脚（SWD）进行物理交互。本质上，DAPLink 是设备上执行 OpenOCD 发送命令的"代理"。
...
**Telnet** 是一个简单的基于文本的网络协议。在调试环境中，OpenOCD 通常在特定端口（通常是端口 4444）上开启 Telnet 服务器。这提供了一个直接的、人类可读的命令行界面来访问 OpenOCD 服务器本身。虽然 GDB 用于应用程序级调试，但 Telnet 界面对于高级任务很有用，如配置 OpenOCD、检查调试探针的状态或发出低级重置命令。
...
<svg viewBox="0 0 880 480" width="100%" height="auto" style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '10px', fontFamily: 'sans-serif' }}>
  ...
  {/* Title */}
  <text x="440" y="30" textAnchor="middle" fontSize="20" fontWeight="bold" fill="#333333">嵌入式调试架构</text>

  {/* 1. HOST PC AREA (Expanded Size) */}
  <rect x="20" y="60" width="420" height="390" rx="10" fill="#f0f4f8" stroke="#cbd5e0" strokeWidth="2" />
  <text x="40" y="90" fontSize="16" fontWeight="bold" fill="#555555">主机 PC（软件）</text>

  {/* GDB Client */}
  <rect x="50" y="120" width="140" height="60" rx="5" fill="#fff3cd" stroke="#ffecb5" strokeWidth="2" />
  <text x="120" y="145" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#856404">GDB 客户端</text>
  ...
  <text x="120" y="325" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#0c5460">Telnet 客户端</text>
  <text x="120" y="345" textAnchor="middle" fontSize="10" fill="#0c5460">(人工 / 脚本)</text>
  ...
  <text x="350" y="180" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#155724">调试服务器</text>
  ...
  <text x="350" y="275" textAnchor="middle" fontSize="11" fill="#155724" fontStyle="italic">解析 GDB/Telnet</text>
  <text x="350" y="290" textAnchor="middle" fontSize="11" fill="#155724" fontStyle="italic">转换为 USB</text>
  ...
  <text x="235" y="135" textAnchor="middle" fontSize="10" fill="#666666" fontWeight="bold">TCP 端口 3333</text>
  <text x="235" y="147" textAnchor="middle" fontSize="10" fill="#666666">(RSP 协议)</text>
  ...
  <text x="235" y="345" textAnchor="middle" fontSize="10" fill="#666666" fontWeight="bold">TCP 端口 4444</text>
  <text x="235" y="357" textAnchor="middle" fontSize="10" fill="#666666">(CLI 命令)</text>
  ...
  <text x="580" y="210" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#383d41">调试探针</text>
  <text x="580" y="230" textAnchor="middle" fontSize="10" fill="#555555">(硬件适配器)</text>

  {/* Firmware Box */}
  <rect x="515" y="250" width="130" height="60" rx="3" fill="#ffffff" stroke="#383d41" strokeWidth="1" strokeDasharray="2,2" />
  <text x="580" y="275" textAnchor="middle" fontSize="11" fontWeight="bold" fill="#d63384">固件：DAPLink</text>
  <text x="580" y="295" textAnchor="middle" fontSize="9" fill="#555555">处理 USB &lt;-&gt; SWD</text>

  {/* 5. TARGET MCU */}
  <rect x="720" y="190" width="130" height="130" rx="5" fill="#ffeeba" stroke="#ffdf7e" strokeWidth="2" />
  <text x="785" y="220" textAnchor="middle" fontSize="14" fontWeight="bold" fill="#856404">目标 MCU</text>
  <rect x="735" y="240" width="100" height="60" rx="2" fill="#ffffff" stroke="#856404" strokeWidth="1" />
  <text x="785" y="265" textAnchor="middle" fontSize="11" fill="#333333">内核</text>
  ...
  <text x="510" y="380" fontSize="12" fontWeight="bold" fill="#333333">关键概念：</text>
  <circle cx="520" cy="400" r="3" fill="#155724" />
  <text x="530" y="404" fontSize="11" fill="#555555">OpenOCD/PyOCD：将 GDB 命令转换为 USB。</text>
  <circle cx="520" cy="420" r="3" fill="#007bff" />
  <text x="530" y="424" fontSize="11" fill="#555555">CMSIS-DAP：标准的 USB 协议。</text>
  <circle cx="520" cy="440" r="3" fill="#d63384" />
  <text x="530" y="444" fontSize="11" fill="#555555">DAPLink：运行在探针上的固件。</text>

</svg>

以下是从您的 IDE 到目标 XIAO 的整个调试链的总结：

1.  **您**（在您的 IDE 中）点击"Start Debugging"。
...
6.  **DAPLink** 将这些命令转换为 **SWD**（Serial Wire Debug）引脚（SWDIO 和 SWCLK）上的电信号。
7.  这些 SWD 信号直接控制目标 XIAO 内部的 **ARM Cortex 核心**，允许 OpenOCD 和 GDB 暂停 CPU、读取内存并执行所有其他调试操作。

简而言之：**GDB** 是指挥官，**OpenOCD** 是翻译官，**CMSIS-DAP** 是他们说的语言，**DAPLink** 是在前线执行命令的士兵。
...
6.  点击 `Replace Driver`。安装完成后，您的系统就准备好了。
...
:::tip PlatformIO 注意事项
如果您使用带有 PlatformIO 扩展的 VS Code，PlatformIO 将在您启动调试会话时自动下载并配置正确版本的 OpenOCD。如果您只使用 PlatformIO，则无需手动安装。
:::
...
右键单击下载的 ZIP 文件并选择"Extract All..."。选择一个稳定的位置，例如 `C:\Users\YourName\AppData\Local\xPacks\OpenOCD`。解压后，您将得到一个类似 `xpack-openocd-0.12.0-7` 的文件夹，其中包含一个 `bin` 目录。
...
转到 [xPack OpenOCD 发布页面](https://github.com/xpack-dev-tools/openocd-xpack/releases)。找到最新版本并下载适合您 Mac 的正确存档（例如，Intel 版本为 `...-darwin-x64.tar.gz`，Apple Silicon 版本为 `...-darwin-arm64.tar.gz`）。
...
您需要告诉您的 shell 在哪里找到 `openocd` 可执行文件。现代 macOS 上的默认 shell 是 Zsh。
...
转到 [xPack OpenOCD 发布页面](https://github.com/xpack-dev-tools/openocd-xpack/releases)。找到最新版本并下载 Linux 存档（`...-linux-x64.tar.gz`）。
...
:::info
如果您打算为 XIAO RA4M1 使用固件烧录功能，您可能需要参考这里的步骤来额外下载和使用 PyOCD。
:::
...
1.  **先决条件**
    确保您的系统上已安装 **Python**。如果没有，请从官方 [python.org](python.org) 网站下载，并确保在安装过程中勾选 **"Add Python to PATH"** 选项。
...
3.  **安装 XIAO RA4M1 支持包**
    XIAO RA4M1 使用 `R7FA4M1AB` 芯片。您需要安装特定的支持包，以便 PyOCD 能够识别它。
...
1.  **先决条件**
    确保通过包管理器安装了 `python3` 和 `pip`（例如，在 Ubuntu 上运行 `sudo apt install python3-pip`）。
...
在您开始使用 DAPLink 调试器功能之前，您需要单独下载 ZIP 文件。该文件包含调试各种 XIAO 开发板所需的所有文件。
...
*   **`XIAO_MG24_..._OpenOCD-v0.12.0/`**
    这是 **XIAO MG24** 开发板的特殊且非常重要的组件。目前，官方 OpenOCD 发行版或标准 xPack 发行版中都不支持此芯片。为了解决这个问题，我们为 Windows（`XIAO_MG24_Win_...`）和 macOS/Linux（`XIAO_MG24_Mac_Linux_...`）提供了完整的独立 OpenOCD 包。当您使用 XIAO MG24 时，您**必须**使用这些特定文件夹中提供的 `openocd` 可执行文件。
...
1.  拿起您的 XIAO 开发板，将其与 Debug Mate 上的母头对齐。
...
2.  在目标开发板上找到 SWD 调试引脚。这些通常标记为 `SWDIO`、`SWCLK`、`GND`，有时还有 `VTref` 或 `VCC`。
3.  按以下方式将线从 XIAO Debug Mate 的扩展接头连接到目标开发板：
...
虽然现代 IDE 提供了图形化的调试界面，但有时您需要一种更直接、低级别的方式与调试器交互。这就是 Telnet 的用武之地。OpenOCD 运行一个 Telnet 服务器，允许您直接连接并发送基于文本的命令来控制目标芯片。这对于快速检查、编写自动化任务脚本或在完整 IDE 不可用或不必要时调试问题非常有用。

### 启用 Telnet 客户端

在使用 Telnet 之前，您可能需要在操作系统上启用它，因为它并不总是默认安装或启用。
...
4.  在出现的新窗口中，向下滚动并找到 **Telnet Client**。
5.  勾选旁边的复选框并点击 **OK**。Windows 将安装该功能。
...
<TabItem value="MacOS" label="MacOS">
在最新版本的 macOS 中，原生 Telnet 客户端可能已被移除。最简单的安装方法是使用 [Homebrew]。打开您的终端并运行：
```bash
brew install telnet
```
</TabItem>
...
在下面的列表中找到您的 XIAO 开发板，并按照您操作系统的具体说明启动 OpenOCD 服务器。此服务器进程将等待来自调试器（如 GDB）的连接。

##### 适用于 Seeed Studio XIAO SAMD21

标准的 OpenOCD 包不包含 XIAO SAMD21 的配置文件。您必须手动将其从我们提供的包中复制到系统的 OpenOCD 安装目录中。
...
打开您的命令行工具并导航到解压包中适合您操作系统的正确文件夹。
...
*   `-f target/rp2040.cfg`：第二个 `-f` 标志加载目标微控制器（XIAO 开发板上的 RP2040 芯片）的配置。
...
本指南将引导您使用这些强大的工具来上传预编译的固件文件。我们使用"OpenOCD/PyOCD"这一称谓是因为大多数兼容的 XIAO 开发板使用 OpenOCD，而 **XIAO RA4M1** 特别需要使用 PyOCD 进行烧录。
...
1.  **OpenOCD 安装：** 对于所有 XIAO 开发板（**除了 XIAO MG24**），您需要[在系统上安装 OpenOCD 并确保可从命令行访问（即添加到系统的 PATH 中）](#install-openocd)。
...
*   **.hex（Intel HEX）：** 二进制代码的 ASCII 文本表示。它包含内存地址、校验和和元数据，使其比 `.bin` 格式更加稳健。
*   **.elf（可执行和可链接格式）：** 最全面的格式。它包含机器代码、内存地址信息，还包含调试信息，如函数名、变量名和源代码行号。这是调试的首选格式。
*   **.bootloader（引导程序文件）：** 引导程序文件是一个专门的固件映像，驻留在微控制器内存的受保护区域中，负责促进固件烧录、设备恢复和启动任务。在 XIAO 开发板上，引导程序允许您将新的应用程序代码上传到设备（通常通过 USB），而无需专用的硬件编程器。如果设备变得无响应或主程序失败，重新烧录引导程序可以帮助恢复开发板。仅使用适用于您特定开发板型号的官方引导程序文件，因为使用错误的文件可能会使设备无法操作，直到使用特殊工具进行修复。
...
因此，我们强烈推荐使用 `PyOCD`，它为 RA4M1 提供开箱即用的支持。
...
:::danger 不要让设备变砖
XIAO RA4M1 禁止烧录 `.elf` 文件，这样做可能会让设备变砖！！！
:::
...
:::danger 不要让设备变砖
XIAO MG24 禁止烧录 `.elf` 文件，这样做可能会让设备变砖！！！
:::
...
此开发板需要使用包中的专用 OpenOCD 版本。在运行命令之前，您**必须**导航到正确的目录。
...
2.  **运行烧录命令：**
    固件的路径现在是 `../examples/Blink_MG24.hex`，因为您在包目录内的两级深度。
...
:::tip
使用 OpenOCD 烧录引导加载程序后再烧录固件 bin，固件 bin 无法正常运行。您需要使用 Arduino 上传程序才能使程序正常运行。

这是因为 Nordic 系列芯片（如 nRF52832）的引导加载程序在启动时需要检查特定内存地址（通常称为"引导加载程序设置页"）的应用程序固件有效性信息。此"设置页"包含固件的 CRC 校验和、长度和有效性标志。

当您使用 OpenOCD 等通用编程工具直接烧录 .bin 文件时，它们只会将固件的二进制数据写入指定的应用程序起始地址，而不会自动生成或写入 Bootloader 所需的"设置页面"。因此，当设备重启时，Bootloader 无法检测到有效的设置页面，并判断没有合法的应用程序，从而拒绝启动固件，通常会进入 DFU（设备固件更新）模式，等待通过 OTA 或串口进行更新。
:::
...
要烧录 Bootloader，请使用与您的操作系统对应的命令。
...
3.  **配置 `platformio.ini`**：这是最关键的步骤。要启用 XIAO Debug Mate 的调试功能并正确识别开发板，您必须修改位于项目根目录的 `platformio.ini` 文件。
    ...
    **添加 Seeed Studio 开发板支持：**
    ...
    配置文件告诉 PlatformIO 从这个自定义仓库下载开发板定义，并指定用于上传和调试的工具。XIAO Debug Mate 作为标准的 **CMSIS-DAP** 调试器运行。
...
对于 nRF52840，PlatformIO 提供两个开发板支持包。选择适合您项目需求的那个。
...
3.  **调试控制**：PlatformIO 将编译您的代码，通过 Debug Mate 上传到 XIAO，并在 `main()` 或 `setup()` 的第一行暂停执行。编辑器顶部将出现调试工具栏，允许您控制程序流程：
    *   **继续（F5）**：恢复执行直到遇到下一个断点或程序结束。
    *   **单步跳过（F10）**：执行当前行。如果该行包含函数调用，将执行整个函数并停在下一行。
    *   **单步进入（F11）**：执行当前行。如果该行包含函数调用，调试器将进入函数并停在其第一行。
    *   **单步跳出（Shift+F11）**：如果在函数内暂停，这将执行函数的剩余部分并返回到调用它的行。
    *   **重启（Ctrl+Shift+F5）**：终止并重启当前调试会话。
    *   **停止（Shift+F5）**：终止调试会话。
...
1.  **设置断点**：在代码编辑器中，将光标移动到您希望暂停的行号左侧。将出现一个淡红色圆点。点击它设置断点（圆点将变为亮红色）。
2.  **运行到断点**：开始调试会话（`F5`）。程序将全速运行，并在到达带有断点的行时自动暂停。
3.  **管理断点**：在 "Run and Debug" 侧面板中，"BREAKPOINTS" 部分列出所有活动断点。您可以从这里启用、禁用或删除它们。
...
2.  **监视变量**：如果您想监视特定变量或表达式，转到 "WATCH" 部分，点击 `+` 图标，输入变量名（例如 `myVariable` 或 `&myVariable` 查看其内存地址）。
...
3.  **修改变量（写入）**：在 "VARIABLES" 或 "WATCH" 部分，您通常可以双击变量的值来输入新值。按 Enter 应用更改。这对于测试不同条件而无需重启程序非常有用。
...
您可以尝试重新烧录 bootloader 来恢复设备。对于 XIAO RA4M1 型号，请参考**[此方法](#适用于-seeed-studio-xiao-ra4m1-2)**进行 bootloader 恢复。

恢复 Bootloader 时，如果出现错误，请尝试将 XIAO RA4M1 连接到 XIAO Debug Mate，然后连接到您的计算机。按住 XIAO RA4M1 上的 B 按钮，同时点击一次 R 按钮，然后释放 R 和 B 按钮。继续重新烧录 Bootloader。

烧录 Bootloader 后，尝试使用 Arduino 等软件重新更新固件。这通常会恢复功能。如果不成功，设备可能已永久损坏且无法修复。任何涉及使用 XIAO Debug Mate 烧录固件的操作都可能需要您承担全部责任。


### Q2: 如何判断设备是否已损坏？

判断 XIAO 设备是否损坏的最直接方法是连接到计算机时不显示串口连接。如果在按住 B 按钮后再插入计算机，设备仍然不显示串口连接，则设备已损坏。


### Q3: 损坏的设备可以修复吗？我可以联系技术支持申请更换吗？

XIAO 设备的损坏通常是由分区或 Bootloader 文件问题引起的。这通常可以通过重新刷写 Bootloader 文件来解决。如果这样做失败了，目前没有其他解决方案。用户必须对自己执行此操作承担全部责任，因此请确保刷写命令和固件经过验证且有效。
