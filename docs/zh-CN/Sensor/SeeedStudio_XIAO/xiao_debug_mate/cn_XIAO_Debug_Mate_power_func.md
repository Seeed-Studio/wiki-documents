...
XIAO Debug Mate 的功率计功能将您的设备转变为一个多功能、高精度的工具，用于测量电压、电流和功耗。通过直观的图形界面和实时数据更新，您可以轻松监控 XIAO 开发板或其他连接硬件在开发各个阶段的功耗。无论您是在优化超低功耗操作还是验证负载下的系统稳定性，功率计都能为您提供清晰、准确的洞察。本节将引导您了解其关键功能以及如何充分利用此功能。
...
XIAO Debug Mate 本身应由标准的 **5V/1A USB 电源**供电。使用功率不足、功率过大或不合规的充电器可能导致操作不稳定或有损坏设备的风险。
...
**导航：**
*   **切换 UI：** 只需**转动滚轮**即可在三个功率计屏幕之间循环。没有光标；整个页面都会改变。
*   **重置数据：** UI 3 上的统计数据只有在您**长按按钮**返回主菜单时才会清除。
...
*   **目的和用例：**
    此 UI 是您进行**实时调试和基本合理性检查**的首选。其高精度允许您看到功耗的微小波动。使用此屏幕快速验证：
    *   设备在空闲时是否消耗预期的电流量。
...
*   **目的和用例：**
    在开发电池供电或节能设备时，电流可能降至微安范围。此 UI **消除了手动单位转换的需要**，让您能够即时了解设备功耗的大小。它非常适合：
...
*   **操作：**
    当您首次进入此屏幕时，计时器和数据累积**自动开始**。切换到 UI 1 或 UI 2 **不会**重置数据，允许您在长期测试运行时检查实时值。数据只有在退出到主菜单时才会清除。

*   **目的和用例：**
    此 UI 对于**部署前验证**至关重要。在选择电池之前，您可以模拟设备的工作负载并使用此屏幕：
...
因此，Debug Mate 测量的功耗将**略高于**从 3.3V 电池实际消耗的功率。您测量的值是电池寿命计算的优秀且安全的**上限估算**。
...
1.  只需将您的 XIAO 开发板直接插入 XIAO Debug Mate 前面的母头。
...
1.  将 Seeeduino V4.2 的 **5V 引脚**连接到 Debug Mate 接头上的 **5V 引脚**。
2.  将 Seeeduino V4.2 的 **GND 引脚**连接到 Debug Mate 接头上的 **GND 引脚**。
