...
<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="/cn/getting_started_with_xiao_debug_mate" target="_blank">
...
<div class="table-center">
	<table align="center">
		...
		<tr>
			<td>LED</td>
			<td>36-LED 矩阵状态指示（默认：串口波特率）</td>
		</tr>
		<tr>
			<td>用户输入</td>
			<td>1 x 用户按钮/启动按钮 <br /> 1 x 滚轮 <br /> 1 x 凹陷式复位按钮</td>
		</tr>
		<tr>
			<td>I/O 接口</td>
...
* ④ **外部扩展排针**：此排针引出 XIAO 主板左右两侧的引脚。它采用双接口设计，顶部为母头（用于杜邦线），底部为公针。这种多功能设计允许整个设备用作另一个系统的子板。

* ⑤ **内部扩展排针**：此排针专门设计用于引出 XIAO 主板背面引脚的信号，这些信号通过弹簧针（③）连接。与排针④一样，它的顶部为母头，底部为公针，方便访问 XIAO 的背面 I/O。
...
* ⑩ **USB-C 端口**：位于侧面的 USB-C 端口，用于为设备供电并促进数据通信，如编程 XIAO 板或 UART 透传。

## 入门指南

### 为 XIAO Debug Mate 安装引脚贴纸

在包装内，您会找到四张引脚图贴纸。这些贴纸设计用于贴在 Debug Mate 的扩展排针旁边，为您提供每个引脚功能的快速便捷参考。
...
这是默认屏幕，用作中央仪表板。它提供所有关键功能的实时概览：
*   **调试状态：** 指示 SWD `DEBUG` 功能是否激活。
*   **UART 状态：** 显示当前波特率和 RX/TX 线路上的活动。
*   **功耗指标：** 显示连接的 XIAO 消耗的瞬时电压（`U`）、电流（`I`）和功率（`P`）。

从此页面，您可以访问其他两个主要功能。只需滚动滚轮以高亮显示 UART 模块（左下）或功率模块（右下），然后单击按钮进入其专用页面。
...
此页面提供高精度功耗分析工具，具有三种不同的 UI 视图，您可以通过滚动滚轮来循环切换。
...
*   **视图 2：低功耗详细视图**
    针对低功耗应用优化，此视图专注于电流和功率。它同时以多种单位显示数值（例如，`18.234 mA` 和 `18234 uA`），为您节省跟踪微安级消耗时手动单位转换的工作。
...
<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/xiao_debug_mate/12.jpg" style={{width:600, height:'auto'}}/></div>
    ...
    :::tip
    第一次进入视图 3 时，功耗计时器将开始计时。当您长按确认按钮退出功耗分析页面时，计时器将重置为零。
    :::

:::tip **功耗测量重要说明：**
功耗分析功能测量通过 XIAO 的 **5V 引脚** 提供给 XIAO 的功率。此测量包括 XIAO 板载电源管理 IC（PMIC）和其他 5V 轨道组件消耗的功率。如果您计划在最终项目中使用直接连接到 XIAO 的 3.3V/BAT 焊盘的 LiPo 电池供电，实际功耗将 **低于** Debug Mate 报告的数值。请将此数据作为有价值的参考，在最终电池计算时请记住这一差异。
:::

### XIAO Debug Mate：功能和使用

有关如何在特定 IDE 和项目中使用 DAPLink 调试器、UART 监视器和功耗分析器的详细指南，请参阅我们的专用功能指南。
...
- [XIAO Debug Mate 功耗计](https://wiki.seeedstudio.com/cn/xiao_debug_mate_power)

## 重置

如果您需要重启 XIAO Debug Mate，您有两个选择：
1.  拔掉并重新插入 USB-C 电源线。
2.  使用隐藏的 **Reset 按钮**。它位于 USB-C 端口正下方的小孔中。您可以使用 SIM 卡取卡器或回形针轻轻按压。
...
设备上的主按钮具有双重功能。在正常操作期间，它充当 **Confirm/Back** 按钮。但是，它也充当进入 ESP32-S3 闪存模式的 **BOOT** 按钮。
...
如果您需要恢复原始出厂固件，可以使用以下方法之一。首先，按照上述部分中的说明将设备置于 **Boot 模式**。
...
2.  解压 zip 文件的内容。
3.  将您的 XIAO Debug Mate 置于 Boot 模式。
4.  运行适合您操作系统的脚本。
...
如果您已安装 `esptool` 作为 ESP-IDF 的一部分或作为 Python 包，您可以手动闪存固件。
...
2.  将您的 XIAO Debug Mate 置于 Boot 模式。
...
4. 按照上面启动部分中的说明将您的 XIAO Debug Mate 置于 **Boot 模式**。
...
:::tip
确保您已安装 PlatformIO。您可以将其安装为 VS Code 扩展或通过 pip 安装为独立的 CLI 工具：`pip install platformio`
:::
...
1.  **与较旧 XIAO 板的兼容性：**
    ...
    如果您遇到不稳定的调试连接，请尝试 **稍微调整** XIAO 板在插座中的位置。您可能不需要将其完全按下。轻微的移动通常可以确保弹簧针与 XIAO 背面的 SWD 焊盘建立牢固接触。

2.  **排针的磨损：**
    母头连接器，像任何机械连接器一样，都有有限的使用寿命。为了最大化其耐用性并确保可靠连接，请遵循以下最佳实践：
    *   插入或拔出 XIAO 开发板时，请**垂直向上或向下**施力。
    *   **避免来回摆动或摇晃** XIAO 开发板，因为这可能会弯曲引脚，扩大母头连接器中的触点，并随着时间推移导致连接不良。
