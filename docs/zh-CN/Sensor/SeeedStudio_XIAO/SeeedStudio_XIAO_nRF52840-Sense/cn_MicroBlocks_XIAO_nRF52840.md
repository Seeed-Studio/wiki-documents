...
这块开发板的一个重要优势是其低功耗的 Nordic 无线电，
它允许 MicroBlocks 通过蓝牙低功耗（BLE）无线编程开发板。

引脚 0..10 是 GPIO 引脚。引脚 0..5 可以作为模拟输入。
这块开发板还有三个小 LED：红色（引脚 11，用户 LED）、蓝色（引脚 12）和绿色（引脚 13）。
所有 LED 都是反向的：将相关引脚设置为 LOW 来点亮 LED。

Sense 版本还包括一个板载 PDM 麦克风、温度传感器和 6 轴 LSM6DS3TR-C IMU，
这些都由 MicroBlocks 的"Microphone"、"Basic Sensors"和"Motion"库支持。
...
下载[固件文件](https://microblocks.fun/downloads/latest/vm/vm_xiao_nrf52840.uf2)，
然后将文件拖到虚拟 USB 驱动器上。
...
如果您的 Xiao nRF52840 有自己的电源，您也可以在不使用 USB 线的情况下连接它，
方法是在连接菜单中选择"连接（BLE）"。
