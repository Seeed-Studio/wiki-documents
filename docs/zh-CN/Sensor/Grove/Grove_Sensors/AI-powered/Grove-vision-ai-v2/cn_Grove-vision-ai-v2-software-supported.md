...
在本节中，我们将引导您使用 SenseCraft AI 模型助手快速部署和观察结果，无需代码，也无需其他开发板：
...
- **Confidence：** Confidence 指的是模型对其预测分配的确定性或概率水平。
...
- **IoU（交并比）：** 这衡量两个框之间的重叠程度。想象您在图片中的物体周围画了一个框，模型也画了一个框。IoU 告诉您这些框的匹配程度。IoU 越高意味着框越接近。
:::
...
- `bool begin(TwoWire *wire = &Wire, uint16_t address = I2C_ADDRESS, uint32_t wait_delay = 2, uint32_t clock = 400000)` —— 初始化 Grove Vision AI V2。
  ...
  **输入参数：**
    ...
    - `uint16_t address` —— 这是 I2C 设备的地址，用于识别连接到 I2C 总线的特定设备。
