...
然后打开一个 cmd 终端并输入以下命令来查询您的计算机 IP

```bash
ipconfig
```

然后将查询到的 IP 输入浏览器，然后添加 8086 端口进入 InfluxDB 的主界面。
...
然后填写 Token，这是我们在开始时谈到的 InfluxDB 网页中的 API Tokens。
...
导入后，您应该能够看到以下效果（如果您没有看到以下界面，请移至文章末尾的常见问题解答部分，那里会有解决方案）
...
然后您应该看到系统列出了一些图片文件名，这些就是刚才 capture 节点拍摄的照片。
...
如果在 Grafana 中发现实时热力图或 reCamera 实时画面无法加载或显示错误，那么您需要点击有问题的面板进行刷新，以热力图面板为例：
