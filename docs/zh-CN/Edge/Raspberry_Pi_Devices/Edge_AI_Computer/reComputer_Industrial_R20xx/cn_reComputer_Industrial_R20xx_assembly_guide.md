---
description: reComputer Industrial R20xx 组装指南
title: reComputer Industrial R20xx 组装指南
keywords:
- Raspberry pi
- Edge Controller
...
slug: /cn/recomputer_industrial_r20xx_assembly_guide
last_update:
  date: 09/28/2025
  author: Nolan Chen
---
...
reComputer Industrial R20xx 支持多种物联网无线通信，包括 4G、5G、LoRa®、Wi-Fi/BLE 和 Zigbee。它支持通过 M.2 插槽扩展 SSD 存储或通过 NPU 加速器实现 AI 功能。如果您选择在购买时直接包含 AI 加速器，您将获得额外的 26TOPS 计算能力。此外，我们的硬件工程服务包括标志定制、包装品牌、标签、固件刷写和镜像服务，提供针对您特定需求的全面支持。
...
reComputer Industrial R20xx 提供多种安装方法。包装中包含 DIN 导轨夹和安装螺丝。按照图示正确将 DIN 导轨夹连接到设备侧面的安装孔上。螺丝牢固拧紧后，您就可以将设备安装到安装导轨上。

#### 安装步骤

- **步骤 1：** 将设备和导轨夹放在标准型材导轨的上边缘所示位置，然后向下推设备。
...
垂直安装方法也适用于 reComputer Industrial R20xx，但是安装支架不包含在包装盒中，需要额外购买。
