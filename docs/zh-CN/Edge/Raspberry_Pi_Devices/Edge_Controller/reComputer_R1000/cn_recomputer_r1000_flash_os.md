...
reComputer R1000 边缘物联网控制器基于高性能的 Raspberry Pi CM4 平台构建，配备四核 A72 处理器，最大支持 8GB RAM 和 32GB eMMC。配备可灵活配置的双以太网接口，还包括 3 个隔离的 RS485 通道，支持 BACnet、Modbus RTU、Modbus TCP/IP 和 KNX 协议。
...
- **步骤 3.** 请将电源线从电源适配器连接到 reComputer R1000 的电源端口。
...
现在请将以太网电缆连接到 reComputer R1000 和与您的主机计算机在同一网络上的路由器。
...
reComputer R1000 开箱即用，预装了必要的驱动程序，因此您无需安装任何额外的驱动程序。但是，如果您自己刷写新的操作系统，则需要单独安装必要的驱动程序。
...
此过程将确保在更新固件之前您的驱动程序是最新的。

对于 reComputer R1000 V1.0，需要在重启后更改 `/boot/firmware/config.txt` 文件并输入以下命令：
