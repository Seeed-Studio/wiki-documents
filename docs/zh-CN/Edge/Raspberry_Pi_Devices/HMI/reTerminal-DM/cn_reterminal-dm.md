...
其中对应 `DO1` 的 `GPIO24` 被设置为 `Output` 和 `pulling down` 状态，`GPIO24` 电平设置为 `High`
...
在您的 Linux 主机或另一台 reTerminal DM 上，您可以编译并运行接收数据的代码。
...
在 reTerminal DM 的复位孔中有一个微型按钮开关。通过用细物体按下此按钮，可以复位 CM4。此按钮连接到 CM4 的 RUN_PG 引脚。此引脚为高电平时表示 CM4 已启动。将此引脚拉低会复位模块。
...
经典的树莓派 40 引脚 GPIO 设计在 reTerminal DM 内部得以保留，引脚定义与树莓派 4B 保持一致。用户需要打开后盖才能使用这些 GPIO，需要注意的是，由于 CM4 IO 资源有限，40 引脚 GPIO 和许多外设接口是复用的，因此在使用 GPIO 时需要特别注意冲突。有关详细的引脚分配信息，请参考下表。
...
:::note
请注意，reTerminal DM 的标准版本在前面板上没有摄像头开孔，因此标准产品不提供摄像头功能。如果您有定制摄像头需求，请联系 odm@seeed.cc
:::
...
**步骤 1.** 请参考 [EC25 4G 模块硬件组装](/cn/reterminal-dm-hardware-guide/#assembly-4g-module-and-antenna) 指南，将 `EC25 4G 模块` 安装到 `4G/LTE PCIe 插槽` 中，您应该看到 *`4G/LTE`* 丝印，并在系统上电前将支持 4G 的 SIM 卡插入 [SIM 卡插槽](/cn/reterminal-dm/#sim-card-slot)。
