...
- **跨平台**：N3uron 可以在大多数版本的 Windows 和 Linux 发行版上运行，也可以在 ARM 架构（如 Raspberry Pi）上运行。

- **无限许可证**：N3uron 采用无限许可证模式，在标签、用户、设备和连接方面提供无限制使用，价格实惠。
...
- **可扩展和多功能架构**：从一开始，N3uron 就被设计为无缝部署具有数百或数千个节点的分布式架构。用户可以使用 N3uron Links 快速连接多个节点，轻松扩展架构以满足需求。
...
N3uron 可以通过我们的一步式自动安装脚本轻松安装，只需在您的 reComputer R1000 终端中运行以下命令：
...
让我们看一个分步示例，了解 N3uron 如何通过 [OPC UA 客户端](https://docs.n3uron.com/docs/opc-ua-client)简化数据收集，以及如何使用 [MQTT 客户端模块](https://docs.n3uron.com/docs/mqtt)将这些数据无缝发布到 MQTT 代理，从而通过 MQTT 实现连续数据推送。

### 配置 OPC UA 客户端

- **步骤 1**：请在本地网络上的不同机器上打开您喜欢的浏览器来访问 N3uron WebUI，使用 reComputer R1000 的 IP 地址和为 WebUI 配置的端口（默认为 HTTP 的 8003 端口或 HTTPS 的 8443 端口）。
...
- **步骤 3**：选择 **OpcUaClient** 作为模块类型，然后点击下方的 **Save** 按钮来实例化该模块。
...
- **步骤 5**：导航到模块内的 OPC Browser 部分，选择之前创建的 DataSim 客户端。
...
- **步骤 2**：选择 MqttClient 作为模块类型，然后点击 Save。
...
- **步骤 5**：创建一个新的发布者并设置目标主题，例如 "/n3/recomputer"。
...
**步骤 7：** 在模板部分，创建一个名为 Main 的新容器。然后，将此容器指定为启动容器。这将设置 Main 为我们的初始 WebVision 屏幕。它将在旁边显示星号符号 (*)。
...
关于 WebVision 还有很多内容可以探索，以增强您的 reComputer R1000 体验。更多详细信息，请参考我们的 [WebVision 知识库](https://docs.n3uron.com/docs/web-vision-configuration)。

您所看到的只是开始。拥抱其令人印象深刻的近 50 个模块所提供的无限可能性。敢于梦想远大，创新，并通过 N3uron 的尖端功能将您的项目提升到新的高度。要深入了解这个强大的平台，请访问 [https://n3uron.com/](https://n3uron.com/) 并踏上激动人心的发现之旅！
