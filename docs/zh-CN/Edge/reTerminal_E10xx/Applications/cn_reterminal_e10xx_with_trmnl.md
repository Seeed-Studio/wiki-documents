...
一旦您拥有材料并购买了 TRMNL 访问权限，就可以继续将设备连接到 TRMNL 平台。如果遇到任何问题或有疑问，可以直接联系 TRMNL 团队：team@usetrmnl.com。
...
- 设备应自动重定向您到 TRMNL 配置页面。如果没有，请在浏览器中手动访问 [http://4.3.2.1](http://4.3.2.1)。
...
在您的 reTerminal E 系列 ePaper 显示屏连接到 Wi-Fi 并记录其 MAC 地址（设备 ID）后，您可以将其添加到您的 TRMNL 账户：

1. **打开 TRMNL Web 界面**  
   - 在您的计算机或移动设备上，打开浏览器并访问 [TRMNL web 界面](https://trmnl.app)。

2. **转到设备页面**  
   - 在 TRMNL 界面中，导航到 **设备** 部分。

3. **添加新设备**  
   - 点击 **添加新设备** 按钮，通常位于设备页面的右上角或中央。
...
5. **注册 reTerminal E 系列 ePaper 显示屏 MAC 地址**

   为了在 reTerminal E 系列 ePaper 显示屏和 TRMNL 之间建立连接，我们需要在 TRMNL 设置页面中绑定正确的设备 MAC 地址。
...
您的 reTerminal E 系列 ePaper 显示屏现在已链接到您的 TRMNL 账户！您现在可以直接从 TRMNL 界面向您的显示屏发送播放列表、图像或其他内容。

> 💡 **提示：** 如果您有多个 reTerminal E 系列，请使用它们各自的设备 ID 为每个设备重复上述步骤。

:::caution
在 TRMNL web 界面中添加设备后，请转到设备的设置页面并 **关闭** **固件早期发布** 和 **启用 OTA 更新** 两个选项。  

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/xiao_075inch_epaper_panel/213.png" style={{width:700, height:'auto'}}/></div>

如果这些选项保持启用状态，您的设备在连接到互联网时可能会自动下载并安装来自 TRMNL 的固件更新。这些更新是为官方 TRMNL 硬件设计的，**与 reTerminal E 系列不兼容**。安装不兼容的固件可能导致您的设备故障或无响应。

始终保持两个选项都禁用，以确保您的 reTerminal E 系列稳定运行。
:::
...
TRMNL 的插件系统是让您的 reTerminal E 系列 ePaper 显示屏真正强大和可定制的关键。插件是模块化的应用程序或小部件，您可以添加到设备中以显示各种信息和内容。
...
您的专业知识和承诺使这种集成成为可能，并大大改善了 reTerminal E 系列 ePaper 显示屏社区的用户体验。
