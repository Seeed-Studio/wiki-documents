...
Seeed Studio XIAO MG24 是一款强大的开发板，支持蓝牙 LE 5.3 和蓝牙网格，使其成为需要无线连接的各种 IoT 应用的理想选择。凭借其卓越的射频性能，XIAO MG24 可在各种距离上提供可靠的高速无线通信，使其成为短距离和长距离应用的多功能解决方案。在本教程中，我们将探索 XIAO MG24 蓝牙功能的基本特性，包括如何扫描附近的蓝牙设备、建立蓝牙连接以及通过该连接传输和接收数据。
...
此示例演示了如何使用 Silicon Labs BLE 栈扫描附近的蓝牙低功耗 (BLE) 设备，打印每个发现设备的地址、RSSI（接收信号强度指示器）、通道和名称。

代码首先定义了一个事件处理函数 `sl_bt_on_event`，它处理 BLE 栈生成的各种蓝牙低功耗 (BLE) 事件。该函数使用 switch 语句来区分事件类型，例如 BLE 设备启动时和从附近设备接收广播报告时。在接收到启动事件时，它初始化串行通信，配置天线控制的 GPIO 引脚，并开始使用指定参数扫描 BLE 设备。
...
为其中一个 XIAO 上传程序后，如果程序运行顺利，那么您可以拿出手机并使用 nRF Connect APP 搜索名为 **XIAOMG24_BLE** 的蓝牙设备，连接它，并点击下图所示的按钮，您将收到温度数据信息。
...
/*
   ...
   启动时，该程序将开始扫描运行 'ble_health_thermometer' 示例并以 "Thermometer Example" 名称广播的另一个开发板。
   一旦找到另一个开发板，它会建立连接，发现其服务和特征，然后订阅温度测量。
   订阅后，该示例开始定期从另一个开发板接收温度数据，并将其打印到串口。
