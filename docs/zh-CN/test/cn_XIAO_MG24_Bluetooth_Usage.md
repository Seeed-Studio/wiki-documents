...
title: Seeed Studio XIAO MG24 蓝牙使用 keywords:
- MG24
- xiao
- ble
- bluetooth
...
Seeed Studio XIAO MG24 是一款强大的开发板，支持 Bluetooth LE 5.3 和 Bluetooth mesh，使其成为需要无线连接的各种 IoT 应用的理想选择。凭借其卓越的射频性能，XIAO MG24 可在各种距离上提供可靠的高速无线通信，使其成为短距离和长距离应用的多功能解决方案。在本教程中，我们将探索 XIAO MG24 蓝牙功能的基本特性，包括如何扫描附近的蓝牙设备、建立蓝牙连接以及通过该连接传输和接收数据。
...
void setup() {
  // 开启天线功能 pinMode(RF_SW_PW_PIN, OUTPUT);  
  digitalWrite(RF_SW_PW_PIN, HIGH);

  delay(100);

  // HIGH -> 使用外置天线 / LOW -> 使用内置天线 pinMode(RF_SW_PIN, OUTPUT);  
  digitalWrite(RF_SW_PIN, HIGH);
```

## 低功耗蓝牙 (BLE) 使用

低功耗蓝牙，简称 BLE，是蓝牙的节能变体。BLE 的主要应用是小数据量的短距离传输（低带宽）。与始终开启的蓝牙不同，BLE 除了在启动连接时，其余时间都保持在睡眠模式。
...
在低功耗蓝牙中，有两种类型的设备：服务器和客户端。XIAO MG24 可以充当客户端或服务器。
...
/*
   ...
   该示例扫描其他 BLE 设备并打印出每个找到设备的地址、RSSI、通道和名称。

   在以下网址了解更多关于 Silabs BLE API 使用的信息：https://docs.silabs.com/bluetooth/latest/bluetooth-stack-api/
...
void sl_bt_on_event(sl_bt_msg_t* evt) {
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    // 当 BLE 设备成功启动时接收此事件 case sl_bt_evt_system_boot_id:
      ...
      // 开始扫描其他 BLE 设备 sc = sl_bt_scanner_set_parameters(sl_bt_scanner_scan_mode_active,  // 模式 16,                              // 间隔 (值 * 0.625 ms)
                                        16);                             // 窗口 (值 * 0.625 ms)
 ...
 * 在 BLE 广播中查找完整的本地名称
 *
 * @param[in] response 从扫描接收的 BLE 响应事件
 *
 * @return 如果找到则返回完整的本地名称，否则返回 "N/A"
 *****************************************************************************/
static String get_complete_local_name_from_ble_advertisement(sl_bt_evt_scanner_legacy_advertisement_report_t* response) {
  int i = 0;
  // 遍历响应数据 while (i < (response->data.len - 1)) {
    ...
    // 如果长度超过最大可能的设备名称长度 if (advertisement_length > 29) {
      continue;
    }

    // 类型 0x09 = 完整本地名称，0x08 缩短名称
    // 如果字段类型匹配完整本地名称 if (advertisement_type == 0x09) {
      // 复制设备名称 char device_name[advertisement_length + 1];
...
:::tip 需要注意的是，在编译之前需要在 'Tools > Protocol stack' 中选择 'BLE (Silabs)'。
...
现在您可以选择 XIAO MG24 主板并上传程序。如果程序运行顺利，打开串行监视器并将波特率设置为 115200，您可以看到以下结果。
...
该程序打印出扫描到的蓝牙设备的名称、MAC 地址、通道和信号。

#### 程序注释

此示例演示了如何使用 Silicon Labs BLE 栈扫描附近的低功耗蓝牙 (BLE) 设备，打印每个发现设备的地址、RSSI（接收信号强度指示器）、通道和名称。

代码首先定义了一个事件处理函数 `sl_bt_on_event`，它处理 BLE 栈生成的各种低功耗蓝牙 (BLE) 事件。该函数使用 switch 语句来区分事件类型，例如 BLE 设备启动时和从附近设备接收广播报告时。在接收到启动事件时，它初始化串行通信，配置天线控制的 GPIO 引脚，并开始使用指定参数扫描 BLE 设备。

当扫描过程检测到来自 BLE 设备的广播报告时，会触发 `sl_bt_evt_scanner_legacy_advertisement_report_id` 情况。在这种情况下，函数为每个检测到的设备递增计数器并提取关键信息，包括设备的地址、RSSI、通道和本地名称。它利用辅助函数 `get_complete_local_name_from_ble_advertisement` 从广播数据中检索设备的完整名称，然后将其打印到串行输出。

辅助函数 `get_complete_local_name_from_ble_advertisement` 遍历广播数据以定位完整的本地名称字段。它检查每个广播记录的类型是否对应于完整的本地名称，并将其作为字符串返回。如果未找到完整名称，函数返回 "N/A"。这种系统化的方法允许应用程序有效地发现和识别附近的 BLE 设备，在扫描过程中提供有价值的信息。

### BLE 服务器/客户端

如前所述，XIAO MG24 可以充当服务器和客户端。让我们看看作为服务器的程序以及如何使用它。将以下程序上传到 XIAO 后，它将充当服务器并向连接到 XIAO 的所有蓝牙设备发送 "Hello World" 消息。
...
static const uint8_t advertised_name[] = "XIAO_MG24 Server";  // 您的 BLE 设备名称 static uint16_t gattdb_session_id;
...
void sl_bt_on_event(sl_bt_msg_t *evt) {
  switch (SL_BT_MSG_ID(evt->header)) {
    // -------------------------------
    // 此事件表示设备已启动且无线电已就绪。
    // 在接收到此启动事件之前不要调用任何栈命令！
    case sl_bt_evt_system_boot_id:
      {
        Serial.println("BLE 栈已启动");

        // 初始化应用程序特定的 GATT 表 ble_initialize_gatt_db();

        // 开始广播 ble_start_advertising();
        Serial.println("BLE 广播已启动");
      }
      break;

    // -------------------------------
    // 此事件表示打开了新连接 case sl_bt_evt_connection_opened_id:
      Serial.println("BLE 连接已打开");
      break;

    // -------------------------------
    // 此事件表示连接已关闭 case sl_bt_evt_connection_closed_id:
      Serial.println("BLE 连接已关闭");
      // 重新启动广播 ble_start_advertising();
      Serial.println("BLE 广播已重新启动");
      break;

    // -------------------------------
    // 此事件表示本地 GATT 数据库中属性的值
    // 被远程 GATT 客户端更改 case sl_bt_evt_gatt_server_attribute_value_id:
      // 检查更改的特征是否为 LED 控制
...
同时，您可以在各大手机应用商店搜索并下载 **nRF Connect** 应用，该应用允许您的手机搜索并连接蓝牙设备。
...
上述程序将把 XIAO 变成一个客户端并搜索附近的蓝牙设备。当蓝牙设备的 UUID 与您提供的 UUID 匹配时，它将连接到该设备并获取其特征值。
...
您可以保留默认的 UUID，或者您可以访问 [uuidgenerator.net](https://www.uuidgenerator.net/) 为您的服务和特征创建随机 UUID。

然后，您创建一个名为"XIAO_MG24 Server"的 BLE 设备。您可以将此名称更改为您喜欢的任何名称。在下面的行中，您将 BLE 设备设置为服务器。之后，您使用之前定义的 UUID 为 BLE 服务器创建一个服务。
...
创建特征后，您可以使用 `sl_bt_gatt_server_notify_all()` 方法设置其值。在这种情况下，我们将值设置为文本"Hello World"。您可以将此文本更改为您喜欢的任何内容。在未来的项目中，此文本可以是传感器读数或灯的状态，例如。

最后，您可以启动服务和广告，以便其他 BLE 设备可以扫描并找到此 BLE 设备。
...
我们需要准备两个 XIAO，一个作为服务器，一个作为客户端。这里是作为服务器的示例程序。XIAO 作为服务器有以下主要任务。

- 首先，使用函数 `getCPUTemp()` 获取 MCU 的当前温度；
- 其次，创建蓝牙服务器；
- 第三，通过蓝牙广告温度值；
- 第四，显示实时温度。
...
&temp_type_characteristic_handle);
  ...
  // 启动健康温度计服务 sc = sl_bt_gattdb_start_service(gattdb_session_id, service_handle);
  app_assert_status(sc);

  // 提交 GATT DB 更改 sc = sl_bt_gattdb_commit(gattdb_session_id);
  app_assert_status(sc);
}
...
为其中一个 XIAO 上传程序后，如果程序运行顺利，那么您可以拿出手机，使用 nRF Connect APP 搜索名为 **XIAOMG24_BLE** 的蓝牙设备，连接它，并点击下图所示的按钮，您将收到温度数据信息。
...
/*
   ...
   启动时，程序将开始扫描运行 'ble_health_thermometer' 示例并以 "Thermometer Example" 名称广播的其他开发板。
   一旦找到其他开发板，它会建立连接，发现其服务和特征，然后订阅温度测量。
   订阅后，示例开始定期从其他开发板接收温度数据，并将其打印到串口。

   在此处了解更多 API 使用信息：https://docs.silabs.com/bluetooth/latest/bluetooth-stack-api/
...
对于上述程序，我们将挑选比较重要的部分进行解释。我们先从服务器程序开始。
...
在教程的前面部分，我们已经讲过在服务器下会有特征，在特征下会有值和其他内容。所以我们在创建广播时需要遵循这一原则。

```c
// 健康温度计服务 const uint8_t thermometer_service_uuid[] = { 0x09, 0x18 };
sc = sl_bt_gattdb_add_service(gattdb_session_id,
                              sl_bt_gattdb_primary_service,
                              SL_BT_GATTDB_ADVERTISED_SERVICE,
                              sizeof(thermometer_service_uuid),
...
// 温度测量特征 const sl_bt_uuid_16_t temp_measurement_characteristic_uuid = { .data = { 0x1C, 0x2A } };
uint8_t temp_initial_value[5] = { 0, 0, 0, 0, 0 };
sc = sl_bt_gattdb_add_uuid16_characteristic(gattdb_session_id,
                                            service_handle,
                                            SL_BT_GATTDB_CHARACTERISTIC_INDICATE,
...
在上述程序中，您可以看到 `sl_bt_gattdb_add_service()` 用于创建服务器。参数是一个特定的 UUID：**0x1809**。在 GATT 规则中，**0x1809** 表示温度计类型数据，同样特征的 UUID：**0x2A1C** 也有特殊含义。在 GATT 中，它表示温度测量。这符合我们温度值的情况，所以这里我定义为这样。您可以在[这里](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/GATT.pdf)阅读 GATT 为我们准备的一些特定 UUID 的含义。

当然，您也可以不遵循 GATT 标准设置 UUID，您只需要确保这两个值是唯一的，不会影响您的客户端通过识别这些 UUID 找到值的能力。您可以访问 [uuidgenerator.net](https://www.uuidgenerator.net/) 为您的服务和特征创建随机 UUID。

最后，我们在 `loop` 中每秒测量并广播一次 MCU 的温度值。

接下来是客户端程序，它看起来会复杂得多。

在程序开始时，仍然是非常熟悉的内容。您需要确保这些内容与您在服务器端配置的内容一致。
...
接下来，我们将编写一个蓝牙协议栈事件处理函数，它主要处理各种蓝牙事件触发的回调任务，包括蓝牙设备的初始化、蓝牙的连接和断开连接，以及搜索附近的蓝牙设备。
...
以下过程是在服务器中查找温度值的关键。首先，在我们成功定位服务器 UUID 并在服务器下找到特征 UUID 后，我们将处理获得的数据，如下面的代码片段所示。最后，通过串口打印出处理后的数据。这种解析方法与蓝牙的数据结构一一对应。
...
:::tip 上述示例给出了单个传感器单个值的最简单示例，来源于 Silicon Labs。如果您想更深入地了解 SiliconLabs BLE API 的使用，我们建议阅读这里的教程。

- [SiliconLabs 蓝牙协议栈 API 参考](https://docs.silabs.com/bluetooth/latest/bluetooth-stack-api/)
