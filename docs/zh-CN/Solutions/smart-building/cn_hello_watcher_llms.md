...
Watcher 的"敏锐之眼"由强大的**本地 AI 视觉芯片（Himax）**驱动，实现快速本地处理的物体识别和目标跟踪。结合可扩展的助手固件，它不仅仅是一个摄像头——它是一个专属的 AI 助手，能够进化并理解你的需求。
...
2.  **连接网络**：启动后，设备将生成一个 Wi-Fi 热点。将您的手机或电脑连接到此热点。| 「[Watcher 网络设置](/cn/device_network_setup)」
