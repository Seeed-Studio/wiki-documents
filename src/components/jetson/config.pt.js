export const listData = [
  {
    id: 1,
    name: 'Carrier Board',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/21.png',
    brands: [
      {
        name: 'reComputer J10x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/J10_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_J1010_J101_Flash_Jetpack/',
      },
      {
        name: 'reComputer J20x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/J20_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_J2021_J202_Flash_Jetpack/',
      },
      {
        name: 'reComputer J30x/40x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/J30_40_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_J4012_Flash_Jetpack/',
      },
      {
        name: 'reComputer J50x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/J50_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reserver_j501_getting_started/',
      },
      {
        name: 'A203',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/A203_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_A203_Flash_System/',
      },
      {
        name: 'A205',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/A205_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_A205_Flash_System/',
      },
      {
        name: 'A603',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/A603_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_A603_Flash_System/',
      },
      {
        name: 'A607',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/A607_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_A607_Flash_System/',
      },
      {
        name: 'A608',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/A608_Carrier_Board.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_A608_Flash_System/',
      }
    ],
  },
  {
    id: 2,
    name: 'reComputer Jetson',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/18.png',
    brands: [
      {
        name: 'reComputer J1010',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/J1010_usage.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_Jetson_Series_Introduction/',
      },
      {
        name: 'reComputer J1020',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/J1020_usage.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_Jetson_Series_Introduction/',
      },
      {
        name: 'reComputer J20x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/18.png',
        href: 'https://wiki.seeedstudio.com/pt-br/recomputer_j20_with_jetson_getting_start/',
      },
      {
        name: 'reComputer J30x/J40x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/18.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_J30_40_with_Jetson_getting_start/',
      },
    ],
  },
  {
    id: 3,
    name: 'reComputer Industrial',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/19.png',
    brands: [
      {
        name: 'reComputer Industrial J40x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/19.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_Industrial_Getting_Started/',
      },
      {
        name: 'reComputer Industrial J30x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/19.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_Industrial_Getting_Started/',
      },
      {
        name: 'reComputer Industrial J20x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/19.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_Industrial_Getting_Started/',
      },
    ],
  },
  {
    id: 4,
    name: 'reServer Industrial',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/23.png',
    brands: [
      {
        name: 'reServer Industrial J40x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/23.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reServer_Industrial_Getting_Started/',
      },
      {
        name: 'reServer Industrial J30x',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/23.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reServer_Industrial_Getting_Started/',
      },
    ],
  },
  {
    id: 5,
    name: 'reServer Jetson',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/22.png',
    brands: [
      {
        name: 'reServer J2032',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/22.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reServer_J2032_Flash_Jetpack/',
      }
    ],
  },
  {
    id: 6,
    name: 'Outros dispositivos',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/nvidia_icon.png',
    brands: [
      {
        name: 'Jetson AGX Orin 32GB',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/AGX_Orin.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Jetson_AGX_Orin_32GB_H01_Flash_Jetpack/',
      },
      {
        name: 'Jetson Xavier AGX H01',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Jetson_Xavier_AGX_H01.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Jetson_Xavier_AGX_H01_Driver_Installation/',
      },
      {
        name: 'Jetson Mate',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Jetson_Mate.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Jetson-Mate/',
      },
      {
        name: 'T906 Mini',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/T906_Mini.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Mini_AI_Computer_T906/',
      },
      {
        name: 'A203 E Mini',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/A203_E_Mini.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_A203E_Flash_System/',
      },
      {
        name: 'A205 E Mini',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/A205_E_Mini.png',
        href: 'https://wiki.seeedstudio.com/pt-br/reComputer_A205E_Flash_System/',
      },
    ],
  }
]

export const listData1 = [
  {
    id: 1,
    name: 'Upload e rotulagem de dados',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/label.png',
    brands: [
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/always_ai.png',
        href: 'https://wiki.seeedstudio.com/pt-br/alwaysAI-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/roboflow.png',
        href: '/pt-br/tags/Roboflow/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/edge impulse.png',
        href: 'https://wiki.seeedstudio.com/pt-br/HardHat/',
      }
    ],
  },
  {
    id: 2,
    name: 'Treinamento de modelos de IA',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/train.png',
    brands: [
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/always_ai.png',
        href: 'https://wiki.seeedstudio.com/pt-br/alwaysAI-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/roboflow.png',
        href: '/pt-br/tags/Roboflow/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/edge impulse.png',
        href: 'https://wiki.seeedstudio.com/pt-br/HardHat/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/ultralytics.png',
        href: '/pt-br/tags/yolov-8/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/yolov5.png',
        href: 'https://wiki.seeedstudio.com/pt-br/YOLOv5-Object-Detection-Jetson/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/deci.png',
        href: 'https://wiki.seeedstudio.com/pt-br/DeciAI-Getting-Started/',
      }
    ],
  },
  {
    id: 3,
    name: 'Otimização de modelos de IA',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/optimize.png',
    brands: [
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/deci.png',
        href: 'https://wiki.seeedstudio.com/pt-br/DeciAI-Getting-Started/',
      }
    ],
  },
  {
    id: 4,
    name: 'Implantação de modelos de IA',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/deploy.png',
    brands: [
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/yolov5.png',
        href: 'https://wiki.seeedstudio.com/pt-br/YOLOv5-Object-Detection-Jetson/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/ultralytics.png',
        href: '/pt-br/tags/yolov-8/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/deci.png',
        href: 'https://wiki.seeedstudio.com/pt-br/DeciAI-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/roboflow.png',
        href: '/pt-br/tags/Roboflow/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/edge impulse.png',
        href: 'https://wiki.seeedstudio.com/pt-br/HardHat/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/always_ai.png',
        href: 'https://wiki.seeedstudio.com/pt-br/alwaysAI-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Cochl.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Cochl.Sense-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Scailable.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Scailable-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/CVEDIA.png',
        href: 'https://wiki.seeedstudio.com/pt-br/CVEDIA-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Lumeo.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Lumeo-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/GenAINerdsLogo1.png',
        href: 'https://wiki.seeedstudio.com/pt-br/gapi_getting_started-with_jetson/',
      },
    ],
  },
  {
    id: 5,
    name: 'Gerenciamento remoto',
    img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/manage.png',
    brands: [
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Lumeo.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Lumeo-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/always_ai.png',
        href: 'https://wiki.seeedstudio.com/pt-br/alwaysAI-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Cochl.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Cochl.Sense-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Scailable.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Scailable-Jetson-Getting-Started/',
      },
      {
        name: ' ',
        img: 'https://files.seeedstudio.com/wiki/reComputer/home_page/Allxon.png',
        href: 'https://wiki.seeedstudio.com/pt-br/Update-Jetson-Linux-OTA-Using-Allxon/',
      },
    ],
  },
]

export const communityList_cv = [
  {
    name: 'Treinar e implantar YOLOv8 no reComputer',
    img: 'https://files.seeedstudio.com/wiki/reComputer/Application/reComputer_is_all_you_need/inference.jpeg',
    URL: 'https://wiki.seeedstudio.com/pt-br/How_to_Train_and_Deploy_YOLOv8_on_reComputer/',
  },
  {
    name: 'Detecção de objetos few-shot com YOLOv5 e Roboflow',
    img: 'https://files.seeedstudio.com/wiki/YOLOV5/pink-flowers-2.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/YOLOv5-Object-Detection-Jetson/',
  },
  {
    name: 'Implantar YOLOv8 usando TensorRT e DeepStream',
    img: 'https://files.seeedstudio.com/wiki/YOLOV8/car.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/YOLOv8-DeepStream-TRT-Jetson/',
  },
  {
    name: 'Maskcam - Monitoramento do uso de máscaras faciais em multidões',
    img: 'https://files.seeedstudio.com/wiki/Jetson-Nano-MaskCam/tu3.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/Jetson-Nano-MaskCam/',
  },
  {
    name: 'AI NVR com reServer Jetson',
    img: 'https://files.seeedstudio.com/wiki/reComputer-Jetson/ai-nvr/result.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/ai_nvr_with_jetson/',
  },
  {
    name: 'Implantar Frigate em um dispositivo reComputer (NVIDIA Jetson)',
    img: 'https://docs.frigate.video/assets/images/media_browser-min-1f8a7c629d1bdbee1c78f99a97a0219a.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/deploy_frigate_on_jetson/',
  },
  {
    name: 'Experimentar rapidamente o YOLOv10 no reComputer',
    img: 'https://github.com/ZhuYaoHui1998/jetson-examples/raw/main/reComputer/scripts/yolov10/assets/webui.png',
    URL: 'https://github.com/ZhuYaoHui1998/jetson-examples/blob/main/reComputer/scripts/yolov10/README.md',
  },
  {
    name: 'Implantação rápida com um clique do Ultralytics YOLOv8 plug-and-play',
    img: 'https://github.com/ZhuYaoHui1998/jetson-examples/raw/main/reComputer/scripts/ultralytics-yolo/images/Ultralytics-yolo.gif',
    URL: 'https://github.com/ZhuYaoHui1998/jetson-examples/blob/main/reComputer/scripts/ultralytics-yolo/README.md',
  },
]

export const communityList_gen = [
  {
    name: 'LLM local customizado: ajuste fino de LLM com Llama-Factory no reComputer',
    img: 'https://files.seeedstudio.com/wiki/reComputer-Jetson/Llama-Factory/training.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/Finetune_LLM_on_Jetson/',
  },
  {
    name: 'Llama2-7B quantizado com MLC LLM no reComputer',
    img: 'https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/Llama-2-7b-chat-hf-q4f16_ft.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/Quantized_Llama2_7B_with_MLC_LLM_on_Jetson/',
  },
  {
    name: 'Executar um LLM local de texto para imagem no reComputer',
    img: 'https://files.seeedstudio.com/wiki/wiki-ranger/Contributions/Nvidia_Jetson_recomputer_LLM_texto-to-image/27_dreamshaperxl.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/How_to_run_local_llm_text_to_image_on_reComputer/',
  },
  {
    name: 'Implantar Whisper no reComputer para fala em texto em tempo real',
    img: 'https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/Real-Time-Whisper.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/Edge/NVIDIA_Jetson/Application/Generative_AI/Whisper_on_Jetson_for_Real_Time_Speech_to_Text/',
  },
  {
    name: 'RAG local baseado em Jetson com LlamaIndex',
    img: 'https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/Llama-2-7b-chat-hf-q4f16_ft.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/Local_RAG_based_on_Jetson_with_LlamaIndex/',
  },
  {
    name: 'Detecção zero-shot no reComputer com Jetson Platform Services',
    img: 'https://files.seeedstudio.com/wiki/reComputer/Application/zero_shot_detection/fig1.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/run_zero_shot_detection_on_recomputer/',
  },
  {
    name: 'Executar VLM no reComputer com Jetson Platform Services',
    img: 'https://files.seeedstudio.com/wiki/reComputer/Application/vlm/vlmgif.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/run_vlm_on_recomputer/',
  },
  {
    name: 'Formatar saída com Langchain',
    img: 'https://files.seeedstudio.com/wiki/reComputer/Application/Format_LLM_Opt/ai_assistant.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/How_to_Format_the_Output_of_LLM_Using_Langchain_on_Jetson/',
  },
  {
    name: 'Chatbot de voz local: implantar Riva e Llama2 no reComputer',
    img: 'https://files.seeedstudio.com/wiki/reComputer/Application/Local_Voice_Chatbot/workflow.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/Local_Voice_Chatbot/',
  },
  {
    name: 'Geração de legendas por voz no Nvidia Jetson',
    img: 'https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/recoder.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/Real%20Time%20Subtitle%20Recoder%20on%20Nvidia%20Jetson/',
  },
  {
    name: 'Assistente de IA local: implantar Ollama e AnytingLLM no Jetson',
    img: 'https://files.seeedstudio.com/wiki/reComputer/Application/local-ai-assistant/ai-assistant.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/local_ai_ssistant/',
  },
  {
    name: 'Executar VLM com interação por voz no reComputer Jetson',
    img: 'https://files.seeedstudio.com/wiki/reComputer/Application/Multimodal_ai/audio_vlm/vlmgif.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/speech_vlm/',
  },
  {
    name: 'Uso do Llama-Factory com um clique',
    img: 'https://github.com/Seeed-Projects/jetson-examples/raw/main/reComputer/scripts/llama-factory/assets/llama-factory-Jetson.png',
    URL: 'https://github.com/Seeed-Projects/jetson-examples/tree/main/reComputer/scripts/llama-factory',
  },
  {
    name: 'Implantar Deep-Live-Cam AI face-swapping com um clique',
    img: 'https://github.com/Seeed-Projects/jetson-examples/raw/main/reComputer/scripts/deep-live-cam/images/WebUI.png',
    URL: 'https://github.com/Seeed-Projects/jetson-examples/blob/main/reComputer/scripts/deep-live-cam/README.md',
  },
  {
    name: 'Executar ComfyUI (GUI do Stable Diffusion) no reComputer',
    img: 'https://github.com/Seeed-Projects/jetson-examples/raw/main/reComputer/scripts/comfyui/images/comfyui.png',
    URL: 'https://github.com/Seeed-Projects/jetson-examples/blob/main/reComputer/scripts/comfyui/README.md',
  },

]

export const communityList_robot = [
  {
    name: 'Instalar ROS1 no reComputer',
    img: 'https://files.seeedstudio.com/wiki/robotics/software/install_ros1/fig2.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/installing_ros1/',
  },
  {
    name: 'Usar LiDAR RoboSense no reComputer',
    img: 'https://files.seeedstudio.com/wiki/robotics/hardware/robosense/fig10.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/robosense_lidar/',
  },
  {
    name: 'Usar múltiplas câmeras CSI no reComputer com ROS',
    img: 'https://files.seeedstudio.com/wiki/robotics/hardware/csi_camera/fig2.png',
    URL: 'https://wiki.seeedstudio.com/pt-br/csi_camera_on_ros/',
  },
  {
    name: 'Executar A-LOAM 3D SLAM no reComputer',
    img: 'https://files.seeedstudio.com/wiki/robotics/software/aloam/fig0.gif',
    URL: 'https://wiki.seeedstudio.com/pt-br/a_loam/',
  },
]