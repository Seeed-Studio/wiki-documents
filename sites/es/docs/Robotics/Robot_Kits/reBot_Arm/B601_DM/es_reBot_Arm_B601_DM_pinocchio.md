...
1. **Análisis cinemático completo**  
   Compatible con cálculos de Cinemática Directa (FK) y Cinemática Inversa (IK), capaz de resolver en tiempo real la pose del efector final del brazo robótico.
...
Configura automáticamente todos los ceros de las articulaciones y muestra los ángulos de las articulaciones en tiempo real.
...
Usar cinemática inversa (IK) en modo MIT para especificar las coordenadas 3D (X, Y, Z) y la orientación (ángulos de Euler) a las que debe moverse el efector final del brazo robótico.
...
Nota: Cuando una persona empuja el brazo con suficiente fuerza como para que la velocidad lineal del efector final > 0.04 m/s o la velocidad angular > 0.08 rad/s, se desbloquea y actualiza el ángulo objetivo en tiempo real.
...
- Reproducción de la animación de la trayectoria completa en MeshCat
