...
Esta guía es para desarrolladores. Explica cómo ejecutar y ampliar la consola web `reBotArm_simulator-DM` en el ReBot Arm Digital Twin & Control Stack para B601-DM. La consola es un front end ligero basado en Node.js + Three.js que lee el URDF y las mallas STL desde el workspace de ROS2 en el mismo repositorio, renderiza en el navegador el cuerpo de 6 GDL y la garra del reBot Arm B601-DM, y se comunica con ROS2 a través de un WebSocket de rosbridge. Admite todo el flujo de trabajo de desarrollo: mirroring de articulaciones, bloqueo de control, compensación de gravedad, agarre visual y control por texto mediante LLM.
...
| MuJoCo (opcional) | 3.10+, solo necesario para el stack completo de simulación física |
...
:::note
Si `setup.sh` no instala automáticamente ROS 2 (por ejemplo, si la fuente apt de ROS aún no se ha añadido al sistema), el instalador descarga automáticamente el paquete oficial `ros2-apt-source` desde GitHub, añade la fuente y vuelve a intentarlo. No necesitas configurar manualmente la fuente apt.
:::
...
Abre `http://localhost:3001` en un navegador y espera a que terminen de cargarse el URDF y el STL; cuando aparezca el modelo 3D significa que el front-end está funcionando. La página ya está conectada al rosbridge local por defecto, así que puedes operar directamente en el panel "ROS2 Bridge".
...
:::note
El comando `./rebotarm` carga internamente el entorno, por lo que no necesitas ejecutar manualmente `source scripts/source_rebotarm_env.sh`. Sin embargo, si ejecutas directamente comandos `ros2` “pelados”, cada nueva terminal sigue necesitando cargar el entorno primero.
:::
...
Un solo comando inicia toda la pila: Fake Driver, física de agarre MuJoCo, servidor de tareas, cámara virtual, detector de color y rosbridge:
...
Flujo de datos: el navegador accede al servidor estático de Node.js mediante `HTTP /api` y se comunica bidireccionalmente con ROS2 a través de `rosbridge WebSocket`; el lenguaje natural es reenviado por Node.js al Text Agent / MCP Server y luego se convierte en llamadas de herramientas estructuradas que entran en ROS2. ROS2 impulsa hacia abajo el driver falso/real y el brazo, y se conecta lateralmente a la simulación física MuJoCo, al servidor de tareas y a la cámara virtual. La página web, el LLM Agent y el robot real no codifican llamadas directas entre sí; están desacoplados mediante topics, servicios y acciones de ROS2.

El punto de entrada unificado `rebotarm` es la forma principal de operar el proyecto:

| Command | Description |
| --- | --- |
| `./rebotarm start web` | Inicia rosbridge + servidor web (carga automáticamente el entorno) |
| `./rebotarm start dm` | Inicia el driver de robot real DM (terminal separada, carga automáticamente el entorno) |
| `./rebotarm start sim` | Inicia toda la pila de simulación MuJoCo (no iniciar junto con el robot real) |
| `./rebotarm doctor` | Comprobación de diagnóstico (equivalente a `./setup.sh --check`) |
| `./rebotarm status` | Ver estado de procesos, puertos, puertos serie y nodos ROS |
| `./rebotarm stop` | Detiene los procesos en segundo plano gestionados por `start web` |

:::note
Todos los comandos `./rebotarm` ejecutan internamente `source scripts/source_rebotarm_env.sh`, por lo que no necesitas cargar el entorno manualmente. Sin embargo, si ejecutas directamente comandos `ros2` “pelados” (como iniciar manualmente un archivo de lanzamiento), aún necesitas cargar el entorno primero:
...
Este script carga, en orden, ROS2 (`/opt/ros/jazzy/setup.bash`), el venv de Python (`.venv/bin/activate`), las rutas de cmeel (extensiones C de Pinocchio) y el workspace (`install/setup.bash`).
:::

<details>
<summary><b>Notas del módulo central</b> (haz clic para expandir)</summary>
...
- Servir los recursos estáticos de front-end bajo `public/`;
...
- Hacer proxy de las solicitudes de chat del LLM `/api/llm/chat` y del health check `/api/llm/health` al servicio HTTP del text-agent en la VM;
...
:::note
`server.js` localiza el workspace de ROS2 mediante la ruta relativa `../reBotArm_ros2_DM/...`. Si mueves el directorio web a otra ubicación, debes actualizar estas rutas en consecuencia, o mantener una copia del modelo de la misma versión que el workspace de ROS2 en el directorio web.
:::

**rebot-sim.js — núcleo de la escena 3D**

`rebot-sim.js` es el núcleo de front-end (unas 1700 líneas), responsable de:
...
- Proporcionar presets de pose, deslizadores de articulaciones, arrastre del TCP, grabación/reproducción/exportación de enseñanza, estimación del sobre de alcance y fantasma de objetivo;
...
El namespace predeterminado es `rebotarm`, y todas las rutas de tópicos/servicios se prefijan con `/rebotarm/`.
...
El bloqueo de control es una protección importante contra operaciones accidentales. `controlAllowed()` comprueba de forma uniforme: cuando ROS no está conectado o el bloqueo de control no está marcado, todos los comandos de control se interceptan y la página solo actualiza el modelo 3D.

**rebot-llm.js — interfaz de control por texto con LLM**
...
Al iniciar primero llama a `/api/llm/health` para hacer un health-check del text-agent; tras el éxito habilita el cuadro de entrada. Los mensajes se envían mediante proxy al text-agent a través de `/api/llm/chat`, y el `text` y los `events` devueltos (proceso de llamada de herramientas) se renderizan en el área de chat. Al detener, envía `{ text: '__reset__', reset: true }` para limpiar el contexto.
...
A continuación se enumeran las interfaces clave de ROS2 a las que el simulador web se suscribe y publica. El namespace predeterminado es `rebotarm`.
...
El control en lenguaje natural no se invoca directamente desde el navegador a ROS. Se hace proxy a través de Node.js. El diseño por capas permite que el LLM entienda la intención mientras que la capa MCP restringe la intención a operaciones estructuradas del robot.

### Iniciar el servidor MCP y el Text Agent
...
Modo de movimiento en simulación (movimiento permitido):

```bash
ros2 launch rebotarm_agent rebotarm_mcp.launch.py motion_mode:=allow
```

Inicia el servicio HTTP del text-agent (para que lo llame la página web):
...
En el panel "LLM text control" de la página web, haz clic en "Start AI assistant". La página primero hace un health-check al text-agent; tras tener éxito habilita el cuadro de entrada. Puedes escribir directamente comandos en lenguaje natural, por ejemplo:
...
La respuesta del text-agent y el proceso de llamada de herramientas se renderizan en el área de chat.
...
Después de cambiarlos, reinicia `./rebotarm start web` (o `node server.js`). Al arrancar, la página lee y muestra el backend proxy actual desde `/api/mcp/config`.
...
- **Resumen de herramientas**: obtiene automáticamente todas las herramientas registradas desde el MCP Server y las agrupa por categoría (estado y diagnóstico, control de habilitación, control de movimiento, control de pinza, compensación de gravedad, agarre visual, grabación y reproducción);
...
Los límites de articulaciones y las poses predefinidas se definen en los objetos `jointDefs` y `presets` al principio de `rebot-sim.js`. Después de modificarlos, actualiza la página para que surtan efecto; no es necesario recompilar. Ten en cuenta que los límites de articulaciones deben ser coherentes con el `<limit>` en el URDF, de lo contrario el modelo web y el comportamiento en ROS no coincidirán.
...
### Ampliar herramientas del LLM

Las herramientas del LLM están definidas por el MCP Server en `rebotarm_agent`. Añadir una nueva herramienta requiere implementarla en el paquete `rebotarm_agent` en el workspace de ROS2; tras recompilar, el text-agent la expone automáticamente. No se necesitan cambios en la parte web; el proceso de llamada de herramientas se devuelve a través del campo `events` de `/api/llm/chat` y se renderiza.

### Modificar las mallas de la pinza en la web

Los STLs de la pinza solo para la web están en `split_meshes/grouped_gripper/`, incluyendo `gripper_base.stl`, `gripper_hardware.stl`, `left_finger.stl` y `right_finger.stl`. Sustituye estos archivos y actualiza la página. No añadas una segunda copia de `urdf/` o `meshes/` en el directorio web; en tiempo de ejecución solo se usan estos cuatro STLs de la pinza.

### Modificar la dirección de conexión de rosbridge

La dirección WebSocket de rosbridge la introduce manualmente el usuario en el panel "ROS2 Bridge" de la página web; no está codificada de forma fija por defecto. Para cambiar la dirección por defecto o preestablecida:
...
- La ruta `BRINGUP_DIR` en `server.js` se resuelve de forma incorrecta (el directorio web se movió a una ubicación que no es monorepo), por lo que no se puede encontrar `src/rebotarm_bringup/description/`;
- `package://rebotarm_bringup/...` en el URDF no se puede mapear; confirma que `loader.packages` apunta a `${origin}/api`;
- Falta el archivo STL o la coincidencia de mayúsculas/minúsculas de la ruta no es correcta (Linux distingue entre mayúsculas y minúsculas).

### 2. Después de conectar con ROS, el estado permanece en "offline"

Comprueba en este orden:

- Si rosbridge se está ejecutando en el lado de Ubuntu y escuchando en `0.0.0.0:9090` (no en `127.0.0.1`);
...
Y confirma que `REBOTARM_TEXT_AGENT_URL` en `.env` apunta a la IP y el puerto correctos de la VM (por defecto `8082`). La página primero llama a `/api/llm/health` para comprobar el estado; si falla, muestra el error específico en el área de mensajes.
...
Los recursos del front-end se sirven de forma estática mediante Node.js; después de cambiar, actualiza el navegador. La versión actual no registra un Service Worker, por lo que no hay caché sin conexión que impida que se actualice la versión antigua. Si el navegador sigue mostrando contenido antiguo, usa una recarga forzada (Ctrl+Shift+R) o borra la caché normal.
...
- [Demo de agarre visual de reBot Arm B601-DM](https://wiki.seeedstudio.com/es/rebot_arm_b601_dm_grasping_demo/)
- [reBot Arm B601-DM Pinocchio y MeshCat](https://wiki.seeedstudio.com/es/rebot_arm_b601_dm_pinocchio_meshcat/)
- [Tutorial de reBot Arm B601-DM con LeRobot](https://wiki.seeedstudio.com/es/rebot_arm_b601_dm_lerobot/)
