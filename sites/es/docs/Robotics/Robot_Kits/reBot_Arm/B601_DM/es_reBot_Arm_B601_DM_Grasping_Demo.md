---
description: Este tutorial muestra cómo construir una demo completa de agarre visual para el reBot Arm B601-DM usando Orbbec Gemini 2 y una canalización de agarre YOLO/OBB.
...
YOLO es una familia ampliamente utilizada de modelos de detección de objetos en tiempo real que puede localizar y clasificar objetivos en una sola pasada hacia adelante. Este tutorial usa YOLO junto con la cámara de profundidad Orbbec Gemini 2 para construir una demo funcional de agarre visual de escritorio para el reBot Arm B601-DM, que cubre la configuración del entorno, la integración de la cámara, la calibración mano-ojo y la validación del agarre.
...
No necesitas GraspNet para `scripts/main.py` o `scripts/ordinary_grasp_pipeline.py`. Configúralo solo cuando quieras ejecutar `scripts/graspnet_camera_demo.py` o `scripts/grasp.py`, que requieren GraspNet, PyTorch con soporte CUDA, los operadores CUDA PointNet2/knn y un checkpoint preentrenado.
...
Después de descargar el peso preentrenado oficial de GraspNet, coloca `checkpoint-rs.tar` en:
...
- Si el punto de agarre se encuentra cerca de la zona central del objetivo
...
Este script ejecuta la estimación de la pose de agarre 6D de GraspNet solo con la cámara RGB-D, sin conectar el brazo robótico. Mantiene una vista previa en vivo de la cámara, usa cajas delimitadoras de YOLO para seleccionar el área objetivo y filtra los candidatos de escena completa de GraspNet factibles según la bbox objetivo.
...
- [Repositorio Graspnet-Baseline](https://github.com/graspnet/graspnet-baseline)
