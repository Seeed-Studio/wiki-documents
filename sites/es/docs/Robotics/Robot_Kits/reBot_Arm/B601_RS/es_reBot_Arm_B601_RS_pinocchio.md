...
:::caution
Si esta configuración no se modifica, el programa se comunicará usando el protocolo de motor Damiao, lo que hará que los motores RS no puedan ser reconocidos ni funcionar correctamente.
:::
...
Antes de ejecutar ejemplos de control en máquina real y depurar motores, necesitas configurar el canal CAN (para PCAN-USB, es necesario configurarlo de nuevo después de reconectar):
...
**Nota**: Los motores Robostride usan la interfaz CAN (por defecto `can0`), con el ID de host/feedback establecido por defecto en `0xFD`. Durante las pruebas del motor, es necesario deshabilitar primero el motor y luego volver a habilitarlo para permitir la lectura y el control normales.
...
:::caution Limita el alcance de la prueba antes de ajustar
Cambios grandes de `kp` / `kd` en varias articulaciones pueden causar oscilación inmediata, sobrecorriente o colisiones con topes mecánicos si cualquier dirección o signo de articulación es incorrecto. Antes de ajustar, despeja el espacio de trabajo del brazo y planea probar **una sola articulación y un solo modo a la vez, en pequeños pasos**.
:::
...
3. **Conserva la indentación YAML**: 2 espacios por nivel, claves separadas de valores por `: `. Una indentación incorrecta hace que falle el análisis de `yaml.safe_load`, y todos los parámetros volverán a los valores por defecto.
...
- **Observación en tiempo de ejecución**: habilita el motor en demo3 / demo4 y revisa `state`; si los parámetros parecen no haber cambiado o el motor se comporta exactamente igual que antes, el YAML se editó incorrectamente o fue sobrescrito por los valores por defecto.
...
Este ejemplo envía directamente los ángulos objetivo de las articulaciones a los motores, sin **planificación de trayectoria ni de velocidad**. Un cambio grande del objetivo puede causar un movimiento repentino a alta velocidad y activar la protección por sobrecorriente.
...
:::danger Opcional — Este ejemplo no tiene planificación de trayectoria suave
Este ejemplo envía la solución de IK directamente como objetivo de articulaciones, **sin planificación de trayectoria ni de velocidad**. Un cambio grande en la pose objetivo puede causar un movimiento repentino a alta velocidad y activar la protección por sobrecorriente.
...
**Salida**:
- Muestra el estado de bloqueo en tiempo real (LOCKED / UPDATE)
...
<div align="center">
    <img width={800}
    src="https://files.seeedstudio.com/wiki/robotics/projects/rebot_arm/meshcat_RS.png" alt="Simulación MeshCat del reBot Arm B601-RS" />
</div>
...
**Características**:
- Muestra la posición y orientación del efector final en tiempo real
...
Simulación interactiva de cinemática inversa, resolviendo automáticamente los ángulos articulares y visualizando una pose objetivo.
...
- Utiliza un perfil de trayectoria de mínimo tirón
...
[LINE_88>Wrapper del visualizador MeshCat, que proporciona una interfaz unificada de visualización del robot.</p>
...
- **El cálculo de CI falla o los resultados son anormales**
