...
2. Haz clic en **Upgrade**, haz clic en **Yes** en el cuadro de confirmación y espera a que finalice el proceso de flasheo del firmware. A continuación, puedes cerrar DfuSeDemo.
...
```
O simplemente cópialo en una memoria USB y conéctala al equipo Ubuntu; siempre que los archivos terminen en ~/Downloads, el directorio actual o /tmp, el script los encontrará automáticamente.
...
Si los transfieres desde otro ordenador (por ejemplo, con scp):

```text
scp flash_pcan_mac.sh pcan_canable_hw.bin seeed@your_MAC_IP:~/Downloads/
```

O simplemente cópialos en una unidad flash USB y conéctala al Mac; siempre que los archivos terminen en ~/Downloads, el directorio actual o /tmp, el script los encontrará automáticamente.
...
<!-- ### 3. Escribir IDs de los motores

:::tip Usuarios del kit preensamblado, por favor omitan este paso
:::
...
2. Haz clic en **Apply Default Template** y confirma que la página indica que la plantilla de parámetros predeterminados reBot Arm RobStride se ha aplicado a las articulaciones 1-7. Esta operación solo carga los valores recomendados en la página; aún no los escribe en los motores.
...
4. Después de que la escritura se complete, MotorBridge Studio lee automáticamente de nuevo los parámetros. La inicialización se realiza correctamente cuando la página indica que la verificación de lectura posterior a la escritura coincide.
