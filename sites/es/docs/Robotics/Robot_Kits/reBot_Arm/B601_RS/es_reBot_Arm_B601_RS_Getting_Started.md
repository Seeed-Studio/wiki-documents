...
El proyecto reBot Arm ha sido publicado como código abierto en [GitHub](https://github.com/Seeed-Projects/reBot-DevArm). Esta guía te llevará a través del inicio rápido del B601-RS, desde el montaje hasta la operación.
...
2. Experimenta las nuevas funciones y detalles de MotorBridge adaptados específicamente al brazo robótico reBot, incluyendo calibración del punto cero con un solo clic, escritura de parámetros, control de motores mediante arrastrar y soltar a través de la interfaz de usuario, y una interfaz integrada de visualización de modelos.

3. Esta herramienta es totalmente compatible con los sistemas operativos **Windows, Ubuntu y macOS**.

:::tip
1. Se ha verificado que el rendimiento de las máquinas virtuales es insuficiente para ejecutar las demostraciones y existen problemas de configuración. Se recomienda utilizar una máquina física con Ubuntu para controlar el brazo robótico.
...
3. El agente utiliza comandos de CLI para completar la escritura de los ID de los motores, mientras que la wiki utiliza un método de interacción mediante interfaz web. Ambos enfoques funcionan.
...
:::tip
  **Usuarios de Git Bash**: Si no se encuentra el comando `conda`, significa que Git Bash no ha cargado el entorno de conda. Primero necesitas inicializarlo:
  ...
  Para permitir que PowerShell active automáticamente el entorno conda:
...
source ~/.bashrc
```
Ejecuta esto después de reiniciar o volver a conectar el PCAN-USB:
```bash
pcan_refresh
```
...
3. Ajusta el interruptor DIP del módulo USB2CAN a **BOOT**, luego conecta el módulo a tu ordenador.
...
```
O simplemente cópielo a una unidad flash USB y conéctela al equipo con Ubuntu: siempre que los archivos terminen en ~/Downloads, el directorio actual o /tmp, el script los encontrará automáticamente.
...
Introduzca su contraseña; espere a que finalice

Tras finalizar, cambie de nuevo a "120R"
...
3.Por favor, extraiga el archivo USB2CAN.zip del paso 1 y coloque flash_pcan_mac.sh y pcan_canable_hw.bin (desde el interior de USB2CAN.zip) en el mismo directorio
...
O simplemente cópielo a una unidad flash USB y conéctela al MAC: siempre que los archivos terminen en ~/Downloads, el directorio actual o /tmp, el script los encontrará automáticamente.
...
El comando anterior asume que los archivos se colocan en la ruta Downloads del Mac; ajústelo según su ruta real
...
Antes de la configuración de los parámetros del motor, tenga en cuenta las siguientes preparaciones y normas de seguridad:

- Prepare 2 abrazaderas de sujeción (tamaño ≥3 pulgadas) y una fuente de alimentación conmutada de 48 V con salida XT30 (elija una marca de buena reputación; no utilice fuentes de alimentación de baja calidad).
- Durante la depuración y el funcionamiento, mantenga una distancia de seguridad de al menos 1 metro.
- No conecte ni desconecte los motores en caliente; desconecte la fuente de alimentación antes de enchufar o desenchufar el conector XT30 2+2.
- No sobrecargue ni haga funcionar los motores a exceso de velocidad; compruebe el cableado y los elementos de fijación antes de la puesta en marcha; no utilice el equipo en entornos húmedos, de alta temperatura o con polvo.
- Establezca parámetros de programa razonables y una función de parada de emergencia para evitar que el equipo se descontrole.
- **Siga estrictamente las reglas anteriores. El vendedor no se hace responsable de ningún riesgo o pérdida causada por operaciones no conformes o errores humanos.**


#### Escritura de puntos cero y depuración mediante la interfaz web

Abra la dirección [motorbridge-studio](https://motorbridge.github.io/motorbridge-studio/) en su navegador, haga clic en la opción Help, copie el comando correspondiente según su sistema operativo y placa controladora, verifique la dirección IP y el número de puerto y, a continuación, pulse Intro en la terminal para ejecutarlo.
...
:::warning Complete la inicialización de parámetros antes del primer uso
...
Primero seleccione `rebot-arm-robstride` en **Robot Model** en [MotorBridge Studio](https://motorbridge.github.io/motorbridge-studio/), escanee y confirme que las articulaciones 1-7 están todas en línea y complete la calibración cero del brazo robótico descrita anteriormente. A continuación, realice los siguientes pasos:

1. Haga clic en **Read Parameters** para leer los parámetros guardados actualmente en todas las articulaciones en línea. Esta operación solo lee datos y no modifica los motores. Espere hasta que la página indique que los parámetros de control se han leído correctamente y conserve los valores actuales como registro.
2. Haga clic en **Apply Default Template** y confirme que la página indica que la plantilla de parámetros predeterminados de reBot Arm RobStride se ha aplicado a las articulaciones 1-7. Esta operación solo carga los valores recomendados en la página; aún no los escribe en los motores.
...
3. Haga clic en **Write Parameters**. Confirme que el brazo robótico está soportado de forma segura y que no hay personas ni obstáculos cerca, y luego confirme la operación de escritura en el cuadro de diálogo. No desconecte la alimentación ni conecte o desconecte cables de motor mientras se están escribiendo los parámetros.
...
4. Una vez completada la escritura, MotorBridge Studio lee automáticamente de nuevo los parámetros. La inicialización se habrá realizado correctamente cuando la página indique que la verificación de lectura posterior a la escritura coincide.
