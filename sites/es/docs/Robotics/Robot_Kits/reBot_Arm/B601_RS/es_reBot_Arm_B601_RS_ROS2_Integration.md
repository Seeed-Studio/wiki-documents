...
3. **Integración con MoveIt 2**  
   Incluye configuración de MoveIt 2 y demostraciones de aplicación, admitiendo planificación simulada mediante el plugin MotionPlanning de RViz y ejecución en hardware real a través del nodo de control de hardware.
...
Antes de comenzar la integración con ROS2, completa la [Guía de inicio rápido de reBot Arm B601-RS](https://wiki.seeedstudio.com/es/rebot_b601_rs_getting_started/), incluyendo el montaje, la configuración de ID de los motores, la inicialización de la posición cero y otras preparaciones básicas.
...
Se recomienda usar el repositorio oficial Seeed-Projects:
...
|---|---|---|
| `hardware_config` | `rebotarm_hardware.yaml` incorporado desde bringup | Ruta de configuración de hardware de capa superior ROS2 |
...
| API | Tipo | Descripción |
|---|---|---|
| `/rebotarm/joints/<joint>/cmd/mit` | `rebotarm_msgs/msg/JointMitCmd` | Comando bruto MIT de una sola articulación |
...
| `/rebotarm/gripper/cmd/mit` | `rebotarm_msgs/msg/JointMitCmd` | Comando bruto MIT de la pinza |
...
El entorno MoveIt usa hardware simulado a través de `ros2_control` y `move_group` para la planificación y ejecución. Es adecuado para validar el modelo, la IK, la planificación de trayectorias y el flujo de demostración en RViz.
...
La planificación con MoveIt puede usarse a través de la GUI de RViz o llamando a nodos, tanto en simulación como en escenas reales.

#### Usar MoveIt en simulación

MoveIt usa la interfaz de hardware virtual de ros2_control para la simulación en RViz:
...
RViz se abre automáticamente y carga el modelo B601-RS. Puedes controlar el movimiento mediante el panel de la GUI a la izquierda.
...
| `rebotarm_moveit_demos/config/draw_square_rs.yaml` | Parámetros de la demo de dibujar un cuadrado |
...
- Si la ruta de las mallas del URDF es `package://rebotarm_bringup/description/...`
