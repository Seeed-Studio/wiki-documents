...
<p align="center">
  <strong>ROS2 Control · SocketCAN · Visualización en RViz · Soporte MoveIt 2 · Compensación de gravedad</strong>
</p>

Este tutorial muestra cómo controlar el reBot Arm B601-RS a través del espacio de trabajo ROS2 `rebotarm_ros2`. El espacio de trabajo encapsula el SDK de Python `reBotArm_control_py`, incluyendo cinemática inversa basada en Pinocchio, planificación de trayectorias y compensación de gravedad, en topics, servicios y acciones de ROS2. Esto facilita el uso de planificación de alto nivel, visualización en RViz, compensación de gravedad, control del gripper y desarrollo de aplicaciones personalizadas.
...
2. **Nodos listos para usar de cinemática, planificación de trayectorias y compensación de gravedad**  
   Proporciona soporte listo para usar de cinemática directa/inversa, planificación de trayectorias, compensación de gravedad y visualización en RViz.

3. **Integración con MoveIt 2**  
   Incluye configuración de MoveIt 2 y demos de aplicación, soportando planificación simulada mediante el plugin MotionPlanning de RViz y ejecución en hardware real a través del nodo de control de hardware.
...
2. Conecta el motor del gripper al mismo bus CAN.
...
- [Instalación de ROS2 Humble en el Wiki de Seeed Studio](https://wiki.seeedstudio.com/es/install_ros2_humble/)
...
### Iniciar solo la visualización en RViz
...
Por lo tanto, todos los topics, servicios y acciones tienen el prefijo `/rebotarm`, por ejemplo:
...
| `/rebotarm/gripper/open` | `rebotarm_msgs/srv/GripperCommand` | Abrir la pinza según el valor configurado |
| `/rebotarm/gripper/close` | `rebotarm_msgs/srv/GripperCommand` | Cerrar la pinza según el valor configurado |
...
| `/rebotarm/gripper/cmd/mit` | `rebotarm_msgs/msg/JointMitCmd` | Comando MIT de pinza en bruto |
| `/rebotarm/gripper/cmd/pos_vel` | `rebotarm_msgs/msg/JointPosVelCmd` | Comando en bruto de posición-velocidad de pinza |
...
La configuración y las demostraciones de MoveIt están incluidas en este espacio de trabajo. Después de instalar las dependencias, vuelve a compilar el espacio de trabajo:
...
| `tcp_rpy` | Orientación del TCP, con un efector final apuntando hacia abajo de forma predeterminada |
| `tcp_yaw_offsets` | Valores alternativos de yaw de IK usados para evitar grandes giros de la articulación 6 |

`draw_square` es útil para verificar si el IK de MoveIt, la planificación de trayectorias y la ejecución funcionan correctamente. Si la planificación falla, primero comprueba si el estado articular inicial, el centro del rectángulo, la orientación del TCP y los límites articulares son razonables.
...
| `open_gripper_position` / `grasp_gripper_position` / `closed_gripper_position` | Posiciones simuladas de la articulación de un solo lado del gripper |
| `hardware_open_gripper_position` / `hardware_closed_gripper_position` | Posiciones de apertura/cierre del motor del gripper en hardware real |

`pick_place` añade un objeto objetivo a la escena de planificación, abre el gripper, se mueve a la pose de recogida, cierra el gripper, se mueve a la pose de colocación y suelta el objeto. Las posiciones simuladas de la articulación del gripper y las posiciones reales del motor del gripper en hardware son parámetros diferentes. Antes de ejecutar en hardware real, confirma la dirección de apertura/cierre del gripper y sus límites.
...
| `rebotarm_moveit_demos/config/draw_square_rs.yaml` | Parámetros de la demo Draw-square |
| `rebotarm_moveit_demos/config/pick_place_rs.yaml` | Parámetros de la demo Pick-place |
...
Si la demo muestra `MoveIt planning failed`, comprueba lo siguiente:

- Confirma que `demo.launch.py` o `hardware.launch.py` se ha iniciado y que `move_group` se está ejecutando correctamente.
- Confirma que el estado articular actual no está obsoleto y que `/joint_states` o `/rebotarm/joint_states` se está publicando con normalidad.
- Confirma que el punto objetivo está dentro del espacio de trabajo alcanzable y que la orientación objetivo no hace que la muñeca o el gripper colisionen con el cuerpo del brazo.
- Confirma que los límites articulares y de velocidad en `joint_limits.yaml` coinciden con el brazo robótico actual.
- Planifica manualmente primero en el plugin MotionPlanning de RViz. Ejecuta la demo solo después de confirmar que la trayectoria está libre de colisiones.
