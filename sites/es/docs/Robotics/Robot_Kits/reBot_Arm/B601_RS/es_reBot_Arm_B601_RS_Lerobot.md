...
Este wiki proporciona tutoriales de depuración para reBot-DevArm y implementa la recopilación de datos y el entrenamiento dentro del framework LeRobot.

:::caution
Los tutoriales de Seeed Studio se actualizan estrictamente de acuerdo con la documentación oficial. Si encuentras problemas de software o de entorno que no se puedan resolver, primero consulta las FAQ al final del artículo, o contacta con el servicio de atención al cliente para unirte al grupo de discusión SeeedStudio LeRobot. También puedes hacer preguntas aquí: [LeRobot GitHub](https://github.com/huggingface/lerobot) o en el [Discord Channel](https://discord.gg/8TnwDdjFGU).
:::
...
:::tip
Para más detalles sobre los paquetes de funciones, consulta:
...
El repositorio de LeRobot incluye un `pyproject.toml`. Crea un entorno Conda e instala todas las dependencias.
...
ffmpeg es una dependencia de decodificación de vídeo. Instálalo con Conda:
...
- De forma predeterminada, se instalará ffmpeg 7.X, que admite el codificador `libsvtav1`.
- Si encuentras problemas de compatibilidad de versión, instala ffmpeg 7.1.1:

  ```bash
  conda install ffmpeg=7.1.1 -c conda-forge
  ```

- Comprueba si el codificador `libsvtav1` es compatible con `ffmpeg -encoders | grep svtav1`.
...
(Omite este paso en un PC.) Antes de continuar en un dispositivo Jetson JetPack 6.0+, instala PyTorch y Torchvision con soporte de GPU en el paso 5 de [este tutorial](https://github.com/Seeed-Projects/reComputer-Jetson-for-Beginners/tree/main/3-Basic-Tools-and-Getting-Started/3.5-Pytorch):
...
Instalar el entorno LeRobot con pip puede reemplazar las versiones originales de PyTorch y Torchvision por compilaciones para CPU, así que comprueba el resultado en Python.
...
:::tip
Si estás usando un dispositivo Jetson, instala PyTorch y Torchvision con soporte de GPU según [este tutorial](https://github.com/Seeed-Projects/reComputer-Jetson-for-Beginners/blob/main/3-Basic-Tools-and-Getting-Started/3.3-Pytorch-and-Tensorflow/README.md#installing-pytorch-on-recomputer-nvidia-jetson).
...
Si usas Jetson (Jetpack 6.x), utiliza las siguientes instrucciones para encontrar el número de puerto CAN correspondiente a tu Jetson.
...
El número de puerto para todos los comandos posteriores del seguidor debe coincidir con la salida aquí.

Si a tu Jetson le falta el controlador PCAN, la comunicación fallará constantemente. Consulta la guía de instalación del controlador PCAN para Jetson en la sección [Quick Start](https://wiki.seeedstudio.com/es/rebot_b601_rs_getting_started/#3pcan-usb).
...
- El reBot 102 leader utiliza un módulo USB-a-UART, normalmente asignado a `/dev/ttyUSB*`
...
**1. Cambiar a la rama de la cámara**
...
<summary> Si usas una cámara genérica </summary>

Para instanciar una cámara, necesitas un identificador de cámara. Este identificador puede cambiar si reinicias tu ordenador o vuelves a conectar tu cámara; este comportamiento depende principalmente de tu sistema operativo.
...
La terminal imprimirá información relevante de las cámaras.
...
Si tienes más cámaras, puedes cambiar el parámetro `--robot.cameras` para añadirlas. Debes prestar atención al formato de `index_or_path`, que está determinado por el último dígito del ID de la cámara mostrado por `python -m lerobot.find_cameras opencv`.
...
:::tip
Si las pulsaciones de tu teclado no responden, es posible que necesites hacer un downgrade de tu versión de pynput, por ejemplo instalando la versión 1.6.8.
...
Si estás usando una GPU RTX 50 series, necesitas añadir `--dataset.video_backend=pyav` para evitar APIs faltantes en la versión preliminar de torchvision. El comando de entrenamiento se convierte en:
...
1. El parámetro `--policy.path` indica la ruta al archivo de pesos de los resultados de tu entrenamiento de política (por ejemplo, `outputs/train/act_rebot_test/checkpoints/last/pretrained_model`). Si subes el archivo de pesos del resultado del entrenamiento del modelo a Hub, también puedes usar el repositorio del modelo (por ejemplo, `${HF_USER}/act_rebot_test`).
...
SmolVLA es un **modelo base ligero para robótica** proporcionado por Hugging Face. Está diseñado para permitirte tomar tu propio conjunto de datos LeRobot grabado y **ajustarlo rápidamente (fine-tuning)** para obtener resultados en robots reales.
...
SmolVLA es un "modelo base". Para que funcione bien en tu mesa, con tus cámaras, pinza y objetos, normalmente necesitas hacer fine-tuning con tus propios datos.
...
Usa `smolvla_base` (el modelo preentrenado de 450M) como punto de partida y haz fine-tuning en tu conjunto de datos. El ejemplo oficial entrena durante 20k pasos; en una sola A100 esto lleva unas 4 horas (solo como referencia; el tiempo real varía según el hardware).
...
La fase de evaluación carga tu modelo ajustado, permite que el robot ejecute la tarea y registra el proceso de evaluación como un nuevo conjunto de datos (para revisar vídeos y analizar resultados).
...
- `--robot.cameras`: Cámbialo a tu cámara real `index_or_path`, y asegúrate de que las claves de cámara (por ejemplo, `front`, `side`) coincidan exactamente con lo que usaste al grabar el conjunto de datos.
...
Opcional: Si quieres "teleoperar manualmente para ajustar" entre episodios de evaluación, puedes añadir teleoperación (rellena según tu dispositivo y configuración):
...
Parámetros comunes (solo específicos de Pi0 / los que se ajustan con más frecuencia):

- `--policy.pretrained_path=lerobot/pi0_base`: Modelo base. Oficialmente también se proporciona `lerobot/pi0_libero` (versión orientada al conjunto de datos Libero), puedes probar cambiarlo según tu tarea.
...
π₀.₅ (Pi0.5) también es un modelo de **Visión-Lenguaje-Acción** propuesto por Physical Intelligence, que puede entenderse como una "versión mejorada" de π₀, centrada en una capacidad reforzada de **generalización en mundo abierto**: no solo rinde bien en escenarios fijos vistos durante el entrenamiento, sino que también completa tareas de forma más fiable en habitaciones nuevas, con objetos nuevos y en nuevas disposiciones.
...
GR00T N1.5 es un modelo base abierto proporcionado por NVIDIA para un razonamiento robótico más general y aprendizaje de habilidades. Es un modelo de **cuerpos cruzados (cross-embodiment)**: puede recibir entradas multimodales como **lenguaje** e **imágenes**, y ejecutar tareas de manipulación en diferentes entornos.

La clave para usarlo en LeRobot es establecer el tipo de política a `--policy.type=groot`. Nota: GR00T N1.5 actualmente tiene requisitos de entorno más altos (depende de FlashAttention y requiere una GPU CUDA). Se recomienda hacer funcionar primero ACT / Pi0 antes de probar GR00T.
...
:::tip
Si la instalación de flash-attn falla, normalmente está relacionada con (1) incompatibilidad de versiones entre PyTorch/CUDA, (2) dependencias de compilación faltantes o (3) un entorno demasiado nuevo/demasiado antiguo. En este caso, consulta primero la documentación oficial de GR00T y la guía de instalación de PyTorch.
:::

**Entrenamiento (Fine-tuning)**

Oficialmente se proporciona un ejemplo de entrenamiento multi-GPU (`accelerate launch --multi_gpu ...`). Si solo tienes una GPU, también puedes intentar ejecutarlo primero en modo de un solo proceso (el soporte/detalle de parámetros está sujeto a la documentación oficial).
...
Explicación de parámetros clave de accelerate:
...
| -- | -- |
| fp16 | Soportado por casi todas las GPUs NVIDIA |
...
> **puedes saltarte esta sección y usar el Método uno (argumentos de línea de comandos).**
...
LeRobot no ajusta automáticamente la tasa de aprendizaje ni los pasos de entrenamiento en función del número de GPUs, para evitar cambiar el comportamiento de entrenamiento sin conocimiento del usuario. Esto difiere de otros frameworks de entrenamiento distribuido de uso común.

Si quieres **ajustar hiperparámetros para entrenamiento multi-GPU**, debes hacerlo manualmente siguiendo estos pasos.

**Impacto en los pasos y estrategia de ajuste**

Dado que el multi-GPU aumenta el tamaño de lote efectivo (batch_size × num_gpus):

(Para entenderlo de forma intuitiva: si el entrenamiento es como caminar, una GPU da un paso de un metro, dos GPUs dan un paso de dos metros. Para alcanzar la misma distancia (datos totales aprendidos por el modelo), el entrenamiento con dos GPUs debería reducir los pasos a la mitad. De forma similar, n GPUs = 1/n.)
...
Entrenamiento con dos GPUs (el tamaño de lote efectivo pasa a 16):
...
Si quieres mantener una "velocidad de aprendizaje" del modelo similar a la de una sola GPU,
normalmente necesitas aumentar la tasa de aprendizaje proporcionalmente al número de GPUs.
...
al usar 2 GPUs, puedes cambiarla a 2e-4:
...
Para configuraciones más avanzadas y resolución de problemas, consulta la documentación de [Accelerate](https://huggingface.co/docs/accelerate/index). Si quieres aprender más sobre entrenamiento con un gran número de GPUs, revisa esta excelente guía: [Ultrascale Playbook](https://github.com/huggingface/ultrascale-playbook).
</details>

<details>
<summary>(Opcional) Uso de inferencia asíncrona para el despliegue</summary>

Sin inferencia asíncrona, el flujo de control de LeRobot puede entenderse como una inferencia secuencial/sincrónica convencional: la política predice un fragmento de acción, luego lo ejecuta y después espera la siguiente predicción. Para modelos más grandes, esto puede causar pausas notables mientras el robot espera nuevos fragmentos de acción. El objetivo de la inferencia asíncrona es permitir que el robot ejecute el fragmento de acción actual mientras precomputa el siguiente, reduciendo el tiempo de inactividad y mejorando la capacidad de respuesta. La inferencia asíncrona se aplica a las políticas compatibles con LeRobot que generan fragmentos de acción, como ACT, OpenVLA, Pi0, SmolVLA. Dado que la inferencia y el control real están desacoplados, la inferencia asíncrona también permite usar máquinas más potentes para la inferencia del robot de forma remota.

Puedes leer más sobre la inferencia asíncrona en esta [entrada de blog](https://huggingface.co/blog/async-robot-inference) de Hugging Face.

Presentemos algunos conceptos básicos:

- **Cliente**: Se conecta al brazo robótico y a las cámaras, recopila observaciones (imágenes, pose del robot, etc.), las envía al servidor; también recibe fragmentos de acción del servidor y los ejecuta en orden.
- **Servidor**: El dispositivo que proporciona la capacidad de cómputo. Recibe datos de la cámara y del robot, infiere (calcula) fragmentos de acción y los envía de vuelta al cliente. Puede ser el mismo dispositivo conectado al robot y a las cámaras, otro ordenador en la misma LAN o un servidor en la nube.
- **Fragmento de acción**: Una serie de comandos de acción del brazo robótico, producidos por la política mediante inferencia en el servidor.
- **Inferencia sincrónica**: Predecir un fragmento, ejecutar un fragmento; el robot tendrá intervalos de inactividad esperando a que se infiera el siguiente fragmento. Cuando el modelo es más grande y el cómputo es insuficiente, la brecha de inferencia es significativa: el brazo se mueve, luego se detiene (inferencia) y luego vuelve a moverse.
- **Inferencia asíncrona**: A diferencia de la inferencia sincrónica, mientras el robot ejecuta el fragmento actual, el servidor ya está calculando el siguiente fragmento; las partes superpuestas se agregan para un control más receptivo.
...
Este enfoque puede aprovechar GPUs más potentes en hosts en la nube. Con buenas condiciones de red, el tiempo de ida y vuelta (latencia de red) puede ser relativamente pequeño en comparación con el tiempo de inferencia, pero esto depende de tu entorno de red real.

> Nota de seguridad: El flujo de inferencia asíncrona de LeRobot tiene riesgos de gRPC sin autenticación y deserialización con pickle. Si el servidor aloja información o servicios importantes, no se recomienda exponer el servicio directamente a Internet pública al desplegar en una red pública. Un enfoque más seguro es usar VPN, túneles SSH o, al menos, restringir las IP de origen del grupo de seguridad a la IP pública de tu cliente.

**Inicio del despliegue con inferencia asíncrona**

**Paso 1: Configuración del entorno**

Primero, instala las dependencias adicionales necesarias para la inferencia asíncrona usando pip. Tanto el cliente como el servidor necesitan tener lerobot instalado con la dependencia extra:
...
**2. Permitir puertos en el cortafuegos / grupos de seguridad**
...
- Despliegue en LAN / en la nube: Se recomienda probar si el cliente puede acceder al puerto del servidor:
...
- `--actions_per_chunk`: Tamaño de cada fragmento de acción (número de acciones). Valores más grandes significan menor frecuencia de inferencia pero resultados por inferencia más estables; valores más pequeños significan un movimiento más suave pero más presión de inferencia en el servidor.
- `--chunk_size_threshold`: Umbral de mezcla entre fragmentos de acción antiguos y nuevos. Cuando el fragmento antiguo se ha ejecutado hasta esta proporción, comienza la mezcla con el nuevo fragmento.
...
- `--actions_per_chunk`: Tamaño de cada fragmento de acción. Si el movimiento del robot es entrecortado/brusco, aumenta este valor; si la respuesta del robot tiene un retraso notable, disminuye este valor.
- `--chunk_size_threshold`: Umbral de mezcla entre fragmentos de acción antiguos y nuevos. Normalmente se empieza probando desde `0.5`.

La inferencia asíncrona necesita equilibrar que la velocidad de generación de fragmentos de acción del servidor sea mayor o igual que la velocidad de consumo del cliente. De lo contrario, la cola de acciones se vaciará y el robot empezará a tartamudear (esto puede verse en la curva de visualización de la cola tocando fondo).
...
- Entrenar ACT con 50 conjuntos de datos lleva aproximadamente 6 horas en un portátil con una RTX 3060 (8 GB), y alrededor de 2-3 horas en ordenadores con GPUs RTX 4090 o A100.
...
- Para el comando de recopilación de datos, asegúrate de que el parámetro `num-episodes` esté configurado para recopilar suficientes datos. No pauses manualmente a mitad de camino, ya que la media y la varianza de los datos solo se calculan después de que se completa la recopilación de datos, las cuales son necesarias para el entrenamiento.
