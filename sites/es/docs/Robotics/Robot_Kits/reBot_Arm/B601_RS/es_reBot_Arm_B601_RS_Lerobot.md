...
Instalar el entorno de LeRobot con pip puede reemplazar el PyTorch y Torchvision originales por compilaciones para CPU, así que comprueba el resultado en Python.
...
:::tip
Si no puedes conectarte al seguidor, ve a la wiki de introducción y prueba a usar la interfaz proporcionada por motorbridge para comprobar si el brazo robótico funciona correctamente.
:::
...
Si tu Jetson no tiene el controlador PCAN, la comunicación fallará constantemente. Consulta la guía de instalación del controlador PCAN para Jetson en la sección [Quick Start](https://wiki.seeedstudio.com/es/rebot_b601_rs_getting_started/#3pcan-usb).
...
La RealSense D435i combina percepción de profundidad, imagen RGB y una IMU, lo que la hace adecuada para aplicaciones de rango medio y cercano como reconstrucción 3D, SLAM y percepción del entorno robótico.
...
Usarlo en LeRobot es sencillo: solo tienes que establecer el tipo de política en `--policy.type=pi0` durante el entrenamiento (no es necesario repetir los conceptos generales de entrenamiento/evaluación cubiertos en la sección ACT).
...
GR00T N1.5 es un modelo base abierto proporcionado por NVIDIA para un razonamiento robótico más general y aprendizaje de habilidades. Es un modelo de **cuerpo cruzado**: puede tomar entradas multimodales como **lenguaje** e **imágenes**, y ejecutar tareas de manipulación en diferentes entornos.

La clave para usarlo en LeRobot es establecer el tipo de política en `--policy.type=groot`. Nota: GR00T N1.5 actualmente tiene mayores requisitos de entorno (depende de FlashAttention y requiere una GPU CUDA). Se recomienda hacer que ACT / Pi0 funcionen primero antes de probar GR00T.
...
| bf16 | Solo compatible con GPUs más nuevas (arquitectura Ampere y posteriores) |
...
- Número de procesos: Número de GPU a usar (normalmente igual al número de GPU)
- IDs de GPU a usar: Pulsa Enter directamente (significa usar todas las GPU)
- Precisión mixta:
  - Preferir fp16
...
LeRobot no ajusta automáticamente la tasa de aprendizaje ni los pasos de entrenamiento en función del número de GPU, para evitar cambiar el comportamiento de entrenamiento sin que el usuario lo sepa. Esto difiere de otros frameworks de entrenamiento distribuido de uso común.
...
Dado que el uso de varias GPU incrementa el tamaño de lote efectivo (batch_size × num_gpus):

(Para entenderlo de forma intuitiva: si el entrenamiento es como caminar, una GPU da un paso de un metro, dos GPU dan un paso de dos metros. Para alcanzar la misma distancia (datos totales aprendidos por el modelo), el entrenamiento con dos GPU debería reducir a la mitad los pasos. De forma similar, n GPU = 1/n.)

Por lo tanto, al entrenar con varias GPU, deberías reducir apropiadamente el número de pasos de entrenamiento.
...
Entrenamiento con dos GPU (el tamaño de lote efectivo pasa a 16):
...
Al usar varias GPU, cada actualización de paso utiliza más muestras.
...
> Nota de seguridad: El flujo de inferencia asíncrona de LeRobot tiene riesgos de gRPC sin autenticación + deserialización con pickle. Si el servidor aloja información o servicios importantes, no se recomienda exponer el servicio directamente a internet pública al desplegar en una red pública. Un enfoque más seguro es usar VPN, túnel SSH o al menos restringir las IP de origen del grupo de seguridad a la IP pública de tu cliente.
...
- `--visualize_action_queue`: Si visualizar o no el tamaño de la cola de acciones en tiempo de ejecución. Cuando está activado, puedes ver de forma más intuitiva si la cola llega con frecuencia al fondo, lo que te ayuda a ajustar actions_per_chunk y chunk_size_threshold.
...
Seeed Studio English Wiki: [Cómo usar el brazo robótico SO100Arm en Lerobot](https://wiki.seeedstudio.com/es/lerobot_so100m/)
