...
:::warning Guía solo para expertos
Esta guía explica cómo volver a grabar la imagen ReachyMiniOS en el CM4 de Reachy Mini. Hacer esto realizará un _restablecimiento de fábrica_ de tu Reachy Mini.
...
:::tip
El archivo `.bmap` es usado por `bmaptool` (Linux/macOS). Si estás grabando con Raspberry Pi Imager (Windows), solo necesitas el archivo de la imagen del sistema operativo.
:::
...
**Paso 4:** Conecta el cable USB (el que se muestra en la imagen de arriba, llamado **USB2**).

**Paso 5:** Enciende el robot.

La eMMC interna ahora debería aparecer como un dispositivo de almacenamiento masivo.
...
**Desmonta** todo el disco (no particiones individuales):
```bash
...
:::note
`unmountDisk` desmonta todos los volúmenes del disco (`bootfs`, `rootfs`...) de una vez.
:::
...
3. Selecciona `Use custom` para el sistema operativo y proporciona la imagen del sistema operativo descargada (`.zip`, o el `.img` extraído)
...
**Paso 2:** Mueve el interruptor de nuevo a DEBUG
