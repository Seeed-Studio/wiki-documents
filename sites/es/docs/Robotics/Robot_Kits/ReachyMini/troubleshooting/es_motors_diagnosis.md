---
description: Guía completa de diagnóstico y resolución de problemas de motores para Reachy Mini usando la app Testbench para detectar y solucionar fallos de motor.
...
Para ayudar a diagnosticar problemas de motor, hemos desarrollado la [app Reachy Mini Testbench](https://huggingface.co/spaces/pollen-robotics/reachy_mini_testbench). Esta app te permite probar motores individuales, comprobar su estado e identificar posibles problemas.

Necesitas instalar esta app usando Reachy Mini Control antes de utilizarla (consulta la [guía de instalación](https://huggingface.co/docs/reachy_mini/platforms/reachy_mini/usage#2-installation)).
...
- Motores parpadeando en rojo y sin responder/moverse.
- Sin error, pero los motores no responden a las órdenes.
- Motores mostrando errores como "Overload Error"
- Motores ausentes: "No motor found on port", "Missing motor", etc.

### Diagnóstico
- Enciende el robot.
- Abre la app Reachy Mini Testbench.
...
=> Si algo no es correcto, el daemon volverá a programar todos los motores cuando lo reinicies.
Si todo es correcto pero sigues teniendo problemas, consulta las verificaciones siguientes.
...
En la mayoría de los casos reportados, el problema afecta al motor número 4 o a uno con etiqueta de QC n.º 2544.
...
Primero, intenta actualizar tu robot a la última versión de software y luego reinícialo. Esto volverá a programar tus motores.
...
Si estás 100% seguro de que la conexión está bien, también puede ser un motor que no se haya programado correctamente. Consulta la sección **Problema de programación** más abajo.
...
- Si **faltan dos motores** pero la conexión está bien, entonces puede que tengas dos motores con el mismo ID en tu kit. Tres es muy poco probable. Consulta la sección "Problema de programación" más abajo.
...
Si uno o más motores no se detectan, puede deberse a un problema de programación que afecte a los IDs de los motores o al baudrate.

Para resolverlo, necesitarás:
1. Haz clic en el botón "Scan all baudrates" en la app Testbench.
...
2. Si hay un motor con un baudrate o/y ID incorrecto, tendrás que volver a programarlo con los parámetros correctos.
Para hacerlo, sigue el procedimiento de programación que se indica a continuación:
- En la sección de reprogramación de motor, elige el motor con el ID o baudrate incorrecto.
...
3. Si tienes varios motores ausentes y estás seguro de que las conexiones están bien, tendrás que volver a programar todos los motores defectuosos/ausentes uno por uno, siguiendo el mismo procedimiento anterior pero desconectando todos los demás motores defectuosos para evitar conflictos de ID.