...
# Teleoperación SO-ARM101 por cable e inalámbrica con XIAO ESP32-C3 y micro-ROS
...
Este tutorial documenta dos formas compatibles de controlar un SO-ARM101 seguidor desde un SO-ARM101 líder:

1. **Líder por cable + seguidor inalámbrico:** el líder usa una placa estándar controladora de servos de bus por USB.
2. **Líder inalámbrico + seguidor inalámbrico:** cada brazo usa un Seeed Studio XIAO ESP32-C3 Bus Servo Adapter.

En ambos modos, el seguidor se comunica con ROS 2 mediante micro-ROS sobre Wi-Fi UDP. El XIAO del líder inalámbrico lee las seis posiciones de los actuadores del líder y publica el estado de solo lectura; nunca escribe comandos de posición al brazo líder.
...
- descubrimiento del Agent en tiempo de ejecución a partir de la dirección Wi-Fi actual del ordenador, de modo que un cambio de dirección del hotspot no requiera volver a flashear;
- un modo de reserva con líder por cable para diagnóstico y operación cuando el líder inalámbrico no está disponible;
...
Este proyecto se desarrolló como un prototipo probado en tierra para una futura demostración de SO-ARM101 montado en dron. Esta guía solo cubre la comunicación del brazo robótico y el enlace de teleoperación. No proporciona un sistema de control o seguridad certificado para vuelo.
...
- 1 × placa estándar controladora de servos de bus por USB para la calibración del líder y modo de reserva por cable
...
Sigue la [guía SO-ARM100/101 LeRobot](https://wiki.seeedstudio.com/es/lerobot_so100m_new/) para instalar LeRobot y configurar los IDs de los servos antes de continuar.
...
Calibra primero el seguidor con un controlador estándar de servos de bus por USB. Sustituye `/dev/ttyACM0` por el puerto correcto:
...
Conecta un XIAO al ordenador por USB cada vez. Ejecuta las pruebas nativas y la compilación para ESP32-C3 antes de subir:
...
El lanzador inicia el Agent y el servicio de descubrimiento, y luego comprueba los topics ROS 2 requeridos. Cuando los XIAO descubren el Agent, sus monitores serie deberían informar:
...
## Paso 6: Ejecutar teleoperación por cable o doble inalámbrica

Elige una de las dos entradas del líder. Para el modo por cable, conecta el líder al ordenador a través de su controlador USB bus-servo normal. Para el modo inalámbrico, alimenta el líder con su fuente de alimentación externa y deja su XIAO conectado a la red Wi‑Fi configurada.
...
3. **Protocolo de inicio de la pose actual:** el primer comando debe estar dentro de `0.05 rad` de la pose medida.
...
- Comprueba si el hotspot habilita el aislamiento de clientes.
...
La EEPROM del servo no coincide con los valores compilados en `servo_bus.cpp`. Vuelve a conectar el seguidor a través de la placa controladora USB, recalíbralo, actualiza los tres arrays de firmware y vuelve a flashear el XIAO.
...
Recalibra ambos brazos y confirma el orden de las articulaciones en ambos archivos JSON. Verifica también que los arrays del seguidor en `servo_bus.cpp` provengan del mismo seguidor físico que está actualmente conectado al XIAO.

## Alcance de la validación

El proyecto se ha probado en la configuración de referencia con:

- los seis servos del seguidor fueron detectados (`servo_mask=0x3f`);
...
La ruta doble inalámbrica y la alternativa por cable están ambas soportadas por el lanzador actual. Se completó una ejecución de resistencia inalámbrica de referencia el 2026-09-20 y duró 783 segundos (unos 13 minutos). La ejecución registró cero timeouts de comando, cero rechazos de comando y cero recuperaciones del líder; la retroalimentación promedió 19.87 Hz, la latencia de acuse de recibo de comandos fue de 66.0 ms en P50, y el RSSI mínimo observado fue de -65 dBm. Estas cifras describen una ejecución de referencia, no una garantía para cada entorno Wi‑Fi.
