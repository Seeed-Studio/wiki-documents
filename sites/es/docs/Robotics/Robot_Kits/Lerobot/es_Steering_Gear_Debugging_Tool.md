...
title: Herramienta de depuración de SO-Arm
keywords:
  - Lerobot
  - SO-ARM
  - Robótica
...
La herramienta de depuración de engranajes de dirección (Seeed_RoboController) es una herramienta gráfica de depuración para los brazos robóticos Lerobot/SO-ARM. Con esta herramienta, puedes detectar rápidamente el estado de los servos, recalibrar los IDs de los servos, escribir valores neutros, controlar servos individuales y gestionar archivos de calibración del brazo robótico, lo que facilita el montaje, la depuración y el mantenimiento del brazo líder y del brazo seguidor.
...
Si algunos servos aparecen como no detectados, primero comprueba si el cableado es correcto, cierra la página y vuelve a abrirla. Si siguen sin detectarse, es posible que los IDs de los servos estén duplicados (posiblemente porque el comando para calibrar el brazo robótico se introdujo accidentalmente como el comando para calibrar los servos). En este caso, es necesario recalibrar los IDs de estos servos.
...
### 3. Recalibrar los IDs de los servos

Asegúrate de que solo un servo esté conectado al brazo robótico.

(Al grabar un ID en un servo, dado que los servos comparten un bus serie, si existen IDs duplicados —por ejemplo, cambiar el ID 4 a 6—, todos los servos con ID 4 en el bus se cambiarán a 6. Por lo tanto, todos los servos deben retirarse y reprogramarse uno por uno).
...
2\. Si no lo es, mueve el brazo robótico a la posición neutra y luego haz clic en **Serial Port Neutral Calibration**. El brazo robótico escribirá la posición actual como valor neutro en el servo.
