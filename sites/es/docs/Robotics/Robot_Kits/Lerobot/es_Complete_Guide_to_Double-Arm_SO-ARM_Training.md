...
Puedes usar `lerobot-find-cameras opencv` para ver los índices de las cámaras, y puedes añadir o eliminar cámaras según sea necesario.
...
- Debes añadir `--resume=true`; de lo contrario, `LeRobotDataset.create()` informará de un error porque el directorio ya existe.
...
Después de la eliminación, el conjunto de datos se reescribirá en su lugar y los datos originales se respaldarán en `~/.cache/huggingface/lerobot/seeed/bimanual_so101_task_old/`. Después de confirmar que el nuevo conjunto de datos es correcto, puedes eliminar manualmente la copia de seguridad:
...
> Lo anterior utiliza los parámetros predeterminados de ACT (`chunk_size=100`, `dim_model=512`, etc.).
