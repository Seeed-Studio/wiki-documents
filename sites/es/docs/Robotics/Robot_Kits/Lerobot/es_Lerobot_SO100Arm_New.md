---
description: Este wiki proporciona el tutorial de montaje y depuración para el SO ARM100 y permite la recopilación de datos y el entrenamiento dentro de la última versión del framework Lerobot.
...
Este wiki proporciona el tutorial de montaje y depuración para el SO ARM10x y permite la recopilación de datos y el entrenamiento dentro del framework Lerobot.
...
3. **Abundantes recursos de aprendizaje**: Proporciona completos recursos de aprendizaje de código abierto como guías de montaje y calibración, y tutoriales para pruebas, recopilación de datos, entrenamiento y despliegue para ayudar a los usuarios a comenzar rápidamente y desarrollar aplicaciones robóticas.
...
## Tabla de contenidos
  ...
  [D. Montaje](https://wiki.seeedstudio.com/es/lerobot_so100m_new/#Montaje)
...
Según se indica, conecta el motor de la pinza. Asegúrate de que sea el único motor conectado a la placa y de que el propio motor aún no esté conectado en cadena con ningún otro motor. Al pulsar [Enter], el script establecerá automáticamente el id y la velocidad en baudios de ese motor.
...
Cuando hayas terminado, el script simplemente finalizará, momento en el cual los motores estarán listos para usarse. Ahora puedes conectar el cable de 3 pines de cada motor al siguiente, y el cable del primer motor (el “giro de hombro” con id=1) a la placa controladora, que ahora se puede fijar a la base del brazo.
...
Elimina por completo los archivos en ~/.cache/huggingface/lerobot/calibration/robots o ~/.cache/huggingface/lerobot/calibration/teleoperators antes de recalibrar. De lo contrario, el sistema puede generar un mensaje de error, ya que los datos de calibración anteriores se almacenan en archivos JSON dentro de estos directorios.
...
Ejecuta directamente el comando de calibración en la terminal. Si el brazo ya se ha calibrado antes, aparecerá el siguiente mensaje
    ...
    "Pulsa ENTER para usar el archivo de calibración proporcionado asociado con el id my_awesome_leader_arm, o escribe 'c' y pulsa ENTER para ejecutar la calibración:"
...
:::tip
En PC (Linux) y dispositivos Jetson, el primer dispositivo USB que conectas normalmente se asigna a `ttyACM0`, y el segundo a `ttyACM1`. Verifica dos veces qué puerto está asignado al líder y al seguidor antes de ejecutar comandos.
:::
...
Si encuentras el error “Could not connect on port '/dev/ttyACM0'. Make sure you are using the correct port., Try running lerobot-find-port” mientras calibras los brazos Leader o Follower, necesitas otorgar los permisos necesarios ejecutando:`sudo chmod 666 /dev/ttyACM*`
:::
...
Esto normalmente significa que la posición actual / desplazamiento de cero de un servo es anormal, lo que hace que el ángulo leído exceda el rango esperado. En ese caso, puedes usar la herramienta SoARM de Seeed Studio para hacer una **calibración de posición media** (escribir la posición actual en el valor medio **2048**), y luego repetir la calibración de todo el brazo.
...
#### 2) Calibración y verificación de la posición media

Ubicación de los scripts:

- `src/tools/servo_middle_calibration.py`: calibración de posición media (escribir la posición actual como **2048**)
...
Ten en cuenta que el id asociado con un robot se usa para almacenar el archivo de calibración. Es importante usar el mismo id al teleoperar, registrar y evaluar cuando uses la misma configuración.
...
El comando de teleoperación hará automáticamente:
...
<details>
<summary> Si usas RealSense D435i/D405 </summary>
...
Introduce el `Serial number` obtenido en el parámetro `serial_number_or_name` del comando de cámara a continuación.
...
<details>
<summary> Si usas cámaras Orbbec Gemini2/Gemini336 </summary>
...
Introduce el `Serial Number` obtenido en el parámetro `serial_number_or_name` del comando de la cámara que se muestra a continuación.
...
Prueba de una sola cámara Orbbec + prueba de cámara estándar:
...
:::warning
Cuando utilices cámaras Intel RealSense en macOS, podrías obtener este error: OSError finding RealSense cameras: failed to set power state, esto se puede resolver ejecutando el mismo comando con permisos sudo. Ten en cuenta que usar cámaras RealSense en macOS es inestable.
:::
...
Si subiste tu conjunto de datos al hub con `--control.push_to_hub=true`, puedes [visualizar tu conjunto de datos en línea](https://huggingface.co/spaces/lerobot/visualize_dataset) copiando y pegando tu repo id obtenido mediante:
...
- **Tipo de política**: Proporcionamos la política con `policy.type=act`. Del mismo modo, puedes cambiar entre políticas como [`act`, `diffusion`, `pi0`, `pi0fast`, `pi0fast`, `sac`, `smolvla`]., lo que cargará la configuración desde `configuration_act.py`. Es importante destacar que esta política se adaptará automáticamente a los estados de los motores de tu robot (por ejemplo, `laptop` y `phone`), a las acciones de los motores y al número de cámaras, ya que esta información ya está almacenada en tu conjunto de datos.
...
2. El nombre del conjunto de datos `dataset.repo_id` comienza con `eval_`. Esta operación registrará por separado videos y datos durante la evaluación, que se guardarán en la carpeta que comienza con `eval_`, como `seeed/eval_test123`.
...
[LIBERO](https://huggingface.co/docs/lerobot/libero) es un benchmark diseñado para estudiar el aprendizaje robótico de por vida. La idea es que los robots no solo se preentrenarán una vez en una fábrica, sino que necesitarán seguir aprendiendo y adaptándose con sus usuarios humanos a lo largo del tiempo. Esta adaptación continua se llama aprendizaje de por vida en la toma de decisiones (LLDM), y es un paso clave hacia la construcción de robots que se conviertan en ayudantes verdaderamente personalizados.
...
LIBERO es ahora parte de nuestra **simulación con evaluación múltiple**, lo que significa que puedes evaluar tus políticas ya sea en un **único conjunto de tareas** o en **múltiples conjuntos a la vez** con solo una bandera.
...
Evalúa una política en varios conjuntos a la vez:
...
GR00T N1.5 es un modelo base abierto de NVIDIA para un razonamiento robótico más general y el aprendizaje de habilidades. Es un modelo de **cruzamiento de cuerpos**: puede tomar entradas multimodales como **lenguaje** e **imágenes**, y ejecutar tareas de manipulación en diferentes entornos.

En LeRobot, la clave es establecer el tipo de política en `--policy.type=groot`. Ten en cuenta que GR00T N1.5 tiene requisitos de entorno más altos (depende de FlashAttention y requiere una GPU CUDA). Se recomienda primero hacer que ACT / Pi0 funcionen de extremo a extremo y luego probar GR00T.
...
1. Primero prepara tu entorno base (Python, CUDA, controladores, etc.). **No** instales `lerobot` todavía.
...
Si estás utilizando una GPU RTX 50 Serie, se deben cumplir los siguientes requisitos: Python=3.10, CUDA=12.8, Torch=2.7.1
...
Si estás utilizando una GPU RTX 50 Serie, se debe cumplir el siguiente requisito: flash_attn=2.8.0
...
La documentación oficial proporciona un ejemplo multi-GPU con `accelerate launch --multi_gpu ...`. Si solo tienes una única GPU, aún puedes empezar haciendo que funcione primero una ejecución de un solo proceso (la compatibilidad/argumentos exactos dependen de la documentación oficial).
...
Las tasas de aprendizaje de LoRA suelen ser ~10× más altas que en el ajuste fino completo. Por ejemplo, si el ajuste fino completo usa comúnmente `1e-4`, LoRA puede empezar desde `1e-3`. Si utilizas un programador de tasa de aprendizaje, la tasa de aprendizaje final suele estar alrededor de `1e-4` como referencia.
...
Si solo usas multi-GPU ocasionalmente (o es tu primera vez), omitir esto está completamente bien.
...
La inferencia asíncrona es aplicable a las políticas compatibles con LeRobot, incluidas las **políticas de acción basadas en bloques** como **ACT, OpenVLA, Pi0 y SmolVLA**.
...
2. **Abrir el puerto en el cortafuegos / grupo de seguridad**
...
Necesitas confirmar y recordar la dirección IP de la LAN del lado del servidor. Cuando el cliente se conecte, lo que debe rellenarse es la IP de la LAN de la máquina que ejecuta policy_server, no la IP del propio cliente.
...
Busca un campo como IPv4 Address . . . . . . . . . . . : 192.168.14.140; esa es la dirección IP de la LAN de esa máquina.
...
En este caso, cuando el cliente se conecte, el --server_address debe ser la dirección IP de la LAN del servidor, es decir,`<LAN IP address>:8080`.
...
En este caso, cuando el cliente se conecte, el --server_address debe ser la dirección IP pública del servidor, es decir, `<server public IP>:8080`.
...
Parámetros de los dispositivos de hardware. Deben mantenerse coherentes con los parámetros utilizados durante la recopilación del conjunto de datos.

- `--task`

La descripción de la tarea. Las políticas de visión y lenguaje como SmolVLA pueden determinar el objetivo de la acción en función del texto de la tarea.
...
Se puede entender como: cuando la proporción restante de la cola de acciones actual cae por debajo de este umbral, el cliente enviará una nueva observación por adelantado y solicitará el siguiente fragmento de acción.

Configurarla en 0.5 aquí significa:

cuando el fragmento de acción actual está aproximadamente a la mitad consumido

el cliente comienza a solicitar el siguiente fragmento de acción
...
Especifica el método de agregación para los intervalos de acción superpuestos.

En la inferencia asíncrona, cuando el fragmento de acción antiguo aún no se ha ejecutado por completo, el nuevo fragmento de acción puede haber llegado ya.

En ese caso, los dos fragmentos se superponen en parte del intervalo de tiempo, y se necesita una función de agregación para combinarlos en la acción final ejecutada.

El significado de weighted_average es:

usar un promedio ponderado para fusionar la parte superpuesta.

Esto generalmente hace que el cambio de acción sea más suave y reduce los cambios bruscos.

- `--debug_visualize_queue_size=True`

Indica si se debe visualizar el tamaño de la cola de acciones en tiempo de ejecución.

Cuando está activado, te permite ver de forma más directa si la cola toca con frecuencia el fondo, lo que te ayuda a ajustar actions_per_chunk y chunk_size_threshold.
...
actions_per_chunk 50 Cuántas acciones produce la política en una sola vez. Valores típicos: 10–50.

chunk_size_threshold 0.5 Cuando la proporción restante de la cola de acciones es ≤ chunk_size_threshold, el cliente envía una nueva solicitud de fragmento de acción. El rango de valores es [0, 1].
...
Lo que la inferencia asíncrona necesita equilibrar es: la velocidad a la que el servidor genera fragmentos de acción debe ser mayor o igual que la velocidad a la que el cliente consume fragmentos de acción. De lo contrario, la cola de acciones se vaciará y el robot comenzará a tartamudear de nuevo (esto se puede ver como la curva tocando el fondo en la visualización de la cola).

La velocidad a la que el servidor genera fragmentos de acción se ve afectada por factores como el tamaño del modelo, el tipo de dispositivo, la VRAM / memoria y la potencia de cálculo de la GPU.

La velocidad a la que el cliente consume fragmentos de acción se ve afectada por los fps de ejecución configurados.
...
- Entrenar ACT en 50 conjuntos de datos lleva aproximadamente 6 horas en un portátil con una RTX 3060 (8GB), y alrededor de 2-3 horas en ordenadores con GPUs RTX 4090 o A100.
