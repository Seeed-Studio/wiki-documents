...
Este paso puede resultar menos amigable para principiantes porque PlatformIO descarga muchas dependencias automáticamente y la instalación puede llevar algo de tiempo. Si aparecen errores durante la instalación, normalmente es mejor esperar con paciencia y solucionar un problema cada vez. Usar herramientas de IA para ayudar a inspeccionar los mensajes de error también puede ahorrar tiempo.
...
Una vez que la instalación de dependencias se haya completado, ejecuta de nuevo la compilación normal:
...
Si la compilación pasa en este punto, tu firmware de salida se ha generado correctamente.
...
La forma exacta en que lo modifiques depende de tu capacidad para leer el código fuente. Aquí empezamos con un ejemplo muy sencillo: modificar la interfaz de la pantalla de inicio.
...
Esta parte gestiona el área utilizada por los iconos de hora, correo, silencio y otros en el lado derecho. Añadí `headerLabelRight` para limitar el máximo límite derecho del texto central y evitar la superposición con el contenido del lado derecho.
...
Ajustes confirmados en funcionamiento:
...
Utiliza un dispositivo Meshtastic cercano como `CLIENT` en la misma malla. Después de que el nodo remoto comience a enviar telemetría, confirma que la pasarela puede recibir:
...
Cuando la carga termine, PlatformIO debería informar que el flasheo fue correcto:
...
Esto significa que el cambio es intencionadamente de alcance reducido. Personaliza la experiencia de usuario para una placa, mientras preserva el comportamiento original para todos los demás objetivos compatibles.
...
1. Una captura de pantalla de `buzz.cpp` que muestre `playStartMelody()` y `playShutdownMelody()` en el mismo encuadre
...
Si el zumbador no suena durante las pruebas, primero comprueba si el modo de zumbador del dispositivo está habilitado. En `playTones()`, el código retorna de forma anticipada cuando el zumbador está deshabilitado o configurado en modo solo notificaciones.
...
- Si parece quedarse atascada durante demasiado tiempo, intenta instalar primero los paquetes por separado:
