...
La versión P1-Pro tiene batería y módulo GPS integrados; para la versión P1, el usuario debe instalar la batería y el módulo GPS manualmente si es necesario.
:::
...
Cambia la región de LoRa y guarda la configuración. Luego `Reboot` el dispositivo, de lo contrario la configuración no surtirá efecto.
...
Además, puedes ajustar el intervalo de transmisión de anuncios. El intervalo de `auto zero hop advert` es de 60-240 minutos. El intervalo de `auto flood advert` es de 3-168 horas. 
...
Para usuarios de Mac, presiona la tecla "Command" y la tecla "Space", así podrás abrir Spotlight. Luego introduce "termial" y pulsa "Return". Esto puede abrir la línea de comandos. 
...
`adafruit-nrfutil --verbose dfu serial --package xiao_nrf52840_ble_bootloader.zip -p COM6 -b 115200 --singlebank --touch 1200`
 ...
 Algunos dispositivos cambiarán su número de puerto después de introducir este comando. Así que si la instalación falla, vuelve a comprobar el número de puerto.
...
## Recursos
- [Tabla de cálculo de la vida útil de la batería del nodo solar](https://files.seeedstudio.com/wiki/SenseCAP/Meshcore/SolarNode/mesh_repeater_power_table_en1.xlsx)
