...
Cuando la barra de progreso esté completamente llena, indica que la grabación en la Flash se ha completado. Entonces el dispositivo se reiniciará automáticamente.
...
El disco desaparecerá después de que el firmware se haya grabado correctamente. En este punto no hay firmware en el dispositivo, por lo que el dispositivo `NO` se reiniciará automáticamente.
...
- **Inundado** significa que se difunde y luego es repetido por todos los repetidores que lo escuchan.
...
- Paso 3: Pega el archivo del bootloader
  ...
  Sustituye todos los archivos del disco con el archivo del bootloader descargado.
...
[Flash Erase](https://wiki.seeedstudio.com/es/get_started_with_other_mesh_firmware/#borrado-de-flash-1) el firmware
