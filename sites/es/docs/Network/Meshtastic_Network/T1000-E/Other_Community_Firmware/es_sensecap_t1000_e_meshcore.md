---
description: Comienza con SenseCAP Card Tracker T1000-E para Meshcore. Guía para la instalación del dispositivo, flasheo del firmware y conexión del dispositivo.
...
Elige `Companion Bluetooth`. Los dispositivos flasheados con el firmware de compañero Bluetooth pueden conectarse a la APP. Si quieres flashear otro firmware, por favor [haz clic aquí](https://docs.meshcore.io/) para ver el tutorial.
...
Haz clic en `Enter DFU Mode`, elige el puerto serie llamado "nRF xxx" o "TinyUSB". Luego haz clic en `Erase Flash` y elige el puerto serie.
...
Haz clic en `Enter DFU Mode`, elige el puerto serie llamado "nRF xxx" o "TinyUSB". Luego haz clic en `Flash` y elige el puerto serie.
...
### Conectarse mediante la App

Haz clic en `Connect` y elige el dispositivo de destino. El ID de dispositivo por defecto es `Meshcore-MAC Address`. La dirección MAC se puede ver en la parte posterior del dispositivo.
...
Introduce el código PIN por defecto `123456` y luego haz clic en `Pair` para conectar el dispositivo.
...
### Conectarse mediante la interfaz web (BLE)
...
4. Cuando se te solicite, introduce la clave de acceso Bluetooth por defecto `123456` y haz clic en **OK**.
...
Para empezar a comunicarte a través de la malla, debes establecer tu región. Esta configuración controla qué rango de frecuencia usa tu dispositivo y debe ajustarse de acuerdo con tu ubicación regional.
...
Ahora que has configurado la región de LoRa en tu dispositivo, puedes continuar configurando cualquier [parámetro de LoRa](https://github.com/meshcore-dev/MeshCore/blob/main/docs/faq.md#5-general) según tus necesidades.
...
|--|--|
|Presionar durante 3 s|Encendido/apagado|
...
MeshCore te permite difundir manualmente tu nombre, posición y clave pública de cifrado, que también se firma para evitar suplantaciones. Cuando haces clic en el botón de anuncio, difunde esos datos a través de LoRa. MeshCore llama a eso un Anuncio. Hay dos formas de anunciar, "cero saltos" y "inundado".

- **Cero saltos** significa que tu anuncio se difunde a cualquiera que pueda oírlo, y eso es todo.
...
- Cambia el cable de carga
 ...
 - Si aún no tienes suerte, mantén pulsado el botón del dispositivo y luego conecta el cable de carga, comprueba si aparece o no un disco en tu PC. Si es así, [haz clic aquí](https://wiki.seeedstudio.com/es/sensecap_t1000_e_meshcore/#volver-a-instalar-el-cargador-de-inicio) para reinstalar el bootloader.
...
- Paso 1: Intenta entrar en el modo DFU manualmente: mantén pulsado el botón del dispositivo y luego conecta **rápidamente** el cable de carga dos veces, el LED verde permanecerá encendido fijo.
...
Entonces debería aparecer "Python xxx" y "pip xxx". Si no es así, intenta instalar Python de nuevo.
...
Nota: los siguientes comandos usan `python3`, sin embargo, si estás en Windows, es posible que tengas que cambiarlo a `python` ya que la instalación de Windows de python 3.x sigue usando el nombre python.exe
...
En el terminal o símbolo del sistema, navega hasta el directorio donde descargaste el paquete zip del bootloader y ejecuta el siguiente comando, sustituyendo el puerto correcto para tu dispositivo:
...
`adafruit-nrfutil --verbose dfu serial --package t1000_e_bootloader-0.9.1-5-g488711a_s140_7.3.0.zip -p **COM6** -b 115200 --singlebank --touch 1200`
 ...
 Algunos dispositivos cambiarán su número de puerto después de introducir este comando. Por lo tanto, si la instalación falla, comprueba de nuevo el número de puerto.
...
Cuando hayas completado los pasos anteriores, entonces [haz clic aquí](https://wiki.seeedstudio.com/es/sensecap_t1000_e_meshcore/#grabación-del-firmware) para grabar el firmware de la aplicación.

**2) El dispositivo no puede entrar en modo DFU, pero el puerto serie puede detectarse**.
...
- Conecta el dispositivo.
   La luz parpadeará brevemente cuando lo conectes. Sigue intentando esto hasta que la luz permanezca encendida, lo que significa que el dispositivo puede volver al modo DFU, luego graba el [bootloader](https://wiki.seeedstudio.com/es/sensecap_t1000_e_meshcore/#flash-the-bootloader)-> [Borrar flash](https://wiki.seeedstudio.com/es/sensecap_t1000_e_meshcore/#flash-erase) -> [grabar el firmware](https://wiki.seeedstudio.com/es/sensecap_t1000_e_meshcore/#flash-firmware)

**3) El dispositivo no puede entrar en modo DFU y no se muestra ningún puerto serie**

- mantén pulsado el botón del dispositivo y luego conecta el cable de carga. Después de que aparezca un disco en tu PC, puede que veas el puerto serie.
...
- **No se reciben datos en el puerto serie**
 ...
 Comprueba si el dispositivo está en modo DFU, la luz verde permanecerá encendida fija cuando el dispositivo esté en modo DFU.
...
- **No se puede abrir el puerto serie**
 ...
 - Después de encender el dispositivo, se apagará o se reiniciará automáticamente al cabo de un tiempo. 
 - El registro del puerto serie se ejecuta durante un tiempo y luego se detiene.

 Esto puede deberse a que se reinicie o apague el dispositivo de forma manual y forzada cuando el dispositivo se encuentra en los siguientes estados: sin haber terminado el proceso de transmisión de mensajes, estando en configuración......
...
- Realiza un [Flash erase](https://wiki.seeedstudio.com/es/sensecap_t1000_e_meshcore/#flash-erase) del dispositivo. Luego vuelve a grabar el firmware más reciente.
...
- [Prueba de consumo y cálculo de la duración de la batería del T1000-E para Meshtastic](https://files.seeedstudio.com/wiki/SenseCAP/Meshtastic/T1000-E%20for%20Meshtastic%20Consumption%20Test%20and%20Battery%20Life%20Calculation.xlsx)
