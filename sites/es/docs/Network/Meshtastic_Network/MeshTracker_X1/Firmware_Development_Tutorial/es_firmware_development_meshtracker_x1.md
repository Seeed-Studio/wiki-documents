---
description: Un tutorial práctico para configurar el entorno del código fuente del firmware de Meshtastic, compilar el MeshTracker X1 y flashear el firmware.
...
url: https://wiki.seeedstudio.com/es/meshtastic_source_code_practical_tutorial_meshtracker_x1/
...
Esto muestra que, en Meshtastic, **el objetivo de compilación para MeshTracker X1** es `seeed_mesh_tracker_X1`.
...
A partir de estos archivos de configuración, puedes ver que L1 define `HAS_SCREEN` y `USE_SSD1306`. Eso significa que utiliza el flujo estándar de pantalla OLED, no una configuración sin pantalla y no una solución de tinta electrónica.
...
Esta parte gestiona el área utilizada por la hora, correo, silencio y otros iconos en el lado derecho. Añadí `headerLabelRight` para limitar el límite derecho máximo del texto central y evitar que se superponga con el contenido del lado derecho.
...
Después de una compilación correcta, la salida suele ubicarse en:
