---
description: Grove - Pantalla Alfanumérica Dual Roja de 0.54 pulgadas
title: Grove - Pantalla Alfanumérica Dual Roja de 0.54 pulgadas
keywords:
  - Grove
...
url: https://wiki.seeedstudio.com/es/Grove-0.54_inch_Red_Dual_Alphanumeric_Display/
---
...
Este módulo es una pantalla alfanumérica de 2 dígitos con alto brillo y retroiluminación roja, cada dígito consiste en un tubo digital de 14 segmentos. La pantalla de 7 segmentos comúnmente utilizada solo puede mostrar los números 0-9 y un número limitado de letras. En contraste, la pantalla digital de 14 segmentos no tiene esta limitación, puede mostrar todos los caracteres, por eso la llamamos pantalla alfanumérica. La altura del dígito es de 0.54 pulgadas y el LED rojo utilizado en este módulo tiene un brillo súper alto, por lo que puedes verlo a unos metros de distancia.

Usualmente, la pantalla de 14 segmentos necesita usar 17 o más cables para controlar la pantalla, lo que significa que una gran cantidad de IO de tu placa de control principal (como Arduino) será ocupada. Con la ayuda del chip HT16K33 integrado y el conector Grove, lo convertimos en una pantalla i2c, solo cuatro cables son suficientes. Sin soldadura problemática y sin cableado complicado, si estás usando [seeeduino](https://www.seeedstudio.com/seeeduino-boards-c-987.html), es realmente plug and play, muy fácil.

Hemos lanzado muchas [pantallas](https://www.seeedstudio.com/displays-c-929.html) para Arduino, raspberry pi, Beaglebone y otros SBC(Single Board Computer), ahora te traemos el Grove - Pantalla Alfanumérica Dual Roja de 0.54", la pantalla de 14 segmentos significa más posibilidad y creatividad. Esperamos que te ayude a crear proyectos más interesantes.

En caso de que solo necesites una pantalla alfanumérica de cuatro dígitos, puedes revisar el [Grove - Pantalla Alfanumérica Cuádruple Roja de 0.54"](https://www.seeedstudio.com/Grove-0-54-Red-Quad-Alphanumeric-Display-p-4032.html).
...
- Grove I2C, solo toma dos pines IO
...
| Pantalla LED                  | JM-S05422AH-001 |
| Controlador Driver LED        | HT16K33         |
| Color LED                     | Rojo            |
| Brillo ultra-alto             | 30mcd           |
| Altura LED                    | 0.54 pulgadas   |
| Segmento LED                  | 14              |
| Tamaño                        | 40 * 40mm       |
| Interfaz de salida            | I2C             |
| Dirección I2C(Seleccionable)  | 0x70(Por defecto) |
...
| Seeeduino V4.2 | Base Shield |Grove - Pantalla Alfanumérica Dual Roja de 0.54"|
|--------------|-------------|-----------------|
...
|[Obtener UNO Ahora](https://www.seeedstudio.com/Seeeduino-V4.2-p-2517.html)|[Obtener UNO Ahora](https://www.seeedstudio.com/Base-Shield-V2-p-1378.html)|[Obtener UNO Ahora](https://www.seeedstudio.com/Grove-0-54-Red-Dual-Alphanumeric-Display-p-4031.html)|
...
- **Paso 1.** Conecta el Grove - Pantalla Alfanumérica Dual Roja de 0.54" al puerto **I2C** del Base Shield.
...
:::note
 Si no tenemos Grove Base Shield, también podemos conectar directamente Grove - Pantalla Alfanumérica Dual Roja de 0.54" al Seeeduino como se muestra a continuación.
:::

| Seeeduino       | Grove - Pantalla Alfanumérica Dual Roja de 0.54" |
|---------------|-------------------------|
| 5V            | Rojo                     |
| GND           | Negro                   |
| SDA           | Blanco                   |
...
- **Paso 1.** Descarga la [Biblioteca Grove - 0.54" Red Dual Alphanumeric Display](https://github.com/Seeed-Studio/Seeed_Alphanumeric_Display_HT16K33) desde Github.
...
- **Paso 4.** Descomenta el **tube.setTubeType(TYPE_2,TYPE_2_DEFAULT_I2C_ADDR)** como se muestra a continuación, tienes que descomentarlo para todos los otros ejemplos también.
...
- **display_scroll_string:** Muestra cadena de desplazamiento que es más larga que 2.
- **display_scroll_number:** Muestra número de desplazamiento que es mayor que 99.
- **number_increment:** Muestra número incremental.
...
Si usas 2 Grove - 0.54" Red Dual Alphanumeric Displays, por favor modifica el código como se muestra a continuación.
...
- **Init(NUMERIC_I2C_ADDR):** inicializa el dispositivo con dirección I2C, Grove - 0.54" Red Quad Alphanumeric Display I2C la dirección predeterminada es 0x71.
...
- **displayNum(1234567,500):** muestra el número, hay 2 parámetros. El primer parámetro es el número y el segundo es el intervalo de desplazamiento(ms). Si el num &lt;100, los tubos muestran número estático. De lo contrario, muestra número con desplazamiento. El intervalo de desplazamiento(ms) solo funciona en modo de desplazamiento y puedes establecer el intervalo como 0.
- **displayString("ABCDEFGHJIYZ",500):** muestra el carácter, hay 2 parámetros. El primer parámetro es el carácter y el segundo es el intervalo de desplazamiento(ms). Si el carácter tiene menos de 2 caracteres, los tubos muestran carácter estático. De lo contrario, muestra caracteres con desplazamiento. El intervalo de desplazamiento(ms) solo funciona en modo de desplazamiento y puedes establecer el intervalo como 0. La función no admite letras minúsculas y las muestra igual que las letras mayúsculas.
- **setTubeSingleChar(FIRST_TUBE,'t'):** establece el carácter individual @ tubo específico.
- **tube.setPoint(true,false):** establece los 2 puntos encendidos y apagados. Este ejemplo establece el primer punto encendido y el segundo apagado.
...
- **[ZIP]** [Archivo de Esquemático Grove - 0.54" Red Dual Alphanumeric Display](https://files.seeedstudio.com/wiki/Grove-0.54-Red-Dual-Alphanumeric-Display/res/Grove-0.54-Red-Dual-Alphanumeric-Display.zip)
- **[PDF]** [Hoja de Datos del Módulo LED](https://files.seeedstudio.com/wiki/Grove-0.54-Red-Dual-Alphanumeric-Display/res/DIP-LED-2-BETY-14-SEG.pdf)
- **[PDF]** [Hoja de Datos HT16K33](https://files.seeedstudio.com/wiki/Grove-0.54-Red-Dual-Alphanumeric-Display/res/HT16K33.pdf)
- **[Librería]** [Librería Grove - 0.54" Red Dual Alphanumeric Display](https://github.com/Seeed-Studio/Seeed_Alphanumeric_Display_HT16K33)
