...
El GPS funciona bien en exteriores, pero su rendimiento es limitado en interiores. Al desplegar balizas Bluetooth BC03 en entornos interiores, los rastreadores SenseCAP pueden escanear las señales de las balizas y estimar distancias basadas en los valores RSSI. Combinado con algoritmos de posicionamiento, esto permite **una precisión de posicionamiento en interiores a nivel de metros (aproximadamente 2–3 metros)**.
...
Proporcionamos un sistema de posicionamiento Bluetooth en interiores de código abierto diseñado para funcionar perfectamente con balizas SenseCAP BLE y rastreadores SenseCAP LoRaWAN.
...
* **Bluetooth® LE 5.0**
* **Consumo de energía ultrabajo**: batería de litio reemplazable, más de **5 años de vida útil de la batería** (0 dBm / 500 ms, configuración predeterminada)
...
---
:::info
BC03 tiene una antena BLE omnidireccional de alta ganancia, bajo consumo de energía y permite la interacción con el usuario:

* **Capacidad mejorada de transmisión de baliza**
Se ha optimizado la estabilidad de la publicidad BLE y la consistencia del RSSI, mejorando la precisión del posicionamiento en interiores y la robustez en entornos complejos.
* **Diseño de antena**
...
Diseño de bajo consumo aún más optimizado. Bajo la misma configuración (0 dBm / 500 ms), BC03 logra un despliegue a gran escala y a largo plazo.
* **LED de estado añadido**
Un LED azul proporciona una indicación intuitiva del estado del dispositivo durante el encendido y el funcionamiento, mostrando el estado de encendido y actualización del dispositivo.
:::
...
| Dimensiones(L\*W\*H)    | Φ50*20.5mm                 |
| Peso                | ~45 g (incluyendo batería) |
| Batería             | Batería de iones de litio, 2400 mAh|
| Chip                | Serie nRF52                |
| Versión Bluetooth   | BLE 5.0 (compatible hacia atrás con BLE 4.1) |
| Protocolo Bluetooth | iBeacon                    |
| Sensores            | Acelerómetro (opcional)    |
| LED                 | 1 × LED azul               |
| App                 | SenseCraft APP             |
| Temperatura de trabajo | -20 ~ 60 °C             |
| Vida útil de la batería | > 5 años (0 dBm / 500 ms en configuración predeterminada) |
...
| Tx Power             | -40 ~ +4dBm (0dBm predeterminado)   |
| Advertising Interval | 20ms ~ 30s (500ms predeterminado)  |
| Password             | 12345678                       |
| Device Name          | 03                             |
