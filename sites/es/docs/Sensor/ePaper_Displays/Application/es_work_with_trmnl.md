...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      ...
      <td align="center">Panel desnudo 7.5" mono<br/>Directamente con XIAO ESP32-C3</td>
    </tr>
    <tr>
      <td><div class="get_one_now_container" style={{textAlign: 'center'}}>
...
Imprime las piezas de la carcasa de código abierto desde la sección [Resources](#resources) y ensambla los componentes en su interior.
...
El panel de papel electrónico XIAO 7.5" se envía con el XIAO ESP32-C3 conectado directamente al panel. Solo conéctalo mediante USB-C; no se requiere ningún ensamblaje adicional.

</TabItem>
</Tabs>

## Paso 3: Flashear el firmware TRMNL
...
:::tip Los kits TRMNL DIY totalmente nuevos ya se envían con firmware TRMNL
...
:::info
Este método solo es aplicable a **reTerminal E1001 / E1002 / E1003**. El reTerminal E-Series Firmware Flasher proporciona paquetes de firmware TRMNL para estos tres dispositivos reTerminal E Serie.
:::

:::caution Prefiere el Método 1 para firmware TRMNL más reciente
El reTerminal E-Series Firmware Flasher proporciona un canal de firmware TRMNL alojado por Seeed para dispositivos reTerminal E Serie. Seeed no actualiza este canal con tanta frecuencia como el TRMNL Web Flasher oficial, y no todas las versiones ascendentes de firmware TRMNL se sincronizan aquí.
...
1. Visita el [reTerminal E-Series Firmware Flasher](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/).
...
4. En la segunda página, revisa la versión de firmware TRMNL disponible para tu dispositivo seleccionado.
...
:::tip Solo reTerminal E1003
Asegúrate de que el interruptor de encendido esté en **ON**. Si el dispositivo está en reposo y el flasheo no comienza, pulsa el botón **Refresh** en la parte superior de la unidad para despertarlo y vuelve a intentarlo.
:::

:::note Comportamiento de la pantalla de reTerminal E1002
El reTerminal E1002 utiliza una pantalla de papel electrónico a todo color, pero el firmware TRMNL actualmente representa el contenido en modo monocromo. Durante la provisión de Wi-Fi, la pantalla AP del E1002 muestra un diseño compacto con el logotipo TRMNL en la esquina inferior derecha. Cuando veas esta pantalla, busca y conéctate al punto de acceso E1002 y luego continúa con el mismo flujo de configuración de Wi-Fi que los otros dispositivos.
:::

Cuando el flasheo haya finalizado, continúa con el [Paso 4: Conectar el dispositivo a Wi-Fi](#paso-4-configurar-wi-fi-y-provisionar-el-dispositivo).
...
4. **Selecciona el entorno correcto de PlatformIO** para tu hardware:
   ...
   - Para **reTerminal E1002**, selecciona el entorno `seeed_reTerminal_E1002` (TRMNL representa el contenido en monocromo en el E1002).
...
## Paso 4: Configurar Wi-Fi y provisionar el dispositivo

El flujo de provisión de Wi-Fi es idéntico para todo el hardware compatible.

> 💡 **¿Problemas para conectar a Wi-Fi?** Consulta la [Guía de resolución de problemas de Wi-Fi del dispositivo TRMNL](https://help.usetrmnl.com/en/articles/10193157-device-wifi-troubleshooting).

### Encender y entrar en modo de provisión

Después de flashear el firmware TRMNL y encender tu dispositivo, entrará automáticamente en modo de provisión si aún no está conectado a Wi-Fi.
...
4. **Introduce el Device ID** que recibiste cuando compraste el acceso BYOD (esto **no** es la dirección MAC — busca el Device ID en el correo de confirmación de compra o en el panel de TRMNL). Haz clic en **Add new device**.
...
El reTerminal E1003 expone tres botones superiores bajo el firmware TRMNL:
...
:::note
La función táctil capacitiva solo es compatible con el firmware SenseCraft HMI. El firmware TRMNL no usa el panel táctil — toda la interacción se realiza mediante los botones físicos anteriores.
:::
...
En la parte superior de la página Plugins verás todos los plugins ya conectados a tu cuenta TRMNL. Algunos ejemplos son Weather, Days Left This Year, Stock Price, RSS Feed, Reddit, Hacker News, Language Learning, Motivational Quote, Custom Text, …
...
Si la pantalla está en reposo, pulsa una vez el botón **Refresh** primero para activar el dispositivo.
...
**Paso 4.** (Opcional) Deja habilitada la **Notificación por correo de batería baja** si aún quieres alertas por correo electrónico sin interrumpir la pantalla.
...
A veces, el programa incorrecto puede hacer que el XIAO pierda su puerto o no funcione correctamente:
...
Agradecimientos especiales a todo el **equipo TRMNL** por su sólido apoyo y ayuda inestimable a lo largo de esta integración. En particular queremos agradecer a **Bogdan**, **Ryan Kulp**, **Fr3d**, **Schappi** y a todos los demás miembros del equipo por su dedicación durante todo el desarrollo y la documentación.

Su experiencia hizo posible esta integración y mejoró enormemente la experiencia para las comunidades de reTerminal E Serie (E1001, E1002 y E1003), TRMNL DIY Kit y XIAO 7.5" ePaper Panel.
