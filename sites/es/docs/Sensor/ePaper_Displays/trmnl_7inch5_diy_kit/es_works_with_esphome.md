---
description: Este artículo describe cómo usar el TRMNL 7.5inch(OG) DIY Kit para funcionar con ESPHome.
...
- **Amplia compatibilidad de dispositivos**: Home Assistant se integra con miles de dispositivos y servicios domésticos inteligentes diferentes, lo que lo hace muy versátil y preparado para el futuro.
...
ESPHome es una herramienta de creación de firmware de código abierto diseñada específicamente para dispositivos ESP8266/ESP32. Te permite crear firmware personalizado usando sencillos archivos de configuración YAML, que luego pueden flashearse en tu dispositivo. Para el TRMNL 7.5inch(OG) DIY Kit, ESPHome sirve como el middleware esencial que habilita la comunicación entre el dispositivo y Home Assistant.
...
:::
Imprime las piezas de la carcasa de código abierto desde la [Resource part](#resources) y ensambla los componentes en su interior.  
...
Prueba el kit TRMNL para comprobar si funciona bien.
...
Dentro del código generado por defecto, el framework de esp32 puede ser `esp-idf`, el cual necesitamos cambiar a `arduino`.
...
**El objetivo principal es mostrarte diferentes formas de instalar el firmware en el dispositivo.**
...
Espera un momento y verás un resultado como en la siguiente imagen. Significa que el código se está ejecutando correctamente.
...
Espera un momento y verás un resultado como en la siguiente imagen. Si falla, puede deberse a una señal débil. Por favor, acerca el dispositivo a tu router. [¿Has encontrado un problema? Haz clic aquí.](#Q4)
...
Cuando veas un resultado como en la siguiente imagen, significa que el código se está ejecutando correctamente.
...
Primero, necesitamos instalar el complemento de editor de archivos. Busca **Studio Code Server** y haz clic en él. Haz clic en **INSTALL** y **START**.
...
Y <span id="ttf">luego</span>, crea una nueva carpeta llamada **fonts** y descarga este archivo y **ponlo dentro de la carpeta fonts**.
...
Si quieres usar otros iconos, puedes hacer clic en el siguiente botón para explorar más.
...
Luego coloca una imagen dentro de la carpeta **image**. Puedes hacer clic en el siguiente botón para descargar una imagen y hacer la prueba.
...
Ten en cuenta que la versión debe ser **igual o superior a 1.11.4**. Después de la instalación, ve a la **página de Configuration**. Necesitamos crear un `access_token` para este complemento.
...
Consulta el siguiente paso para crear un token y pégalo aquí.
...
Ve al final de la **página de Security** y crea un token, luego cópialo y pégalo en el complemento **Puppet**.
...
Al iniciar el complemento se lanzará un nuevo servidor en el puerto 10000. Cualquier ruta que solicites devolverá una captura de pantalla de esa página. Deberás especificar el tamaño de la ventana gráfica que deseas.
...
Para reducir la paleta de colores para pantallas E Ink® puedes añadir el parámetro eink. El valor representa el número de colores (incluido el negro) que se van a usar. Por ejemplo, para una pantalla E Ink® de 2 colores:
...
Este ejemplo muestra una configuración integral de ESPHome para el kit TRMNL de papel electrónico de 7,5 pulgadas para bricolaje. El código YAML integra múltiples funciones para crear un panel de visualización inteligente e interactivo que funciona a la perfección con Home Assistant.
...
Este ejemplo muestra cómo combinar lecturas de sensores, datos de Home Assistant y entrada del usuario para crear una pantalla inteligente siempre encendida y con muchas funciones usando ESPHome y el kit TRMNL de papel electrónico de 7,5 pulgadas.
