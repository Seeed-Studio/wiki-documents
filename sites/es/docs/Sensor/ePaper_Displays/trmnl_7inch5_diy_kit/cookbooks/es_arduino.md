...
El TRMNL 7.5" (OG) DIY Kit es una plataforma de desarrollo versátil que incorpora como cerebro el potente XIAO ESP32-S3 Plus. Este DIY Kit combina la capacidad de procesamiento del ESP32-S3 con una hermosa pantalla ePaper de 7.5 pulgadas, creando la base perfecta para proyectos de visualización de información de bajo consumo. Esta guía se centra en programar el TRMNL DIY Kit usando el framework de Arduino, proporcionándote el conocimiento esencial para utilizar sus distintas funciones de hardware.

## Primeros pasos

Antes de profundizar en las funciones específicas, configuremos nuestro entorno de desarrollo para el TRMNL 7.5" (OG) DIY Kit.
...
:::
Imprime las piezas de la carcasa de código abierto desde [Resource part](#resource-part) y monta los componentes en su interior.  
...
Para programar el TRMNL DIY Kit con Arduino, necesitarás configurar el IDE de Arduino con soporte para ESP32.
...
**Paso 5.** Conecta tu TRMNL DIY Kit al ordenador usando un cable USB-C.

**Paso 6.** Selecciona el puerto correcto en **Tools > Port**.

## Ejemplos de Arduino

Ahora exploremos las funciones principales del TRMNL DIY Kit con ejemplos de código en Arduino.

### Botones de usuario

El TRMNL DIY Kit incluye tres botones programables por el usuario (D1, D2, D4) y un botón de reinicio. Vamos a crear un ejemplo sencillo para detectar pulsaciones de botones.
...
3. En la función `loop()`, leemos el estado de cada botón. Como usamos resistencias pull-up, los pines leen LOW cuando los botones están pulsados.

4. Invertimos las lecturas (con el operador `!`) para que `true` signifique "pulsado" y `false` signifique "liberado".

5. Si se pulsa cualquier botón, imprimimos el estado de todos los botones en el Monitor Serie.

6. Un pequeño retardo evita lecturas repetidas rápidas cuando se mantiene pulsado un botón.
...
El TRMNL 7.5" (OG) DIY Kit incluye un circuito para monitorizar el voltaje de la batería conectado al pin D0 (GPIO1). Una característica importante de este diseño es que también incluye un pin de control (GPIO6) para activar/desactivar la alimentación del ADC, lo que ayuda a conservar batería cuando no se está midiendo activamente.

#### Ejemplo de monitorización del voltaje de la batería
...
- `ADC_EN_PIN` (GPIO6/A5): Controla la alimentación del circuito de medición del ADC

2. Definimos un `CALIBRATION_FACTOR` (0.968) para ajustar con precisión la exactitud de la lectura de voltaje.

3. En la función `setup()`:

- Inicializamos la comunicación serial

- Configuramos el pin ADC_EN como salida y lo ponemos en LOW (deshabilitado) para ahorrar energía
...
- Imprimimos el voltaje en el Monitor Serial

- Determinamos y mostramos el estado de la batería en función de umbrales de voltaje

- Esperamos 5 segundos antes de tomar la siguiente lectura

5. La función `readBatteryVoltage()`:

- Habilita el circuito del ADC poniendo ADC_EN_PIN en HIGH

- Espera brevemente a que el circuito se estabilice

- Toma 30 lecturas y las promedia para obtener resultados más estables

- Deshabilita el circuito del ADC para ahorrar energía
...
Una característica clave de esta implementación es la capacidad de desactivar el circuito de medición de la batería cuando no se está utilizando. El TRMNL DIY Kit está diseñado para un bajo consumo de energía, y este enfoque ayuda a prolongar la vida útil de la batería al alimentar el circuito divisor de voltaje solo cuando se necesita una medición real.

El factor de calibración (0.968) compensa las tolerancias de los componentes en el divisor de voltaje y ayuda a garantizar lecturas precisas. Es posible que este valor necesite un ligero ajuste para tu placa específica si observas que las lecturas se desvían de forma constante en comparación con una medición con multímetro.

Tomar múltiples lecturas y promediarlas ayuda a reducir el ruido y proporciona mediciones de voltaje más estables, lo cual es especialmente importante para la monitorización de baterías, donde pequeños cambios de voltaje pueden ser significativos para determinar la capacidad restante.

## Pantalla ePaper

El TRMNL 7.5" (OG) DIY Kit incluye una hermosa pantalla ePaper en blanco y negro de 7,5 pulgadas que ofrece una visibilidad clara en diversas condiciones de iluminación y un consumo de energía ultrabajo. En esta sección, exploraremos cómo configurar y controlar la pantalla ePaper usando Arduino.

### Configuración de hardware

Antes de comenzar a programar, asegurémonos de que la pantalla ePaper esté correctamente conectada a la placa TRMNL:

**Paso 1.** Conecta la pantalla ePaper al conector de 24 pines en la placa controladora. El conector admite inserción a ciegas, por lo que no puedes insertarlo de forma incorrecta.
...
**Paso 2.** Después de insertar el cable plano, asegúralo cerrando el mecanismo de bloqueo.

:::caution
El cable plano es frágil y puede dañarse fácilmente. Evita doblarlo lateralmente tanto como sea posible.
:::
...
Para controlar la pantalla ePaper, utilizaremos la biblioteca Seeed_GFX, que proporciona compatibilidad completa con varios dispositivos de pantalla de Seeed Studio.
...
:::note
Si has instalado previamente la biblioteca TFT_eSPI, es posible que debas eliminarla temporalmente o cambiarle el nombre en la carpeta de bibliotecas de Arduino para evitar conflictos, ya que Seeed_GFX es un fork de TFT_eSPI con funciones adicionales.
:::
...
**Paso 6.** Ve a la [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/), y selecciona "TRMNL 7.5" (OG) DIY Kit" como se muestra en la imagen.
...
Exploremos un sencillo ejemplo de reloj analógico que demuestra cómo dibujar gráficos en la pantalla ePaper. [Este ejemplo](https://github.com/Seeed-Studio/Seeed_GFX/blob/master/examples/ePaper/Basic/Clock/Clock.ino) crea una clásica esfera de reloj con agujas de horas, minutos y segundos.
...
Este elegante ejemplo crea un reloj analógico clásico en la pantalla de ePaper. Veamos en detalle cómo funciona:

1. **Inicialización y configuración:**
   - El código usa compilación condicional con `#ifdef EPAPER_ENABLE` para asegurarse de que solo se ejecute en hardware con soporte para pantalla de e-paper.
   - Declaramos variables para rastrear las posiciones de las manecillas del reloj y sus posiciones anteriores.
   - La hora inicial se establece en función de la hora de compilación del sketch usando la macro `__TIME__`.

2. **Función `setup`:**
   - Inicializamos la pantalla de e-paper con `epaper.begin()`.
   - La pantalla se rellena con blanco como fondo usando `epaper.fillScreen(TFT_WHITE)`.
   - Dibujamos la esfera del reloj como un círculo negro con interior blanco.
   - Las marcas de las horas se dibujan como 12 líneas alrededor de la circunferencia.
   - Las marcas de los minutos se dibujan como 60 puntos con un énfasis especial en las posiciones de los cuartos de hora.
   - Se dibuja un pequeño círculo negro en el centro de la esfera del reloj.
...
3. **Función `loop`:**
   - Cada segundo (cuando `millis()` supera `targetTime`), actualizamos las manecillas del reloj.
   - Incrementamos los segundos, minutos y horas según sea necesario.
   - Calculamos los ángulos de cada manecilla en función de la hora actual.
   - Las posiciones de las manecillas se calculan usando funciones trigonométricas:
     - Manecilla de las horas: 30 grados por hora (más un ajuste por los minutos)
     - Manecilla de los minutos: 6 grados por minuto (más un ajuste por los segundos)
     - Manecilla de los segundos: 6 grados por segundo
   - Para reducir actualizaciones innecesarias y prolongar la vida útil de la pantalla, solo borramos y redibujamos las manecillas de horas y minutos una vez por minuto (cuando los segundos = 0).
   - La manecilla de los segundos se actualiza cada segundo borrando su posición anterior y dibujándola en la nueva posición.
...
4. **Función auxiliar:**
   - La función `conv2d` convierte una representación en cadena de un número a un valor entero, utilizada para analizar los valores de hora, minuto y segundo de la hora de compilación.

**Notas importantes sobre este ejemplo:**

1. **Actualizaciones de la pantalla:** A diferencia de las pantallas LCD u OLED, las pantallas ePaper no están diseñadas para actualizaciones frecuentes. Este ejemplo actualiza la pantalla cada segundo, lo cual está bien para fines de demostración, pero en una aplicación del mundo real, quizá quieras actualizar con menos frecuencia para prolongar la vida útil de la pantalla.

2. **Actualizaciones parciales:** Este ejemplo demuestra actualizaciones parciales al borrar y redibujar solo las partes necesarias de la pantalla, lo cual es más eficiente que refrescar toda la pantalla.

3. **Compilación condicional:** Las directivas `#ifdef EPAPER_ENABLE` garantizan que el código solo se compile y ejecute si la pantalla de e-paper está correctamente configurada en el sistema.

4. **Funciones de dibujo:** El ejemplo muestra varias funciones de dibujo:
   - `fillCircle()` para crear la esfera del reloj
   - `drawLine()` para dibujar las manecillas del reloj y las marcas de las horas
   - `drawPixel()` y `fillCircle()` para las marcas de los minutos
   - `drawCentreString()` para texto centrado

Este ejemplo de reloj analógico proporciona un excelente punto de partida para crear tus propias aplicaciones gráficas en la pantalla de ePaper del TRMNL 7.5" (OG) DIY Kit.
...
- `display.println("text")`: Imprime texto con un salto de línea

Los colores disponibles para esta pantalla monocroma son:
