...
sidebar_position: 3
sidebar_label: ESPHome - Pantalla
last_update:
  date: 08/05/2026
  author: Citric
...
### ¿Por qué usar la pantalla ePaper reTerminal E Serie con Home Assistant?
...
3. **Larga duración de batería**: Combinada con el modo de deep sleep, la pantalla puede funcionar durante meses con una sola carga de batería y seguir proporcionando información valiosa de un vistazo.
...
Estas ventajas hacen que la pantalla ePaper reTerminal E Serie sea una opción ideal para crear una pantalla de información siempre encendida y de bajo consumo para tu configuración de Home Assistant.

### Integración con ESPHome

ESPHome es una herramienta de creación de firmware de código abierto diseñada específicamente para dispositivos ESP8266/ESP32. Te permite crear firmware personalizado usando sencillos archivos de configuración YAML, que luego se pueden flashear en tu dispositivo. Para la reTerminal E Serie, ESPHome actúa como el middleware esencial que habilita la comunicación entre el dispositivo y Home Assistant.

El sistema funciona convirtiendo tu configuración YAML en un firmware completo que se ejecuta en tu dispositivo ESP. Este firmware se encarga de todas las tareas complejas de conexión a tu red, comunicación con Home Assistant y control de la pantalla ePaper. Combinado con Home Assistant, ESPHome proporciona una plataforma robusta para crear pantallas y controles de automatización del hogar sofisticados.

Veamos cómo configurarlo y sacar el máximo partido a esta pantalla tan versátil.
...
**Paso 2.** Selecciona la tarjeta de plataforma **ESPHome** y luego elige tu modelo de reTerminal (E1001 / E1002 / E1003 / E1004).

**Paso 3.** En el paso de configuración, marca las funciones disponibles en tu dispositivo — por ejemplo, pantalla, botones, zumbador, LED, batería, SHT4x, RTC, microSD, micrófono, táctil (E1003) o suspensión profunda.

**Paso 4.** Genera el YAML, luego usa **Copy to clipboard** o **Download file** e impórtalo en tu panel de ESPHome.
...
Ponle al dispositivo un nombre que te guste y selecciona **ESP32-S3** como tipo de chip, luego haz clic en **SKIP**.
...
:::tip
Por favor, actualiza tu versión de ESPHome a **2026.7.0** o posterior para la plataforma de pantalla `it8951`.
:::
...
:::tip
Por favor, actualiza tu versión de ESPHome a **2026.7.0** o posterior para el modelo `seeed-reterminal-e1004`.
:::
...
:::tip
Si tu Host de Home Assistant (Raspberry PI/Green/Yellow, etc.) está cerca, recomendamos usar este método ya que es más sencillo.
:::
...
Haz clic en las opciones siguiendo la imagen para instalar el código en el dispositivo. [¿No se ha encontrado el puerto cuando el dispositivo está en modo de suspensión profunda?](#puerto)
...
Espera un momento y verás una respuesta como en la siguiente imagen. Significa que el código se está ejecutando correctamente.
...
Espera un momento y verás una respuesta como en la siguiente imagen. Si falla, puede deberse a una señal débil. Por favor, acerca el dispositivo a tu router. [¿Has encontrado un problema? Haz clic aquí.](#P4)
...
Este ejemplo de código YAML configura la interfaz SPI y la pantalla ePaper reTerminal E Series para un proyecto ESPHome. La sección `lambda` contiene comandos de dibujo que representan formas simples en la pantalla:
...
:::tip
Por favor, actualiza tu versión de ESPHome a **2026.7.0** o posterior.
:::
...
Debido a limitaciones de espacio, no profundizaremos demasiado en los métodos de dibujo y los principios de otros patrones; si es necesario, se recomienda al lector leer [esta parte de ejemplos detallados de ESPHome](https://esphome.io/components/display/).

### Obtener valores de Home Assistant para mostrarlos

Para mostrar datos meteorológicos en tu dispositivo de pantalla ePaper reTerminal E Series, necesitarás acceder a los datos de temperatura, condiciones meteorológicas y velocidad del viento desde Home Assistant. La integración Open-Meteo proporciona datos meteorológicos fiables a los que se puede acceder a través de las Herramientas de Desarrollador.
...
#### Acceso a los datos meteorológicos en las Herramientas de Desarrollador
...
Para usar estos datos meteorológicos en tu configuración ESPHome para el reTerminal E Series, deberás configurar una conexión API de Home Assistant en tu configuración YAML de ESPHome:
...
Esta configuración crea entidades de sensor en tu dispositivo ESPHome que extraen datos de la integración meteorológica de Home Assistant. Luego puedes usar estos sensores para actualizar la pantalla ePaper reTerminal E Series con la información meteorológica actual.

:::tip
Para los datos de pronóstico, deberás usar las entidades `weather.open_meteo_forecast` que contienen valores previstos para días futuros.
:::
...
Después de compilar el código anterior y subirlo a tu dispositivo, es posible que primero veas **NaN** mostrado en la pantalla, no te preocupes, esto es normal. Esto se debe a que el dispositivo aún no se ha añadido al entorno de Home Assistant, por lo que reTerminal todavía no ha podido adquirir los datos de Home Assistant. Solo tenemos que seguir los pasos siguientes para añadir el dispositivo.
...
Paso 3. Home Assistant debería detectar automáticamente tu dispositivo de pantalla ePaper reTerminal E Serie mediante mDNS. Si aparece en la sección de dispositivos detectados, haz clic en Configure para añadirlo.
...
Paso 5. Introduce la dirección IP de tu dispositivo de pantalla ePaper reTerminal E Serie y la clave de cifrado de la API si has configurado una.

Paso 6. Una vez conectado, tu pantalla ePaper reTerminal E Serie aparecerá como un dispositivo en Home Assistant con todos sus sensores y componentes disponibles.
...
Aquí tienes el contenido mejorado de la Demo 3 con formato optimizado, colocación de imágenes y descripciones adicionales:

### Dibujo de fuentes TrueType

Este ejemplo muestra cómo visualizar iconos personalizados en tu pantalla ePaper reTerminal E Serie utilizando fuentes TrueType. Material Design Icons proporciona una amplia gama de símbolos escalables perfectos para pantallas de tinta electrónica.
...
La biblioteca Material Design Icons contiene miles de iconos que puedes usar en tus proyectos. A continuación se explica cómo encontrar y usar diferentes iconos:
...
:::tip
Para paneles de control del tiempo, considera usar iconos como `F0590` (soleado), `F0591` (parcialmente nublado), `F0593` (lluvioso) y `F059E` (ventoso).
:::

Combinando estos iconos con los datos meteorológicos de Home Assistant que configuramos antes, puedes crear una pantalla meteorológica dinámica que muestre las condiciones actuales usando los iconos apropiados.
...
Paso 1. Asegúrate de tener instalado el complemento **Studio Code Server** en Home Assistant. Si aún no lo has instalado, sigue las instrucciones del ejemplo anterior.
...
:::tip
Actualiza tu versión de ESPHome a la **2025.11.1** o superior.
:::
...
:::tip
Actualiza tu versión de ESPHome a la **2026.7.0** o posterior. El E1003 utiliza un panel de escala de grises de 16 niveles; da preferencia al tipo de imagen `GRAYSCALE` y a un redimensionado mayor para el panel de 1872×1404 cuando sea necesario.
:::
...
Paso 6. Guarda tu configuración y súbela a tu reTerminal E Serie. Cuando la actualización se complete, tu pantalla de papel electrónico mostrará la imagen.
...
:::caution
Recuerda que las pantallas de papel electrónico tienen tasas de refresco limitadas. La configuración `update_interval: 300s` significa que tu pantalla se actualizará solo cada 5 minutos. Ajusta este valor según tus necesidades, pero ten en cuenta que las actualizaciones frecuentes pueden reducir la vida útil de las pantallas de papel electrónico.
:::

Al combinar imágenes con texto y otros elementos de visualización tratados en ejemplos anteriores, puedes crear paneles ricos e informativos en tu reTerminal E Serie.
...
- **[ESPHome Cookbook: RTC, SD Card & Microphone](/es/reterminal_e10xx_with_esphome_rtc_sd_microphone)** - sincronización horaria RTC PCF8563, pines de alimentación/detección de tarjeta microSD y inicialización del micrófono PDM integrado.
...
La reTerminal E Serie utiliza un puente CH340K USB-a-UART en UART0. Mantén esta configuración de registro en tu YAML:
