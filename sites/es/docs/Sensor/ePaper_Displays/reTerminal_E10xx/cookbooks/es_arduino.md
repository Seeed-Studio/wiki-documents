...
Esta página se centra en **controlar la pantalla de tinta electrónica** desde Arduino. Si quieres usar el LED integrado, el zumbador, los botones, el sensor SHT4x, el monitor de batería o la ranura para tarjeta microSD, dirígete a **[Arduino Cookbook: Onboard Peripherals](https://wiki.seeedstudio.com/es/reterminal_e10xx_with_arduino_peripherals)**. Para el RTC, modos de bajo consumo, el micrófono integrado y el dibujo con toque capacitivo, consulta **[Arduino Cookbook: RTC, Low Power, Audio & Touch](https://wiki.seeedstudio.com/es/reterminal_e10xx_with_arduino_peripherals_2)**.

El código base compartido — configuración del IDE de Arduino, paquete de placas ESP32, instalación de `Seeed_GFX`, generación de `driver.h` — también se encuentra en **[Work with Arduino](https://wiki.seeedstudio.com/es/epaper_work_with_arduino)**. Échale un vistazo primero si eres nuevo en Arduino en pantallas de tinta electrónica de Seeed.
:::

## Introducción

La reTerminal E Serie es la línea HMI industrial de Seeed Studio, basada en la XIAO ESP32-S3 y que incorpora pantallas de tinta electrónica. Este recetario te guía por todo lo necesario para renderizar texto, gráficos e imágenes en la pantalla:

- Descripción general de hardware y enlaces de compra para E1001 / E1002 / E1003 / E1004.
...
- **Ejemplos avanzados específicos de cada panel** con Seeed_GFX — **escala de grises de 4 niveles en E1001** y **escala de grises de 16 niveles en E1003**.
...
Para programar la pantalla de tinta electrónica de reTerminal E Serie con Arduino, necesitarás configurar el IDE de Arduino con soporte para ESP32.
...
**Paso 1.** Descarga e instala el [IDE de Arduino](https://www.arduino.cc/en/software) y lanza la aplicación de Arduino.
...
Para controlar la pantalla de tinta electrónica, usaremos la biblioteca Seeed_GFX, que proporciona compatibilidad completa con varios dispositivos de pantalla de Seeed Studio.
...
Veamos un ejemplo sencillo que demuestra operaciones básicas de dibujo en la pantalla de tinta electrónica en blanco y negro.
...
La pantalla de tinta electrónica a todo color admite colores rojo, negro y blanco, lo que permite interfaces visualmente más ricas.
...
**Paso 5.** Sube el sketch a tu reTerminal E1002. La pantalla mostrará gráficos en color que demuestran las capacidades de color completo de la pantalla de tinta electrónica.
...
#### Programar reTerminal E1003 (tinta electrónica de 10,3 pulgadas)

Sigue el mismo flujo de trabajo usando la biblioteca Seeed_GFX para configurar y controlar la tinta electrónica en el reTerminal E1003.
...
#### Programar reTerminal E1004 (tinta electrónica a todo color de 13,3 pulgadas)

Utiliza la biblioteca Seeed_GFX para configurar y controlar la pantalla de tinta electrónica a todo color E Ink® Spectra™ 6 en el reTerminal E1004.
...
**Paso 5.** Sube el sketch a tu reTerminal E1004 para verificar el renderizado en color, primitivas de dibujo, renderizado de texto y comportamientos de refresco de pantalla completa.
...
El siguiente ejemplo primero pinta cuatro franjas horizontales — una por nivel de gris — para que puedas verificar visualmente la paleta, y luego copia un mapa de bits en escala de grises de 800×480 en la pantalla. La biblioteca Seeed_GFX ya incluye esto como un ejemplo listo para flashear, incluido el `image.h` preconvertido, por lo que no necesitas generar tú mismo ningún dato de mapa de bits.

**Paso 1.** Abre el sketch de ejemplo de la biblioteca Seeed_GFX: **File > Examples > Seeed_GFX > ePaper > Gray > GrayLevel4**. El sketch y su `image.h` asociado se abrirán en el editor.
...
**Paso 4.** Sube el sketch. La pantalla primero muestra cuatro franjas en escala de grises — negro en la parte superior, luego gris oscuro, gris claro y blanco en la parte inferior — y luego se limpia y representa el mapa de bits desde `image.h`.
...
:::tip
La actualización en escala de grises de 4 niveles es aproximadamente 4 veces más lenta que una actualización en blanco y negro de 1 bit porque el controlador lleva cada píxel a través de cuatro voltajes objetivo en lugar de dos. Úsala para contenido estático como fotos, ilustraciones o paneles con muchos detalles, y mantente en el modo estándar de 1 bit para actualizaciones rápidas de la interfaz de usuario.
:::
...
**Paso 3.** La biblioteca `Seeed_GxEPD2` requiere la `Adafruit GFX Library` para funcionar, la cual también debes instalar. La forma más sencilla de hacerlo es a través del Library Manager: ve a **Tools > Manage Libraries...**, busca "Adafruit GFX Library" y haz clic en "Install".
