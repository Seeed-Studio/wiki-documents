...
La reTerminal E Serie representa el último avance de Seeed Studio en soluciones HMI industriales, con ESP32-S3 como controlador principal e integrando pantallas de tinta electrónica. Esta guía te explicará cómo programar la pantalla de tinta electrónica en los dispositivos reTerminal E Serie usando Arduino IDE, lo que te permitirá crear interfaces y aplicaciones personalizadas con excelente visibilidad y un consumo de energía ultrabajo.
...
La **reTerminal E1001 incorpora una pantalla de tinta electrónica en blanco y negro de 7,5 pulgadas**, mientras que la **reTerminal E1002 está equipada con una pantalla de tinta electrónica a todo color de 7,3 pulgadas**. Ambas pantallas ofrecen una excelente visibilidad en diversas condiciones de iluminación con un consumo de energía ultrabajo, lo que las hace ideales para aplicaciones industriales que requieren pantallas siempre encendidas con un uso mínimo de energía.
...
**Paso 6.** Carga el sketch en tu reTerminal E1002. La pantalla mostrará gráficos coloridos que demuestran las capacidades de color completo de la pantalla de tinta electrónica.
...
Además de Seeed_GFX, también puedes usar la biblioteca `GxEPD2` para controlar la pantalla de tinta electrónica del reTerminal. `GxEPD2` es una biblioteca potente y popular que admite una amplia gama de pantallas de tinta electrónica.
...
Aquí tienes el código de ejemplo para mostrar "Hello World!" en la pantalla de tinta electrónica en blanco y negro del reTerminal E1001 usando la biblioteca `GxEPD2`. Establece `EPD_SELECT` en `0` para seleccionar el controlador para el E1001.
...
Para el reTerminal E1002, simplemente necesitas cambiar el valor de `EPD_SELECT` a `1`. Esto seleccionará el controlador apropiado para la pantalla de tinta electrónica a todo color de 7,3 pulgadas. El resto del código permanece igual.
...
- **Detección de tarjeta:** La función `isCardInserted()` lee el `SD_DET_PIN`. En el hardware del reTerminal, este pin se pone en nivel BAJO cuando hay una tarjeta presente.
...
Este ejemplo completo combina las funcionalidades de las secciones anteriores. Escribiremos un programa que lea un archivo de imagen Bitmap (`.bmp`) desde una tarjeta MicroSD y lo muestre en la pantalla de tinta electrónica del reTerminal. Esto demuestra una aplicación práctica y real del dispositivo.
...
2. **Guardar como un BMP estándar:** El código está diseñado para leer archivos BMP de 24 o 32 bits **sin comprimir**. Usar un editor de imágenes profesional es la mejor forma de asegurarte de que el formato sea correcto. Recomendamos el software libre y de código abierto **GIMP**:
...
- **Bucle de dibujo**: La función lee la imagen desde la tarjeta SD una fila a la vez. Para cada píxel de la fila, extrae los valores de color Rojo, Verde y Azul.
