...
- **Procesamiento de audio avanzado** :Impulsado por XVF3800 con AEC, formación de haz múltiple, desreverberación, detección de DoA, supresión dinámica de ruido, rango de AGC de 60 dB
...
## Primeros pasos (firmware I2S vs USB)
...
Para usar la reSpeaker XVF3800 con XIAO ESP32S3, asegúrate de que el firmware de la reSpeaker XVF3800 sea la versión I2S y esté actualizado a la última versión.
Visita esta [sección](https://wiki.seeedstudio.com/es/respeaker_xvf3800_introduction/#update-firmware).

:::note
Para trabajar con la XIAO ESP32S3, necesitas flashearla con el firmware I2S. Sin embargo, el firmware I2S no admite USB DFU (el dispositivo no se detecta como dispositivo USB). Cambia al modo seguro, ya que admite tanto USB DFU como I2C DFU. Para obtener más información, visita esta [sección](https://wiki.seeedstudio.com/es/respeaker_xvf3800_introduction/#safe-mode).
:::
...
| ----------------------------- | ---------------------------------------------------------------------------------------------------- |
| **Procesador de audio principal**      | **XMOS XVF3800**, gestiona el procesamiento de audio incluyendo AEC, formación de haz, supresión de ruido, etc.       |
...
| **LEDs RGB**                  | **12x WS2812** LEDs RGB direccionables individualmente para retroalimentación visual (por ejemplo, estado, actividad de voz). |
...
| **Pads IO no utilizados (XIAO)**     | Pads de E/S adicionales soldados y conectados al módulo XIAO.                                                 |
...
| **Funciones avanzadas de audio**   | AEC, formación de haz, desreverberación, **detección de DoA**, supresión de ruido basada en DNN, AGC de 60 dB.         |
...
Cuando se utiliza la XIAO ESP32S3, no se necesita una antena externa; simplemente conecta las ranuras de la antena para usar la antena PCB integrada.
...
La reSpeaker XVF3800 expone 3 pines de entrada (GPI) y 5 pines de salida (GPO) para control externo. Puedes usarlos para leer estados de botones o controlar hardware como el LED de silencio, el amplificador o los LEDs.
...
:::note
Instala la [Arduino Audio Tools library](https://github.com/pschatzmann/arduino-audio-tools) para poder ejecutar los ejemplos proporcionados.
:::
