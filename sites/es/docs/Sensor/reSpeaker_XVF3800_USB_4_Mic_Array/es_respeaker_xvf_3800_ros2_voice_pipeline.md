...
Crea una nueva regla udev para garantizar los permisos adecuados para la ReSpeaker USB Mic Array:
...
> Omite la Parte 6 si ya tienes un espacio de trabajo `~/ros2_ws`.
...
Busca la línea que diga **reSpeaker** o **XVF3800**. Anota el número entre corchetes — ese es tu `MIC_INDEX`. Actualiza `config.env` con ese número.
...
Se abrirá una ventana que muestra la tortuga. Ahora di:
...
Todos los ajustes se encuentran en `config.env`. Edítalo y vuelve a compilar para aplicar los cambios.
...
Creado con:
- [Seeed Studio reSpeaker XVF3800](https://wiki.seeedstudio.com/es/respeaker_xvf3800_introduction/) — matriz de micrófonos con XMOS XVF3800
