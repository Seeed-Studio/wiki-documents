---
description: Tutorial básico de pantalla redonda XIAO
...
Seeed Studio Round Display para XIAO es una placa de expansión compatible con todas las placas de desarrollo XIAO. Incorpora en un lado una pantalla táctil de cobertura completa, diseñada como un disco de 39 mm. Contiene RTC integrado, chip de carga, ranura para tarjeta TF dentro de su tamaño compacto, perfecta para pantallas interactivas en hogares inteligentes, wearables y más.
...
<table align="center">
 ...
 <tr>
  <td>v1.1</td>
  <td>1.Cambiar el IC de carga de batería de litio<br />2.Añadir un interruptor a A0 y D6</td>
  <td>2023/4/7</td>
  <td>Linus.Liao</td>
 </tr>
</table>
...
- **Placa de expansión con pantalla táctil capacitiva**: Pantalla redonda de 1,28 pulgadas, resolución 240×240, 65K colores, que ofrece una exhibición de imágenes clara y colorida
...
- **Periféricos ricos**: Incorpora RTC, chip de carga de batería, ranura para tarjeta TF, conector JST 1.25, todo dentro de su tamaño compacto
- **Diseño del tamaño de un reloj**: Con diseño circular de 39 mm, adecuado para proyectos portables y con espacio limitado
...
Desde el 7 de abril de 2023, la Round Display renovada añade un interruptor de 2 bits para controlar la retroiluminación de la pantalla y la lectura del voltaje de la batería. Para que el usuario pueda elegir libremente si usar el pin o liberarlo. Haz clic [aquí](https://wiki.seeedstudio.com/es/seeedstudio_round_display_usage/#botón-ke--gpio) para leer más detalles.
...
Si quieres aprovechar todas las capacidades de la Round Display y tener una gran experiencia, te recomendamos encarecidamente que compres nuestra serie XIAO como placa base para la Round Display.
...
:::caution
Ten en cuenta que al conectar la XIAO, **el conector Type-C de la XIAO debe mirar hacia el exterior de la Round Display**. Si accidentalmente inviertes la polaridad, no te preocupes demasiado, la Round Display tiene un circuito de protección de alimentación que no se dañará fácilmente, pero no te recomendamos mantener la conexión invertida durante mucho tiempo.
:::

La orientación recomendada para la Round Display es: cuando te enfrentes a la Round Display, el conector Type-C de la XIAO mira hacia la derecha, de modo que el botón de encendido/apagado de la Round Display quede en la esquina inferior izquierda.
...
Para usar la Round Display, necesitamos programar la serie XIAO. La herramienta de programación recomendada es el Arduino IDE, y necesitas configurar el entorno de Arduino para la XIAO y añadir el paquete de la placa.
...
Para usar la Round Display, necesitamos instalar previamente dos bibliotecas. Una es la biblioteca del controlador del dispositivo Round Display, que se utiliza para controlar las funciones de pantalla y táctiles del dispositivo. La otra es la biblioteca de visualización gráfica, que proporciona algunas interfaces muy comunes para dibujar gráficos.
...
Como probablemente podamos deducir del tutorial anterior, Round Display utiliza principalmente las bibliotecas **LVGL**, **TFT_eSPI** y **Arduino GFX**. Por motivos de espacio, presentaremos el uso de las bibliotecas **LVGL** y **TFT_eSPI** por separado con el ejemplo de dibujar una esfera.

- Puedes conocer la interfaz y el uso de la biblioteca **TFT_eSPI** haciendo clic **[aquí](https://wiki.seeedstudio.com/es/using_lvgl_and_tft_on_round_display#interfaces-comunes-para-la-biblioteca-tft)**.

- Puedes conocer la interfaz y el uso de la biblioteca **LVGL** haciendo clic **[aquí](https://wiki.seeedstudio.com/es/using_lvgl_and_tft_on_round_display#interfaces-comunes-para-la-biblioteca-lvgl)**.
...
Ve a la [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/), y selecciona "TRMNL 7.5" (OG) DIY Kit" como se muestra en la imagen.
...
Ahora puedes subir el ejemplo a tu TRMNL DIY Kit y ver la pantalla de tinta electrónica en acción.
...
Una vez que el hardware y el software estén listos, empezamos a subir nuestro primer programa de ejemplo. Este programa de ejemplo se puede utilizar para comprobar si el reloj RTC de Round Display funciona correctamente. Debido a que esta demo requiere una pequeña cantidad de memoria, es adecuada para todos los modelos XIAO.
...
Asegúrate de que el interruptor de Round Display esté en la posición ON.
...
Debido a que la demo HardwareTest es un poco compleja y ocupa una mayor cantidad de memoria, algunos modelos XIAO pueden no ejecutarse correctamente por falta de memoria. Hemos creado una tabla de pruebas para tu referencia, utilizando la biblioteca TFT y la biblioteca Arduino GFX.
...
Este programa de ejemplo se puede utilizar para comprobar si el reloj RTC, la tarjeta SD y las funciones táctiles de Round Display funcionan correctamente.
...
R: Por favor, comprueba que el interruptor de Round Display esté encendido. Si estás utilizando el XIAO ESP32C3, es posible que también tengas que pulsar Reset después de subir el programa para que funcione.

### P2: Si quiero conectar Seeed Studio XIAO ESP32S3 Sense a esta pantalla de expansión, ¿habrá un conflicto con dos ranuras para tarjetas TF?

R: Esto no genera ningún conflicto. Las diferentes ranuras para tarjetas SD se controlan mediante chip select; si quieres usar la ranura para tarjeta microSD en Sense, el pin de chip select debe ser **21**, si quieres usar la ranura para tarjeta microSD en Round Display, el pin de chip select debe ser **D2**.

Tenemos [ejemplos](https://wiki.seeedstudio.com/es/xiao_esp32s3_camera_usage/#proyecto-i-making-a-handheld-camera) de uso tanto del hardware como de tarjetas microSD en el tutorial de la cámara S3 Sense.
...
- **[PDF]** [Hoja de datos de la pantalla de cristal líquido 1.28'' a-Si TFT](https://files.seeedstudio.com/wiki/round_display_for_xiao/GJX0128A4-15HY_Datasheet.pdf)
- **[PDF]** [Esquemático de Seeed Studio Round Display for XIAO](https://files.seeedstudio.com/wiki/round_display_for_xiao/SeeedStudio_Round_Display_for_XIAO_v1.0_SCH_230308.pdf)
- **[PDF]** [Esquemático de Seeed Studio Round Display for XIAO v1.1](https://files.seeedstudio.com/wiki/round_display_for_xiao/SeeedStudio_Round_Display_for_XIAO_v1.1_SCH_230407.pdf)
- **[PDF]** [Hoja de datos GJX0128A4-15HY](https://files.seeedstudio.com/wiki/round_display_for_xiao/GJX0128A4-15HY_Datasheet.pdf)
- **[ZIP]** [Esquemático y PCB de Seeed Studio Round Display for XIAO](https://files.seeedstudio.com/wiki/round_display_for_xiao/SeeedStudio_Round_Display_for_XIAO_v1.0_SCH&PCB_230308.zip)
- **[ZIP]** [Esquemático y PCB de Seeed Studio Round Display for XIAO v1.1](https://files.seeedstudio.com/wiki/round_display_for_xiao/SeeedStudio_Round_Display_for_XIAO_v1.1_SCH&PCB_230407.zip)
