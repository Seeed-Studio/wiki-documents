...
url: https://wiki.seeedstudio.com/es/reTerminal/
---
...
| 09/03/2021 | 2021-06-02-Raspbian(modified)-32-bit | V1.6 | v1.6 | Cambiar chip de expansión de IO de MCP23008-E a PCA9554, <br />Cambiar microchip de cifrado de ATECC608A-SSHDA-B a ATECC608A-TNGTLSS-G [Más información](#../reTerminal-FAQ#q13-how-to-check-if-the-encryption-chip-is-atecc608a-sshda-b-or-atecc608a-tngtlss-g) |
...
- Puerto Ethernet Gigabit y dos puertos USB 2.0 Tipo A
...
<table style={{tableLayout: 'fixed', width: 743}}>
  ...
  <tbody>
    ...
    <tr>
      <td>Tecnología</td>
      <td>LPDDR4 con ECC integrado en el chip</td>
    </tr>
    <tr>
      <td>eMMC</td>
      <td>Capacidad</td>
    ...
    <tr>
      <td>Expansión de IO<br />interna</td>
...
:::note
Presta mucha atención a la orientación del reTerminal en el diagrama anterior. La pantalla LCD y los botones integrados están en el lado derecho, mientras que la parte trasera del reTerminal está en el lado izquierdo. Además, todo el dispositivo está volteado boca abajo.
:::
...
Debes preparar el siguiente hardware antes de empezar con reTerminal
...
### <span id="jump2">Flashear Raspberry Pi OS/ Ubuntu OS de 64 bits u otro OS en la eMMC</span>

Podemos flashear **Raspberry Pi OS/ Ubuntu OS de 64 bits u otro OS** en el **almacenamiento eMMC** del CM4 en el reTerminal. Una vez que se instalen los controladores necesarios, solo tienes que conectar el puerto USB Tipo C del reTerminal a tu PC, y se mostrará como una unidad externa. Comienza retirando la carcasa trasera del reTerminal.
...
- **Paso 1.** Descarga el **instalador de configuración de rpiboot** haciendo clic en **[este enlace](https://github.com/raspberrypi/usbboot/raw/master/win32/rpiboot_setup.exe)** para instalar los controladores necesarios y la herramienta de arranque
...
Ahora puedes saltar **[aquí](#iniciar-sesión-en-raspberry-pi-os-ubuntu-os-u-otro-os-usando-ssh-a-través-de-wi-fi-ethernet)**
...
- **Paso 7.** Conecta reTerminal al PC mediante un cable USB Tipo‑C
...
Aquí puedes **establecer un nombre de host, habilitar SSH, establecer una contraseña, configurar Wi‑Fi, establecer la configuración regional** y más
...
- **Paso 14.** Vuelve a poner el **interruptor de modo de arranque (Boot Mode)** en la posición original y ensambla la carcasa del reTerminal

### Iniciar sesión en Raspberry Pi OS/ Ubuntu OS u otro sistema operativo usando SSH a través de Wi‑Fi/ Ethernet

Si quieres iniciar sesión en Raspberry Pi OS usando **SSH a través de Wi‑Fi/ Ethernet**, puedes seguir los pasos a continuación. Primero configura Wi‑Fi/Ethernet en el reTerminal y luego haz SSH desde Windows/Mac/Linux.

**Nota:** Si has configurado Wi‑Fi y habilitado SSH usando Raspberry Pi Imager, puedes omitir los pasos 1 - 4
...
- **Paso 3.** Introduce la contraseña de la siguiente manera
...
### <span id="jump1">Instalar los controladores de reTerminal después de flashear un nuevo Raspberry Pi OS/ Ubuntu OS u otro sistema operativo</span>

reTerminal viene con los controladores necesarios preinstalados de fábrica, por lo que no necesitas instalar controladores adicionales. Sin embargo, si flasheas un nuevo sistema operativo por tu cuenta, debes instalar los controladores necesarios por separado. El hardware que necesita controladores incluye la **pantalla LCD de 5 pulgadas, panel táctil, acelerómetro, sensor de luz, LEDs de usuario, botones de usuario, RTC, zumbador, coprocesador criptográfico**.

:::note
Para **sistemas operativos de 32 bits** necesitarás añadir el siguiente paso antes de ejecutar `sudo ./scripts/reTerminal.sh`. Después de completar la instalación, recuerda **reiniciar** reTerminal para que los cambios surtan efecto.
...
Mientras tanto, si estás utilizando la última Raspberry Pi OS oficial de 32 bits, su nuevo kernel es incompatible con este producto, lo que dará lugar a errores en la actualización de los controladores y a una pantalla en blanco. Si realmente necesitas una imagen de sistema operativo de 32 bits para desarrollo, recomendamos usar una versión de imagen de 32 bits más antigua.
:::
...
### Flashear Raspberry Pi OS con controladores de Seeed en la eMMC

- **Paso 1.** Como en los pasos normales de grabación de una imagen, primero debes seleccionar el modelo de dispositivo:
...
| --------------------------- | ---------------- | ------------------------------------------------------------ |
| [2024-03-14-Raspbian-reTerminal-arm64](https://files.seeedstudio.com/wiki/ReTerminal/RPiOS_Images/2024-03-14-Raspbian-reTerminal/2024-03-14-Raspbian-reTerminal-arm64.zip)              | Debian12(bookworm) de 64 bits, versión de Linux 6.6.20 | lastest |
| [2024-03-14-Raspbian-reTerminal-armhf](https://files.seeedstudio.com/wiki/ReTerminal/RPiOS_Images/2024-03-14-Raspbian-reTerminal/2024-03-14-Raspbian-reTerminal-armhf.zip) |  Debian12(bookworm) de 32 bits, versión de Linux 6.6.20           | lastest |
| [2022-07-21-Raspbian-reTerminal-arm64](https://files.seeedstudio.com/wiki/ReTerminal/RPiOS_Images/2022-07-21-Raspbian-reTerminal-arm64/64bit-20220721T012743Z-001.zip)               | Debian11(Bullseye) de 64 bits, versión de Linux 5.15.32-v7+  | release |
| [2022-07-21-Raspbian-reTerminal-armhf](https://files.seeedstudio.com/wiki/ReTerminal/RPiOS_Images/2022-07-21-Raspbian-reTerminal-arm64/32bit-20220721T012743Z-001.zip)                    | Debian11(Bullseye) de 32 bits, versión de Linux 5.15.32-v7+  | release |
...
Si instalas la última versión bookworm de la imagen, no la configures haciendo clic en el botón de opciones avanzadas. No entres en esta interfaz para ninguna configuración inicial.
¡De lo contrario, pueden producirse situaciones inesperadas de error de controladores!
...
Consulta el siguiente vídeo sobre cómo desmontar reTerminal:
...
Para las preguntas frecuentes, [haz clic aquí](https://wiki.seeedstudio.com/es/reTerminal-FAQ) para visitar el wiki de Preguntas Frecuentes sobre el uso de reTerminal
...
- **[Web Page]** [Documentación oficial de Raspberry Pi](https://www.raspberrypi.org/documentation/)
