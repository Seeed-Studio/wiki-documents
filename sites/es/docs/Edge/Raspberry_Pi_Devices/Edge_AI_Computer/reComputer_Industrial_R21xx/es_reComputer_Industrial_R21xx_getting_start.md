...
El reComputer Industrial R21xx no viene con un botón de encendido por defecto, y el sistema se iniciará automáticamente una vez que se conecte la alimentación. Al apagar, por favor selecciona la opción de apagado en el sistema operativo y espera a que el sistema se apague completamente antes de cortar la alimentación. Para reiniciar el sistema, simplemente reconecta la alimentación.
...
El reComputer Industrial R21xx cuenta con 4 indicadores LED que sirven para señalar el estado operacional de la máquina. Por favor consulta la tabla a continuación para las funciones específicas y el estado de cada LED:
<div class="table-center">
  <table border="1" cellspacing="0" cellpadding="6">
    <thead>
      <tr>
...
El reComputer Industrial R21xx contiene un zumbador activo, que puede ser usado para varios propósitos como alarmas y notificaciones de eventos. Ingresa en la terminal del reComputer Industrial R21xx:
...
> Nota
> Para detalles sobre las pruebas de DI, consulta la sección [enlace](https://wiki.seeedstudio.com/es/recomputer_industrial_R21xx_configure_system/di-(digital-input)-testing).
...
El reComputer Industrial R21xx está equipado con 1x puerto USB Type-C y 4x puertos USB Type-A. Consulta la tabla a continuación para sus funciones y descripciones.
...
> **Nota**  
> Tenga en cuenta que la versión estándar del reComputer Industrial R2000 no incluye un módulo 5G/4G.
> Si requiere funcionalidad 5G/4G, debe adquirir por separado un módulo 5G/4G adicional.
...
Hay un Mini Interruptor de Botón ubicado en el orificio de reinicio del reComputer AI Industrial R2135. Al presionar este botón con un objeto delgado, el CM4 puede ser reiniciado. Este pin cuando está alto señala que el CM4 ha iniciado. Llevar este pin a bajo reinicia el módulo.
...
El reComputer Industrial R2000 viene con dos puertos Ethernet RJ45. ETH0 es una interfaz CM5 Gigabit Ethernet nativa que soporta tres velocidades diferentes: 10/100/1000 Mbit/s. Puede habilitar la entrega de power-over-Ethernet (PoE) a través de esta interfaz, proporcionando energía al reComputer Industrial R2000. El otro ETH1 soporta 10/100 Mbit/s que es convertido desde USB.
...
El reComputer Industrial R2000 cuenta con un circuito RTC alimentado por el condensador, permitiéndole mantener la funcionalidad de cronometraje incluso en caso de pérdida de energía.
...
La placa base reComputer Industrial R21xx cuenta con dos ranuras Mini-PCIe. La ranura Mini-PCIe 1 soporta módulo 4G, módulo LoRa® usando el protocolo USB y módulo Zigbee usando protocolo USB; mientras que la ranura Mini-PCIe 2 soporta módulo LoRa® usando el protocolo USB y SPI y módulo Zigbee usando protocolo USB. Además, el módulo 4G y el módulo LoRa® no deben usarse al mismo tiempo, no se pueden conectar dos módulos LoRa® en la placa.
...
El reComputer Industrial R21xx está alimentado por el CM5 con una versión Wi-Fi/BLE integrada, proporcionando los mismos parámetros Wi-Fi/BLE que el CM5. Para información detallada de parámetros, consulte el sitio web oficial de Raspberry Pi.  

### Módulo 4G

La placa base reComputer Industrial R21xx cuenta con una ranura M.2 B-KEY y una ranura Mini-PCIe, ambas ranuras M.2 B-KEY y Mini-PCIe soportan un módulo 4G. El módulo 4G EC25 de Quectel ha sido completamente probado para ser compatible con el reComputer Industrial R21xx.

### Módulo 5G

La placa base reComputer Industrial R21xx cuenta con una ranura M.2 B-KEY, soportando un módulo 4G/5G usando el protocolo USB. El módulo 4G EC25 de Quectel ha sido completamente probado para ser compatible con el reComputer Industrial R21xx.

> Nota
> Tenga en cuenta que si requiere funcionalidad 4G/5G, es necesario comprar el módulo 4G correspondiente y antena externa, y seguir las instrucciones en la sección 4.5 "Ensamblar Módulo 4G/LoRa®/Zigbee y Antena".
...
> Nota
> Tenga en cuenta que si requiere funcionalidad LoRa®, es necesario comprar el módulo LoRa® correspondiente y antena externa, y seguir las instrucciones en la sección 4.5 "Ensamblar Módulo 4G/LoRa®/Zigbee y Antena".
...
El TPM cuenta con el OPTIGA™ TPM SLB9670 de Infineon que cumple con la especificación TPM 2.0 del Trusted Computing Group (TCG) y se recomienda como chip de encriptación para el reComputer Industrial R2000. El chip cuenta con una interfaz SPI aplicada para el puerto J26 en la placa, para habilitar una raíz de confianza para integridad de plataforma, atestación remota y servicios criptográficos.
...
El UPS es 7F, que opera en serie. El módulo UPS está posicionado entre los componentes DC5V y CM5, con una señal GPIO utilizada para alertar a la CPU en caso de pérdida de energía del suministro de 5V. Al recibir esta señal, la CPU ejecuta un script urgente antes de que se agote la energía del supercondensador, iniciando un comando "$ shutdown".
La duración de respaldo proporcionada por el UPS depende en gran medida de la carga del sistema. A continuación se muestran algunos escenarios típicos probados con un módulo CM5 con 4GB de RAM, almacenamiento eMMC de 32GB y un módulo Wi-Fi.
