---
description: Crea una imagen Yocto completa de desarrollo CUDA desde el repositorio de Seeed y flaséala en un reComputer Super.
title: Crear y flashear una imagen Yocto para reComputer Super
keywords:
  - Yocto
  - OpenEmbedded
  - reComputer Super
...
url: https://wiki.seeedstudio.com/es/build_and_flash_yocto_for_recomputer_super/
---

# Crear y flashear una imagen Yocto para reComputer Super

Esta guía compila una imagen Yocto de Seeed para **reComputer Super J401 con un módulo Jetson Orin NX de 16 GB** y la flashea en la unidad NVMe del Jetson.

El ejemplo utiliza `seeed-image-jetson-development`. Esta imagen proporciona un entorno de desarrollo en el lado del objetivo con CUDA Toolkit y `nvcc`, archivos de desarrollo CUDA/cuDNN/TensorRT/VPI/OpenCV, herramientas de compilación y depuración, ejemplos de NVIDIA y paquetes de prueba.
...
:::note
Este es un sistema OpenEmbedded/Yocto, no un sistema de archivos raíz Ubuntu instalado por NVIDIA SDK Manager. La gestión de paquetes, el contenido del sistema de archivos y el entorno de escritorio difieren de JetPack Ubuntu.
:::
...
- Acceso a `sudo` para instalar paquetes en el host y flashear
...
Si BitBake indica que la distribución del host no es compatible, utiliza un host Linux compatible con Yocto en lugar de ignorar la advertencia de validación del host.

## SKU de módulo utilizado en esta guía
...
Esta guía utiliza `0000`. Utiliza siempre un directorio de compilación separado para cada combinación de carrier y SKU de módulo.
...
La primera compilación recomendada utiliza `build.sh all`. Valida los metadatos, compila los device trees de Seeed, comprueba los archivos de arranque y compila la imagen completa y el archivo tegraflash en orden.
...
Compila la imagen de desarrollo en el lado del objetivo:
...
El archivo `.tegraflash-tar.zst` es el archivo autocontenido que se utiliza para flashear.
...
El asistente verifica el SKU del módulo y comprueba que la imagen rootfs, el DTB, el BPMP DTB, la configuración de pinmux, la configuración de voltaje de pines y los scripts de flasheo estén presentes. Luego muestra el directorio preparado y el siguiente comando exacto.

:::tip
Utiliza un directorio de salida nuevo o vacío en el SSD local del host. No extraigas el paquete de flasheo en la unidad de destino del Jetson.
:::
...
4. Conecta el puerto de dispositivo/depuración USB Tipo C al host Linux con un cable de datos.
...
Para el módulo Orin NX de 16 GB utilizado en esta guía, la salida debe incluir un dispositivo similar a:

```text
0955:7323 NVIDIA Corp. APX
```

Otros módulos Super compatibles utilizan diferentes ID de producto USB:
...
Verifica el entorno de desarrollo en el lado del objetivo:
...
La sección anterior utilizó `all`, que es la ruta recomendada para una primera compilación. Para depurar o recompilar una etapa, ejecuta el mismo flujo de trabajo explícitamente:
...
Si el sistema de archivos raíz ya está compilado y solo es necesario regenerar el archivo tegraflash, utiliza:
...
La imagen de desarrollo flasheada ya admite la compilación directamente en el Jetson. Compila el SDK de host solo cuando un PC x86_64 deba compilar aplicaciones de forma cruzada para el objetivo:
...
### No se puede encontrar el archivo de flasheo
...
`prepare-flash.sh` usa por defecto `demo-image-full`, por lo que omitir `--image seeed-image-jetson-development` haría que buscara el archivo de flasheo equivocado.

### El flasheo se detiene en `Waiting for USB storage device flashpkg`

En este punto, el host está esperando a que el initrd de Jetson exponga un dispositivo de almacenamiento masivo USB temporal. Comprueba el cable USB, usa un puerto USB de la placa base, retira dispositivos de almacenamiento USB innecesarios y confirma que el Jetson sigue siendo visible con `lsusb`.
