...
Después de compilar el programa ejecutable motion_detection en un sistema Linux, súbelo a reCamera mediante SCP. Luego, tras acceder de forma remota a la terminal SSH de reCamera, ejecuta el programa directamente usando el siguiente comando:
...
Después de reiniciar la interfaz de Windows, es posible que se pierdan los cuadros ya seleccionados. Necesitas hacer clic en "Read from Device" para recuperar los cuadros de detección existentes. Si la cámara actual se mueve o la escena cambia y no necesitas modificar el área de detección, puedes hacer clic en "Reconstruct Background" para redefinir la imagen estática.
