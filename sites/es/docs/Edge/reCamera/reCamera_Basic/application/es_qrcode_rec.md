---
description: Este ejemplo demuestra cómo realizar reconocimiento de códigos QR en tiempo real en reCamera y enviar fotogramas de vídeo y resultados de reconocimiento a un PC mediante UDP para su visualización.
...
url: https://wiki.seeedstudio.com/es/reCamera/reCamera_Basic/application/recamera_qrcode_udp_bak/
---
...
Este ejemplo muestra cómo configurar un **sistema de reconocimiento de códigos QR en tiempo real** usando reCamera. El sistema captura fotogramas de vídeo en tiempo real con la cámara, decodifica códigos QR usando la biblioteca **Quirc** y envía los fotogramas de vídeo y los resultados de reconocimiento a un PC para su visualización mediante los protocolos **RTSP** y HTTP.
...
- Script de Python en el PC para visualización en tiempo real de vídeo y resultados
...
El directorio del repositorio después de clonar se verá así:
```text
...
El directorio `main` contiene el código fuente que se ejecutará en recamera, y `recamera_qr_win_client` contiene el código fuente para la visualización de resultados que se ejecutará en dispositivos x86. Antes de compilar, debes modificar el directorio especificado en el archivo `CMakeLists.txt` en el directorio principal para que apunte al directorio donde descomprimiste el paquete precompilado descargado en el paso anterior.
