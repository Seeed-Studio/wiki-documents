...
url: https://wiki.seeedstudio.com/es/reCamera/reCamera_Basic/application/use_cpenclaw_to_control_the_recamera_gimbal_bak/
---
...
Después de la instalación, puedes entrar directamente en la página web para intentar chatear con él. Si en este momento puedes obtener respuesta con normalidad, entonces has instalado y configurado correctamente OpenClaw. Continúa paso a paso. Si hay algún problema con la instalación de OpenClaw, por favor averigua la causa y resuélvelo por tu cuenta.
...
### 2.2 Descarga el proyecto reCamera Gimbal en GitHub
...
openclaw.json se encuentra en la carpeta donde instalaste OpenClaw. Este archivo contiene todas las configuraciones que necesitas para acceder al modelo de IA. Lo que tienes que hacer es añadir el siguiente contenido sobre reCamera Gimbal a openclaw.json:
...
Si has configurado las Skills según el paso 2, en la interfaz de OpenClaw, después de hacer clic en "Skills" en el lado izquierdo y luego en "WORKSPACE SKILLS", deberías poder ver el "recamera-gimbal" que has configurado localmente"
...
Asegúrate de que tu modelo esté configurado correctamente. Después de hacer clic en "Agent" en el lado izquierdo, podrás ver el modelo que has configurado en "Primary model". Aquí estoy usando el modelo GLM4.7.
