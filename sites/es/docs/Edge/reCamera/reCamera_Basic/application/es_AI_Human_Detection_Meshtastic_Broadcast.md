...
Ahora configuraremos el script `udp_receiver.py` en tu PC. Asegúrate de que tu PC tenga instalado Python y las bibliotecas **OpenCV** y **NumPy**. Luego, descarga el script `udp_receiver.py` desde el enlace [udp_receiver.py](https://files.seeedstudio.com/wiki/reCamera/Applications/Meshtastic/udp_receiver.py) y ejecútalo en tu PC. En este ejemplo, se ejecuta en Windows Powershell:
...
2.Haz clic en "Serial", luego haz clic en "New Device" y selecciona el puerto de tu nodo Mesh.
...
3.Entra en la página de configuración del dispositivo; aquí, el nombre de mi dispositivo es "Meshtastic ff28". 
...
7.Habilita este módulo y luego establece el intervalo de transmisión en 15 segundos.
...
10.Configura 'Role' en 'SECONDARY' y selecciona '128 bit' para el formato de Pre-Shared Key. Luego haz clic en 'Generate' para crear una Pre-Shared Key aleatoria de 128 bits. **Recuerda esta clave y guárdala en un archivo de texto para usarla más tarde.** Además, establece el Name en 'reCamera'.
