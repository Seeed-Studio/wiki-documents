---
description: Proporcionar un tutorial para integrar reCamera con Home Assistant. Home Assistant está instalado en reComputer.
...
Home Assistant es un sistema de plataforma de hogar inteligente de código abierto, y el usuario puede conectar fácilmente sus propios dispositivos de Internet de las Cosas al sistema para una gestión unificada. Sin embargo, el principal punto especial de Home Assistant no es solo realizar la visualización de varios dispositivos de detección, sino lograr el enlace automático de dispositivos multiplataforma; solo una plataforma de hogar inteligente así es suficientemente vital.
...
## Instalación de Home Assistant (tomando como ejemplo el reComputer AI R2130-12)

### Preparación del sistema

Si Home Assistant ya ha sido instalado, entonces por favor ve directamente al paso "Find the path of the configuration file" para la configuración posterior.
...
2.Permitir que el usuario actual use Docker
...
Con el fin de persistir el archivo de configuración:
...
(este paso puede tardar un rato, la velocidad depende de tu red)
...
Después de iniciar Docker, espera de 1 a 2 minutos y luego accede desde el navegador:
...
El arranque inicial te llevará a la interfaz de inicialización. A continuación, puedes empezar a crear una cuenta y configurar tu hogar inteligente.
...
Puedes verlo mediante las siguientes instrucciones:
...
### Configurar la automatización para recibir datos de reCamera
...
## La configuración de reCamera (tomando como ejemplo la versión 2002w)

### Después de insertar la reCamera en el ordenador, accede a 192.168.42.1

Si la interfaz no se puede abrir de forma continua, consulta la sección [Factory Reset](https://wiki.seeedstudio.com/es/recamera_getting_started/#factory-reset) para restablecer el dispositivo. Después del restablecimiento, visita de nuevo 192.168.42.1. Tras el restablecimiento, se te pedirá que cambies la contraseña.

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/reset.png" /></div>

### Después de iniciar sesión, haz clic en la esquina inferior derecha para entrar en "Workspace"

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/logging_in.png" /></div>

### Después de entrar en el Workspace, configura los nodos.
...
Se han añadido cuatro nodos nuevos: "debug1", "debug2", "function1" y "HTTP request".  
debug1 y debug2 se utilizan para observar la información de salida, son opcionales; function1 se utiliza para procesar datos; el nodo HTTP request se utiliza para enviar datos a Home Assistant

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/workspace_ha_3.png" /></div>

A continuación, necesitamos modificar el nodo original "Model Info Handle" para que solo emita la cantidad de un objetivo de detección específico.

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/workspace_ha_4.png" /></div>

Haz doble clic para abrir este nodo y sustituye el código por el siguiente contenido:
...
Este código solo muestra el número de personas detectadas. Si deseas hacer cambios, puedes modificar el objeto de detección.  
Puedes saber qué tipos de objetos se pueden detectar comprobando el nodo del modelo.
...
| **Dirección URL**| `http://<your IP>:8123/api/webhook/recamera_detection`|
| **Respuesta**| Objeto JSON|

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/workspace_ha_8.png" /></div>

Después de configurarlo, haz clic en el botón "Deploy" en la esquina superior derecha. Una vez que la implementación sea exitosa, aparecerá un aviso.

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/workspace_ha_9.png" /></div>

Aquí, necesitas realizar un paso más, que es conectar la WiFi a reCamera. Debe estar en la misma red que la que configuraste para reComputer (ten en cuenta que la versión POE no tiene funcionalidad WiFi y requiere el uso de un switch con función POE para conectarse en el mismo segmento de red).  
...
## Ver la interfaz general de Home Assistant

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/HA.png" /></div>

Si aún no se ha observado ningún efecto, primero comprueba si el enlace no está conectado correctamente. Es decir, verifica si el Webhook está funcionando correctamente:  
...
Si tiene éxito, la notificación de Home Assistant mostrará:
