---
description: Aprende a implementar un sistema bilingüe de reconocimiento de voz en streaming activado por la mirada y totalmente en el dispositivo en reCamera Pro, con vistas de estado en terminal, web y RTSP.
title: Despertar visual y reconocimiento de voz sin conexión
keywords:
  - reCamera Pro
  - RV1126B
  - RKNN
...
# Crea un sistema de despertar visual y reconocimiento de voz sin conexión con reCamera Pro
...
El código fuente y las futuras versiones están disponibles en el [repositorio de GitHub recamera_pro_face_stt](https://github.com/yyling0101-a11y/recamera_pro_face_stt). El repositorio contendrá el proyecto completo después de que se suba este directorio.

## Características y flujo de procesamiento
...
Copia el **contenido** del directorio de despliegue en un único directorio de la reCamera Pro. Para un dispositivo en `192.168.42.1`:
...
Durante el funcionamiento normal, los eventos relevantes aparecen en el terminal:
...
| `--fps N` | `30` | Frecuencia de fotogramas de cámara solicitada; la tasa de procesamiento depende del tiempo de inferencia. |
...
| `--no-stt` | STT habilitado | No iniciar el reconocimiento de voz; probar solo la máquina de estados de despertar visual. |

#### Reconocimiento de voz y endpointing
...
| `--audio-channel N` | `0` | Canal a usar; establece `-1` para promediar todos los canales. |
...
| `--stt-start-timeout-ms N` | `4000` | Tiempo de espera si el habla no comienza después del despertar visual, en milisegundos. |
...
| `--rtsp-fps N` | `15` | Frecuencia de fotogramas de salida RTSP declarada; la tasa real depende del rendimiento de inferencia. |
...
| `--dashboard` | Off | Mostrar un panel en el terminal interactivo local. No se puede combinar con `--debug`. |
| `--debug` | Off | Imprimir registros de inicio, por fotograma de pose, nivel de audio, RTSP, contracción de tensores y rendimiento de STT. No se puede combinar con `--dashboard`. |

## Dos formas de monitorizar el sistema

### Panel en terminal

Usa `--dashboard` en un terminal interactivo:

```bash
./visual_wake_app --dashboard
```

El terminal debe tener al menos 80×18 caracteres. El panel izquierdo muestra el estado de STT, texto parcial y resultados finales; el panel derecho muestra el recuento de rostros, el estado de orientación hacia la cámara, los ángulos de pose, el estado de los clientes RTSP y la latencia de visión. Pulsa `Ctrl-C` para restaurar el terminal normal. `--dashboard` no se puede combinar con `--debug`.
...
La superposición muestra cajas de rostro, los 98 puntos de referencia, yaw/pitch sin procesar, `facing=YES/NO` y el estado actual. Verde significa que la pose está dentro de los umbrales de entrada, naranja significa que es válida pero está fuera de los umbrales, y rojo significa que la pose no es válida.
