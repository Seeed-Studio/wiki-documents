---
description: Instala y utiliza la reCamera Pro Development Skill para convertir modelos ONNX, compilar aplicaciones nativas RKNN y desarrollar flujos de trabajo de cámara, audio, GStreamer y RTSP mediante conversaciones en lenguaje natural.
title: Desarrollar con agentes de codificación de IA
keywords:
  - reCamera
  - reCamera Pro
  - AI coding agent
...
:::note
Esta skill está diseñada específicamente para **reCamera Pro con el SoC RV1126B y Linux aarch64**. No está pensada para la plataforma reCamera SG2002/riscv64.
:::
...
La skill utiliza el siguiente contrato de destino al desarrollar aplicaciones de IA nativas:
...
### Selecciona tu agente de codificación de IA

Clona la rama que coincida con el agente que utilizas:

| Agente de codificación de IA | Rama | Directorio de skill a nivel de usuario |
| --- | --- | --- |
...
Si tu agente admite instalar skills desde un repositorio Git, envíale la siguiente solicitud:
...
Para utilizar otro agente compatible, sustituye `main` por el nombre de su rama de la tabla anterior. El script de instalación específico de la rama copia la skill en el directorio de skills a nivel de usuario de ese agente.

Reinicia el agente después de la instalación si la skill no aparece en la sesión actual.

:::tip
Conserva el repositorio clonado. Para actualizar la skill más adelante, extrae los últimos cambios de la misma rama del agente y sigue el README y las instrucciones de instalación de esa rama.
:::

## Usar la skill

Puedes nombrar explícitamente la skill en tu prompt:
...
Después de instalar la skill, también puedes describir la tarea de forma natural. El agente cargará la guía relevante de conversión de modelos, compilación cruzada, cámara, audio o streaming.
...
La skill incluye una plantilla mínima de aplicación con RKNN Runtime y GStreamer. El agente adapta el preprocesamiento y el posprocesamiento al modelo en lugar de tratar la plantilla como un detector completo.
...
3. Convertir el modelo con RKNN-Toolkit2 2.3.2 para `rv1126b`; utilizar datos de calibración representativos para INT8.
...
6. Verificar que el ejecutable generado es aarch64 y que sus rutas de RKNN, GStreamer, libc, ABI de C++ y tiempo de ejecución son compatibles con el dispositivo.
...
De forma predeterminada, la skill compila e inspecciona artefactos localmente. La transferencia de archivos y la ejecución en la reCamera Pro solo se realizan cuando tú lo solicitas y autorizas explícitamente.
...
- Utiliza la API C de RKNN Runtime para aplicaciones nativas C/C++. `rknn-toolkit-lite2` no se utiliza en este flujo de trabajo.
- No enlaces bibliotecas x86_64 del ordenador de desarrollo en una aplicación de destino aarch64.
...
- La aplicación se vincula contra un RKNN Runtime compatible con la placa y utiliza el runtime existente del dispositivo; no debe reemplazar el RKNN Runtime instalado en la reCamera Pro.

## Solución de problemas

| Problema | Posible causa | Solución |
| --- | --- | --- |
| La skill no aparece en la lista | El agente no ha recargado su directorio de skills a nivel de usuario | Reinicia el agente y verifica que la skill se haya instalado en el directorio mostrado arriba |
...
| La conversión RKNN falla | La versión del toolkit, el soporte de operadores ONNX o el contrato de entrada del modelo son incompatibles | Pídele al agente que inspeccione primero el modelo ONNX y utilice RKNN-Toolkit2 2.3.2 |
...
- [Repositorio de reCamera Pro Development Skill](https://github.com/Seeed-Projects/recamera-pro-development-skill)
