---
description: Integra reCamera Pro con Home Assistant para enviar un mensaje y una instantánea de la cámara cuando se detecte un sonido específico, usando MQTT.
title: "Home Assistant: Alerta de foto activada por sonido"
keywords:
  - reCamera
  - reCamera Pro
  - Home Assistant
...
Descarga el [script install.py](https://drive.google.com/file/d/1nFBHJNkOUPqcAAUCYw43IhGhDWIYNhUX/view?usp=drive_link), ejecútalo e introduce el directorio de configuración de HA para completar toda la instalación automáticamente.
...
2. Copiar los archivos frontend a `www/recamera_chat/`
...
Después de la instalación, salta a [Paso 4: Reiniciar Home Assistant](#paso-4-reiniciar-home-assistant).
...
- **HA OS**: accesible vía Samba o SSH en `/config`
...
:::warning
El número de versión en `module_url` (por ejemplo, `?v=13`) debe coincidir con el número de versión en el archivo `panel.js`. Si los números de versión no coinciden, el navegador puede cargar una caché antigua, haciendo que el panel no se muestre.
:::
...
4. El lado de reCamera suscrito a `recamera/chat/to_camera` debería recibir el mensaje.
...
Después de detener el proceso principal, descarga el programa en Python: [reCamera Program](https://drive.google.com/file/d/1hQZNFvYzIFAPasy6_DWrxtMavjDmzrV_/view?usp=drive_link)
...
- **MQTT_MESSAGE**: mensaje a enviar cuando se active, puedes introducir el contenido que quieras mostrar en el panel de HA.
