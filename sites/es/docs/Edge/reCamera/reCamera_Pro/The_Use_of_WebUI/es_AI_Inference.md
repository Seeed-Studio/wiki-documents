...
url: https://wiki.seeedstudio.com/es/recamera_pro_ai_inference_legacy/
---
<!-- LEGACY PAGE (reCamera Pro wiki restructure, phase 2): this page has been superseded by Build_Your_App/configure_detection.md (https://wiki.seeedstudio.com/es/recamera_pro_ai_inference/), which now serves the original slug /recamera_pro_ai_inference. This file is kept for history as a draft (slug /recamera_pro_ai_inference_legacy) and is excluded from production builds. Do not link here. -->

## Índice de migración de contenido (Página heredada)

| Sección original | Nuevo destino |
| --- | --- |
| Feature Overview, Model Configuration, Inference Configuration, Real-time Monitoring | [Configure Detection](https://wiki.seeedstudio.com/es/recamera_pro_ai_inference/) (este slug) |
| Model Management, Upload Model | [Upload and Configure an RKNN Model](https://wiki.seeedstudio.com/es/recamera_pro_rknn_upload/) |
| SenseCraft Model Conversion | [SenseCraft ONNX-to-RKNN](https://wiki.seeedstudio.com/es/recamera_pro_sensecraft/) |
| Inference Output Configuration | [HTTP/UART Results](https://wiki.seeedstudio.com/es/recamera_pro_http_uart/), [MQTT](https://wiki.seeedstudio.com/es/recamera_pro_mqtt/) |
| Model format/compatibility summary | [Model Compatibility and Output Formats](https://wiki.seeedstudio.com/es/recamera_pro_model_compatibility/) |
| Web UI field reference | [Web UI Reference](https://wiki.seeedstudio.com/es/recamera_pro_webui_reference/) |
...
- **SenseCraft Model Conversion**: Convertir modelos ONNX a modelos RKNN que reCamera Pro puede ejecutar.
- **Inference Configuration**: Seleccionar el modelo a ejecutar, activar/desactivar la inferencia y configurar la frecuencia de inferencia.
- **Real-time Inference Monitoring**: Ver los resultados de salida del modelo y el estado de inferencia en tiempo real.
- **Inference Output Configuration**: Salida de los resultados de inferencia a sistemas externos mediante HTTP, MQTT o UART.
...
El área de Model Management se utiliza para mostrar los modelos de IA que ya se han subido o que están integrados en el dispositivo actual. La lista mostrará el archivo de modelo, el nombre del modelo, el framework de inferencia, el tipo de algoritmo, el tipo de tarea, la versión, el tamaño del archivo y las operaciones disponibles.
...
La página actual admite la subida de archivos de modelo RKNN. Después de subirlos, el modelo aparecerá en la lista de Model Management, y los usuarios podrán seguir configurando la información del modelo, las categorías y los parámetros de postprocesado.

La subida de modelos es adecuada para los siguientes escenarios:

- Uso de modelos de detección entrenados por el usuario
- Sustitución de modelos predeterminados
...
La página de Inferencia de IA proporciona una entrada a **View SenseCraft Models**. Los usuarios pueden entrar en la interfaz de conversión de modelos SenseCraft para convertir modelos ONNX en modelos RKNN que reCamera Pro puede ejecutar.

<img width={800} src="https://files.seeedstudio.com/wiki/reCamera-Pro/web_ui_wiki/msedge_FQNy2aeXpQ.png" />

En la interfaz de conversión de modelos, los usuarios pueden crear tareas de conversión, subir archivos de modelo ONNX y, opcionalmente, subir un conjunto de datos de cuantificación. Si no se proporciona un conjunto de datos de cuantificación, el sistema puede usar un conjunto de datos predeterminado para completar la conversión.
...
| ------------------ | ----------------------------------------- |
| Model Name         | Establece el nombre visible del modelo         |
| Framework          | Selecciona el framework de ejecución del modelo, por ejemplo, RKNN |
| Version            | Establece la información de versión del modelo           |
| Type               | Establece el tipo de tarea del modelo, por ejemplo, Object Detection |
| Algorithm          | Selecciona el algoritmo del modelo, por ejemplo, YOLOv5 |
| Author             | Introduce la información del autor del modelo            |
| Description        | Añade una descripción del modelo                     |
| Detection Categories | Establece los nombres de las categorías de salida del modelo |
| Post-processing Configuration | Establece parámetros como IOU, Confidence, max_obj |

Para modelos de detección definidos por el usuario, las categorías de detección se pueden añadir manualmente en la página de configuración, o se puede importar una lista de categorías de forma masiva mediante un archivo TXT. Después de configurar las categorías, los resultados de inferencia se mostrarán y se exportarán de acuerdo con los nombres de categoría definidos por el usuario.
...
| Confidence  | Umbral de confianza de detección de objetos           |
| max_obj     | Número máximo de objetos a exportar por fotograma   |

Después de la configuración, haz clic en **Save** para aplicar la configuración del modelo.

## Configuración de inferencia

El área de Inference Configuration se utiliza para controlar el estado de ejecución y el modelo seleccionado de la tarea de inferencia de IA.
...
| Parámetro         | Descripción                               |
| ----------------- | ----------------------------------------- |
| Inference Enable  | Activa o desactiva la función de inferencia de IA   |
| Running Model     | Selecciona el modelo que se ejecutará actualmente         |
| Inference Frequency (FPS) | Establece la frecuencia de inferencia del modelo       |
| Inference Status  | Muestra si la tarea de inferencia actual se está ejecutando |
| Real-time Inference FPS | Muestra la tasa de fotogramas de inferencia real actual |
...
## Monitorización de inferencia en tiempo real

La página de Inferencia de IA proporciona un área de monitorización de salida de inferencia en tiempo real para ver los resultados de inferencia del modelo. Los resultados de inferencia se muestran en tiempo real como registros, lo que facilita a los desarrolladores depurar la salida del modelo, comprobar las coordenadas de las cajas delimitadoras, los ID de clase, las puntuaciones de confianza y las marcas de tiempo.
...
Esta página proporciona funciones de control de salida en tiempo real, como deshabilitar la monitorización, pausar la visualización en tiempo real, limpiar registros, etc., lo que facilita a los usuarios observar y gestionar el contenido de salida durante la depuración.

## Configuración de salida de inferencia

Además de ver los registros de inferencia en tiempo real en la interfaz web, reCamera Pro también admite la salida de resultados de inferencia a sistemas externos. Los usuarios pueden seleccionar la plantilla de salida, el tipo de tarea y el método de salida en **Inference Output Configuration**.
