...
# Solucionar problemas de llamadas API
...
Varios intentos fallidos de inicio de sesión han activado un bloqueo basado en IP. Espera `sWaittime` segundos hasta que el bloqueo caduque y luego inténtalo de nuevo.
...
Es posible que la versión actual del firmware no implemente ese endpoint, o que la ruta esté construida de forma incorrecta. Verifica si la ruta necesita `/cgi-bin/entry.cgi` y comprueba la versión del firmware.

Para la lista completa de códigos de estado de respuesta, consulta [Códigos de error de la API](/es/recamera_pro_api_error_codes/). Para el uso general de la API, comienza con la [Introducción a la API](/es/recamera_pro_api_introduction/).
