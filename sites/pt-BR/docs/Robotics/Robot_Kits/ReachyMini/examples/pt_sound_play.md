...
- **`--wav`**: Reproduz um arquivo WAV usando a API `play_sound()`.
- **`--live`**: Envia um tom senoidal contínuo usando a API de baixo nível `push_audio_sample()`, útil para fontes de áudio em tempo real, como mecanismos de conversão de texto em fala ou entrada de microfone.
...
- `--live`: Envia um tom senoidal contínuo.
- `--tone-hz <freq>`: Frequência da onda senoidal em Hz (somente no modo `--live`, padrão: 440).
- `--backend`: Backend de mídia a ser usado (`default`, `local` ou `webrtc`).

Veja o exemplo completo em: [sound_play.py](https://github.com/pollen-robotics/reachy_mini/tree/main/examples/sound_play.py)