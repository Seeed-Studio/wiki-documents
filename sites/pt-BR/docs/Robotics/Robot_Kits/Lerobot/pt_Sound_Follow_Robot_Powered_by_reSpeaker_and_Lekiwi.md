---
description: Este wiki fornece uma estrutura de implementação abrangente para equipar o Lekiwi Robot Kit com capacidades de seguir som usando ReSpeaker Mic Array v2.0 e reComputer Jetson Mini, cobrindo integração de hardware, configuração de ambiente e adaptação de software, enquanto demonstra a funcionalidade de rastreamento de fonte de áudio em tempo real.
...
1. Consulte [este wiki](https://wiki.seeedstudio.com/pt-br/lerobot_lekiwi/) para preparar o robô Lekiwi, incluindo a montagem da estrutura mecânica e a configuração dos motores.
2. Use conectores impressos em 3D para montar o reSpeaker no Lekiwi.
3. Use cabos USB para conectar o reSpeaker e o controlador de servos ao reComputer.
...
### Dependências do reSpeaker

Instale as dependências do reSpeaker no ambiente virtual do lerobot.

**Passo 1.** Instalar `pyusb`

```bash
pip install pyusb==1.0.2
```

**Passo 2.** Configurar permissões de acesso ao dispositivo USB
...
Como nossa configuração de hardware não utiliza uma câmera nem o braço robótico SOArm101, precisamos comentar as configurações relacionadas.
...
Modifique a função send_action para desativar/remover as seções de código relacionadas ao braço robótico.
