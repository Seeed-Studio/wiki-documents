...
Use ros launch para iniciar o nó, porque o arquivo de launch passa a configuração de hardware de `rebotarm_bringup/config`.
...
| `/rebotarm/gripper/command` | `control_msgs/action/GripperCommand` | Ação padrão do gripper |
...
:::caution
Os tópicos de comando de baixo nível são destinados a depuração e experimentos. Eles não executam IK, planejamento de trajetória ou verificações de validade URDF. Para movimento em nível de aplicação, prefira serviços e ações como `/move_to_pose`, `/follow_joint_trajectory` e `/gripper/set`.
:::
...
Verifique os pacotes MoveIt e os pontos de entrada das demonstrações:
...
| `rebotarm_moveit_demos/config/draw_square_rs.yaml` | Parâmetros da demonstração de draw-square |
| `rebotarm_moveit_demos/config/pick_place_rs.yaml` | Parâmetros da demonstração de pick-place |
...
- Se o workspace foi carregado com source: `source install/setup.bash`
...
Em seguida, certifique-se de que o terminal atual carregou o ambiente ROS2 com source:
...
Se ainda assim não puder ser encontrado, verifique se o caminho de busca do Python atual contém o caminho de pacotes Python do ROS2:

```bash
python3 -c "import sys; print('\n'.join(sys.path))"
```

Após carregar o Jazzy com source, você deverá ver um caminho semelhante a `/opt/ros/jazzy/lib/python3.12/site-packages`. Se você usar Humble, substitua `jazzy` por `humble` nos comandos.
