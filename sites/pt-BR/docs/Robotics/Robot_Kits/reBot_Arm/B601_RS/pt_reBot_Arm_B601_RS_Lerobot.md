---
description: Este wiki fornece um guia completo para o reBot Arm B601-RS para implementar coleta de dados e treinamento dentro do framework LeRobot.
...
[reBot Arm B601-RS](https://wiki.seeedstudio.com/pt-br/rebot_b601_rs_getting_started/) é um projeto de braço robótico open source lançado pela Seeed, dedicado a reduzir a barreira para o aprendizado de inteligência incorporada. Nós abrimos todo o design estrutural e o código sem reservas, tornando a tecnologia de robótica acessível a todos.
...
O Ubuntu x86 não requer nenhuma outra configuração de dependência de vídeo. Continue com a etapa 5.
...
Em um dispositivo Jetson JetPack 6.0+, certifique-se de ter instalado o PyTorch e o Torchvision com suporte a GPU seguindo a etapa 5 deste [tutorial](https://github.com/Seeed-Projects/reComputer-Jetson-for-Beginners/tree/main/3-Basic-Tools-and-Getting-Started/3.5-Pytorch) antes de executar os seguintes comandos.
...
GPUs NVIDIA RTX série 50 exigem a versão de prévia do PyTorch com CUDA 12.8 ou posterior:
...
O B601-RS precisa ser calibrado apenas uma vez após a montagem. Aqui está o comando de calibração. Consulte a figura para a posição zero (garra totalmente fechada).
...
:::tip
**Observações de calibração do reBot 102 leader**:
...
Se esta for a primeira conexão, você poderá receber um erro informando que `/dev/ttyACM0` não pode ser encontrado. Isso ocorre porque o brltty está ocupando a porta serial. Execute as etapas a seguir:
...
<section className="rebot-step-item">
  <span className="rebot-step-number">3</span>
  <div className="rebot-step-content">
    <h4>Conceder permissões ao dispositivo USB</h4>
...
<section className="rebot-step-item">
  <span className="rebot-step-number">5</span>
  <div className="rebot-step-content">
    <h4>Executar o exemplo RealSense</h4>
...
<section className="rebot-step-item">
  ...
  <div className="rebot-step-content">
    <h4>Revisar as configurações de parâmetros</h4>
...
<section className="rebot-step-item">
  <span className="rebot-step-number">5</span>
  <div className="rebot-step-content">
    <h4>Executar o exemplo Orbbec</h4>
    <p className="rebot-step-label">Etapa 5</p>

Teste de Orbbec única:
...
<section className="rebot-step-item">
  ...
  <div className="rebot-step-content">
    <h4>Solucionar problemas comuns</h4>
...
<TabItem value="hugging-face-hub" label="Enviar para o Hugging Face Hub">
...
Se você estiver usando uma GPU RTX série 50, será necessário adicionar `--dataset.video_backend=pyav` para contornar APIs ausentes na versão de prévia do torchvision. O comando de treinamento se torna:
...
SmolVLA é um **modelo de base leve para robôs** fornecido pela Hugging Face. Ele foi projetado para permitir que você use seu próprio dataset gravado do LeRobot e **faça fine-tuning rapidamente** para obter resultados em robôs reais.

Simplificando, suas entradas/saídas são:

- Entrada: filmagens de múltiplas câmeras + estado atual do robô (sensores/juntas etc.) + uma instrução de tarefa em linguagem natural
- Saída: um bloco contínuo de ações para conduzir o braço robótico a executar a tarefa

```bash
pip install -e ".[smolvla]"
```

**Coletando datasets (recomendado)**

SmolVLA é um “modelo de base”. Para ter um bom desempenho na sua bancada, com suas câmeras, garra e objetos, normalmente é necessário fazer fine-tuning com seus próprios dados.
...
- Experiência: gravar apenas 25 episódios geralmente é insuficiente. Tanto a qualidade quanto a quantidade dos dados importam.

**Treinamento**

Use `smolvla_base` (o modelo pré-treinado de 450M) como ponto de partida e faça fine-tuning no seu dataset. O exemplo oficial treina por 20k passos; em uma única A100 isso leva cerca de 4 horas (apenas como referência; o tempo real varia conforme o hardware).

Se você não tiver uma GPU disponível, considere treinar via um notebook Colab (veja o tutorial oficial).
...
- Se você ficar sem memória, reduza primeiro `--batch_size`. Quando estiver rodando, aumente gradualmente.
- Para ver os parâmetros disponíveis: `lerobot-train --help`

**Avaliação**

A fase de avaliação carrega seu modelo com fine-tuning, permite que o robô execute a tarefa e grava o processo de avaliação como um novo dataset (para revisar vídeos e analisar resultados).
...
- `--robot.port`: Altere para a porta serial reconhecida na sua máquina (comumente `can0` ou `/dev/ttyACM0`).
- `--robot.id`: O ID do seu robô (deve corresponder ao que você usou durante calibração/gravação).
- `--robot.cameras`: Altere para o `index_or_path` da sua câmera real e garanta que as chaves das câmeras (por exemplo, `front`, `side`) correspondam exatamente às que você usou ao gravar o dataset.
- `--dataset.single_task`: Deve corresponder à descrição da tarefa usada ao gravar o dataset.
- `--dataset.repo_id`: O nome do dataset de saída da avaliação; se você estiver logado no Hugging Face, ele será criado/enviado para a sua conta.
- `--policy.path`:
  - Se o modelo estiver local: preencha com o caminho dos pesos dentro do diretório de saída do treinamento (por exemplo, `outputs/train/my_smolvla/checkpoints/last/pretrained_model`)
...
π₀ (Pi0) é um modelo de **Visão-Linguagem-Ação** proposto pela Physical Intelligence para um controle de robôs mais “geral”. Você pode entendê-lo assim: ele consegue tanto ver imagens de câmera quanto entender uma instrução em linguagem natural e então gerar ações para controlar o braço robótico.

Usá-lo no LeRobot é simples: basta definir o tipo de policy como `--policy.type=pi0` durante o treinamento (não é necessário repetir os conceitos gerais de treinamento/avaliação já abordados na seção ACT).
...
Parâmetros comuns (apenas os específicos de Pi0 / mais frequentemente ajustados):

- `--policy.pretrained_path=lerobot/pi0_base`: Modelo base. Oficialmente também é fornecido `lerobot/pi0_libero` (versão orientada ao dataset Libero); você pode tentar alternar com base na sua tarefa.
...
π₀.₅ (Pi0.5) também é um modelo de **Visão-Linguagem-Ação** proposto pela Physical Intelligence, que pode ser entendido como uma “versão aprimorada” de π₀, com foco em uma capacidade reforçada de **generalização em mundo aberto**: não apenas ter bom desempenho em cenários fixos vistos durante o treinamento, mas também concluir tarefas de forma mais confiável em novos ambientes, com novos objetos e novas disposições.

A “generalização” que ele aborda pode ser dividida grosseiramente em três níveis (exemplos para entendimento):

- **Camada física**: pegar uma colher/prato desconhecido sabendo como segurá-lo (cabo/borda) e operar em ambientes desorganizados.
...
Para usá-lo no LeRobot: basta definir o tipo de policy como `--policy.type=pi05`.
...
Parâmetros comuns (relacionados a Pi0.5):
...
- `--policy.normalization_mapping=...`: Se as estatísticas de normalização do seu dataset não corresponderem / estiverem ausentes, você pode usar esse mapeamento para forçar um método de normalização.

Se o seu dataset não tiver estatísticas de quantis (exigidas por algumas versões/formats), o projeto oficial também fornece uma abordagem com script de conversão: completar/convertar as estatísticas do dataset antes do treinamento (consulte a documentação oficial para detalhes).
...
GR00T N1.5 é um modelo base aberto fornecido pela NVIDIA para raciocínio robótico mais geral e aprendizado de habilidades. É um modelo de **cross-embodiment**: ele pode receber entradas multimodais como **linguagem** e **imagens**, e executar tarefas de manipulação em diferentes ambientes.

A chave para usá-lo no LeRobot é definir o tipo de policy para `--policy.type=groot`. Observação: o GR00T N1.5 atualmente possui requisitos de ambiente mais altos (depende de FlashAttention e requer uma GPU CUDA). Recomenda-se fazer o ACT / Pi0 funcionar primeiro antes de tentar o GR00T.
...
2. Instale o PyTorch (faixa de versão conforme os requisitos oficiais):
...
:::tip
Se a instalação do flash-attn falhar, geralmente está relacionada a (1) incompatibilidade de versão entre PyTorch/CUDA, (2) dependências de compilação ausentes ou (3) ambiente muito novo/muito antigo. Nesse caso, consulte primeiro a documentação oficial do GR00T e o guia de instalação do PyTorch.
:::

**Treinamento (Fine-tuning)**

A documentação oficial fornece um exemplo de treinamento multi-GPU (`accelerate launch --multi_gpu ...`). Se você tiver apenas uma GPU, também pode tentar executá-lo primeiro no modo de processo único (o suporte/detalhes de parâmetros estão sujeitos à documentação oficial).
...
- `--steps`, `--batch_size`: Etapas de treinamento e tamanho do batch. Modelos grandes são sensíveis à VRAM; se não rodar, reduza primeiro o `batch_size`.
...
Após o treinamento, você pode usar `lerobot-record` para avaliação/gravação como em outras policies. Para usuários de braço único reBot B601-RS, consulte o seguinte comando:
...
PEFT (Parameter-Efficient Fine-Tuning) é um conjunto de métodos e ferramentas de "adaptação eficiente em parâmetros" para adaptar grandes modelos pré-treinados a novas tarefas **sem atualizar todos os parâmetros do modelo**. Para as policies pré-treinadas do LeRobot (por exemplo, SmolVLA, π₀, etc.), você normalmente pode treinar apenas um pequeno número de parâmetros "adaptadores" (por exemplo, LoRA) para obter resultados próximos ao full fine-tuning, reduzindo ao mesmo tempo o uso de VRAM e o custo de treinamento.
...
- `--peft.r`: Rank do LoRA. Em geral, um rank mais alto significa maior capacidade de expressão, mas também mais parâmetros e uso de VRAM.

**Especificando Camadas para Injetar LoRA (Opcional)**

Por padrão, o PEFT normalmente injeta LoRA nas camadas de projeção mais críticas do modelo (por exemplo, `q_proj`, `v_proj` da atenção, etc.) e pode cobrir adicionalmente camadas de projeção relacionadas a estado/ação. Se você precisar direcionar camadas diferentes, use `--peft.target_modules` para especificar as camadas alvo.
...
A taxa de aprendizado do LoRA normalmente pode ser uma ordem de grandeza maior do que no full fine-tuning (experiência comum: ~10x). Por exemplo, o full fine-tuning comumente usa `1e-4`, enquanto o LoRA pode começar em `1e-3`; se você tiver decaimento de taxa de aprendizado (scheduler) habilitado, a taxa de aprendizado final também é comumente definida em torno de `1e-4` como referência.
...
| bf16 | Suportado apenas em GPUs mais recentes (arquitetura Ampere e mais novas) |

Se a sua GPU não suportar bf16, escolha fp16 na sua configuração do accelerate ou especifique explicitamente fp16 na linha de comando.
...
> **você pode pular esta seção e usar o Método Um (argumentos de linha de comando).**
...
Se você apenas **usar multi-GPU ocasionalmente**, ou se esta for sua primeira tentativa, **não há problema nenhum em não usá-lo**.
...
- Número de processos: Número de GPUs a usar (geralmente igual ao número de GPUs)
- IDs de GPU a usar: Pressione Enter diretamente (significa usar todas as GPUs)
- Precisão mista:
  - Preferir fp16
...
LeRobot não ajusta automaticamente a taxa de aprendizado ou os passos de treinamento com base no número de GPUs, para evitar alterar o comportamento de treinamento sem o conhecimento do usuário. Isso é diferente de outros frameworks de treinamento distribuído comumente usados.

Se você quiser **ajustar hiperparâmetros para treinamento multi-GPU**, precisa fazer isso manualmente seguindo estes passos.

**Impacto nos passos e estratégia de ajuste**

Como o uso de múltiplas GPUs aumenta o tamanho de batch efetivo (batch_size × num_gpus):
...
Portanto, ao treinar com múltiplas GPUs, você deve reduzir adequadamente o número de passos de treinamento.
...
Treinamento com duas GPUs (o tamanho de batch efetivo se torna 16):
...
Ao usar múltiplas GPUs, cada atualização de passo usa mais amostras.

Se você quiser manter a "velocidade de aprendizado" do modelo semelhante à de uma única GPU,
normalmente precisa aumentar a taxa de aprendizado proporcionalmente ao número de GPUs.
...
- Manter os passos de treinamento inalterados

Desde que o processo de treinamento seja estável, os resultados ainda são utilizáveis.

Para configurações mais avançadas e solução de problemas, consulte a documentação do [Accelerate](https://huggingface.co/docs/accelerate/index). Se você quiser aprender mais sobre treinamento em um grande número de GPUs, confira este excelente guia: [Ultrascale Playbook](https://github.com/huggingface/ultrascale-playbook).
</TabItem>

<TabItem value="async-inference" label="Inferência assíncrona (opcional)">

Sem inferência assíncrona, o fluxo de controle do LeRobot pode ser entendido como inferência sequencial/síncrona convencional: a política prediz um bloco de ações, depois o executa, e então espera pela próxima predição. Para modelos maiores, isso pode causar pausas perceptíveis enquanto o robô espera por novos blocos de ações. O objetivo da inferência assíncrona é permitir que o robô execute o bloco de ações atual enquanto pré-computa o próximo, reduzindo o tempo ocioso e melhorando a capacidade de resposta. A inferência assíncrona se aplica a políticas suportadas pelo LeRobot que produzem blocos de ações, como ACT, OpenVLA, Pi0, SmolVLA. Como a inferência e o controle real são desacoplados, a inferência assíncrona também permite usar máquinas mais poderosas para inferência do robô remotamente.
...
- **Inferência síncrona**: Prediz um bloco, executa um bloco; o robô terá intervalos ociosos esperando o próximo bloco ser inferido. Quando o modelo é maior e o poder de computação é insuficiente, o intervalo de inferência é significativo — o braço se move, depois pausa (inferência), depois se move novamente.
...
Este é o caso mais simples — o servidor escuta em 127.0.0.1, e o cliente também se conecta a 127.0.0.1:porta. Os exemplos de comando na documentação oficial seguem este cenário.

**2. Implantação em LAN**

O robô e as câmeras se conectam a um dispositivo leve, e o servidor de política é executado em outro dispositivo de alta computação na mesma LAN.
...
O servidor de política é executado em um host em nuvem publicamente acessível, e o cliente se conecta via rede pública.
Essa abordagem pode aproveitar GPUs mais potentes em hosts em nuvem. Com boas condições de rede, o tempo de ida e volta (latência de rede) pode ser relativamente pequeno em comparação com o tempo de inferência, mas isso depende do seu ambiente de rede real.
...
- `--chunk_size_threshold`: Limite de mesclagem entre blocos de ações antigos e novos. Quando o bloco antigo é executado até essa proporção, começa a mesclagem com o novo bloco.
...
**Etapa 5: Ajuste os parâmetros com base no comportamento do robô**
...
- `--actions_per_chunk`: Tamanho de cada bloco de ações. Se o movimento do robô estiver aos trancos/irregular, aumente esse valor; se a resposta do robô tiver um atraso perceptível, diminua esse valor.
