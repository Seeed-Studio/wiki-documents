...
(Ignore esta etapa em um PC.) Antes de continuar em um dispositivo Jetson JetPack 6.0+, instale o PyTorch e o Torchvision com suporte a GPU na etapa 5 deste [tutorial](https://github.com/Seeed-Projects/reComputer-Jetson-for-Beginners/tree/main/3-Basic-Tools-and-Getting-Started/3.5-Pytorch):
...
Entre eles, `repo_id` pode ser modificado livremente, e `push_to_hub=false`. Por fim, o conjunto de dados será salvo no diretório `~/.cache/huggingface/lerobot` na pasta home, onde a pasta `seeed_rebot_b601_rs/test` mencionada acima será criada.
...
:::tip
Se as teclas do teclado não estiverem respondendo, talvez seja necessário fazer o downgrade da sua versão do pynput, por exemplo, instalando a versão 1.6.8.
...
Se as teclas Right Arrow/Left Arrow/ESC não responderem durante a gravação:
...
SmolVLA é um **modelo base leve para robótica** fornecido pela Hugging Face. Ele foi projetado para permitir que você use seu próprio conjunto de dados LeRobot gravado e **faça o fine-tuning rapidamente** para obter resultados em robôs reais.
...
- Saída: um bloco de ação contínua para mover o braço robótico e executar a tarefa
...
- Se ficar sem memória, reduza primeiro `--batch_size`. Quando estiver funcionando, aumente-o gradualmente.
...
- `--robot.cameras`: Altere para a sua câmera real `index_or_path` e garanta que as chaves da câmera (por exemplo, `front`, `side`) correspondam exatamente ao que você usou ao gravar o dataset.
- `--dataset.single_task`: Deve corresponder à descrição da tarefa usada ao gravar o dataset.
- `--dataset.repo_id`: O nome do dataset de saída da avaliação; se você estiver logado no Hugging Face, ele será criado/enviado na sua conta.
- `--policy.path`:
...
π₀ (Pi0) é um modelo de **Visão-Linguagem-Ação** proposto pela Physical Intelligence para um controle de robô mais "geral". Você pode pensar nele assim: ele consegue tanto ver imagens da câmera quanto entender uma instrução em linguagem natural, e então gerar ações para controlar o braço robótico.
...
Se o seu dataset não tiver estatísticas de quantis (exigidas por algumas versões/formats), a documentação oficial também fornece uma abordagem com script de conversão: completar/convertar as estatísticas do dataset antes do treinamento (consulte a documentação oficial para detalhes).
...
A chave para usá-lo no LeRobot é definir o tipo de policy como `--policy.type=groot`. Observação: o GR00T N1.5 atualmente tem requisitos de ambiente mais altos (depende de FlashAttention e requer uma GPU CUDA). Recomenda-se fazer o ACT / Pi0 funcionar primeiro antes de tentar o GR00T.
...
:::tip
Se a instalação do flash-attn falhar, geralmente está relacionada a (1) incompatibilidade de versão entre PyTorch/CUDA, (2) dependências de build ausentes ou (3) ambiente muito novo/muito antigo. Nesse caso, consulte primeiro a documentação oficial do GR00T e o guia de instalação do PyTorch.
:::

**Treinamento (Fine-tuning)**

A documentação oficial fornece um exemplo de treinamento multi-GPU (`accelerate launch --multi_gpu ...`). Se você tiver apenas uma GPU, também pode tentar executá-lo primeiro em modo de processo único (suporte / detalhes de parâmetros estão sujeitos à documentação oficial).
...
- `--steps`, `--batch_size`: Etapas de treinamento e tamanho de batch. Modelos grandes são sensíveis à VRAM; se não rodar, reduza o `batch_size` primeiro.
...
- Ambiente de computação: This machine
...
Como o multi-GPU aumenta o tamanho de batch efetivo (batch_size × num_gpus):
...
Você pode ler mais sobre inferência assíncrona neste [post no blog](https://huggingface.co/blog/async-robot-inference) da Hugging Face.
...
- **Inferência síncrona**: Prediz um bloco, executa um bloco; o robô terá intervalos ociosos esperando o próximo bloco ser inferido. Quando o modelo é maior e o poder de computação é insuficiente, a lacuna de inferência é significativa — o braço se move, depois pausa (inferência), e então se move novamente.
- **Inferência assíncrona**: Diferente da inferência síncrona, enquanto o robô executa o bloco atual, o servidor já está calculando o próximo bloco; partes sobrepostas são agregadas para um controle mais responsivo.
...
Robô e câmeras se conectam a um dispositivo leve, e o servidor de políticas é executado em outro dispositivo com alta capacidade de computação na mesma LAN.
...
- `--actions_per_chunk`: Tamanho de cada bloco de ações (número de ações). Valores maiores significam menor frequência de inferência, mas resultados mais estáveis por inferência; valores menores significam movimento mais suave, porém com maior carga de inferência no servidor.
