...
Este projeto combina os poderosos recursos de computação do Pinocchio com a visualização intuitiva do MeshCat, fornecendo um conjunto completo de ferramentas de análise de cinemática e depuração para o reBot Arm B601-RS.
...
|-----------|--------------|
| Modelo do Braço Robótico | Kit Montado reBot Arm B601-RS com Garra |
...
Este tutorial assume que o braço já responde no barramento CAN, que as juntas estão zeradas e que o operador está familiarizado com os limites de segurança (permaneça dentro de **70% do espaço de trabalho de alcance do braço**). Pular o Guia Rápido pode levar a motores mal configurados, juntas travadas ou queda do braço.
:::
...
:::caution
Enquanto o braço robótico estiver executando exemplos, ele deve operar dentro de 70% do espaço de trabalho de alcance do braço. Permanecer fora do espaço de trabalho por um período prolongado fará com que o motor da segunda junta entre em proteção contra travamento, resultando na queda do braço.
:::
...
Este Wiki é para o **reBot Arm B601-RS**. Antes de executar quaisquer exemplos, altere a configuração de hardware em `config/rebotarm.yaml` da versão DM para a versão RS:
...
**Observação**: Os motores Robostride usam a interface CAN (`can0` padrão), com ID de host/feedback padrão `0xFD`. Durante o teste do motor, é necessário desabilitar o motor primeiro e depois reabilitá-lo para permitir leitura e controle normais.
...
:::tip Esta seção aborda apenas “onde / como mudar”, não “qual valor usar”
...
| Versão de hardware | Arquivo de configuração do motor | Entrada de comutação |
|---|---|---|
...
1. **Pare qualquer script em execução**. O motor fica habilitado quando você edita o YAML, as alterações não entram em vigor imediatamente, e é fácil disparar comportamentos inconsistentes.
...
3. **Preserve a indentação do YAML**: 2 espaços por nível, chaves separadas de valores por `: `. Indentação incorreta faz o `yaml.safe_load` falhar na análise, e todos os parâmetros voltarão para os padrões.
...
Alterar drasticamente `kp` / `kd` em várias juntas simultaneamente — se a direção ou o sinal de uma junta estiver errado — pode causar instantaneamente oscilações, sobrecorrente ou travamentos bruscos. **Itere uma junta e um modo por vez, em pequenos passos**.
:::
...
```
:::danger
Este exemplo **não possui planejamento de trajetória nem de velocidade**. Ângulos de junta muito grandes farão os motores se moverem em velocidade muito alta, e podem até **disparar diretamente a proteção de sobrecorrente do motor**. Recomendações:

- Primeiro verifique com **ângulos pequenos** (por exemplo, mova uma única junta apenas 5~10 graus), confirme que a resposta e a direção do motor estão corretas antes de aumentar;
- Esta seção **não possui versão com trajetória suave embutida**. Se você precisar de transições suaves entre múltiplos alvos, controle cuidadosamente seus alvos e o tempo, ou consulte a seção seguinte [Controle de IK com Trajetória Suave (8_arm_traj_control.py)](#demo8-controle-de-traj) e porte a abordagem de planejamento de mínimo tranco / aceleração-desaceleração para o seu próprio script;
...
Para o **reBot Arm B601-RS**, o modo POS_VEL (Posição‑Velocidade) requer **ajuste de parâmetros separado** para obter resultados aceitáveis (os parâmetros padrão geralmente têm desempenho pior que o modo MIT). Portanto, ele **não é recomendado** no fluxo de trabalho normal, mas como alguns usuários realmente precisam dele, **este demo é mantido para referência e ajuste sob demanda**. Se você não tiver necessidade especial, use o exemplo em modo MIT acima.
:::
...
```
:::danger
Este exemplo **não possui planejamento de trajetória nem de velocidade**. Ângulos alvo grandes farão os motores se moverem em velocidade muito alta, e podem até **disparar diretamente a proteção de sobrecorrente do motor**. Recomendações:

- Primeiro verifique com **ângulos pequenos** (por exemplo, mova o efetuador final apenas 5~10 cm a partir de sua posição atual), confirme que a pose e a direção estão corretas antes de aumentar;
...
Use cinemática inversa (IK) em modo MIT para planejar automaticamente uma trajetória de movimento com aceleração/desaceleração uniforme ou suave dentro do tempo alvo, evitando vibração severa das juntas.
...
- Exibir informações de pose formatadas
...
**Recursos**:
- Planejar da posição atual para a pose alvo
...
- **O efeito da compensação de gravidade é ruim**
  Isso pode ser causado por erros estruturais e precisão de usinagem. A compensação de gravidade neste projeto depende de URDF e Pinocchio. Você pode tentar corrigir o URDF para parâmetros que você realmente mediu (você pode pedir ajuda à IA para esta etapa).

- **Os motores Robostride não conseguem ler o status**
  Problemas de configuração de protocolo interno no motorbridge podem impedir que motores RS consultem status como motores DM. Por favor, julgue com base nos efeitos de movimento reais ou tente usar o comando `ping` para confirmar a comunicação normal com o motor.
