...
| `/rebotarm/safe_home` | `std_srvs/srv/Trigger` | Retorna para a posição inicial segura |
...
Isso primeiro para a compensação de gravidade, depois move o braço de volta para a posição inicial segura e o desabilita.
...
| `tcp_rpy` | Orientação do TCP, padrão para um garra voltada para baixo |
...
| `hardware_open_gripper_position` / `hardware_closed_gripper_position` | Posições de abertura/fechamento do motor da garra de hardware |
