...
O projeto reBot Arm foi disponibilizado como código aberto no [GitHub](https://github.com/Seeed-Projects/reBot-DevArm). Este guia o levará pelo início rápido do B601-DM, desde a montagem até a operação.
...
2. Como alternativa, você pode escolher nosso gabinete open source para fonte de alimentação MeanWell 24V 14,6A para montagem própria. As instruções em texto e a lista de materiais (BOM) estão em código aberto no [repositório GitHub](https://github.com/LAN-GER/reBot-DevArm/tree/main/hardware/reBot_B601_DM) (recomendado apenas para desenvolvedores com experiência relevante em montagem de fontes de alimentação).
...
- Compre 2 grampos de fixação (≥3 polegadas) e uma fonte de alimentação chaveada 24V 15A com saída XT30 (use marca regular, sem produtos inferiores).
- Mantenha distância ≥1 m durante a depuração/operação.
- Não faça conexão a quente dos motores; corte a alimentação antes de conectar/desconectar a interface XT30 2+2.
...
Exporte os parâmetros completos de um motor intacto do mesmo modelo por meio do software host, importe esses parâmetros para a unidade com falha, atualize seu respectivo CAN ID, salve os parâmetros gravados e, em seguida, prossiga com a calibração do ponto zero.
...
- Ao realizar a calibração do ponto zero via **DM_Tools_v.1.8.0.1.exe (apenas para Windows)**, evite clicar nos botões Read ou Set ao lado do campo CAN ID. A interface de depuração se comunica pelo barramento CAN; clicar em Set unificará o CAN ID de todos os motores conectados ao CANBUS.
