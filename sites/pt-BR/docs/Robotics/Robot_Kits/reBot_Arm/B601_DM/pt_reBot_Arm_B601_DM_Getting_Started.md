...
- Compre 2 grampos de fixação (≥3 polegadas) e uma fonte de alimentação chaveada de 24V 15A com saída XT30 (use marcas confiáveis, não produtos de baixa qualidade).
...
Confiamos que você concluiu a montagem do braço robótico, a inicialização do ponto zero, a configuração dos IDs dos motores e outras etapas de preparação seguindo o vídeo. Agora você está pronto para explorar oficialmente a série de tutoriais e ferramentas que lançamos.
...
2. Experimente novos recursos e detalhes do MotorBridge para o braço robótico reBot, incluindo configuração de ponto zero com um clique, gravação de parâmetros, controle de motores via interface de arrastar e soltar e uma interface integrada de visualização de modelo.
...
- Esse problema geralmente ocorre quando a calibração de parâmetros é acionada acidentalmente durante a configuração do ID, o que sobrescreve os parâmetros predefinidos de fábrica, como a inércia do motor. Use **DM_Tools_v.1.8.0.1.exe (Windows-only)** disponível em:
...
- Ao realizar a calibração do ponto zero por meio do **DM_Tools_v.1.8.0.1.exe (Windows-only)**, evite clicar nos botões Read ou Set ao lado do campo CAN ID. A interface de depuração se comunica pelo barramento CAN; clicar em Set unificará o CAN ID de todos os motores conectados ao CANBUS.
