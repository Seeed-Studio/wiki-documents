...
- Leia atentamente as instruções a seguir para garantir um processo de montagem tranquilo.
  ...
  3. Os parafusos enviados possuem trava química aplicada. Use ferramentas adequadas ou uma parafusadeira elétrica (recomendado). Ajuste o torque para (3–6 kgf·cm) para evitar espanar. Se ocorrer espanar, pare imediatamente e substitua ou realinhe o parafuso. Parafusos com trava química espanados são difíceis de remover e podem danificar as peças.
...
- Compre 2 grampos de fixação (≥3 polegadas) e uma fonte de alimentação chaveada 24V 15A com saída XT30 (use marca confiável, não produtos de baixa qualidade).
...
## Passo 3: Calibrar o reBot Arm e Começar
...
2. Experimente novos recursos e detalhes do MotorBridge para o braço robótico reBot, incluindo configuração de ponto zero com um clique, gravação de parâmetros, controle de motores via arrastar-e-soltar na interface gráfica e uma interface integrada de visualização de modelo.
...
- Esse problema geralmente ocorre quando a calibração de parâmetros é acionada acidentalmente durante a configuração de ID, o que sobrescreve parâmetros predefinidos de fábrica, como a inércia do motor. Use **DM_Tools_v.1.8.0.1.exe (apenas Windows)** disponível em:
...
- Ao realizar a calibração do ponto zero via **DM_Tools_v.1.8.0.1.exe (apenas Windows)**, evite clicar nos botões Read ou Set ao lado do campo CAN ID. A interface de depuração se comunica pelo barramento CAN; clicar em Set unificará o CAN ID de todos os motores conectados ao CANBUS.
