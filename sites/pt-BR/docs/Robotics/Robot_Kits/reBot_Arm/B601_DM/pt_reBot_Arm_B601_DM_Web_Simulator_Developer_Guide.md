...
<p align="center">
  <strong>Visualização em Three.js · Carregamento de URDF · Ponte rosbridge · Controle LLM/MCP</strong>
</p>
...
3. **Ponte rosbridge bidirecional**  
   `ReBotRosClient` encapsula o protocolo JSON do rosbridge e assina o estado das juntas, estado do gripper, status do braço, imagem da câmera virtual e resultados de detecção de visão, e publica comandos de junta única, comandos do gripper e poses alvo.
...
Fluxo de dados: o navegador acessa o servidor estático Node.js via `HTTP /api` e se comunica bidirecionalmente com o ROS2 via `rosbridge WebSocket`; a linguagem natural é encaminhada pelo Node.js para o Text Agent / MCP Server e então convertida em chamadas de ferramentas estruturadas que entram no ROS2. O ROS2 aciona o driver Fake/real e o braço para baixo, e se conecta lateralmente à simulação física MuJoCo, ao servidor de tarefas e à câmera virtual. A página web, o Agente LLM e o robô real não codificam chamadas uns para os outros; eles são desacoplados por meio de tópicos, serviços e ações do ROS2.
...
| `./rebotarm status` | Ver o status de processos, portas, portas seriais e nós ROS |
...
:::note
`server.js` localiza o workspace ROS2 através do caminho relativo `../reBotArmController_ROS2-main/...`. Se você mover o diretório web para outro local, precisará atualizar esses caminhos de acordo ou manter uma cópia do modelo da mesma versão que o workspace ROS2 no diretório web.
:::
...
- Fornecer predefinições de pose, sliders de juntas, arraste do TCP, gravação/reprodução/exportação de ensino, estimativa de envelope de alcance e alvo fantasma;
...
A resposta do text-agent e o processo de chamada de ferramenta são renderizados na área de chat.
...
- **Visão geral das ferramentas**: obtém automaticamente todas as ferramentas registradas do MCP Server e as agrupa por categoria (status e diagnóstico, controle de habilitação, controle de movimento, controle da garra, compensação de gravidade, apreensão visual, gravação e reprodução);
...
- **Entrada em linguagem natural**: digite comandos em linguagem natural na caixa de chat à direita; eles passam pelo endpoint `/chat` via cadeia LLM → MCP, e a resposta e o processo de chamada de ferramenta são exibidos na área de log em tempo real.
...
| `reBotArm_simulator-DM/public/lib/STLLoader-umd.js` | Carregador de malha STL |
...
- O arquivo STL está faltando ou a caixa de letras do caminho não corresponde (Linux diferencia maiúsculas de minúsculas).
...
- [Integração ROS2 do reBot Arm B601-DM](https://wiki.seeedstudio.com/pt-br/rebot_arm_b601_dm_ros2_integration/)
