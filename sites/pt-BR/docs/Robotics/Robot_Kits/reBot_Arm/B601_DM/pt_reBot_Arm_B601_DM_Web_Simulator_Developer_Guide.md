...
2. **Carregamento direto de URDF + STL**  
   `URDFLoader` lê `reBot-DevArm_fixend.urdf` e as malhas STL do corpo do braço a partir de `src/rebotarm_bringup/description/` no workspace ROS2 do mesmo repositório, portanto o modelo do corpo não precisa de uma segunda cópia no diretório web. As malhas visuais do gripper são armazenadas separadamente em `split_meshes/grouped_gripper/` no diretório web, porque o URDF termina em `end_link`.
...
Antes de iniciar o desenvolvimento do simulador web, conclua as etapas em [reBot Arm B601-DM Quick Start](https://wiki.seeedstudio.com/pt-br/rebot_b601_dm_getting_started/), incluindo montagem do braço, configuração de ID dos motores, inicialização do ponto zero e verificações básicas de conectividade. Também é recomendável concluir a configuração do workspace ROS2 em [reBot Arm B601-DM ROS2 Integration](https://wiki.seeedstudio.com/pt-br/rebot_arm_b601_dm_ros2_integration/), porque o simulador web depende do URDF e das malhas STL no mesmo repositório.
...
`setup.sh` clona automaticamente o SDK para `reBotArmController_ROS2-main/third_party/reBotArm_control_py/` (travado em um commit verificado). Se `~/reBotArm_control_py/` já existir, ele é reconhecido automaticamente e não é clonado novamente.
...
O `pyproject.toml` do SDK declara `requires-python >=3.10,<3.12`, mas este projeto o referencia via `sys.path` em vez de instalação via pip, portanto ele funciona bem no Python 3.12. Se `pip install -e .` relatar conflito de versão, ignore essa etapa e apenas certifique-se de que o diretório esteja em `reBotArmController_ROS2-main/third_party/reBotArm_control_py/` ou `~/reBotArm_control_py/` (o código pesquisa esses caminhos automaticamente).
:::
...
- Cria o ambiente virtual Python (`reBotArmController_ROS2-main/.venv`, com `--system-site-packages`)
...
:::note
Se o `setup.sh` não instalar o ROS 2 automaticamente (por exemplo, se a fonte apt do ROS ainda não tiver sido adicionada ao sistema), o instalador fará o download automático do pacote oficial `ros2-apt-source` do GitHub, adicionará a fonte e tentará novamente. Você não precisa configurar a fonte apt manualmente.
:::

### Etapa 2. Configurar variáveis de ambiente

O `setup.sh` já criou o `.env` a partir de `.env.example`. Para alterar a porta ou o destino do proxy, edite o `.env`:
...
Este comando carrega automaticamente o ambiente ROS2 e inicia o rosbridge (reutilizando um listener existente se a porta já estiver em uso) e o servidor web Node.js. Depois que ele iniciar, o terminal imprimirá a URL de acesso:
...
Abra `http://localhost:3001` em um navegador e aguarde o carregamento do URDF e do STL; o aparecimento do modelo 3D significa que o front-end está funcionando. A página já está conectada ao rosbridge local por padrão, então você pode operar diretamente no painel "ROS2 Bridge".
...
:::note
O comando `./rebotarm` carrega o ambiente internamente, então você não precisa executar `source scripts/source_rebotarm_env.sh` manualmente. No entanto, se você executar comandos `ros2` puros diretamente, cada novo terminal ainda precisará carregar o ambiente primeiro.
:::
...
Todos os nós iniciam em sequência; é considerado sucesso se não houver `ERROR`.

</details>

Este script é internamente equivalente a `reBotArmController_ROS2-main/scripts/start_rebot_mujoco_all.sh`. Por padrão, ele inicia o Fake Driver, o robot_state_publisher, o grasp físico MuJoCo, o servidor de tarefas, a câmera RGB superior, o detector de cores e o rosbridge. Em seguida, execute `./rebotarm start web` em outro terminal para iniciar a página web. Depois que o navegador se conectar ao ROS, você poderá usar o demo de grasping visual.
...
| `./rebotarm start dm` | Iniciar o driver do robô real DM (terminal separado, carrega o ambiente automaticamente) |
...
Este script carrega, na seguinte ordem, o ROS2 (`/opt/ros/jazzy/setup.bash`), o venv do Python (`.venv/bin/activate`), os caminhos do cmeel (extensões C do Pinocchio) e o workspace (`install/setup.bash`).
:::
...
- Encaminhar as requisições de chat LLM `/api/llm/chat` e o health check `/api/llm/health` para o serviço HTTP do text-agent na VM;
...
:::note
`server.js` localiza o workspace ROS2 por meio do caminho relativo `../reBotArmController_ROS2-main/...`. Se você mover o diretório web para outro local, será necessário atualizar esses caminhos de forma correspondente ou manter uma cópia do modelo da mesma versão que o workspace ROS2 no diretório web.
:::
...
- Anexar o grupo visual do gripper exclusivo da web (4 STLs) ao `end_link`, com faixa de acionamento de 0–90mm;
- Implementar o resolvedor de cinemática inversa DLS (damped least squares) `IKSolver`, com suporte a arraste de TCP e resolução de pose alvo;
...
:::note
O referencial de coordenadas Three.js na web difere do referencial ROS. O padrão do Three.js é Y para cima, enquanto o padrão do ROS é Z para cima. `rebot-sim.js` realiza a conversão com `threeToRos(v)`: `{ x: v.x, y: -v.z, z: v.y }`. Ao desenvolver recursos de pose personalizados, você deve usar essa conversão, caso contrário as coordenadas ficarão incorretas.
:::
...
**rebot-llm.js — UI de controle por texto com LLM**
...
Na inicialização, ele primeiro chama `/api/llm/health` para verificar a integridade do text-agent; após o sucesso, ele habilita a caixa de entrada. As mensagens são encaminhadas para o text-agent por meio de `/api/llm/chat`, e o `text` e `events` retornados (processo de chamada de ferramenta) são renderizados na área de chat. Ao parar, ele envia `{ text: '__reset__', reset: true }` para limpar o contexto.
...
| `/rebotarm/mujoco/target_pose` | `geometry_msgs/msg/PoseStamped` | Pose alvo de arraste de TCP |
...
Para o referencial de coordenadas, o Three.js na web usa por padrão Y para cima, enquanto o ROS usa por padrão Z para cima. Todas as poses de TCP são convertidas com `threeToRos()` antes de serem publicadas no ROS:
...
O controle em linguagem natural não é chamado diretamente do navegador para o ROS. Ele é encaminhado por meio do Node.js. O design em camadas permite que o LLM compreenda a intenção enquanto a camada MCP restringe a intenção em operações estruturadas do robô.
...
O MCP Dashboard é uma entrada de depuração independente e não precisa do simulador web. Iniciá-lo leva dois passos:
...
| `reBotArm_simulator-DM/public/index.html` | Entrada da single-page app e layout do painel de controle |
| `reBotArm_simulator-DM/public/css/rebot-sim.css` | Estilos de tema escuro |
| `reBotArm_simulator-DM/public/js/rebot-sim.js` | Cena 3D, IK, ensino, núcleo de arraste |
...
- Falha ao clonar o SDK: verifique a rede e a acessibilidade ao GitHub, ou clone manualmente para `reBotArmController_ROS2-main/third_party/reBotArm_control_py/` e execute novamente;
- `colcon build` falhou: verifique se o `rosdep` está inicializado (`sudo rosdep init && rosdep update`), depois execute `./setup.sh` novamente.
