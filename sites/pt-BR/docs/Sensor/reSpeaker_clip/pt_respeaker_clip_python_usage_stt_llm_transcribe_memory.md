---
description: Aprenda a criar um aplicativo para o reSpeaker Clip com uma camada de memória usando o Python SDK. Este tutorial aborda comunicação BLE e Wi‑Fi, gravação de áudio, transcrição, diarização, sumarização e correspondência de memória local para discussões repetidas.
title: Crie seu próprio app para reSpeaker Clip adicionando camada de memória usando Python SDK
keywords:
  - reSpeaker Clip
  - camada de memória
  - Python SDK
...
Este guia mostra como transformar o reSpeaker Clip em um assistente acionado por voz que não apenas transcreve, faz diarização ou resume gravações, mas também se lembra do que foi dito antes. Ao adicionar uma camada de memória em cima do fluxo de trabalho do Python SDK, seu app pode comparar novas transcrições com conversas passadas e notificá‑lo quando detectar uma discussão semelhante.

O resultado é uma base prática para criar assistentes de reunião inteligentes, tomadores de notas pessoais ou aplicativos de conhecimento acionados por voz com o reSpeaker Clip.
...
3. Pare a gravação. Quando a sincronização for concluída, o app:
   ...
   - converte para um `.wav` mono de 16 kHz usando PyAV,
...
O app oferece suporte a quatro padrões de fluxo de trabalho:
...
- **Camada de memória** — é executada em toda gravação, independentemente de qual aba foi usada.
...
- **Firebase** — opcional se você quiser mudar para uma opção de armazenamento baseada em Firestore para a camada de memória.

As chaves não são codificadas no código‑fonte. Elas podem ser armazenadas temporariamente em memória para a sessão atual do servidor ou persistidas localmente para uso futuro.

## Requisitos

- Python 3.10+
- Um dispositivo reSpeaker Clip pareado para gravação e streaming reais
- Nenhuma instalação separada do ffmpeg é necessária porque o PyAV inclui codecs
- Integração opcional com nuvem está disponível para equipes que desejam explorar Firebase ou outro armazenamento de memória baseado em banco de dados
...
Adicione sua chave de API Groq ou chave de API Speechmatics, inicie a gravação e pare quando terminar. O app então exibirá uma breve notificação em forma de toast com o resultado da sessão.
...
Se você quiser explorar uma experiência de memória apoiada em nuvem, o Firebase pode ser usado como uma extensão opcional para armazenar e compartilhar memórias entre dispositivos. Na prática, você só precisa preparar o lado da nuvem e configurar o backend para usá‑la. O fluxo de trabalho de memória local do app continua sendo o padrão, e mudar para Firebase exige um pequeno ajuste no backend para direcionar a camada de memória ao armazenamento em nuvem em vez dos arquivos locais.
