---
description: Aprenda a criar seus próprios aplicativos com o reSpeaker Clip SDK usando Python. Este tutorial aborda comunicação BLE e Wi-Fi, controle de gravação de áudio, sincronização de arquivos, configuração do dispositivo e muito mais.

title: Crie seu próprio app para reSpeaker Clip com Transcrição/Diária/Sumário usando o SDK em Python
keywords:
  - reSpeaker clip
  - ble
  - wifi
...
2. Escolha uma aba e clique em Start. O áudio é transmitido do dispositivo
   em segundo plano enquanto ele grava (sincronização contínua — igual à
   ferramenta original `clip-web`). A aba de onde você iniciou define o
   pipeline que será executado nessa gravação.
3. Pare a gravação. Quando a sincronização final terminar, o app:
   - codifica o áudio mesclado em `.ogg` (Opus),
   - converte para um `.wav` mono de 16kHz (via PyAV — sem necessidade de instalar ffmpeg separadamente),
    - **Aba Transcription:** envia o `.wav` para o Groq e recebe texto simples de volta.
    - **Aba Diarization:** envia o `.wav` para o Speechmatics com
      `diarization: "speaker"`, obtém a transcrição JSON em nível de palavra
      e a agrupa em turnos de fala (`S1`, `S2`, ...).
    - **Aba Summary:** envia o `.wav` para o Groq para transcrição e
      depois envia a transcrição para a API de chat do Groq (`openai/gpt-oss-20b`) para
      gerar atas de reunião estruturadas (título, pontos principais,
      itens de ação, decisões).
    - envia o resultado para o navegador pelo WebSocket existente.
4. Cada aba tem sua própria lista de "Recordings" (filtrada pelo pipeline
   sob o qual a sessão foi gravada), com reprodução e um botão
   Process/Re-run por gravação.


  ## Chaves de API

Cada aba tem seu próprio cartão de Settings — chave Groq nas abas Transcription e
Summary (compartilhada — defina uma vez e use em ambas) e chave Speechmatics
na aba Diarization. Nada é codificado em hard nem versionado. As chaves ficam
na memória durante o tempo de vida do processo do servidor. Marque "Remember on this
machine" para também persistir em `app/settings.local.json` (ignorado pelo git)
para que sobrevivam a uma reinicialização.

- **Groq:** obtenha uma chave em https://console.groq.com — usada tanto para
  transcrição (`whisper-large-v3-turbo`, chamada síncrona única e rápida)
  quanto para sumarização (compleções de chat `openai/gpt-oss-20b`).
- **Speechmatics:** obtenha uma chave em https://portal.speechmatics.com — usa
  a API REST em lote com `diarization: "speaker"` (enviar → consultar →
  buscar transcrição JSON → agrupar em turnos de fala), ponto de operação
  `enhanced` por padrão. Consulte
...
- Python 3.10+
- Não é necessário instalar ffmpeg separadamente — a conversão para WAV usa PyAV
  (`av` no PyPI), que traz suas próprias bibliotecas de codec
  incluídas, inclusive no Windows
- Um dispositivo reSpeaker Clip pareado (BLE) para a gravação real —
  esta parte não pode ser testada sem o hardware
...
Para usar transporte via WiFi em vez de BLE:
