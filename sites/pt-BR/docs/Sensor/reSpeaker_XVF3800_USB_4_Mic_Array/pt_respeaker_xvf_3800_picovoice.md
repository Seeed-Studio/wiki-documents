...
title: reSpeaker Controlar Wakeword e NLU com Pico-voice
keywords:
  - reSpeaker
  - python
  - picovoice
...
Processamento de Linguagem Natural (NLP) é o campo amplo que permite que máquinas processem a linguagem humana, enquanto Compreensão de Linguagem Natural (NLU) é um subconjunto que se concentra especificamente em extrair o significado ou a intenção dessa linguagem. Fala para Texto (STT) converte áudio falado em texto, que então é normalmente passado para o NLU para entender a intenção do usuário. O Picovoice Rhino adota uma abordagem diferente ao pular o STT e converter diretamente a fala em intenção (fala para intenção), melhorando a eficiência e a precisão.
...
Usar uma palavra de ativação junto com um sistema avançado de matriz de microfones como o reSpeaker possibilita uma interação por voz **eficiente e confiável** em sistemas incorporados. A palavra de ativação garante que o sistema só ative o processamento completo de fala quando necessário, reduzindo o consumo de energia e o uso de CPU em hardware embarcado. A matriz de microfones aprimora isso fornecendo **formação de feixe, supressão de ruído e detecção de direção na matriz de microfones circular**, permitindo reconhecimento preciso da palavra de ativação mesmo em ambientes ruidosos. Essa combinação minimiza disparos falsos e melhora a capacidade de resposta geral do sistema. Ela também oferece melhor interação humano-robô ao garantir que o dispositivo responda apenas quando for explicitamente acionado e possa focar no locutor correto.

## Como usar Wakeword com Pico-voice
...
Após a instalação, um código de exemplo pode ser usado para inicializar o gravador de áudio, carregar a palavra-chave selecionada e escutar continuamente a palavra de ativação. Uma vez que a palavra-chave é detectada, o sistema pode acionar ações adicionais, como gravar ou processar comandos de voz.
...
Navegue até o Picovoice Console e abra a página de palavra de ativação personalizada. Selecione o idioma de sua preferência e defina uma palavra de ativação, por exemplo “hi flex”. Você pode testar a palavra de ativação no console para verificar seu desempenho de detecção.
...
Um contexto define o conjunto de comandos falados, intenções (intents) e slots para um domínio específico; aqui, criamos um contexto **“Bumblebee”** para controlar o ReSpeaker Flex usando comandos de voz. Acesse o console Rhino Speech-to-Intent no Picovoice e crie um novo contexto chamado **Bumblebee** usando o modelo **“Empty”**.
...
No nível superior, um contexto é uma coleção de intents que representam ações do usuário dentro de um domínio. Por exemplo, crie um intent chamado “Gesture” no contexto HiFlex para representar comandos de voz relacionados a gestos.
...
### Adicionar expressões a um intent

Um usuário pode expressar a mesma intenção de várias maneiras, e cada variação é chamada de expressão. Para o intent **Gesture**, adicione expressões como **“wave your head” e “shake your antenna”**, pois ambas correspondem ao mesmo grupo de ações.
...
Modifique as expressões existentes para incluir o slot recém-criado. Ao adicionar um slot, use o símbolo `$` para indicar um slot, depois selecione o tipo de slot desejado na lista suspensa de preenchimento automático e atribua a ele um nome.
