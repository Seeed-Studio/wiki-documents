---
title: Kit de Bootcamp Wio Link
nointro: null
keywords:
  - docs
  - docusaurus
image: https://wiki.seeedstudio.com/pt-br/Wio_Link_Bootcamp_Kit/
slug: /Wio_Link_Bootcamp_Kit
sku: 110020050
last_update:
  date: 01/11/2022
  author: gunengyu
createdAt: '2023-01-12'
updatedAt: '2026-03-16'
url: https://wiki.seeedstudio.com/pt-br/Wio_Link_Bootcamp_Kit/
---

![](https://files.seeedstudio.com/wiki/Wio_Link_Bootcamp_Kit/img/Wio_Link_Bootcamp_Kit_product_view_1200_s.jpg)

Se você não tem habilidades de programação e quer construir alguns gadgets legais para o seu dia a dia, este kit será um ponto de partida perfeito. Ele contém um relé, um gravador, uma tira de LEDs, um buzzer e um motor de servo que executarão ações básicas necessárias na vida diária. A parte mais incrível deste kit é a placa de desenvolvimento Wio Link que pode ser utilizada para vários dispositivos: sem exigir nenhuma habilidade de programação. Todas as ações que você quiser que seus módulos executem e a operação de gravação de código no Wio Link podem ser concluídas com toques em um aplicativo móvel. E você pode enviar instruções para seus dispositivos pela Internet em qualquer lugar. Portanto, o Wio Link será uma ferramenta perfeita para tornar as coisas ao seu redor mais inteligentes para todos. Agora vamos começar a construir algumas coisas úteis.

[![](https://files.seeedstudio.com/wiki/common/Get_One_Now_Banner.png)](https://www.seeedstudio.com/depot/Wio-Link-Bootcamp-Kit-p-2613.html?cPath=19_20)

:::caution
     A função Wio IFTTT está em fim de vida (EOL). Mas o aplicativo Wio APP está disponível. Você pode usar a API do Wio APP para ler o status do sensor e controlar o atuador. 
:::
Recursos
--------

-   Inclui módulos funcionais comuns e frequentemente usados para monitoramento de ambiente
-   Porta Grove como interface, função mais prática e menos trabalho
-   Completamente fácil de tornar as coisas ao seu redor mais inteligentes e adicionar diversão à vida cotidiana.
-   Custo-benefício
-   Peso: 285 g

Lista de peças
----------

| Nome das peças                                                                                           | Quantidade |
|----------------------------------------------------------------------------------------------------------|----------|
| [Grove - Relay](/pt-br/Grove-Relay/)                                                                            | 1PC     |
| [Grove - Recorder](/pt-br/Grove-Recorder_v2.0/)                                                                 | 1PC     |
| Grove - Fita de LED WS2812 à prova d'água - 30 LEDs 1 metro                                              | 1PC     |
| [Grove - Buzzer](/pt-br/Grove-Buzzer/)                                                                          | 1PC     |
| [Wio Link](/pt-br/Wio_Link/)                                                                                    | 1PC     |
| [Cabo Micro USB - 48cm](https://www.seeedstudio.com/Micro-USB-Cable-48cm-p-1475.html?queryID=39d1b009a50fc3dc9e5a02a476ecb407&objectID=1405&indexName=bazaar_retailer_products) | 1PC     |
| [Grove - Servo](https://www.seeedstudio.com/Grove-Servo.html?queryID=ef605ab9a8a236dec86d63eec8c17796&objectID=1626&indexName=bazaar_retailer_products)                                | 1PC     |

Uma demonstração simples
-------------

Esta demonstração pode ser usada como um indicador de resultado de compilação.

### Guia preliminar

-   Travis CI
-   [Wio Link](/pt-br/Wio_Link/)

### Pré-requisitos

-   [Wio Link APP](https://www.seeedstudio.com/Wio-Link-Event-Kit-p-2611.html?queryID=3ab174447ce3813d2105c3db0e9e783a&objectID=537&indexName=bazaar_retailer_products)
-   [Wio Link](/pt-br/Wio_Link/)
-   [Grove Relay](https://www.seeedstudio.com/Grove-Relay.html?queryID=f15824e61eef2f0aa449144b0da3587c&objectID=1804&indexName=bazaar_retailer_products) × 3
-   Uma conta Travis CI
-   Uma conta GitHub
-   Semáforo

### Um passo a passo detalhado

Acesse [Recipe](https://www.seeedstudio.com/recipe/1068-traffic-light-indicates-travis-ci-compiled-results.html) para o manual detalhado.


<!-- This Markdown file was created from https://www.seeedstudio.com/wiki/Wio_Link_Bootcamp_Kit -->

## Suporte Técnico & Discussão sobre o Produto
 se você tiver qualquer problema técnico, envie o problema para o nosso [fórum](http://forum.seeedstudio.com/). 
Obrigado por escolher nossos produtos! Estamos aqui para fornecer diferentes tipos de suporte para garantir que sua experiência com nossos produtos seja a mais tranquila possível. Oferecemos vários canais de comunicação para atender a diferentes preferências e necessidades.

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>