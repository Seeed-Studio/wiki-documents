---
title: Kit de Eventos Wio Link
nointro: null
keywords:
  - docs
  - docusaurus
image: https://wiki.seeedstudio.com/pt-br/Wio_Link_Event_Kit/
slug: /Wio_Link_Event_Kit
sku: 110020048
last_update:
  date: 01/11/2022
  author: gunengyu
createdAt: '2023-02-21'
updatedAt: '2026-03-03'
url: https://wiki.seeedstudio.com/pt-br/Wio_Link_Event_Kit/
---

![](https://files.seeedstudio.com/wiki/Wio_Link_Event_Kit/img/Wio_Link_Event_Kit_product_view_1200_s.jpg)

O Kit de Eventos Wio Link é um kit de monitoramento de eventos que inclui alguns módulos funcionais Grove típicos acionados por eventos para construir dispositivos inteligentes de IoT. A parte mais incrível deste kit é a placa de desenvolvimento Wio Link, que pode ser utilizada para vários dispositivos: sem exigir nenhuma habilidade de programação. Todas as ações que você deseja que seus módulos executem e a operação de gravação de código no Wio Link podem ser concluídas com toques em um aplicativo móvel. E você pode enviar instruções para seus dispositivos pela internet de qualquer lugar. Portanto, o Wio Link será uma ferramenta perfeita para tornar as coisas ao seu redor mais inteligentes para todos. Agora vamos nos familiarizar com alguns módulos acionados por eventos incluídos neste kit.

[![](https://files.seeedstudio.com/wiki/common/Get_One_Now_Banner.png)](https://www.seeedstudio.com/depot/Wio-Link-Event-Kit-p-2611.html)


:::caution
     A função Wio IFTTT está em fim de vida (EOL). Mas o aplicativo Wio APP está disponível. Você pode usar a API do Wio APP para ler o status do sensor e controlar o atuador. 
:::
Características
--------

-   Inclui módulos funcionais comuns de monitoramento de eventos frequentemente usados.
-   Totalmente fácil para tornar as coisas ao seu redor mais inteligentes.
-   Porta Grove integrada, função mais prática e menos trabalho.
-   Peso: 145 g

Lista de peças
----------

| Nome das peças                                                                                          | Quantidade |
|----------------------------------------------------------------------------------------------------------|----------|
| [Grove - IR Distance Interrupt](/pt-br/Grove-Digital_Distance_Interrupter_0.5_to_5cm-GP2Y0D805Z0F)                                     | 1PC     |
| [Grove - Button](/pt-br/Grove-Button/)                                                                          | 1PC     |
| [Grove - Magnetic Switch](https://www.seeedstudio.com/Grove-Magnetic-Switch.html?queryID=d18ad991c05de7f74bbabc475ad4f905&objectID=1827&indexName=bazaar_retailer_products)                                                        | 1PC     |
| [Grove - 3-Axis Digital Accelerometer(±1.5g)](/pt-br/Grove-3-Axis_Digital_Accelerometer-1.5g/)                  | 1PC     |
| [Wio Link](/pt-br/Wio_Link/)                                                                                    | 1PC     |
| [Cabo Micro USB - 48cm](https://www.seeedstudio.com/Micro-USB-Cable-48cm-p-1475.html?queryID=a8b80828fdcd52ad1ae49daa5ed97191&objectID=1405&indexName=bazaar_retailer_products) | 1PC     |

Uma demonstração simples
-------------

Esta demonstração pode ser usada como um indicador de resultado de compilação.

### Guia preliminar

-   [Travis CI](https://travis-ci.org/)
-   [Wio Link](/pt-br/Wio_Link/)

### Material necessário

-   [Wio Link APP](https://www.seeedstudio.com/Wio-Link-Event-Kit-p-2611.html?queryID=3ab174447ce3813d2105c3db0e9e783a&objectID=537&indexName=bazaar_retailer_products)
-   [Wio Link](/pt-br/Wio_Link/)
-   [Grove Relay](https://www.seeedstudio.com/Grove-Relay.html?queryID=f15824e61eef2f0aa449144b0da3587c&objectID=1804&indexName=bazaar_retailer_products) × 3
-   Uma conta Travis CI
-   Uma conta GitHub
-   Semáforo
### Um guia detalhado de como fazer

Acesse a [Receita](https://www.seeedstudio.com/recipe/1068-traffic-light-indicates-travis-ci-compiled-results.html) para o manual detalhado:


<!-- This Markdown file was created from https://www.seeedstudio.com/wiki/Wio_Link_Event_Kit -->

## Suporte Técnico & Discussão de Produto
 se você tiver qualquer problema técnico, envie o problema para o nosso [fórum](http://forum.seeedstudio.com/). 
Obrigado por escolher nossos produtos! Estamos aqui para fornecer a você diferentes tipos de suporte para garantir que sua experiência com nossos produtos seja a mais tranquila possível. Oferecemos vários canais de comunicação para atender a diferentes preferências e necessidades.

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>