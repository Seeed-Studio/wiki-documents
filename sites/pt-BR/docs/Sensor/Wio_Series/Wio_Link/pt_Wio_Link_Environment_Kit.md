---
title: Kit de Ambiente Wio Link
nointro: null
keywords:
  - docs
  - docusaurus
image: https://wiki.seeedstudio.com/pt-br/Wio_Link_Environment_Kit/
slug: /Wio_Link_Environment_Kit
sku: 110020049
last_update:
  date: 01/11/2022
  author: gunengyu
createdAt: '2023-01-12'
updatedAt: '2026-03-16'
url: https://wiki.seeedstudio.com/pt-br/Wio_Link_Environment_Kit/
---

![](https://files.seeedstudio.com/wiki/Wio_Link_Environment_Kit/img/Wio_Link_Environment_Kit_product_view_1200_s.jpg)

Nós estamos sempre intimamente conectados com o ambiente ao nosso redor. Portanto, é importante para nós obter alguns detalhes de um ambiente. Este kit foi projetado para executar essa tarefa perfeitamente, pelo menos em um nível básico. Neste kit, você pode encontrar módulos funcionais comuns para coletar dados do ambiente em nossa casa, como umidade ou nível de luz em ambientes internos. A parte mais surpreendente deste kit é a placa de desenvolvimento Wio Link, que pode ser utilizada para vários propósitos, sem exigir nenhuma habilidade de programação. Todas as ações que você deseja que seus módulos executem e a operação de gravação de código no Wio Link podem ser concluídas com toques em um aplicativo móvel. E você pode enviar instruções para seus dispositivos pela Internet de qualquer lugar. Portanto, o Wio Link será uma utilidade perfeita para tornar as coisas ao redor mais inteligentes para todos. Então vamos entender melhor as condições em que você se encontra.

[![](https://files.seeedstudio.com/wiki/common/Get_One_Now_Banner.png)](https://www.seeedstudio.com/depot/Wio-Link-Environment-Kit-p-2612.html)


:::caution
     A função Wio IFTTT está em fim de vida (EOL). Mas o Wio APP está disponível. Você pode usar a API do Wio APP para ler o estado do sensor e controlar o atuador. 
:::
Características
--------

-   Inclui módulos funcionais de monitoramento ambiental comuns e frequentemente usados
-   Porta Grove integrada, função mais prática e menos trabalho
-   Custo-benefício
-   Peso: 154 g

Lista de peças
----------

| Nome das peças                                                                                                            | Quantidade |
|---------------------------------------------------------------------------------------------------------------------------|----------|
| [Grove - Moisture Sensor](/pt-br/Grove-Moisture_Sensor/)                                                                         | 1PC     |
| [Grove - Temp&Humi Sensor](https://www.seeedstudio.com/Grove-I2C-High-Accuracy-Temp-Humi-Sensor-SHT35.html?queryID=ea8d1729f76e80d6b1b53c492c74fcdb&objectID=35&indexName=bazaar_retailer_products)                | 1PC     |
| [Grove - Digital Light Sensor](https://www.seeedstudio.com/Grove-Digital-Light-Sensor-TSL2561.html?queryID=c7822737fbcac80a47232a5fb1ac1e1d&objectID=1594&indexName=bazaar_retailer_products)      | 1PC     |
| [Grove - Air quality sensor v1.3](https://www.seeedstudio.com/Grove-Air-Quality-Sensor-v1-3-Arduino-Compatible.html?queryID=c98f1d7e388efeeee92d1815abe557cb&objectID=700&indexName=bazaar_retailer_products) | 1PC     |
| [Wio Link](/pt-br/Wio_Link/)                                                                                                     | 1PC     |
| [Cabo Micro USB - 48cm](https://www.seeedstudio.com/Micro-USB-Cable-48cm-p-1475.html?queryID=a8b80828fdcd52ad1ae49daa5ed97191&objectID=1405&indexName=bazaar_retailer_products)                  | 1PC     |

Uma demonstração simples
-------------

Esta demonstração pode ser usada como um indicador de resultado de compilação.

### Guia preliminar

-   [Travis CI](https://travis-ci.org/)
-   [Wio Link](/pt-br/Wio_Link/)

### Material necessário

-   [Wio Link APP](https://www.seeedstudio.com/Wio-Link-Event-Kit-p-2611.html?queryID=3ab174447ce3813d2105c3db0e9e783a&objectID=537&indexName=bazaar_retailer_products)
-   [Wio Link](/pt-br/Wio_Link/)
-   [Grove Relay](https://www.seeedstudio.com/Grove-Relay.html?queryID=f15824e61eef2f0aa449144b0da3587c&objectID=1804&indexName=bazaar_retailer_products) × 3
-   Uma conta Travis CI
-   Uma conta GitHub
-   Semáforo


### Um passo a passo detalhado

Acesse [Recipe](https://www.seeedstudio.com/recipe/1068-traffic-light-indicates-travis-ci-compiled-results.html) para o manual detalhado.


<!-- This Markdown file was created from https://www.seeedstudio.com/wiki/Wio_Link_Environment_Kit -->

## Suporte Técnico e Discussão de Produto
 if you have any technical issue.  submit the issue into our [forum](http://forum.seeedstudio.com/). 
Obrigado por escolher nossos produtos! Estamos aqui para fornecer a você diferentes formas de suporte para garantir que sua experiência com nossos produtos seja a mais tranquila possível. Oferecemos vários canais de comunicação para atender a diferentes preferências e necessidades.

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>
