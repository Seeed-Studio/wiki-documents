---
title: Fonte de Alimentação para Protoboard 5V/3,3V
nointro: null
keywords:
  - docs
  - docusaurus
image: https://wiki.seeedstudio.com/pt-br/5V-3.3V_Breadboard_Power_Supply/
slug: /5V-3.3V_Breadboard_Power_Supply
sku: 106990021
last_update:
  date: 02/03/2022
  author: gunengyu
createdAt: '2023-02-06'
updatedAt: '2026-03-13'
url: https://wiki.seeedstudio.com/pt-br/5V-3.3V_Breadboard_Power_Supply/
---

|![](https://files.seeedstudio.com/wiki/5V-3.3V_Breadboard_Power_Supply/img/Supply.jpg)|![](https://files.seeedstudio.com/wiki/5V-3.3V_Breadboard_Power_Supply/img/5VAnd3.3V_Breadboard_Power_Supply_v1.2.jpg)|
|---|---|
|Fonte de Alimentação para Protoboard 5V/3,3V v1.1|Fonte de Alimentação para Protoboard 5V/3,3V v1.2|

<p style={{textAlign: 'center'}}><a href="https://www.seeedstudio.com/5V-3.3V-Breadboard-Power-Supply-p-566.html" target="_blank"><img src="https://files.seeedstudio.com/wiki/Seeed-WiKi/docs/images/300px-Get_One_Now_Banner-ragular.png" /></a></p>

Histórico de atualização de versão:

* Fonte de Alimentação para Protoboard 5V/3,3V v1.2: melhoria na facilidade de uso

<table>
  <tbody><tr>
      <th> Revisão</th>
      <th> Descrições</th>
      <th> Data de lançamento</th>
</tr>
    <tr style={{fontSize: '90%'}}>
      <td width="300px"> Fonte de Alimentação para Protoboard 5V/3,3V v1.1</td>
      <td width="500px"> Lançamento público inicial</td>
      <td width="200px"></td>
</tr>
    <tr style={{fontSize: '90%'}}>
      <td> Fonte de Alimentação para Protoboard 5V/3,3V v1.2</td>
      <td></td>
</tr></tbody></table>

1. Remover chave desnecessária

2. Substituir mini USB por micro USB
<div>
  2 de dezembro de 2014
</div>

## Suporte Técnico e Discussão de Produto

Obrigado por escolher nossos produtos! Estamos aqui para fornecer diferentes tipos de suporte e garantir que sua experiência com nossos produtos seja a mais tranquila possível. Oferecemos vários canais de comunicação para atender a diferentes preferências e necessidades.

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>
