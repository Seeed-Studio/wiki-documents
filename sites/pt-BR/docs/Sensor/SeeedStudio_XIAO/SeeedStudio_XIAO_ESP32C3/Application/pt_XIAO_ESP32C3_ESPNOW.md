...
title: Protocolo ESP-NOW na Série XIAO ESP32
keywords:
  - ESPNOW
...
# Começando com o protocolo ESP-NOW na Série XIAO
...
Este Wiki explicará o que é o protocolo ESP-NOW e ensinará como usar a Série XIAO ESP32 para se comunicar usando esse protocolo. O processo será muito simples. Para que todos possam utilizar o protocolo ESP-NOW na série XIAO ESP32, preparamos três tipos de XIAO ESP32: C6/C3/S3 para comunicação. Então, vamos começar essa jornada!

A propósito, se você acabou de adquirir esta placa, clique neste link — ele explicará como começar.
...
A definição oficial fornecida: ESP-NOW é um protocolo de comunicação sem fio definido pela Espressif que permite o controle direto, rápido e de baixo consumo de energia de dispositivos inteligentes sem a necessidade de um roteador. Ele pode coexistir com Wi-Fi e Bluetooth LE, suportando múltiplas séries de SoCs como Lexin ESP8266, ESP32, ESP32-S e ESP32-C. O ESP-NOW é amplamente utilizado em áreas como eletrodomésticos inteligentes, controle remoto e sensores.

Características principais

- De acordo com o método de conexão por endereço MAC, o emparelhamento pode ser feito rapidamente sem condições de rede, e os dispositivos podem ser conectados de forma um para muitos, um para um, muitos para um e muitos para muitos
- ESP-NOW é um protocolo de comunicação sem fio baseado na camada de enlace de dados, que simplifica as cinco camadas superiores do protocolo OSI em uma única camada, sem a necessidade de adicionar cabeçalhos de pacotes nem desempacotar camada por camada. Isso alivia consideravelmente o atraso causado pela perda de pacotes durante o congestionamento da rede e possui uma velocidade de resposta mais alta

Comparado ao Wi-Fi e Bluetooth

- Wi-Fi: O ESP-NOW suporta comunicação ponto a ponto entre dispositivos, portanto tem menor consumo de energia e maior velocidade de transmissão, além de uma distância de comunicação mais longa.
- Bluetooth: O ESP-NOW não requer um processo de emparelhamento, tornando-o mais simples e fácil de usar. Portanto, tem menor consumo de energia e maior velocidade de transmissão.

Mas o ESP-NOW é adequado para cenários de aplicação que exigem comunicação rápida, confiável, de baixo consumo e ponto a ponto, enquanto o Bluetooth e o Wi-Fi são mais adequados para ambientes de rede complexos e cenários com um grande número de dispositivos.

## Preparação de Hardware

Neste projeto, considerando que algumas pessoas podem ter apenas o XIAO ESP32S3, XIAO ESP32C3 ou XIAO ESP32C6, para que você possa aprender melhor a comunicação ESPNOW, este exemplo utiliza três modelos XIAO ESP32 — XIAO ESP32S3, XIAO ESP32C3 e XIAO ESP32C6 — para se comunicarem entre si. Você só precisa ajustar levemente o código para usar quaisquer dois ou três dos modelos acima na prática. Sem mais explicações, vamos ver como o código a seguir é implementado. Vamos lá!

Se você ainda não possui dois dispositivos da série XIAO ESP32, aqui estão os links de compra.
...
Vamos primeiro entender o framework geral do código. Esta instância usa XIAO ESP32S3, XIAO ESP32C3 e XIAO ESP32C6 — três placas ESP32. O XIAO ESP32S3 atua como remetente, e o XIAO ESP32C6 e o XIAO ESP32C3 atuam como receptores. Claro, este é apenas o papel atribuído neste código. Então, com a minha explicação abaixo, se você quiser alterar, adicionar ou remover o papel do receptor e do remetente, será muito simples. Vamos nos envolver!

### Parte 1. Código do Remetente XIAO ESP32S3
...
- `SenderXIAOS3_MACAddress_Requir()`  
  - Função : Definir o modo WiFi como STA e obter o endereço MAC para imprimir na porta serial
- `SenderXIAOS3_Send_Data()`
  - Função : Enviar mensagem específica
- `SenderXIAOS3_Send_Data_cb()`
  - Função : Esta é uma função de callback; quando executada, indica se a mensagem impressa foi entregue com sucesso e para qual endereço MAC
- `Association_ReceiverXIAOC3_peer() and Association_ReceiverXIAOC6_peer`
  - Função : Adicionar nós pares; se precisar de mais receptores, você pode criar nós e escrever uma mensagem que corresponda ao remetente e ao receptor
- `esp_now_register_send_cb()`
  - Função : Registrar uma função de callback para verificar se foi enviado para a camada MAC
  - Valor de retorno : camada MAC recebeu os dados com sucesso:[ESP_NOW_SEND_SUCCESS] caso contrário [ESP_NOW_SEND_FAIL]
...
- `#define ESPNOW_WIFI_CHANNE`
  - Função : canal em que o remetente e o receptor estão localizados
- `#define MAX_ESP_NOW_MAC_LEN`
  - Função : comprimento do endereço MAC
...
- `static uint8_t Receiver_XIAOC3_MAC_Address[MAX_ESP_NOW_MAC_LEN] and static uint8_t Receiver_XIAOC6_MAC_Address`
  - Função : Armazenei os endereços MAC do meu XIAO ESP32C3 e XIAO ESP32C6. Eles servem como receptores.
  - Complemento : Observe que estes são meus endereços MAC e não podem ser inseridos diretamente.
- `NO_PMK_KEY`
  - Função : Optar por parear dispositivos sem criptografia
...
- `ReceiverXIAOC3_Recive_Data_cb()`
  - Função : Esta é uma função de callback; quando executada, indica se a mensagem impressa foi entregue com sucesso e para qual endereço MAC
- `Association_SenderXIAOS3_peer()`
  - Função : Adicionar um nó de canal para o XIAO ESP32S3 enviar mensagens para ele
- `esp_now_register_send_cb()`
  - Função : Registrar uma função de callback para verificar se foi enviado para a camada MAC
  - Valor de retorno : camada MAC recebeu os dados com sucesso:[ESP_NOW_SEND_SUCCESS] caso contrário [ESP_NOW_SEND_FAIL]
...
- `static uint8_t XIAOS3_Sender_MAC_Address[MAX_ESP_NOW_MAC_LEN]`
  ...
  - Complemento : Por favor, note que estes são meus endereços MAC e não podem ser copiados!
- `NO_PMK_KEY`
  - Função : Escolher parear dispositivos sem criptografia
...
#### Resultado do Remetente XIAO ESP32S3
...
#### Resultado do Receptor XIAO ESP32C3
...
#### Resultado do Receptor XIAO ESP32C6
...
- Suporta comunicação entre múltiplos dispositivos, permitindo que um dispositivo envie dados para vários dispositivos.
...
- Geralmente utilizado para comunicação sem fio de curto alcance (dezenas de metros).

## Solução de Problemas

### Questão 1 : Incapaz de conectar, o programa não reportou nenhum erro

- Verifique se o endereço MAC do seu XIAO ESP32 está correto
- Verifique se o canal Wi-Fi do XIAO ESP32 conectado está no mesmo canal
- Reinicie seu XIAO ESP32 e reabra o monitor serial

### Questão 2 : Mensagem recebida, mas incompleta

- Ao detectar tanto o remetente quanto o receptor, há uma similaridade nos membros da estrutura

## Recursos

- **[Documentos Oficiais Espressif]** [ESPRESSIF ESP-IDF ESP-NOW](https://docs.espressif.com/projects/esp-idf/zh_CN/stable/esp32/api-reference/network/esp_now.html?highlight=espnow#esp-now)
