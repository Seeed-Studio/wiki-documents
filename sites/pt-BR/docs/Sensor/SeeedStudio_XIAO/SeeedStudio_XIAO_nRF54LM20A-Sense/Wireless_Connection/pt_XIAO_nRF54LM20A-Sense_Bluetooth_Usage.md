...
- Este tutorial é baseado no sistema de build PlatformIO e no Zephyr RTOS. Se você não estiver familiarizado com a criação de um projeto PlatformIO para o XIAO nRF54LM20A, consulte [Getting Started with Seeed Studio XIAO nRF54LM20A](https://wiki.seeedstudio.com/pt-br/xiao_nrf54lm20a_getting_started/).
...
3. Implemente o formato dos dados de advertising e a lógica de atualização em `main.c`.
...
3. Implemente a lógica da aplicação BLE em `main.c`.
...
4. Configure o projeto PlatformIO em `platformio.ini`.
```ini
...
2. Ative as configurações relacionadas a Bluetooth em `prj.conf`
...
<div style={{textAlign: 'center'}}>
  <img
    ...
    alt="Comunicação BLE LBS entre duas placas XIAO"
    style={{width: 600, height: 'auto'}}
  />
</div>
