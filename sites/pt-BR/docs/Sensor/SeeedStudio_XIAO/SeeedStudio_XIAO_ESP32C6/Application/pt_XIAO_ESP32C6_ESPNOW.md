...
url: https://wiki.seeedstudio.com/pt-br/xiao_esp32c6_espnow/
---
...
- ESP-NOW é um protocolo de comunicação sem fio baseado na camada de enlace de dados, que simplifica o protocolo de cinco camadas OSI superiores em uma única camada, sem a necessidade de adicionar cabeçalhos de pacotes e desempacotar camada por camada. Ele alivia muito o atraso e a latência causados pela perda de pacotes durante a congestão da rede e possui uma velocidade de resposta mais alta

Comparado ao Wi-Fi e Bluetooth

- Wi-Fi : ESP-NOW suporta comunicação ponto a ponto entre dispositivos, então ele tem menor consumo de energia e maior velocidade de transmissão, além de um alcance de comunicação mais longo.
- Bluetooth : ESP-NOW não requer um processo de pareamento, tornando o uso mais simples e fácil. Portanto, ESP-NOW tem menor consumo de energia e maior velocidade de transmissão.

Mas o ESP-NOW é adequado para cenários de aplicação que exigem comunicação rápida, confiável, de baixo consumo e ponto a ponto, enquanto Bluetooth e Wi-Fi são mais adequados para ambientes de rede complexos e cenários com um grande número de dispositivos.

## Preparação de Hardware

Neste projeto, para considerar que algumas pessoas podem ter apenas XIAO ESP32S3, XIAO ESP32C3 ou XIAO ESP32C6, a fim de que você possa aprender melhor a comunicação ESP-NOW, este exemplo usa três modelos XIAO ESP32, XIAO ESP32S3, XIAO ESP32C3 e XIAO ESP32C6, para se comunicarem entre si. Você só precisa mover o código levemente para usar quaisquer dois ou três dos modelos acima na operação prática. Sem mais explicações, vamos dar uma olhada em como o código a seguir é implementado. Vamos lá!

Se você ainda não tem quaisquer duas peças da série XIAO ESP32, aqui estão os links de compra.
...
Vamos primeiro entender o framework geral do código. Este exemplo usa XIAO ESP32S3, XIAO ESP32C3 e XIAO ESP32C6. Existem três placas ESP32, XIAO ESP32S3 como transmissor, e XIAO ESP32C6 e XIAO ESP32C3 como receptores. Claro, isso é apenas a atribuição de função neste código. Em seguida, através da minha explicação abaixo, se você quiser alterar, adicionar ou remover o papel do receptor e do transmissor, será muito simples. Vamos nos envolver!
...
- `espnow_init()`  
  - Função : Inicializar a funcionalidade ESPNOW
  - Valor de retorno : inicialização bem-sucedida : [ESP_OK] Falha : [ESP_FAIL]
- `espnow_deinit()`
  - Função : Desinicializar a funcionalidade ESPNOW, todas as informações relacionadas aos dispositivos pareados serão excluídas
...
- `SenderXIAOS3_Send_Data_cb()`
  - Função : Esta é uma função de callback; quando for executada, indica se a mensagem impressa foi entregue com sucesso e para qual endereço MAC
- `Association_ReceiverXIAOC3_peer() and Association_ReceiverXIAOC6_peer`
  - Função : Adicionar nós peer; se precisar de mais receptores, você pode criar nós e escrever uma mensagem que corresponda ao emissor e ao receptor
...
- `#define ESPNOW_WIFI_CHANNE`
  - Função : canal em que o transmissor e o receptor estão localizados
...
- `static uint8_t Receiver_XIAOC3_MAC_Address[MAX_ESP_NOW_MAC_LEN] and static uint8_t Receiver_XIAOC6_MAC_Address`
  - Função : Eu armazenei os endereços MAC dos meus XIAO ESP32C3 e XIAO ESP32C6. Eles servem como receptores.
  - Complemento : Observe que estes são meus endereços MAC e não podem ser usados diretamente.
- `NO_PMK_KEY`
...
- `static uint8_t XIAOS3_Sender_MAC_Address[MAX_ESP_NOW_MAC_LEN]`
  - Função : Eu armazenei os endereços MAC do meu XIAO ESP32S3
  - Complemento : Observe que este é o meu endereço MAC e não pode ser usado diretamente！
- `NO_PMK_KEY`
  - Função : Escolher parear dispositivos sem criptografia
...
- `espnow_init()`  
  ...
  - Valor de retorno: inicialização bem‑sucedida: [ESP_OK] Falha: [ESP_FAIL]
- `espnow_deinit()`
  - Função: desinicializar a função ESPNOW, todas as informações relacionadas a dispositivos pareados serão apagadas
  - Valor de retorno: inicialização bem‑sucedida: [ESP_OK]
- `Receiver_MACAddress_requir()`  
  - Função: definir o modo WiFi para STA e obter o endereço MAC para imprimir na porta serial
- `ReceiverXIAOC6_Send_Data()`
  - Função: enviar mensagem específica
- `ReceiverXIAOC6_Recive_Data_cb()`
  - Função: esta é uma função de callback; quando for executada, imprime se a mensagem foi entregue com sucesso e para qual endereço MAC
- `Association_SenderXIAOS3_peer()`
  - Função: adicionar um nó de canal para que o XIAO ESP32S3 envie mensagens para ele
...
- `#define ESPNOW_WIFI_CHANNE`
  - Função: canal em que remetente e receptor estão localizados
- `#define MAX_ESP_NOW_MAC_LEN`
  - Função: tamanho do endereço MAC do destinatário
- `#define MAX_CHARACTERS_NUMBER`
  - Função: número máximo de caracteres aceitos ou enviados
- `#define BAUD 115200`
  - Função: definir a taxa de baud da porta serial
- `static uint8_t XIAOS3_Sender_MAC_Address[MAX_ESP_NOW_MAC_LEN]`
  - Função: aqui eu armazenei os endereços MAC do meu XIAO ESP32S3
  - Complemento: observe que estes são os meus endereços MAC e não podem ser usados!
- `NO_PMK_KEY`
  - Função: escolher parear dispositivos sem criptografia
...
#### Resultado do XIAO ESP32S3 Remetente
...
- Normalmente usado para comunicação sem fio de curto alcance (dezenas de metros).

## Solução de Problemas

### Pergunta 1 : Incapaz de conectar, o programa não relatou nenhum erro

- Verifique se o endereço MAC do seu XIAO ESP32 está correto ou não
- Verifique se o canal Wi‑Fi do XIAO ESP32 conectado é o mesmo
- Redefina seu XIAO ESP32 e reabra o monitor serial

### Pergunta 2 : Mensagem recebida, mas incompleta

- Ao detectar tanto o remetente quanto o receptor, há semelhança nos membros da estrutura
