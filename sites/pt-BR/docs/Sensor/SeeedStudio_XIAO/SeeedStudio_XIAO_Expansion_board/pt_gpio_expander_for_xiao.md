...
title: Expansor de IO para XIAO
...
O IO Expander para XIAO é uma placa de expansão de última geração projetada para aprimorar os recursos da série Seeed Studio XIAO. Alimentada pelo chip MCP23017, esta placa oferece 16 pinos de IO adicionais, permitindo que os usuários expandam seus projetos sem limitações. Seja você um entusiasta que deseja experimentar com mais componentes ou um profissional em busca de uma solução confiável de expansão de IO, esta placa foi feita para atender às suas necessidades. Sua compatibilidade com a série XIAO garante uma integração perfeita, tornando seu processo de desenvolvimento mais simples e eficiente.

## Recursos

- Integração perfeita com XIAO: projetado para funcionar perfeitamente com a série Seeed Studio XIAO.
- 16 pinos de IO adicionais: alimentado pelo MCP23017, fornece 16 pinos de IO extras para seus projetos.
- Interface I2C com endereço configurável: o endereço I2C padrão é 0x21, mas pode ser configurado para 0x20.
- Design robusto: construído com materiais de alta qualidade para garantir longevidade e confiabilidade.
...
- Dimensões: 21 mm x 17,8 mm

## Aplicações

O IO Expander para XIAO é versátil e pode ser usado em uma infinidade de aplicações, incluindo, mas não se limitando a:

- Sistemas de automação residencial: aumente o número de dispositivos que você pode controlar na sua configuração de casa inteligente.
- Robótica: adicione mais sensores, motores ou outros componentes ao seu robô sem ficar sem pinos de IO.
- Consoles de jogos: projete controladores personalizados ou outros periféricos com uma grande quantidade de botões e chaves.
- Sistemas de controle industrial: gerencie mais dispositivos e sensores na sua instalação industrial.
...
Esta seção fornece uma visão detalhada dos vários componentes e interfaces na placa XIAO IO Expander.
...
Por padrão, quando você usa a placa de expansão, os pads de saída adicionais na área 8 são habilitados. Se você não quiser habilitá-los, precisa cortar com uma faca os dois pads adjacentes na área J1.
...
Este também é um pino de saída que pode ser usado para aterrar outros componentes.

### 8. Pads de saída adicionais

Estes são alguns pads de saída adicionais, incluindo GND, INTB, INTA, RST. Se você desejar soldar esses pinos para uso em outro lugar, pode fazê-lo.
...
Existem três maneiras principais de usar esta placa de expansão,

**Modo 1: SMD**

Se o seu XIAO estiver com os pinos sem solda, você pode escolher o método SMD direto para soldar o XIAO e a placa de expansão na placa de circuito impresso (PCB) para usar a função de expansão de GPIO.
...
:::note
As PCBs mostradas são apenas para exibição e não estavam disponíveis para venda nas prateleiras na data deste artigo.
:::

**Modo 2: XIAO conecta-se diretamente com a placa de expansão através da fileira de pinos**
...
Depois que a soldagem estiver concluída, você pode prosseguir para conectar a placa de expansão ao controlador principal XIAO RP2040.
...
Antes de começar a programar a placa, você precisará de uma biblioteca específica para o XIAO. Baixe a biblioteca MCP23017 a partir deste [link do GitHub](https://github.com/limengdu/Adafruit-MCP23017-Arduino-Library). Depois de baixá-la, instale a biblioteca em seu ambiente de programação.
...
Na Arduino IDE, abra um novo sketch e copie o seguinte código de exemplo:
...
Envie o código acima para o seu XIAO. Depois que o código for carregado com sucesso, você verá que todos os pinos estão em nível alto de 3,3 V, exceto o pino PB7.

:::caution
Esta placa de expansão só pode usar entradas e saídas digitais, não funções de entrada e saída PWM ou analógica.
:::

### Endereçamento de pinos

Ao usar operações de pino único, como _pinMode(pinId, dir)_ ou _digitalRead(pinId)_ ou _digitalWrite(pinId, val)_, os pinos são endereçados usando os IDs abaixo. Por exemplo, para definir o modo de _GPB0_, use _pinMode(8, ...)_. **OBSERVAÇÃO** Os MCP23008 e MCP23S08 possuem apenas pinos _GPAx_.
...
**Resposta**: Certifique-se de que o módulo XIAO esteja corretamente conectado à placa de expansão. Verifique também se as bibliotecas necessárias estão instaladas e se a placa e a porta corretas estão selecionadas na Arduino IDE.

### 2. Posso usar o IO Expander para XIAO com outros microcontroladores?

**Resposta**: Sim, o IO Expander é projetado principalmente para o módulo XIAO, mas pode ser usado com outros microcontroladores que suportem comunicação I2C. Talvez seja necessário ajustar o código e as conexões de acordo.
...
**Resposta**: Certifique-se de que as conexões estejam firmes e que não haja interferência. Usar resistores de pull-up ou pull-down pode ajudar a estabilizar os pinos de entrada. Além disso, verifique se a fonte de alimentação é estável e pode fornecer a corrente necessária para todos os dispositivos conectados.
