...
keywords:
  - docs
  - docusaurus
image: https://wiki.seeedstudio.com/pt-br/Grove-UART_Wifi/
...
url: https://wiki.seeedstudio.com/pt-br/Grove-UART_Wifi/
---
...
* Grove - Usado para se conectar a um processador através de um soquete em uma placa base, como um Seeeduino ou Grove Base Shield.
...
:::note
    ...
    **Nota 2:** Cada módulo Grove vem com um cabo Grove quando você compra. Caso você perca o cabo Grove, você pode clicar [aqui](https://www.seeedstudio.com/Grove-Universal-4-Pin-Buckled-20cm-Cable-%285-PCs-pack%29-p-936.html) para comprar
:::
...
* **Passo 2.** Consulte [Como instalar biblioteca](https://wiki.seeedstudio.com/pt-br/How_to_install_Arduino_Library) para instalar a biblioteca para o Arduino.
...
* **Etapa 4.** Faça upload da demonstração. Se você não sabe como fazer upload do código, consulte [How to upload code](https://wiki.seeedstudio.com/pt-br/Upload_Code/).
...
Nossa placa de módulo já vem com um firmware gravado com as configurações de fábrica; você pode gravar outro firmware nela se quiser. Clique [aqui](https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/res/Grove-Uart_Wifi_Firmware-code.zip) para baixar o código‑fonte do firmware de configuração de fábrica.

### Materiais necessários

* É necessário um conversor USB para serial para atualização de firmware; você pode escolher o [UartSBee V5](https://www.seeedstudio.com/UartSBee-V5-p-1752.html) se não souber onde conseguir um.
...
**Etapa 1.** Conecte uma extremidade do cabo conversor Grove‑Jump ao soquete Grove no Grove - Uart Wifi e conecte a outra extremidade ao UartSBee V5, como mostrado a seguir.

![](https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/img/UART_v1.jpg)

**Etapa 2.** Em seguida, conecte os cabos como na figura a seguir:

![enter image description here](https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/img/Grove_uart_wifi_firmware_connect2.jpg)

### Software

**Etapa 1.** Baixe a ferramenta de gravação e o firmware
...
**Etapa 2.** Pressione e segure o botão até o LED vermelho acender, o que significa que ele está pronto para gravar o firmware.

**Etapa 3.** Inicie os arquivos executáveis na pasta FLASH DOWNLOAD TOOLS (clique duas vezes) para fazer as configurações conforme os passos a seguir:
...
Os **Materiais necessários** e a **Conexão de hardware** são os mesmos da seção [Firmware update](https://wiki.seeedstudio.com/pt-br/Grove-UART_Wifi/#Atualização-de-firmware)

Vamos para a parte de software  

#### Software

Você pode usar qualquer ferramenta serial que preferir; aqui usamos o Arduino. E certifique‑se de ter conectado o **conversor USB para serial** ao seu PC.

**Etapa 1.** Abra a IDE do Arduino, clique em **Tools** e escolha a **Port** correspondente.

![](https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/img/1.png)

<div>
  **Etapa 2.** Em seguida, clique no botão <embed src="https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/img/COM.png" /> no canto superior direito para abrir o **Serial Monitor** do Arduino.
</div>

**Etapa 3.** Configure o Serial Monitor como na figura a seguir. Em particular: 2- Selecione **Both NL & CR**, 3-Defina a **baud rate** para 115200

![](https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/img/result.png)

**Etapa 3.** Digite o comando AT de que você precisa na linha de comando e clique no botão **Send**. Você verá as informações de retorno como na figura acima.
...
|AT+GSLP| Entrar no modo de hibernação profunda|
|ATE| Ativar/Desativar eco de comandos AT|
...
|AT+SLEEP |Modo de suspensão|
|AT+RFPOWER| Definir potência de transmissão RF|
|AT+RFVDD| Definir potência de transmissão RF de acordo com VDD33|
...
|AT+CWJAP_CUR| Conexão atual a AP (Não será salva na Flash)|
|AT+CWJAP_DEF| Conexão padrão a AP (Salva na Flash)|
|AT+CWLAP| Lista APs disponíveis|
|AT+CWQAP| Desconectar da AP|
...
|AT+CWDHCP| Ativar/Desativar DHCP (Obsoleto)|
|AT+CWDHCP_CUR| Ativar/Desativar DHCP atual (Não será salvo na Flash)|
|AT+CWDHCP_DEF| Ativar/Desativar DHCP padrão (Salvo na Flash)|
|AT+CWAUTOCONN| Conectar automaticamente à AP ao ligar|
...
|AT+CIPAPMAC_CUR| Definir endereço MAC do softAP (Não será salvo na Flash)|
|AT+CIPAPMAC_DEF| Definir endereço MAC do softAP (Salvo na Flash)|
|AT+CIPSTA| Definir endereço IP da estação (Obsoleto)|
|AT+CIPSTA_CUR| Definir endereço IP da estação (Não será salvo na Flash)|
|AT+CIPSTA_DEF| Definir endereço IP da estação (Salvo na Flash)|
|AT+CIPAP| Definir endereço IP do softAP (Obsoleto)|
|AT+CIPAP_CUR| Definir endereço IP do softAP (Não será salvo na Flash)|
|AT+CIPAP_DEF| Definir endereço IP do softAP (Salvo na Flash)|
...
|AT+CIPBUFSTATUS| Verificar status do buffer de envio TCP|
...
|AT+CIPSTO| Definir tempo limite quando o ESP8266 funcionar como servidor TCP|
|AT+CIUPDATE| Atualizar firmware pela rede|
|AT+PING| Dar ping em um endereço IP ou nome de host|

### Comandos AT da Seeed

|Command |Description|
|-------------|---------------|
|AT+LEDON| Ligar o LED azul LINK|
|AT+LEDOFF |Desligar o LED azul LINK|
...
* **[PDF]** [Esquemático em PDF](https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/res/Grove-UART_WiFi_v1.0.pdf)

* **[Zip]** [Esquemático em Eagle](https://files.seeedstudio.com/wiki/Grove-Uart_Wifi/res/Grove-UART_WiFi_sch_pcb.zip)
