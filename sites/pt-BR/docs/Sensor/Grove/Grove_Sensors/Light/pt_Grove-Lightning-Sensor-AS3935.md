---
description: Introdução ao Grove Lightning Sensor AS3935
...
url: https://wiki.seeedstudio.com/pt-br/grove-lightning-sensor-as3935/
---
...
O Grove Lightning Sensor é um sensor altamente sensível que detecta campos elétricos livres no ar e, usando um algoritmo proprietário interno, determina se eles são causados pela frente de uma tempestade com eventos de relâmpagos. Ele é muito fácil de usar como um dispositivo plug and play, usando o cabo padrão Grove, e pode aceitar vários ajustes para ser otimizado para o ambiente da aplicação, tanto interno quanto externo.

O uso é para alerta preventivo sobre a aproximação da frente de relâmpagos de uma tempestade, a fim de evitar danos a equipamentos elétricos ou pessoas e para pesquisa meteorológica, pois pode fornecer vários dados sobre a intensidade e o número de eventos. Este sensor pode ser totalmente integrado com outros sensores em uma estação meteorológica de previsão e ser muito útil na agricultura para a proteção de culturas em caso de tempestades iminentes. Pode detectar a frente de relâmpagos de uma tempestade a até 30 – 40 km de distância, quando ainda está fora de vista.

### Recursos

- Sensor extremamente compacto e fácil de usar.

- Muito sensível a campos elétricos no ar, seleciona apenas aqueles que atendem à forma de onda típica da frente de relâmpagos; os outros são rejeitados.

- Comunicação através do protocolo I2C com interface fácil para as placas de desenvolvimento Arduino, Seeeduino ou Raspberry Pi.

- Consumo extremamente baixo, pode entrar em modo de suspensão para reduzir o consumo e prolongar a vida útil da bateria.

- Envia em um pino IRQ o sinal de interrupção sempre que uma frente de relâmpagos é validada. Útil para conexão a microcontrolador e controle por software.

- Interface totalmente compatível com o sistema Grove.

- Fácil integração em estações meteorológicas ou dispositivos de alerta para prevenir danos em equipamentos eletrônicos ou seres humanos.

## Visão Geral do Hardware

Antes de tudo começar, é bastante essencial ter alguns parâmetros básicos do produto. A tabela a seguir fornece informações sobre as características do Grove Lightning Sensor.
...
<table align="center">
 <tr>
     ...
     <th align="center">Faixa/Valor</th>
 </tr>
 <tr>
     <td align="center">Tensão de Entrada</td>
     <td align="center">5 Volt ou 3.3 Volt (selecionar as ilhas de solda
 pads) apropriadas)</td>
 </tr>
 <tr>
     <td align="center">Comunicação</td>
     <td align="center">Padrão I2C, SPI usando a saída fornecida</td>
 </tr>
 <tr>
     <td align="center">Alcance máximo de detecção</td>
     <td align="center">40 km</td>
 </tr>
 <tr>
     <td align="center">Programável</td>
     <td align="left">Sim, registradores apropriados podem ser programados para se adequarem à aplicação e ao ambiente onde o sensor está instalado.</td>
 </tr>
</table>

## Primeiros Passos

### Seleção da alimentação do módulo

O Grove Lightning Sensor suporta duas tensões de alimentação diferentes - 5V e 3.3V. Você precisará determinar sua tensão de alimentação e realizar uma simples soldagem antes de usar o módulo.

Se você optar pela alimentação de **5 Volt**, então solde as ilhas (5V) e (0) do Grove Lightning Sensor juntas, como na figura abaixo.
...
Se você optar pela alimentação de **3.3 Volt**, então solde as ilhas do Grove Lightning Sensor juntas, como na figura abaixo. Este é o caso se você estiver usando a placa Seeed Arduino Breakout for Linkt Smart 7688 Duo, que trabalha em 3.3 Volt.
...
:::note
O Grove Lightning Sensor pode suportar uma inserção hot swap com a alimentação das placas **ON**, mas isso não é recomendado; sempre desligue a alimentação antes de conectar o Grove Lightning Sensor ao conector da placa.
:::

## Exemplo com Arduino

O código Arduino fornecido aqui permite dialogar com o sensor e modificar todos os registradores relevantes que determinam o melhor comportamento durante a operação. O sensor é fornecido com valores base nos registradores, bem descritos no código, e este é um bom ponto de partida para a aplicação. Algum trabalho é necessário para obter o melhor ajuste para a aplicação, e isso é uma tarefa muito desafiadora ao trabalhar com o sensor, porque ele se mostra extremamente adaptável ao ambiente em que é utilizado.

**Passo 1.** Inicie o aplicativo Arduino.

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/seeed_logo/arduino.jpg" style={{width:800, height:'auto'}}/></div>

**Passo 2.** Selecione o modelo da sua placa de desenvolvimento e adicione-o à IDE Arduino.
...
### Demonstração: Usando o Grove Lightning Sensor no Seeeduino
...
**Passo 3**. Conecte o Grove Lightning Sensor à porta I2C do Seeeduino V4.
...
**Passo 5**. Envie o seguinte programa para o Seeeduino.
...
Se o sensor detectar relâmpagos, você verá os seguintes resultados.
...
Para fins de teste, você pode usar o ignitor de um isqueiro para simular relâmpagos e verificar se o sensor está funcionando corretamente.
