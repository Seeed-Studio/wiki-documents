---
description: reSpeaker Flex é um poderoso sistema de voz modular com XMOS XVF3800, com arrays de 4 microfones circulares e lineares intercambiáveis para captura de áudio 360° precisa ou direcional — perfeito para robótica e dispositivos inteligentes.
...
O reSpeaker Flex é um sistema modular de processamento de voz com arquitetura dividida, construído em torno do processador XMOS XVF3800, projetado especificamente para robôs e terminais de interação inteligente. Diferente de arrays de microfones convencionais tudo‑em‑um, o Flex separa a placa principal de processamento da placa de array de microfones; as duas são conectadas por um cabo flat FPC flexível, permitindo que o array seja embutido em qualquer posição física dentro do gabinete do produto enquanto mantém a eletrônica de processamento em outro local.
O sistema suporta duas configurações intercambiáveis de array de microfones: um **array circular de 4 microfones** para captura omnidirecional de 360°, e um **array linear de 4 microfones** para captação direcional frontal com supressão traseira. Ambas as placas se conectam à mesma placa principal por meio de uma interface FPC padronizada de 24 pinos, e o sistema opera em modo USB (UAC 2.0 plug‑and‑play) ou em modo I2S para integração embarcada. Um módulo XIAO ESP32S3 opcional pode ser soldado na placa principal para conectividade sem fio e controle expandido.
...
- **Design modular dividido**: Placa principal e placa de array de microfones são fisicamente separadas, conectadas via cabo flat FPC para posicionamento flexível em qualquer chassi de produto

- **Opções de configuração de microfones**: Troque entre o array circular de 4 microfones (captação 360°, espaçamento de 44 mm) ou o array linear de 4 microfones (diretividade frontal de 180°, espaçamento de 33 mm) usando a mesma placa principal
...
- **Modos duplos de operação**: USB UAC 2.0 para plug‑and‑play com PCs e SBCs; modo I2S para integração diretamente embarcada
...
- **Amplificador de alto‑falante on‑board**: Suporta acionamento de alto‑falantes de 10 W a 4 Ohms via conector JST, com saída para fones de ouvido AUX de 3,5 mm
...
- **SDK Python**: Formato de gravação, volume, parâmetros de algoritmo e roteamento de canais ajustáveis via Python
...
|--------------------|------------|
| **Processador Principal de Áudio** | XMOS XVF3800 (firmware v3.2.1), lida com todo o DSP de áudio, incluindo AEC, formação de feixe, DoA e supressão de ruído |
...
| **Conector AUX de 3,5 mm** | Saída lateral para fones de ouvido acionada pelo DAC on‑board |
| **Conector JST para Alto‑falante** | Conector montado na vertical que aciona alto‑falantes de até 10 W a 4Ω; ilha de solda preservada para conexão de alto‑falante cabeada |
...
| **LED PWR** | Indicador verde de energia ligada |
| **Ilha de Depuração** | Ilha reservada para XMOS XTAG4 (não soldada de fábrica) |
| **Headers de E/S do XMOS 3800** | Headers/ilhas de pinos expostos para I2C, I2S, 5V/GND, pinos de alto‑falante e IO restante do XVF3800 |
...
| **GPIO do XIAO ESP32S3** | GPIO restante pode ser usado para expansão de IO e periféricos |
...
| **Padrão de Captação** | Captura de áudio omnidirecional de 360° |
| **Interface** | Conector FPC para a placa principal |
| **Fixação** | 3 × orifícios de fixação M3 para fixação em gabinete ou caixa |




### Placa de Array Linear
...
- **IO restante do XIAO** — disponibilizado em headers/ilhas de pinos rotulados para expansão pelo usuário

Uma antena Wi‑Fi FPC (para o XIAO ESP32S3 sem antena on‑board) é incluída como acessório em todas as variantes equipadas com XIAO.
...
A interface FPC do array de microfones é um conector de 24 pinos com passo de 0,5 mm e trava, e o cabo flat FPC de 20 cm incluído na caixa é chaveado para esse conector.
...
|-----|------------|------|----------------------|-------|
| 1 | MIC_VDD | Power | Alimentação dos microfones | Fonte de 3,3 V |
| 2 | MIC_VDD | Power | Alimentação dos microfones | Pino duplo para estabilidade aprimorada de energia |
...
- Firmware I2S
    - Suporta I2C DFU.
    - Permite atualizações de firmware por meio da interface I2C.
...
- Enquanto estiver segurando o **botão Boot, reconecte a alimentação.**
...
:::note 
Certifique-se de baixar todo o repositório.
:::
