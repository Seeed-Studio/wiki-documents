...
O reSpeaker Flex é um sistema modular de processamento de voz com arquitetura dividida, construído em torno do processador XMOS XVF3800, projetado especificamente para robôs e terminais de interação inteligentes. Diferente dos arrays de microfones convencionais “tudo em um”, o Flex separa a placa principal de processamento da placa do array de microfones — as duas conectadas por um cabo flat FPC flexível — permitindo que o array seja embutido em qualquer posição física dentro do gabinete do produto enquanto mantém a eletrônica de processamento em outro local.
O sistema suporta duas configurações intercambiáveis de array de microfones: um **array circular de 4 microfones** para captura omnidirecional em 360°, e um **array linear de 4 microfones** para captação direcional frontal com supressão traseira. Ambas as placas se conectam à mesma placa principal através de uma interface FPC padronizada de 24 pinos, e o sistema opera em modo USB (UAC 2.0 plug-and-play) ou em modo I2S para integração embarcada. Um módulo opcional XIAO ESP32S3 pode ser soldado na placa principal para conectividade sem fio e controle expandido.
...
- **Amplificador de alto-falante integrado**: Suporta acionamento de alto-falantes de 10 W, 4 Ohms via conector JST, com saída de fones de ouvido AUX de 3,5 mm
...
- **Flexibilidade de firmware**: Duas variantes de firmware USB (2 canais e 6 canais), firmware I2S, atualização DFU via dfu-util e persistência de configuração entre ciclos de energia
...
| **Codec de áudio** | TLV320AIC3104, lida com conversão de áudio e saída DAC |
...
| **Conector AUX de 3,5 mm** | Saída lateral para fones de ouvido acionada pelo DAC integrado |
| **Conector JST para alto-falante** | Conector montado na vertical que aciona alto-falantes de até 10 W a 4 Ω; ilhas de solda preservadas para conexão direta do alto-falante |
...
| **Conectores de IO do XMOS 3800** | Pinos/ilhas expostos para I2C, I2S, 5V/GND, pinos de alto-falante e IO restante do XVF3800 |
...
| **GPIO do XIAO ESP32S3** | GPIOs restantes podem ser usados para expansão de IO e periféricos |
...
A placa principal do reSpeaker Flex inclui uma área dedicada para soldagem de um módulo Seeed Studio XIAO ESP32S3, permitindo conectividade Wi-Fi e Bluetooth em cima do pipeline de processamento de áudio do XVF3800. Os SKUs que incluem XIAO (-C4-1 e -L4-1) são enviados com esse módulo já montado; os SKUs base (-C4-0 e -L4-0) deixam a área desocupada.
Quando o XIAO é instalado, as seguintes conexões são feitas entre ele e o XVF3800:

- **Barramento I2S** — barramento duplo (reprodução e gravação) com BCLK, MCLK e LRCLK compartilhados para streaming de áudio bidirecional
- **Barramento I2C** — para leitura e gravação de parâmetros de configuração do XVF3800 a partir do XIAO
- **Linha RST** — o XIAO pode acionar um reset de hardware no XVF3800 por meio de um pino de IO dedicado
- **IO restante do XIAO** — disponibilizado em pinos/ilhas identificados para expansão pelo usuário

Uma antena Wi-Fi FPC (para o XIAO ESP32S3 sem antena integrada) é incluída como acessório em todas as variantes equipadas com XIAO.
...
A interface FPC do array de microfones é um conector de 24 pinos, passo de 0,5 mm, com trava, e o cabo flat FPC de 20 cm incluído na caixa é chaveado para este conector.
...
O botão de reset (RST) fornece um reset de hardware para o reSpeaker Flex quando pressionado, reiniciando o chip e reinicializando o sistema desde o começo, como em um ciclo completo de energia.
...
Aqui você pode ver como conectar os alto‑falantes usando o conector de fone de ouvido AUX de 3,5 mm ou a interface JST de alto‑falante onboard, dependendo da sua preferência de saída de áudio.
...
Se você já tiver gravado o **firmware I2S** e quiser voltar para o **firmware USB**, pode entrar no **Safe Mode** e gravar novamente o firmware USB usando **USB DFU**.
...
- Você precisa **gravar um novo firmware**, mas o firmware atual **não está respondendo**.
...
- Mantendo pressionado o **botão Boot, reconecte a alimentação.**
- O **LED vermelho começará a piscar**, confirmando que o dispositivo entrou no **Safe Mode.**
Agora o dispositivo está executando o **firmware do Safe Mode armazenado na partição Factory.**
...
:::note 
Certifique‑se de que você precisará baixar todo o repositório.
:::
...
Duas variantes de firmware estão disponíveis: **respeaker_xvf3800_usb_dfu_firmware_v2.0.x.bin**, que fornece áudio de 2 canais, e **respeaker_xvf3800_usb_dfu_firmware_6chl_v2.0.x.bin**, que fornece áudio de 6 canais. Ambas as versões do firmware operam a uma taxa de amostragem de 16 kHz com profundidade de 32 bits.

Você pode explorar esses arquivos de firmware neste link

| Firmware                         | Canais | Notas                                                   |
|----------------------------------|--------|---------------------------------------------------------|
| respeaker_flex_ua-io16-6ch-cir.bin | 6 canais | 16 kHz, 6 canais usando arranjo de microfones circular  |
| respeaker_flex_ua-io16-6ch-lin.bin | 6 canais | 16 kHz, 6 canais usando arranjo de microfones linear    |
| respeaker_flex_ua-io16-cir.bin     | 2 canais | 16 kHz, saída estéreo usando arranjo de microfones circular |
| respeaker_flex_ua-io16-lin.bin     | 2 canais | 16 kHz, saída estéreo usando arranjo de microfones linear   |
| respeaker_flex_ua-io48-cir.bin     | 2 canais | 48 kHz, saída estéreo usando arranjo de microfones circular |
| respeaker_flex_ua-io48-lin.bin     | 2 canais | 48 kHz, saída estéreo usando arranjo de microfones linear   |

Conecte o reSpeaker Flex ao seu PC via cabo USB. Observe que você precisa usar a porta XMOS USB‑C (perto do botão RST) para gravar o firmware da XMOS.
...
| Firmware                                      | Canais | Notas                                                     |
|-----------------------------------------------|--------|-----------------------------------------------------------|
| respeaker_flex_inthost-lr16-cir-i2c.bin      | 2 canais | 16 kHz, saída estéreo, arranjo de microfones circular   |
| respeaker_flex_inthost-lr16-lin-i2c.bin      | 2 canais | 16 kHz, saída estéreo, arranjo de microfones linear|
| respeaker_flex_inthost-lr48-cir-i2c.bin      | 2 canais | 48 kHz, saída estéreo, arranjo de microfones circular|
| respeaker_flex_inthost-lr48-lin-i2c.bin      | 2 canais | 48 kHz, saída estéreo, arranjo de microfones linear|
...
`dfu-util` é uma ferramenta de linha de comando para atualização de firmware de dispositivos via USB.
...
3. Vá para **Audio Setting** e configure:
...
No alsamixer, use as teclas de seta esquerda/direita para navegar até o dispositivo de som correto. Use a seta para cima para aumentar o volume.
