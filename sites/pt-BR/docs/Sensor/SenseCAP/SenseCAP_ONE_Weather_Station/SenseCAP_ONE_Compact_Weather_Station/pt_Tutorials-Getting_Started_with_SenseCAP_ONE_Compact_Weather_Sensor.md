---
description: Primeiros passos com o sensor meteorológico compacto SenseCAP ONE
title: Primeiros passos com o sensor meteorológico compacto SenseCAP ONE
keywords:
  - SenseCAP
...
---
# Primeiros passos com o sensor meteorológico compacto SenseCAP ONE
...
- A interface USB Type-C permite conectar o seu computador ao dispositivo com um cabo USB Type-C normal para configuração.

- A interface de dados principal pode ser conectada ao cabo M12 de 8 pinos, suportando múltiplos protocolos de barramento
...
Observação: o cabo deve estar alinhado com a parte inferior antes de ser inserido. Caso contrário, os pinos desalinhados podem causar comunicação anormal.
...
Ao usar o dispositivo com função de aquecimento, é necessária uma fonte de alimentação separada de 24 V (recomenda-se 24V@1A). O fio cinza 5 é conectado ao negativo da fonte de alimentação, e o fio rosa 6 é conectado ao polo positivo da fonte de alimentação.
...
Existem dois métodos principais de instalação: montado em um poste com uma luva ou em uma plataforma com uma placa de flange.
...
Recomenda-se que o diâmetro do poste seja menor ou igual a 75 cm.
...
Selecione o software para o respectivo sistema operacional, Windows, macOS ou Linux, conforme suas necessidades.
...
2. Defina o Baud rate para 9600.
...
Nas configurações de aplicação, você pode definir o ciclo para a ferramenta ler os dados do sensor, sendo o mínimo 2 s, e um intervalo de pontos para a curva.
...
Na página de atualização, você precisará escolher atualizar o firmware da placa principal ou o firmware da placa driver. Selecione o arquivo de firmware no seu repositório local e clique em "Update Now". Se houver uma interrupção inesperada de energia durante o processo de atualização, a atualização não será executada. Você precisará seguir o mesmo processo para atualizar o firmware.
...
1. Abra `SenseCAP One Configuration Tool`
...
- Se você receber a mensagem 0XA correspondente na janela de recebimento serial, a configuração foi bem-sucedida. Caso contrário, verifique a porta COM e a taxa de transmissão.
...
(download em &lt;https://www.modbustools.com/download.html&gt;).
...
Configure os parâmetros de conexão: taxa de transmissão 9600bps, 8 bits de dados,
...
3. Retorna dados de temperatura 0x00006E8C (Hex), convertidos para decimal = 28300, obtenha a temperatura do ar correspondente dividindo por 1000, temperatura do ar = 28300/1000 = 28,3 °C
...
Ler registrador 0x0000~0x0005.

Enviar comando: 0A 04 00 00 00 06 71 73（Código de verificação）；

Retorno: 26 04 40 00 00 70 80（Temperatura）00 00 95 10（Umidade） 06 07 94 40（Pressão do ar）99 09（Código de verificação）；Ler registrador 0x0008~0x0013. Enviar comando: 0A 04 00 08 00 0C 70 B6（Código de verificação）；
...
Ler registrador 0x0000-0x001F e 0x0030-0x0033.
...
Consulta dos dados do sensor de ruído (endereço: 40, 0x28) :
...
Se a consulta for bem-sucedida, as seguintes informações são retornadas:
...
### Formato de Comando de Configuração
...
A comunicação SDI-12 adota três fios, dos quais dois são fios de alimentação do sensor e o outro é o fio de sinal SDI-12.
...
O sensor é alimentado por uma fonte de alimentação CC de 3,6~16 V. Após o sensor ser ligado, ele entra imediatamente no modo de suspensão e aguarda o equipamento de aquisição de dados enviar instruções. O SDI-12 usa uma taxa de baud de 9600 bps, 1 bit de início (nível alto), 7 bits de dados (alto 0 e baixo 1, anti-lógica), 1 bit de paridade par e 1 bit de parada.
...
E conecte o fio vermelho (Vin+ positivo de alimentação) e o fio marrom (Vin- terra de alimentação) à fonte de alimentação de 12 V.
...
- Envie o comando de consulta de endereço do dispositivo "?!", se você conseguir ver a resposta "0", significa que a conexão está OK.
...
Ler temperatura do ar, umidade do ar, pressão barométrica, intensidade luminosa
...
Em seguida, envie "Comando de leitura do valor medido 0D0!" para obter os 6 valores medidos desta medição, que são direção mínima do vento 345,9 graus, direção máxima do vento 347,5 graus, direção média do vento 346,3 graus, velocidade mínima do vento 2,8 m/s, e velocidade máxima do vento 2,8 m/s, velocidade média do vento 2,8 m/s.
...
Em seguida, envie "comando de medição contínua 0R2! o dispositivo retorna 4 valores medidos: precipitação acumulada 1,2 mm, duração da precipitação acumulada 20 segundos, intensidade de precipitação 1,2 mm/h, intensidade máxima de precipitação 72,0 mm/h.
...
A janela de tempo médio padrão é de 5 s. Dentro desta janela, o dispositivo coletará dados de velocidade e direção do vento cinco vezes e retornará um valor médio.
