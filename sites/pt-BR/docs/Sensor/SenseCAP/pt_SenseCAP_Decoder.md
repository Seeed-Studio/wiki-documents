...
O decodificador SenseCAP é usado para decodificar as mensagens LoRaWAN enviadas pelos dispositivos SenseCAP LoRaWAN®. Após a decodificação, as aplicações dos usuários receberão mensagens mais amigáveis e legíveis.

### Rastreador SenseCAP T1000

[**SenseCAP T1000**](https://www.seeedstudio.com/sensecap-t1000-tracker?utm_source=emailsig&utm_medium=emailsig&utm_campaign=emailsig) é um rastreador LoRaWAN® compacto que utiliza GNSS/Wi-Fi/Bluetooth para rastreamento preciso de localização em ambientes internos e externos. Ele conta com capacidades de autoajuste geográfico, armazenamento local de dados e uma impressionante autonomia de bateria de vários meses. Além disso, é equipado com sensores de temperatura, luz e movimento, tornando-o ideal para uma variedade de aplicações baseadas em localização.
...
<summary>Para ChirpStack(V3)</summary>
...
[**SenseCAP T2000 Tracker**](https://www.seeedstudio.com/SenseCAP-Asset-Tracker-T2000-A-p-6580.html), um rastreador de ativos LoRaWAN® de nível industrial, suporta posicionamento por GNSS, Bluetooth e Wi-Fi para rastreamento confiável em ambientes internos e externos. Ele possui proteção IP67, um acelerômetro embutido de 3 eixos que detecta o estado de movimento e um botão antiviolação que aciona um alarme de máxima prioridade se o dispositivo for removido. O T2000-A e o T2000-B suportam operação de bateria de longa duração, enquanto o T2000-C com energia solar e bateria recarregável garante uso contínuo em ambientes externos, tornando a série ideal para rastreamento de ativos de longo prazo e livre de manutenção.
...
A [Wio Tracker 1110 Dev Board](https://www.seeedstudio.com/Wio-Tracker-1110-Dev-Board-p-5799.html) é baseada no [Wio-WM1110 Wireless Module](https://www.seeedstudio.com/Wio-WM1110-Module-LR1110-and-nRF52840-p-5676.html) e integra o transceptor LoRa® [LR1110 da Semtech](https://www.semtech.com/products/wireless-rf/lora-edge/lr1110) e uma interface de rádio multiuso para geolocalização, sendo uma plataforma de desenvolvimento de rastreamento baseada em LoRa, fácil de usar.

Com seu tamanho compacto e interfaces ricas, a Wio Tracker 1110 Dev Board vem convenientemente equipada com uma antena integrada para fácil implantação. Ela suporta o ambiente de desenvolvimento Arduino e a pilha de protocolo LoRaWAN, o que a torna ideal para projetos de IoT relacionados a rastreamento.
...
<summary>Para Chirp Stack</summary>
...
[SenseCAP S210X](https://www.seeedstudio.com/catalogsearch/result/?q=s210x) é uma série de sensores LoRaWAN® sem fio. Ele pode cobrir uma faixa de transmissão de 2 km em cenários urbanos e 10 km em cenários com linha de visada, mantendo um menor consumo de energia durante o processo de transmissão. Junto com uma bateria substituível que suporta até 10 anos de uso e um invólucro industrial IP66. Ele suporta temperatura de operação de -40 ~ 85℃ e pode ser implantado em ambientes severos. SenseCAP S210X é compatível com o protocolo LoRaWAN® V1.0.3 e pode funcionar com um gateway LoRaWAN®.
...
### Data Logger SenseCAP S2100

[SenseCAP S2100 Data Logger](https://www.seeedstudio.com/SenseCAP-S2100-LoRaWAN-Data-Logger-p-5361.html) é um dispositivo versátil que pode se conectar a sensores MODBUS-RTU RS485/Analógico/GPIO, permitindo a fácil transmissão de dados para a rede LoRaWAN. Com seu design LoRa e IP66, este dispositivo apresenta estabilidade e confiabilidade impressionantes e pode cobrir um longo alcance de transmissão mantendo consumo de energia ultrabaixo. Ele é perfeito para uso externo e pode ser alimentado por bateria ou conectado a uma fonte de alimentação externa de 12 V para ainda maior flexibilidade. Quando conectado à fonte de alimentação de 12 V, a bateria interna substituível atua como fonte de alimentação de backup. Além disso, o Data Logger S2100 é otimizado para OTA com Bluetooth integrado, tornando a configuração e as atualizações rápidas e simples. Para completar, o conversor S2110 permite que o Data Logger S2100 se conecte a Sensores Grove, tornando-o uma excelente escolha para sensores LoRaWAN em nível industrial feitos por você mesmo e para implantação em pequena escala.
...
### SenseCAP S2120 Sensor Climático 8 em 1

[SenseCAP S2120 8-in-1 LoRaWAN Weather Sensor](https://www.seeedstudio.com/sensecap-s2120-lorawan-8-in-1-weather-sensor-p-5436.html) mede temperatura do ar, umidade, velocidade do vento, direção do vento, precipitação, intensidade de luz, índice UV e pressão barométrica. Ele permite baixo custo de manutenção devido ao seu consumo de energia ultrabaixo, desempenho confiável, Bluetooth integrado e serviço de app para configuração OTA e gerenciamento remoto de dispositivos. Ele suporta aplicações em múltiplos cenários como quintais, jardins, agricultura inteligente, meteorologia, cidade inteligente, etc.
...
[SenseCAP A1101 - LoRaWAN Vision AI Sensor](https://www.seeedstudio.com/SenseCAP-A1101-LoRaWAN-Vision-AI-Sensor-p-5367.html) é um sensor de imagem inteligente com TinyML Edge AI habilitado. Ele suporta uma variedade de modelos de IA, como reconhecimento de imagem, contagem de pessoas, detecção de alvos, reconhecimento de medidores etc. Também suporta o treinamento de modelos com TensorFlow Lite.
...
Antes de configurar o decodificador, configure corretamente seus sensores e gateway de acordo com o manual do produto e, em seguida, conecte-se ao servidor de rede LoRaWAN necessário.

Tomamos o The Things Stack como exemplo; configure o decodificador de acordo com as etapas a seguir:

##### Configurar o Decodificador de Payload

- Navegue até a guia `Payload Formats` do seu dispositivo.
- Selecione `Custom` para `Payload Format`
- Copie e cole todo o conteúdo de `decoder.js` na área de texto `decoder`.
- Clique em `save payload functions`
...
##### Verificar as Mensagens Decodificadas

Você pode testar primeiro o script de decodificação com um payload de exemplo.

Para fazer isso, copie um pacote de dados brutos como `01 01 10 98 53 00 00 01 02 10 A8 7A 00 00 AF 51` na entrada de texto `Payload` e selecione o `FPort` de acordo com o manual do dispositivo; em seguida, clique no botão `Test`. Você verá abaixo uma estrutura JSON analisada com sucesso.
...
Então vamos conferir a mágica do script. Navegamos até a guia `Live Data`, e você pode expandir qualquer mensagem enviada para verificar os `Event Fields` no payload. Esses campos são preenchidos pelo script.
...
Se você estiver assinando as mensagens com a API TTN's MQTT Data, também receberá os campos de payload em JSON analisados.
