...
Alimentada pelo **XIAO ESP32-S3 Plus**, a placa de display EE05 suporta uma ampla variedade de displays ePaper de 24 pinos e permite design e implantação de UI sem código com o SenseCraft HMI. Com IO de extensão e um conector de bateria com chave de energia, é ideal para aplicações de display inteligente de baixo consumo.
...
- **Conector de bateria com chave:** Fornece conexão simples de bateria e integra uma chave, permitindo gerenciamento eficiente de energia e economia de energia.
- **Design e implantação de UI sem código com SenseCraft HMI:** Projete e implemente painéis personalizados com facilidade usando o SenseCraft HMI, nossa nova plataforma sem código com tecnologia de IA. Crie sua UI por meio de simples elementos de arrastar e soltar, escolha entre nossos ricos modelos ou deixe a IA ajudar você. Conecte dados em tempo real a partir de APIs web e, em seguida, implemente seu painel final em um display ePaper com apenas alguns cliques. Do design à configuração de dados até a implantação, tudo acontece perfeitamente em uma única plataforma.
...
Aqui, usamos um display monocromático de 7,5" como exemplo. As etapas são idênticas para outras telas suportadas; você só precisa selecionar o identificador de tamanho de tela apropriado no driver.
...
- **Verifique a fonte de alimentação**: Para telas maiores (como a de 7,5"), certifique-se de que sua fonte de alimentação possa fornecer corrente adequada durante a atualização da tela.
