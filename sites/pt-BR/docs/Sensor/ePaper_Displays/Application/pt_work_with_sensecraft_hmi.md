---
description: Use o SenseCraft Seeedash para criar e implantar dashboards sem código para dispositivos de display ePaper compatíveis da Seeed.
title: Trabalhar com SenseCraft Seeedash
keywords:
  - display ePaper
  - SenseCraft Seeedash
...
# Trabalhar com SenseCraft Seeedash

[SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) é a plataforma de design de interface sem código da Seeed Studio para dispositivos de display ePaper. Você pode criar dashboards, galerias de imagens, calendários, páginas de RSS, páginas de conteúdo web e outras telas de informação sempre ativas em um navegador e, em seguida, implantá-las em um dispositivo compatível via Wi-Fi.

Este Wiki é um guia de aplicação para Display ePaper. Ele explica o caminho mais curto de um dispositivo ePaper da Seeed até uma página SenseCraft Seeedash funcional e usa uma placa de display ePaper mais uma tela ePaper como exemplo de configuração de hardware.
...
Use este guia quando quiser usar o SenseCraft Seeedash com um produto ePaper compatível da Seeed e concluir rapidamente o seguinte fluxo de trabalho:

1. Confirmar ou gravar o firmware SenseCraft Seeedash.
2. Conectar o dispositivo ao Wi-Fi.
3. Adicionar o dispositivo ao seu workspace do SenseCraft Seeedash.
4. Implantar sua primeira página no display ePaper.

Para o manual completo da plataforma, incluindo configurações de conta, detalhes do editor, modelos, widgets de dados e notas de versão, consulte a [documentação oficial do SenseCraft Seeedash](https://sensecraft-hmi-docs.seeed.cc/en/overview/).

## Antes de começar

Prepare os seguintes itens antes de usar o SenseCraft Seeedash:

- Um dispositivo de display ePaper compatível da Seeed.
- Uma conta SenseCraft. Você pode criar uma a partir da página de login do [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi).
- Uma rede Wi-Fi de 2,4 GHz.
- Um cabo de dados USB-C se o seu dispositivo precisar de gravação de firmware.
- Um computador executando um navegador que suporte gravação via porta serial pela ferramenta web do SenseCraft Seeedash.

:::tip
Diferentes produtos ePaper usam diferentes builds de firmware Seeedash. Sempre selecione o firmware que corresponde exatamente ao seu dispositivo e tamanho de tela.
:::

## Documentação oficial do SenseCraft Seeedash

O SenseCraft Seeedash possui um site de documentação dedicado. Use este Wiki para o fluxo de início rápido do Display ePaper e use a documentação oficial para instruções completas da plataforma.
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      <td>Visão geral</td>
      <td>Entender o que é o SenseCraft Seeedash e o que ele pode fazer.</td>
...
As etapas abaixo usam a **XIAO ePaper Display Board (ESP32-S3) - EE04** com o **display ePaper Spectra 6 de 7,3"** como exemplo. Outros dispositivos ePaper compatíveis da Seeed seguem o mesmo fluxo de plataforma, mas o build de firmware, o tamanho da tela e a montagem de hardware podem ser diferentes.
...
:::note
Este exemplo não é uma lista de compatibilidade. Para a matriz atual de hardware e firmware compatíveis, consulte sempre a [página de hardware compatível do SenseCraft Seeedash](https://sensecraft-hmi-docs.seeed.cc/en/supported_hardware/).
:::

## Etapa 1: Confirmar ou gravar o firmware Seeedash

A primeira etapa é garantir que o seu dispositivo ePaper esteja executando o firmware SenseCraft Seeedash.

<Tabs groupId="hmi-hardware">
<TabItem value="reterminal" label="reTerminal E Series" default>

O reTerminal E Series é projetado para funcionar com o SenseCraft Seeedash pronto para uso. Ligue o dispositivo e continue para a [Etapa 2: Conectar o dispositivo ao Wi-Fi](#Etapa-2:-Conectar-o-dispositivo-ao-Wi-Fi).

Se você tiver gravado anteriormente outro firmware, abra a [página de dispositivo do SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi/device), escolha o modelo do seu dispositivo e siga as instruções de firmware exibidas na tela.
...
Para uma placa de display mais uma tela ePaper separada, grave o firmware Seeedash que corresponde à sua placa e ao seu painel.

**Etapa 1.** Faça login no [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) e, em seguida, abra **Tools** no menu superior.
...
**Etapa 5.** Aguarde o display ePaper atualizar e exibir o código QR de configuração.
...
Depois que o dispositivo estiver executando o firmware Seeedash, conecte-o à sua rede Wi-Fi local.

**Etapa 1.** Conecte seu telefone ou computador ao ponto de acesso Wi-Fi exibido na tela ePaper. Este ponto de acesso temporário não requer senha.

O nome do ponto de acesso depende do tipo de dispositivo. Para uma placa de display ePaper com uma tela separada, o nome pode aparecer como `ePaper DIY Kit-xxxx`. Para um dispositivo reTerminal E Series, o nome pode aparecer como `reTerminal E100x-xxxx`. Em ambos os casos, `xxxx` geralmente representa os quatro últimos caracteres do endereço MAC.
...
**Passo 2.** Escaneie o código QR na tela de ePaper. Se a página de configuração não abrir automaticamente, abra um navegador e acesse `192.168.4.1`.
...
**Passo 3.** Selecione sua rede Wi-Fi local, insira a senha e clique em **Connect**.

:::tip
Use uma rede Wi-Fi de 2,4 GHz. Muitos dispositivos baseados em ESP32-S3 não se conectam a redes Wi-Fi de 5 GHz.
:::

**Passo 4.** Aguarde até que o dispositivo entre na rede e exiba um código de pareamento.
...
## Passo 3: Adicionar o dispositivo ao SenseCraft Seeedash

Após a configuração do Wi-Fi, adicione o dispositivo ao seu workspace do SenseCraft Seeedash.

**Passo 1.** Abra a página **Device** e clique em **Add Device**.
...
**Passo 2.** Insira um nome para o dispositivo e o código de pareamento exibido na tela de ePaper e, em seguida, clique em **Create**.
...
**Passo 3.** Confirme que o dispositivo aparece no seu painel.

## Passo 4: Fazer o deploy da sua primeira página

Depois que o dispositivo estiver pareado, você poderá fazer o deploy de conteúdo do SenseCraft Seeedash para a tela de ePaper.
...
O exemplo abaixo mostra uma página do SenseCraft Seeedash feita o deploy para um display de ePaper de 7,3".
...
- Digite novamente a senha do Wi-Fi com cuidado.
...
- Verifique novamente o modelo do dispositivo, o tamanho da tela e a entrada de firmware na página SenseCraft Seeedash Tools.
...
### P4: Por que a página feita o deploy parece diferente da visualização no editor?
...
- Evite texto muito pequeno se a resolução da tela for baixa.
- Use cores de alto contraste para telas monocromáticas ou em escala de cinza.
- Consulte a documentação oficial para detalhes sobre o editor e o deploy.

## Recursos

- [Plataforma SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi)
- [Documentação oficial do SenseCraft Seeedash](https://sensecraft-hmi-docs.seeed.cc/en/overview/)
- [Hardware compatível com SenseCraft Seeedash](https://sensecraft-hmi-docs.seeed.cc/en/supported_hardware/)
- [Introdução ao SenseCraft Seeedash](https://sensecraft-hmi-docs.seeed.cc/en/guides/getting_started/)
- [Notas de versão do SenseCraft Seeedash](https://sensecraft-hmi-docs.seeed.cc/en/release_note/)
