...
O suporte de biblioteca relacionado ao toque está planejado para um futuro lançamento open-source, proporcionando maior flexibilidade para personalizar os painéis. Mais atualizações serão compartilhadas em breve.
...
- Visuais nítidos com display ePaper de alta resolução e 16 níveis de escala de cinza
...
- **Displays inteligentes para escritórios:** Mostre disponibilidade de salas de reunião, agendas de escritório ou orientações de direção em displays ePaper de baixo consumo e sempre ligados.
...
Certifique-se de que o **interruptor de energia do dispositivo esteja em ON** e use um **cabo USB-C de dados** (não apenas para carregamento). Abra o Terminal e execute:
```bash
ls /dev/tty.wch*
...
**Passo 1.** Conecte-se ao ponto de acesso Wi-Fi do dispositivo a partir do seu smartphone ou computador. O nome do AP aparecerá na tela (sem necessidade de senha). As credenciais da rede são `reTerminal E1003-{MAC Address}`.
...
**Passo 5.** O aplicativo enviará as credenciais de Wi-Fi para o dispositivo e o adicionará à sua conta SenseCraft. Uma mensagem "Device added successfully" aparecerá ao concluir. Agora você pode tocar em **Start exploring!** para prosseguir.
...
1. **Selecione seu dispositivo** — conecte o reTerminal E1003 ao seu computador via cabo USB-C, certifique-se de que o interruptor de energia esteja em **ON**, depois clique em **Select** e escolha seu dispositivo na lista.
...
2. Não é possível gravar o firmware corretamente quando o dispositivo está desligado ou em modo de suspensão. Se você selecionar a porta correta para o seu dispositivo, mas nunca vir o progresso de gravação do firmware, talvez seja necessário despertar o dispositivo pressionando o botão **Refresh** na parte superior da unidade e tentar novamente.
...
O [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) possui um Wiki dedicado com tutoriais completos e documentação de recursos. Para guias detalhados (configuração de conta, gravação de firmware, uso avançado do editor, implantação etc.), consulte:
...
Se você vir o caminho do dispositivo na saída, o driver estará instalado corretamente e seu reTerminal estará pronto para uso!
