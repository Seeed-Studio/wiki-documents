---
description: Este artigo descreve como fazer o reTerminal E Series ePaper Display funcionar com Arduino.
...
# Introdução ao reTerminal E Series ePaper Display no Arduino
...
O reTerminal E Series representa o mais recente avanço da Seeed Studio em soluções HMI industriais, apresentando o ESP32-S3 como controlador principal e displays ePaper integrados. Este guia irá conduzi-lo pela programação do display ePaper em dispositivos reTerminal E Series usando a Arduino IDE, permitindo criar interfaces e aplicações personalizadas com excelente visibilidade e consumo de energia ultrabaixo.
...
**Passo 5.** Conecte o seu reTerminal E Series ePaper Display ao computador usando um cabo USB-C.
...
#### Programando o reTerminal E1001 (ePaper preto e branco de 7,5 polegadas)
...
#### Programando o reTerminal E1002 (ePaper colorido de 7,3 polegadas)
...
**Passo 6.** Envie o sketch para o seu reTerminal E1002. O display mostrará gráficos coloridos demonstrando as capacidades de cores completas do display de ePaper.
...
**Passo 4.** Vá para a [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/) e selecione **reTerminal E1003** na lista de dispositivos.
...
**Passo 4.** Vá para a [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/) e selecione **reTerminal E1004** na lista de dispositivos.
...
**Passo 1.** Vá para o repositório GxEPD2 no GitHub. Clique no botão "Code" e selecione "Download ZIP" para salvar a biblioteca no seu computador.
...
**Passo 2.** Na IDE Arduino, instale a biblioteca a partir do arquivo baixado. Navegue até **Sketch > Include Library > Add .ZIP Library...** e selecione o arquivo ZIP que você acabou de baixar.

**Passo 3.** A biblioteca `GxEPD2` requer a `Adafruit GFX Library` para funcionar, que você também deve instalar. A maneira mais fácil de fazer isso é através do Library Manager: vá para **Tools > Manage Libraries...**, procure por "Adafruit GFX Library" e clique em "Install".
...
Aqui está o código de exemplo para exibir "Hello World!" no display de ePaper preto e branco do reTerminal E1001 usando a biblioteca `GxEPD2`. Defina `EPD_SELECT` como `0` para selecionar o driver para o E1001.
...
Para o reTerminal E1002, você simplesmente precisa alterar o valor de `EPD_SELECT` para `1`. Isso selecionará o driver apropriado para o display de ePaper colorido completo de 7,3 polegadas. O restante do código permanece o mesmo.
...
4. **Eliminação de ruído (debouncing)**: Um simples atraso de 200ms após cada pressionamento de botão evita múltiplas detecções de um único toque devido ao bounce mecânico.
...
2. **Medição**: Usa `measureHighPrecision()` para leituras precisas (leva ~8,3ms)
...
- Para uma bateria LiPo totalmente carregada, espere cerca de 4,2V
...
:::note
O reTerminal E Series suporta apenas cartões MicroSD de até 64GB formatados com o sistema de arquivos **Fat32**.
:::
...
- **Detecção do cartão:** A função `isCardInserted()` lê o `SD_DET_PIN`. No hardware do reTerminal, esse pino é mantido em nível BAIXO quando um cartão está presente.
...
Se o cartão MicroSD não for usado no seu projeto, recomendamos desligar o dispositivo e remover o cartão antes de executar o programa de exibição. Se o cartão tiver sido inserido no reTerminal, você precisará adicionar o código acima para garantir que a tela seja exibida corretamente, independentemente de estar usando um cartão MicroSD ou não.
