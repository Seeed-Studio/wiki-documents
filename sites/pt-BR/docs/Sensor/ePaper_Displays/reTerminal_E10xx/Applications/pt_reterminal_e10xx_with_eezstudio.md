...
# Introdução ao uso do display ePaper da reTerminal E Series com o EEZ Studio

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/EEZStudio/eez.jpg" style={{width:600, height:'auto'}}/></div>

## Introdução

Este guia irá conduzi-lo pelo processo de criação de uma interface de usuário profissional para a reTerminal E Series usando a ferramenta de design visual [**EEZ Studio**](https://www.envox.eu/studio/studio-introduction/). Você aprenderá como criar elementos de UI, gerar o código necessário e, por fim, fazer o deploy no dispositivo usando a Arduino IDE. Esse processo permite que você construa interfaces personalizadas com excelente visibilidade e consumo de energia ultrabaixo, ideal para aplicações de IHM.
...
- Prototipagem rápida – Teste e valide rapidamente ideias de design, economizando tempo e custo de desenvolvimento.
- Suporte multiplataforma – Construa aplicações que podem rodar em múltiplos sistemas operacionais e alvos embarcados.
...
Embora o EEZ Studio e o SquareLine Studio sejam ambos ferramentas de design de GUI, eles se concentram em objetivos e cenários de uso diferentes:
...
<table class="EEZStudio-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Focado principalmente em geração de display/UI, não em integração com dispositivos externos</td>
        </tr>
        <tr>
            <th>Open source</th>
        ...
        <tr>
            ...
            <td>Gera principalmente código de UI para ser compilado em um projeto embarcado</td>
        </tr>
    </tbody>
</table>

## Introdução ao EEZ Studio
...
Na lista de templates de projeto à esquerda, localize e selecione o projeto “LVGL”.

- `Name:` Dê um nome ao arquivo do seu projeto. Para facilitar a explicação do conteúdo subsequente em nossos artigos, iremos nos referir a ele aqui como EEZ_UI.
- `LVGL Version:` Especifica a versão da biblioteca LVGL usada pelo projeto. Selecione 9.x no menu suspenso.
- `Location:` Especifique o caminho de armazenamento dos arquivos do projeto no seu computador. O caminho padrão normalmente aponta para uma pasta de projetos padrão dentro do diretório de download ou instalação do EEZ Studio (por exemplo, C:\Users\SeuUsuario\Documents\EEZ Studio\Projects ou um caminho semelhante). Se você precisar alterar o local de armazenamento, clique no ícone de pasta (ou reticências ...) à direita da caixa de texto Location.
...
Ao construir seu projeto de UI LVGL para a reTerminal com E-paper, você precisa configurar corretamente as Project Settings do EEZ Studio para garantir que os arquivos de UI gerados sejam compatíveis com o seu ambiente Arduino ou embarcado.

Passo 1. Abrir as Project Settings
...
Passo 2. Configurar Build Output e LVGL Include
...
Passo 3. Definir a resolução do display

Em seguida, ainda em Settings → General, configure a resolução do seu display:
- Display width: 800
- Display height: 480

🧩 Esses parâmetros definem a resolução alvo da tela E-paper para o layout da UI e o mapeamento de coordenadas.
...
No EEZ Studio, você pode montar interfaces rapidamente arrastando e soltando componentes. Utilize ferramentas como Styles, Fonts, Bitmaps e Themes para controlar com precisão os efeitos visuais, criando uma experiência de usuário profissional e diferenciada.

A seguir está uma introdução às ferramentas recomendadas：
...
- Styles: Atributos visuais usados para unificar e reutilizar elementos da interface, garantindo consistência e manutenção eficiente.
- Bitmap: Comumente usado para planos de fundo, ícones, logotipos etc., para aprimorar a apresentação visual.
- Fonts: Garantem legibilidade clara do texto, ao mesmo tempo em que suportam conteúdo multilíngue e estilo de marca.
- Themes: Definem a estética geral da interface, permitindo alternâncias rápidas como modo claro/escuro.
- Groups: Organizam vários widgets em conjunto para gerenciamento e layout unificados.

Neste tutorial, vou guiá-lo no design da UI da página inicial de um site ou aplicativo. Depois que você dominar as técnicas principais, poderá aplicá-las para criar com facilidade qualquer interface que desejar.
...
- Selecione o canvas cujo a cor de fundo você precisa alterar.
...
- Clique no ícone Bitmaps na barra lateral à direita para adicionar uma imagem.

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/EEZStudio/a3.jpg" style={{width:900, height:'auto'}}/></div>

- Dê nomes às imagens que você selecionar; você precisará escolher as imagens com base em sua convenção de nomenclatura mais tarde.
...
- Clique na caixa Fonts na coluna mais à direita para adicionar seus formatos de fonte.
...
O conteúdo acima descreve o uso básico desses cinco componentes. Você pode organizá‑los de acordo com o diagrama de referência ou com a interface de UI que você mesmo projetar.

### Geração de Código e Implantação
...
Clique no ícone Compile (botão de marca de verificação) para verificar se há erros de sintaxe ou lógica no projeto.
Clique no ícone Build (botão de chave inglesa) para gerar arquivos como código da UI, dados de imagem e dados de fonte.
...
Ao concluir, o EEZ Studio exibirá um aviso em verde informando “Build successful”, indicando que o código foi gerado com sucesso.

## Preparação do Ambiente

Para programar o ePaper Display do reTerminal E Series com Arduino, você precisará configurar a Arduino IDE com suporte a ESP32.
...
**Step 5.** Conecte o reTerminal E Series ePaper Display ao computador usando um cabo USB‑C.

**Step 6.** Selecione a porta correta em **Tools > Port**.

## Programação do ePaper Display

O **reTerminal E1001 possui um ePaper display preto e branco de 7,5 polegadas**, enquanto o **reTerminal E1002 é equipado com um ePaper display colorido completo de 7,3 polegadas**. Ambos os displays oferecem excelente visibilidade em várias condições de iluminação com consumo de energia ultrabaixo, tornando‑os ideais para aplicações industriais que exigem displays sempre ligados com consumo mínimo de energia.
...
:::note
Se você já instalou a biblioteca TFT_eSPI anteriormente, talvez seja necessário removê‑la temporariamente ou renomeá‑la na pasta de bibliotecas do Arduino para evitar conflitos, pois a Seeed_GFX é um fork da TFT_eSPI com recursos adicionais para displays da Seeed Studio.
:::
...
**Step 2.** Crie um novo arquivo chamado `driver.h` na mesma pasta do seu sketch. Você pode fazer isso clicando no botão de seta na Arduino IDE e selecionando "New Tab", depois nomeando‑o como `driver.h`.
...
O ePaper display colorido completo oferece suporte às cores vermelho, preto e branco, permitindo interfaces visualmente mais ricas.
...
## Implantando o projeto EEZ Studio no Arduino
...
Esses dois arquivos de driver subjacentes precisam ser adicionados ao nosso diretório de projeto.
...
Porte os seguintes arquivos de projeto (incluindo quatro arquivos) para a Biblioteca do Arduino para uso em projetos Arduino.
O `EEZ_UI file` é gerado durante o processo de compilação no EEZStudio. Você precisa localizar o caminho onde o salvou.
...
Três botões correspondem a três telas diferentes, começando com o verde: HOME\Workstation\Plant em sequência.
...
- `pinMode(..., INPUT_PULLUP):` Configura o pino do botão como modo de entrada e habilita o resistor de pull-up interno. Isso garante que o pino permaneça em nível alto (HIGH) quando o botão não é pressionado e se conecte ao terra, tornando-se baixo (LOW), quando pressionado.

- `ui_init()` and `loadScreen():` Essas funções inicializam a biblioteca de interface de usuário LVGL e carregam a tela especificada
...
- Atualização do E-ink: `e1002_display_should_refresh()` e `e1002_display_refresh()` gerenciam a atualização da tela E-ink. Diferente das telas LCD, o E-ink não pode atualizar em tempo real. Normalmente, ele requer atualizações seletivas ou de tela cheia em intervalos específicos para economizar energia e prolongar a vida útil. Este código implementa esse mecanismo de atualização sob demanda.
...
## Diagrama de Exibição da Interface
