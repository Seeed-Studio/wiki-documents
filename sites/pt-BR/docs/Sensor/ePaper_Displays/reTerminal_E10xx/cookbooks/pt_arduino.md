---
description: Cookbook Arduino para reTerminal E1001 / E1002 / E1003 / E1004 — controle o display ePaper a partir do Arduino usando as bibliotecas Seeed_GFX e GxEPD2, incluindo exemplos Hello World mais escala de cinza de 4 níveis no E1001 e de 16 níveis no E1003.
title: 'Arduino Cookbook: Display ePaper (reTerminal E Series)'
...
# Arduino Cookbook: Display ePaper (reTerminal E Series)
...
Esta página se concentra em **controlar a tela ePaper** a partir do Arduino. Se você quiser usar o LED onboard, buzzer, botões, sensor SHT4x, monitor de bateria ou slot para cartão microSD, vá para **[Arduino Cookbook: Periféricos Onboard](https://wiki.seeedstudio.com/pt-br/reterminal_e10xx_with_arduino_peripherals)**. Para RTC, modos de baixo consumo, microfone onboard e desenho com toque capacitivo, consulte **[Arduino Cookbook: RTC, Baixo Consumo, Áudio e Toque](https://wiki.seeedstudio.com/pt-br/reterminal_e10xx_with_arduino_peripherals_2)**.

O boilerplate compartilhado — configuração da IDE Arduino, pacote de placas ESP32, instalação de `Seeed_GFX`, geração de `driver.h` — também está em **[Trabalhar com Arduino](https://wiki.seeedstudio.com/pt-br/epaper_work_with_arduino)**. Dê uma olhada nisso primeiro se você é novo em Arduino em ePaper da Seeed.
:::

## Introdução

A reTerminal E Series é a linha HMI industrial da Seeed Studio, construída sobre o XIAO ESP32-S3 e com displays ePaper integrados. Este cookbook percorre tudo o que você precisa para renderizar texto, gráficos e imagens na tela:
...
- Dicas de solução de problemas para falhas de atualização do ePaper e de upload.
...
**Etapa 4.** Copie o código de configuração gerado e cole-o no arquivo `driver.h`. O código deve se parecer com isto:

```cpp
#define BOARD_SCREEN_COMBO 520 // reTerminal E1001 (UC8179)
```

**Etapa 5.** Envie o sketch para o seu reTerminal E1001. Você deverá ver o display exibindo vários gráficos, incluindo linhas, texto e formas, demonstrando os recursos básicos de desenho.
...
**Etapa 1.** Abra o sketch de exemplo colorido da biblioteca Seeed_GFX: **File > Examples > Seeed_GFX > ePaper > Colorful > HelloWorld**

**Etapa 2.** Ative o OPI PSRAM na IDE Arduino: **Tools > PSRAM > OPI PSRAM**

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/open_opi_psram.jpg" style={{width:800, height:'auto'}}/></div>

**Etapa 3.** Crie um novo arquivo chamado `driver.h` na mesma pasta do seu sketch, seguindo o mesmo processo de antes.
...
**Etapa 5.** Envie o sketch para o seu reTerminal E1002. O display exibirá gráficos coloridos demonstrando os recursos de cores completas do display ePaper.
...
<TabItem value="Programming reTerminal E1003" label="Programando reTerminal E1003">

#### Programando reTerminal E1003 (ePaper de 10,3 polegadas)

Siga o mesmo fluxo de trabalho usando a biblioteca Seeed_GFX para configurar e controlar o ePaper no reTerminal E1003.

**Etapa 1.** Abra um sketch de exemplo da biblioteca Seeed_GFX: **File > Examples > Seeed_GFX > ePaper > Basic > HelloWorld**

**Etapa 2.** Ative o OPI PSRAM na IDE Arduino: **Tools > PSRAM > OPI PSRAM**

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/open_opi_psram.jpg" style={{width:800, height:'auto'}}/></div>

**Etapa 3.** Crie um novo arquivo chamado `driver.h` na mesma pasta do seu sketch.
...
**Etapa 4.** Copie o código de configuração gerado e cole-o no arquivo `driver.h` para o E1003.

```cpp
#define BOARD_SCREEN_COMBO 522 // reTerminal E1003 (ED103TC2)
```

**Etapa 5.** Envie o sketch para o seu reTerminal E1003 para verificar primitivas de desenho, renderização de texto e comportamentos de atualização em tela cheia.
...
<TabItem value="Programming reTerminal E1004" label="Programando reTerminal E1004">

#### Programando reTerminal E1004 (ePaper colorido de 13,3 polegadas)

Use a biblioteca Seeed_GFX para configurar e controlar o display ePaper colorido E Ink® Spectra™ 6 no reTerminal E1004.

**Etapa 1.** Abra o sketch de exemplo colorido da biblioteca Seeed_GFX: **File > Examples > Seeed_GFX > ePaper > Basic > HelloWorld**
...
**Etapa 4.** Copie o código de configuração gerado e cole-o no arquivo `driver.h` para o E1004.

```cpp
#define BOARD_SCREEN_COMBO 523 // reTerminal E1004 (T133A01)
```

**Etapa 5.** Envie o sketch para o seu reTerminal E1004 para verificar a renderização de cores, primitivas de desenho, renderização de texto e comportamentos de atualização em tela cheia.
...
Os sketches Hello World acima são propositalmente mínimos para caberem em todos os modelos. Os painéis monocromáticos no E1001 e E1003 na verdade suportam tons de cinza em múltiplos níveis além do simples preto e branco — 4 níveis no E1001 e 16 níveis no E1003 — e o Seeed_GFX expõe ambos os modos por meio de `epaper.initGrayMode(...)` mais um conjunto de constantes de paleta `TFT_GRAY_*`. Os dois exemplos abaixo percorrem cada um deles.

<Tabs>
<TabItem value="E1001 Advanced" label="E1001 — 4 níveis de cinza" default>

#### 4 níveis de cinza no reTerminal E1001

O painel monocromático de 7,5" do reTerminal E1001 pode renderizar **4 níveis de cinza** em vez de apenas preto e branco. O Seeed_GFX expõe isso por meio de `epaper.initGrayMode(GRAY_LEVEL4)` e quatro constantes de paleta:

<div class="table-center">
	<table align="center">
		<tr>
			<th align="center">Constante</th>
			<th align="center">Tom renderizado</th>
		</tr>
		<tr>
		...
		<tr>
			...
			<td align="center">Cinza escuro</td>
		</tr>
		<tr>
			<td align="center"><code>TFT_GRAY_2</code></td>
			<td align="center">Cinza claro</td>
		</tr>
		<tr>
...
O exemplo abaixo primeiro pinta quatro faixas horizontais — uma por nível de cinza — para que você possa verificar visualmente a paleta e, em seguida, transfere um bitmap em escala de cinza de 800×480 para a tela. A biblioteca Seeed_GFX já inclui isso como um exemplo pronto para gravação, incluindo o `image.h` pré-convertido, então você não precisa gerar nenhum dado de bitmap por conta própria.

**Etapa 1.** Abra o sketch de exemplo da biblioteca Seeed_GFX: **File > Examples > Seeed_GFX > ePaper > Gray > GrayLevel4**. O sketch e seu `image.h` correspondente serão abertos no editor.

**Etapa 2.** Ative o OPI PSRAM na IDE Arduino: **Tools > PSRAM > OPI PSRAM**.

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/open_opi_psram.jpg" style={{width:800, height:'auto'}}/></div>

**Etapa 3.** Adicione um arquivo `driver.h` junto ao exemplo (mesmo fluxo de trabalho do Hello World) e selecione a combinação de placa–tela E1001:

```cpp
#define BOARD_SCREEN_COMBO 520 // reTerminal E1001 (UC8179)
```

**Etapa 4.** Envie o sketch. O display primeiro mostra quatro faixas em tons de cinza — preto na parte superior, depois cinza escuro, cinza claro e branco na parte inferior — e então limpa e renderiza o bitmap de `image.h`.
...
O array `L4_GRAY` em `image.h` é apenas um bitmap em escala de cinza de 800×480 pré-convertido para um array em C. Para substituir pela sua própria imagem, gere novamente o array a partir de uma fonte em escala de cinza de 800×480 usando qualquer conversor padrão de "imagem para array em C" e substitua `L4_GRAY` em `image.h`. O sketch em si não precisa ser alterado.
:::

:::tip
A atualização em 4 níveis de cinza é aproximadamente 4× mais lenta do que uma atualização preto e branco de 1 bit porque o controlador leva cada pixel por quatro tensões alvo em vez de duas. Use-a para conteúdo estático como fotos, ilustrações ou dashboards ricos em detalhes e mantenha o modo padrão de 1 bit para atualizações rápidas de interface.
:::

</TabItem>
<TabItem value="E1003 Advanced" label="E1003 — 16 níveis de cinza">

#### 16 níveis de cinza no reTerminal E1003

O painel de 10,3" do reTerminal E1003 eleva o nível para **16 níveis de cinza** em resolução de 1404×1872. O Seeed_GFX expõe isso por meio de `epaper.initGrayMode(GRAY_LEVEL16)` e dezesseis constantes de paleta de `TFT_GRAY_0` (preto) até `TFT_GRAY_15` (branco):
...
<div class="table-center">
	<table align="center">
		...
		<tr>
			...
			<td align="center">14 tons de cinza intermediários (do mais escuro → ao mais claro)</td>
		</tr>
		<tr>
...
**Passo 1.** Abra o sketch de exemplo da biblioteca Seeed_GFX: **File > Examples > Seeed_GFX > ePaper > Gray > GrayLevel16**. O sketch é aberto junto com o `driver.h` incluído (já definido como `BOARD_SCREEN_COMBO 522` para o painel E1003 ED103TC2) — você não deve precisar editá-lo.

**Passo 2.** Ative a OPI PSRAM na IDE do Arduino: **Tools > PSRAM > OPI PSRAM**.

**Passo 3.** Envie o sketch. O display exibirá 16 faixas horizontais em escala de cinza, de preto puro na parte superior a branco puro na parte inferior.
...
:::note
`Seeed_GxEPD2` é o fork personalizado da Seeed da biblioteca original `GxEPD2`, com drivers e otimizações dedicados para a série reTerminal E10xx. Recomendamos fortemente o uso deste fork em vez da biblioteca original para garantir total compatibilidade com o seu dispositivo reTerminal.
:::
...
O reTerminal E1001 possui um display ePaper preto e branco de 7,5" (800×480, painel GDEY075T7, controlador UC8179). O exemplo abaixo demonstra várias telas, incluindo uma de abertura, informações do sistema, tipografia, geometria, padrões e um layout de dashboard.
...
A figura a seguir mostra o efeito real de exibição do exemplo E1001:
...
#### Programando o reTerminal E1002 (Tela de 7,3" e 6 cores)

O reTerminal E1002 possui uma tela ePaper de 7,3" e 6 cores (800×480, painel GDEP073E01, controlador ED2208). Ele suporta Preto, Branco, Vermelho, Verde, Azul e Amarelo. O exemplo abaixo demonstra a renderização de cores em várias telas.

Após instalar a biblioteca `Seeed_GxEPD2`, você pode encontrar este exemplo na Arduino IDE em **File > Examples > Seeed_GxEPD2 > GxEPD2_reTerminal_E1002**, ou localizá-lo manualmente em `Seeed_GxEPD2/examples/GxEPD2_reTerminal_E1002/GxEPD2_reTerminal_E1002.ino`.
...
A figura a seguir mostra o efeito real de exibição do exemplo E1002:
...
#### Programando o reTerminal E1003 (Tela monocromática de 10,3")

O reTerminal E1003 possui uma tela ePaper monocromática de 10,3" (1872×1404, painel ED103TC2, controlador IT8951). Ele suporta escala de cinza de 16 níveis e requer OPI PSRAM. Este exemplo depende de um arquivo de driver personalizado `GxEPD2_ED103TC2_1872x1404.h` incluído na pasta do exemplo.

Após instalar a biblioteca `Seeed_GxEPD2`, você pode encontrar este exemplo na Arduino IDE em **File > Examples > Seeed_GxEPD2 > GxEPD2_reTerminal_E1003**, ou localizá-lo manualmente em `Seeed_GxEPD2/examples/GxEPD2_reTerminal_E1003/`.

::::note
O E1003 requer que o **OPI PSRAM** esteja habilitado na Arduino IDE: **Tools > PSRAM > OPI PSRAM**. O framebuffer de ~321 kB fica na PSRAM.
::::
...
A figura a seguir mostra o efeito real de exibição do exemplo E1003:
...
A figura a seguir mostra o efeito real de exibição do exemplo E1004:
...
O código-fonte completo do exemplo E1004 (incluindo todas as 6 telas de demonstração com implementações completas para Splash, Color Palette, Typography, Geometry, Patterns e Dashboard) está disponível no repositório em `Seeed_GxEPD2/examples/GxEPD2_reTerminal_E1004/`. O exemplo apresenta cartões de cores, barras de progresso multicoloridas, cartões de status e registros de atividades otimizados para o grande display de 13,3" e 6 cores.
...
- **reTerminal E1003 (10.3" Monocromático)**: O controlador IT8951 suporta nativamente **escala de cinza de 16 níveis** por meio de seu modo de forma de onda GC16. Um framebuffer de 4 bpp (~1,25 MB) é alocado em PSRAM, e cada pixel pode renderizar um de 16 níveis de cinza.
...
O controlador UC8179 do E1001 pode ser alternado de seu modo normal de 1 bit para um modo de escala de cinza de 4 níveis enviando tabelas LUT personalizadas (VCOM, LUTWW, LUTKW, LUTWK, LUTKK). Este exemplo cria um `Gray4Canvas` (2 bpp, 96 KB) e usa `Adafruit_GFX` para desenho, depois envia dois planos de bits para o controlador para renderização em escala de cinza.

Após instalar a biblioteca `Seeed_GxEPD2`, você pode encontrar este exemplo na Arduino IDE em **File > Examples > Seeed_GxEPD2 > GxEPD2_reTerminal_E1001_Gray4**, ou localizá-lo manualmente em `Seeed_GxEPD2/examples/GxEPD2_reTerminal_E1001_Gray4/GxEPD2_reTerminal_E1001_Gray4.ino`.
...
#### Programando o reTerminal E1003 — escala de cinza de 16 níveis

O controlador IT8951 do E1003 oferece suporte nativo a escala de cinza de 16 níveis por meio do seu modo de forma de onda GC16. Este exemplo aloca um framebuffer de 4bpp (~1,25 MB) na PSRAM e usa `Adafruit_GFX` para desenhar por meio de uma classe `Gray16Canvas`, depois envia o quadro como dados 8BPP para o IT8951 para atualização GC16.

::::note
Este exemplo requer que a **OPI PSRAM** esteja habilitada na IDE Arduino: **Tools > PSRAM > OPI PSRAM**. O framebuffer de ~1,25 MB fica na PSRAM.
::::

Após instalar a biblioteca `Seeed_GxEPD2`, você pode encontrar este exemplo na IDE Arduino em **File > Examples > Seeed_GxEPD2 > GxEPD2_reTerminal_E1003_Gray16**, ou localizá-lo manualmente em `Seeed_GxEPD2/examples/GxEPD2_reTerminal_E1003_Gray16/GxEPD2_reTerminal_E1003_Gray16.ino`.
