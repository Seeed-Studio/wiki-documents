---
description: Este artigo descreve como usar o TRMNL 7.5inch(OG) DIY Kit com Arduino.
...
O gabinete em formato L é muito semelhante.
...
O TRMNL DIY Kit possui três botões programáveis pelo usuário (D1, D2, D4) e um botão de reset. Vamos criar um exemplo simples para detectar pressionamentos de botões.
...
O TRMNL 7.5" (OG) DIY Kit inclui um circuito para monitorar a tensão da bateria conectado ao pino D0 (GPIO1). Um recurso importante deste projeto é que ele também inclui um pino de controle (GPIO6) para habilitar/desabilitar a alimentação do ADC, o que ajuda a economizar bateria quando não estiver medindo ativamente.
...
2. Definimos um `CALIBRATION_FACTOR` (0.968) para ajustar com precisão a exatidão da leitura de tensão.
...
3. **Função loop:**
   ...
   - Para reduzir atualizações desnecessárias e estender a vida útil do display, apagamos e redesenhamos os ponteiros de hora e minuto apenas uma vez por minuto (quando seconds = 0).
...
Este exemplo de relógio analógico oferece um ótimo ponto de partida para criar suas próprias aplicações gráficas no display de ePaper do TRMNL 7.5" (OG) DIY Kit.
