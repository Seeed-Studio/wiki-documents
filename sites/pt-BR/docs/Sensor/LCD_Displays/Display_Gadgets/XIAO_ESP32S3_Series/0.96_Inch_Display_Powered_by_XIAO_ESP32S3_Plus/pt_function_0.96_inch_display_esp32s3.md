...
:::note
Todos os demos desta página exigem **esp32 Boards by Espressif (3.3.11)** conforme descrito em [Getting Started](/pt-br/getting_started_0.96_inch_display_esp32s3), além da biblioteca **Seeed_GFX2** instalada manualmente conforme descrito abaixo.
:::
...
- O display IPS de 0,96'' **não possui controlador de toque, slot para cartão SD nem conector Grove** — ele só tem um pad de teste I2C de 4 pinos no lado traseiro — portanto, não são necessárias bibliotecas de toque, SD ou Grove.
:::
...
2. Navegue até `code_GFX2/Function/` e abra a pasta mostrada na linha **Code location** de cada demo. Por exemplo, o demo GraphicTest para esta placa está em `code_GFX2/Function/096_ESP32/xiao_esp32s3_096_graphictest/`.
...
O display IPS de 0,96'' possui uma **LSM6DS3** IMU de 6 eixos onboard (acelerômetro de 3 eixos + giroscópio de 3 eixos) conectada via I2C em D4/D5 no endereço **0x6A**. A linha de interrupção de movimento em **D14** oferece suporte a wake-up por hardware e detecção de gestos.

:::note
A IMU onboard é a **LSM6DS3** (endereço I2C `0x6A`). O demo Electronic Quicksand faz sondagem por um sensor compatível com QMI8658 como fallback defensivo. O demo Raise to Wake tem como alvo os registradores de wake-up da LSM6DS3 onboard.
:::
...
O IMU é lido via I2C (D4/D5). O sketch procura um IMU em ambos os endereços conhecidos — primeiro QMI8658, depois LSM6DS3 — e usa aquele que responder. Os valores brutos de aceleração são filtrados com um passa‑baixa e usados para derivar um vetor de gravidade. Quando você inclina a placa:
...
As partículas próximas à superfície fluem livremente (maior mobilidade); as partículas enterradas mais profundamente se compactam firmemente (menor mobilidade) — imitando como a areia real se comporta.
...
As partículas fluem em direção à borda mais baixa à medida que você inclina a placa. Quando o display fica na horizontal, o demo mantém a direção de gravidade anterior.
...
2. **Suspensão automática** — após 8 segundos sem atividade, o sketch desliga o backlight e exibe a mensagem "Sleep — Move to wake". Por padrão, o demo usa uma **suspensão apenas do display**: o ESP32‑S3 continua em execução (assim a porta serial USB CDC permanece conectada) e simplesmente desliga o painel. A interrupção de wake do IMU no D14 permanece armada, então a detecção de movimento continua ativa. (Defina `ENABLE_LIGHT_SLEEP` como `true` no sketch para usar o light sleep real do ESP32 com wake‑up por GPIO — observe que o USB CDC pode cair enquanto o chip dorme.)
3. **Wake‑up** — quando o usuário pega a placa, o IMU detecta movimento e coloca o D14 em nível ALTO. O sketch liga novamente o backlight e redesenha a interface — o LCD e o IMU não são reinicializados.
...
Este demo transforma o Display IPS de 0,96'' em um pequeno gravador de voz. Pressione USR1 para capturar um clipe de 5 segundos do microfone PDM onboard para a Flash onboard e depois pressione USR2 para reproduzi‑lo por meio de um amplificador I2S externo.
...
<div class="table-center">
  <table align="center">
    <tr><th>Pads I2S</th><th>Pino XIAO</th><th>MAX98357A</th></tr>
...
**Passo 5.** Selecione a **Porta** correta e clique em **Upload**.
...
Os botões usam os resistores de pull-up internos do XIAO. Uma leitura simples é assim:
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td><code>BAT_ADC</code></td><td><strong>D16</strong></td><td>Entrada analógica que lê a tensão dividida da bateria. Conectada internamente a um divisor de tensão de 316 kΩ / 160 kΩ. <strong>Não use este pino externamente.</strong></td></tr>
  </table>
</div>
...
O sketch inicializa o display com `Board_XIAO_0inch96_LCD<13, 12>` e `Config_Seeed_0inch96_LCD_ST7789` (80×160, BGR, rotação 2), então faz a leitura de **D16** doze vezes (a cada 700 µs) usando `analogReadMilliVolts()` com resolução de 12 bits e atenuação de 11 dB. Ele faz a média das amostras para obter a tensão bruta do divisor, multiplica pela razão do divisor para obter a tensão da bateria (`Calc = D16 × 2.975`) e desenha ambas como duas linhas amarelas centralizadas. A tela é atualizada apenas quando qualquer um dos valores muda de forma significativa (D16 ≥ 0,02 V ou Calc ≥ 0,05 V).
...
A tela mostra a tensão bruta do divisor D16 na linha superior e a tensão calculada da bateria (`Calc`) na linha inferior. Com uma bateria LiPo conectada, `Calc` se aproxima da tensão no terminal da bateria. Uma leitura também pode aparecer apenas com alimentação via USB, portanto `Calc` por si só não pode confirmar se uma bateria está conectada.
...
- **[Demo]** [Código de demonstração da placa XIAO Display](https://github.com/Seeed-Projects/Display-Gadgets) — todas as demos de Function estão no diretório `code_GFX2/Function/096_ESP32/`
