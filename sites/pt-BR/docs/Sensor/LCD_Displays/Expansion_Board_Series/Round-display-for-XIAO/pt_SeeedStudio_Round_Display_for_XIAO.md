...
# Introdução ao Seeed Studio Round Display para XIAO
...
<table align="center">
 ...
 <tr>
     ...
     <td>USB Type-C: 5V @35 mA <br></br> Carregamento da bateria: 3.7V @37mA</td>
 </tr>
 <tr>
...
Antes de começarmos, podemos consultar as imagens a seguir para entender o design dos pinos do Round Display e facilitar nossa compreensão da função do Round Display.
...
Se você quiser aproveitar totalmente os recursos do Round Display e ter uma ótima experiência, recomendamos fortemente que você adquira nossa série XIAO como placa-mãe para o Round Display.
...
A fileira de pinos na parte de trás do Round Display é projetada para a série XIAO. Se você já tiver um XIAO em mãos, não precisa preparar cabos adicionais, apenas alinhe os pinos do XIAO e conecte-os diretamente ao Round Display.
...
:::caution
Observe que, ao conectar o XIAO, **o conector Type-C do XIAO deve estar voltado para fora do Round Display**. Se você inverter a polaridade acidentalmente, não se preocupe muito, o Round Display possui um circuito de proteção de energia que não será facilmente danificado, mas não recomendamos que você permaneça na conexão invertida por muito tempo.
:::
...
3. Instale a biblioteca adicionando o arquivo ZIP na Arduino IDE. Vá em **Sketch > Include Library > Add .ZIP Library** e selecione o arquivo ZIP baixado.
...
Como podemos perceber pelo tutorial acima, o Round Display utiliza principalmente as bibliotecas **LVGL**, **TFT_eSPI** e **Arduino GFX**. Por questão de espaço, apresentaremos o uso das bibliotecas **LVGL** e **TFT_eSPI** separadamente com o exemplo de desenhar um mostrador.

- Você pode aprender sobre a interface e o uso da biblioteca **TFT_eSPI** clicando **[aqui](https://wiki.seeedstudio.com/pt-br/using_lvgl_and_tft_on_round_display#interfaces-comuns-para-tft-library)**.

- Você pode aprender sobre a interface e o uso da biblioteca **LVGL** clicando **[aqui](https://wiki.seeedstudio.com/pt-br/using_lvgl_and_tft_on_round_display#interfaces-comuns-para-lvgl-library)**.
...
## Acenda o seu Round Display

### Demo 1: Arduino Life - Como usar a biblioteca Seeed_GFX no round display
...
Quando o hardware e o software estiverem prontos, começamos a fazer o upload do nosso primeiro programa de exemplo. Este programa pode ser usado para verificar se o relógio RTC do Round Display está funcionando corretamente. Como este demo requer uma pequena quantidade de memória, é adequado para todos os modelos XIAO.

:::tip
Se você quiser modificar este demo para implementar telas mais complexas, preste atenção ao tamanho da memória do XIAO que você está usando. O tamanho específico pode ser encontrado na página de introdução desse XIAO.
:::

Você pode encontrar este programa de exemplo na Arduino IDE em **File -> Examples -> Seeed Arduino Round display -> TFT_Clock**.
...
Como o demo HardwareTest é um pouco complexo e ocupa uma quantidade maior de memória, alguns modelos XIAO podem falhar em executar com sucesso devido à memória insuficiente. Criamos uma tabela de teste para sua referência, usando a biblioteca TFT e a biblioteca Arduino GFX.
...
Você pode encontrar este programa de exemplo na Arduino IDE em **File -> Examples -> Seeed Arduino Round display -> HardwareTest**.
...
R: Isso não cria conflito. Os diferentes slots de cartão SD são controlados via chip select; se você quiser usar o slot de cartão microSD no Sense, o pino de chip select deve ser **21**; se quiser usar o slot de cartão microSD no Round Display, o pino de chip select deve ser **D2**.

Temos [exemplos](https://wiki.seeedstudio.com/pt-br/xiao_esp32s3_camera_usage/#projeto-i-making-a-handheld-camera) de uso de ambos os hardwares e cartões microSD no tutorial da câmera S3 Sense.

### Q3: Por que meu XIAO RP2040 apresenta um erro C++ muito estranho ao usar o código de HardwareTest com o Round Display?
...
- **[PDF]** [Folha de dados do CI de carregamento](https://files.seeedstudio.com/wiki/round_display_for_xiao/charge-IC-datasheet.pdf)
