...
title: Usando LVGL e TFT para toda a linha XIAO
keywords:
  - XIAO
  - Round Dislay
  - lvgl
...
# Usando LVGL e TFT no Seeed Studio Round Display para toda a linha XIAO
...
Obrigado por adquirir os produtos Seeed Studio Round Display. Nesta parte do tutorial, vamos focar em como usar a biblioteca `Seeed_GFX` e a biblioteca `LVGL` para desenhar vários mostradores ricos e interessantes no Round Display, e apresentar o uso de algumas funções comuns dessas duas bibliotecas úteis porém complexas, do simples ao avançado. Por meio do conteúdo deste tutorial, espero que você possa usar este produto para desenhar o mostrador ideal. Tenha uma ótima experiência de aprendizado!
...
Ao instalar o XIAO com o Round Display, deixe o conector Type-C do XIAO voltado para fora do Round Display e, em seguida, conecte cada pino aos dois headers de 7 pinos.
...
[Seeed_GFX](https://github.com/Seeed-Studio/Seeed_GFX) (baseada em modificações na biblioteca TFT_eSPI, com amplo suporte para os produtos de display da Seeed) é uma biblioteca de gráficos e fontes rica em recursos, compatível com Arduino IDE para processadores de 32 bits. A biblioteca TFT fornecida pelo XIAO Round Display é baseada na biblioteca após a compatibilidade entre XIAO e XIAO Round Display, o que permite o uso em toda a linha XIAO.
...
A biblioteca TFT também nos fornece métodos para desenhar formas complexas, como retângulos arredondados, arcos arredondados, formas bem suaves, etc.
...
- Converta o arquivo de imagem para um formato de array em C/C++ que o Arduino possa reconhecer. Você pode usar ferramentas online como **Image2CPP** para converter uma imagem bitmap em um formato de array.
...
:::note
No código de exemplo acima, quando o programa sai da função `loop()`, `spr.deleteSprite();` é chamado para excluir o objeto TFT_eSprite e liberar o espaço de memória. Dessa forma, ao criar um objeto TFT_eSprite no próximo loop, o espaço de memória liberado anteriormente pode ser usado para evitar o desperdício de recursos de memória.
:::

Para mais informações sobre as funções da biblioteca TFT e seu uso, recomendamos a leitura dos arquivos **TFT_eSPI.h** e **TFT_eSPI.cpp** na biblioteca.
...
- O caminho padrão para este arquivo em sistemas **Windows** é:

`C:\Users\{UserName}\Documents\Arduino\libraries\Seeed_GFX\User_Setups\Setup66_Seeed_XIAO_Round.h`

- O caminho padrão para este arquivo em sistemas **MacOS** é:

`\Users\{UserName}\Documents\Arduino\libraries\Seeed_GFX\User_Setups\Setup66_Seeed_XIAO_Round.h`

Por favor, ative ou desative algumas funções desnecessárias de acordo com o uso real.
Por exemplo, se você quiser usar fontes personalizadas, então você deve remover o comentário de `#define SMOOTH_FONT`, caso contrário, você provavelmente terá erros durante a execução.

Claro, os arquivos de cabeçalho das suas fontes personalizadas devem ser salvos no mesmo diretório que os arquivos ino, o que é uma etapa necessária para contornar erros.

### Programa de exemplo de mostrador baseado em TFT

Escrevemos um programa de mostrador para o Round Display que pode ser usado diretamente. Devido à limitação de espaço de memória de alguns modelos XIAO, este programa apenas executa a função básica de movimento do ponteiro e não é projetado para outras funções. Os usuários podem usar este programa para aprender o uso da biblioteca TFT e a função de movimento do ponteiro, e concluir o desenvolvimento de mostradores mais complexos de acordo com a situação real.
...
A biblioteca LVGL é uma biblioteca gráfica embarcada de uso geral que oferece um conjunto rico de controles de interface gráfica do usuário, como botões, rótulos, listas, etc., que podem ser usados para construir várias interfaces de usuário. Diferente da biblioteca TFT, a biblioteca LVGL fornece uma interface gráfica orientada a objetos e abstrata, que é mais fácil de usar e manter, mas pode trazer alguns compromissos em desempenho e confiabilidade.

A biblioteca LVGL é uma ferramenta muito útil ao construir interfaces de usuário complexas que reduzem a carga de trabalho de escrita e manutenção de código. A biblioteca TFT, por outro lado, é mais adequada para algumas aplicações que exigem gráficos de alto desempenho, como processamento de imagem em tempo real, renderização de vídeo, etc.

### Interfaces comuns para a biblioteca LVGL

A API da biblioteca LVGL é muito rica e complexa, e esperamos que todos que usam LVGL reservem um tempo para ler a documentação oficial de introdução do LVGL.
...
### Desenhando interfaces de UI complexas com o SquareLine Studio
...
Em seguida, daremos uma breve introdução ao uso do software na tela redonda, métodos de configuração, para ajudá-lo a começar rapidamente a usar o software para projetar algumas interfaces.

:::caution
Recomendamos que você use a versão **v1.5.1** do SquareLine Studio. Esta também é a versão mais recente do software no momento da escrita deste Wiki.
...
#### Passo 1. Baixar o SquareLine Studio

Você pode ir ao site oficial do SquareLine Studio clicando [aqui](https://squareline.io/), depois clique em **TRY IT FOR FREE** para baixar o software para o seu computador.
...
Se o seu computador estiver usando este software pela primeira vez, então você terá um teste gratuito de 30 dias, caso contrário, a versão gratuita também é capaz de desenhar até 5 páginas, usando 50 widgets.

:::caution
Se você é um usuário de primeira viagem, por favor não se registre para entrar em uma conta que não tenha saldo, isso pode custar todo o seu teste gratuito de 30 dias!
:::

#### Passo 2. Configurar as informações da interface da tela

Em seguida, podemos abrir o software e começar criando uma página de exibição em branco.

Como estamos usando programação em Arduino, então o que criamos também precisa ser um arquivo Arduino.

Nossa tela circular tem uma resolução de tela de **240*240** e suporta apenas profundidade de cor de **16bit**. Além disso, o nome e o tema do projeto precisam ser definidos por você, aqui usarei o estilo escuro como tema.
...
E se você for rápido demais, como eu, e esquecer de definir o formato do mostrador e criar o projeto? Sem problemas, no canto superior esquerdo da interface principal, você também pode encontrar a aba Project Settings para fazer alterações nas configurações que acabou de fazer.
...
:::caution
Observe que, por favor, conclua a configuração do seu projeto antes de começar oficialmente a desenhar e certifique-se de que ela corresponda às especificações da nossa tela, caso contrário tudo o que você desenhar pode não ser exibido corretamente na tela.

Nem tudo pode ser modificado depois que o projeto foi criado, como o nome do projeto. Por favor, não use nenhum idioma diferente do inglês ou caracteres especiais no nome do projeto, e por favor não use o sinal **"-"**, substitua o sinal **"-"** por **"_"**. Caso contrário, o programa exportado pode apresentar erros durante a compilação!
:::

#### Passo 3. Entendendo o layout funcional do software
...
- **Painel de Hierarquia e Animação**: Esta área permite configurar diferentes páginas de mostrador, camadas de exibição e animações.

- **Widgets**: Aqui você pode escolher quais componentes usar para adicionar na página de exibição. Esses componentes são integrados ao software e podem ser usados diretamente. Componentes que não estão disponíveis aqui precisarão ser adicionados posteriormente no seu próprio software de programação.
...
- **Assets & Console**: Assets mostra os recortes de imagem que você adicionou, e esses recortes de imagem adicionados podem ser usados nos componentes que suportam inserção de imagens. Console mostrará algumas mensagens de erro (se houver) que ocorreram durante a configuração.

- **Área de Configuração**: O principal objetivo aqui é configurar as propriedades do componente.

Primeiro teremos um conhecimento geral da interface do software e, em seguida, vamos guiá-lo pelo entendimento prático do uso de cada parte.

#### Passo 4. Use o software para realizar suas ideias
...
- Capa do álbum da minha música favorita como plano de fundo.
...
Selecione **Panel** nos widgets, você pode clicar nele e ele será automaticamente colocado no centro do mostrador, ou você pode arrastá-lo para a posição do mostrador onde deseja colocá-lo.

Colocaremos a imagem exibida no Panel em vez de diretamente no fundo do mostrador porque eu não gosto que todo o mostrador seja essa imagem, e o Panel pode ser redimensionado livremente.

Neste ponto, você pode ver que já há uma série de propriedades na Área de Configuração que podemos definir. As configurações de cada componente são geralmente semelhantes, com uma ou duas opções que podem ser um pouco diferentes.
...
> - **Click focusable** - Adiciona estado de foco ao objeto quando clicado
> - **Checkable** - Alterna o estado marcado quando o objeto é clicado
> - **Snappable** - Se o snap de rolagem estiver habilitado no pai, ele pode encaixar neste objeto
...
> - **Ignore layout** - Permite posicionar o objeto pelos layouts
> - **Floating** - Não rola o objeto quando o pai rola e ignora o layout

> **Scroll Flags**
>
> - **Scrollable** - Torna o objeto rolável
> - **Scroll elastic** - Permite rolar internamente, mas com velocidade mais lenta
> - **Scroll momentum** - Faz o objeto rolar mais quando é "arremessado"
> - **Scroll one** - Permite rolar apenas um filho encaixável
> - **Scroll chain** - Permite propagar a rolagem para um pai
> - **Scroll on focus** - Rola automaticamente o objeto para torná-lo visível quando estiver em foco
...
> - **Disable** - Estado desabilitado
> - **Focusable** - Focado via teclado ou codificador ou clicado via touchpad/mouse
> - **Pressed** - Sendo pressionado

> **Configurações de estilo**: Estilos podem ser usados para adicionar efeitos aos widgets ou às suas partes. Você pode adicionar uma cor de fundo personalizada, borda, sombra, etc. Em Configurações de Estilo, você pode adicionar ou modificar os valores desses parâmetros.
>
...
>
> - **Arc**: O estilo Arc pode ser usado naqueles widgets que possuem o componente Arc.
...
>   - **Bg radius** - O raio que você usa para deixar os cantos do fundo arredondados
>   - **Gradient direction** - A direção do gradiente. Pode ser horizontal ou vertical.
>   - **Clip corner** - Ative para recortar o conteúdo excedente no canto arredondado.
...
>   - **Recolor** - Com a função Recolor, você pode usar uma cor sobre a imagem de fundo. Defina a profundidade da cor alterando o parâmetro alfa.
>   - **Bg image tiled** - Se ativado, a imagem de fundo será ladrilhada
> - **Blend**: Usando o estilo Blend, você pode misturar as cores de pixel da parte atual do widget com as cores do objeto subsequente.
>   - **Blend mode** - Escolha entre quatro opções.
>     - **Normal** - Estado padrão
>     - **Additive** - Soma de pixels
>     - **Subtractive** - Subtração de pixels
>     - **Multiply** - Multiplicação de pixels
...
> - **Line**: O estilo Line pode ser usado naqueles widgets que possuem o componente Line.
...
>   - **Outline pad** - Distância em pixels a partir da lateral do widget
...
>     - **Clicked** - Um objeto foi pressionado por um curto período de tempo e depois liberado. Não é chamado se houver rolagem
...
>     - **Screen loaded** - Uma tela foi carregada, chamado quando todas as animações são concluídas
>     - **Screen unloaded** - Uma tela foi descarregada, chamado quando todas as animações são concluídas
>     - **Screen load start** - O carregamento de uma tela foi iniciado, disparado quando o atraso de mudança de tela expira
>     - **Screen unload start** - O descarregamento de uma tela foi iniciado, disparado imediatamente quando lv_scr_load/lv_scr_load_anim é chamado
>       - **Checked** - Um widget marcado
>       - **Unchecked** - Um widget desmarcado
>       - **Gesture** - Direção de deslizamento do toque do dedo
> - **Add Event**
...
>     - **Play Animation**: Você pode reproduzir as animações criadas no Painel de Animação.
>       - **Animation** - A animação selecionada
>       - **Target** - O widget de destino no qual você gostaria de usar a animação
...
:::note
Para ocupar o mínimo possível de memória na placa-mãe, insira a imagem o máximo possível na resolução do mostrador para comprimir, não use uma imagem muito grande!
:::
...
Vale a pena notar aqui que todos os componentes têm linhas de moldura por padrão, então, para não afetar a estética, precisamos remover as linhas de moldura. A maneira de fazer isso é definir a transparência da cor da Border para 0.

Portanto, **se você quiser remover qualquer cor ou qualquer segmento de linha, você pode fazê-lo definindo a transparência para 0**.
...
Em seguida, adicionamos texto no meio (usando widgets **Label**), que mostra o artista e o nome da música. No estilo, podemos alterar o tamanho da fonte, a cor e outros conteúdos.
...
Abaixo do texto, adicionamos alguns componentes de reprodução (usando widgets **Imgbutton**), como o botão de reproduzir/pausar e o botão de alternar faixa para cima/para baixo.
...
Imgbutton é um tipo especial de botão; a maior diferença entre ele e o Button normal é que o Imgbutton pode ser configurado com três estados de estilo: antes de pressionar o botão, ao pressionar e após soltar o botão, respectivamente. Esse processo é muito semelhante ao cenário em que pressionamos para alternar o estado de reprodução. Se você achar que o botão de avançar/retroceder não precisa de uma função tão complexa, também pode usar o Button diretamente.

Então agora definimos as imagens dos botões Press e Release para o estilo de reprodução, e somente no estado Disable é o estilo de pausa.
...
Em seguida, adicionamos um evento, cuja função é, quando o usuário pressionar o botão, o estado mudar para Disable, de modo que o efeito de alternância da imagem seja alcançado.
...
Se você quiser verificar o efeito, pode clicar no botão de play no canto superior direito da área de trabalho e, em seguida, clicar no botão de play para ver o efeito da alternância.
...
Depois adicionamos o último componente, que é a barra de volume e a barra de progresso de reprodução. Faremos isso usando Arc.

Para Arc, as principais coisas que precisamos ajustar são a cor e o tamanho dos anéis.
...
- INDICATOR: Refere-se à configuração de estilo do arco da área de indicação inicial. A configuração de **Arc** em INDICATOR é geralmente usada.
...
- KNOB: Refere-se à configuração deste círculo na imagem. Se você não precisar deste círculo, pode definir sua transparência para 0. O tamanho deste círculo precisa ser definido dentro do Arc em INDICATOR.
...
Este é o efeito que quero alcançar.
...
Se você verificou que é clicável, então pode clicar no botão Run e arrastar o arco, assim você pode obter o efeito de alterar a barra de volume.
...
Com a interface de música quase projetada, podemos muito bem adicionar em seguida uma nova interface principal.
...
Em seguida, projete um evento que alterne a tela principal para a tela de música. Por exemplo, eu o projeto aqui para alternar para a interface de reprodução de música deslizando o dedo para a direita sob a interface principal.

Como a interface principal é a personagem principal, este evento deve ser adicionado à interface principal em Screen.
...
Se você quiser ter um efeito de animação que mude lentamente após um deslize, então Speed pode ser mantida em 500; se quiser alternar imediatamente, então Speed deve ser definida como 0.
...
O ajuste da posição do ponteiro requer paciência, pois precisamos garantir que o ponteiro esteja girando em torno de um ponto fixo na imagem.

Transform é definido para o posicionamento da imagem. O Pivot na aba Image abaixo define as coordenadas do ponto de rotação. A maneira geral de definir isso é primeiro ajustar o Transform para garantir que o ponto fixo do ponteiro esteja no centro do mostrador e, em seguida, ajustar um pouco as coordenadas do Pivot. Quando todos os ângulos puderem fazer com que o ponto fixo do ponteiro não se mova, então esses parâmetros são os mais adequados.
...
Depois que todas as posições dos ponteiros estiverem determinadas, você pode começar a adicionar novos efeitos de animação. As configurações de efeito de animação para diferentes ponteiros podem ser encontradas na figura a seguir.
...
Por fim, apenas definimos a animação do movimento do ponteiro para ser reproduzida enquanto a tela principal é carregada.
...
:::tip
Os tutoriais acima basicamente cobriram 80% dos cenários de uso do software, e muitos dos outros componentes são muito semelhantes. Por fim, aqui estão algumas dicas e sugestões para você ao usar o software.

1. Você precisa colocar todos os componentes com funcionalidade de toque no topo, caso contrário isso irá interferir na implementação da funcionalidade de toque do componente.

    Por padrão, o último componente colocado pelo software fica no topo. Isso significa que, na aba Hierarchy, os componentes que estão dispostos mais para baixo tendem a estar no topo. Na interface de música que você acabou de projetar, a camada superior é o Arc para ajustar o volume do som, o que causa um problema porque o Arc é um retângulo que ocupa todo o mostrador e afetará o toque do botão de play, então normalmente você precisa ajustar a camada do botão de play para o topo da camada do Arc para que isso não afete o toque.
...
2. Desligue o máximo possível de recursos desnecessários para economizar mais memória.

    Cada componente terá alguns Flags marcados por padrão, mas nem todos são necessários. Embora os padrões não causem problema, desligar os Flags indesejados fará com que sua interface de mostrador rode com mais fluidez.

    Como o álbum de fundo na interface de música, não há necessidade de clique nem de animação, e é bom desligar a opção em Flags. Mas ao desativar também é preciso considerar a situação real, por exemplo, na cena de deslizar para alternar a página do mostrador, desligar a função de alguns Flags fará com que o deslizamento seja desativado, então ainda é necessário, de acordo com o efeito em execução, decidir com critério o que fechar.
...
3. Certifique-se de dar um nome exclusivo a todos os seus componentes e animações, eventos, etc.
    ...
    O software só pode economizar o seu tempo de desenhar alguns padrões e animações, mas efeitos mais complexos podem exigir que você os programe posteriormente para serem alcançados. Então é importante poder encontrar rapidamente onde seus componentes estão localizados no código. Nomear componentes, eventos e animações é fundamental!

:::

#### Passo 5. Exportar e aplicar programas

Depois que sua interface de mostrador estiver desenhada, você pode considerar exportar a UI para um programa e enviá-lo para o XIAO via Arduino. Clique no canto superior esquerdo do software e clique em **Export** -> **Create Template Project**.
...
Em seguida, selecione o caminho para salvar e o código será exportado automaticamente. O modelo de projeto exportado terá os seguintes arquivos.
...
O que precisamos fazer, então, é primeiro colocar as bibliotecas e arquivos de configuração necessários na pasta libraries do Arduino. Em seguida, modificar o arquivo .ino para garantir que a funcionalidade funcione.

Copie a pasta **ui** e o arquivo **lv_conf.h** do diretório da pasta libraries exportado pelo SquareLine Studio para o diretório raiz da sua biblioteca do Arduino.
...
Então, podemos abrir diretamente o arquivo .ino dentro da pasta ui. Em seguida, precisamos fazer alterações nos seguintes arquivos para garantir que o programa seja compilado sem problemas.
...
<table align="center">
 ...
 <tr>
  <th>Definindo a biblioteca TFT a ser usada e importando a biblioteca da tela redonda</th>
...
Se você encontrar uma mensagem de erro informando que alguns componentes não estão definidos após a compilação, então muito provavelmente você não substituiu o arquivo **lv_conf.h** no diretório raiz da biblioteca Arduino pelo arquivo **lv_conf.h** exportado pelo SquareLine Studio.

Para economizar memória na placa-mãe, o arquivo lv_conf.h fornecido por padrão desativa alguns dos recursos do lvgl. Mas se você usar essas funções no desenho do seu mostrador, será necessário ativá-las manualmente.
...
Então tudo o que precisamos fazer é alterar o 0 após essa fonte no arquivo lv_conf.h para 1, o que significa que a fonte está habilitada.
...
Se você encontrar um erro semelhante, pode verificar novamente se o recurso de personalização está habilitado.

### Programa de mostrador baseado em LVGL

Criamos dois estilos de mostrador para a tela circular como referência para os usuários. Devido à lógica complexa da interface, isso exigirá uma certa quantidade de desempenho e memória na placa-mãe XIAO. Se o seu XIAO estiver compilando o seguinte programa de mostrador com erro de memória insuficiente, então talvez você precise atualizar o seu XIAO ou reduzir a funcionalidade dos mostradores.
