---
description: ChatGPT & DALL·E com Indicator
...
url: https://wiki.seeedstudio.com/pt-br/SenseCAP_Indicator_Application_ChatGPT/
---

# SenseCAP Indicator - ChatGPT - DALL·E Desenvolvimento de Aplicativo
...
Este guia fornecerá informações sobre como organizar a demonstração da OpenAI para adição, exclusão e modificação rápidas de programas de acordo com o BSP (Board Support Package) fornecido.

## Pré-requisitos

- Um [SenseCAP Indicator](https://www.seeedstudio.com/SenseCAP-Indicator-D1-p-5643.html)
- A ferramenta [ESP-IDF](https://github.com/espressif/esp-idf) instalada no seu computador

:::tip
Se você quiser aprender como alterar a interface do usuário (UI), pode consultar o guia: [Como criar sua própria UI](/pt-br/SenseCAP_Indicator_How_to_Create_your_own_UI)

Se você ainda não instalou a ferramenta IDF, pode seguir as instruções no guia: [How_To_Flash_The_Default_Firmware](/pt-br/SenseCAP_Indicator_How_To_Flash_The_Default_Firmware)
:::
...
O código principal relacionado ao fluxograma de inicialização da OpenAI é mostrado abaixo:
...
O aplicativo SenseCAP Indicator x ChatGPT x DALL·E é desenvolvido com base na arquitetura MVC (Model-View-Controller). O fluxo de trabalho do projeto indica sua dependência da arquitetura MVC.

Nessa arquitetura, os componentes são estruturados da seguinte forma:

- View: A View lida com vários sinais acionados por eventos e utiliza a LVGL (Light and Versatile Graphics Library) para dirigir o display.

:::note
Para criar rapidamente a interface do usuário (UI), você pode usar o [SquareLine Studio](https://squareline.io/), que também é utilizado em nosso projeto. Recomendamos usar o SquareLine Studio versão 1.3.0 para garantir uma experiência fluida e contínua ao seguir este guia..
:::

- Model: O Model contém o arquivo `indicator_openai.c`, que inclui a função `indicator_openai_init()`. Quando executada na entrada do Model, essa função envia solicitações para a OpenAI, recebe respostas e as analisa para exibição na tela por meio da View.

Aqui estão as principais funções e o fluxo de trabalho do Model (depois que [a Chave de API](/pt-br/Sensor/SenseCAP/SenseCAP_Indicator/Set_An_API_Key) for salva):

#### **Fluxograma do ChatGPT**

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/SenseCAP/SenseCAP_Indicator/model_openai.png"/></div>

#### **Fluxograma do DALL·E**
...
Para utilizar o serviço OpenAI, precisamos implementar funções que possam enviar solicitações para a OpenAI, receber respostas e analisar a resposta JSON. O trecho de código a seguir ilustra a estrutura de código necessária:
...
A função `chat_request(...)` em `indicator_openai.c` é responsável por enviar solicitações para a API da OpenAI, receber respostas e analisar a resposta JSON. Ela gera uma requisição HTTP encapsulando os dados fornecidos pelo usuário e se comunica com o servidor usando `mbedtls_send_then_recv(...)`.

Para adicionar um prompt ao seu aplicativo, você pode modificar a variável `data_buf` na função `chat_request(...)` da seguinte forma:
...
Nessa função, `mbedtls_send_then_recv` é chamada para realizar o método de requisição e obtenção.

</TabItem>
<TabItem value="DALL·E" label="Código DALL·E">
...
> Princípio: Quando uma solicitação é bem-sucedida, ela retorna um endereço de URL. Ao baixar e decodificar a imagem a partir desse link de URL, ela pode ser exibida.

Da mesma forma que em `ChatGPT Code`, na solicitação inicial utilizaremos um prompt para obter a URL da imagem. Depois de adquirir a URL, tentaremos baixar a imagem para o buffer local usando a URL obtida.
...
<summary>Código ChatGPT & DALL·E</summary>

Para o código detalhado e mais recente, consulte [SenseCAP Indicator OpenAI](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32/tree/main/examples/indicator_openai).
...
1. [SenseCAP Indicator X ChatGPT](/pt-br/SenseCAP_Indicator_ChatGPT)
2. [SenseCAP Indicator X DALL·E](/pt-br/SenseCAP_Indicator_DALL-E)
3. **Demo SDK**: O Demo SDK para o SenseCAP Indicator está disponível no [GitHub](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32).
4. **Guia do Usuário do SenseCAP Indicator**: O Guia do Usuário fornece informações detalhadas sobre o software e o hardware da Placa SenseCAP Indicator. Você pode lê-lo [aqui](/pt-br/Sensor/SenseCAP/SenseCAP_Indicator/Get_started_with_SenseCAP_Indicator).
5. **Guia de Chat completions da OpenAI**: Se você é novo na Chat API, este guia irá ajudá-lo a começar. Você pode encontrá-lo [aqui](https://platform.openai.com/docs/guides/chat/chat-completions-beta).
6. **Arquivo `indicator_openai.c`**: Este arquivo contém as principais funções para a integração com `ChatGPT` e `DALL·E`. Você pode visualizá-lo [aqui](https://raw.githubusercontent.com/Seeed-Solution/SenseCAP_Indicator_ESP32/main/examples/indicator_openai/main/model/indicator_openai.c).
7. **Guia de Primeiros Passos para ESP-IDF**: Este guia fornece todas as etapas para configurar e usar o ESP-IDF para compilar projetos. Você pode acessá-lo [aqui](https://docs.espressif.com/projects/esp-idf/en/latest/get-started/index.html).

## Suporte Técnico

**Precisa de ajuda com o seu SenseCAP Indicator? Estamos aqui para ajudá-lo!**

Se você encontrar qualquer problema ou tiver alguma dúvida ao seguir este tutorial, sinta-se à vontade para entrar em contato com nosso suporte técnico. Estamos sempre aqui para ajudar!

Visite nosso [Canal Oficial da Seeed no Discord](https://discord.gg/kpY74apCWj) para fazer suas perguntas ou o [GitHub discussions](https://github.com/Seeed-Solution/SenseCAP_Indicator_ESP32/discussions) para compartilhar tudo o que você quiser!
