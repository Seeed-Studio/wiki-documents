...
url: https://wiki.seeedstudio.com/pt-br/ReSpeaker_Mic_Array_v2.0/
---
...
| ReSpeaker Mic Array v2.0 | XVSM-2000 está EOL, muda o MCU para XVF-3000 e reduz os Mics de 7 para 4. | 25 Jan, 2018       |
...
Integra algoritmos DSP avançados que incluem Acoustic Echo Cancellation (AEC), beamforming, desreverberação, supressão de ruído e controle de ganho.

- **<font face="" size="3" font color="ff0000">②</font> Microfone Digital:**
O MP34DT01-M é um microfone digital MEMS omnidirecional, ultracompacto e de baixo consumo, construído com um elemento de detecção capacitiva e uma interface CI.

- **<font face="" size="3" font color="ff0000">③</font> RGB LED:**
LED RGB de três cores.

- **<font face="" size="3" font color="ff0000">④</font> Porta USB:**
Fornece energia e controla o mic array.

- **<font face="" size="3" font color="ff0000">⑤</font> Conector de Fone de Ouvido 3,5mm:**
...
O WM8960 é um codec estéreo de baixo consumo com drivers de alto-falante Classe D para fornecer 1 W por canal em cargas de 8 W.
...
Para Android, nós o testamos com [emteria.OS](https://help.emteria.com/kb/emteria-os-installation)(Android 7.1) no Raspberry. Conectamos o mic array v2.0 à porta USB do Raspberry Pi e selecionamos o ReSpeaker mic array v2.0 como dispositivo de áudio. Aqui está a tela de gravação de áudio.
...
| 6_channels_firmware.bin | 6  |Canal 0: áudio processado para ASR <br /> Canal 1: dados brutos do mic1 <br />Canal 2: dados brutos do mic2 <br />Canal 3: dados brutos do mic3 <br />Canal 4: dados brutos do mic4 <br />Canal 5: reprodução mesclada|
...
- **Windows:** Gravação e reprodução de áudio funcionam bem por padrão. O driver libusb-win32 só é necessário para controlar LEDs e parâmetros DSP no Windows. Usamos [uma ferramenta prática - Zadig](http://zadig.akeo.ie/) para instalar o driver libusb-win32 tanto para `SEEED DFU` quanto para `SEEED Control` (o ReSpeaker Mic Array tem 2 dispositivos no Gerenciador de Dispositivos do Windows).
...
| 6       | [r, g, b, 0] * 12              | pixel_ring.custimize()         | modo personalizado, define cada LED para sua própria cor                                                         |
...
- Etapa 2. Crie um [led_control.py](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/led_control.py) com o código abaixo e execute 'python led_control.py'
...
- Etapa 2. Crie um [DOA.py](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/DOA.py) com o código abaixo dentro da pasta usb_4_mic_array e execute 'python DOA.py'
...
- Etapa 2. Crie um [VAD.py](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/VAD.py) com o código abaixo dentro da pasta usb_4_mic_array e execute 'python VAD.py'
...
:::note
Para o limite de VAD, também podemos usar o GAMMAVAD_SR para configurar. Consulte [Tuning](https://wiki.seeedstudio.com/pt-br/ReSpeaker_Mic_Array_v2.0/#Ajuste-Fino) para mais detalhes.
:::
...
- Etapa 2, copie o código abaixo e cole em [get_index.py](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/get_index.py).
...
- Etapa 5, altere `RESPEAKER_INDEX = 2` para o número de índice. Execute o script python [record.py](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/record.py) para gravar um discurso.
...
- Etapa 2. Use [get_index.py](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/get_index.py) para obter o índice do dispositivo.
...
- Etapa 3. Modifique o índice do dispositivo e os canais de [record.py](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/record.py) e então extraia a voz.
...
A3: Sim, podemos conectar o mic array v2.0 à porta usb do Raspberry e seguir o [Raspberry Pi Quick Start Guide with Script](https://github.com/alexa/avs-device-sdk/wiki/Raspberry-Pi-Quick-Start-Guide-with-Script) para fazer a interação por voz com a alexa.

**Q4: Vocês têm um exemplo para o Mic array v2.0 com sistema ROS?**

A4: Sim, agradecemos ao Yuki por compartilhar o pacote para integrar o [ReSpeaker Mic Array v2 com o middleware ROS (Robot Operating System)](https://github.com/furushchev/respeaker_ros).

**Q5: Como habilitar a porta de áudio de 3,5 mm para receber o sinal juntamente com a porta usb?**

A5: Faça o download do [novo firmware](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/i2s_i1o2.bin) e grave o XMOS seguindo as instruções de [How to update firmware](https://wiki.seeedstudio.com/pt-br/ReSpeaker_Mic_Array_v2.0/#update-firmware).

## Recursos

- **[PDF]** [Esquemático do ReSpeaker MicArray v2.0](https://files.seeedstudio.com/products/107990053/ReSpeakerMicArrayv2.0.1Schematic.zip)
- **[PDF]** [Folha de Produto do ReSpeaker MicArray v2.0](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/ReSpeaker%20MicArray%20v2.0%20Product%20Brief.pdf)
- **[PDF]** [Modelo 3D do ReSpeaker MicArray v2.0](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/RESPEAKER%20MIC%20v2.0.pdf)
- **[SKP]** [Modelo 3D do ReSpeaker MicArray v2.0](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/Respeaker%20Microphone%20Array%20v2.0_20180316.skp.zip)
- **[STP]** [Modelo 3D do ReSpeaker MicArray v2.0](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/RESPEAKER%20MIC-3D%20v2.0.stp.zip)
- **[PDF]** [Folha de Produto do XVF3000](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/XVF3000-3100-product-brief_1.4.pdf)
- **[PDF]** [Datasheet do XVF3000](https://files.seeedstudio.com/wiki/ReSpeaker_Mic_Array_V2/res/XVF3000-3100-TQ128-Datasheet_1.0.pdf)
- **[Github]** [ReSpeaker Mic Array v2 com middleware ROS (Robot Operating System)](https://github.com/furushchev/respeaker_ros)
