...
Se forem exibidos números de versão, Python e pip estão prontos para uso.
...
Se o build passar neste ponto, a saída do seu firmware foi gerada com sucesso.
...
**Passo 3: Fazer o seu próprio firmware**
...
Em seguida, selecione o arquivo de firmware que você acabou de gerar e grave-o no dispositivo.
...
2. Ajustar o comportamento de botões, GPS, Bluetooth e outros módulos
...
- O carimbo de tempo de envio pela malha é atualizado somente depois que um envio real pela malha é bem-sucedido
...
Comparado com o Projeto A e o Projeto B, este projeto altera apenas um arquivo-fonte e apenas duas funções. É um bom exemplo de como fazer uma alteração de comportamento específica de uma placa sem afetar o restante da base de código do Meshtastic.
...
Essa única captura de tela já explica tanto a alteração de código quanto o escopo específico da placa, então você não precisa de uma imagem separada apenas para explicar que as outras placas não são afetadas.
...
No meu build de teste, esse alvo foi compilado com sucesso após a alteração da melodia.
...
3. Uma imagem de verificação em dispositivo real ou um vídeo curto mostrando o teste de ligar ou desligar o T1000-E

Se o buzzer não tocar durante o teste, primeiro verifique se o modo de buzzer do dispositivo está habilitado. Em `playTones()`, o código retorna imediatamente quando o buzzer está desabilitado ou definido para modo somente notificação.
