---
description: Tutorial de configuração de Gateway MQTT Meshtastic baseado em XIAO ESP32S3 & Wio-SX1262 Kit. Possibilita mensagens globais, fornece backup quando os sinais da malha estão fracos e garante entrega mais rápida de mensagens via internet. 
...
Integrar MQTT com Meshtastic estende a comunicação off-grid ao fazer a ponte entre redes distantes. Isso possibilita mensagens globais, fornece um backup quando os sinais da malha estão fracos e garante uma entrega mais rápida de mensagens via internet. Perfeito para áreas remotas e situações de emergência.
...
Configurar um Gateway Meshtastic requer um dispositivo que possa executar tanto LoRa quanto Wi‑Fi. Além disso, se você quiser compartilhar a localização do seu gateway via mqtt, pode adicionar um módulo GPS.
...
**2**: Em seguida, use um cabo USB adequado para conectar o dispositivo ao PC. Talvez seja necessário desligar o dispositivo e então **manter pressionado o botão BOOT** enquanto conecta o cabo USB.

**3**: Siga as instruções fornecidas para realizar as operações de gravação subsequentes. Selecione o dispositivo como "**Seeed XIAO S3**", Firmware como **o mais recente**, e então clique em "**Flash**". Não se esqueça de marcar "**Full Erase and Install**" se quiser sobrescrever o firmware anterior.
...
### Passo 2 Verifique o ID do seu Dispositivo

O ID do dispositivo normalmente são os quatro últimos dígitos do endereço MAC. Você pode abrir o monitor da porta serial para verificar.
...
### Passo 3 Conexão do APP

Abra o aplicativo Meshtastic Android e conecte‑se ao seu dispositivo via Bluetooth. O código PIN padrão é 123456
...
### Passo 4 Configurar o Gateway Mesh-mqtt do Xiao ESP32S3
...
- Escolha `OK to MQTT`
- Clique em Send
- O dispositivo reinicia e reconecta automaticamente. O dispositivo será reiniciado e reconectará automaticamente. Isso pode levar de 30 s a 2 min.
...
- (Opcional) Se você quer que outros usuários mqtt o vejam no mapa, selecione `Map Reporting` e `I agree`.
- Clique em Send
- O dispositivo reinicia e reconecta automaticamente. O dispositivo será reiniciado e reconectará automaticamente. Isso pode levar de 30 s a 2 min.
...
- ative `Uplink enabled` e `Downlink enabled` para seu canal primário
- Clique em Send
...
Após essa configuração, seu dispositivo não poderá se conectar ao seu APP via Bluetooth. Mas poderá se conectar ao seu APP via rede.

- Preencha o SSID (Nome do Wi‑Fi) e a senha do Wi‑Fi. A rede Wi‑Fi que você pretende usar deve ser 2.4G.
- Clique em Send
