...
Clique duas vezes no botão RST para entrar manualmente no modo DFU. Você verá que um disco chamado `TRACKER L1` aparecerá após 10~15 s.
...
4. Quando solicitado, insira a senha Bluetooth mostrada na tela do dispositivo e clique em **OK**. Essa senha não é `123456`.
...
Você pode ir ao APP em `Position Settings` para ativar o GPS. Ou você pode ir à página de GPS do dispositivo e pressionar o joystick de quatro direções para ligar ou desligar o GPS.
...
O dispositivo não responde, nenhum LED, não é possível parear com o seu app. Se você achar que o dispositivo está completamente morto após gravar seu próprio firmware, você também pode tentar reinstalar o bootloader.
...
Verifique se o Python 3 e o pip estão disponíveis (se o pip estiver ausente, execute `python -m ensurepip --upgrade` primeiro):
...
Se o pip estiver ausente, execute `python3 -m ensurepip --upgrade` primeiro.
...
Quando você tiver concluído as etapas acima, siga [Firmware Flashing](https://wiki.seeedstudio.com/pt-br/get_started_with_other_mesh_firmware/#firmware-flashing) para gravar o firmware de aplicação.
