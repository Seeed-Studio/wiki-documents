...
---
:::danger note
Quando o dispositivo estiver nos estados abaixo, não reinicie ou desligue manualmente. Caso contrário, o dispositivo poderá ficar inutilizado.
1. Não tiver concluído o processo de transmissão de mensagem
2. Estiver sendo configurado
:::
## Tutorial em Vídeo

### Parte 1: Configuração após abrir a caixa
...
:::caution note
Antes de gravar o firmware, certifique-se de que você comprou o `T1000-E for Meshtastic`. Não grave o firmware em outro modelo de tracker que não suporte Meshtastic. Por favor, `não use NRF-OTA` para atualizar o firmware, isso pode fazer com que o dispositivo fique completamente inutilizado.
:::
...
Pressione uma vez para ligar o dispositivo, haverá uma melodia ascendente e o LED ficará aceso por cerca de 1s.
...
Se você quiser enviar mensagens de texto e se comunicar com outros nós no website, agora você pode conectar o dispositivo ao [Meshtastic Website](https://client.meshtastic.org/messages/broadcast/0). 
...
[Clique aqui](https://client.meshtastic.org/messages/broadcast/0) para ir para o website. 
 <p style={{textAlign: 'center'}}><img src="https://files.seeedstudio.com/wiki/SenseCAP/wio_tracker/MeshtasticWeb.png" alt="pir" width={1000} height="auto" /></p>
  ...
  Passo 2: Adicione o novo dispositivo 
...
Para começar a se comunicar pela malha, você deve definir a sua região. Essa configuração controla qual faixa de frequência o seu dispositivo usa e deve ser definida de acordo com a sua localização regional.
...
:::info
**EU_868** deve obedecer a uma limitação de ciclo de trabalho horário de 10%, calculada a cada minuto com base em uma janela contínua de 1 hora. Seu dispositivo parará de transmitir se você alcançar esse limite, até que seja permitido novamente.
:::

Agora que você definiu a região LoRa no seu dispositivo, pode continuar configurando qualquer [LoRa Configs](https://meshtastic.org/docs/configuration/radio/lora/) conforme suas necessidades.
...
Navegue para `Settings` -> `Telemetry(Sensors)` -> Ative os sensores.
...
Navegue para `Settings` -> `External Notification` -> Ative `GPIO` -> Defina `Output Pin GPIO`.
...
Verifique [External Notification Config](https://meshtastic.org/docs/configuration/module/external-notification/) para mais detalhes.
...
Por favor, defina o GPS como habilitado. Você pode ajustar o intervalo de atualização e o intervalo de broadcast para obter informações de localização mais atualizadas.

<p style={{textAlign: 'center'}}><img src="https://files.seeedstudio.com/wiki/SenseCAP/Meshtastic/MeshGPS.png" alt="pir" width={500} height="auto" /></p>

Para IOS, por favor ative o `Accurate Location`. Caso contrário, o posicionamento pode apresentar desvio.
...
Navegue para `Settings` -> `Firmware Updates` e verifique a versão atual do firmware.
...
:::danger
NÃO GRAVE OUTRO FIRMWARE QUE NÃO SEJA O FIRMWARE T1000-E, ISSO PODE FAZER COM QUE O DISPOSITIVO CONGELE.
:::

O seguinte firmware bloqueará o seu dispositivo：
...
Clique em `Enter DFU Mode`, aparecerá uma porta serial chamada `T1000-E xxx`, clique nela e conecte, o LED verde ficará aceso fixo e deverá aparecer uma unidade chamada `T1000-E`.
...
Conecte o cabo USB ao seu PC, pressione e segure o botão do dispositivo, então conecte o cabo de carregamento **rapidamente** duas vezes, o LED verde ficará aceso fixo e deverá aparecer uma unidade chamada `T1000-E`.
...
:::caution note
Antes de gravar o firmware, por favor grave primeiro o firmware de apagamento!
:::
...
Baixe o firmware de apagamento e copie para a unidade.
...
Esse processo pode levar algum tempo, aguarde até a unidade desaparecer e então abra um monitor serial para concluir o processo de apagamento.
...
- Às vezes parece que está desligado, mas na verdade é porque o LED e o buzzer não foram ativados. Verifique os seguintes parâmetros: 
 ...
 - Se ainda não funcionar, pressione e segure o botão do dispositivo e então conecte o cabo de carregamento para ver se um disco aparece no seu PC. Se aparecer, [clique aqui](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#device-bricked) para reinstalar o bootloader

### Dispositivo travado em boot loop
...
- Etapa 1: Tente entrar no modo DFU manualmente: pressione e segure o botão do dispositivo, então conecte o cabo de carregamento **rapidamente** duas vezes, o LED verde ficará aceso fixo.
...
O dispositivo não responde, não há LED e não pode ser pareado com o seu App.
...
Use este método se você tiver problemas ao instalar com o PyPi ou quiser modificar a ferramenta. Primeiro faça o clone deste repositório e entre em sua pasta.
...
Depois de concluir as etapas acima, você pode seguir esta [etapa](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#flash-the-application-firmware) para gravar o firmware do aplicativo.

**2) O dispositivo não consegue entrar no modo DFU, mas a porta serial pode ser detectada**.

- Abra uma ferramenta de porta serial

- Ajuste o baud rate para `1200`.

- Conecte o dispositivo.
   A luz piscará brevemente quando você conectá‑lo. Continue tentando até que a luz permaneça acesa, isso significa que o dispositivo voltou ao modo DFU, então [grave o bootloader](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#flash-the-bootloader) -> [apague o flash](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#step-2-flash-erase) -> [grave o firmware](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#step-3-flash-firmware).
...
- Se ainda não funcionar, desconecte o cabo de carregamento e deixe o dispositivo por alguns dias até que a bateria descarregue completamente; então conecte o cabo de carregamento e tente parear novamente.
...
- **Não é possível abrir a porta serial**
 ...
 - O log da porta serial foi executado por um tempo e depois parou.

 Isto pode ser causado por reiniciar ou desligar o dispositivo manualmente e à força quando o dispositivo está nos seguintes estados: não ter concluído o processo de transmissão de mensagens, estar sendo configurado......
...
- [Clique aqui](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#step-2-flash-erase) para executar um flash erase no dispositivo. E então regravar o firmware mais recente.
...
### Qualidade do Sinal
  ...
  - **SNR** reflete a qualidade do enlace de comunicação. Dispositivos normais geralmente operam acima de -7 dB. Dispositivos com SNR menor que -10 dB indicam desempenho ruim.

  - **RSSI** é determinado em conjunto pelo dispositivo e pelo ambiente ao seu redor. Dispositivos normais geralmente operam acima de -110 dBm. Dispositivos com RSSI menor que -115 dBm são considerados como tendo desempenho ruim. 

      Para obter o melhor efeito de sinal, use o dispositivo em uma área aberta, desobstruída e com interferência mínima.

### Definição dos Pinos Pogo
