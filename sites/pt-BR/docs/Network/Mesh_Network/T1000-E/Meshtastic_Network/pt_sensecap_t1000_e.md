...
:::info
**EU_868** deve obedecer a uma limitação de ciclo de trabalho horário de 10%, calculada a cada minuto em uma janela contínua de 1 hora. O seu dispositivo deixará de transmitir se você atingir esse limite, até que seja permitido novamente.
:::
...
Clique em `Enter DFU Mode`, aparecerá uma porta serial chamada `T1000-E xxx`, clique nela e conecte-a, o LED verde ficará aceso continuamente e deverá aparecer uma unidade chamada `T1000-E`.
...
Esse processo pode levar algum tempo; aguarde até a unidade desaparecer e, em seguida, abra um monitor serial para concluir o processo de apagamento.
...
- Se ainda não funcionar, tente `reinstalar o bootloader`. Conecte um cabo USB a um computador. Mantenha o botão do dispositivo pressionado e, em seguida, conecte o dispositivo ao computador; veja se um disco aparece no seu PC. Se aparecer, [reinstale o bootloader](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#device-never-turns-on).
...
- Passo 1: Tente entrar manualmente no modo DFU: pressione e segure o botão do dispositivo e então conecte o cabo de carregamento **rapidamente** duas vezes; o LED verde ficará aceso continuamente.
...
- Passo 2: [Erase Flash](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#step-2-flash-erase).

- Passo 3: [Flash Firmware](https://wiki.seeedstudio.com/pt-br/sensecap_t1000_e/#step-3-flash-firmware).

### Dispositivo inutilizado (bricked)

#### Descrição

O dispositivo não responde, nenhum LED, não é possível parear com o seu aplicativo.
...
Para usuários Windows, talvez seja necessário adicionar o caminho manualmente. Copie o local de instalação mostrado na última etapa. Em seguida, adicione-o da seguinte forma:
