---
description: Guia rápido do Gateway LoRaWAN reComputer R1225
title: Guia rápido do Gateway LoRaWAN reComputer R1225
keywords:
  - Gateway
  - SenseCAP
...
:::note
A solução de alimentação utiliza um diodo retificador de ponte para proteção contra polaridade reversa e é compatível com entradas CA e CC. Isso garante que **independentemente de como os terminais positivo e negativo da fonte de alimentação forem conectados**, o circuito não será danificado. Ao usar um retificador de ponte, a polaridade da tensão de saída permanece fixa, independentemente da polaridade CC de entrada, fornecendo proteção eficaz contra polaridade reversa.
:::
...
:::tip
Vale notar que o módulo PoE fornecido com o reComputer R1225 é compatível com o padrão IEEE 802.3af e pode fornecer uma potência máxima de 12,95W. **Portanto, se houver necessidade de conectar periféricos de alta potência, como SSD ou módulos 4G, a alimentação PoE pode não ser suficiente**. Nesse caso, é recomendado usar o terminal CA/CC para alimentação, a fim de garantir o funcionamento estável e confiável do dispositivo.
:::
...
Ao usar o R1225 pela primeira vez, se houver roteadores e cabos Ethernet por perto, você pode optar por conectar o R1225 à rede por meio de uma conexão cabeada.
...
:::tip
Se o Wi-Fi estiver configurado, você pode acessar a interface LuCI via endereço IP do R1225 sem um cabo Ethernet, desde que o seu computador e o R1225 estejam na mesma rede.
:::
...
- Passo 3: Obtenha o nome de usuário e a senha do seu dispositivo
  ...
  - Username：admin
  - Password：null  (**Nenhuma senha é necessária para o primeiro login**)
...
Conecte o Gateway LoRaWAN reComputer R1225 à energia, e o dispositivo será ligado automaticamente.

- Passo 2: Conectar ao hotspot AP

  - SSID : R1225-XXXX (**4 últimos dígitos do SN**)
  - Password : 1234567890
...
Quando o reComputer R1225 é ligado pela primeira vez, ele habilita automaticamente o modo hotspot. O SSID padrão para o hotspot é “R1225-XXXX (**4 últimos dígitos do SN**)”, e a senha padrão é “1234567890”; conecte seu computador a este hotspot AP.
...
Clique para abrir a página “Rede e Internet” nas configurações do sistema e, em seguida, role até o final para encontrar o endereço IP.
...
Insira o endereço IP (192.168.2.1) no seu navegador para entrar no Console Local.  
Em seguida, insira o nome de usuário e a senha do seu dispositivo e clique no botão Login.
...
- Passo 2: Clique no botão `Scan` para escanear o Wi-Fi
...
- Passo 2: Marque `Enable LoRa functionality`
...
- Etapa 6: Insira o `Gateway Name` e `Gateway ID (EUI: 64)`
  ...
  - Stats interval(secs): O intervalo esperado, em segundos, em que o gateway envia suas estatísticas
...
Após concluir todas as etapas de configuração, o gateway deve ser exibido como **`Online`** na plataforma ChirpStack.
