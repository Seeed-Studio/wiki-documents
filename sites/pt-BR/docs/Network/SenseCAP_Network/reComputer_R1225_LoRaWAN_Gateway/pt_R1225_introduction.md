...
O reComputer R1225 é um gateway e controlador IoT LoRaWAN industrial poderoso e flexível, baseado em Raspberry Pi, alimentado pelo CM4 e equipado com recursos de IA. Ele possui 3 interfaces RS485, portas Ethernet duplas e suporta os protocolos BACnet e Modbus, fornecendo todas as funcionalidades necessárias para um dispositivo IoT de borda industrial robusto e confiável. Com suporte para aceleradores de IA NPU, é uma solução ideal para aplicações de controle de acesso remoto, especialmente em sistemas BMS, BAS e iBMS.
...
- Indicadores LED duplos e claros permitem que os usuários verifiquem rapidamente o status operacional
...
### Capacidades sem fio abrangentes
...
<table >
  <tbody>
    ...
    <tr data-style="height: 18px;" style={{height: 18}}>
      ...
      <td data-style="height: 18px; width: 63.1933%;" colSpan={2} style={{height: 18, width: '63.1933%'}}>Padrão IEEE 802.3af, 12,95W </td>
    </tr>
    <tr data-style="height: 18px;" style={{height: 18}}>
    ...
    <tr data-style="height: 18px;" style={{height: 18}}>
      ...
      <td data-style="height: 18px; width: 63.1933%;" colSpan={2} style={{height: 18, width: '63.1933%'}}>3 x bloco de terminais de 3 pinos (isolados)</td>
    </tr>
    <tr data-style="height: 18px;" style={{height: 18}}>
    ...
    <tr data-style="height: 18px;" style={{height: 18}}>
      ...
      <td data-style="height: 18px; width: 63.1933%;" colSpan={2} style={{height: 18, width: '63.1933%'}}>750 g; R1225 com módulo 4G: 800 g</td>
    </tr>
  </tbody>
</table>
</div>
...
O reComputer R1225 suporta três opções de alimentação: AC, terminal DC e PoE. Por padrão, o reComputer R1225 é alimentado pelo terminal AC/DC (adaptador de energia oficial regional SKU:110061505/110061506).  **Um módulo PoE (SKU:110991925) está incluído**, proporcionando flexibilidade adicional de alimentação.
...
O reComputer R1225 não inclui um botão de energia por padrão. O sistema será ligado automaticamente assim que a alimentação for conectada. Ao desligar, selecione a opção de desligamento no sistema operacional e aguarde o desligamento completo do sistema antes de cortar a alimentação. Para reiniciar o sistema, basta reconectar a alimentação.
...
| ACT           | Verde          |        | No Linux, este pino piscará para indicar acesso à eMMC.<br /> Se ocorrer algum erro durante a inicialização, este LED piscará em um <br />padrão de erro que pode ser decodificado usando a tabela de consulta [no site da Raspberry Pi](https://www.raspberrypi.com/documentation/computers/configuration.html#led-warning-flash-codes).|
...
Consulte o Guia de [Quick Start](https://wiki.seeedstudio.com/pt-br/r1225_quick_start/#access-the-sensecap-gateway-os) para acessar a interface Luci.

Navegue até `System` > `LED Configuration`
...
<br />
<br />
O reComputer R1225 utiliza um slot para cartão SIM de tamanho padrão, comumente encontrado em aplicações industriais, que requer um cartão SIM padrão com dimensões de 25 mm x 15 mm.
:::note
...
O slot SSD no reComputer R1225 é projetado para acomodar SSDs NVMe M.2 2280 com capacidades de 128 GB, 256 GB, 512 GB e 1 TB. Este slot permite expansão de armazenamento em alta velocidade, possibilitando aos usuários melhorar o desempenho e a capacidade do sistema.
...
Este comando exibirá uma lista de todos os discos conectados ao seu sistema, incluindo o SSD se ele for detectado corretamente. Procure pelas entradas que representam o seu SSD. Elas normalmente começam com /dev/sd seguido por uma letra (por exemplo, /dev/sda, /dev/sdb, etc.).
...
O reComputer R1225 vem com duas portas Ethernet RJ45. ETH0 é uma interface Ethernet Gigabit nativa do CM4 que suporta três velocidades diferentes: 10/100/1000 Mbit/s. Um módulo PoE adicional pode ser adquirido para habilitar fornecimento de energia via Ethernet (PoE) através desta interface, fornecendo energia ao reComputer R1225. A outra porta, ETH1, suporta 10/100 Mbit/s e é convertida de USB.

#### Arquitetura da Interface de Rede

O reComputer R1225 está equipado com **duas portas Ethernet físicas** (ETH0 e ETH1). Essas duas portas desempenham funções diferentes com base na arquitetura host-container:

|   Porta  |       Função               | Descrição |
|----------|---------------------------|-----------|
| **ETH0** | Interface do container (LXC) | Esta interface é **mapeada diretamente (passthrough) do hardware para o container LXC** por meio da configuração de rede LXC do host. O container OpenWrt tem controle total sobre esta interface, gerenciando-a como uma porta WAN ou LAN padrão. Todo o tráfego em nível de aplicação (uplink LoRaWAN, MQTT, acesso à interface Web, etc.) flui por esta porta. |
| **ETH1** | Interface do host (Debian)   | Esta interface é **gerenciada pelo sistema host Debian**. Ela é usada para tarefas de gerenciamento em nível de host, como acesso SSH ao host, operações de gerenciamento de containers, atualizações de firmware e comunicação de monitoramento de UPS. Ela permanece isolada da pilha de rede do container. |

Essa separação garante que, mesmo que a rede do container esteja mal configurada ou inacessível, a interface de gerenciamento do host permaneça acessível para recuperação e manutenção.
...
Consulte o Guia de [Quick Start](https://wiki.seeedstudio.com/pt-br/r1225_quick_start/#wifi-connection) para conexão Wi-Fi
...
passo2. Conecte-se à rede Wi-Fi:
...
passo3. Após o dispositivo ser ligado, ele se conectará automaticamente ao Wi-Fi.  
  Se você quiser excluir as informações de Wi-Fi salvas:

  ```bash
  nmcli con del network-ssid
  ```

  Após a conexão ser desconectada, conecte-se a outro Wi-Fi.
...
[Por favor, clique aqui para instruções de montagem](/pt-br/recomputer_r1000_assembly_guide/#assemble-ssd).
...
1. Armazenamento de alta capacidade: cartões SSD podem ser utilizados para necessidades de armazenamento de alta capacidade.<br />
2. Unidade de boot com imagem: outro uso envolve utilizar o SSD tanto como armazenamento de alta capacidade quanto para armazenar imagens de sistema, permitindo inicializar diretamente a partir do cartão SSD.<br />
É importante notar que nem todos os cartões SSD disponíveis no mercado suportam o segundo uso. Portanto, se você pretende usá-lo como unidade de boot e não tem certeza de qual modelo comprar, recomendamos optar pelo nosso **SSD de 1TB(<a href="https://www.seeedstudio.com/NVMe-M-2-2280-SSD-1TB-p-5767.html">SKU [112990267]</a>)** recomendado. Este modelo foi testado e verificado quanto à funcionalidade de boot, reduzindo o risco de problemas de compatibilidade e minimizando os custos de tentativa e erro.
:::
...
:::note
[Por favor, clique aqui para instruções de montagem](https://wiki.seeedstudio.com/pt-br/recomputer_r1000_assembly_guide/#assemble-tpm-20-module).
:::
...
Um GPIO25 entre a CPU e a entrada de energia CC/CA é usado para alarmar a CPU quando a fonte de alimentação de 5V cai. Então a CPU deve fazer algo urgente em um script antes do esgotamento de energia do supercapacitor e executar um `$ shutdown`
<br />
...
Use `/boot/overlays/README` como referência, depois modifique `/boot/config.txt`.
...
Uma DSI (J24) é reservada na placa, para uso especial. Os usuários devem adquirir plug-ins de acordo com suas próprias necessidades.
...
- [Tabela de Consumo de Energia do R1225](https://files.seeedstudio.com/wiki/reComputer_1225_LoRaWAN_Gateway/res/R1225_Power_Consumption_Table.xlsx)
