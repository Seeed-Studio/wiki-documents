---
description: Configuração LNS do Gateway Multi-Plataforma SenseCAP M2
title: Configuração LNS do Gateway Multi-Plataforma SenseCAP M2
keywords:
  - SenseCAP Network
...
url: https://wiki.seeedstudio.com/pt-br/Network/SenseCAP_Network/SenseCAP_M2_Multi_Platform/SenseCAP_M2_MP_Gateway_LNS_Configuration/
---


## Configuração do Gateway

Configure o gateway pela interface Web UI, consulte o [Quick Start](https://files.seeedstudio.com/products/SenseCAP%20M2/Quick%20Start%20for%20SenseCAP%20M2%20Multi-Platfrom%20Gateway%20&%20Sensors.pdf) para fazer login no Luci.
...
Em seguida, você verá a página do painel (dashboard).
...
Deve haver um Region ID, clique nele e confirme as informações, que devem ser iguais às configurações feitas na etapa anterior.
...
**Expected uplink interval**: Personalizar, padrão 3600s
...
Navegue até `Codec` e selecione `JavaScript functions`, copie o [SenseCAP Decoder for TTN](https://github.com/Seeed-Solution/SenseCAP-Decoder) e envie-o.
...
Defina o Name e o Gateway ID (você pode clicar para gerar o ID aleatoriamente) e envie.
...
Dê um nome à sua aplicação e envie.
...
Navegue até a sua aplicação e clique em `Add device`.
...
Cole o EUI do seu dispositivo e selecione o perfil de dispositivo que adicionamos antes, depois envie.
...
Verifique os `Events` do seu dispositivo; você receberá o pacote de join quando o dispositivo entrar na rede.
...
Este capítulo é para desenvolvimento de serviços em nuvem; a diretriz a seguir é apenas para referência.

### MQTT

#### Tópico

A integração MQTT expõe todos os eventos conforme documentado pelos tipos de eventos.
...
|join|Evento de join do dispositivo|
...
Quando o object.valid for true, significa que a análise dos dados foi bem-sucedida; então você pode percorrer o object.messages e extrair o tipo de dado de que precisa.
...
O `measurementId` na mensagem ‘report_telemetry’ consulte [SenseCAP Measurement ID](https://sensecap-statics.seeed.cn/hardware/lorapp/httpserver/src/constants/sensor-name-lang-dictionary.json) para mais detalhes.
...
1) Descrição de exemplo de payload de evento Up para o Data logger SenseCAP:
...
A integração HTTP fará requisições POST para o endpoint ou endpoints de evento configurados (podem ser configuradas múltiplas URLs, separadas por vírgulas). O parâmetro de query da URL event indica o tipo do evento.
...
`command`: Consulte o comando de downlink no **Manual do Usuário do Dispositivo** para mais detalhes.

|Topic|application/APPLICATION_ID/device/DEV_EUI/command/down|
| - | :- |
|devEUI|EUI do Dispositivo|
|confirmed|true/false(se o payload deve ser enviado como dados de downlink confirmados ou não)|
...
2) Definir o intervalo de upload dos sensores LoRaWAN SenseCAP S210x para 1 min:
