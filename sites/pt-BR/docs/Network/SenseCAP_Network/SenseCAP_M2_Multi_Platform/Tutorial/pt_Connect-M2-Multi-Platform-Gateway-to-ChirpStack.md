...
Antes de começar, faça [Login](https://www.chirpstack.io/application-server/use/login/) no [ChirpStack Application Server](https://www.chirpstack.io/application-server/).

As credenciais padrão são: Nome de usuário: admin; Senha: admin

:::tip Note
Se você ainda não conectou sua instância do [ChirpStack Application Server](https://www.chirpstack.io/project/application-server/) a uma instância do [ChirpStack Network Server](https://www.chirpstack.io/project/network-server/), você precisa fazer isso primeiro. Consulte [Network servers](https://www.chirpstack.io/application-server/use/network-servers/). Além disso, você precisa conectar a organização ao network-server criando um [Service profile](https://www.chirpstack.io/application-server/use/service-profiles/).
:::
...
**Gateway EUI**: O EUI do gateway pode ser encontrado na etiqueta do dispositivo ou no Local Console
...
:::note
Selecione a MAC version/Regional parameters revision/ADR algorithm de acordo com o seu dispositivo. Para detalhes, consulte: https://lora-alliance.org/resource_hub/
:::
...
Clique na aplicação à qual você deseja adicionar seu dispositivo. Na guia **Devices**, clique em **Add device**
...
Configure o gateway pela interface Web UI, consulte o [Quick Start](https://files.seeedstudio.com/products/SenseCAP/M2_Multi-Platform_Gateway/Quick_Start_for_SenseCAP_Gateway_&_Sensors.pdf) para fazer login primeiro no Local Console.
