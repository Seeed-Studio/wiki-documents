...
## Fazer stake de um nó Flux CUMULUS
...
3. Na página Trade, selecione spot e no Market selecione FLUX/USDT

4. Insira a quantidade de USDT e certifique-se de que o Flux convertido seja maior que 1000
...
### Etapa 4 Criar um novo nó Flux Cumulus
...
3. Envie o Flux do Payments portfolio para outro portfolio.

4. Agora você pode implantar o novo nó Flux Cumulus no SenseCAP M4 Square!
...
3. O dispositivo fará a auto-configuração automaticamente. A luz laranja constante em L3 indica que a configuração e a conexão à internet foram bem-sucedidas.
...
### Etapa 2 Instalar o SenseCAP Hotspot App e o Zelcore App

1. O SenseCAP Hotspot App foi projetado para configurar e gerenciar dispositivos. Por favor, escaneie o QR code a seguir para instalar o SenseCAP Hotspot App.
...
1. Pressione o pino na parte inferior esquerda do dispositivo e a tampa de extremidade será levantada, então retire-a.
...
- Selecione o dispositivo para Connect
...
2. Insira Identity Key, Collateral TX ID, Output Index, Zel ID e Kadena Address a partir do Zelcore APP, baixe o Zelcore para mais informações.
...
4. Quando começar a sincronizar a cadeia Flux, vá para o Zelcore App e insira o IP público e o nome no seu FluxNode. Em seguida, clique no botão Start para iniciar seu FluxNode.

Se o seu FluxNode estiver funcionando bem e passando nos benchmarks, levará 1 bloco para ser processado para que o status do seu FluxNode mude para Started e de 1 a 10 blocos (2 a 20 minutos) para ficar Confirmed. Confirmed é o status final. Você pode verificar no FluxOS atualizando a página inicial; se o nó for Confirmed, isso será indicado na página Status.
...
UPnP significa universal plug and play, que basicamente permite que dispositivos na sua rede solicitem que portas sejam abertas pelo seu roteador para tráfego de entrada. UPnP pode ser uma solução mais fácil se o seu roteador for compatível e seguir os padrões adequados de upnp.
...
- 6 Agora o M4 UPnP está habilitado e o status de UPnP é ENABLE
...
O nó Fractus foi criado para aumentar o armazenamento da rede Flux. Um nó Fractus é um nó de nível Cumulus que fornece pelo menos 10 TB de armazenamento. Os nós Fractus ganharão 15% adicionais de Flux além da recompensa nativa de bloco Flux.
...
- 80 MB/s de velocidade de gravação em disco.
- 100 Mb/s de download/upload.

![](https://wdcdn.qpic.cn/MTMxMDI3MDEwODc4Njk2MTk_47467_AZXsjpYcOQweNFnJ_1675844077?w=1200&h=654)

- 1 SenseCAP M4 Square implantou FluxNode
- 2 Conecte uma unidade de 10 TB na porta USB3.1 do SenseCAP M4 Square
- 3 Conecte o SenseCAP Hotspot App ao M4 via Bluetooth
...
- 6 Agora o M4 Fractus está aberto e o status do Fractus é True
...
Para mais informações sobre o seu SenseCAP M4 Square, adicione-o ao dashboard SenseCAP.
...
- Adicionar via Bluetooth: se o seu M4 tiver implantado FluxNode, conecte o M4 ao SenseCAP Hotspot App via Bluetooth e então adicione-o ao dashboard.
...
Conecte o M4 ao SenseCAP Hotspot App via Bluetooth e então abra o aplicativo Flux e clique no botão Restore Block. O M4 restaurará os blocos automaticamente e não há necessidade de outras etapas. O tempo de restauração depende da rede do dispositivo, certifique-se de que o dispositivo tenha uma boa conexão de rede.

Excluir
------

**Por favor, opere com cuidado!** Exclua o FluxNode CUMULUS que foi implantado no SenseCAP M4. Conecte o M4 ao SenseCAP Hotspot App via Bluetooth e então abra o aplicativo Flux e clique no botão Delete para excluir o FluxNode.

Reimplantar
--------

**Por favor, opere com cuidado!** Reimplantar excluirá automaticamente primeiro o FluxNode implantado. Conecte o M4 ao SenseCAP Hotspot App via Bluetooth e então abra o aplicativo Flux e clique no botão Redeploy e insira as novas informações do FluxNode para reimplantar.

**Lista de aplicativos descentralizados compatíveis**
====================================
