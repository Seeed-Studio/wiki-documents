---
description: Explore modelos pré-treinados, treinamento personalizado, implantação, visualização em tempo real e fluxos de trabalho de saída de modelo no SenseCraft AI para dispositivos de IA de borda compatíveis.
title: 'SenseCraft AI: Treine e Implemente Modelos de IA de Borda'
...
A página inicial do SenseCraft AI funciona como o hub central, oferecendo aos usuários uma visão geral das principais funcionalidades da plataforma. A barra de navegação na parte superior da página apresenta quatro seções principais: **Home**, **Applications**, **Models** e **Community**.

- **Home** – Página inicial da plataforma.
- **Applications** – Navegue pela **Application Square** para encontrar aplicativos prontos para uso ou gerencie os seus em **My Applications** (incluindo **Create New Applications** e **Create New Workspace for reCamera**).
- **Models** – Contém a **Model Library** (modelos pré-treinados), o **Workspace** por dispositivo, **Training** (Image Classification Detection, Audio Classification Detection, Image Object Detection) e **My Models** (com **Add Model**).
- **Community** – Conecte-se com outros usuários e colaboradores do SenseCraft AI.

O foco principal da página inicial é a seção **Start your journey: Deploy a pretrained Model**, que orienta os usuários em um processo passo a passo para implementar um modelo pré-treinado usando hardware da Seeed Studio. O processo é dividido em três etapas principais:
...
<div style={{textAlign:'center'}}>
  <img
    ...
    alt="Página inicial do SenseCraft AI mostrando o fluxo de trabalho de Detecção de Gestos, desde a descoberta do modelo até a implantação e saída do sensor antes de um dispositivo ser conectado"
    style={{width:950, maxWidth:'100%', height:'auto'}}
  />
</div>

1. Selecione um modelo pré-treinado no repositório de modelos.
1. Implemente e visualize em tempo real os resultados do modelo usando o recurso "Deploy and Preview Vision".
1. Aplique o modelo ao hardware da Seeed Studio conectado e visualize a saída do sensor.
...
[SenseCraft AI](https://sensecraft.seeed.cc/ai/model?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_ai_model_library) é uma plataforma aberta que permite aos usuários navegar por todos os modelos de IA públicos e páginas Iniciais sem fazer login. Você só precisa se cadastrar e entrar quando precisar implementar um modelo ou compartilhar o seu próprio modelo.

[SenseCraft AI](https://sensecraft.seeed.cc/ai/model?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_ai_model_library) e [SenseCraft Data Platform](https://sensecap.seeed.cc/portal/?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_data_portal#/login) (plataforma original SenseCAP Cloud) são ambos serviços de software fornecidos pela seeed studio para os usuários; os usuários só precisam criar uma conta em qualquer uma das plataformas e, em seguida, podem usar a mesma conta para entrar em ambas.

### Sign up

1. Digite seu nome e e-mail válido e, em seguida, clique em **get captcha**
...
2. Obtenha o código de verificação no seu e-mail e insira-o na página de cadastro
...
3. Digite sua senha e outras informações de usuário para concluir o cadastro.
...
Entre com a conta de e-mail que você cadastrou
...
Se você esquecer a senha da sua conta, insira sua conta válida e obtenha o código de verificação para definir uma nova senha.

O período de validade do código de verificação é de 10 min, conclua a redefinição dentro de 10 minutos.
...
1. Acesse a página da conta de usuário e clique no botão "Change your password".

2. Digite a senha antiga e a nova senha para alterar a senha.
...
A seção de Modelos Pré-treinados do site SenseCraft AI é um repositório abrangente de modelos de IA que os usuários podem acessar e implementar facilmente em seus dispositivos. O repositório de modelos atualmente abriga uma coleção impressionante de mais de 400 modelos, com mais sendo adicionados continuamente.
...
- **Supported Devices**: Os usuários podem selecionar modelos que sejam compatíveis com o hardware específico que estão usando, garantindo integração perfeita e desempenho ideal.

- **Task**: Os modelos são categorizados de acordo com as tarefas para as quais foram projetados, como Detecção, Classificação ou Segmentação. Isso permite que os usuários identifiquem rapidamente modelos que estejam alinhados com os requisitos de seus projetos.

- **Publisher**: Os usuários também podem filtrar modelos com base no publisher, facilitando encontrar modelos de fontes confiáveis ou de desenvolvedores específicos.
...
A área central da página de Modelos Pré-treinados apresenta informações essenciais sobre cada modelo, incluindo seu nome, uma breve descrição e uma representação visual. Essa visão geral rápida ajuda os usuários a ter uma noção do que cada modelo oferece e de como ele pode se encaixar em seus projetos.

Para acessar informações mais detalhadas sobre um modelo específico, os usuários podem simplesmente clicar no card do modelo. Isso os levará a uma página dedicada para esse modelo, onde poderão encontrar descrições detalhadas, métricas de desempenho e instruções passo a passo sobre como instalar e usar o modelo em seus dispositivos.
...
Além dos modelos de IA públicos disponíveis no repositório, o SenseCraft AI também oferece um espaço personalizado para usuários que treinaram ou fizeram upload de seus próprios modelos. Ao fazer login em sua conta SenseCraft AI, os usuários podem acessar a seção "My Own Models", onde podem encontrar e gerenciar seus modelos privados.
...
- **Treinamento Rápido**: Esta opção permite que os usuários gerem um modelo de reconhecimento de classe única simplesmente inserindo o nome do objeto. Conforme explicado no site, "Based on YOLO - World object detection model, you can quickly generate a single-class recognition model by inputting text."
...
A opção Treinamento Rápido dentro do treinamento de Detecção de Objetos é alimentada pelo modelo de detecção de objetos YOLO-World, que é um sistema de detecção de objetos em tempo real de última geração. Quando um usuário insere o nome de um objeto, o sistema aproveita o conhecimento pré-treinado do modelo YOLO-World para gerar um modelo de reconhecimento de classe única especificamente ajustado para detectar esse objeto.
...
Ao se basear no modelo YOLO-World, a opção Treinamento Rápido herda seus recursos robustos de extração de características e de localização de objetos. O modelo pré-treinado serve como uma base sólida, permitindo que os usuários gerem rapidamente um modelo de reconhecimento de classe única sem a necessidade de grandes quantidades de dados de treinamento ou de recursos computacionais extensivos.

No entanto, é importante reconhecer que a opção Treinamento Rápido pode ter limitações em termos de adaptabilidade e precisão. Como o modelo gerado depende do conhecimento pré-existente do modelo YOLO-World, ele pode nem sempre capturar as características únicas ou variações do objeto especificado pelo usuário. Isso pode levar a redução de precisão ou detecções falsas em determinados cenários.

- **Treinamento com Coleta de Imagens**: Esta opção exige que os usuários insiram o nome do objeto e enviem imagens relevantes. O site descreve esse recurso da seguinte forma: "Based on YOLO - World object detection model, you can customize the training for text and image, which can improve the detection accuracy of the generated model."
...
A opção Treinamento com Coleta de Imagens no SenseCraft AI permite que os usuários treinem um modelo de detecção de objetos personalizado usando seu próprio conjunto de dados, sem a necessidade de anotação manual de imagens. Esse recurso é baseado no modelo de detecção de objetos YOLO-World e utiliza uma abordagem de treinamento especializada que elimina a exigência de rotulagem de caixas delimitadoras ou segmentação de objetos.

O princípio-chave por trás dessa opção de treinamento é o conceito de aprendizado fracamente supervisionado. No aprendizado fracamente supervisionado, o modelo aprende a detectar objetos usando apenas rótulos em nível de imagem, sem a necessidade de localização precisa de objetos ou anotações de caixas delimitadoras. O modelo YOLO-World, que serve como base para o Treinamento com Coleta de Imagens, foi projetado para aproveitar essa abordagem de forma eficaz.

Durante o processo de treinamento, os usuários fornecem um conjunto de imagens juntamente com os nomes dos objetos que desejam detectar. O modelo então aprende a associar os padrões visuais e características presentes nas imagens com os nomes de objetos fornecidos. Ao expor o modelo a uma ampla variedade de imagens contendo os objetos de interesse, ele aprende a generalizar e a detectar esses objetos em novas imagens ainda não vistas.
...
Ao eliminar a necessidade de anotação manual de imagens, a opção Treinamento com Coleta de Imagens reduz significativamente o esforço e o tempo necessários para criar um modelo de detecção de objetos personalizado. Os usuários podem simplesmente coletar um conjunto de dados de imagens contendo os objetos que desejam detectar, fornecer os nomes dos objetos e deixar que o modelo aprenda a reconhecer esses objetos automaticamente.

No entanto, é importante observar que a qualidade e a diversidade do conjunto de dados ainda desempenham um papel crucial no desempenho do modelo resultante. A capacidade do modelo de generalizar e detectar objetos com precisão depende da variedade e da representatividade das imagens de treinamento. Os usuários devem se esforçar para coletar um conjunto de dados que cubra diferentes aparências de objetos, poses, fundos e condições de iluminação para garantir um desempenho robusto.

Ao fornecer essas duas opções de treinamento, o SenseCraft AI permite que os usuários criem modelos de detecção de objetos personalizados que são otimizados para suas necessidades específicas. A opção Treinamento Rápido é ideal para usuários que precisam de um modelo simples de reconhecimento de classe única e desejam gerá-lo rapidamente. Por outro lado, a opção Treinamento com Coleta de Imagens é adequada para usuários que exigem um modelo mais preciso e personalizado, pois permite que eles forneçam seus próprios dados de treinamento na forma de nomes de objetos e imagens.
...
1. Para adicionar um modelo, você precisa preencher as seguintes informações:
...
2. Clique em Next quando as informações estiverem completas.
...
1. Insira as seguintes informações sobre os parâmetros do modelo.
2. Publish the model to the public AI model library é marcado por padrão, o modelo ficará visível para todos após ser salvo; se desmarcado, o modelo ficará visível apenas para você após ser salvo.

|     | Conteúdo|
| --- | --- |
| Formato do Modelo | 1 O formato correto para o modelo<br />2 Opções:ONNX, Tensor RT, Pytorch<br />3 A plataforma suportará mais formatos de modelo |
| Tarefa | 1 O tipo de tarefa do modelo<br />2 Opções:Detection,Classification,Segment,Pose |
| Framework de IA | 1 O framework de IA do modelo<br />2 Opções:YOLOV5,YOLOV8,FOMO,MobileNetV2,PFLD<br />3 A plataforma suportará mais frameworks de IA |
| Classes | 1 Classes ou rótulos que o modelo categoriza para uma tarefa ou problema específico<br />2 Certifique-se de que o ID da classe e o nome da classe correspondam corretamente. |
| Arquivo do Modelo | Envie um arquivo de modelo no formato de sua escolha. |
| Precisão do Modelo | 1 Precisão do modelo<br />2 Opções:Int8,Float16,Float32 |
...
:::note
Para garantir o desenvolvimento saudável de nossa plataforma, revisaremos os modelos e conteúdos publicados pelos usuários. Se for encontrado qualquer conteúdo ilegal, não compatível ou que infrinja direitos, ele não será permitido para publicação e poderá ser excluído conforme o caso.
...
**Publicar Modelo**: Publicar um modelo privado que ficará disponível para todos os usuários.
...
**Privatizar Modelo**: Privatizar um modelo público; o modelo ficará visível apenas para você.
...
**Excluir Modelo**: Excluir um modelo privado; modelos públicos não podem ser excluídos.
...
Depois que um usuário tiver enviado com sucesso um modelo treinado, ele poderá navegar até a página específica do dispositivo dentro do Espaço de Trabalho. Na seção "Process", os usuários podem observar o fluxo de detecção em tempo real do dispositivo conectado, permitindo que eles visualizem o desempenho do modelo em ação.
...
Além da visualização em tempo real, o Workspace também oferece a capacidade de ajustar finamente o parâmetro de limiar de confiança do modelo. Esse recurso permite que os usuários ajustem a sensibilidade do modelo à detecção de objetos, possibilitando encontrar um equilíbrio entre precisão e revocação.

Ao manipular o limiar de confiança, os usuários podem controlar o comportamento do modelo em termos de detecção de objetos. Um limiar de confiança mais alto fará com que o modelo seja mais seletivo, detectando apenas objetos com um alto grau de certeza. Por outro lado, um limiar de confiança mais baixo tornará o modelo mais sensível, detectando objetos mesmo com pontuações de confiança menores.

Essa capacidade de ajuste fino permite que os usuários adaptem o modelo aos seus requisitos específicos, otimizando seu desempenho com base nas características de sua aplicação e do ambiente em que o dispositivo opera.
...
O Workspace vai além da implantação e visualização do modelo ao fornecer aos usuários as ferramentas para prototipar e desenvolver rapidamente aplicações usando os modelos treinados. A seção "Output" oferece uma variedade de opções para que os usuários interajam com os resultados do modelo e os integrem em suas aplicações desejadas.

Tomando o XIAO ESP32S3 Sense como exemplo, o Workspace oferece suporte a vários protocolos e interfaces de comunicação, como MQTT, GPIO e porta serial. Essas opções permitem que os usuários transmitam perfeitamente a saída do modelo para outros sistemas, acionem ações com base na detecção de objetos ou realizem processamento adicional sobre os resultados detectados.

Ao oferecer essas opções de saída, o SenseCraft AI simplifica o processo de integração dos modelos treinados em aplicações práticas. Os usuários podem experimentar rapidamente diferentes métodos de comunicação e desenvolver protótipos que aproveitem os recursos de detecção de objetos de seus modelos.
...
- [Centro de documentação do SenseCraft AI](/pt-br/sensecraft-ai/sensecraft-ai-main/)
- [Treinar um modelo de classificação de imagens](/pt-br/sensecraft-ai/tutorials/sensecraft-ai-training-classification/)
- [Treinar um modelo de detecção de objetos](/pt-br/sensecraft-ai/tutorials/sensecraft-ai-training-object-detection/)
- [Primeiros passos com a SenseCraft Data Platform](/pt-br/cloud/sensecraft-data/sensecraft-data-platform/overview/)
