...
A página inicial do SenseCraft AI funciona como o hub central, oferecendo aos usuários uma visão geral das principais funcionalidades da plataforma. A barra de navegação na parte superior da página apresenta cinco seções principais: **Home**, **Pretrained Models**, **Training**, **Vision Workspace** e **About SenseCraft AI**.
...
1. Selecionar um modelo pré‑treinado no repositório de modelos.
1. Implantar e pré‑visualizar os resultados do modelo em tempo real usando o recurso "Deploy and Preview Vision".
...
[SenseCraft AI](https://sensecraft.seeed.cc/ai/#/model) e [SenseCraft Data Platform](https://sensecap.seeed.cc/portal/#/login) (plataforma original SenseCAP Cloud) são ambos serviços de software fornecidos pela seeed studio para os usuários; os usuários só precisam se cadastrar em uma conta em qualquer uma das plataformas e, em seguida, podem usar a mesma conta para fazer login em ambas.

### Cadastro

1. Insira seu nome e e‑mail válido e, em seguida, clique em **get capcha**
...
- **Quick Training**: essa opção permite que os usuários gerem um modelo de reconhecimento de classe única simplesmente inserindo o nome do objeto. Conforme explicado no site, "Based on YOLO - World object detection model, you can quickly generate a single-class recognition model by inputting text."
...
O Vision Workspace vai além da implantação e visualização de modelos, fornecendo aos usuários ferramentas para criar rapidamente protótipos e desenvolver aplicações usando os modelos treinados. A seção "Output" oferece uma variedade de opções para que os usuários interajam com os resultados do modelo e os integrem em suas aplicações desejadas.

Tomando o XIAO ESP32S3 Sense como exemplo, o Vision Workspace oferece suporte a vários protocolos e interfaces de comunicação, como MQTT, GPIO e Serial Port. Essas opções permitem que os usuários transmitam perfeitamente a saída do modelo para outros sistemas, acionem ações com base na detecção de objetos ou realizem processamento adicional sobre os resultados detectados.
...
A saída Serial Port oferece uma maneira simples de estabelecer comunicação entre o dispositivo e outros sistemas, permitindo que os usuários transmitam os resultados do modelo para processamento ou visualização adicionais.
