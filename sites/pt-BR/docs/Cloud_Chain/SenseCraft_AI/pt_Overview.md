...
- **Models** – Abriga a **Model Library** (modelos pré-treinados), o **Workspace** por dispositivo, **Training** (Image Classification Detection, Audio Classification Detection, Image Object Detection) e **My Models** (com **Add Model**).
...
1. Aplique o modelo ao hardware conectado da Seeed Studio e visualize a saída do sensor.
...
Descendo mais na página, os usuários encontrarão uma introdução ao recurso "Training Models". Esta seção categoriza o conteúdo relacionado ao treinamento de modelos, facilitando para os usuários encontrar as informações de que precisam para treinar seus próprios modelos de IA usando os recursos da plataforma.

Por fim, a página inicial apresenta o recurso "Sharing Vision AI Models", que incentiva a colaboração e o compartilhamento de conhecimento entre a comunidade SenseCraft AI. Esse recurso permite que os usuários compartilhem seus modelos treinados com outras pessoas, promovendo um senso de comunidade e permitindo que os usuários construam sobre o trabalho uns dos outros.

## Conta de usuário

[SenseCraft AI](https://sensecraft.seeed.cc/ai/model?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_ai_model_library) é uma plataforma aberta que permite aos usuários navegar por todos os modelos de IA públicos e páginas iniciais sem fazer login. Você só precisa se cadastrar e entrar quando precisar implantar um modelo ou compartilhar seu próprio modelo.

[SenseCraft AI](https://sensecraft.seeed.cc/ai/model?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_ai_model_library) e [SenseCraft Data Platform](https://sensecap.seeed.cc/portal/?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_data_portal#/login) (plataforma original SenseCAP Cloud) são ambos serviços de software fornecidos pela seeed studio para os usuários; os usuários só precisam se cadastrar para uma conta em qualquer uma das plataformas e, em seguida, podem usar a mesma conta para entrar em ambas as plataformas.
...
3. Digite sua senha e outras informações de usuário para concluir seu registro.
...
Entre com o e-mail da sua conta registrada
...
1. Visite a página da conta de usuário e clique no botão "Change your password".
...
- **Dispositivos compatíveis**: Os usuários podem selecionar modelos que sejam compatíveis com o hardware específico que estão utilizando, garantindo integração perfeita e desempenho ideal.

- **Tarefa**: Os modelos são categorizados de acordo com as tarefas para as quais foram projetados, como Detecção, Classificação ou Segmentação. Isso permite que os usuários identifiquem rapidamente os modelos que se alinham com os requisitos de seus projetos.

- **Publicador**: Os usuários também podem filtrar modelos com base no publicador, facilitando a localização de modelos de fontes confiáveis ou de desenvolvedores específicos.
...
A área central da página de Modelos Pré-treinados exibe informações essenciais sobre cada modelo, incluindo seu nome, uma breve descrição e uma representação visual. Essa visão geral rápida ajuda os usuários a ter uma noção do que cada modelo oferece e de como ele pode se encaixar em seus projetos.
...
O treinamento de Classificação é baseado em TensorFlow e é totalmente baseado na web, eliminando quaisquer limitações de sistema operacional. Esse recurso permite que os usuários treinem modelos usando imagens capturadas pela câmera do computador local ou por produtos da Seeed Studio. Para treinar um modelo, os usuários precisam apenas coletar 40–50 imagens por classe, sem necessidade de rotulagem manual. O processo de treinamento é rápido, levando apenas alguns minutos para gerar um modelo. Além disso, a interface web fornece funcionalidade de visualização em tempo real, permitindo que os usuários vejam instantaneamente os resultados de seu modelo treinado.
...
A opção Quick Training na seção de treinamento de Detecção de Objetos é alimentada pelo modelo de detecção de objetos YOLO-World, que é um sistema de detecção de objetos em tempo real de última geração. Quando um usuário insere o nome de um objeto, o sistema aproveita o conhecimento pré-treinado do modelo YOLO-World para gerar um modelo de reconhecimento de classe única especificamente ajustado para detectar esse objeto.
...
Ao se basear no modelo YOLO-World, a opção Quick Training herda seus recursos robustos de extração de características e localização de objetos. O modelo pré-treinado serve como uma base sólida, permitindo que os usuários gerem rapidamente um modelo de reconhecimento de classe única sem a necessidade de grandes quantidades de dados de treinamento ou recursos computacionais extensos.

No entanto, é importante reconhecer que a opção Quick Training pode ter limitações em termos de adaptabilidade e precisão. Como o modelo gerado depende do conhecimento pré-existente do modelo YOLO-World, ele pode nem sempre capturar as características ou variações únicas do objeto especificado pelo usuário. Isso pode levar à redução de precisão ou a detecções falsas em determinados cenários.

- **Image Collection Training**: Esta opção exige que os usuários insiram o nome do objeto e façam upload de imagens relevantes. O site descreve esse recurso da seguinte forma: "Based on YOLO - World object detection model, you can customize the training for text and image, which can improve the detection accuracy of the generated model."
...
- Dispositivo compatível: escolha em qual dispositivo o modelo será implantado; atualmente a plataforma oferece suporte a dispositivos Jetson, XIAO ESP32-S3, etc.
- Imagem de exemplo de inferência do modelo: faça upload de uma imagem dos resultados de inferência do modelo
...
2. Publicar o modelo na biblioteca pública de modelos de IA é marcado por padrão, o modelo ficará visível para todos após salvar; se desmarcado, o modelo ficará visível apenas para você após salvar.

|     | Conteúdo|
| --- | --- |
| Formato do Modelo | 1 O formato correto para o modelo<br />2 Opções:ONNX, Tensor RT, Pytorch<br />3 A plataforma oferecerá suporte a mais formatos de modelo |
| Tarefa | 1 O tipo de tarefa do modelo<br />2 Opções:Detection,Classification,Segment,Pose |
| Framework de IA | 1 O framework de IA do modelo<br />2 Opções:YOLOV5,YOLOV8,FOMO,MobileNetV2,PFLD<br />3 A plataforma oferecerá suporte a mais frameworks de IA |
...
:::note
Para garantir o desenvolvimento saudável da nossa plataforma, revisaremos os modelos e conteúdos publicados pelos usuários. Se for encontrado qualquer conteúdo ilegal, não compatível ou que infrinja direitos, ele não poderá ser publicado e poderá ser excluído conforme necessário.
Agradecemos sua compreensão e apoio na manutenção de um ambiente de plataforma saudável!
:::
...
A seção Workspace do SenseCraft AI é dedicada a operações específicas de dispositivos e à implantação de modelos treinados. Ela fornece uma interface contínua para que os usuários integrem seus modelos personalizados com vários dispositivos de hardware e visualizem os resultados em tempo real. Atualmente, os dispositivos compatíveis incluem Grove Vision AI V2, XIAO ESP32S3 Sense, NVIDIA Jetson e reCamera.
...
A saída pela Porta Serial fornece uma maneira simples de estabelecer comunicação entre o dispositivo e outros sistemas, permitindo que os usuários transmitam os resultados do modelo para processamento ou visualização adicionais.
