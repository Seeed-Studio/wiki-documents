---
description: Instalação do Water Cooling Kit
title: Guia de instalação do Water Cooling Kit
keywords:
  - Raspberry_Pi
image: https://files.seeedstudio.com/wiki/wiki-platform/S-tempor.png
slug: /Cooler_Device
sku: 110070128,114070261
last_update:
  date: 1/24/2024
  author: Parker
createdAt: '2024-01-24'
updatedAt: '2026-03-03'
url: https://wiki.seeedstudio.com/pt-br/Cooler_Device/
---

<!-- ---
name: Water Cooling Kit for Raspberry Pi 5
category: 
bzurl: 
prodimagename:
surveyurl: 
sku: 110070128
tags:
--- -->

<div align="center"><img width={500} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/1.png" /></div>

<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="https://www.seeedstudio.com/High-Performance-Liquid-Cooler-for-Raspberry-Pi-5-p-5854.html" target="_blank">
            <strong><span><font color={'FFFFFF'} size={"4"}> Adquira agora 🖱️</font></span></strong>
    </a>
</div>

<br />
Este artigo irá orientá-lo na instalação do Water Cooling Kit para Raspberry Pi 5.

## A instalação do Water Cooling Kit possui as seguintes duas partes de instalação：

- [Parte1: Conexão da bomba de água com o Radiador](#jump1)

- [Parte2: Conexão do Radiador com o Raspberry Pi 5](#jump2)

### Parte1: Conexão da bomba de água com o Radiador {#jump1}

#### Passo1: Instalar a mangueira de silicone

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/2.jpg" /></div>

Conecte a bomba de água e o Radiador conforme mostrado.

#### Passo2: Adicionar fluido de arrefecimento à bomba de água

<div style={{textAlign:'center'}}><iframe width={560} height={315} src="https://www.youtube.com/embed/s06g69DCaT4" title="YouTube video player" frameBorder={0} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>

<br />

:::tip

1. Não ligue a bomba de água sem adicionar fluido de arrefecimento, caso contrário isso danificará a bomba de água.
2. É recomendado usar água purificada ou líquido de arrefecimento projetado especificamente para sistemas de water cooling de computadores.
3. Adicionar água da torneira pode causar obstruções no radiador.
4. Após encher o fluido de arrefecimento, o equipamento funcionará silenciosamente. Se o ruído estiver alto, por favor elimine as bolhas antes de adicionar o fluido de arrefecimento.

:::

### Parte2: Conexão do Radiador com o Raspberry Pi 5 {#jump2}

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/3.png" /></div>

Existem dois conjuntos de almofadas térmicas de silicone, sendo um conjunto de reserva.

Distinga as almofadas de dissipação de calor de silicone nº 3, nº 4 e nº 5:

A almofada de dissipação de calor de silicone nº 3 é mais grossa que a nº 4, e a almofada de dissipação de calor de silicone nº 5 é a menor em área.

:::note
  Há um filme plástico na almofada do dissipador de calor. Preste atenção em retirar o filme plástico ao usar.
:::

#### Passo1: Instalar o cooler inferior

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/4.jpg" /></div>
<br />
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/5.jpg" /></div>

#### Passo2: Instalar as 5 almofadas de borracha para dissipação de calor

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/6.jpg" /></div>
<br />
Preste atenção em retirar o filme plástico, caso contrário isso afetará o efeito de dissipação de calor.
<br />
Por favor, cole o dissipador de calor de silicone na posição correspondente em ordem.
<br />
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/7.jpg" /></div>
<br />
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/8.jpg" /></div>
<br />
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/9.jpg" /></div>
<br />
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/10.jpg" /></div>
<br />
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/11.jpg" /></div>
<br />

#### Passo3: Instalar parafusos e porcas

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/12.jpg" /></div>
<br />
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/13.jpg" /></div>
<br />
Primeiro, instale a coluna de cobre do Radiador.
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/14.jpg" /></div>
<br />
Em seguida, instale o Radiador na parte superior do Raspberry Pi 5.
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/15.jpg" /></div>
<br />
Por fim, os parafusos são instalados e apertados na placa traseira.
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/16.jpg" /></div>
<br />
Você ficará com essas peças sobressalentes.
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/Water_Cooling_Kit/images/17.jpg" /></div>
<br />
Depois de terminar, você pode ligar o Water Cooling Kit para resfriar seu Raspberry Pi 5.

## Suporte Técnico & Discussão do Produto

Obrigado por escolher nossos produtos! Estamos aqui para oferecer diferentes tipos de suporte para garantir que sua experiência com nossos produtos seja a mais tranquila possível. Oferecemos vários canais de comunicação para atender a diferentes preferências e necessidades.

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>
