---
description: Truques do Techbox
title: Truques do Techbox
keywords:
  - Kit com Cursos
image: https://files.seeedstudio.com/wiki/wiki-platform/S-tempor.png
slug: /Techbox_Tricks
sku: 110020006
last_update:
  date: 2/17/2023
  author: jianjing Huang
createdAt: '2023-02-17'
updatedAt: '2026-03-03'
url: https://wiki.seeedstudio.com/pt-br/Techbox_Tricks/
---

<!-- ---
name: Techbox Tricks
category: Discontinued
bzurl:
oldwikiname: Techbox_Tricks
prodimagename:
bzprodimageurl: https://www.research.net/r/Techbox_Tricks
sku:
tags:

--- -->

![](https://files.seeedstudio.com/wiki/Techbox_Tricks/img/110020006_p1.jpg)

TechBox é uma série de kits modulares de eletrônica para entusiastas de DIY, iniciantes em eletrônica e educadores.
Com o recurso de plug and make, o TechBox Tricks de nível iniciante coloca você em um bom começo em uma jornada de criação sem qualquer necessidade de conhecimento de programação.

## Recursos

* Minúsculo, poderoso, plug-and-make

* 84 combinações diferentes para brincar

* Tutoriais passo a passo proporcionam uma experiência de aprendizado e criação e são cheios de diversão

* Suporte e recursos completos da família Grove

## Lista de peças

![](https://files.seeedstudio.com/wiki/Techbox_Tricks/img/110020006_p2.jpg)

* Logic DC Jack (placa principal) *1

* Grove tricks *1

    1.[Light Switch](https://seeeddoc.github.io/Grove-Light_Sensor/)

    2.[Tilt Switch](https://seeeddoc.github.io/Grove-Tilt_Switch/)

    3.[Approach switch](https://seeeddoc.github.io/Grove-Line_Finder/)

    4.[Touch switch](https://seeeddoc.github.io/Grove-Touch_Sensor/)

    5.[Colorful LED](https://seeeddoc.github.io/Grove-LED_Socket_Kit/)

    6.[Buzzer](https://seeeddoc.github.io/Grove-Buzzer/)

* Motor DC *1

* Cabos Grove *2

* Cabo ramificado Grove *1

* Cabo de bateria *1

* Bateria de 9V *1

## Uso

### Passo 1: Preparações

Pegue 4 módulos de entrada e 2 módulos de saída com Grove tricks.

![](https://files.seeedstudio.com/wiki/Techbox_Tricks/img/Grove_tricks_depart.jpg)

### Passo 2: Princípios básicos

Há dois soquetes na placa principal (Logic DC Jack). Coloque um módulo de entrada à esquerda e um módulo de saída à direita. Agora basta ligar o interruptor para começar as criações!

#### Notas

* Inserir a ranhura dos cabos Grove diretamente voltada para a ranhura do soquete.

* O soquete à esquerda é para entradas e o da direita é para saída.

* Mudar para “Off” significa desligar. Mudar para “Not” significa que a placa principal agora é uma porta lógica NOT. Mudar para “And” significa ligar e, além disso, a placa principal será uma porta lógica AND somente se um cabo ramificado Grove for inserido no soquete esquerdo.

### Passo 3: Experimente agora

#### Projeto 1

![](https://files.seeedstudio.com/wiki/Techbox_Tricks/img/Application_1.jpg)

Somente se o interruptor estiver em "AND" e o Touch Switch for pressionado, o Buzzer soará.

#### Projeto 2

![](https://files.seeedstudio.com/wiki/Techbox_Tricks/img/Application_2.jpg)

Somente se o interruptor estiver em "AND" e o Touch Switch for pressionado, o Buzzer soará e o LED acenderá.

#### Projeto 3

![](https://files.seeedstudio.com/wiki/Techbox_Tricks/img/Application_3.jpg)

Somente se o interruptor estiver em "AND" e o Touch Switch for pressionado e o Light Switch estiver bem iluminado ao mesmo tempo, o Buzzer soará.

#### Projeto 4

![](https://files.seeedstudio.com/wiki/Techbox_Tricks/img/Application_4.jpg)

Somente se o interruptor estiver em "AND" e o Touch Switch for pressionado e o Light Switch estiver bem iluminado ao mesmo tempo, os módulos de saída serão ativados.

### Outros exemplos

Sinta-se à vontade para adicionar seu projeto à lista!

## Visualizador de esquemático online

<div className="altium-ecad-viewer" data-project-src="https://files.seeedstudio.com/wiki/Techbox_Tricks/res/Grove-Tricks_v1.0_EAGLE.zip" style={{borderRadius: '0px 0px 4px 4px', height: 500, borderStyle: 'solid', borderWidth: 1, borderColor: 'rgb(241, 241, 241)', overflow: 'hidden', maxWidth: 1280, maxHeight: 700, boxSizing: 'border-box'}}>
</div>

## Recursos

* **[PDF]**   [Grove - Tricks v1.0 位号.pdf](https://files.seeedstudio.com/wiki/Techbox_Tricks/res/Grove-Tricks_v1.0_%E4%BD%8D%E5%8F%B7.pdf)

* **[PDF]**  [Grove - Tricks v1.0_sch.pdf](https://files.seeedstudio.com/wiki/Techbox_Tricks/res/Grove-Tricks_v1.0_sch.pdf)

* **[Eagle]**   [File:Grove - Tricks v1.0 EAGLE.zip](https://files.seeedstudio.com/wiki/Techbox_Tricks/res/Grove-Tricks_v1.0_EAGLE.zip)
* **[PDF]**[Grove - Tricks v1.0 sch](https://files.seeedstudio.com/wiki/Techbox_Tricks/res/Grove%20-%20Tricks%20v1.0.pdf)

* **[EAGLE]**[Grove - Tricks v1.0 pdf](https://files.seeedstudio.com/wiki/Techbox_Tricks/res/Grove%20-%20Tricks%20v1.0.sch)

## Suporte técnico e discussão sobre o produto

Obrigado por escolher nossos produtos! Estamos aqui para fornecer diferentes formas de suporte para garantir que sua experiência com nossos produtos seja o mais tranquila possível. Oferecemos vários canais de comunicação para atender a diferentes preferências e necessidades.

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>
