...
url: https://wiki.seeedstudio.com/pt-br/Wifi_Shield_V1.2/
---
<!-- ---
...
Este shield WiFi apresenta o módulo TCP/IP RN171 para permitir que seu Arduino/Seeeduino se conecte a redes sem fio até 802.11b/g. O protocolo de comunicação padrão do shield com o Arduino é UART/Serial, e você pode selecionar quais pinos digitais (D0 a D7) usar para RX e TX com duas fileiras de jumpers que incorporamos. O shield também possui dois conectores Grove on-board para I2C e Serial para permitir que o shield seja usado com qualquer um de nossos dispositivos Grove. Uma antena on-board permite que o shield cubra uma faixa mais ampla e transmita sinais mais fortes. O módulo RN171 suporta os protocolos de comunicação TCP, UDP, FTP e HTTP para atender às necessidades da maioria dos projetos de rede sem fio e Internet das Coisas (IoT), por exemplo redes de casas inteligentes, controle de robôs, estações meteorológicas pessoais. O shield é muito bem documentado com nossos exemplos abaixo e seu [manual do usuário](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/WiFly-RN-UM.pdf).
...
<table>
  <tbody>
    ...
    <tr>
      <td>Placa compatível diretamente</td>
    ...
    <tr>
      <td>Autenticação WiFi segura</td>
      <td>WEP-128, WPA-PSK (TKIP), WPA2-PSK (AES)</td>
    </tr>
    <tr>
      <td>Aplicações de rede integradas</td>
      <td>Cliente DHCP, cliente DNS, ARP, ICMP ping, FTP, TELNET, HTTP, UDP, TCP</td>
    </tr>
    <tr>
      <td>Certificação</td>
      <td>RN171 : FCC, CE</td>
...
## Interface e Componentes do Shield

O shield WiFi é compatível com qualquer placa de desenvolvimento Arduino/Seeeduino, pois requer apenas dois pinos digitais de sua escolha entre D0-D7 para comunicação UART/serial. Para instalar, simplesmente empilhe o shield sobre a placa Arduino/Seeeduino.

### Visão geral

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Wifi_shield_v1.2_block1.png)

**1.Conexões Serial Peripheral Interface (SPI) (MOSI, SCK, MISO):** Esses pinos não estão conectados a nenhum dos pinos do Arduino, eles são independentes e o nível lógico de saída/entrada deles é 3,3 V. Eles podem ser usados para se comunicar com o Arduino via SPI, mas será necessário um conversor de nível lógico de 3,3 V entre esses pinos e os do Arduino. A taxa de dados no modo SPI pode chegar a 2 Mbps.
    **RES_Wifi:** O Wifi shield possui um "Botão de Reset" on-board para o módulo RN-171, você também pode reiniciar o RN-171 via software enviando o comando de reset. Adicionalmente, se você quiser conectar este pino ao pino digital 6 do Arduino, simplesmente solde o pad rotulado como "P5" no shield.

**2.RN171:** um módulo sem fio de consumo de energia super baixo com pilha TCP/IP integrada.

**3.Antena:** conector I.PEX.

**4.Seção de breakout do RN171:** O módulo RN171 possui seus próprios pinos de entrada analógica e GPIO, aos quais o shield fornece acesso por meio desta seção de breakout. Os pinos GPIO (IO3, IO7, IO8 e IO9) são tolerantes a 3,3 V enquanto os pinos de entrada analógica (S_0 e S_1) podem ler 0-400 mV (não exceder 1,2 V). O RN171 pode ser configurado para usar esses pinos por software ou eles podem ser conectados a outros pinos para usar outras funções do RN171, como modo adhoc. A tensão de VCC depende da alimentação do WiFi shield.

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Wifi_shield_v1.2_breakout.png)

**5.Área de seleção UART/Serial:** Duas fileiras de jumpers permitem que você selecione quais pinos RX e TX deseja usar para se comunicar com o Arduino.
...
O shield WiFi usa quaisquer dois pinos digitais de sua escolha entre D0 e D7 para se comunicar com o módulo WiFi RN171, porém tenha em mente que D0 e D1 são usados pelo Arduino para fins de programação e comunicação serial e usá-los pode interferir com essas duas funções.
...
O RN-171 é um módulo de rede sem fio TCP/IP completo e autônomo. Devido ao seu pequeno formato e consumo de energia extremamente baixo, o RN-171 é perfeito para aplicações móveis sem fio. Ele incorpora um rádio de 2,4 GHz, processador SPARC de 32 bits, pilha TCP/IP, relógio em tempo real, acelerador criptográfico, gerenciamento de energia e interfaces para sensores analógicos.

Na configuração mais simples o hardware requer apenas quatro conexões (PWR, TX, RX e GND) para criar uma conexão de dados WiFi sem fio. Adicionalmente, as entradas de sensor analógico do RN171 podem ser usadas como pinos de entrada analógica, sua faixa é de 0-400 mV (não exceder 1,2 V CC).

**Alimentação:** A tensão de operação do módulo RN-171 é tipicamente 3,3 VCC, portanto um regulador de tensão e um tradutor de nível lógico são projetados no WiFi shield. O regulador LD1117 no shield converte para 3,3 VCC, que alimenta o módulo RN171. Entretanto, devido ao esquema de julgamento automático da fonte de alimentação, o RN-171 pode ser alimentado tanto pelo pino 3V3 quanto pelo pino 5V. Mas a alimentação será de 5 V se forem fornecidos tanto 3,3 V quanto 5 V para a placa. Se estiver usando com uma placa Arduino/Seeeduino simplesmente empilhe o WiFi shield sobre a placa.
...
**GPIO_6 :** O pino GPIO6 do módulo WiFi RN171 está por padrão conectado somente ao LED rotulado D5 no WiFi shield. Este LED é usado para exibir o status da conexão ao Ponto de Acesso (AP). Porém, se você quiser conectar o GPIO6 ao pino digital 5 do Arduino, simplesmente solde o pad rotulado como "P6" no WiFi shield.

### Indicadores de Status por LED
...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td><strong>OFF</strong>: significa que o módulo não está associado a uma rede.<br/><strong>Solid ON</strong>: indica que está associado e o acesso à Internet está OK</td>
      <td>Conectado ao GPIO6 do módulo RN171</td>
    </tr>
    <tr>
      <td>D1</td>
      <td>LED vermelho. Indica o status da conexão TCP/IP.</td>
      <td><strong>Solid ON</strong>: conectado via TCP.<br/><strong>Fast Toggle (2 times/second)</strong>: Sem endereço IP ou o módulo está em modo de comando.<br/><strong>Slow Toggle (once/second)</strong>: Endereço IP está OK.</td>
      <td>Conectado ao GPIO4 do módulo RN171</td>
    </tr>
    <tr>
      <td>RST</td>
      <td>LED vermelho. Status de reset do módulo WiFi.</td>
      <td><strong>Solid ON</strong>: O botão de reset (WIFI_RST) está sendo pressionado.</td>
      <td>Conectado ao Reset do módulo RN171.</td>
    </tr>
    <tr>
      <td>PWR</td>
      <td>LED verde. Indica o status de energização do módulo WiFi.</td>
      <td><strong>Solid ON</strong>: O módulo/shield está energizado.</td>
...
Criamos uma biblioteca para ajudá-lo a fazer a interface com o shield; nesta seção mostraremos como configurar a biblioteca e apresentaremos algumas das funções.

### Configuração

1.[Baixe o código da biblioteca como um arquivo zip da página do Wifi Shield no github.](https://github.com/Seeed-Studio/WiFi_Shield)

2.Descompacte o arquivo baixado na sua pasta …/arduino/libraries/ .
...
* **Descrição:**
  * Usada para ingressar em um ponto de acesso WiFi
* **Sintaxe:**
  * join(const char *ssid, const char*phrase, int auth)
* **Parâmetros:**
  * **ssid:** O nome do ponto de acesso ao qual você deseja que o shield se conecte
  * **phrase:** A senha/frase do ponto de acesso ao qual você deseja que o shield se conecte
  * **auth:** O tipo de autenticação do ponto de acesso ao qual você deseja que o shield se conecte. Pode ser uma das seguintes constantes:
    * WIFLY_AUTH_OPEN
    * WIFLY_AUTH_WEP
    * WIFLY_AUTH_WPA1
    * WIFLY_AUTH_WPA1_2
    ...
    * WIFLY_AUTH_ADHOC
* **Retorno:**
  * **boolean:** true se a conexão ao ponto de acesso foi bem-sucedida, false caso contrário.
* **Exemplo:**
...
* **Descrição:**
  * Algumas das nossas funções (por exemplo, join(), reboot(), save()) atuam como encapsuladores para os comandos de texto listados no manual do usuário do módulo RN171. A função sendCommand() permite que você crie sua própria função encapsuladora se as nossas não atenderem às suas necessidades.
* **Sintaxe:**
...
* **Parâmetros:**
  ...
  * **timeout:** O tempo permitido antes de considerar a saída uma requisição/resposta inválida
* **Retorno:**
...
O módulo RN-171 do WiFi shield é configurado enviando-lhe os comandos encontrados em [seu datasheet](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/WiFly-RN-UM.pdf). Você pode escrever um sketch para enviar os comandos automaticamente, mas este é um ótimo exemplo pelo qual recomendamos que você passe, porque ele ensinará exatamente como o WiFi shield e o RN-171 funcionam.

Para prosseguir siga os passos abaixo, também criamos um vídeo se você preferir assistir

[Vídeo - Introdução ao WiFi Shield do Seeeduino.](https://www.youtube.com/watch?v=8dCrAaN16lE)
...
No sketch abaixo criamos um objeto UART para nos permitir enviar e receber dados do RN-171/WiFi Shield. Em seguida usamos esse objeto em conjunto com a biblioteca WiFly para enviar dados para o shield. O objeto Serial do Arduino é usado para imprimir os dados que recebemos do shield e para receber os comandos que queremos enviar ao shield via objeto WiFly/UART.
...
O módulo WiFly RN-171 no WiFi shield pode operar em dois modos: dados e comando. Quando está em modo de dados, o shield é capaz de receber e iniciar conexões. Quando está em modo de comando, podemos configurar o módulo usando os comandos listados em seu datasheet.

**Para entrar no modo de comando, siga estes passos:**

1.Abra o Monitor Serial do Arduino.
2.Defina o monitor serial para "No line ending", taxa de baud em 9600.
...
3.O módulo emitirá uma resposta, conforme descrito na tabela, para cada comando.

| Comandos | Descrição |
|---|---|
| scan | Este comando realiza uma varredura ativa de sondagem de pontos de acesso em todos os 13 canais. Quando você usa este comando, o módulo retorna o endereço MAC, intensidade do sinal, nome SSID e modo de segurança dos pontos de acesso que encontrar. |
| get ip | Este comando exibe as configurações de endereço IP e número da porta |

Para uma lista completa de comandos de configuração, consulte o [Guia de Referência](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/WiFly-RN-UM.pdf) do RN-171 a partir da página 11.

### Exemplo 2: Conectar a um Ponto de Acesso / Roteador de Internet

Neste exemplo mostraremos como conectar o WiFi shield a um ponto de acesso (seu roteador de internet) com e sem você digitar os comandos necessários:

#### Conectando Digitando Comandos

Esta seção ensinará como conectar o WiFi shield a um ponto de acesso usando comandos do datasheet do RN-171; ao passar por esta seção você saberá exatamente o que está acontecendo em segundo plano quando usa nossas bibliotecas WiFi para Arduino.

Faça o seguinte:

1.Carregue o código do Exemplo Um na sua placa Arduino

2.**Entre no modo de comando:**
    1.Defina o monitor serial para "No line ending", taxa de baud em 9600.
    2.Digite *$$* no Monitor Serial do Arduino e pressione Enter.

3.Defina o monitor serial para "Carriage return".

4.**Faça a varredura dos pontos de acesso disponíveis:**
    1.Digite *scan* e pressione Enter. A janela do Monitor Serial do Arduino exibirá uma lista de valores separados por vírgulas para cada ponto de acesso que o WiFi shield encontrou. Da esquerda para a direita, o terceiro valor é o modo de segurança, o último valor é o SSID. Este exemplo mostra um modo de segurança 4 com um nome de SSID MySSID: 01,01,-88,**04**,1104,1c,00,45:56:78:be:93:1f,**MySSID**

5.Na lista de pontos de acesso encontrados, encontre aquele que corresponde ao seu roteador de internet e anote o modo de segurança e o SSID, pois precisaremos desses dois valores para nos conectar a ele.
...
6.**Defina o modo de segurança no shield:**
    ...
    2.Os modos de segurança suportados pelo WiFi shield estão listados na Figura 1 abaixo.

7.**Defina a frase do ponto de acesso**
    1.Digite *set wlan phrase myPhrase*. Substitua *myPhrase* pela senha/chave de segurança do seu ponto de acesso. **Observação:** se o tipo de segurança do seu ponto de acesso for WEP use *key* em vez de *phrase* no comando acima.
    2.A frase do ponto de acesso (roteador de internet) é a senha que você usa para se conectar a ele a partir do seu PC. No Windows você pode encontrá-la como mostrado na imagem animada abaixo:

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/How_to_find_network_security_key_password.gif)

8.**Entrar no ponto de acesso**
  ...
  3.A palavra "Associated!" será exibida na janela do Monitor Serial do Arduino se for bem-sucedido.

Uma descrição dos comandos que você digitou nos passos acima está disponível na tabela abaixo. Uma descrição mais detalhada de cada comando pode ser encontrada no manual do usuário do RN171.

| Número | Comandos | Descrição |
|---|---|---|
| 1 | scan | Este comando realiza uma varredura ativa de sondagem de pontos de acesso em todos os 13 canais. Quando você usa este comando, o módulo retorna o endereço MAC, intensidade do sinal, nome SSID e modo de segurança dos pontos de acesso que encontrar. |
...
| 3 | set wlan phrase seeed-mkt | Diga ao WiFi shield sua frase secreta. |
| 4 | join SEEED-MKT | Diga ao WiFi shield para se conectar; "SEEED-MKT" é o nome do ponto de acesso que escolhemos para conectar. Depois de enviar o comando, o módulo deve agora se conectar e imprimir informações sobre a conexão. (Se a conexão falhar, tente enviar o comando novamente até funcionar) |
...
| 4 | WPA2-PSK |
| 5 | Não usado |
| 6 | Modo AD hoc (conectar a qualquer rede ad hoc) |
| 8 | WPE-64 |

#### Conectando Usando Nossas Bibliotecas WiFi

Agora que você sabe como conectar a um ponto de acesso digitando cada comando, é hora de usar as bibliotecas e exemplos que fornecemos.

Para ver o código necessário para conectar a um ponto de acesso vá para "File -> Examples -> Wifi_Shield -> wifi_test". Altere o código para usar seu próprio SSID (nome do ponto de acesso) e KEY (senha do seu ponto de acesso), depois carregue o sketch para a sua IDE do Arduino.
...
Com o sketch enviado para a sua placa Arduino, abra a janela do monitor serial. Se o shield tiver conseguido se conectar ao ponto de acesso, uma mensagem "OK" será exibida junto com as informações de conexão resultantes do comando "get everything". Se o shield não conseguir se conectar ao ponto de acesso, uma mensagem "Failed" será exibida.

#### Configurando o Shield para Conectar ao Ligar

O shield pode ser configurado para se conectar ao ligar; você só precisa fazer isso uma vez:
...
Agora o shield irá se conectar ao ponto de acesso automaticamente ao ligar.

Uma descrição do que cada comando faz pode ser encontrada no datasheet do RN-171 e na tabela abaixo.

| Número | Comandos | Descrição |
|---|---|-----|
| 1 | set wlan  | "ssid" é o nome do ponto de acesso ao qual você gostaria de se conectar automaticamente |
| 2 | set wlan join 1 | Isso diz ao módulo para tentar conectar automaticamente ao SSID armazenado na memória. |
...
| Número | Comandos | Descrição |
|---|---|---|
| 1 | set ip dhcp 0 | Desligar o DHCP. |
| 2 | set ip address  | Defina o endereço IP desejado. |

### Exemplo 3: Comunicando com a Rede (Introdução)

Este exemplo mostrará como um dispositivo como seu PC e/ou telefone pode se comunicar com o WiFi shield.

**Siga estes passos:**

1.Configure o módulo com os passos 1‑7 na seção do Exemplo 2 *Connecting By Typing Commands*

2.Defina a porta IP de escuta como "80" enviando o comando "set ip local 80"

3.Conecte/Associe seu shield a um ponto de acesso como mostrado no passo 8 na seção do Exemplo 2 *Connecting By Typing Commands*
...
6.Abra seu navegador e digite o endereço IP do seu shield na barra de URL do navegador e pressione Enter para acessá‑lo.
...
O navegador agora está aguardando dados; o módulo WiFi pode enviar valores de sensores, servir páginas web ou quaisquer outros dados diretamente de volta ao navegador! Neste caso, o navegador está aguardando uma página web. Se o módulo WiFi responder com uma página em formato HTML, o navegador irá exibí‑la. Os próximos exemplos ensinarão como fazer todas essas coisas legais.

### Exemplo 4: Usando o WiFi Shield como Servidor Web (Servindo Páginas Web a Partir do Shield)

Como você viu no Exemplo 3, um navegador de internet/web é capaz de se conectar ao WiFi shield. Uma vez que a conexão foi estabelecida (quando o navegador envia sua requisição HTTP), o WiFi shield pode então enviar código HTML de volta para o navegador exibir como uma página web. Neste exemplo você aprenderá o que é necessário para que o shield responda a um navegador web.

**Passo 1: Código Arduino**

Envie o código a seguir para sua placa Arduino, substituindo "myssid" e "mypassword" pelos valores do seu ponto de acesso, respectivamente:
...
**Passo 2: Obter o Endereço IP do Shield**

Abra a janela do monitor serial e aguarde até que a mensagem "Web server ready" seja exibida. O monitor serial também exibirá o endereço IP do WiFi shield:

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Wifi-shield-led-control-serial-comm-window.png)

**Passo 3: Visitando a página web**

Agora acesse esse endereço IP no seu navegador. A página web abaixo deve ser exibida; ela contém um link para o Yahoo! e o Google e um botão que ainda não faz nada:

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Wifi-shield-simple-webserver-page.png)

Quando a página web é acessada, a janela do monitor serial também exibirá a string "New Browser Request!", como mostrado abaixo:

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Wifi-shield-simple-webserver-arduino-serial-window-response.png)

**Nota:** No caso de alguns navegadores, como o Google Chrome, mesmo digitar a URL na barra já envia uma requisição de página web; isso ocorre porque esses navegadores tentam obter o título da página para a conveniência do usuário antes mesmo de ele/ela visitar a página.

### Exemplo 5: Controlando os Pinos Digitais do Arduino a Partir de uma Página Web (Alternando LEDs a Partir de uma Página Web)

Neste exemplo criaremos uma página web com três botões para controlar três pinos digitais diferentes no Arduino.

Para este tutorial siga os passos abaixo. Também criamos um vídeo em que explicamos o código em mais detalhes.
...
Envie o código a seguir para sua placa Arduino, mas substitua "mySSID" e "myPassword" pelo nome SSID e senha do seu ponto de acesso:
...
Acesse o endereço IP em um navegador web. Uma página com três botões, como a abaixo, deverá ser exibida. Clique nos botões para controlar os LEDs.

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Wifi-shield-led-control-webpage.png)

O Arduino também responderá de volta ao navegador web com o estado do pino, e o navegador exibirá isso em uma janela de alerta.

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Wifi-shield-led-control-arduino-response.png)

A janela do monitor serial também mostrará quando um navegador envia uma solicitação para visitar a página web ou controlar os pinos dos LEDs.
...
Nós criamos um app Android que pode alternar os pinos digitais do Arduino através do WiFi shield; para ver como o app funciona e aprender sobre o código, assista ao vídeo neste link:

**Software**

Baixe o projeto/código-fonte do Android Studio a partir deste link:
...
O módulo RN-171 no WiFi shield tem a capacidade de atuar como um cliente HTML (basicamente um navegador web baseado em texto), isso significa que podemos usar o shield para enviar e receber dados de um servidor web. Neste exemplo você aprenderá a usar o shield com uma API (Application Programming Interface) web que exibe os dados de clima de qualquer cidade (isto é, temperatura, umidade etc.).

O nome da API que usaremos é [OpenWeatherMap](http://openweathermap.org/api); quando você envia o nome de uma cidade e país para esse site, ele retorna uma string JSON com informações climáticas. Se você quiser exibir o clima de Londres, Reino Unido, por exemplo, você acessaria a seguinte URL [http://api.openweathermap.org/data/2.5/weather?q=London,uk](http://api.openweathermap.org/data/2.5/weather?q=London,uk), que retornaria uma string JSON como a seguinte, onde os dados de clima e outras informações estão embutidos.
...
Vamos em frente e recuperar a string JSON de clima para São Francisco, EUA. A URL que nosso WiFi shield precisa acessar é a seguinte (você pode testá-la em seu navegador web):
...
A Seção 13 do [manual do WiFly](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/WiFly-RN-UM.pdf) ensina diferentes maneiras de se conectar a um servidor web, mas em todos os casos precisamos especificar o nome do servidor (ou o endereço IP se o servidor não tiver um nome de domínio) e depois os dados que desejamos enviar.

Os comandos que precisamos enviar ao WiFi shield para receber a string JSON do servidor OpenWeatherMap são os seguintes:
...
Neste exemplo mostraremos como enviar informações do WiFi shield para um programa de terminal no PC. Faremos um console Arduino simples com menus que lhe darão a opção de ver o estado dos pinos digitais do Arduino e alterná-los.
...
**Passo 3: Obtenha o Endereço IP e a Porta do Shield**
...
**Passo 4: Configure o Terminal TCP e Conecte-se ao Shield**
...
Na aba "Port" do programa RealTerm, digite o endereço IP e a porta do seu shield, por exemplo 192.168.0.10:80, depois clique no botão "Open"; o menu principal, gravado no Arduino, deverá ser exibido no terminal.
...
Por exemplo, para alternar o estado do pino 13 insira "2" e pressione "Send ASCII"; em seguida, quando solicitado com "Enter the pin number you wish you toggle" digite "13" e clique em "Send ASCII". O Arduino deverá responder "Pin Toggled!" e voltar ao menu principal; agora insira "1" e pressione "Send ASCII" para ver o estado atual dos pinos.

![](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/img/Realterm-send-tab.png)

### Exemplo 9: WiFi Shield e Relay Shield

Agora que você sabe como enviar e receber informações para e do WiFi shield, pode ver como seria fácil controlar qualquer tipo de dispositivo via web.

Se você deseja controlar dispositivos de alta potência, como o abajur da sua mesa, um motor ou uma bomba d’água via página web ou aplicativo de celular, recomendamos o nosso [Relay Shield V2.0](https://seeeddoc.github.io/Relay_Shield_V2.0/).

O Relay Shield V2.0 utiliza os pinos 4, 5, 6 e 7, portanto é totalmente compatível com o código dos exemplos desta página.
...
Para obter o SSID do shield, faça o upload do código do Exemplo 1 para o seu Arduino e abra o monitor serial; o shield responderá com seu SSID como no exemplo abaixo, em que neste caso **WiFly-EZX-1b** é o SSID.

```cpp
AP mode as WiFly-EZX-1b on chan 1
```

Agora você já deve conseguir se conectar ao seu WiFi shield como um ponto de acesso; por exemplo, o SSID deverá estar visível na lista de redes WiFi disponíveis no seu PC.

Para saber mais sobre o modo adhoc, consulte o [WiFly RN User Manual](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/WiFly-RN-UM.pdf), seção 16 "Adhoc Networking Mode"

## Leitura Relacionada

* [O que é um Seeeduino](https://seeeddoc.github.io/Seeeduino_v3.0/)
...
* **[Eagle]**  [Arquivos Eagle do WiFi Shield v1.2](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/Wifi_shield_v1.2_eagle.zip)
...
* **[PDF]**  [Esquemático em PDF do Wifi Shield v1.2](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/Wifi_Shield_V1.2_schematic.pdf)
* **[Datasheet]**  [Datasheet do RN-171](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/WiFly-RN-171.pdf)
* **[Library]**  [Biblioteca do Wifi Shield](https://github.com/Seeed-Studio/WiFi_Shield)
* **[Manual]**  [Manual do Usuário do Módulo WiFi](https://files.seeedstudio.com/wiki/Wifi_shield_v1.2/res/WiFly-RN-UM.pdf) - Aqui você encontrará todos os comandos para o módulo RN-171 no shield.

## Suporte

* [FAQ sobre o Wifi Shield](http://support.seeedstudio.com/knowledgebase/articles/456737-wifi-shield)： *Clique aqui para ver se suas dúvidas já foram solucionadas.*
