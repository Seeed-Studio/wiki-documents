...
O dispositivo aplica um limite de tentativas falhas baseado em IP. Após repetidas senhas incorretas, `iLoginAttempts` é incrementado. Quando o limite é atingido, o dispositivo retorna `iStatus=-3` com um valor `sWaittime`. Você deve aguardar o fim do bloqueio antes de tentar novamente.
