...
| Method | Path | Purpose |
|---|---|---|
| GET | `/system/device-info` | Obter número de série do dispositivo, versão do firmware, modelo da placa do sensor e da placa base |
...
| `sSensorModel` | Modelo da placa do sensor |
...
`bFirst=true` indica que o dispositivo está no fluxo de primeiro login e a senha padrão deve ser alterada.
...
| `url` | Caminho de download; adicione o IP do dispositivo no início para obter uma URL completa |
