...
Este wiki explica como integrar a reCamera Pro com o Home Assistant (HA) para enviar automaticamente uma mensagem personalizada e um snapshot atual da câmera para o painel do Home Assistant quando um som específico for detectado. Usando o protocolo MQTT para comunicação bidirecional, quando a reCamera Pro detecta um som especificado (por exemplo, "help") com um nível de confiança acima de um limite, ela envia automaticamente uma mensagem de alerta e uma imagem ao vivo para o HA.
...
Baixe o [script install.py](https://drive.google.com/file/d/1nFBHJNkOUPqcAAUCYw43IhGhDWIYNhUX/view?usp=drive_link), execute-o e entre no diretório de configuração do HA para concluir toda a instalação automaticamente.
...
:::danger
`www/` e `custom_components/` são diretórios **irmãos** e ambos devem ser colocados diretamente no diretório raiz de configuração. Não coloque `www/` dentro de `custom_components/`, caso contrário a barra lateral não será exibida.
:::
...
Como este exemplo exige que a câmera tire fotos, você precisa parar primeiro o processo principal da reCamera Pro para liberar os recursos da câmera. Execute o seguinte comando:
...
- **MQTT_MESSAGE**: Mensagem a ser enviada quando disparar; você pode preencher com o conteúdo que deseja exibir no painel do HA.
...
1. Após iniciar o programa de detecção de som, a reCamera Pro monitora continuamente os sons ambientes.
2. Quando o som especificado (por exemplo, "help") for detectado com um nível de confiança acima do limite, ela captura automaticamente a cena atual.
