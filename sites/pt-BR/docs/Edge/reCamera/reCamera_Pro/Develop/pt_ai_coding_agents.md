---
description: Instale e use a reCamera Pro Development Skill para converter modelos ONNX, criar aplicativos RKNN nativos e desenvolver fluxos de trabalho de câmera, áudio, GStreamer e RTSP por meio de conversas em linguagem natural.
title: Desenvolver com agentes de codificação de IA
keywords:
  - reCamera
  - reCamera Pro
  - AI coding agent
...
A [reCamera Pro Development Skill](https://github.com/Seeed-Projects/recamera-pro-development-skill) empacota instruções específicas da reCamera Pro, scripts, referências técnicas e um template de aplicativo nativo em C++ para agentes de codificação de IA. Depois de instalá-la, você pode descrever um aplicativo em linguagem natural e deixar que o agente prepare o fluxo de trabalho de conversão de modelo e desenvolvimento nativo.

Por exemplo, você pode pedir ao agente para:

- Converter um modelo ONNX para RKNN para a NPU RV1126B
- Criar ou modificar um aplicativo nativo C/C++ com RKNN Runtime
...
A skill usa o seguinte contrato de destino ao desenvolver aplicativos de IA nativos:
...
| Formato do modelo | ONNX como entrada, RKNN como saída |
| RKNN Toolkit | RKNN-Toolkit2 2.3.2 |
| RKNN Runtime | 2.3.2 |
| Framework de câmera | GStreamer |
| Linguagem do aplicativo nativo | C/C++ |

## Instalar a Skill

### Selecione seu agente de codificação de IA

Clone o branch correspondente ao agente que você usa:

| Agente de codificação de IA | Branch | Diretório de skill em nível de usuário |
| --- | --- | --- |
...
:::tip
Mantenha o repositório clonado. Para atualizar a skill mais tarde, faça pull das alterações mais recentes do mesmo branch do agente e siga o README e as instruções de instalação desse branch.
:::
...
:::warning
Não adivinhe o pré-processamento, a ordem de cores, a normalização ou a semântica de saída de um modelo. Forneça a origem do modelo original e imagens de calibração representativas ao solicitar quantização INT8.
:::

### Criar um aplicativo nativo de câmera
...
A skill inclui um template mínimo de aplicativo com RKNN Runtime e GStreamer. O agente adapta o pré-processamento e o pós-processamento ao modelo em vez de tratar o template como um detector completo.
...
4. Comparar a saída RKNN com o modelo ONNX de origem antes de integrá-lo em um aplicativo.
5. Criar ou adaptar o aplicativo nativo em C/C++ e compilá-lo cruzado contra um sysroot compatível com a reCamera Pro.
...
- Use a API C do RKNN Runtime para aplicativos nativos em C/C++. `rknn-toolkit-lite2` não é usado neste fluxo de trabalho.
- Não faça link de bibliotecas x86_64 do computador de desenvolvimento em um aplicativo de destino aarch64.
- Não alimente bytes de câmera NV12 diretamente em um modelo RGB ou BGR. O pipeline ou aplicativo deve realizar o pré-processamento exato exigido pelo modelo.
...
- O aplicativo faz link contra um RKNN Runtime compatível com a placa e usa o runtime existente do dispositivo; ele não deve substituir o RKNN Runtime instalado na reCamera Pro.
...
| Os frames da câmera têm cores ou resultados de inferência incorretos | A conversão NV12-para-RGB/BGR ou a normalização não correspondem ao modelo | Verifique novamente o contrato do modelo e torne o pré-processamento explícito |
