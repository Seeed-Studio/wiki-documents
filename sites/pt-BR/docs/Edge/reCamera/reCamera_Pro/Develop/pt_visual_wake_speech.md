---
description: Aprenda como implantar um sistema bilíngue de reconhecimento de fala em streaming, acionado por olhar e totalmente em dispositivo, no reCamera Pro, com visualizações de status no terminal, na web e via RTSP.
title: Despertar Visual e Reconhecimento de Fala Offline
keywords:
  - reCamera Pro
  - RV1126B
  - RKNN
...
# Construa um Sistema de Despertar Visual e Reconhecimento de Fala Offline com reCamera Pro
...
Este projeto implementa um fluxo de interação natural e totalmente em dispositivo para o reCamera Pro (RV1126B). A câmera detecta um rosto e estima a pose da cabeça. Somente depois que uma pessoa olha para a câmera de forma contínua o aplicativo abre o microfone e inicia o reconhecimento de fala em streaming. Em seguida, ele entra em um breve período de resfriamento para evitar disparos repetidos.

Ele é adequado para assistentes de voz, quiosques de exposição e pontos de entrada de voz na borda com foco em privacidade. Visão, avaliação de pose, extração de características de fala e inferência Zipformer são executadas localmente. O painel web não requer serviço em nuvem, processo Node.js, CDN ou fonte externa.
...
O repositório contém modelos de visão, modelos Zipformer, o painel web e o script de compilação cruzada. Não misture versões arbitrárias do RKNN Runtime: este projeto usa **RKNN 2.3.2** e tem como alvo `rv1126b`.

## Compilar e implantar

### 1. Configurar o SDK do reCamera Pro
...
Copie o **conteúdo** do diretório de implantação para um diretório no reCamera Pro. Para um dispositivo em `192.168.42.1`:
...
| `--fps N` | `30` | Taxa de quadros de câmera solicitada; a taxa de processamento depende do tempo de inferência. |
...
| `--stt-chunk-ms N` | `160` | Duração de cada bloco de áudio passado para STT, em milissegundos. |
...
| `--debug` | Desativado | Imprimir logs de inicialização, pose por quadro, nível de áudio, RTSP, contração de tensores e desempenho de STT. Não pode ser combinado com `--dashboard`. |
...
## Visualizar o stream de vídeo anotado
...
1. Use `--debug` para observar yaw/pitch enquanto estiver voltado para a câmera e, em seguida, ajuste os limites de entrada.
...
| O despertar visual nunca é acionado | Câmera incorreta, limites muito rígidos ou rosto pequeno | Use `--debug` e a sobreposição RTSP para verificar `/dev/video13`, pontos de referência e yaw/pitch. |
