...
Há várias interfaces no módulo Wifi Halow, que são:
...
- ***Keep-Alive***: Interruptor para a função Keep-Alive para manter a conexão com o servidor (o intervalo de tempo do Keep-Alive pode ser ajustado nas configurações para um mínimo de 1s) para evitar desconexões
...
Obrigado por escolher nossos produtos! Estamos aqui para fornecer diferentes tipos de suporte para garantir que sua experiência ao usar nossos produtos seja a mais tranquila possível. Oferecemos vários canais de comunicação para atender a diferentes preferências e necessidades.
