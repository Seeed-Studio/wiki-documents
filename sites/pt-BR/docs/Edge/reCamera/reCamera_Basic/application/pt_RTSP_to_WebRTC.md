...
url: https://wiki.seeedstudio.com/pt-br/reCamera/reCamera_Basic/application/rtsp_to_webrtc_with_recamera_bak/
---
...
- **Fornecer um canal de transporte para visualização de resultados de inferência de IA**: Após a reCamera executar inferência de IA na borda, envie os resultados de inferência junto com o vídeo, e o PC poderá exibir em tempo real, no navegador, os resultados da análise de IA, adequado para construir aplicações de inspeção remota, segurança inteligente e outras.
...
| Recepção e Decodificação de Vídeo | Servidor PC | OpenCV + FFmpeg | Recebe o stream RTSP e decodifica para frames brutos |
...
| `mqtt_broker.py` | MQTT Broker 3.1.1 embutido (implementação pura em asyncio) e cliente de assinatura, recebe resultados de inferência YOLO enviados a partir do Node-RED da reCamera |
| `detection_store.py` | Módulo de armazenamento de resultados de inferência, armazena em cache os dados mais recentes de caixas de detecção e rótulos para renderização sobre os frames de vídeo |
| `video_sources.py` | Camada de abstração de protocolo de vídeo |
| `index.html` | Página do player frontend, fornece seleção de protocolo e funcionalidade de reprodução de vídeo WebRTC |
...
Após o início do serviço, você verá o seguinte log de saída:
...
3. **Recepção de vídeo e fusão de resultados de inferência**
...
- Se aparecerem erros `non-existing PPS` no log do servidor, isso ocorre porque um quadro-chave ainda não foi recebido, o que é normal. A recuperação será automática após a chegada do quadro-chave (aproximadamente 1 segundo).
