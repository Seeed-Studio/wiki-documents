...
url: https://wiki.seeedstudio.com/pt-br/reCamera/reCamera_Basic/application/sound_source_localization_of_recamera_gimbal_bak/
---
...
Neste demo, conectamos o reSpeaker USB Mic Array da Seeed Studio ao nosso reCamera Gimbal com tecnologia RISC-V. Ao utilizar Localização de Fonte Sonora (DOA), o sistema permite que os motores brushless do gimbal girem automaticamente e fiquem voltados para a direção do locutor em tempo real. É uma exploração super divertida de como nossos dispositivos multimodais de borda podem interagir perfeitamente!  
...
## 2. Obtenha o código-fonte e configure o ambiente de trabalho Python
...
Este demo é apenas o começo. Incentivamos muito que todos realizem desenvolvimento secundário com base nele e ampliem mais formas de uso avançadas! Por exemplo:

-** Acionamento por ligação **: Você pode modificar o script Node-RED ou Python para fazer o reCamera Gimbal começar a gravar ** ou *tirar fotos* * imediatamente após girar para a posição da fonte sonora.

-**Segurança inteligente**: Combinado com Home Assistant, pode rastrear e gravar automaticamente quando ocorrer ruído anormal no ambiente.

-**Confirmação dupla visual e auditiva**: Combine detecção de alvo YOLO11n com rastreamento auditivo (por exemplo: ouvir um som -> virar para ver -> reconhecimento visual para confirmar se é uma "pessoa").
...
Substitua "192.168.31.198" no código reSpeaker.py pelo endereço IP real do reCamera Gimbal após conectá-lo ao WiFi!
...
3. **Calibração de orientação física**: A orientação '0 de' do array de microfones do reSpeaker pode não ser exatamente a mesma que o '0 de' padrão do reCamera voltado para a frente. Se você perceber que o pan-tilt sempre desvia de um ângulo fixo após girar (por exemplo, sempre desvia 90 graus), você pode clicar duas vezes diretamente no novo nó de função 'Calculate & Throttle' recém-criado no Node-RED, remover o comentário do código relevante e modificar o cálculo de compensação: 'targetYaw = (targetYaw +90)% 360;'.

4. - - Design anti-tremor **: Como o código Python atualiza e envia dados a cada 0,1 segundo, se todos esses sinais de alta frequência forem enviados ao controlador do motor (barramento CAN), é fácil fazer com que o barramento do dispositivo sobrecarregue e trave. Portanto, escrevi a lógica anti-tremor no nó de função do Node-RED (o gerador só será acionado quando o ângulo mudar '>5 °' ou o intervalo de tempo for '>1 second').
