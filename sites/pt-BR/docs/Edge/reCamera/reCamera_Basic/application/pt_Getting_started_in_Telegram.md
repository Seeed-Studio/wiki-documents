---
description: Fornecer um tutorial para integrar a reCamera com o Telegram. Depende do n8n.
...
url: https://wiki.seeedstudio.com/pt-br/reCamera/reCamera_Basic/application/getting_started_in_Telegram_work_with_recamera_bak/
---
...
Entre vários cenários de aplicação de IoT, a troca de informações e o processamento automático entre dispositivos geralmente demonstram mais o valor do sistema do que apenas a capacidade de reconhecimento. A reCamera oferece capacidades estáveis de reconhecimento visual de IA local, enquanto o n8n fornece uma plataforma flexível de orquestração visual para processamento de dados e automação de processos. Quando os dois são combinados, as informações de detecção deixam de ficar limitadas aos dispositivos locais e passam a se tornar uma fonte de eventos que pode ser ainda mais aproveitada.

Este tutorial apresenta um método de integração simples e eficiente: os resultados de reconhecimento da reCamera são enviados em forma de texto para o n8n, e então o n8n os envia automaticamente para o Bot pessoal do Telegram criado pelo usuário via API. Por meio desse elo, qualquer evento de reconhecimento pode ser imediatamente enviado para o dispositivo móvel, construindo assim um sistema de notificações leve que abrange vários dispositivos e plataformas. Todo o processo não requer a escrita de código complexo; basta concluir a configuração necessária dos nós para atingir rapidamente o objetivo.
...
Na primeira execução, a imagem do n8n será baixada automaticamente, com alguns centenas de MB, levando um pouco de tempo
...
Agora entramos na interface de configuração. Aqui precisamos modificar dois lugares. Primeiro, selecione "HTTP Method" como "POST". Em seguida, modifique "Path" para qualquer nome. Aqui vou usar "recamera_detect" como exemplo.
...
Clique no sinal de mais – na caixa de pesquisa digite "code" – clique no primeiro nó "Code"
...
Depois de clicar para entrar, clique em "Start" no canto superior direito. Neste momento, será retornada uma mensagem contendo o seu ID. Lembre-se desse ID, pois ele será usado mais tarde.

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/Telegram_12.png" /></div>

### (3) continuar a configurar o n8n e adicionar um terceiro nó

Clique no sinal de mais – na caixa de pesquisa digite "telegram" – clique no primeiro nó "Telegram"

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/Telegram_13.png" /></div>

Depois de clicar, continue digitando "message" e selecione "Send a text message"

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/Telegram_14.png" /></div>

Após entrar no nó, selecione "Create new credentila"
...
Depois de clicar em "Save", o sistema verificará se a API existe. Se estiver tudo correto, aparecerá a mensagem "Connection tested successfully". Neste momento, clique em Exit no canto superior direito.
...
Primeiro, abra a interface do n8n na máquina virtual, clique duas vezes no nó "Webhook" que você configurou anteriormente, entre na interface de configuração do nó Webhook e clique em "Production URL" para copiar o seguinte endereço "POST"

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/Telegram_20.png" /></div>

As etapas de configuração anteriores são as mesmas do [Home Assistant](https://wiki.seeedstudio.com/pt-br/getting_started_for_home_assistant_with_recamera/), apenas ao configurar o nó "http request", o endereço de URL precisa ser modificado, ou seja, o endereço POST que você acabou de copiar na máquina virtual, e depois modificar o localhost para o endereço IP da sua máquina virtual
...
## 5. Ativar Active e entrar na interface Executions para visualizar a mensagem

Clique no interruptor "Inactive" no canto superior direito para deixá-lo verde como Active e, em seguida, clique em "Executions"
...
Em seguida, verifique o seu Telegram, deve haver muitos avisos de mensagens; neste momento, você terá enviado com sucesso a mensagem da reCamera para o Telegram!!!

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/Telegram_24.png" /></div>

Se você quiser visualizar o conteúdo da detecção de um determinado horário, pode clicar duas vezes nesse carimbo de data e hora e, em seguida, clicar duas vezes em "Code in JavaScript" para visualizar o OUTPUT à direita.
