---
description: Fornecer um tutorial para integrar a reCamera com o Home Assistant. O Home Assistant está instalado no reComputer.
title: Introdução ao Home Assistant
keywords:
  - reCamera
  - Home Assistant
  - reComputer
...
url: https://wiki.seeedstudio.com/pt-br/reCamera/reCamera_Basic/application/getting_started_for_home_assistant_with_recamera_bak/
---

# Introdução ao Home Assistant com reCamera

## Introdução

Home Assistant é um sistema de plataforma de casa inteligente de código aberto, e o usuário pode facilmente conectar seus próprios dispositivos de Internet das Coisas ao sistema para gerenciamento unificado. No entanto, o principal diferencial do Home Assistant não é apenas realizar a visualização de vários dispositivos de detecção, mas sim realizar a vinculação automática de dispositivos multiplataforma; somente uma plataforma de casa inteligente assim é suficientemente dinâmica.
...
(esta etapa pode levar algum tempo, a velocidade depende da sua rede)
...
Após o Docker ser iniciado, aguarde de 1 a 2 minutos e então acesse pelo navegador:
...
Você pode visualizá-las através dos seguintes comandos:
...
Se a interface não puder ser aberta continuamente, consulte a seção [Factory Reset](https://wiki.seeedstudio.com/pt-br/recamera_getting_started/#factory-reset) para redefinir o dispositivo. Após a redefinição, visite 192.168.42.1 novamente. Após a redefinição, você será solicitado a alterar a senha.
...
Quatro novos nós foram adicionados: "debug1", "debug2", "function1" e "HTTP request".  
debug1 e debug2 são usados para observar as informações de saída, opcionais ou não; function1 é usado para processar dados; o nó HTTP request é usado para enviar dados ao Home Assistant
...
Você pode saber quais tipos de objetos podem ser detectados verificando o nó do modelo.
...
Depois de concluir a configuração, clique no botão "Deploy" no canto superior direito. Quando a implantação for bem-sucedida, haverá um aviso.

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/workspace_ha_9.png" /></div>

Aqui, você precisa realizar mais uma etapa, que é conectar o WiFi à reCamera. Ela deve estar na mesma rede que a que você configurou para o reComputer (observe que a versão POE não possui funcionalidade WiFi e requer o uso de um switch com função POE para se conectar no mesmo segmento de rede).  
...
## Visualizar a interface de visão geral do Home Assistant

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reCamera/HA.png" /></div>

Se ainda não houver nenhum efeito observado, primeiro verifique se o link não está conectado corretamente. Ou seja, verifique se o Webhook está funcionando corretamente:  
...
Se for bem-sucedido, a notificação do Home Assistant mostrará:
