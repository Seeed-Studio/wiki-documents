...
url: https://wiki.seeedstudio.com/pt-br/reCamera/reCamera_Basic/getting_start/recamera_2002_series_quick_start_guide
---
...
Comece desembalando seu reCamera. Dentro da caixa, você deverá encontrar:
...
Conecte o cabo USB do reCamera ao seu PC, então você pode acessar ```192.168.42.1``` para visualizar a página de carregamento do reCamera.
...
**Notas para configuração de rede WiFi:**

1. **Garanta a mesma rede**: Certifique-se de que o reCamera e o seu computador estejam conectados à **mesma rede WiFi**. Isso garante que o desenvolvimento e a depuração subsequentes possam prosseguir normalmente.

2. **Desative o VPN no computador**: Se você tiver um VPN ativado no seu computador, por favor **desative o VPN** antes de configurar o WiFi. VPNs podem causar problemas de conexão de rede e impedir que você acesse o reCamera normalmente.

3. **Como alternar a conexão WiFi**:
   ...
   - Selecione na lista a rede WiFi (SSID) à qual você deseja se conectar
...
Uma vez conectado, você pode encontrar o IP do reCamera no backend do seu roteador e então acessar o site e a plataforma Node-RED.

### Método 3: Modo AP (disponível apenas nas versões 2002w)

**Configuração de rede sem fio em modo AP**

Ao ligar, o reCamera ativará o Access Point (AP) para que você altere a preferência de rede. Abra a lista de Wi-Fi do seu telefone ou laptop; você deverá ver um nome de Wi-Fi chamado `reCamera_******`.
...
Conecte-se a este AP, e um site deverá aparecer automaticamente para você. Se não, você sempre pode digitar **192.168.16.1** no seu navegador para acessar a página. Em seguida, você pode selecionar a qual WiFi deseja que o reCamera se conecte.
Se você tiver mais de um reCamera, pode consultar o [Device Management](https://wiki.seeedstudio.com/pt-br/recamera_getting_started/#multi-devices-management) para descobrir qual é qual.
...
- **Área de trabalho**: `ip_address/#/workspace` (para versão do OS 0.1.4 e superior)
...
**Verifique a versão do sistema operacional do seu dispositivo**. Há uma grande atualização entre as versões 0.1.3 e 0.1.4. Para uma experiência de uso mais suave, recomendamos atualizar para 0.1.5 ou superior. Vá para ```Sidebar -> System -> Software Update``` para verificar a versão do software e aplicar e instalar a versão mais recente do sistema operacional. Para instruções de atualização, consulte o [OTA upgrade guideline](https://wiki.seeedstudio.com/pt-br/recamera_getting_started/#ota-upgrade-from-013-to-latest-version).

### Atualização OTA da 0.1.3 para a versão mais recente

Se você recebeu o primeiro lote de reCamera, pode atualizar o sistema operacional para a versão mais recente seguindo estas instruções.
...
**Passo 5**: Você entrará na página de carregamento para alterar a senha. Agora você pode experimentar a versão mais recente do reCamera com o painel do Node-RED seguindo [esta instrução de download](https://wiki.seeedstudio.com/pt-br/recamera_getting_started/#access-recamera-preview-dashboard).
...
Ao integrar o dispositivo com o Node-RED, fornecemos um exemplo de interface de painel integrada onde você pode pré-visualizar o fluxo de vídeo e alterar os modelos desejados. Você pode livremente ajustar e integrar com sua própria aplicação.

Se a versão do seu sistema operacional for 0.1.5 ou superior, o painel de pré-visualização será exibido assim que você fizer login.

Se você atualizar o sistema operacional por conta própria, poderá baixar o dashboard jsonflow a partir do Sensecraft AI
...
**Passo 2**. Navegue até a plataforma Sensecraft AI, selecione o primeiro Dashboard Demo oficial e clique em ```Clone```. É necessário login para este site.
...
**Passo 4**. Em seguida, o espaço de trabalho do Node-RED com o fluxo pré-construído será exibido.

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/dashboarddownload4.png" /></div>

**Passo 5**. Clique em ```Deploy``` e verifique se o fluxo já foi iniciado, depois clique em ```dashboard```, então o painel será exibido.

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/dashboarddownload5.png" /></div>

Certifique-se de verificar se o fluxo foi iniciado ou não. Às vezes, importar um fluxo não iniciará o fluxo automaticamente, deixando o dashboard vazio. Clique em `Start` no menu suspenso.
...
Alterando os `function nodes` no fluxo, você pode adicionar mais objetos a serem contados ou alterar completamente essa função para outras aplicações de visão computacional.
...
Sinta-se à vontade para ajustar e brincar com o fluxo da maneira que quiser. Apenas tenha cuidado para **não** colocar muitos `debug nodes` ou `preview nodes` após os model nodes, pois imprimir vários resultados do modelo de IA consumirá CPU e poderá fazer o dispositivo travar.
