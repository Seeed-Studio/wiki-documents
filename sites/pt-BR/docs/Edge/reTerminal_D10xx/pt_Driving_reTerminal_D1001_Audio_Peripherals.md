...
| **I2S_BCK** | GPIO32 | **Clock de bit**: Sincroniza cada bit individual do fluxo de dados de áudio. |
...
| **EN_PA**   | **EXP_GPO11** | **Habilitação do PA**: Mapeado para o pino **P13** no PCA9535. Definir este pino em nível ALTO habilita o amplificador de potência externo. |
...
<div class="github_container" style={{textAlign: 'center'}}>
    ...
    <strong><span><font color={'FFFFFF'} size={"4"}>Baixar o código de exemplo</font></span></strong> <svg aria-hidden="true" focusable="false" role="img" className="mr-2" viewBox="-3 10 9 1" width={16} height={16} fill="currentColor" style={{textAlign: 'center', display: 'inline-block', userSelect: 'none', verticalAlign: 'text-bottom', overflow: 'visible'}}><path d="M8 0c4.42 0 8 3.58 8 8a8.013 8.013 0 0 1-5.45 7.59c-.4.08-.55-.17-.55-.38 0-.27.01-1.13.01-2.2 0-.75-.25-1.23-.54-1.48 1.78-.2 3.65-.88 3.65-3.95 0-.88-.31-1.59-.82-2.15.08-.2.36-1.02-.08-2.12 0 0-.67-.22-2.2.82-.64-.18-1.32-.27-2-.27-.68 0-1.36.09-2 .27-1.53-1.03-2.2-.82-2.2-.82-.44 1.1-.16 1.92-.08 2.12-.51.56-.82 1.28-.82 2.15 0 3.06 1.86 3.75 3.64 3.95-.23.2-.44.55-.51 1.07-.46.21-1.61.55-2.33-.66-.15-.24-.6-.83-1.23-.82-.67.01-.27.38.01.53.34.19.73.9.82 1.13.16.45.68 1.31 2.69.94 0 .67.01 1.3.01 1.49 0 .21-.15.45-.55.38A7.995 7.995 0 0 1 0 8c0-4.42 3.58-8 8-8Z" /></svg>
    </a>
</div><br />

:::tip
Navegue até o diretório `driver_examples/01_I2SCodec` dentro do repositório para encontrar o código-fonte específico e os arquivos de projeto para este exemplo de áudio.
:::
...
O amplificador de potência (PA) externo é controlado via o expansor PCA9535. Habilitar o PA é crucial porque, sem isso, nenhum som audível chegará aos alto-falantes, mesmo que o codec esteja funcionando corretamente.
...
O ES8311 deve ser configurado para corresponder às definições de I2S (taxa de amostragem, largura de dados) definidas no ESP32-P4. Isso é feito via barramento I2C. Antes de compilar o projeto, você pode personalizar o comportamento do áudio modificando as macros em `main/example_config.h`:
...
#### Etapa 4. Entrada principal e criação de tarefa

O aplicativo principal inicializa os periféricos e então entrega a lógica de reprodução a uma tarefa dedicada do FreeRTOS.
...
**DMA (Direct Memory Access)** permite que o periférico I2S busque dados diretamente da memória sem intervenção da CPU. O **pré-carregamento** do buffer DMA é uma técnica crítica para evitar o ruído de “estalo” que ocorre quando o hardware I2S é iniciado com um buffer vazio, causando uma mudança súbita de offset DC.
...
- **Verifique**: Confirme se o amplificador de potência (PA) está habilitado. O sinal `EN_PA` é controlado via o expansor de IO PCA9535 no pino **P13**. Certifique‑se de que `pca9535_init()` é chamado e configura corretamente o registrador de saída (Porta 1, Bit 3).
- **Verifique**: Confirme as conexões de I2S e certifique‑se de que a função `i2s_channel_enable()` é chamada para o canal TX.

### P2: Áudio distorcido ou com ruídos de estalo
- **Verifique**: Certifique‑se de que a configuração de clock do I2S (MCLK, BCLK, WS) corresponde à taxa de amostragem do seu arquivo de áudio. Uma incompatibilidade em `EXAMPLE_SAMPLE_RATE` pode causar problemas de tom e velocidade.
- **Verifique**: Confirme que o pré-carregamento de DMA está implementado conforme mostrado no **Passo 5**. O pré-carregamento evita o som de "estalo" causado por iniciar um canal com um buffer vazio.

### P3: Falha de comunicação I2C com ES8311 ou PCA9535
- **Verifique**: Confirme as conexões I2C SDA (GPIO20) e SCL (GPIO21). Certifique‑se de que nenhum outro periférico esteja em conflito com esses pinos.
- **Verifique**: Confirme se os endereços I2C estão corretos: **0x20** para PCA9535 e **0x18** para ES8311.
