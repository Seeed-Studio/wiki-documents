...
A correção normalmente exige duas partes: instalar os módulos de kernel que correspondem ao kernel `6.8.12-1021-tegra` do JetPack 7.2 e descompactar os arquivos de firmware correspondentes em `/lib/firmware`.

:::note
Este guia é destinado a sistemas JetPack 7.2 usando o kernel `6.8.12-1021-tegra`. Os módulos de driver de reparo são compilados para esse kernel. Se `uname -r` mostrar um kernel diferente, use o pacote de driver para essa versão exata de kernel.
:::
...
- `dmesg` relata erros de carregamento de firmware `iwlwifi` ou `rtw89`.
...
Para AX210, descompacte os arquivos de firmware e PNVM `ty-a0-gf-a0`:
...
| `modprobe iwlwifi` relata `Invalid argument` | Verifique se `rfkill.ko` existe com `find /lib/modules/$(uname -r) -name "rfkill.ko"`. Se estiver ausente, use a imagem BSP correta ou reconstrua os módulos do kernel. |
...
- Realtek RTL8852BE usa a pilha de driver `rtw89` e firmware em `/lib/firmware/rtw89/`.
- Esses pacotes de reparo são construídos para o kernel `6.8.12-1021-tegra` do JetPack 7.2.
