---
description: Otimize a memória no JetPack 7.2 por meio de medição, habilidades NVIDIA Jetson, ajustes de BSP sem monitor ou sem câmera e configurações de inferência de LLM eficientes em memória.
...
- o [workflow NVIDIA Skills](/pt-br/rapid_prototyping_on_jetson_with_nvidia_skills_bk/) para diagnóstico do dispositivo, auditoria de memória e implantação sem monitor;
- o [guia TensorRT Edge-LLM](/pt-br/deploy_tensorrt_edge_llm_on_jetpack7.2/) para inferência em FP16, INT8 e INT4 no JetPack 7.2.
...
| Configuração de serviços | Alvo sem monitor, parar servidores de modelo duplicados, desabilitar serviços de usuário não utilizados | Médio | Normalmente reboot |
...
| `no-camera` | Sem CSI, GMSL ou outro pipeline de câmera | RCE, VI, ISP, NVCSI e carveouts de firmware correspondentes |
...
| INT4 | Menor memória de pesos entre os caminhos suportados | Modelos grandes ou implantações com múltiplos serviços e DRAM limitada |
...
| --- | --- |
| `MemAvailable` em idle | Mede a sobrecarga do sistema e dos serviços |
...
| Tempo até o primeiro token | Detecta regressões causadas por swap ou workspaces restritos |
...
O [JetPack 7.2 Deep Dive](/pt-br/jetpack72_deep_dive_bk/) registrou a memória após o carregamento de um modelo de 27B caindo de aproximadamente **24,6 GB** no JetPack 6.2 para **14,7 GB** no JetPack 7.2 na comparação da Seeed. Trate esse resultado como uma referência específica de carga de trabalho, não como uma redução garantida para todo modelo.
