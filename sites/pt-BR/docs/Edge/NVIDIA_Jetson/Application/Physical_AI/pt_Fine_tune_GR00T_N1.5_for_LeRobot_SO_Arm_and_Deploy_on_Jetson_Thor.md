...
title: Ajustar Fino o Isaac GR00T N1.5 para o Braço LeRobot SO-101 e Implantar no Jetson Thor
keywords:
  - Thor
  - Robótica
  - Seeed
...
url: https://wiki.seeedstudio.com/pt-br/fine_tune_gr00t_n1.5_for_lerobot_so_arm_and_deploy_on_jetson_thor_bk/
---

# Ajustar Fino o GR00T N1.5 para o Braço LeRobot SO-101 e Implantar no Jetson AGX Thor

## Introdução

Este wiki explica como **ajustar fino o NVIDIA Isaac GR00T N1.5** para o **braço LeRobot SO-101** e implantá-lo no **NVIDIA Jetson Thor**. Ele abrange:
...
O **Kit de Desenvolvimento NVIDIA® Jetson AGX Thor™** é uma potência para IA física e robótica humanoide. Ele é construído em torno da GPU NVIDIA Blackwell e inclui **128 GB de memória de alta velocidade**, oferecendo **até 2.070 FP4 TFLOPS** de computação de IA enquanto opera dentro de um envelope de energia de **40–130 W** (uso comum até 130 W).

Você pode obter o Kit de Desenvolvimento Jetson AGX Thor na Seeed Studio aqui: [Seeed – NVIDIA Jetson AGX Thor™ Developer Kit](https://www.seeedstudio.com/NVIDIA-Jetson-AGX-Thor-Developer-Kit-p-9965.html)
<div align="center">
...
Em **10 de setembro de 2025**, a versão mais recente de imagem de sistema disponível para o Thor é a **38.2**. Se você precisar regravar o sistema do Thor, siga as instruções desta seção.
Itens Necessários para a Gravação:

- Um dispositivo host com mais de 25 GB de espaço em disco disponível (Ubuntu ou Windows OS suportado)
- Uma unidade USB com capacidade de pelo menos 16 GB
- Um monitor e um cabo de vídeo DP/HDMI
- Um ambiente de alimentação capaz de fornecer mais de 240 W
- Um teclado com interface USB

Primeiro, baixe a imagem de sistema em formato ISO para o Thor a partir do site oficial da NVIDIA. Clique [aqui](https://developer.nvidia.com/embedded/jetpack/downloads) para acessar a página de download:
...
Depois de baixar o arquivo de imagem ISO e instalar com sucesso o **Balena Etcher**, insira a unidade USB na máquina host. Em seguida, inicie o **Balena Etcher** para criar uma unidade USB inicializável para gravar o Thor:
:::danger
Este processo irá formatar a unidade USB. Certifique-se de fazer backup de quaisquer dados importantes antes.
:::

Selecione o arquivo de imagem ISO baixado no seu armazenamento local e, em seguida, escolha o dispositivo de destino — ou seja, sua unidade USB.
<mark>Certifique-se de verificar cuidadosamente o nome do dispositivo de destino e o diretório de montagem!</mark>  Clique em `Flash!` e aguarde a conclusão do processo. Quando terminar, a unidade USB para gravar o sistema no Thor estará pronta:
<div align="center">
...
Em seguida, insira a unidade USB preparada, o teclado, o cabo de vídeo (DP/HDMI) e a fonte de alimentação (Type-C) na placa Thor para iniciar o processo de gravação.
<div align="center">
...
Ligue o Thor e entre na interface de boot. Selecione `Boot Manager` e, em seguida, escolha a unidade USB que foi inserida no Thor (com base no nome da sua unidade USB). Pressione Esc para retornar ao menu anterior e selecione `Continue`:
<div align="center">
...
Uma grande quantidade de informações de log será exibida na tela. Aguarde aproximadamente **15 minutos**. Quando essa etapa for concluída, o dispositivo reiniciará automaticamente e avançará para a próxima interface. Aguarde até que a barra de **Update Progress** atinja 100%, o que indica que o processo de gravação foi concluído com sucesso:
<div align="center">
  <img src="https://files.seeedstudio.com/wiki/other/option.png" width="450"/>
  <img src="https://files.seeedstudio.com/wiki/other/flash-M2.png" width="450"/>
</div>
...
Observe que as dependências listadas são **apenas para referência** — instale pacotes adicionais de acordo com os requisitos individuais de cada projeto.
...
Compilar a versão de GPU do PyTorch a partir do código-fonte no Thor pode resultar em problemas de compatibilidade. Para conveniência, fornecemos um arquivo `.whl` pré-compilado para ajudar desenvolvedores a configurar rapidamente um ambiente de desenvolvimento com PyTorch no Thor.
...
Se o arquivo wheel esperado não estiver disponível, o desenvolvedor precisará compilar manualmente as dependências necessárias para concluir a configuração do ambiente de desenvolvimento.
...
<mark>Esta imagem é apenas para referência, e os desenvolvedores são livres para escolher se desejam usá-la com base em suas necessidades específicas.</mark>
...
A imagem pode ser obtida diretamente do Docker Hub e inclui dependências comumente usadas, como `PyTorch`, `TensorRT` e `FlashAttention`:
...
Para executar o Docker no Thor, consulte o comando a seguir. Substitua `your_docker_img:tag` pelo nome e tag da sua imagem Docker, ou use o ID da imagem:
...
Para um tutorial detalhado sobre a coleta de dados, consulte o seguinte link:  
...
:::warning
**O Jetson pode não ser capaz de transmitir simultaneamente duas câmeras USB com os mesmos requisitos de largura de banda. Se você conectar duas câmeras USB, elas devem ser conectadas a chips de hub USB diferentes!**
:::
...
A base USB Type-C deve ser conectada a portas específicas no Thor para garantir que os periféricos funcionem corretamente, conforme mostrado abaixo:
<div align="center">
...
Após o SO-ARM ter sido totalmente montado, é necessário realizar a calibração. Consulte a subseção do seguinte link para o procedimento de calibração:  
...
:::note
**Ao calibrar os braços robóticos, NÃO conecte nenhuma câmera USB, pois isso pode causar conflitos de porta ou atribuições de porta incorretas.**
:::

**Após executar o script de calibração, mova manualmente cada junta do braço robótico para garantir que ela atinja todo o seu alcance de movimento! Caso contrário, poderá haver uma incompatibilidade entre as poses dos braços líder e seguidor durante a teleoperação.**
...
Em geral, é recomendável instalar uma câmera no punho/garra do braço robótico e outra câmera na superfície da mesa, para garantir a cobertura adequada da postura do braço.
...
### Treinamento da Policy

Com base nos dados coletados, o procedimento para treinar uma Policy pode ser encontrado na subseção do seguinte link:  
[https://wiki.seeedstudio.com/pt-br/lerobot_so100m_new/#train-a-policy](https://wiki.seeedstudio.com/pt-br/lerobot_so100m_new/#treinar-uma-policy)
...
:::note
Se você optar por treinar a policy na nuvem, certifique-se de que seu conjunto de dados seja carregado antecipadamente para o servidor em nuvem ou baixado do Hugging Face Hub. No entanto, devido a possíveis problemas de rede ao baixar diretamente do Hugging Face Hub, é altamente recomendável enviar manualmente o conjunto de dados para o seu servidor em nuvem.
:::

## Usar NVIDIA Brev para treinar policies

O NVIDIA Brev fornece acesso simplificado a instâncias de GPU NVIDIA em plataformas de nuvem populares, configuração automática de ambiente e opções flexíveis de implantação, permitindo que os desenvolvedores comecem a experimentar instantaneamente.
...
**Os desenvolvedores podem treinar modelos diretamente no terminal do notebook. Abaixo está um exemplo para treinar um GR00T N1.5 e exportá-lo do servidor em seguida.**

Instale o Conda no servidor:
...
Clone o repositório Isaac-GR00T: 
```bash
git clone https://github.com/NVIDIA/Isaac-GR00T
cd Isaac-GR00T
```

Crie o ambiente gr00t:
```bash
...
:::warning
Se você treinar ou fizer fine-tuning do GR00T em uma plataforma em nuvem, deve usar uma GPU com arquitetura Ampere ou mais recente (por exemplo, RTX A6000 ou GeForce RTX 4090). **V100 (Volta) não é suportada** para treinamento ou fine-tuning do GR00T.
:::

Para mais detalhes sobre o fine-tuning do modelo, consulte:
...
Fine-tuning do modelo GR00T N1.5:
...
:::note
As configurações padrão de fine-tuning exigem ~25G de VRAM. Se você não tiver tanta VRAM, tente adicionar a flag `--no-tune_diffusion_model` ao script gr00t_finetune.py.
:::
...
:::note
Se você quiser treinar ou fazer fine-tuning do Gr00t em um servidor, pode consultar este [link](https://github.com/NVIDIA/Isaac-GR00T).
:::

**Após a conclusão do treinamento, você talvez queira baixar o modelo para a sua máquina local. Porém, a barra lateral do Jupyter Notebook não oferece suporte à navegação direta para dentro das pastas de treinamento de modelo.**

A solução é: primeiro comprima a pasta de destino em um arquivo `.zip` ou `.tar.gz` e, em seguida, baixe o arquivo compactado pela interface do notebook. Consulte a figura abaixo como exemplo.
<div align="center">
  <img src="https://files.seeedstudio.com/wiki/other/train5.png" width="600"/>
</div>

## Inferência do Isaac GR00T N1.5 no Thor

O Jetson AGX Thor, como uma poderosa plataforma de computação e implantação de borda, fornece recursos suficientes para suportar inferência de modelos em grande escala. Nesta seção, com base no conteúdo apresentado anteriormente, demonstramos como executar a inferência do GR00T N1.5 no Thor.

GR00T N1.5 é um sistema de referência open-source lançado pela NVIDIA Research na área de aprendizado de robôs. Ele tem como objetivo fornecer uma estrutura unificada para treinamento e inferência de IA incorporada, com foco especial em aprendizado por imitação e aprendizado de política impulsionados por modelos em larga escala.
...
Use o seguinte comando para iniciar o contêiner, substitua `<image_name>:<tag>` pelo que você estiver usando:
...
Faça git clone do código-fonte do Gr00t e instale-o:
...
Se você não baixou nenhum modelo GR00T pré-treinado e não planeja usar uma versão personalizada, pode usar o seguinte comando para fazer o fine-tuning com base nos pesos do Hugging Face:
...
Este script fará automaticamente o download do modelo GR00T pré-treinado do Hugging Face e iniciará o processo de fine-tuning.

Se você deseja usar um modelo GR00T pré-treinado armazenado localmente, modifique o comando da seguinte forma:
...
:::note
As configurações padrão de fine-tuning exigem cerca de 25G de VRAM. Se você não tiver tanta VRAM, tente adicionar a flag `--no-tune_diffusion_model` ao script gr00t_finetune.py.
:::

### Executando Inferência com GR00T N1.5

Para obter desempenho ideal, recomenda-se reproduzir o mais fielmente possível a configuração do mundo real usada durante a coleta de dados ao implantar modelos GR00T com fine-tuning. Isso porque as capacidades de generalização do modelo são limitadas.
<div align="center">
...
Quando a inicialização for bem-sucedida, o terminal deverá exibir logs semelhantes a:
<div align="center">
  <img src="https://files.seeedstudio.com/wiki/other/deploy1.png" width="600"/>
</div>

Abra outro terminal e use o seguinte comando para entrar no mesmo contêiner a partir de um shell diferente:

```bash
sudo docker exec -it <container id> /bin/bahs
```

Em seguida, neste segundo shell, inicie o cliente de inferência:
...
Certifique-se de que cada junta se mova em toda a sua faixa de movimento para garantir um comportamento adequado de teleoperação e inferência.
:::

Depois que tudo estiver configurado, o GR00T N1.5 poderá ser implantado com sucesso no Jetson AGX Thor. O sistema e o ambiente de hardware agora estão verificados para suportar inferência completa:
<div align="center">
...
Isso geralmente ocorre devido a problemas de rede.
...
**P2: Como faço upload de dados para a plataforma de treinamento?**  
...
Se você usar um pendrive de instalação Jetson em um Jetson AGX Thor Developer Kit que já foi usado ou configurado anteriormente, será necessário habilitar o SoC Display Hand-Off no `UEFI setup menu`.
Na verdade, a gravação continua em segundo plano durante esse período; após algum tempo, a área de trabalho do Ubuntu irá aparecer.

**P4: Não tenho saída de vídeo no monitor (conectado via um KVM)**  
...
**P5: Após gravar o Thor, o HDMI às vezes não mostra sinal em inicializações subsequentes**  
Uma solução alternativa que tem se mostrado eficaz é mudar para o conector DisplayPort (DP) para saída de vídeo.

**P6: Nenhuma entrada de teclado é detectada durante a gravação da imagem do Thor**  
Recomendamos usar um teclado com fio durante a gravação. Teclados sem fio podem ter compatibilidade limitada durante o processo de gravação do Thor.

**P7: O fine-tuning em nuvem do GR00T informa “GPU not supported”**  
Não use GPUs anteriores à `Ampere architecture` para o fine-tuning em nuvem do GR00T. (por exemplo, RTX A6000 ou GeForce RTX 4090). V100 (Volta) não é suportada para treinamento ou fine-tuning do GR00T.
...
**P10: Ao gravar com uma placa de captura, o sistema depois diz que a senha do usuário está incorreta**  
Ao usar uma área de trabalho com placa de captura durante a gravação, pode ocorrer um bug de entrada de teclado (a entrada não corresponde ao que é digitado). Verifique cuidadosamente a senha do usuário ao defini-la.

**P11: Onde fica a porta de depuração do Thor?**  
A porta de depuração Type-C do Thor fica escondida sob a tampa magnética. Vários nós de dispositivo serial podem ser criados; use aquele com o menor índice para fazer login no Thor via serial.

**P12: Não é possível executar scripts LeRobot para inferir ACT na imagem GR00T**  
Não recomendamos invocar APIs LeRobot para inferência de `ACT` em um ambiente Python 3.12. Scripts de `Calibration` e `find port` estão ok. Observe que o Ubuntu 24 usa Python 3.12 como padrão localmente.

**P13: Como verificar a utilização do SM da GPU no Thor?**  
Execute:
```bash
...
Não conecte a câmera antes da placa driver do SO-ARM; fazer isso pode causar mapeamento incorreto do dispositivo serial. Certifique-se também de que as permissões da porta serial foram concedidas.
