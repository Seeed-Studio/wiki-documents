---
description: Treine tarefas oficiais do Microduck, visualize checkpoints PT e execute inferência ONNX multi‑política controlada por teclado no MuJoCo no Jetson.
...
url: https://wiki.seeedstudio.com/pt-br/ai_robotics_microduck_rl_official_policies/
...
Aumente a contagem de ambientes somente quando a memória e a temperatura permitirem. Uma sequência de recuo prática é `4096 → 2048 → 1024 → 512`.
...
## Executar a Demonstração Oficial de ONNX Multi‑Política
...
| `O` | Espacate frente‑trás de seis segundos, depois retorno à posição em pé ou caminhada |
...
A política de caminhada acompanha continuamente os comandos de velocidade e de giro do teclado.
...
### Espacate Frente‑Trás

A antiga política de equilíbrio em uma perna foi substituída por um movimento de
duplo apoio mais estável. Pressione `O` para executar a política treinada: o pé
esquerdo se move para frente, o pé direito se move para trás, ambos os pés
permanecem no chão e o robô retorna à política de ficar em pé ou caminhar após o ciclo de fase de seis segundos.

Os artefatos incluídos são:
...
- Os arquivos PT em `models/checkpoints/` são resultados de treinamento de caminhada no Jetson incluídos com esta demonstração; eles não são releases oficiais de PT da Pollen Robotics.
