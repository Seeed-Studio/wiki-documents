---
description: Compile uma imagem Yocto completa com CUDA a partir do repositório da Seeed e grave-a em um reComputer Super.
title: Compilar e Gravar uma Imagem Yocto para o reComputer Super
keywords:
  - Yocto
  - OpenEmbedded
  - reComputer Super
...
url: https://wiki.seeedstudio.com/pt-br/build_and_flash_yocto_for_recomputer_super/
---

# Compilar e Gravar uma Imagem Yocto para o reComputer Super

Este guia compila uma imagem Yocto da Seeed para o **reComputer Super J401 com um módulo Jetson Orin NX 16GB** e a grava na unidade NVMe do Jetson.
...
- Um cabo de dados USB Tipo-C conectado diretamente ao PC host
- Acesso `sudo` para instalar pacotes no host e realizar a gravação

:::warning
Reserve **pelo menos 400 GB de espaço livre em disco** no PC host. Os downloads do Yocto, cache de estado compartilhado, arquivos temporários de compilação, sistema de arquivos raiz e o pacote de gravação extraído podem consumir várias centenas de gigabytes. Um SSD local no host é fortemente recomendado.
:::
...
Este guia usa `0000`. Sempre use um diretório de compilação separado para cada combinação de placa base e SKU de módulo.
...
## Compilar a Imagem Completa de Desenvolvimento com `all`
...
Confirme o diretório de compilação ativo, a placa base e o SKU do módulo:
...
:::warning
Não continue se o `MACHINE` exibido ou o SKU do módulo não corresponderem ao hardware. Não reutilize este diretório de compilação para outra placa base ou SKU de módulo.
:::
...
A primeira compilação faz o download e compila um grande número de componentes e pode levar várias horas. O comando para imediatamente se a validação de metadados, a compilação da device tree, as verificações de arquivos de boot ou a compilação da imagem falharem.
...
O auxiliar verifica o SKU do módulo e checa se a imagem rootfs, DTB, DTB do BPMP, configuração de pinmux, configuração de tensão dos pinos e scripts de gravação estão presentes. Em seguida, imprime o diretório preparado e o próximo comando exato.
...
4. Conecte a porta de dispositivo/depuração USB Tipo-C ao host Linux com um cabo de dados.
...
O script inicializa temporariamente um initrd de gravação via USB, expõe a unidade NVMe de destino ao host, grava as partições e obtém o status final do dispositivo. Não desconecte o USB nem a alimentação enquanto ele estiver em execução.
...
2. Retorne a chave de recuperação de **RESET** para a posição normal.
...
A seção anterior usou `all`, que é o caminho recomendado para uma primeira compilação. Para depurar ou recompilar uma etapa, execute o mesmo fluxo de trabalho explicitamente:
...
Se o sistema de arquivos raiz já estiver compilado e apenas o arquivo tegraflash precisar ser regenerado, use:
...
## Opcional: criar um SDK de desenvolvimento cruzado x86_64

A imagem de desenvolvimento gravada já oferece suporte à compilação diretamente no Jetson. Crie o SDK para host apenas quando um PC x86_64 precisar compilar aplicações de forma cruzada para o alvo:
...
Este instalador opcional não é necessário para criar a imagem do Jetson ou gravar o dispositivo.

## Solução de problemas

### O diretório de build indica a máquina ou SKU errados

Crie um novo diretório de build com `prepare-workspace.sh`. Não edite um diretório de build existente para alternar placas carrier ou SKUs de módulo.

### O arquivo compactado de flash não pode ser encontrado

Certifique-se de que o mesmo nome de imagem seja passado tanto para o build quanto para a preparação de flash:
