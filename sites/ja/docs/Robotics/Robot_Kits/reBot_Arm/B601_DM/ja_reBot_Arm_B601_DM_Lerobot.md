...
[reBot Arm B601-DM](https://wiki.seeedstudio.com/ja/rebot_b601_dm_getting_started/) は、Seeed が立ち上げたオープンソースのロボットアームプロジェクトで、エンボディドインテリジェンス学習のハードルを下げることを目的としています。構造設計とコードを余すところなくオープンソース化し、ロボティクス技術を誰もが利用できるようにしています。
...
- NVIDIA GPU（CUDA 12+、任意。GPU 加速による学習と推論用）
- Python 3.10
...
- 既定では ffmpeg 7.X がインストールされます（libsvtav1 エンコーダをサポート）
...
- `ffmpeg -encoders | grep svtav1` で libsvtav1 エンコーダがサポートされているか確認できます
...
B601-DM は組み立て後に一度だけキャリブレーションを行う必要があります。以下がキャリブレーションコマンドです。ゼロ位置（グリッパーが完全に閉じた状態）については図を参照してください。
...
高精度な深度とカラーのアライメントにより同期した RGB と深度ストリームを提供します。ステレオ深度センシングと内蔵 6 軸 IMU を組み合わせることで、物体検出、3D 認識、マッピング、ナビゲーションなどのロボットタスクに非常に適しています。コンパクトな設計と Orbbec SDK の完全なサポートにより、研究用途と実環境での運用の両方に適しています。
...
- `depth_alpha` は深度画像のスケーリング係数を制御します。`0.2` を初期値として使用し、その後表示結果に基づいて微調整することをお勧めします。
...
:::danger
テレオペレーション中に、マスタースレーブ型ロボットアームで電源断、電源接触不良、信号線の脱落などが発生した場合は、まずプログラムコードを停止し、ロボットアームをホームのゼロ位置に戻す必要があります。その後で電源を再接続し、プログラムを再起動してください。これにより、データの乱れによるロボットアームの暴走や潜在的な安全上の危険を防ぐことができます。
:::
...
:::tip
`fourcc: "MJPG"` 形式の画像は圧縮されています。より高い解像度を試すことができ、`YUYV` 形式を試すことも可能です。ただし、後者では画像の解像度と FPS が低下し、ロボットアームの動作にラグが生じます。現在、`MJPG` 形式では、`1920*1080` の解像度で `30FPS` を維持しながら 3 台のカメラをサポートできます。しかし、同じ USB HUB を介して 2 台のカメラを 1 台のコンピュータに接続することは依然として推奨されません。
:::
...
- データセットをアップロードするために Hugging Face Hub の機能を使用したい場合で、まだ設定していない場合は、[Hugging Face settings](https://huggingface.co/settings/tokens) から生成できる書き込み権限付きトークンを使ってログインしていることを確認してください：
...
- **スケール**：50 エピソード以上を記録（位置ごとに 10 エピソード）。
- **一貫性**：
  - カメラは固定したままにする。
  - 同じ把持動作を維持する。
  - 操作対象の物体がカメラ映像内で見えるようにする。
- **段階的な拡張**：
  - まずは信頼性の高い把持を実現してから、（新しい位置、把持手法、カメラ調整などの）バリエーションを追加する。
...
2. データセット名 `dataset.repo_id` は `eval_` で始まります。この操作により、評価中の動画とデータが別途記録され、`seeed/eval_test123` のような `eval_` で始まるフォルダに保存されます。
...
SmolVLA は「基盤モデル」です。自分のテーブル上で、自分のカメラ・グリッパ・物体を使って良好に動作させるには、通常、自前のデータでファインチューニングする必要があります。

- まずは約 50 エピソードから始めます（少なすぎると学習・汎化性能が低下する可能性があります）。
- タスクに「変数」（例：テーブル上のキューブ位置の違い）がある場合は、各バリエーションに十分なデモを用意してください：
...
- `--robot.id`: あなたのロボット ID（キャリブレーション／記録時に使用したものと一致している必要があります）。
...
- `--dataset.repo_id`: 評価出力データセット名。Hugging Face にログインしている場合は、あなたのアカウント配下に作成／アップロードされます。
- `--policy.path`:
...
オプション：評価エピソード間で「手動テレオペで微調整」したい場合は、テレオペを追加できます（お使いのデバイスと設定に応じて記入してください）：
...
LeRobot での使用は簡単で、トレーニング時にポリシータイプを `--policy.type=pi0` に設定するだけです（ACT セクションで説明した一般的なトレーニング／評価の概念を繰り返す必要はありません）。
...
π₀.₅ (Pi0.5) も Physical Intelligence が提案した **Vision-Language-Action** モデルであり、「π₀ のアップグレード版」と理解できます。特に **オープンワールド一般化** 能力の強化に重点が置かれています。
...
- `--policy.normalization_mapping=...`: データセットの正規化統計が合っていない／欠けている場合、このマッピングを使って正規化方法を強制指定できます。
...
GR00T N1.5 は NVIDIA が提供するオープンなファウンデーションモデルです。LeRobot で使用する際のポイントは、ポリシータイプを `--policy.type=groot` に設定することです。注意：GR00T N1.5 は現在、より高い環境要件があり（FlashAttention に依存し、CUDA GPU が必要）、まずは ACT / Pi0 を動かしてから GR00T を試すことを推奨します。
...
:::tip
flash-attn のインストールに失敗する場合、多くは (1) PyTorch／CUDA バージョンの不整合、(2) ビルド依存関係の不足、(3) 環境が新しすぎる／古すぎる、といった要因に関連しています。この場合は、まず公式の GR00T ドキュメントと PyTorch インストールガイドを参照してください。
:::

**トレーニング（ファインチューニング）**

公式からはマルチ GPU トレーニング例（`accelerate launch --multi_gpu ...`）が提供されています。GPU が 1 枚しかない場合は、まずシングルプロセスモードでの実行も試すことができます（対応状況／パラメータ詳細は公式ドキュメントに従います）。
...
- `--dataset.repo_id`: あなたのトレーニングデータセット（Hub 上の `username/dataset_name` または対応するローカルキャッシュの `repo_id`）。
...
トレーニング後は、他のポリシーと同様に `lerobot-record` を使って評価／記録を行うことができます。reBot B601-DM 単腕ユーザーは、以下のコマンドを参照してください：
...
PEFT（Parameter-Efficient Fine-Tuning）は、大規模事前学習モデルを新しいタスクに適応させるための「パラメータ効率の良い適応」手法とツールの集合であり、**すべてのモデルパラメータを更新することなく**適応を行うことができます。LeRobot の事前学習済みポリシー（SmolVLA や π₀ など）の場合、通常は少数の「アダプタ」パラメータ（LoRA など）だけを学習することで、VRAM 使用量と学習コストを抑えつつ、ほぼフルファインチューニングに近い結果を得ることができます。
...
特定のモジュールを（LoRA を注入するだけでなく）「フルに学習」させたい場合は、`--peft.full_training_modules` を使って指定します。たとえば `state_proj` のみをフル学習させるには：
...
LoRA の学習率は、通常のフルファインチューニングよりも 1 桁大きく設定できることが多いです（一般的な経験則：およそ 10 倍）。たとえば、フルファインチューニングでは `1e-4` をよく使いますが、LoRA では `1e-3` から始めることができます。学習率減衰（スケジューラ）を有効にしている場合、最終的な学習率も `1e-4` 前後に設定されることが多いです。
...
まず、lerobot 環境にトレーニング高速化システムをインストールします：
...
**GPU 数や混合精度など、あなたのハードウェア環境を設定ファイルとして保存し、
...
- Mixed precision:
  - fp16 を推奨
  - GPU が bf16 をサポートしていることが確認できていれば、bf16 を選択してもよい
...
LeRobot は、ユーザーに知らせずにトレーニング挙動が変わってしまうことを避けるため、GPU の数に応じて学習率やトレーニングステップ数を自動調整しません。これは、他の一般的な分散トレーニングフレームワークとは異なります。
...
直感的に理解するために言い換えると：トレーニングが「歩くこと」のようなものだとすると、1 枚の GPU は 1 ステップで 1 メートル進み、2 枚の GPU は 1 ステップで 2 メートル進むイメージです。同じ距離（モデルが学習する総データ量）に到達するには、2 GPU のトレーニングではステップ数を半分にすべきです。同様に、n 枚の GPU なら 1/n になります。
...
より高度な設定やトラブルシューティングについては、[Accelerate](https://huggingface.co/docs/accelerate/index) のドキュメントを参照してください。多数の GPU でのトレーニングについてさらに学びたい場合は、優れたガイドである [Ultrascale Playbook](https://github.com/huggingface/ultrascale-playbook) をチェックしてみてください。
</details>

<details>
<summary>（オプション）デプロイのために Async Inference を使用する</summary>

Async inference を使わない場合、LeRobot の制御フローは従来型の逐次／同期推論として理解できます。すなわち、ポリシーがアクションチャンクを予測し、それを実行し、次の予測を待つ、という流れです。モデルが大きくなると、ロボットが新しいアクションチャンクを待つ間に顕著な待ち時間が発生することがあります。Async inference の目的は、ロボットが現在のアクションチャンクを実行している間に次のチャンクを事前計算し、アイドル時間を減らして応答性を向上させることです。Async inference は、ACT、OpenVLA、Pi0、SmolVLA など、アクションチャンクを出力する LeRobot 対応ポリシーに適用できます。推論と実際の制御が分離されるため、Async inference によって、より高性能なマシンをリモートからロボット推論に利用することも可能になります。

Hugging Face のこの[ブログ記事](https://huggingface.co/blog/async-robot-inference)で、非同期推論についてさらに詳しく読むことができます。

ここで、いくつかの基本概念を紹介します：

- **クライアント**：ロボットアームとカメラに接続し、観測値（画像、ロボットの姿勢など）を収集してサーバーに送信します。また、サーバーからアクションチャンクを受信し、それらを順番に実行します。
...
- **同期推論**：1 チャンクを予測して 1 チャンクを実行します。次のチャンクが推論されるのを待つ間、ロボットにはアイドルギャップが発生します。モデルが大きく計算資源が不足している場合、この推論ギャップは顕著になり、アームは動いては一時停止（推論）、そして再び動く、という挙動になります。
...
- このドキュメントのチュートリアルに従う場合は、推奨されている GitHub リポジトリ `https://github.com/Seeed-Projects/lerobot.git` を git clone してください。本ドキュメントで推奨しているリポジトリは検証済みの安定版であり、公式の LeRobot リポジトリは常に最新版へ更新されているため、データセットバージョンの違いやコマンドの違いなど、予期せぬ問題が発生する可能性があります。
...
- 50 セットのデータで ACT を学習するには、RTX 3060（8GB）を搭載したノート PC で約 6 時間、RTX 4090 や A100 GPU を搭載したコンピュータでは約 2〜3 時間かかります。

- データ収集中は、カメラの位置、角度、周囲の照明が安定していることを確認してください。カメラに映り込む不安定な背景や歩行者の量を減らしてください。デプロイ環境が大きく変化しすぎると、ロボットアームがうまく把持できなくなる可能性があります。

- データ収集コマンドでは、十分なデータを収集できるように `num-episodes` パラメータが設定されていることを確認してください。途中で手動で一時停止しないでください。データ収集完了後にのみデータの平均値と分散が計算され、これらは学習に必要です。

- プログラムが USB カメラから画像データを読み取れないと表示する場合は、USB カメラがハブ経由で接続されていないことを確認してください。USB カメラはデバイスに直接接続し、高速な画像転送速度を確保する必要があります。

:::tip
ソフトウェアの問題や環境依存の問題が発生し、解決できない場合は、このチュートリアルの最後にある FAQ セクションを確認することに加えて、速やかに [LeRobot platform](https://github.com/huggingface/lerobot) または [LeRobot Discord channel](https://discord.gg/8TnwDdjFGU) に問題を報告してください。
:::
