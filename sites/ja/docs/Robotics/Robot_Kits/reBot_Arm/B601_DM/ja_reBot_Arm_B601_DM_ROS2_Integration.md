...
このチュートリアルでは、reBot Arm B601-DM 向けに ROS2 control ワークスペース `rebotarm_ros2` を実行する方法を説明します。このワークスペースは、低レベルの `reBotArm_control_py` Python SDK を ROS2 のトピック、サービス、アクションにラップし、上位レベルのプランニング、ビジュアルグラスピング、RViz 可視化、カスタムアプリケーション開発との統合を容易にします。
...
代わりに、現在のユーザーを `dialout` グループに追加することを推奨します。変更を有効にするには、一度ログアウトしてから再ログインしてください：
...
デフォルトでは Seeed-Projects 公式リポジトリを使用します：
...
:::caution
ロボットを使用する前に、次の点に注意してください：**アームコントローラは高い自由度を持ちます。コントローラを有効化したりアームに給電したりする前に、作業空間に人や障害物がないことを必ず確認してください。事故を避けるため、すべての動作コマンドを慎重に確認してください。危険な操作は固く禁じられており、その結果については自己責任となります。**
:::
...
:::tip
`driver.launch.py` が `rebotarm_bringup/config` から設定ファイルを渡すのとは異なり、コントローラを直接実行した場合はデフォルトの SDK アーム設定が使用されます。通常の利用では、ROS を通して起動することを推奨します。
:::
...
| API | 型 | 説明 |
|---|---|---|
| `/rebotarm/joint_states` | `sensor_msgs/msg/JointState` | 6 軸ジョイントの位置、速度、トルク |
| `/rebotarm/arm_status` | `rebotarm_msgs/msg/ArmStatus` | 制御モード、有効状態、ステートマシン、エラーコード |
| `/rebotarm/joints/<joint>/state` | `rebotarm_msgs/msg/JointMotorState` | 単一ジョイントのモータ状態 |
| `/rebotarm/gripper/state` | `rebotarm_msgs/msg/JointMotorState` | グリッパーモータの状態 |
...
| `/rebotarm/set_zero` | `rebotarm_msgs/srv/SetZero` | 全ジョイントまたは単一ジョイントのゼロ位置を設定 |
| `/rebotarm/move_to_pose_ik` | `rebotarm_msgs/srv/MoveToPoseIK` | IK 事前チェックと目標ジョイント解の計算 |
| `/rebotarm/gripper/set` | `rebotarm_msgs/srv/SetGripper` | グリッパーモータ位置を rad 単位で設定 |
...
6 つのジョイントすべてを同時に制御します。単位は rad です：
...
| ファイル | 説明 |
|---|---|
| `arm.yaml` | 6 つのアームジョイントのモータ、フィードバック ID、および制御パラメータ |
| `gripper.yaml` | グリッパーモータ ID、フィードバック ID、ベンダー、および制御パラメータ |
...
| `joint_state_rate` | `100.0` | `/rebotarm/joint_states` の publish レート |
| `cmd_arbitration` | `reject` | アーム軌道実行中の低レベルコマンドの仲裁方法。`reject` または `preempt`。グリッパーの低レベルコマンドはアーム軌道を preempt しません |
...
MoveIt 2 は、本ドキュメントで使用しているモーションプランニングフレームワークであり、逆運動学、衝突チェック、軌道計画および実行を行います。デモは専用パッケージに分離されており、アプリケーションフローがベースドライバから分離されるようになっています。
...
MoveIt 環境は、計画と実行のために `ros2_control` と `move_group` を通じてシミュレートされたハードウェアを使用します。これは、RViz 内でモデル、IK、軌道計画、およびデモフローを検証することを目的としています。

このリポジトリは実機ハードウェアにも対応しています。実機を接続する前に、アームのゼロ設定、ジョイント方向、ジョイント制限、速度制限、およびグリッパー範囲がすべて正しいことを確認するか、リポジトリのデフォルト設定のままにしてください。

### MoveIt 環境セットアップ

まず ROS2 環境が利用可能であることを確認してください。現在 source されている ROS ディストリビューションに対して、`ROS_DISTRO` を通じてパッケージをインストールできます：
...
繰り返しになりますが：実機でデモを実行する前に、作業エリアに人や障害物がないことを確認し、RViz で計画経路を検証し、いつでもコントローラを停止できるように準備しておいてください。
...
`draw_square` は `gripper_tcp` を同一平面上の長方形の 4 つの頂点へ移動させます。
デフォルトパラメータ：
...
| `pick_tcp_rpy` / `place_tcp_rpy` | pick と place 用の TCP 姿勢 |
| `object_dimensions` | Planning-scene 上の物体の寸法（メートル） |
| `max_gripper_width` | グリッパの最大開口幅（合計）。デフォルトは `0.09m` |
| `open_gripper_position` / `closed_gripper_position` | シミュレートされた片側グリッパ関節の開閉位置 |
...
| `rebotarm_moveit_config/config/initial_positions.yaml` | シミュレートされたハードウェアの初期関節位置 |
| `rebotarm_moveit_demos/config/draw_square.yaml` | draw-square デモのパラメータ |
| `rebotarm_moveit_demos/config/pick_place.yaml` | pick-place デモのパラメータ |
...
これは、シリアルポートがすでに別のプロセスによって使用されていることを意味します。よくある原因としては、以前に起動した ROS2 ノード、SDK のサンプル、終了していないデバッグスクリプトなどがあります。まずプロセスを確認します：
...
ログアウトして再ログインし、変更を有効にします。一時的なデバッグであれば、次を実行することもできます：
...
- [reBot Arm B601-DM Getting Started](https://wiki.seeedstudio.com/ja/rebot_b601_dm_getting_started/)
- [reBot Arm B601-DM Visual Grasping Demo](https://wiki.seeedstudio.com/ja/rebot_arm_b601_dm_grasping_demo/)
- [reBot Arm B601-DM Pinocchio and MeshCat](https://wiki.seeedstudio.com/ja/rebot_arm_b601_dm_pinocchio_meshcat/)
- [reBot Arm B601-DM LeRobot Tutorial](https://wiki.seeedstudio.com/ja/rebot_arm_b601_dm_lerobot/)
