...
Web シミュレータ開発を始める前に、[reBot Arm B601-DM クイックスタート](https://wiki.seeedstudio.com/ja/rebot_b601_dm_getting_started/) の手順（アームの組み立て、モーター ID 設定、ゼロ点初期化、基本的な接続確認など）を完了してください。
...
最も軽量な実行方法です：Web サーバーのみを起動し、ROS2 との接続は行いません。ポーズのデモ、ティーチング、UI 開発に適しています：
...
ブラウザで `http://localhost:3001` を開きます。ジョイントスライダーのドラッグ、ポーズプリセット、TCP ドラッグ、ティーチ＆レコードが利用できますが、すべての操作は 3D モデルのみに影響し、ハードウェアや ROS ノードは一切駆動されません。
...
1 つのコマンドで、Fake Driver、MuJoCo 物理グラスプ、タスクサーバー、仮想カメラ、色検出器、rosbridge を含むフルスタックを起動します：
...
このスクリプトは内部的には `reBotArm_ros2_DM/scripts/start_rebot_mujoco_all.sh` と同等です。デフォルトでは Fake Driver、robot_state_publisher、MuJoCo 物理グラスプ、タスクサーバー、オーバーヘッド RGB カメラ、色検出器、rosbridge を起動します。その後、別のターミナルで `./rebotarm start web` を実行して Web ページを起動します。ブラウザが ROS に接続したら、ビジュアルグラスピングのデモを利用できます。
...
<details>
<summary><b>コアモジュールに関する注意</b>（クリックして展開）</summary>
...
| `/rebotarm/arm_status` | `rebotarm_msgs/msg/ArmStatus` | イネーブル状態、モード、ステートマシン |
...
| `/rebotarm/move_to_pose_ik` | `rebotarm_msgs/srv/MoveToPoseIK` | IK 解サービス |
...
MCP Dashboard は独立したデバッグ用エントリであり、Web シミュレータを必要としません。起動手順は 2 ステップです：
...
| `reBotArm_simulator-DM/public/js/rebot-sim.js` | 3D シーン、IK、ティーチング、ドラッグのコア処理 |
