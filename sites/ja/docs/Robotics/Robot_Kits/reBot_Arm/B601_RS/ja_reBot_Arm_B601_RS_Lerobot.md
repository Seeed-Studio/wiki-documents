...
[reBot Arm B601-RS](https://wiki.seeedstudio.com/ja/rebot_b601_rs_getting_started/) は、Seeed が立ち上げたオープンソースのロボットアームプロジェクトで、エンボディドインテリジェンス学習のハードルを下げることを目的としています。すべての構造設計とコードを余すところなくオープンソース化し、ロボティクス技術を誰もが利用できるようにしています。
...
:::caution
Seeed Studio のチュートリアルは、公式ドキュメントに基づいて厳密に更新されています。ソフトウェアや環境に関する問題が解決できない場合は、まず記事末尾の FAQ を確認するか、カスタマーサービスに連絡して SeeedStudio LeRobot 議論グループに参加してください。こちらでも質問できます：[LeRobot GitHub](https://github.com/huggingface/lerobot) または [Discord Channel](https://discord.gg/8TnwDdjFGU)。
:::
...
画面の指示に従って、上図に示すゼロ位置までリーダーアームを動かします。
...
テレオペレーション中に、マスタースレーブロボットアームで電源断、電源接触不良、または信号線の脱落が発生した場合は、まずプログラムコードを停止し、ロボットアームをホームのゼロ位置に戻してください。その後で電源を再接続し、プログラムを再起動します。これにより、データの乱れによるロボットアームの暴走や安全上のリスクを防ぐことができます。
...
高精度な深度‐カラーアライメントを備えた同期RGBおよび深度ストリームを提供します。ステレオ深度センシングと内蔵6軸IMUを組み合わせることで、物体検出、3D認識、マッピング、ナビゲーションなどのロボットタスクに非常に適しています。コンパクトな設計と完全な Orbbec SDK サポートにより、研究用途と実環境での導入の両方に適しています。
...
- `depth_alpha` は深度画像のスケーリング係数を制御します。`0.2` を起点として、表示結果に基づいて微調整するのがよいでしょう。
...
- Hugging Face Hub の機能を使ってデータセットをアップロードしたい場合で、まだ設定していないときは、[Hugging Face settings](https://huggingface.co/settings/tokens) から生成できる書き込み権限付きトークンを使ってログインしていることを確認してください：
...
4. `mean is infinity. You should either initialize with stats as an argument or use a pretrained model` というエラーが出る場合、`--robot.cameras` パラメータ内の `front` や `side` といったキーワードは、データセット収集時に使用したものと厳密に一致させる必要があることに注意してください。
...
- まずは約 50 エピソードから始めます（少なすぎると学習／汎化性能が低くなりがちです）。
- タスクに「変数」（例：テーブル上のキューブ位置が異なる）がある場合は、それぞれのバリエーションに十分なデモがあるようにします：
  - 例：5 つの位置 × 各 10 エピソード = 50 エピソード
- 経験則：25 エピソードだけの記録では不十分であることが多いです。データは質と量の両方が重要です。

**学習（トレーニング）**

`smolvla_base`（事前学習済み 450M モデル）を出発点として、自分のデータセットでファインチューニングします。公式の例では 20k ステップ学習しており、単一の A100 で約 4 時間かかります（あくまで参考値であり、実際の時間はハードウェアによって異なります）。
...
- `--robot.cameras`：実際のカメラの `index_or_path` に変更し、カメラキー（例：`front`、`side`）はデータセット記録時に使用したものと完全に一致させてください。
...
オプション：評価エピソードの合間に「手動テレオペで微調整」したい場合は、テレオペを追加できます（自分のデバイスと設定に応じて記入）：
...
- `--policy.train_expert_only=true`（VRAM 節約テク）：大きなモデル（VLM）部分を凍結し、「アクションエキスパート」と射影層のみを学習します。より多くの VRAM を節約できますが、学習可能な容量は制限されます。まずは入門用や小規模データでの素早い実験に適しています。
...
- **物理レイヤー**：見慣れないスプーン／皿でも、どこを持てばよいか（柄／縁など）を理解して把持し、散らかった環境でも操作できること。
...
データセットに分位点統計が含まれていない場合（バージョン／フォーマットによっては必須）、公式からは変換スクリプトを使う方法も提供されています。学習前にデータセット統計を補完／変換してください（詳細は公式ドキュメントを参照）。
...
（直感的に理解するための例：学習が歩行のようなものだとすると、1 枚の GPU は 1 メートルの 1 歩を進み、2 枚の GPU は 1 歩で 2 メートル進みます。同じ距離（モデルが学習したデータ量）に到達するには、2 GPU 学習ではステップ数を半分にすべきです。同様に、n 枚の GPU なら 1/n になります。）
...
より高度な設定やトラブルシューティングについては、[Accelerate](https://huggingface.co/docs/accelerate/index) のドキュメントを参照してください。多数の GPU での学習についてさらに知りたい場合は、こちらの優れたガイドを参照してください：[Ultrascale Playbook](https://github.com/huggingface/ultrascale-playbook)。
</TabItem>

<TabItem value="async-inference" label="Async Inference (Optional)">

async inference を使わない場合、LeRobot の制御フローは従来の逐次／同期推論として理解できます：ポリシーがアクションチャンクを予測し、それを実行し、次の予測を待ちます。モデルが大きくなると、ロボットが新しいアクションチャンクを待つ間に顕著な一時停止が発生することがあります。async inference の目的は、現在のアクションチャンクをロボットに実行させながら、次のチャンクを事前に計算し、アイドル時間を減らして応答性を高めることです。async inference は、ACT、OpenVLA、Pi0、SmolVLA など、アクションチャンクを出力する LeRobot 対応ポリシーに適用されます。推論と実際の制御が分離されているため、async inference により、より高性能なマシンをリモートからロボット推論に利用することも可能になります。

async inference についてさらに知りたい場合は、Hugging Face のこの[ブログ記事](https://huggingface.co/blog/async-robot-inference)を参照してください。
...
- **Synchronous inference**：1 チャンクを予測して 1 チャンクを実行する方式で、次のチャンクが推論されるまでロボットにアイドル時間が発生します。モデルが大きく計算資源が不足している場合、推論のギャップが大きくなり、アームが動いては一時停止（推論）、また動く、という挙動になります。
...
ロボットとカメラは軽量デバイスに接続され、ポリシーサーバーは同一 LAN 上の別の高性能デバイスで動作します。
...
注意：上記のコマンドは現在のターミナルセッションのみに影響します。新しいターミナルウィンドウを開いた場合は、再度実行する必要があります。
...
非同期推論では、サーバー側のアクションチャンク生成速度がクライアント側の消費速度以上になるようにバランスを取る必要があります。そうでない場合、アクションキューが空になり、ロボットがカクつき始めます（これはキューの可視化カーブが下限に達することで確認できます）。
