...
2. あるいは、当社のオープンソース 24V 14.6A MeanWell 電源エンクロージャを選択し、自分で組み立てることもできます。テキストによる説明書と BOM は [GitHub リポジトリ](https://github.com/LAN-GER/reBot-DevArm/tree/main/hardware/reBot_B601_RS) で公開されています（関連する電源組み立て経験を持つ開発者にのみ推奨されます）。
...
3. エージェントは CLI コマンドを使用してモーター ID の書き込みを行いますが、この wiki では Web UI を用いた操作方法を説明しています。どちらの方法でも問題ありません。
...
1. Miniforge をインストールし、仮想環境を作成して、他の環境パッケージとの競合によるデモの失敗を避けます。
...
ブラウザで [motorbridge-studio](https://motorbridge.github.io/motorbridge-studio/) のアドレスを開き、Help オプションをクリックして、使用しているオペレーティングシステムとドライバボードに応じたコマンドをコピーします。IP アドレスとポート番号を確認し、ターミナルで Enter キーを押して実行します。
