...
このチュートリアルでは、ROS2 ワークスペース `rebotarm_ros2` を通して reBot Arm B601-RS を制御する方法を説明します。このワークスペースは、Pinocchio ベースの逆運動学、軌道計画、重力補償を含む `reBotArm_control_py` Python SDK を ROS2 のトピック、サービス、アクションとしてラップします。これにより、上位レベルのプランニング、RViz 可視化、重力補償、グリッパ制御、およびカスタムアプリケーション開発を容易に行うことができます。

:::note
このチュートリアルでは、`Ubuntu 24.04 + ROS2 Jazzy` を主な参照環境として使用します。ROS2 Humble / Ubuntu 22.04 でも同じワークフローに従うことができます。
:::
...
2. **すぐに使える運動学・軌道計画・重力補償ノード**  
   順運動学 / 逆運動学、軌道計画、重力補償、および RViz 可視化のサポートをすぐに使える形で提供します。
...
| 項目 | 推奨要件 |
|---|---|
| オペレーティングシステム | Ubuntu 24.04、Ubuntu 22.04 は参照として使用可能 |
| ROS2 | Jazzy、Humble は参照として使用可能 |
...
ROS2 連携を開始する前に、組み立て、モータ ID 設定、ゼロ位置初期化などの基本準備を含む [reBot Arm B601-RS クイックスタート](https://wiki.seeedstudio.com/ja/rebot_b601_rs_getting_started/) を完了してください。
...
colcon、pip、Git、およびこのワークスペースに必要な ROS パッケージをインストールします。以下のコマンドは、現在のターミナルの `ROS_DISTRO` の値を使用します：
...
:::caution
ロボットアームを使用する前に、次の点に注意してください：**ロボットアームコントローラは高い自由度を持ちます。コントローラを有効化したりアームに給電したりする前に、作業空間に人や障害物がないことを必ず確認してください。事故を防ぐため、すべての動作コマンドを慎重に確認してください。危険な操作は固く禁じられており、その結果についてはすべて自己責任となります。**
:::
...
長期間 B601-RS のみを使用する場合は、`src/rebotarm_bringup/config/rebotarm_hardware.yaml` 内の `default_model` を `rs` に手動で変更できます。その後は、`model:=rs` を省略し、実際の CAN チャンネルパラメータだけを指定できます：
...
RViz でモデルが小さく表示される場合は、左側の `Views` パネルからビューを調整します：
...
| API | 型 | 説明 |
|---|---|---|
| `/rebotarm/joint_states` | `sensor_msgs/msg/JointState` | RViz 用の可視グリッパジョイントを含む、6 軸ジョイントの位置、速度、エフォート |
| `/rebotarm/arm_status` | `rebotarm_msgs/msg/ArmStatus` | 制御モード、有効状態、ステートマシン、エラーコード |
| `/rebotarm/joints/<joint>/state` | `rebotarm_msgs/msg/JointMotorState` | 単一ジョイントモータの状態 |
...
| `/rebotarm/safe_home` | `std_srvs/srv/Trigger` | セーフホーム位置へ戻る |
...
| `/rebotarm/gripper/open` | `rebotarm_msgs/srv/GripperCommand` | 設定済みの値に従ってグリッパーを開く |
| `/rebotarm/gripper/close` | `rebotarm_msgs/srv/GripperCommand` | 設定済みの値に従ってグリッパーを閉じる |
...
これにより、まず重力補償を停止し、その後アームをセーフホーム位置に戻してから無効化します。
...
| ファイル | 説明 |
|---|---|
| `rebotarm_hardware.yaml` | B601-RS 向けの ROS2 上位レイヤーハードウェア選択と SDK パラメータ上書き設定 |
...
この変更後、`model:=...` が明示的に渡されない場合は、自動的に B601-RS が使用されます。
...
| `model` | `rs` | このチュートリアルでは、B601-RS の設定を選択するために `rs` を明示的に渡します |
...
| `disable_after_safe_home` | `true` | セーフホーム完了後にモーターを無効化するかどうかを制御 |
...
:::caution
低レベルコマンドトピックは、デバッグおよび実験用途を想定しています。IK、軌道計画、URDF 妥当性チェックは行いません。アプリケーションレベルのモーションには、`/move_to_pose`、`/follow_joint_trajectory`、`/gripper/set` などのサービスやアクションを優先して使用してください。
:::

## MoveIt 2

MoveIt 2 はロボットアーム向けのモーションプランニングフレームワークです。ここでは主に、逆運動学、衝突チェック、軌道計画、および軌道実行を担当します。デモパッケージでは、アプリケーションフローを低レベルドライバから分離しています。詳細については公式の [MoveIt 2 Documentation](https://moveit.picknik.ai/main/index.html) を参照してください。
...
MoveIt 環境は、`ros2_control` と `move_group` を通じてシミュレートされたハードウェアを使用して計画と実行を行います。これは、RViz 内でモデル、IK、軌道計画、およびデモフローを検証するのに適しています。

このリポジトリは実機ハードウェアもサポートします。実機を接続する前に、アームのゼロ設定、関節方向、関節制限、速度制限、およびグリッパーの開閉範囲が正しいこと、またはリポジトリのデフォルト設定のままであることを確認してください。

### MoveIt 環境セットアップ

まず ROS2 環境が利用可能であることを確認します。次のコマンドは、現在 source されている `ROS_DISTRO` 向けの依存関係をインストールします：
...
MoveIt のプランニングは、シミュレーションおよび実機の両方のシーンで、RViz GUI またはノード呼び出しを通じて使用できます。

#### シミュレーションで MoveIt を使用
...
:::caution
実機でデモを実行する前に、ロボットアームの作業空間に人や障害物がないことを確認し、まず RViz で計画経路を確認し、いつでもコントローラを停止できるように準備しておいてください。
:::

### Draw-square デモを実行する

まず MoveIt 環境を起動し、その後、別のターミナルで次を実行します：
...
`draw_square` は、`gripper_tcp` を同一平面上の長方形の 4 つの頂点に沿って移動させます。デフォルトのパラメータは次の場所にあります：
...
`draw_square` は、MoveIt の IK、軌道計画、および実行が正しく動作しているかを検証するのに有用です。計画が失敗する場合は、まず開始関節状態、長方形の中心、TCP の姿勢、および関節制限が妥当かどうかを確認してください。
...
| Parameter | Description |
|---|---|
| `ready_point` | Pick/Place の前後で使用される待機関節位置 |
| `pick_position` | `base_link` における物体の底面中心位置 |
| `pick_tcp_rpy` / `place_tcp_rpy` | Pick および Place 用の TCP 姿勢 |
| `object_dimensions` | 計画シーン内の物体の寸法（メートル単位） |
| `max_gripper_width` | グリッパーの合計最大開口幅。デフォルトは `0.09m` |
| `open_gripper_position` / `grasp_gripper_position` / `closed_gripper_position` | シミュレーション用片側グリッパー関節位置 |
| `hardware_open_gripper_position` / `hardware_closed_gripper_position` | 実機グリッパーモータの開閉位置 |

`pick_place` は、ターゲット物体を計画シーンに追加し、グリッパーを開き、Pick 姿勢へ移動し、グリッパーを閉じ、Place 姿勢へ移動して物体を解放します。シミュレーション用のグリッパー関節位置と、実機グリッパーモータ位置は別々のパラメータです。実機で実行する前に、グリッパーの開閉方向とリミットを必ず確認してください。

### MoveIt 設定ファイル

| File | Description |
|---|---|
| `rebotarm_moveit_config/config/rebotarm_rs.urdf.xacro` | MoveIt が使用する B601-RS ロボットモデル（グリッパーと `gripper_tcp` を含む） |
...
| `rebotarm_moveit_config/config/ompl_planning.yaml` | OMPL プランナーパラメータ |
...
| `rebotarm_moveit_demos/config/draw_square_rs.yaml` | Draw-square デモのパラメータ |
| `rebotarm_moveit_demos/config/pick_place_rs.yaml` | Pick-place デモのパラメータ |
...
- URDF のメッシュパスが `package://rebotarm_bringup/description/...` になっているか
...
- `demo.launch.py` または `hardware.launch.py` が起動されており、`move_group` が正しく動作していることを確認する。
- 現在の関節状態が古くなっていないこと、および `/joint_states` または `/rebotarm/joint_states` が正常に publish されていることを確認する。
- 目標点が到達可能な作業空間内にあり、目標姿勢によって手首やグリッパーがアーム本体と干渉しないことを確認する。
- `joint_limits.yaml` 内の関節制限および速度制限が、現在のロボットアームと一致していることを確認する。
- まず RViz の MotionPlanning プラグインで手動で計画を行い、経路に衝突がないことを確認してからデモを実行する。
...
これは通常、以前の ROS2 プロセスが異常終了した後に残った FastDDS の共有メモリロックファイルが原因です。サービスやアクションが引き続き正常に応答している場合、この警告は通常制御に影響しません。

これをクリーンアップするには、関連する ROS2 プロセスを停止してから、次を実行します：
...
共有メモリトランスポートを一時的にバイパスしたい場合は、ROS2 を起動する前に次を設定します：
