---
description: このチュートリアルでは、reBot Arm B601-RS 上で MIT 位置制御を行うために、MotorBridge と CAN バスを用いて rebot_control を使用する方法を説明します。グリッパ制御、温度保護、安全な原点復帰も含まれます。
title: reBot Arm B601-RS で MIT 位置制御を始める
keywords:
  - reBot
  - B601-RS
  - MIT
...
| パラメータ | 仕様 |
|-----------|---------------|
| アームモデル | reBot Arm B601-RS 組立済みキット（グリッパ付き） |
| 自由度 (DOF) | 6+1（グリッパを含む） |
...
| **電源** | DC 48V 15A |
| **依存パッケージ** | `motorbridge`, `pyyaml`；オプションで `pynput`（Esc キー用） |
...
| `return_zero.max_speed_deg_s` | 通常の原点復帰のピーク速度 | 30°/s |
| `return_zero.thermal_max_speed_deg_s` | 熱保護時の原点復帰ピーク速度 | 30°/s |
| `return_zero.min_time_s` | 原点復帰の最短時間 | 3.0 s |
| `return_zero.settle_time_s` | 原点復帰後にゼロで保持する時間 | 0.30 s |
...
| しきい値（デフォルト） | 動作 |
|---------------------|----------|
| ≥ 80°C | 温度警告（モータごとに過熱に入るたび 1 回）、動作継続 |
| ≥ 125°C | 動作停止、熱保護用ピーク速度で低速原点復帰後に無効化 |
...
| ゼロ復帰中の 2 回目の Ctrl+C | ゼロ復帰を中断し、即座に無効化 |
| 通信エラー | 緊急無効化、ゼロ復帰なし |
...
すべてのサンプルは**プロジェクトルート**から実行してください。各スクリプトの先頭には **Expected motion（期待される動き）** が記載されており、起動時に `[Expected / 预期]` 行を出力します。
...
4. **Ctrl+C** を押して終了します。`stop(return_to_zero=False)` は**ゼロ位置に戻さずに** CAN をクローズします。
...
- **`Permission denied` / can0 を開けない**  
  CAN インターフェースが `up` になっており、ユーザーがネットワークデバイスにアクセスできることを確認してください。必要に応じて `ip link` に `sudo` を使用するか、udev ルールを確認します。

- **モーターが反応しない / 有効化に失敗する**  
  1. ビットレートが 1 Mbps であることを確認する；  
  2. `config/rebotarm_rs.yaml` 内の `channel`、`host_id`、モーター ID がハードウェアと一致していることを確認する；  
...
- **グリッパーが動かない**  
  グリッパーは **J7 / CAN ID 7** です。`set_joint_angles` に **7 個の値**（最後がグリッパー）を渡すか、`set_joint_angle(7, angle)` を使用してください。
...
- **通信エラー後の挙動**  
  制御ループの通信が失敗した場合、プログラムは**ゼロ位置への復帰なしで緊急的に無効化**し、不良な通信状態での動作を避けます。
