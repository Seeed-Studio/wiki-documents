...
description: このアプリケーションケースでは、最新の YOLO11n モデルを reCamera エッジコンピューティングプラットフォーム上で動作させた際の、極限性能と実測データを示します。
keywords:
  - reCamera
  - エッジ AI
  - YOLO11
...
ここでは、reCamera がわずか **1.5W** の消費電力で YOLO11n の物体検出およびインスタンスセグメンテーションモデルをいかにスムーズに駆動できるかをご覧いただけます。
...
画面左上の OSD 情報には、ハードウェアの時間消費内訳がリアルタイムで厳密に表示されます：**前処理 (Pre-process)**、**推論 (Inference)**、**後処理 (Post-process)**、および **合計 (Total)** の時間消費です。

### YOLO11n インスタンスセグメンテーションのリアルタイムストリーミング
<div style={{textAlign: 'center'}}>
  <img width="800" src="https://files.seeedstudio.com/wiki/reCamera/Applications/yolo_benchmark/yolo-benchmark-seg.gif" alt="YOLO11n セグメンテーションデモ" />
</div>
<br/>

### YOLO11n 物体検出のリアルタイムストリーミング
<div style={{textAlign: 'center'}}>
  <img width="800" src="https://files.seeedstudio.com/wiki/reCamera/Applications/yolo_benchmark/yolo-benchmark-detect.gif" alt="YOLO11n 検出デモ" />
</div>
...
| モデルタスク種別 | 入力解像度 | 量子化形式 | 最大フレームレート (FPS) | 単一フレームのエンドツーエンドレイテンシ | 平均動作電力 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **YOLO11n 物体検出** | 640 x 640 | INT8 | **20 FPS** | **50 ms** | **1.5 W** |
| **YOLO11n インスタンスセグメンテーション** | 640 x 640 | INT8 | **10 FPS** | **100 ms** | **1.5 W** |

<div style={{textAlign: 'center'}}>
  <img width="800" src="https://files.seeedstudio.com/wiki/reCamera/Applications/yolo_benchmark/recamera_benchmark_information.png" alt="YOLO11n ベンチマーク" />
</div>
...
* **ms (Milliseconds)**: すなわち 1 秒の 1000 分の 1 です。ここでは、**単一画像を処理するためのエンドツーエンドの総時間消費**を指します。50 ms は、デバイスが 1 フレームの映像を処理するのに（前処理、NPU 推論、後処理、その他すべてのステップを含めて）最小で ***0.05 秒*** しかかからないことを意味します。
...
* **レイテンシ性能**: 検出モデルにおける最小 50ms の超低エンドツーエンドレイテンシは、高速移動する物体も容易に捉えられることを意味します。一方で、セグメンテーションモデルは高負荷な Mask デコードオペレータを追加しているにもかかわらず、最大 10 FPS/S のスムーズな体験を維持できます。
...
### ステップ 1: ベンチマーク実行ファイルとモデルを入手する
まず、以下のリンクからコンパイル済み bin ファイル、変換済み `.cvimodel` モデルファイル、および Python スクリプトをダウンロードします：
...
<div style={{textAlign: 'center'}}>
  <img width="800" src="https://files.seeedstudio.com/wiki/reCamera/Applications/yolo_benchmark/recamera_benchmark_list.png" alt="YOLO11n リスト" />
</div>

### ステップ 2: ファイルを reCamera デバイスにアップロードする
...
### ステップ 3: ベンチマークテストを実行する
reCamera デバイス上で次のコマンドを実行して、ベンチマークテストを開始します：
...
テスト結果を確認したい場合は、Windows ターミナル上で次のコマンドを実行し、udp スクリプトを起動して reCamera のビデオストリームを受信します：
```bash
