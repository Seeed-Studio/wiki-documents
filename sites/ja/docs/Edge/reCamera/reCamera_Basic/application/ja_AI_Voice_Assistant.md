...
description: このドキュメントでは、reCamera をベースに STT/LLM/TTS を用いたインテリジェント音声対話デモを紹介し、reCamera がポーズ認識によって音声インタラクションをトリガーし、iFlytek Spark 大規模モデルに接続して、STT→LLM→TTS の完全な対話パイプラインを実装する方法を示します。
keywords:
  - reCamera
  - AI Edge Vision
  - 音声インタラクション
...
url: https://wiki.seeedstudio.com/ja/reCamera/reCamera_Basic/application/ai_voice_assitant_with_recamera_bak/
---
...
このプロジェクトでは、自然言語を通じて reCamera と対話する方法をデモします。ビジュアル推論によって音声録音がトリガーされた後、reCamera は録音データをサーバーに送信し、完全な **STT (Speech-to-Text) → LLM (Large Language Model Reasoning) → TTS (Text-to-Speech)** パイプラインで処理します。合成された音声は再生のために reCamera に返され、自然な言語での会話を実現します。
...
- **スマート入退室管理アシスタント**：入口に reCamera を設置し、訪問者は追加のインタラクティブスクリーンを必要とせず、音声だけで本人確認の登録、伝言の残置、経路案内の取得などを行うことができます。

- **工場の安全点検パートナー**：産業環境において、作業者の手がふさがっている場合でも、ジェスチャーで音声インタラクションをトリガーし、AI アシスタントに設備の状態や操作マニュアルを尋ねたり、異常を報告したりできます。

- **アクセシビリティ支援インタラクション**：視覚障害者や身体の不自由なユーザーに音声制御の入り口を提供し、簡単なジェスチャートリガーでデバイスと自然言語による会話を行い、周囲の情報を取得したりコマンドを送信したりできるようにします。
...
| 音声認識 (STT) | PC サーバー | iFlytek Speech Dictation API | 音声からテキストへの変換 |
| 大規模モデル推論 (LLM) | PC サーバー | Spark Large Model Spark Lite | インテリジェントな応答を生成 |
| 音声合成 (TTS) | PC サーバー | iFlytek Speech Synthesis API | テキストから音声への変換 |
...
ワークフロー内の Model ノードには、いくつかの事前学習済みモデルが付属しています。ここでさまざまなモデルパラメータを選択・設定できます。このデモでは、人のポーズを検出するために YOLO11n Pose モデルを使用します。
...
Function ノードにはポーズ判定ロジックが含まれており、**肩のキーポイント間距離** と **肘のキーポイント間距離** を比較して、音声インタラクションをトリガーするかどうかを判定します。Model ノード内の Confidence と IOU パラメータを調整して誤検出を減らしたり、以下の Function ノード内のロジックコードを変更して追加機能を実装したりできます。
...
このワークフローでは、LED の制御と録音のためにシステムコマンドを実行する **Exec ノード** を使用します。該当ノードをダブルクリックし、実際の設定に合わせて reCamera の root パスワードを変更してください。
```
...
<p align="center">LED オン ノードのパラメータ設定</p>
...
返送された音声は **aplay** コマンドで再生されます。TTS モデルの出力形式（16kHz、モノラル、16 ビット）に一致するように、正しい音声パラメータを指定する必要があります。
...
[repository](https://github.com/hunter5299/Node-Red-project/tree/main/reCamera_ai_voice_assitant) から AI Voice Assistant 用のサーバーサイド Python コードを取得します。プロジェクトコードを PC にダウンロードしたら、サービスディレクトリに移動し、Python の依存関係をインストールします：
...
4. **音声処理と対話生成**
   録音完了後、音声データは HTTP Request を介して PC 上の Flask サービスに POST され、次を実行します：
...
5. **音声再生**
   PC は WAV 音声を返し、reCamera は `aplay` コマンドを通じて応答音声を再生します。

## 注意事項

- 現在の録音間隔は **10 秒** に設定されています。STT → LLM → TTS の処理時間がこの間隔を超えると、複数回のトリガーによってパイプラインが輻輳する可能性があります。処理時間を短縮するため、LLM の応答語数を制御することを推奨します（現在のシステムプロンプトでは 50 語以下に制限しています）。
...
弊社製品をお選びいただきありがとうございます。特定のカスタマイズ目標に関するガイダンスが必要な場合や、ワークフローをさらに拡張したい場合は、いつでもお気軽にお問い合わせください。製品をできるだけスムーズにご利用いただけるよう、さまざまなレベルのサポートを提供しています。お好みやニーズに応じて選べる、複数のコミュニケーションチャネルをご用意しています。
