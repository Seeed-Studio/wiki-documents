...
description: 本ドキュメントでは、公式の Google MediaPipe ハンドジェスチャー認識スイートを reCamera（Sophon/Bitmain SG200X）へ完全に移植し、「palm detection → landmark detection → embedding → classification」というパイプラインを構築して、その結果を UDP 経由で PC にストリーミングし可視化する方法を説明します。
keywords:
  - reCamera
  - ハンドジェスチャー認識
  - MediaPipe
...
url: https://wiki.seeedstudio.com/ja/reCamera/reCamera_Basic/application/recamera_hand_gesture_bak/
---
...
- **PC 1 台**（可視化用の Python 受信プログラムを実行するため。同一ローカルネットワーク上で reCamera と接続されている必要があります）
...
2. プログラムが「"[Heartbeat] Before the first retrieveFrame(RGB888) call..."」と表示されたまま停止する場合は、reCamera を再起動してください。
:::
