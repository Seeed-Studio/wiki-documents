...
url: https://wiki.seeedstudio.com/ja/reCamera/reCamera_Basic/getting_start/recamera_2002_series_quick_start_guide
---
...
reCamera へようこそ！このガイドでは、デバイスをすばやくセットアップし、強力な AI ビジョン機能を使い始めるまでの手順を説明します。初心者の方でも経験豊富なユーザーの方でも、このステップバイステップの手順に従うことで、インストール、設定、初回利用までをスムーズに進めることができます。
...
2. **コンピュータの VPN を無効化**：コンピュータで VPN を有効にしている場合は、WiFi を設定する前に **VPN をオフにしてください**。VPN によりネットワーク接続の問題が発生し、reCamera に正常にアクセスできなくなる可能性があります。
...
イーサネットポートを使用したい場合は、同梱のケーブルでルーターに接続できます。なお、これは **POE（Power over Ethernet）ではありません**。デバイスの電源投入には引き続き Type-C ケーブルが必要です。カメラの**背面のネジを外し**、ポートに接続してください。
...
電源投入時、reCamera はネットワーク設定を変更できるようアクセスポイント（AP）を起動します。スマートフォンまたはノート PC の Wi-Fi リストを開くと、`reCamera_******` という名前の Wi-Fi が表示されるはずです。
...
この AP に接続すると、自動的に Web ページがポップアップ表示されます。表示されない場合は、ブラウザで **192.168.16.1** と入力してページにアクセスできます。そこで reCamera を接続したい WiFi を選択できます。
...
- Wi-Fi またはイーサネット接続時の IP: `ifconfig` で確認するか、ルーターのネットワーク管理画面で確認してください。
...
**ステップ 3**: 数分待ってシステムの更新が完了するのを待ちます。

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/OSupgrade3.png" /></div>

**ステップ 4**: デバイスを再起動し、Web サイトをリロードします。

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/OSupgrade4.png" /></div>

**ステップ 5**: ローディングページが表示され、パスワードを変更します。その後、[this downloading instruction](https://wiki.seeedstudio.com/ja/recamera_getting_started/#access-recamera-preview-dashboard) に従って Node-RED ダッシュボード付きの最新バージョンの reCamera を体験できます。
...
デバイスを Node-RED と統合することで、ビデオストリームのプレビューや使用したいモデルの変更ができる、組み込みのダッシュボード UI サンプルを提供しています。自由にカスタマイズして、ご自身のアプリケーションと統合できます。

OS バージョンが 0.1.5 以上の場合、ログイン後すぐにプレビューダッシュボードが表示されます。
...
**ステップ 5**. ```Deploy``` をクリックしてフローがすでに起動していることを確認し、次に ```dashboard``` をクリックすると、そのダッシュボードが表示されます。

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/dashboarddownload5.png" /></div>

フローが開始されているかどうかを必ず確認してください。フローをインポートしても自動的に開始されない場合があり、その場合ダッシュボードは空になります。ドロップダウンメニューから `Start` をクリックしてください。
...
**Step 6**. より正確な結果を得るために、```IoU``` と ```Confidence``` のしきい値を調整できます。
...
このサンプルフローでは、**Counting Selection** ドロップダウンリストを使って、Person、Cat、Dog、Bottle などの目的の物体の数をカウントするデモを構築しました。
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/dashboard1.png" /></div>

フロー内の `function nodes` を変更することで、カウントする物体を追加したり、この機能自体をまったく別のコンピュータビジョンアプリケーションに変更したりできます。
<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/counting_demo.png" /></div>

どのような種類の物体が検出可能かは、`model node` を見ることで確認できます。
...
これらの提供されているワークフローでは、デフォルトで `YOLO11n detection model` が選択されています。これはリアルタイム物体検出用の最新のコンピュータビジョンモデルです。ノード内で他のモデルに置き換えることもできます。利用可能な他のモデルを確認したい場合は、この[wik](https://wiki.seeedstudio.com/ja/recamera_on_device_models/)を参照してください。

**Intersection over Union (IoU)**: IoU は、予測されたバウンディングボックスと正解のバウンディングボックスの重なりを評価するために使用される指標です。2 つのボックスの交差領域を和集合領域で割った比として計算されます。IoU の値は通常 0 から 1 の範囲です。ここでは 0 ～ 100 のスケールに標準化しており、IoU 値が 0 の場合は予測ボックスと正解ボックスの重なりがまったくないことを表します。値が 100 の場合は完全に一致し、2 つのボックスが完全に重なっていることを意味します。

**Confidence**: YOLO モデルにおける Confidence は、予測されたバウンディングボックスに物体が含まれている確率と、その予測の正確さを表します。0 から 100.0 の値を取ります。

## ダッシュボードをカスタマイズしてコミュニティで共有する

ユーザーの皆様が開発・作成したダッシュボードをコミュニティで共有していただくことを推奨しています。[GitHub](https://github.com/Seeed-Studio/OSHW-reCamera-Series/tree/main/Node-RED_Flow/public_flow) 上にフォルダを作成しました。皆様には **pull request** によってダッシュボードをそこに共有していただき、こちらで整理して公開したいと考えています。
ここでは、ダッシュボードにフィルライト機能を追加し、ノードを 1 つ追加して reCamera を制御する簡単な例を示します。

**Step 1**. ```button group``` ノードをワークスペースにドラッグし、以下の設定で編集します。
...
デバイスのパスコードを忘れたなどの理由でデバイスをリセットしたい場合は、**User** ボタンを長押ししながらデバイスに電源を接続してください。デバイスの `red light` が点滅ではなく**点灯し続ける**状態になったら、User ボタンを離します。

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/image-1.gif" /></div>

:::note
これにより、開発したアプリケーションやローカルに保存された画像・動画を含むユーザー情報が削除されます。操作は慎重に行ってください。
:::
...
複数の reCamera をお持ちの場合は、デバイス背面に記載されている **mac address** または **S/N number** で区別できます。

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/mac_address_tag.png" /></div>

または、USB または AP を介してデバイスをノート PC に接続した後、次のコマンドを使用して mac address を確認できます。
...
デフォルトのカメラビューの向きは、以下の画像のように type-c が**下向き**になっています。その他の向きにすると、学習させた検出モデルの内容によっては、モデルの精度に影響を与える可能性があることに注意してください。
