...
url: https://wiki.seeedstudio.com/ja/recamera_pro_rknn_model_conversion_legacy/
---
<!-- LEGACY PAGE (reCamera Pro wiki restructure, phase 2): this page has been superseded by Use_Your_Own_Model/rknn_toolkit2_conversion.md (https://wiki.seeedstudio.com/ja/recamera_pro_rknn_model_conversion/), which now serves the original slug /recamera_pro_rknn_model_conversion. This file is kept for history as a draft (slug /recamera_pro_rknn_model_conversion_legacy) and is excluded from production builds. Do not link here. -->

# RKNN-Toolkit2 を使った reCamera Pro 向けモデルの変換と量子化

<div align="center"><img width={1000} src="https://files.seeedstudio.com/wiki/reCamera-Pro/Secondary_Development/Model_Conversion/20260903-190057.png" alt="reCamera Pro 向けに ONNX モデルを RKNN 形式へ変換および量子化するワークフロー" /></div>
...
このガイドでは、FP16 ベースラインモデルと INT8 量子化モデルを作成します。変換処理はホスト PC 上で行い、接続された reCamera Pro は最終的な検証とデプロイにのみ必要です。

:::note 始める前に
このガイドでは **RKNN-Toolkit2 2.3.2**、Python 3.10、および `target_platform='rv1126b'` を使用します。Toolkit のバージョンは、reCamera Pro ファームウェアに同梱されている RKNN Runtime と揃えてください。別の reCamera 製品や Rockchip SoC 向けの手順は使用しないでください。
:::
...
Python のバージョンは変更可能です。RKNN-Toolkit2 は現在 Python 3.6 から 3.12 をサポートしています。使用する Python バージョンに一致する wheel をインストールしてください。wheel ファイル名中の `cp` の後ろの数字が対応する CPython バージョンを表します。

<div align="center"><img width={1000} src="https://files.seeedstudio.com/wiki/reCamera-Pro/Secondary_Development/Model_Conversion/image.png" alt="RKNN-Toolkit2 の Python wheel バージョンの例" /></div>
...
[公式 Rockchip リポジトリ](https://github.com/airockchip/rknn-toolkit2) から **2.3.2** リリースをダウンロードします。リリースパッケージ内の `rknn-toolkit2/packages/x86_64` から、CPython 3.10 に対応する wheel と requirements を選択します。
...
静的なバッチサイズ 1 の ONNX 入力を推奨します。モデルを [Netron](https://netron.app/)、ONNX checker、`model-inspect`、またはそのエクスポートプログラムで確認してください。これらの値は元のモデル実装から確認し、ファイル名から推測しないでください。
...
| リサイズポリシー | stretch、crop、letterbox | 実行時と完全に同一である必要がある |
| 出力と後処理 | scores、boxes、masks など | 予測結果を解釈するために必要 |

たとえば、アプリケーションが `[0,255]` の範囲の uint8 RGB ピクセルを入力し、モデルが `[0,1]` の float を想定している場合、`mean_values=[[0, 0, 0]]` と `std_values=[[255, 255, 255]]` を使用します。RKNN は `(input - mean) / std` を適用します。正規化が ONNX グラフ内に含まれている場合は、二重適用を避けるために恒等値（`mean=[0,0,0]`, `std=[1,1,1]`）を使用してください。
...
Ultralytics YOLO モデルの場合、Ultralytics から RKNN へ直接エクスポートできます。このとき中間の ONNX モデルが作成され、RKNN-Toolkit2 が自動的に呼び出されるため、別途 ONNX 変換スクリプトを書く必要はありません。
...
次の例では、reCamera Pro 向けの FP16 RKNN モデルをエクスポートします。`yolo11n.pt` は、たとえば `runs/detect/train/weights/best.pt` のような学習済みチェックポイントに置き換えてください。
...
`data` は、ホストからアクセス可能な画像パスを含む有効な YOLO データセット YAML を指している必要があります。reCamera Pro のデプロイシナリオを代表する画像を使用し、精度評価用には別途ホールドアウト画像を確保してください。Ultralytics は、エクスポートが成功しても、タスク精度やデバイス性能を保証するものではありません。
:::

:::tip 直接エクスポートと手動ワークフローの使い分け
...
Toolkit が未サポートのオペレータ、動的入力、無効なグラフを報告した場合は、ONNX モデルを修正または再エクスポートしてください。ビルドを完了させるためだけにオペレータを削除しないでください。

---

## 6. INT8 キャリブレーションデータセットの準備

INT8 キャリブレーション画像は、デプロイ時の入力に近いものである必要があります。実用的な出発点としては、およそ 100～500 枚の画像を使用し、想定される照明、背景、物体サイズ、視点、モーションブラー、カメラノイズ、および重要な失敗ケースをカバーしてください。キャリブレーション画像は精度評価用画像とは分けておきます。キャリブレーションセットが小さすぎると、INT8 モデルの出力が元の ONNX モデルと異なる結果になる可能性があります。

リストは 1 行につき 1 つの画像パスを記述します。PNG、JPEG、BMP、NPY 入力が一般的に使用されます。
...
:::caution 前処理を一貫させる
キャリブレーションデータでは、推論時と同じチャネル順序、リサイズポリシー、数値入力の取り扱い規約を使用する必要があります。代表的でない画像、RGB/BGR の取り違え、または前処理の不整合は、INT8 精度の大きな低下を引き起こす可能性があります。
:::
...
デフォルトの INT8 設定は、通常は最初のテストとして十分に有効です。タスクへの影響を測定した後にのみ、量子化アルゴリズム、ハイブリッド量子化、または重み圧縮を試してください。
...
2. 同じサンプルで FP16 と INT8 の結果を比較し、許容できない劣化を調査します。
3. reCamera Pro 上で、エクスポートされた `.rknn` をファームウェアに対応した RKNN Runtime で読み込みます。テンソル属性を問い合わせ、入力タイプ、レイアウト、サイズ、前処理、および後処理を検証します。
4. デバイス上でウォームアップ後のレイテンシと持続的な挙動を測定します。

:::note ホスト側検証の限界
ホスト側の RKNN シミュレータと RV1126B NPU は、異なる実行環境です。ホストでの比較は変換と前処理を確認するものですが、最終的な受け入れには、エクスポートされた `.rknn` を reCamera Pro 上でテストすることが必要です。
:::
...
| 非対応のオペレータ | 再エクスポートするか、意味的に同等なサポートされているオペレーションを使用します。 |
...
ONNX ソース、変換スクリプト、キャリブレーションリストのチェックサム、`.rknn` のチェックサム、およびエクスポートされたモデルとの前処理／後処理の取り決めを保持してください。次に、モデルを reCamera Pro RKNN Runtime C API と統合し、デバイス上でテストします。
