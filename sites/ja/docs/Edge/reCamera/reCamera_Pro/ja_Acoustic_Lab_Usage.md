...
description: この記事では、reCamera Pro の Acoustic Lab 機能を使用して環境音サンプルを収集し、カスタムサウンドモデルを学習させ、それらを音声トリガーイベントとしてデプロイする方法を紹介します。
keywords:
  - reCamera
  - reCamera Pro
  - Acoustic Lab
...
reCamera Pro には、Acoustic Lab と呼ばれるローカル音声学習専用プラットフォームが搭載されています。このプラットフォームを通じて、ユーザーは環境音サンプルを収集し、カスタムサウンドモデルを学習させ、インターネット接続なしで音声トリガーイベントとしてデプロイできます。この記事では、Acoustic Lab 内の各機能モジュールについて詳しく説明し、音声収集、モデル学習からデプロイと利用までの一連のワークフローを解説します。
...
6. **ステータス情報**：最初のパラメータはアルゴリズムの実行時間を示し、2 つ目のパラメータは現在のワークスペース内のサウンドモデル数を示します。
...
内蔵サウンドモデルに加えて、ユーザーは自身のニーズに応じてカスタムサウンドモデルを学習させることができます。
...
5. 右上の **Add category** ボタンをクリックしてサウンドカテゴリを追加し、名前を付けます（例：学習させたい音の名前）。
6. **Record** ボタンをクリックしてサウンドサンプルの録音を開始します（推奨時間：10 秒）。その後の手順はステップ 4 と同じです。
...
サウンドサンプルの収集が完了したら、モデル学習を実行できます。

1. **Hyperparameter** ボタンをクリックしてハイパーパラメータ設定画面に入ります（特別な要件がない限り、デフォルトのままにしてください）。
2. **Train model** ボタンをクリックしてサウンドモデルの学習を開始します。
...
1. Acoustic Lab 画面下部で **Deploy** オプションを見つけます。**MODELS** リストから先ほど学習したモデルを探し、クリックして選択・デプロイします。
   - (1) モデルリスト
   - (2) リアルタイム音声スペクトログラム
...
3. **Sound Event Trigger** オプションを見つけ、右側の **Configuration** ボタンをクリックしてサウンドトリガー設定画面に入ります。
4. 先ほど学習したサウンドカテゴリを選択し、**Confirm** ボタンをクリックしてサウンドトリガーの設定を完了します。
...
- 現在のワークスペースが学習時に使用したものと同じであることを確認してください。
- Acoustic Lab ページを再読み込みするか、入り直してみてください。

**問題：サウンドトリガーの感度が低い、または誤検知が多い**
...
当社製品をお選びいただきありがとうございます。私たちは、製品をできるだけスムーズにご利用いただけるよう、さまざまなレベルのサポートを提供しています。お好みやニーズに応じて選べる複数のコミュニケーションチャネルをご用意しています。
