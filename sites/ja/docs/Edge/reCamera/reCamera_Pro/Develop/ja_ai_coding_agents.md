---
description: reCamera Pro Development Skill をインストールして使用し、ONNX モデルの変換、ネイティブ RKNN アプリケーションのビルド、そして自然言語での対話を通じてカメラ、オーディオ、GStreamer、RTSP ワークフローを開発します。
title: AI コーディングエージェントで開発する
keywords:
  - reCamera
  - reCamera Pro
  - AI coding agent
...
使用しているエージェント名を明示して、対応するブランチとインストールディレクトリが選択されるようにします。
...
スキルがインストールされていれば、タスクを自然に説明するだけでも構いません。エージェントが、関連するモデル変換、クロスコンパイル、カメラ、オーディオ、ストリーミングに関するガイダンスを読み込みます。
...
2. ONNX モデルの契約を調査し、そのソース、ライセンス、入力レイアウト、正規化、色順序、出力セマンティクスを記録する。
3. `rv1126b` 向けに RKNN-Toolkit2 2.3.2 を使用してモデルを変換し、INT8 用には代表的なキャリブレーションデータを使用する。
...
| 実行ファイルが x86-64 である | aarch64 クロスコンパイラではなくホストコンパイラが使用された | ターゲット互換のコンパイラと reCamera Pro sysroot を用いて再ビルドする |
