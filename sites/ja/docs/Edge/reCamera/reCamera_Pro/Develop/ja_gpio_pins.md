...
この記事では、reCamera Pro 上で GPIO ピンを使用する方法について説明します。ピン番号の計算式を用いて目的の GPIO ピンを特定する方法、sysfs インターフェースを介してピンをエクスポートし方向やレベルを設定する方法、そして Python スクリプトを使って GPIO 出力を制御したり入力レベルを読み取ったりする方法を学びます。

:::note
reCamera Pro には外部に露出している汎用 GPIO が 2 本しかなく、その出力電圧は 12V〜21V（DC-IN 入力に依存）であり、ほとんどのデバイスには適していません。ただし、拡張インターフェース上の UART または CAN ピンを汎用 GPIO として再設定することができます。これらのピンは標準的な 3.3V レベルを出力します。
:::
...
バンク内のオフセットは次のように計算されます：`グループ番号 × 8 + グループ内ピン番号`。
...
まず、デバイスのターミナルにアクセスする必要があります。次のいずれかの方法で行えます。
...
- [reCamera Pro 公式ドキュメント](/ja/recamera_pro_getting_started/)

## テクニカルサポートと製品ディスカッション

弊社製品をお選びいただきありがとうございます。私たちは、製品をできるだけスムーズにご利用いただけるよう、さまざまなサポートオプションを用意しています。お好みやニーズに合わせて選べる複数のコミュニケーションチャネルを提供しています。
