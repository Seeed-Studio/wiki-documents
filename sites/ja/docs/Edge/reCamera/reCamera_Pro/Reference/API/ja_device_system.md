...
| `displaySteps` | 現在のバッテリー残量ステップ数 |
| `totalSteps` | バッテリー残量の総ステップ数 |
...
設定をインポートすると、現在のデバイス設定は上書きされます。事前に現在の設定をエクスポートしてバックアップしてください。
...
工場出荷時リセットを行うと、デバイスのすべての設定が消去されます。実行前に必ずバックアップを取得してください。
