---
description: このページでは、すべてのインターフェースに共通するルール（レスポンス形式、フィールド命名規則、URL の連結方法など）について説明します。最初にこのページを読むことを推奨します。そうすることで、後から個別のインターフェースを確認する際に、このページへ頻繁に戻って参照する必要がなくなります。
title: 共通規約
keywords:
  - Edge AI
  - reCamera Pro
