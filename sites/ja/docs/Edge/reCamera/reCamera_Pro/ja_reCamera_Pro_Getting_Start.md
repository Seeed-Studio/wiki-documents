...
:::tip よくあるトラブルシューティング
Windows システムで `192.168.42.1` を入力してもページが読み込まれない場合は、本ドキュメントの末尾にある [4. よくある質問とトラブルシューティング (FAQ)](#4-よくある質問とトラブルシューティング-faq) セクションに直接移動し、ネットワークアダプタの確認を行ってください。
:::
...
1. Web メインインターフェースへのログインに成功したら、左側のナビゲーションバーで **Device Info** をクリックします。
2. サブメニューまたは右側の設定エリアで **Network Settings** をクリックします。
3. Wi-Fi 機能を有効にします（デフォルトでオフの場合）。有効にすると、システムが周辺の無線ネットワークを自動的にスキャンします。
...
Wi-Fi によって割り当てられたローカルエリアネットワーク IP（例：`192.168.4.200`）を取得した後は、コンピュータと reCamera Pro が同じローカルエリアネットワーク内にある限り、ブラウザにこの IP アドレス（例：`http://192.168.4.200`）を直接入力してデバイスにログインできます。
...
reCamera を USB ケーブルで接続した後、MAC コンピュータがインターネットに接続できない場合、仮想ネットワークカードが MAC の WIFI を引き継いでしまい、MAC がインターネットに接続できなくなることが原因です。次の手順に従って問題を解決してください：

1. ***MAC のシステム設定 -> ネットワーク*** を開きます。
2. 右下（または右下側）にある、***内部に 3 つのドット「...」がある円***（または MAC のバージョンによっては下向き矢印）のアイコンを探します。
3. ***サービスの順序を設定...(Set Service Order...)*** をクリックします。
4. 表示されたリストで、WIFI を一番上にドラッグし、USB 仮想ネットワークカード（USB_Gadget など）より前に配置されていることを確認します。
5. 「OK」をクリックして保存します。これにより、MAC でインターネットに接続する際は優先的に WIFI 接続が使用され、reCamera にアクセスする場合のみ USB 接続が使用されるようになります。
