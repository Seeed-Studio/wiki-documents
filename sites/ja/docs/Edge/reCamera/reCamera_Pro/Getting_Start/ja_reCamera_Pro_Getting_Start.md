...
url: https://wiki.seeedstudio.com/ja/recamera_pro_getting_started_legacy/
---
<!-- LEGACY PAGE (reCamera Pro wiki restructure, phase 2): this page has been superseded by Getting_Started/first_ai_detection.md (https://wiki.seeedstudio.com/ja/recamera_pro_getting_started/), which now serves the original slug /recamera_pro_getting_started. This file is kept for history as a draft (slug /recamera_pro_getting_started_legacy) and is excluded from production builds. Do not link here. -->

## コンテンツ移行インデックス（レガシーページ）

このページは reCamera Pro Wiki 再構成（フェーズ 2）において分割されました。各セクションの移行先は次のとおりです：

| 元のセクション | 新しい場所 |
| --- | --- |
| Introduction、§1 Hardware Connection、§2 Web UI Login | [First AI Detection](https://wiki.seeedstudio.com/ja/recamera_pro_getting_started/)（このスラッグ） |
| §3 Wi-Fi Configuration | [Wi-Fi と Web UI アクセス](https://wiki.seeedstudio.com/ja/recamera_pro_wifi_access/) |
| §4 Resetting the Device | [ファームウェアの更新と復旧](https://wiki.seeedstudio.com/ja/recamera_pro_firmware_update/) |
| §5 FAQ & Troubleshooting | [トラブルシューティング](https://wiki.seeedstudio.com/ja/recamera_pro_faqs/) |
| 概要サマリー | [reCamera Pro ホーム](https://wiki.seeedstudio.com/ja/recamera_pro/) |
...
3. Wi-Fi 機能を有効にします（デフォルトでオフの場合）。システムが自動的に周辺の無線ネットワークをスキャンします。
...
デバイスを工場出荷時設定に戻す方法は 2 つあります：

1. 側面のボタンを約 15 秒間長押しし、プロンプト音が聞こえるまで押し続けます。これにより、デバイスがリセットモードに入ったことを示します。この状態で、緑色のステータスランプが連続点灯するまで待ち、再度デバイスにログインします。
...
USB ケーブルでデバイスに接続した後、コンピュータがインターネットに接続できない場合は、次を参照してください：[ネットワークエラー図](https://wiki.seeedstudio.com/ja/recamera_network_connection/#computer-network-error)

---

## 技術サポートと製品ディスカッション

当社製品をお選びいただきありがとうございます。お客様が当社製品をスムーズにご利用いただけるよう、さまざまなサポートを提供しています。お好みやニーズに応じて選択いただける複数のコミュニケーションチャネルをご用意しています。
