...
:::tip よくあるトラブルシューティング
Windows システムで `192.168.42.1` を入力してもページが読み込まれない場合は、本ドキュメントの末尾にある [4. Frequently Asked Questions and Troubleshooting (FAQ)](#4-よくある質問とトラブルシューティングfaq) セクションに直接移動し、ネットワークアダプタの確認を行ってください。
:::
...
デバイスをインターネットに接続したり、ローカルエリアネットワーク内でワイヤレス管理を行えるようにするには、Wi-Fi 接続を設定してください。
...
この時点で、**USB Type-C ケーブルを安全に抜去**して、完全なワイヤレスでの操作および管理を実現できます。
:::
...
reCamera を USB ケーブルで接続した後、MAC コンピュータがインターネットに接続できない場合、仮想ネットワークカードが MAC の WIFI を占有し、MAC がインターネットに接続できなくなるためです。以下の手順に従って問題を解決してください：

1. ***MAC のシステム設定 -> ネットワーク*** を開きます。
2. 右下（または右下付近）に、***内部に 3 つのドット「...」がある円***（または MAC のバージョンによっては下向き矢印）のアイコンを探します。
