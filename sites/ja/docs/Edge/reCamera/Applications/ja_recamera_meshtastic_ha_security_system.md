...
このチュートリアルでは、かなりギークな「ネットワーク不要セキュリティ」ソリューションを提供します。つまり、**reCamera PoE** の強力なエッジ AI 演算能力を活用してリアルタイムの人物検出を行います。ターゲットを検出すると、基板上のハードウェアピンを介して即座に **XIAO ESP32S3 + Wio-SX1262 モジュール（送信機）** をトリガーします。アラート信号は低周波の電波に変換され、数キロメートルにわたる複雑な地形を伝搬して屋内の **Wio-SX1262 受信機** に捕捉され、ローカルの **Home Assistant (HA)** システムへプッシュされる MQTT メッセージへとシームレスに変換されます。  

この一連の経路ではクラウドサーバーを一切必要とせず、プライバシー・低消費電力・超長距離通信を高い次元で両立できます。  
...
- reCamera PoE が YOLO 人物検出モデルを実行し、人を検出すると GPIO ピンから Low レベル信号を出力する  
- XIAO ESP32S3（送信機）が D2 ピンで Low レベルのトリガーを検出し、Wio-SX1262 を介して LoRa 無線パケットを送信する  
- XIAO ESP32S3（受信機）が Wio-SX1262 経由で LoRa パケットを受信し、MQTT プロトコルを用いて WiFi 経由で Home Assistant にプッシュする  
...
- ジャンパワイヤ延長ケーブル：数本。reCamera の GPIO と XIAO ESP32S3 間の物理信号配線に使用  
...
**Step 1**: XIAO ESP32S3 を USB-C データケーブルでパソコンに接続します。  
...
**Step 4**: **Settings → Radio Config → LoRa** に移動し、`Region` をお住まいの地域で合法な周波数帯（例：`CN` や `US`）に変更し、`Modem Preset` を応答速度を高めるために **`Short Turbo`** に設定します。  
...
:::note Note  
必ずこのキーをコピーしてオフラインで保存してください。後で送信機側に入力する必要があり、両端のキーは完全に一致していなければなりません。  
:::  
...
**Step 2 — ローカルネットワークへの接続**: **Device Config → Network** で WiFi を有効にし、自宅の WiFi の SSID とパスワードを入力します。  
...
送信機は GPIO ピンのレベル変化を監視し、Low レベルを検出したときに LoRa アラートパケットを送信する役割を担います。  

:::note Note  
送信機側では WiFi と MQTT を**有効にしないでください**。GPIO トリガーの受信と LoRa 無線信号の送信に専念させます。  
:::  

**Step 1**: 受信機側の ESP32 を取り外し、送信機用の ESP32 に差し替え、Meshtastic Web Client で **Connect** をクリックしてそのまま接続します。  

**Step 2 — RF パラメータの同期**: **Radio Config → LoRa** に移動し、受信機側と一致していることを確認します（Region と Modem Preset が完全に同じであること）。MQTT を有効にする必要はありません。  
...
**Step 3 — Pre-Shared Key の入力**: **Channels** タブで、受信機側で生成した `Pre-Shared Key` を余さず貼り付けます。  
...
| Detection Triggered Type | LOGIC_LOW | Low レベルトリガー（reCamera は人物検出時に Low レベルを出力） |  
...
reCamera のターミナルで次のコマンドを実行し、オンボードの YOLO 人物検出モデルを起動します。最初のパラメータはモデルファイルのパス（reCamera 付属のモデルをそのまま使用可能）、2 つ目のパラメータはビデオストリーム転送スクリプトを実行しているホストの IP アドレスです。  

```bash
sudo ./model_detector ./Models/model.cvimodel 192.168.4.7
```  

### 4.4 ESP32 送信機を reCamera に接続する  

上記のソフトウェア設定が完了したら、ジャンパーワイヤーを使用して XIAO ESP32S3（送信機）を reCamera に物理的に接続します：  

- 黒いケーブル：ESP32 の **GND** ピン → reCamera の **GND** ピン に接続  
- 緑のケーブル：ESP32 の **D2** ピン → reCamera の GPIO 信号出力ピン に接続  
...
:::note Note  
LoRa モジュールが十分な送信電力を確保できるように、ESP32 送信機には（USB ケーブルまたはバッテリー経由で）別途電源を供給してください。  
:::  
...
開始する前に、受信機側の ESP32 の電源が入っており動作していること、HA ホストと同じ WiFi ネットワークに接続されていること、MQTT アドレスが正しく設定されていることを確認してください。  
...
reCamera が人を検知すると、Meshtastic 送信機は LoRa 経由で受信機にアラート情報を送信し、その後受信機は `Human`（送信機側で設定した Friendly Name）を含むメッセージを MQTT 経由で HA にプッシュします。MQTT リッスンインターフェース上で、リアルタイムにアラートメッセージを確認できます：  
...
Home Assistant でオートメーションルールを作成し、MQTT メッセージ内の `payload.text` が `Human detect` のときに（モバイル通知、アラームとの連動、ログ記録などの）アラートアクションをトリガーするように設定することで、「ネットワーク不要のセキュリティ」システム全体のクローズドループを完成させます。  
...
- 送信機と受信機の `Pre-Shared Key` は同一である必要があります。異なる場合、パケットを正しく復号できません  
...
両端の LoRa `Region` と `Modem Preset` が完全に同一であるか、`Pre-Shared Key` が一致しているかを確認してください。さらに、2 台のデバイス間の距離が有効な LoRa 通信範囲内であるか、アンテナが正しく取り付けられているかも確認してください。  

### Q2: reCamera が人を検知しているのに、HA がアラートを受信しない場合は？  

次の順序で切り分けを行ってください：reCamera の GPIO 信号出力が正常かどうか（テスターで測定可能）；送信機側の Detection Sensor モジュールの Monitor Pin が正しく 3（D2）に設定されているか；受信機側の WiFi および MQTT 設定が正しいか、MQTT Address に HA ホストの IP が入力されているか。  

### Q3: Meshtastic 受信機が WiFi と MQTT に正常に接続できているかを確認するには？  
...
当社製品をお選びいただきありがとうございます。製品をできるだけスムーズにご利用いただけるよう、さまざまなレベルのサポートを提供しています。お好みやニーズに合わせて選べる複数のコミュニケーションチャネルをご用意しています。  
