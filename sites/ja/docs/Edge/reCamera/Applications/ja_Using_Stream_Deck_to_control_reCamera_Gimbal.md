...
- **ロジック概要**:
  - デフォルトのズーム倍率は `1.0` です。
  - 回転中（cw で増加 / ccw で減少）に **0.1 ステップ** でズームレベルを調整し、押してリセットすると `1.0` に戻します。
...
まず、このガイドに従って reCamera を最新バージョン 0.2.4 にアップグレードしてください: [reCamera OS アップグレードチュートリアル](https://wiki.seeedstudio.com/cn/recamera_os_version_control/)。すでにバージョン 0.2.4 を実行している場合は、この手順をスキップできます。その後、右上の **"Deploy"** ボタンをクリックして、新しく作成したノードをボードにデプロイします。
