---
description: このデモでは、reCamera 上でリアルタイムの顔検出、属性解析（年齢/性別/人種）、感情認識を行い、その結果を UDP 経由で PC にストリーミングして表示する方法を示します。
title: reCamera での UDP 顔解析
keywords:
  - reCamera
  - 顔検出
  - 年齢 性別 人種
...
このデモでは、reCamera を使ってリアルタイムの顔解析システムを構築する方法を説明します。このシステムは次の処理を行います：
