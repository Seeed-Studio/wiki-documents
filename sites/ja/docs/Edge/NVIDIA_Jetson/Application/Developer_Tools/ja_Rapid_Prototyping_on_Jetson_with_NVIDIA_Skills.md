---
description: デバイス検査から実行可能なプロトタイプ作成とデモ用パッケージングまで、Jetson アプリケーション提供を加速する、再現可能なスキル駆動ワークフローを学びます。
title: NVIDIA Skills を用いた Jetson 上での高速プロトタイピング
keywords:
  - Jetson
  - NVIDIA Skills
  - Claude Code
...
url: https://wiki.seeedstudio.com/ja/rapid_prototyping_on_jetson_with_nvidia_skills/
---

# NVIDIA Skills を用いた Jetson 上での高速プロトタイピング
...
Jetson プロジェクトが行き詰まる原因は、開発者がモデルコードを書けないからではほとんどありません。多くの場合、もっと前段階の、デプロイ周りの「つなぎ作業」で止まります。具体的には、デバイス状態の確認、JetPack 互換性の確認、実現可能なインストールパスの選定、最初の実行可能なパイプラインの構築、出力の検証、そして結果をデモ可能な形にパッケージングする作業です。実プロジェクトでは、最初の数日がしばしばこの部分で消えてしまいます。

ここで役に立つのが **エージェント + スキル ワークフロー** です。Jetson に特化したスキルは、デバイス診断、メモリ検査、環境準備、モデルインポートなど、限定されたタスク群に対して、AI コーディングエージェントに構造化された手順を与えます。何もないシェルから始めて、散在するドキュメントからワークフローを再構成する代わりに、Jetson を理解したデリバリーパスからスタートできます。

この手法は、特定のエージェントフレームワークに結び付いているわけではありません。再利用可能なスキルを読み込み、ファイルを検査し、コマンドを実行し、リモートデバイスと対話できるあらゆるエージェントが、同じパターンの恩恵を受けられます。この Wiki では、ワークフロー全体を分かりやすく示せるという理由から、例として **Claude Code** を使用します。
...
:::note
この記事でいう「Skills」とは、**NVIDIA によって作成され、ワークフローで使用するコーディングエージェントに読み込まれる再利用可能なエージェントスキル** を指します。これは、Seeed の **Jetson DevelopTool の「Skills」モジュール**（別の GUI 自動化機能）とは異なります。その GUI モジュールを探している場合は、[DevelopTool Skills モジュール](/ja/jetson_developtool_skills/) の記事を参照してください。
:::

## NVIDIA Skills とは

**スキル** とは、AI コーディングエージェントがオンデマンドで読み込む、再利用可能な手順と参考資料の束です。スキルを呼び出すと、エージェントはタスク固有のガイダンスを受け取ります。何を検査するか、どの規約に従うか、どのツールを使うか、どの出力を検証するか、といった内容です。Claude Code では、`/<skill-name>` という名前でスキルを明示的に指定するのは、その挙動をトリガーする方法の 1 つにすぎません。いったんスキルがワークスペースにインストールされると、エージェントはそれらの中から自動的に選択することもできます。

Jetson 開発者にとって特に関連性の高い NVIDIA スキルリポジトリが 2 つあります：

- Jetson 向けの一般的なワークフロー用 **[NVIDIA/skills](https://github.com/NVIDIA/skills)**

この記事では、これらのスキルを使って、繰り返し発生する立ち上げ作業を減らし、「デバイスが利用可能」な状態から「プロトタイプが検証済み」になるまでの道のりを短縮します。

## Jetson のデリバリーが遅くなる理由

Jetson アプリケーションの最初のバージョンでは、ハードウェアステータス、JetPack とランタイムの互換性、ARM 上でのパッケージインストールの選択、モデルの配置、起動コマンド、出力パス、そして最低限の性能検証を調整する必要があります。問題は、どれか 1 つのステップが不可能であることではありません。問題は、これらのステップがさまざまなツールやドキュメント、場当たり的なシェル作業に分散しているため、本来より長くデリバリーパスがあいまいなままになってしまうことです。

スキル駆動ワークフローの実用的な価値は、次のような状態を：

| 従来のパス | スキル駆動のパス |
| --- | --- |
| 行動前に複数のドキュメントを読む | タスクにフォーカスしたスキルから始める |
| デバイス状態を手作業で確認する | エージェントに Jetson を診断させる |
| 依存関係パスを当てずっぽうで決める | エージェントに Jetson を理解したインストールパスを使わせる |
| 最初の試作プロトタイプを手作業で構築する | エージェントに最小限の実行可能バージョンをスキャフォールドさせる |
| 成果物の内容を手作業でチームメイトに説明する | エージェントにファイル、コマンド、出力を要約させる |
| デモ用インターフェースをゼロから素早く作る | エージェントにプロトタイプを共有用にラップさせる |

## スキルが得意なこと

Jetson の作業において、スキルが最も力を発揮するのは、タスクがまだ不確実性の削減に関する段階にあるときです。つまり、現在のデバイスを検査すること、より Jetson に適したセットアップパスを選ぶこと、モデルやテストアセットの周りに最初の実行可能なプロトタイプを生成すること、作成されたものを説明すること、そして結果を、表示や再実行が可能な形にパッケージングすることです。一方で、スキルは本番エンジニアリングの代替としてはあまり有用ではありません。スキルは立ち上げと検証を加速できますが、最高の性能、本番レベルの信頼性、利用シーンに対する精密な精度、スケールや保守性を見据えた最終的なアーキテクチャ選定を保証するものではありません。この境界は重要です。というのも、このワークフローの目的は、信頼できるベースラインに素早く到達し、そのうえで意図的に堅牢化していくことだからです。
...
すべてのプロジェクトに、すべてのスキルが必要なわけではありません。よりよい考え方は、特定のブロッカーが現れたときに、適切なスキルを使うというメンタルモデルです。

### まずは `jetson-diagnostic` から始める

Jetson がデプロイ準備完了かどうか、まだ分からない場合に最初に使います。実際には、エージェントに十分なライブデバイスコンテキストを与え、ワークフローの残りを通常ブロックする質問、すなわち、どの Jetson モデルが動作しているか、どの JetPack/L4T バージョンがインストールされているか、どれだけメモリが空いているか、デバイスが熱的に制約されているかどうか、主要なサービスやハードウェアエンドポイントが見えているかどうか、といった点に答えられるようにします。
...
Jetson では、x86 Linux や一般的な AI チュートリアルからコピーしたパッケージインストール手順は、しばしば失敗します。`jetson-package` は、エージェントに Jetson を理解したソースやデプロイパスを優先させたいとき、特に PyTorch や OpenCV などの依存関係について、ホストパッケージ、wheel、コンテナのどれを選ぶかを決める際に使用します。
...
モデルのロードが遅い、プロセスが kill される、推論負荷の下でデバイスが不安定に感じられる、といった場合に有用です。現在 DRAM を何が使用しているか、以前のプロセスが本当にメモリを解放したか、設定変更によってもっともらしく聞こえるだけでなく、実際に測定可能な差が出たかどうかについて、エージェントにデータに基づく見方を与えます。
...
### デバイスがアプライアンスとして動作している場合は `jetson-headless-mode` を使う
...
デバイスと環境が把握できたら、次の目標は完璧さではありません。目標は、出力を検査可能な **最小限の有用なプロトタイプ** に到達することです。

以下の例では、ビルドステップとして `deepstream-import-vision-model` を使用します。

## プロトタイプの受け入れ基準

最適化をさらに進める前に、何をもってプロトタイプが成功と見なせるかを定義します。最初の Jetson プロトタイプは、明確なコマンドで再実行でき、入力パスと出力パスが明示され、結果をすぐに確認でき、現在のリソース状況がもはや謎でなくなっているとき、たいてい十分な状態と言えます。主なボトルネックが見えていて、次のエンジニアリングステップが自明であれば、そのプロトタイプはすでに役目を果たしています。最初のパスの目的は、製品を完成させることではありません。不確実性を十分に素早く減らし、次の反復を当てずっぽうではなく意図的なものにすることです。

## 前提条件

例のワークフローに従う前に、次のものを準備してください：
...
- Jetson 上に、モデルのダウンロード、一時ファイル、出力用の十分なストレージ
- すでに Jetson 上に配置されている、ビデオクリップなどの **テストアセット**

この例では、テスト用ビデオ `top_view.mp4` が Jetson 上の `/home/seeed/workspace/traffic_monitor/` 配下に配置されているものとします。
...
`claude` コマンドを起動したら、`/<skill-name>` と入力して、エージェントに特定のスキルを明示的に読ませることができます。これは、どのワークフローを使いたいかすでに分かっている場合に便利です。しかし、一度スキルがワークスペースにインストールされると、エージェントはそれらの存在を把握しており、あなたのリクエストに応じて自動的に選択できます。実際には、診断、パッケージ選択、メモリ検査、プロトタイプ生成をそれぞれ別々のセッションとして扱うのではなく、1 回のパスで複数のスキルを連鎖させて組み合わせることがあります。
...
まず、Jetson が実際にビジョンワークロードを処理できる状態かどうかを確認します。次のようなプロンプトを使うと、エージェントはデバイスを調査し、生のテレメトリをただダンプするのではなく、プロトタイプにとって重要なブロッカーを報告するのに十分なコンテキストを得られます。
...
```
:::note
上記の IP アドレスとパスワードは例示値です。自分の Jetson の認証情報に置き換えてください。`seeed` は一部の Seeed Jetson デバイスでのデフォルトパスワードであり、本番機では変更する必要があります。
:::
...
診断ステップで、プロトタイプを実行するには環境が十分クリーンでないことが分かった場合は、むやみに先へ進むのではなく、このタイミングで準備を行います。`jetson-package` は、エージェントが Jetson 互換の依存関係パスを選択するのに役立ち、`jetson-memory-audit` と `jetson-headless-mode` は、メモリプレッシャーや不要なデスクトップのオーバーヘッドが最初の障害になる場合に有用です。
...
実務的な利点としては、前段の診断で本当に必要だと分かったときにだけセットアップ作業に時間を費やせる、という点があります。

### ステップ 3. 最小限の実行可能なプロトタイプを構築する

ここからは、環境の準備完了からアプリケーションの立ち上げへと進みます。
...
このステップの終わりまでに、メインの推論スクリプトまたはアプリのエントリポイント、明確な再実行パス、注釈付きの出力動画などの成果物、そして生成されたファイルが既知のワークスペースに配置されている状態を目指します。これらの要素が揃っていれば、最終的なスタックがまだ変更の余地を残していても、ベースラインはすでに有用です。
...
ここからワークフローの効果が現れ始めます。最初のパスでスクリプトやパスを手作業で配線する代わりに、実行可能なベースラインを評価し、どこにより深いエンジニアリング投資を行うべきかを判断できます。

### ステップ 4. 出力を検証しアーキテクチャを確認する

最初のパスができたら、エージェントに対して、何を作成したのか、そしてそれをどう再実行するのかを説明させます。
...
アルゴリズムがコマンドラインから動作するようになったら、次に残るギャップはたいていデモ性です。
...
| モデルの読み込みに失敗する、または推論が不安定 | メモリプレッシャーが最初のボトルネックになっている | `jetson-memory-audit` を使用する |
...
| デモがターミナル上でしか動作しない | エンジニアリングのベースラインは存在するが、共有可能ではない | エージェントに Web UI またはサービスラッパーとしてパッケージングするよう依頼する |

## プロトタイプから本番へ

このワークフローは、探索、立ち上げ、検証の段階で最も力を発揮します。ベースラインが実証されたら、次のステップは通常、手動でのハードニングです。
...
ここでの主なポイントは、特定のエージェントが特定の車両速度検出デモを構築できる、ということではありません。より有用なポイントは、**スキルを構造化されたワークフローとして扱うことで、Jetson アプリケーションのデリバリーが大幅に高速化される**ということです。まずデバイスを診断し、必要な箇所だけを準備し、最小限の実行可能なプロトタイプを生成し、出力とボトルネックを検証し、その結果を共有可能な形にパッケージングする。これが再現性のあるパターンです。車両速度の例は、その一つのインスタンスに過ぎません。
