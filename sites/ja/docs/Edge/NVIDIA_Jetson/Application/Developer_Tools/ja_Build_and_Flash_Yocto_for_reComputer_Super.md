---
description: Seeed リポジトリから完全な CUDA 開発 Yocto イメージをビルドし、reComputer Super にフラッシュします。
...
url: https://wiki.seeedstudio.com/ja/build_and_flash_yocto_for_recomputer_super/
---
...
- ホストパッケージのインストールおよびフラッシュ用の `sudo` 権限
...
3. 電源を接続します。
4. データ通信対応ケーブルで、USB Type-C デバイス/デバッグポートを Linux ホストに接続します。
