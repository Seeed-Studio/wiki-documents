...
[EasySkyMesh](https://github.com/IoTThinks/EasySkyMesh) は、低消費電力ファームウェアの構築に焦点を当てた [MeshCore](https://meshcore.io/) ベースのコミュニティプロジェクトです。このファームウェアにより、L1 Pro は MeshCore 通信機能を維持しながら **18 日間** のバッテリー駆動を実現できます。
...
EasySkyMesh は、**省電力とセンサーデータ収集** のための機能を追加します。センサーデータは MeshCore ノードによって収集され、既存の MeshCore ネットワークを介して送信できます。これは特に次の用途に有用です：
...
EasySkyMesh のリリースでは、ハードウェアやアップグレードシナリオの違いに応じて、異なるファームウェアファイルが提供される場合があります。新規インストールの場合、プロジェクトは一般的にクリーンインストール用のファームウェアイメージを提供します。既存インストールがある場合、既存の設定を保持したいときはアップグレード用ファームウェアを使用してください。
...
PowerSaving はデフォルトではオフになっています。MeshCore CLI を通じて PowerSaving を有効にできます。
...
さらに多くのコマンドを使用するには、[こちらをクリック](https://github.com/IoTThinks/EasySkyMesh/wiki/PowerSaving#11-repeaters-and-room-servers)してください。

## センサーサポート

EasySkyMesh は MeshCore を拡張してセンサーサポートを追加し、センサーの測定値を収集して既存の MeshCore ネットワークを介して送信できるようにします。現在、このプロジェクトでは、いくつかの一般的な I2C センサーのサポートが文書化されており、次のものが含まれます：
...
| VL53L0X       | 距離                                    | `0x29`      |
 ...
 一部のファームウェアバージョンでは、BME280、BME680、BMP280 などのセンサーに対して I2C アドレスを自動的に選択できます。対応する EasySkyMesh バージョンでは、`sensor` CLI コマンドを使用して I2C および GPS シリアルピンを確認できます：
