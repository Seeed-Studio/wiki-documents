...
# Wio Tracker L1 を用いた MeshCore 上での AI 画像伝送
...
画像は**直接送信されるわけではありません**。送信側はローカルの AI エンコーダを使用して、画像を非常に小さなバイナリファイルに変換します。受信側は元のピクセルを復元しません。その代わりに、ローカルの AI デコーダが圧縮データを使用して、見た目が類似した画像を生成します。
...
1. MeshCore Open App をインストールします。[こちらをクリック](https://discord.com/channels/@me/1547501987703037965/1547520066478936134)して、画像伝送をサポートする APP リリースをインストールしてください。
2. MeshCore ノードを APP に接続します。[こちらをクリック](https://wiki.seeedstudio.com/ja/get_started_with_other_mesh_firmware/#connect-via-app)して、L1 接続ガイドを参照してください。
...
APP の設定で画像伝送を有効にし、ml モデルファイルをダウンロードします。
...
受信ノードが圧縮データを受信すると、デコーダが画像再構成を実行します。再構成された画像はチャット画面に表示されます。
<div style={{textAlign:'center'}}>
...
出力画像は、元の画像をピクセル単位で完全にコピーしたものではありません。デコーダは、送信されていない追加のディテールを生成する場合があります。そのため、この機能はシーン認識、アウトドアでの通信、遠隔監視、視覚情報の共有に適しています。一方で、証拠収集、本人確認、科学的な画像解析、正確な画像再現を必要とする用途には適していません。
