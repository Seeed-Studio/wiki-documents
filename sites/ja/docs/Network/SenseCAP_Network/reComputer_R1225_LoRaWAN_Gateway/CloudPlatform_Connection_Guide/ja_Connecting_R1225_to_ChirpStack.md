...
ChirpStack は、LoRaWAN ネットワーク向けのオープンソースコンポーネントを提供します。これらを組み合わせることで、デバイス管理用の使いやすい Web インターフェースと統合用の API を含む、すぐに利用可能なソリューションを構成できます。
...
:::note
MAC version / Regional parameters revision / ADR algorithm はデバイスに応じて選択してください。詳細については、https://lora-alliance.org/resource_hub/ を参照してください。
:::
...
Web UI を介してゲートウェイを設定します。まずローカルコンソールにログインするには、[Quick Start](https://files.seeedstudio.com/products/SenseCAP/M2_Multi-Platform_Gateway/Quick_Start_for_SenseCAP_Gateway_&_Sensors.pdf) を確認してください。
...
実際の選択に応じて、Region と Frequency プランを選択します。
...
LoRaWAN デバイスを ChirpStack に追加した後、デバイスが（OTAA の場合）アクティベートしてデータを送信できることを確認します。
