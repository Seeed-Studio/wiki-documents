---
description: MeshCore ファームウェアのソースコード環境を構築し、ソーラーノードをコンパイルしてファームウェアを書き込むための実践的なチュートリアルです。
...
プロジェクトを配置したいフォルダを開きます。そのフォルダをターミナルで開きます。[ここをクリック](https://github.com/meshcore-dev/MeshCore)してプロジェクトを git clone します。
...
対象ボード用の environment を探します。ここでは例として Solar node repeater を使用します：
...
コードを変更します。対象ボードの `variant.h` ファイルを変更することを推奨します。
...
コンパイルして、生成された uf2 ファイルをソーラーノードに書き込みます。