...
- Windows の場合、Python が `PATH` に追加されていることを確認するか、ターミナルを再起動してから再試行してください。
- macOS の場合、まず `python3` / `pip3` が既に存在するか確認し、必要な場合のみ Homebrew で Python をインストールします。
...
- 値がそこに表示されなくても、メッシュリンクが失敗していることを自動的に意味するわけではありません。
