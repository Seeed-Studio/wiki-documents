...
## MeshCore Node Map を使用する理由

Node Map はノードの位置をプロットすることで、[MeshCore](https://meshcore.io/#hardware) ネットワークのリアルタイムな可視化を提供し、ユーザーがネットワークトポロジーやノード分布を把握できるようにします。 
...
MeshCore Node Map は、ノードが定期的に地理的な位置情報をネットワークに報告することに依存しています。これは特に次のような用途に有効です：

- **近くのリピータの発見**: 利用可能なリピータを特定して接続性を拡張する
- **自分のネットワークの監視**: ノードの状態と位置を追跡する
- **デバッグと最適化**: リピータやゲートウェイの配置を最適化する


## MeshCore 公式マップのチュートリアル

最も一般的に使われている MeshCore Node Map は公式の `MeshCore Node Map` です。[Click here](https://map.meshcore.io/) をクリックしてウェブサイトにアクセスします。

### コンパニオンの設定

コンパニオンノードを MeshCore Node Map 上に表示するには、そのデバイスが位置情報を送信するように正しく設定されている必要があります。以下のように設定します：
...
リピータまたはルームサーバーを MeshCore Node Map 上に表示するには、そのデバイスが位置情報を送信するように正しく設定されている必要があります。 

まずリピータにログインします。リピータのデフォルトの管理者パスワードは `password` です。 
...
GPS を有効にして位置情報を取得します（一部のリピータファームウェアはリアルタイム位置情報の更新をサポートしていないため、その場合は手動で位置を選択する必要があります）。
...
デバイス名、ロール、設定情報を確認できます。そのため、このマップを使用することで、稼働中のノードがどこにあるかを簡単に把握できます。

## （上級者向け）独自の Node Map フロントエンドを構築する

上級ユーザーは、カスタムの Node Map フロントエンドを構築することも可能です。次のワークフローを使用することを推奨します： 
