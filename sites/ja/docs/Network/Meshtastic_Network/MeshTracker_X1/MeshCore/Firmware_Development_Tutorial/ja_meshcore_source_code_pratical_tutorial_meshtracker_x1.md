---
description: MeshCore ファームウェアのソースコード環境をセットアップし、MeshTracker X1 をコンパイルしてファームウェアを書き込むための実践的なチュートリアルです。
...
url: https://wiki.seeedstudio.com/ja/meshcore_source_code_pratical_tutorial_meshtracker_x1/
...
対象ボード用の environment を探します。ここでは T1000-E Bluetooth Companion を例に説明します：
...
デバイスのボタンを押し続けたまま、充電ケーブルを素早く 2 回接続すると、緑色の LED が点灯し続けます。ポップアップしたディスクに uf2 ファイルをドラッグします。uf2 ファイルは `.pio\build\t1000e_companion_radio_ble` にあります。
...
- 再現手順を正確に記述する
- 使用ボード、IDE バージョン、ライブラリバージョン、および関連するコードスニペットを含める
...
→ 事前の issue は不要なので、そのまま pull request を作成してください
