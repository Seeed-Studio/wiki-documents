---
description: Meshtastic ファームウェアのソースコード環境をセットアップし、MeshTracker X1 をビルドしてファームウェアを書き込むための実践チュートリアルです。
...
url: https://wiki.seeedstudio.com/ja/meshtastic_source_code_practical_tutorial_meshtracker_x1/
...
このチュートリアルは、Meshtastic ファームウェアのソースコードをこれから触り始めるユーザー向けの内容です。Windows と macOS の両方で共通するワークフローを含んでいます。目的はシンプルで、公式リポジトリをクローンし、ビルドを一度成功させ、UI を 1 か所だけ簡単に変更し、その変更済みファームウェアをデバイスに書き込んで動作を確認することです。

すでに Git、Python、または PlatformIO に慣れている場合は、該当するセクションを飛ばして、ハンズオンのパートから始めても構いません。

:::tip
このガイドでは、Windows と macOS の両方で共通して使えるコマンドを掲載しています。スクリーンショットの多くは Windows 環境で撮影していますが、macOS での全体的なワークフローもほぼ同じです。
:::
...
インストール中で最も重要なステップは **PATH 環境変数の設定** です。次を選択してください。

**Git from the command line and also from 3rd-party software**

その他のオプションは、基本的にデフォルトのままで問題ありません。そのまま `Next` をクリックし続けてください。
...
すでにターミナルで有効な Git バージョンが返ってくる場合は、改めてインストールする必要はありません。
...
最初のコマンドで Python が見つかる場合、通常は 2 つ目のコマンドでそのままインストールできます。
...
`python` や `pip` を使いたい場合は、自分でシェルエイリアスを設定しても構いません。ただし macOS では、`python3` と `pip3` を使う方が一般的に安定しています。
...
このステップは、PlatformIO が多くの依存関係を自動的にダウンロードし、インストールに時間がかかるため、初心者には少しとっつきにくく感じられるかもしれません。インストール中にエラーが出た場合は、慌てずに 1 つずつ原因を切り分けて対処するのがよいでしょう。エラーメッセージの確認には、AI ツールを活用すると時間の節約にもなります。
...
この段階では、まだコードの編集を急がないでください。まずはプロジェクトが最後までビルドプロセスを完走できることを確認します。

最初は、次の 3 つのタスクから始めることをおすすめします。
...
つまり、実際のボード単位の環境定義は、通常 `variants/.../platformio.ini` 以下に配置されています。
...
これにより、Meshtastic において **MeshTracker X1 のビルドターゲット** が `seeed_mesh_tracker_X1` であることが分かります。

**最小限の変更内容のまとめ**

エンドツーエンドの最小限の練習だけを完了したい場合は、次のキーとなるステップに集中してください。
...
ビルドが失敗したときは、まず PlatformIO に現在の環境で必要な依存関係をインストールさせるとよいでしょう。
...
具体的にどのように変更するかは、ソースコードを読む力に依存します。ここでは、ホーム画面の UI を変更するという、非常にシンプルな例から始めます。
...
ここは時刻の値が表示されない場合に使われる分岐です。同じ境界制御をここにも追加する必要があります。
...
これは、時刻なしの分岐で `made by AE` を描画する実装です。
...
- Windows の場合、Python が `PATH` に追加されていることを確認するか、ターミナルを開き直して再試行します。
