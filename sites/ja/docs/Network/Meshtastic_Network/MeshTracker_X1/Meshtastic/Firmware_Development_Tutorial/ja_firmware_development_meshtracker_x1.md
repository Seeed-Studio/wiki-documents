---
description: Meshtastic ファームウェアのソースコード環境をセットアップし、MeshTracker X1 をビルドして、ファームウェアを書き込むための実践的なチュートリアルです。
...
インストール後、**現在開いている PowerShell と VS Code のターミナルウィンドウをすべて閉じてから**、新しく PowerShell ウィンドウを開き、次を実行します。
...
このステップは、PlatformIO が多くの依存関係を自動的にダウンロードし、インストールに時間がかかるため、初心者には少しとっつきにくく感じられるかもしれません。インストール中にエラーが出た場合は、慌てずに 1 つずつ問題を切り分けて対処するのが最善です。エラーメッセージの確認には、AI ツールを活用すると時間の節約にもなります。
...
ここでは、例として **Wio Tracker L1 Pro** をターゲットとして使用します。

![img](https://files.seeedstudio.com/wiki/SenseCAP/Meshtastic/Practical-Tutorial/img/image11.png)

このことから、Meshtastic において **MeshTracker X1 のビルドターゲット** は `seeed_mesh_tracker_X1` であることがわかります。

**最小限の変更内容のまとめ**

エンドツーエンドの最小限の練習だけを完了したい場合は、次の主要なステップに集中してください。
...
画面が上のスクリーンショットと似た状態になれば、ビルドプロセスは正しく開始されています。最初のビルドには時間がかかることが多いので、焦らず待ちましょう。
...
- エラーメッセージが通常より絞り込まれ、トラブルシューティングが容易になります。
...
この部分は、右側の時刻、メール、ミュートなどのアイコンが使用する領域を処理します。ここでは、中央テキストの最大右端境界を制限し、右側のコンテンツと重ならないようにするために `headerLabelRight` を追加しました。
...
これは、時刻がない分岐で `made by AE` を描画する実装です。
