...
メッシュ通信を開始するには、まずリージョンを設定する必要があります。この設定はデバイスが使用する周波数帯域を制御し、お住まいの地域に応じて設定する必要があります。
...
GPS を有効にしてください。最新の位置情報を取得するために、更新間隔とブロードキャスト間隔を調整できます。
...
- NodeDB をリセットしても問題が解決しない場合は、デバイス設定やその他の原因の可能性について引き続きトラブルシューティングを行ってください。
