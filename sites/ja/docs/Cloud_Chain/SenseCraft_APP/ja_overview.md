...
SenseCraft APP は、データ可視化とデバイス管理のための強力なアプリです。

## アプリのハイライト

- 時間の節約 - スキャンアンドプレイ体験を提供し、ユーザーが4ステップで全体の設定を完了できます
- 技術的背景が少ない、または全くない人でもセンサーデータを簡単に始められます
- クリーンなユーザーインターフェースでデータを取得・表示
- 時間間隔の設定：バッテリー効率を向上
- 閾値の設定：アクションが必要なデータに焦点を当てる - 近日公開
- データが安全範囲外の場合にアラームを送信 - 近日公開

## ダウンロード

SenseCraft App は iOS と Android の両方のバージョンで利用できます。
...
SenseCraft はデバイス設定とリモート管理をサポートします。SenseCAP Portal プラットフォームとその他の機能を使用するには、アカウントを登録してください。

:::tip Note
Server Location の `Global` を選択してください。<a href="http://sensecap.seeed.cc">SenseCAP Portal</a> からもアカウントを作成できます
:::
...
- デバイスページで、右上角の `+` をクリックして新しいデバイスを追加できます。
...
- 右上角のベルアイコンをクリックしてメッセージセンターに入ります。
...
1. Add アイコンまたは Add Event ボタンをクリックしてイベントアラートを作成し、Add Event ページの Conditioins で条件オプションを追加し、Add ボタンをクリックしてデバイスを選択します。
...
2. デバイスを選択し、測定タイプを選択し、アラーム条件を設定し、より大きいまたはより小さいなどの条件を選択してプログレスバーをドラッグして値を設定し、次のステップをクリックします。
...
3. Add Event ページの Back to normal actions でデバイスが正常に戻った際に通知を送信するかどうかを選択し、Save をクリックし、Event 名を入力し、Submit をクリックしてイベントアラームを正常に追加してイベントリストに戻ります。
...
4. Device ページで Message Center をクリックしてアラームメッセージを表示し、Device warning デバイスがトリガーしたアラームプッシュメッセージ、System notification システムメッセージを表示します。
...
5. アラームメッセージをクリックすると、デバイスが条件項目をトリガーし、アラームメッセージをプッシュし、クリックしてアラーム詳細を表示します。アラームリストに戻るとステータスが既読に変更され、編集ボタンをクリックしてメッセージを選択し、既読、削除などの操作ができます。
...
6. システムメッセージを切り替え、クリックしてシステムプッシュメッセージの詳細を表示します。
...
SenseCraft アプリは、**新しいテンプレートの追加、テンプレートのインポート、テンプレートのダウンロード、プリセットテンプレートの適用**を含むテンプレート機能セットを提供し、デバイス設定を簡素化・高速化します。これらの機能は、ユーザーのセットアップ時間を効率的に削減し、大規模インストールの展開を合理化します。

### 新しいテンプレートの追加
新しいテンプレートを追加する方法は2つあります。

**方法 1** <br />
1. `User` → `Device Bluetooth Configuration` に移動します。<br />
2. 右上角の `Template` ボタンをタップします。<br />
3. `+` アイコン → `Add New Template` をタップします。<br />
4. Device Type を選択します。<br />
5. 必要に応じてテンプレートパラメータを設定します。<br />
6. `Confirm` をタップしてテンプレートを保存します。<br />
7. Template ページに戻ると、`Save successful` が表示されます。<br />
...
**方法 2** <br />
1. Bluetooth Configuration Mode に入り、`Advanced Configuration` を選択します。<br />
2. `Setting` ページに移動します。<br />
3. 右上角の `Template` アイコンをタップします。<br />
4. 必要に応じてテンプレートパラメータを調整します。<br />
5. `Save as Template` を選択します。<br />
6. Template Name を入力します。<br />
...
### テンプレートのインポート
1. `User` → `Device Bluetooth Configuration` に移動します。<br />
2. 右上角の `Template` をタップします。<br />
3. Template ページで `+` アイコンをタップします。<br />
...
2. `Settings` ページを開き、右上角の `Template` アイコンをタップします。<br />
3. `Choose Template` を選択します。<br />
4. Preset Template ページで、希望するテンプレートを選択します。<br />
