...
:::tip note
SenseCAP Mate App は正式に `SenseCraft` App に改名されました！
:::

# イントロダクションと使い方

SenseCraft APP は、データの可視化とデバイス管理のための強力なアプリです。

## アプリの特長

- 時間を節約 - スキャン＆プレイの体験を提供し、4 ステップで全体の設定を完了できます
- 技術的な知識が少ない、またはまったくない人でも、センサーデータを簡単に使い始めることができます
- クリーンなユーザーインターフェースでデータを取得・閲覧できます
- 時間間隔を設定：バッテリー効率を向上
- しきい値を設定：アクションが必要なデータに集中できます - 近日対応予定
- データが安全範囲外になった場合にアラームを送信 - 近日対応予定
...
SenseCraft はデバイスの設定とリモート管理をサポートしています。SenseCAP Portal プラットフォームやその他の機能を利用するには、アカウント登録を行ってください。

:::tip Note
Server Location では `Global` を選択してください。<a href="http://sensecap.seeed.cc">SenseCAP Portal</a> からアカウントを作成することもできます。
:::
...
2. デバイスを選択し、測定タイプを選択してアラーム条件を設定します。より大きい、より小さいなどの条件を選択し、スライダーをドラッグして値を設定し、次のステップをクリックします。
...
3. Add Event ページの Back to normal actions で、デバイスが正常状態に戻ったときの動作を設定し、通知を送信するかどうかを選択します。Save をクリックし、Event 名を入力して Submit をクリックすると、イベントアラームが正常に追加され、Event リストに戻ります。
...
6. システムメッセージに切り替え、クリックしてシステムのプッシュメッセージの詳細を表示します。
...
SenseCraft アプリは、**Add New Template、Import Template、Download Template、Apply Preset Template** を含む一連のテンプレート機能を提供し、デバイス設定を簡素化し高速化します。これらの機能により、ユーザーは設定時間を効率的に短縮し、大規模な導入の展開を合理化できます。
...
3. `+` アイコンをタップし、`Add New Template` を選択します。<br />
4. デバイスタイプを選択します。<br />
...
6. システムのファイルマネージャーで、`“.seeed”` 拡張子の設定ファイルを選択します。<br />
