...
- Object Detection: ここでは、SenseCraft AI を使用して物体検出モデルを学習させるための詳細なガイドを確認できます。
...
- XIAO ESP32S3 Sense Model Output: ここでは、XIAO ESP32S3 Sense ボード上でモデル出力を扱う方法について説明します。このサブセクションはさらに次のように分かれています。

  - **via GPIO**: このパートでは、モデル出力を XIAO ESP32-S3 Sense の GPIO ピンにマッピングし、モデルの予測に基づいて外部ハードウェアを制御する方法に関するガイドを提供します。
