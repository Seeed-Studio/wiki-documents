---
description: XIAO ESP32C3 を搭載した 7.5 インチ E-Ink ディスプレイは、Arduino を介してデータを表示するためのコンパクトで省電力なソリューションです。
title: Arduino 対応
keywords:
  - ePaper ディスプレイ
  - arduino
...
# Arduino で始める XIAO 7.5" ePaper パネル
...
XIAO 7.5" ePaper パネルは Arduino IDE を使ってプログラムできるため、Arduino エコシステムに慣れたメイカーや開発者でも簡単に扱うことができます。本ガイドでは、Arduino 開発環境のセットアップ方法と、基本的なサンプルから始める手順を説明します。

ePaper パネルの特長:

- 800x480 解像度の 7.5 インチ モノクロ ePaper ディスプレイ
- ワイヤレス接続用の XIAO ESP32-C3 マイコン
- ポータブル用途向けの内蔵 2000mAh バッテリー
- プログラミングと電源供給用の USB Type-C インターフェース
- スタンド一体型のコンパクトなデザイン

Arduino プログラミングを通じて、次のことが可能です:

- テキストやグラフィックの表示
- カスタムユーザーインターフェースの作成
- リアルタイムデータやセンサー値の表示
- インタラクティブなアプリケーションの構築
- ePaper のゼロ電力保持を活用した低消費電力アプリケーションの実装
...
まず、まだ Arduino IDE をお持ちでない場合は、[Arduino IDE](https://www.arduino.cc/en/software) にアクセスして最新バージョンをダウンロードしてください。

:::tip
初めて Arduino を使用する場合は、[Getting Started with Arduino](https://wiki.seeedstudio.com/ja/Getting_Started_with_Arduino/) を強く参照することをおすすめします。
:::

### ステップ 2. ESP32 ボードサポートをインストール

**File** -> **Preferences** に進み、**Additional Boards Manager URLs** に次の URL を追加します。[詳細な手順を見るにはここをクリックしてください。](http://localhost:3000/XIAO_ESP32C3_Getting_Started/#ソフトウェア-セットアップ)
...
そのため、ご使用のデバイスまたはコンポーネントの種類を必ず確認してください。
:::
...
その後、**Tools** -> **Board** -> **XIAO ESP32C3** および **Tools** -> **Port** -> **ボードが接続されているポートを選択** に進みます。次に **Upload** をクリックしてコードを書き込みます。
