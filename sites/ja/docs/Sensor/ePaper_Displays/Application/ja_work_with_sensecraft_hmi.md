---
description: SenseCraft Seeedash を使用して、対応する Seeed ePaper ディスプレイデバイス向けのノーコードダッシュボードを設計およびデプロイします。
title: SenseCraft Seeedash を使う
keywords:
  - ePaper ディスプレイ
  - SenseCraft Seeedash
  - ノーコードダッシュボード
...
# SenseCraft Seeedash を使う

[SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) は、Seeed Studio が提供する ePaper ディスプレイデバイス向けノーコードインターフェース設計プラットフォームです。ブラウザ上でダッシュボード、画像ギャラリー、カレンダー、RSS ページ、Web コンテンツページ、その他の常時表示情報画面を設計し、Wi-Fi 経由で対応デバイスにデプロイできます。

この Wiki は ePaper ディスプレイアプリケーションガイドです。Seeed の ePaper デバイスから動作する SenseCraft Seeedash ページまでの最短ルートを説明し、1 枚の ePaper ディスプレイボードと 1 枚の ePaper スクリーンを使用したハードウェア構成を例として扱います。
...
<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="https://sensecraft.seeed.cc/hmi" target="_blank" rel="noopener noreferrer">
            <strong><span><font color={'FFFFFF'} size={"4"}> SenseCraft Seeedash を開く</font></span></strong>
    </a>
</div><br />

## このガイドを使うタイミング

SenseCraft Seeedash を対応する Seeed ePaper 製品と一緒に使用し、次のワークフローをすばやく完了したいときに、このガイドを使用してください。

1. SenseCraft Seeedash ファームウェアを確認または書き込む。
2. デバイスを Wi-Fi に接続する。
3. デバイスを自分の SenseCraft Seeedash ワークスペースに追加する。
4. 最初のページを ePaper ディスプレイにデプロイする。

アカウント設定、エディタの詳細、テンプレート、データウィジェット、リリースノートなどを含む完全なプラットフォームマニュアルについては、[公式 SenseCraft Seeedash ドキュメント](https://sensecraft-hmi-docs.seeed.cc/en/overview/)を参照してください。

## 始める前に

SenseCraft Seeedash を使用する前に、次のものを準備してください。

- 対応する Seeed ePaper ディスプレイデバイス
- SenseCraft アカウント。 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) のログインページから作成できます。
- 2.4 GHz Wi-Fi ネットワーク
- デバイスにファームウェア書き込みが必要な場合は USB-C データケーブル
- SenseCraft Seeedash Web ツールからのシリアルポート書き込みをサポートするブラウザを実行しているコンピュータ

:::tip
ePaper 製品ごとに異なる Seeedash ファームウェアビルドを使用します。必ず、自分のデバイスと画面サイズに正確に一致するファームウェアを選択してください。
:::

## 公式 SenseCraft Seeedash ドキュメント

SenseCraft Seeedash には専用のドキュメントサイトがあります。ePaper ディスプレイのクイックスタートフローにはこの Wiki を使用し、完全なプラットフォーム手順には公式ドキュメントを使用してください。
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      <td>概要</td>
      <td>SenseCraft Seeedash が何であり、何ができるかを理解したいとき。</td>
    ...
    <tr>
      ...
      <td><a href="https://sensecraft-hmi-docs.seeed.cc/en/guides/getting_started/" target="_blank" rel="noopener noreferrer">Getting Started を開く</a></td>
    </tr>
    <tr>
      <td>Workspace と Canvas</td>
      <td>エディタレイアウト、ウィジェット、データソース、ページ作成フローについて学びたいとき。</td>
      <td><a href="https://sensecraft-hmi-docs.seeed.cc/en/guides/workspace/" target="_blank" rel="noopener noreferrer">Workspace ガイドを開く</a></td>
    </tr>
    <tr>
      <td>AI Generation</td>
      <td>テキストプロンプトから画像やページレイアウトを生成したいとき。</td>
      <td><a href="https://sensecraft-hmi-docs.seeed.cc/en/guides/ai_gen/" target="_blank" rel="noopener noreferrer">AI Generation を開く</a></td>
    </tr>
    <tr>
      <td>リリースノート</td>
      <td>最新のプラットフォームおよびファームウェアの更新を確認したいとき。</td>
...
:::note
この例は対応リストではありません。現在の対応ハードウェアおよびファームウェアマトリクスについては、必ず [SenseCraft Seeedash Supported Hardware ページ](https://sensecraft-hmi-docs.seeed.cc/en/supported_hardware/)を参照してください。
:::

## ステップ 1: Seeedash ファームウェアを確認または書き込む

最初のステップは、ePaper デバイスが SenseCraft Seeedash ファームウェアを実行していることを確認することです。

<Tabs groupId="hmi-hardware">
<TabItem value="reterminal" label="reTerminal E シリーズ" default>

reTerminal E シリーズは、箱から出してすぐに SenseCraft Seeedash と連携できるように設計されています。デバイスの電源を入れ、[ステップ 2: デバイスを Wi-Fi に接続する](#ステップ-2-デバイスを-wi-fi-に接続する) に進んでください。

以前に別のファームウェアを書き込んだことがある場合は、[SenseCraft Seeedash デバイスページ](https://sensecraft.seeed.cc/hmi/device)を開き、デバイスモデルを選択して、画面の指示に従ってファームウェアを操作してください。
...
ディスプレイボードと別体の ePaper スクリーンを使用する場合は、ボードとパネルに対応した Seeedash ファームウェアを書き込みます。

**Step 1.** [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) にサインインし、上部メニューから **Tools** を開きます。
...
**Step 2.** 自分のディスプレイに対応するファームウェアエントリを選択します。この例では、**7.3" Full-Color Display 800 x 480** エントリを選択します。
...
**Step 3.** 自分のハードウェアに対して利用可能な最新のファームウェアバージョンを選択します。
...
**Step 4.** ボードを USB-C データケーブルでコンピュータに接続し、**Flash** をクリックします。

**Flash** をクリックすると、ブラウザにシリアルポート選択ウィンドウが表示されます。自分のデバイスに対応するポートを選択します。
...
**Step 5.** ePaper ディスプレイがリフレッシュされ、セットアップ用 QR コードが表示されるまで待ちます。
...
デバイスが Seeedash ファームウェアを実行していることを確認したら、ローカルの Wi-Fi ネットワークに接続します。

**Step 1.** ePaper 画面に表示されている Wi-Fi アクセスポイントに、スマートフォンまたはコンピュータを接続します。この一時的なアクセスポイントにパスワードは不要です。
...
**ステップ 2.** ePaper 画面上の QR コードをスキャンします。設定ページが自動的に開かない場合は、ブラウザを開いて `192.168.4.1` にアクセスします。
...
## ステップ 3: デバイスを SenseCraft Seeedash に追加する

Wi-Fi 設定が完了したら、デバイスを SenseCraft Seeedash ワークスペースに追加します。
...
デバイスがペアリングされると、SenseCraft Seeedash から ePaper 画面へコンテンツをデプロイできます。

最も早く始める方法は、既存のテンプレートまたは組み込みの作成ツールのいずれかを使用することです。詳細なエディタの操作手順については、[公式入門ガイド](https://sensecraft-hmi-docs.seeed.cc/en/guides/getting_started/)に従ってください。
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      <td>Canvas</td>
      <td>テキスト、画像、ウィジェット、図形、データソースからページを構築します。</td>
...
以下の例は、SenseCraft Seeedash ページを 7.3 インチ ePaper ディスプレイにデプロイした様子を示しています。
...
<div style={{textAlign:'center'}}>
    ...
    <p style={{fontSize:'0.9em', color:'#555', marginTop:10}}><em>7.3 インチ ePaper ディスプレイでの表示例</em></p>
</div>
...
最も一般的な原因は、サポートされていない Wi-Fi バンドを使用しているか、パスワードを間違えて入力していることです。

- 2.4 GHz の Wi-Fi ネットワークを使用してください。
- Wi-Fi パスワードを注意深く再入力してください。
- セットアップ中はデバイスをルーターの近くに移動してください。
- 以前にデバイスを設定したことがある場合、保存されたネットワーク設定を消去して最初からやり直したいときにのみ **Full Flash** を使用してください。

### Q3: フラッシュ後に画面に期待したセットアップ用 QR コードが表示されないのはなぜですか？

ファームウェアが接続されている画面と一致していないか、画面が正しく接続されていない可能性があります。

- SenseCraft Seeedash Tools ページで、デバイスモデル、画面サイズ、ファームウェア項目を再確認してください。
...
- 画面解像度が低い場合は、非常に小さいテキストは避けてください。
- モノクロまたはグレースケール画面では、コントラストの高い色を使用してください。
- エディタとデプロイの詳細については、公式ドキュメントを確認してください。

## リソース

- [SenseCraft Seeedash プラットフォーム](https://sensecraft.seeed.cc/hmi)
- [SenseCraft Seeedash 公式ドキュメント](https://sensecraft-hmi-docs.seeed.cc/en/overview/)
- [SenseCraft Seeedash 対応ハードウェア](https://sensecraft-hmi-docs.seeed.cc/en/supported_hardware/)
- [SenseCraft Seeedash 入門ガイド](https://sensecraft-hmi-docs.seeed.cc/en/guides/getting_started/)
- [SenseCraft Seeedash リリースノート](https://sensecraft-hmi-docs.seeed.cc/en/release_note/)
