---
description: ESPHome と Home Assistant で互換性のある Seeed ePaper 製品を駆動するためのリファレンスガイド - 書き込みパス、汎用 YAML スケルトン、および各製品のクックブックの所在。
...
このページは、[ESPHome](https://esphome.io/) を通じて互換性のある Seeed ePaper 製品を駆動し、[Home Assistant](https://www.home-assistant.io/) と連携させるための**リファレンスマニュアル**です。ここでは、すべてのハードウェアで共通する部分を扱います：
...
:::tip
**TRMNL クラウドダッシュボード**（YAML 不要・プラグインベース）をお探しですか？その場合は [Work with TRMNL](/ja/reterminal_e10xx_trmnl) を参照してください。**Seeed のノーコード・ビジュアル HMI** をお探しの場合は、[Work with SenseCraft HMI](/ja/EE04_with_hmi) を参照してください。
:::
...
2. ダッシュボードがスターターの `<device-name>.yaml` を生成します。その本体を、クックブックにある製品固有の YAML に置き換えます（形については下のステップ 2 を参照）。
3. **Install** をクリック → 初回の書き込みでは **ESPHome を実行しているコンピュータに接続** します。2 回目以降の書き込みは OTA による無線更新です。
