...
2. **ドライバ / 拡張ボード** — MCU とユニバーサルまたは大判 ePaper スクリーンを組み合わせて独自製品を構築。
3. **DIY キット & ベアパネル** — TRMNL や XIAO など特定プラットフォーム向けに最適化された、メイカーコミュニティ向け設計。

これら 3 つのラインはすべて、同じ**ソフトウェアエコシステム**を共有しています：SenseCraft Seeedash（ノーコード）、Home Assistant / ESPHome、TRMNL、Arduino / ESP-IDF / PlatformIO、Zephyr、さらに SquareLine Vision、EEZ Studio、Lopaka といったビジュアル UI ツールです。ハードウェアファミリを 1 つ選べば、残りのツールチェーンは共通のままです。
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      <td>箱から出してすぐに ePaper ダッシュボードを動かしたい（はんだ付けや追加配線なし）</td>
      ...
      <td>バッテリ、筐体、温湿度センサ、ブザー、ボタン、Wi-Fi をすべて統合。SenseCraft Seeedash ファームウェアをプリインストールして出荷。</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>TRMNL プラットフォーム向けに事前チューニング済み；ESPHome と Arduino にも対応。</td>
    </tr>
    <tr>
...
:::tip
解像度、リフレッシュ時間、部分更新対応、認証、MSRP などの仕様比較と、詳細なソフトウェア対応マトリクスについては、専用ページ **[reTerminal E シリーズ概要](https://wiki.seeedstudio.com/ja/reterminal_e10xx_main_page)** を参照してください。
:::
...
### ESP32-S3 ドライバボード（Wi-Fi / BLE、SenseCraft Seeedash）
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      ...
      <td>大判カラー専用ドライバ；Wi-Fi + BLE 搭載；デジタルサイネージに最適。</td>
    ...
    <tr>
      ...
      <td>T-CON タイミング内蔵；オンボード SHT40 による波形補償；XIAO ESP32-S3 Plus 搭載。</td>
    ...
    <tr>
      ...
      <td>EE04 のレイアウトを刷新；同じ XIAO ESP32-S3 ベースで最新ファームウェアをサポート。</td>
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      ...
      <td align="center">24 ピン / 50 ピン汎用</td>
      <td>Bluetooth LE + NFC；XIAO nRF52840 Plus 搭載；ESL やオフラインタグに最適。</td>
    ...
    <tr>
      ...
      <td align="center">24 ピン汎用（最新）</td>
      <td>EN04 のレイアウトを刷新；同じ nRF52840 ベースで最新ファームウェアをサポート。</td>
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      ...
      <td>任意の XIAO モジュール向けプラグアンドプレイ拡張ボード；一般的な小型 ePaper スクリーンに対応。</td>
...
自分で一から組み立てたいメイカー向けに、柔軟な 2 つのオプションがあります。
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      ...
      <td>7.5インチ モノクロ ePaper + ESP32 ドライバボード + アクリルスタンド。TRMNL クラウドプラットフォーム向けに事前チューニング済みで、ESPHome と Arduino も実行できます。</td>
    ...
    <tr>
      ...
      <td>すでに XIAO を持っていて画面を追加したい、コンパクトで低コストなプロジェクト向け。</td>
...
すべての Seeed ePaper 製品は、同じソフトウェアエコシステムに収束します。自分のスキルレベルに合ったワークフローを選択してください — 組み合わせて使うこともできます：
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      <td><a href="https://wiki.seeedstudio.com/ja/sensecraft_hmi_overview" target="_blank" rel="noopener noreferrer"><strong>SenseCraft Seeedash</strong></a></td>
      <td align="center">ノーコード（クラウド）</td>
      <td>AI ジェネレータ、ギャラリー、キャンバス、RSS / Web コンテンツ、OTA デプロイメント。reTerminal E シリーズのデフォルトファームウェアです。</td>
    ...
    <tr>
      ...
      <td>GPIO、センサ、ネットワーク、カスタム描画などを完全にプログラム制御できます。</td>
      <td>すべての ESP32-S3 製品（E1001–E1004、EE02–EE05、TRMNL Kit、XIAO Panel）</td>
    </tr>
    <tr>
      <td><a href="https://wiki.seeedstudio.com/ja/epaper_work_with_zephyr" target="_blank" rel="noopener noreferrer"><strong>Zephyr RTOS</strong></a></td>
      <td align="center">RTOS</td>
      <td>west、デバイスツリー、カーネルサービス、ドライバ、書き込み、デバッグを含む、上流 Zephyr ボードワークフロー。</td>
    ...
    <tr>
      ...
      <td align="center">ビジュアル UI デザイン</td>
      <td>HMI 画面をビジュアルに設計し、Arduino IDE 経由でデプロイします。</td>
...
- [SenseCraft Seeedash Documentation](https://wiki.seeedstudio.com/ja/sensecraft_hmi_overview/)
