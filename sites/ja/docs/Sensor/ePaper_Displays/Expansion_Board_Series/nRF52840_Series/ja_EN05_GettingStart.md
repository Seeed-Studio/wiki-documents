...
- **さまざまなソフトウェアプラットフォームに対応:** Arduino、Platform IO、ESPHome など、お好みの開発環境でコーディングでき、このドライバボードは両方のアプローチをサポートし、プロジェクトに最大限の柔軟性を提供します。
...
その後、**Tools** -> **Board** -> **Seeed nRF52 Boards** -> **Seeed XIAO nRF52840 Plus** および **Tools** -> **Port** -> **ボードが接続されているポートを選択** に進みます。

次に **Upload** をクリックしてコードを書き込みます。
これで ePaper スクリーンに表示が出るはずです！
