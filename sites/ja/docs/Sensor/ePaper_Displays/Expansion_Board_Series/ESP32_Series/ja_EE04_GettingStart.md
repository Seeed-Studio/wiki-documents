...
**XIAO ESP32-S3** Plus を搭載したディスプレイボード EE04 は、**24 ピン**および **50 ピン**の ePaper ディスプレイの両方をサポートします。JST 2.0 mm バッテリーコネクタと電源スイッチ、内蔵充電 IC を備え、1 つのリセットボタンと 3 つのユーザーボタンが付属しています。デジタルサイネージ、電子ラベル、携帯型情報ボードなど、低消費電力の ePaper プロジェクトに最適です。
...
- **ユーザーフレンドリーなボタン:** 1 つのリセットボタンと 3 つのユーザープログラム可能なボタンを備え、プロジェクトの開発を加速し、柔軟なカスタマイズ機能を提供します。
...
⚠️ ジャンパ設定が間違っていると、ePaper が表示されなかったり、異常な表示になる場合があります。電源を入れる前に、必ずジャンパ位置を再確認してください。
...
ここでは、5.83 インチディスプレイを例として使用します。手順はすべての 24 ピン画面で同じで、違いはドライバ内で適切な画面サイズを選択する点だけです。
...
⚠️ コネクタを逆向きにしないでください！ケーブルを上下逆に挿すと、ePaper が表示されなくなったり、画面やボードを破損したりする可能性があります。
下図は正しい接続例です：
:::
...
### XIAO ePaper Display Board(ESP32-S3) - EE04 のバッテリー動作
...
:::tip
自分でバッテリー電圧を読み取るコードを書きたい場合は、analogRead() 関数の前に 10ms の delay を入れると、より正確になります。
:::
...
GFX ライブラリ内の Bitmap サンプルを見つけて選択します。
...
次に、https://sensecraft.seeed.cc/hmi/tools/dither にある SenseCraft Seeedash Tool にアクセスし、表示したい画像をアップロードします。
...
:::tip 
表示された色が元のデザインと比べて反転して見える場合は、コードを生成する前に Seeedash ツールで Invert Colors オプションを選択してください。
:::
...
Arduino スケッチ内の image.c または image.h ファイルにある既存の配列を、先ほどコピーした配列で上書きします。
