...
<table>
  ...
  <tr>
    <td>プロセッサ</td>
    <td>ESP32-C3 SoC<br/>4 ステージパイプラインを備え、最大 160 MHz で動作する RISC-V シングルコア 32-bit チッププロセッサ</td>
  </tr>
  <tr>
    <td>ワイヤレス</td>
  ...
  <tr>
    <td>電源（標準）</td>
    <td>最大 3.3V 出力電流: 500mA<br/>試験条件: BAT ピン入力 @ 3.8V<br/>供給能力: 3A<br/>充電電流: 380mA（急速） / 40mA（トリクル）<br/>入力電圧 (VIN): 5V<br/>入力電圧 (BAT): 3.7V</td>
  </tr>
  <tr>
    <td>ディープスリープ時消費電力</td>
...
- **強力な CPU:** ESP32-C3、最大 160 MHz で動作する 32­bit RISC­-V シングルコアプロセッサ
...
- **超小型サイズ:** 親指ほどの大きさ (21x17.8mm) の XIAO シリーズ定番フォームファクタで、ウェアラブルデバイスや小型プロジェクトに最適
...
| GND             |           |           |                                     |                      |
| 3V3             | 3V3_OUT   |           |                                     | 電源出力         |
| D0              | アナログ    | GPIO2     | ADC1_CH2,                           | GPIO, ADC            |
| D1              | アナログ    | GPIO3     | ADC1_CH3                            | GPIO, ADC            |
| D2              | アナログ    | GPIO4     | ADC1_CH4, FSPIHD, MTMS              | GPIO, ADC            |
| D3              | アナログ    | GPIO5     | ADC2_CH0, FSPIWP, MTDI              | GPIO, ADC            |
...
- 5V - これは USB ポートからの 5V 出力です。外部電源入力としても使用できますが、その場合は外部電源とこのピンの間に必ず何らかのダイオード（ショットキー、信号用、電力用）を入れ、アノードをバッテリー側、カソードを 5V ピン側に接続する必要があります。
- 3V3 - これはオンボードレギュレータからの安定化出力です。700mA まで取り出すことができます。
- GND - 電源 / データ / 信号用のグラウンド
...
**Note:** LED に流れる電流を制限し、過電流による LED の焼損を防ぐため、必ず約 150Ω の抵抗を直列に接続してください。

### ソフトウェアの準備

- **Step 1.** お使いのオペレーティングシステムに応じて、最新版の Arduino IDE をダウンロードしてインストールします。
...
**Tools > Board > ESP32 Arduino** に移動し、"**XIAO_ESP32C3**" を選択します。ボードの一覧は少し長いので、一番下までスクロールする必要があります。
...
- **Step 2.** **Upload** ボタンをクリックして、コードをボードに書き込みます。
...
書き込みが完了すると、接続した LED が 1 秒間隔で点滅するのが確認できます。これは接続が成功したことを意味し、これで XIAO ESP32C3 を使ってさらに多くのプロジェクトに取り組むことができます。
...
:::caution
はんだ付けの際に、プラス端子とマイナス端子を短絡させてバッテリーや機器を焼損させないよう、十分注意してください。
:::

**バッテリー使用上の注意：**

1. 規格を満たした、認定済みのバッテリーを使用してください。
2. バッテリー使用中でも、XIAO はデータケーブルを介してコンピュータ機器に接続できます。XIAO には保護回路チップが内蔵されており、安全ですのでご安心ください。
3. XIAO ESP32C3 はバッテリー駆動時には（特定のプログラムを書き込んでいない限り）LED が点灯しません。LED の状態だけで XIAO ESP32C3 が動作しているかどうかを判断しないでください。必ずご自身のプログラムの動作から適切に判断してください。
...
しかし、バッテリー電圧測定専用のピンを使用したい場合は、[msfujino](https://forum.seeedstudio.com/u/msfujino) 氏の巧妙な方法を参照できます。XIAO ESP32C3 のために多くの経験と労力を共有してくださった [msfujino](https://forum.seeedstudio.com/u/msfujino) 氏に、ここで特別な感謝を申し上げます。

基本的な動作の考え方は次のとおりです：バッテリー電圧を 200k で 1/2 に分圧し、A0 ポートに接続することで電圧を監視できるようにします。

<div align="center"><img src="https://files.seeedstudio.com/wiki/XIAO_WiFi/3.png" alt="pir" width="300" height="auto"/></div>

データシート上では、公称のフルスケール AD 変換電圧は 2500mV とされていますが、チップごとのばらつきが大きく、実際には ±10% 程度あります。私のチップではフルスケールが 2700mV でした。

幸いなことに、各チップの較正補正値はフューズ領域に書き込まれており、`analogReadMilliVolts()` 関数を使用することで、特別な処理をしなくても補正済みの電圧値を読み取ることができます。

AD 変換結果とマルチメータで測定した電圧はよく一致しており、誤差は約 5mV 程度で、実用上問題ありません。

また、特に通信中にはスパイク状の誤差が発生するため、これを除去するには 16 回の平均化が必要でした。

以下はバッテリー電圧をテストするための手順です。
...
上記に基づいてバッテリー電圧を測定しようとする場合は、十分な実作業スキルと、より高度なはんだ付けスキルを身につけてから行うことをお勧めします。また、バッテリーの短絡など危険な行為には十分注意してください。
:::
...
もしXIAOがディープスリープに入る前に素早くシリアルモニタをオンにできれば、下図のようなメッセージ出力が確認できます。これはXIAOが現在「スリープ」状態であることを意味します。

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/XIAO_WiFi/15.png" style={{width:700, height:'auto'}}/></div>

:::tip
ディープスリープモードに入ると、XIAOのポートは表示されなくなり、ポート番号を再び確認するにはXIAOをウェイクアップさせる必要があります！
:::

このプログラムでは、**D1** のLowレベルでウェイクアップするようにしています。つまり、D1ピンにボタンを接続しておけば、そのボタンを押したときにXIAOが起動します。
...
### Q1: Arduino IDEでボードへのコード書き込みが止まってしまう

まず、ボードをPCに接続した状態で **RESET BUTTON** を1回押して、ボードをリセットしてみてください。それでも解決しない場合は、**BOOT BUTTON** を押し続けたままボードをPCに接続し、**BOOT** ボタンを離して **bootloader mode** に入ります。

### Q2: Arduino IDEでボードがシリアルデバイスとして表示されない

上記 **Q1** と同じ手順を実行してください。

### Q3: ブートローダを工場出荷時ファームウェアで書き直したい

ボードを **USB Type-C** 経由でPCに接続し、**ESP RF Test Tool** を使用してブートローダを工場出荷時ファームウェアで書き換えるだけです。

- **Step 1.** **BOOT BUTTON** を押し続けたままXIAO ESP32C3をPCに接続し、**bootloader mode** に入ります

- **Step 2.** 接続されたら、BOOT BUTTONを離します

- **Step 3.** [このページ](https://www.espressif.com/en/support/download/other-tools) にアクセスし、**ESP RF Test Tool and Test Guide** をダウンロードします
...
- **Step 5.** ChipTypeとして **ESP32C3**、COMポート、BaudRateとして **115200** を選択し、**open** をクリックします
...
- **Step 6.** **Flash** を選択し、**Select Bin** をクリックします
...
- **Step 7.** [XIAO ESP32C3の工場出荷時ファームウェア](https://files.seeedstudio.com/wiki/XIAO_WiFi/Resources/ESP32-C3_RFTest_108_2b9b157_20211014.bin) をダウンロードし、それを選択します。

- **Step 8.** 最後に **Load Bin** をクリックします
...
**ハードウェア設計**
- **📄[データシート]** [Espressif ESP32-C3 データシート](https://files.seeedstudio.com/wiki/XIAO_WiFi/Resources/esp32-c3_datasheet.pdf )
...
- **🔗[Wiki]** [First Look at the Seeed Studio XIAO ESP32-C3](https://sigmdel.ca/michel/ha/xiao/xiao_esp32c3_intro_en.html )  
  - XIAO ESP32C3の優れたイントロ記事で、主な機能と基本的な使い方をカバーしています。
