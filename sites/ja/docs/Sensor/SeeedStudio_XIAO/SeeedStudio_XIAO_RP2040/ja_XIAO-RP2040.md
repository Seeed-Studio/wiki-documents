...
2 つのボードの主な違いは、外部に引き出されている GPIO の数、利用可能な周辺インターフェース、およびオンボードの電源管理機能です。
...
- **共通の XIAO フットプリント:** 両バージョンとも標準の 14 ピン XIAO レイアウトを採用し、11 本の GPIO ピンと 3 本の電源ピンを備えています
- **コンパクトなフォームファクタ:** ウェアラブルデバイスやスペースに制約のあるアプリケーション向けの親指サイズ 21 × 17.8 mm デザイン
...
- **柔軟な実装オプション:** ブレッドボードに適した共通ピンに加え、キャッスレーテッドエッジと SMD 対応パッドを備え、カスタムキャリアボードに対応
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td colspan="2">
        Arduino、PlatformIO、MicroPython、CircuitPython、TinyGo、Rust、Zephyr など
        <a href="https://wiki.seeedstudio.com/ja/xiao_topic_page/">今後さらに追加予定</a>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        ...
        <a href="https://www.seeedstudio.com/Seeed-Studio-XIAO-RP2040-Pre-Soldered-p-6333.html">
          Seeed Studio XIAO RP2040 事前はんだ付け済み
        </a>
      </td>
...
:::caution
一般的な I/O ピンについて：
MCU の動作電圧は 3.3V です。一般 I/O ピンに接続される入力電圧が 3.3V を超えると、チップが損傷する可能性があります。
...
| RGB_LED    | WS2812B データ         | —            | GPIO12/NEOPIX     | シングルワイヤ WS2812B カラーデータ制御                     |
...
ユーザーのプログラミング処理が失敗したとき、Seeed Studio XIAO RP2040 のポートが消えてしまう場合があります。次の操作を行うことで、この問題を解決できます。
...
BAT バッテリーコネクタが背面に搭載されているのは XIAO RP2040 Plus のみですので、ご注意ください。
...
はんだ付けの際には、正極と負極を短絡させてバッテリーや機器を損傷しないよう、十分注意してください。バッテリーに電力が残っている場合は、決して基板にはんだ付けしないでください。基板が焼損するおそれがあります。通電中に短絡が発生すると重大なリスクを伴うため、アダプターの使用を推奨します。
...
バッテリー電圧測定回路は、**TPS22916CNYFPR** ロードスイッチを使用して測定経路を有効または無効にします。バッテリー電圧を測定する必要があるとき、MCU は **BAT_EN (GPIO24)** を介して測定経路を有効にし、スタンバイモード中のバッテリー消費を低減します。<br/>
