...
:::warning
**ディープスリープ中は USB ペリフェラルが無効になります：** ディープスリープ中は、内部 USB ペリフェラル（USB-Serial-JTAG）を含むすべてのデジタルペリフェラルの電源がオフになります。デバイスがディープスリープ中の場合、**USB 経由のシリアル出力は利用できません**。デバッグが必要な場合は、ハードウェア UART ピンに接続した外部 USB-UART チップを使用してください。
:::
...
:::warning
**ライトスリープ中は USB ペリフェラルが無効になります：** 省電力のため、内部 USB ペリフェラル（USB-Serial-JTAG）はライトスリープ中に無効になります。つまり、デバイスがライトスリープ中の場合、**USB 経由のシリアル出力は利用できません**。USB ポートを使用してシリアルログを確認している場合、スリープ期間中は出力が表示されません。デバッグするには、ハードウェア UART ピンに接続した外部 USB-UART チップを使用するか、GPIO ウェイクアップを使用してデバイスの復帰後に出力を監視することを検討してください。
:::
