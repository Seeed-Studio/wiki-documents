---
description: reSpeaker Flex は強力なモジュラー音声システムで、XMOS XVF3800 を搭載し、交換可能な円形および直線型の 4 マイクアレイにより、精密な 360° もしくは指向性のオーディオキャプチャを実現します。ロボット工学やスマートデバイスに最適です。
title: Xiao ESP32S3 を使用した reSpeaker Flex の HTTP ストリーミング
keywords:
  - reSpeaker flex
  - xvf3800
  - ESP32S3
...
url: https://wiki.seeedstudio.com/ja/respeaker_flex_xiao_http_stream/
---

## はじめに 

このガイドでは、ESP32-S3（XIAO ESP32S3）を使用して reSpeaker Flex XVF3800 マイクアレイからオーディオを取得し、5 秒間の 16 kHz ステレオ 32-bit PCM オーディオを録音し、大きなオーディオバッファを扱うために PSRAM にデータを保存する方法について説明します。録音されたオーディオは Wi-Fi 経由で Python ベースの Flask HTTP サーバーにアップロードされ、そこで受信され標準的な WAV ファイルとして保存されます。この構成は、音声キャプチャ、リモートオーディオロギング、音声認識（STT）や機械学習ベースのオーディオ解析を含む音声処理パイプラインなどのアプリケーションに適しています。
