...
- **高度なオーディオ処理** :XVF3800 により駆動され、AEC、マルチビームフォーミング、残響除去、DoA 検出、動的ノイズ抑制、60dB の AGC レンジを実現

- **一意のデバイスシリアル番号** :内蔵 SN により、複数デバイスの展開や高度なデバイス管理が可能

- **2 つの動作モード** :PC へ即座に接続できる USB プラグアンドプレイモードと、組み込みシステムと統合するための INT-Device（I2S）モードを搭載し、ファームウェアを切り替えることで USB または I2C コマンド経由で設定可能
...
- **同等またはそれ以上の音質** :前世代モデルと比較して同等またはそれ以上の音質
...
| **オープンソース互換性** | **Arduino、Raspberry Pi、PC/Mac** で動作し、**XIAO シリーズ** とも互換性があります。                   |
...
- 複数の出力オプションを備えたステレオ I2S 入出力と、XVF3800 のパラメータ設定および管理用の I2C インターフェース。
- IO ピン経由での XIAO リセット
- インターフェースおよびはんだパッド
...
リセット（RST）ボタンは XVF3800 のハードウェアリセットを提供します。押すとチップが再起動し、電源を入れ直した場合と同様に、システムが最初から再初期化されます。
...
USB ファームウェアは、USB ハードウェアインターフェースを介して通信する際に、**Windows、Linux、macOS** などのホスト OS で使用するよう設計されています。
...
| ファームウェア | チャンネル数 | 備考 |
|---------|----------|-------|
| respeaker_xvf3800_i2s_dfu_firmware_v1.0.x.bin | 2 | 処理済み 2 チャンネル出力 <br /> Channel 0: Conference <br /> Channel 1: ASR |
...
- ReSpeaker XVF3800 を接続し、デバイスが検出されているか確認します。
...
GitHub から完全なファームウェアリポジトリをここからダウンロードします。[`XMOS XVF 3800`](https://github.com/respeaker/reSpeaker_XVF3800_USB_4MIC_ARRAY)
...
2. **Pi-Apps から Audacity をインストールする**
...
reSpeaker XVF3800 には、デバイスの動作を設定したり、パラメータデータを設定・読み取りしたり、パラメータデータをデバイスに保存したりできる制御インターフェースが搭載されています。ユーザーは USB または I2C インターフェースを介してデバイスを制御できます。サンプルホストアプリケーションとして、Linux、macOS、Raspberry Pi OS 向けの xvf_host、Windows 向けの xvf_host.exe が提供されており、reSpeaker XVF3800 の制御インターフェースに簡単に接続できます。
...
- ReSpeaker XVF3800 の **LED リング** は、光を使って方向を表示します。
- `xvf_host` ツールを使って DoA の値を読み取ることもできます。
...
- **Free running beam**: 常に方向をスキャンするビーム
- **Auto selected beam**: 最終的な音声品質のために選択されたビーム（DoA 表示に使用）
...
- **自動選択ビーム**: 最適なオーディオのために最終的に選ばれたビーム（DoA 表示に使用）

**音声インジケータ**

- スピーチエナジーは音声信号の強さを示し、音声の音量メーターのようなものです。
- 誰かが話しているかどうか、またどのくらい大きな声／どのくらい近いかを検出するために使用されます。
...
スタートメニューを開き、「Device manager」と入力します。関連する reSpeaker XVF 3800 デバイスを見つけて右クリックし、［デバイスのアンインストール］を選択します。その後、デバイスを再起動します（USB を抜き差し）。すると Windows が正しいサウンドカードドライバを再インストールします。
