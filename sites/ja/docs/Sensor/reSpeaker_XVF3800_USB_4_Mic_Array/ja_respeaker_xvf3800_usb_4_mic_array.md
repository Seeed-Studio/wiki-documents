...
- **4 マイクロフォンアレイ** :4 つの高性能マイクを円形に配置し、最大 5 メートルの 360° 遠距離音声キャプチャに対応
...
お使いの ReSpeaker 上の **ミュートボタン** は、マイクロフォンアレイからの **音声キャプチャを一時的に無効化** するために使用します。
ミュートボタンを押すとどうなりますか？
...
- Factory パーティションに保存されている **Safe Mode ファームウェア** は、最も柔軟性があります。**USB DFU と I2C DFU の両方**をサポートします。
...
ファームウェアファイル **respeaker_xvf3800_i2s_dfu_firmware_v1.0.x.bin** は [here](https://github.com/respeaker/reSpeaker_XVF3800_USB_4MIC_ARRAY/tree/master/xmos_firmwares/i2s) から入手できます。このファームウェアは **2 チャンネル** オーディオと **32 ビット** 深度をサポートします。
...
Home Assistant ファームウェアは、Home Assistant との統合向けに特別に設計された、もう 1 つの I2S ベースのファームウェアです。この最適化されたファームウェアは、48 kHz のサンプリングレートを持つ 2 チャンネルオーディオを使用し、Home Assistant 環境内でより良い互換性とパフォーマンスを提供します。
...
2. **上下の矢印キー** を使って **XVF3800** サウンドカードをハイライトします。
3. 選択を確定するには **Enter** を押します。
