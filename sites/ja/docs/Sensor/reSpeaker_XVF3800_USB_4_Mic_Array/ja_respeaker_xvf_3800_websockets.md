---
description: ReSpeaker XVF3800 USB 4-Mic Array からのオーディオを WebSocket 経由で Python サーバーへストリーミングします。XIAO ESP32S3 上で 48 kHz ステレオ 32-bit PCM オーディオをキャプチャし、PSRAM にバッファリングして大きなオーディオバッファを扱い、WebSocket 経由で送信して PC 上に WAV ファイルとして保存します。
title: reSpeaker XVF3800 USB Mic Array と XIAO ESP32S3 による WebSocket オーディオストリーミング
keywords:
  - reSpeaker
  - XIAO
  - ESP32S3
...
このガイドでは、XVF3800 マイクアレイから ESP32-S3（XIAO ESP32S3）を使用してオーディオを取得し、48 kHz ステレオ 32-bit PCM オーディオを 5 秒間録音し、大きなオーディオバッファを扱うために PSRAM にデータを保存する方法について説明します。録音されたオーディオは、その後 Wi-Fi 経由で Python ベースの WebSocket サーバーに送信され、受信後に標準的な WAV ファイルとして保存されます。このアプローチは、音声キャプチャ、リモートオーディオロギング、音声認識（STT）や機械学習ベースのオーディオ解析を含む音声処理パイプラインなどの用途に適しています。
...
:::note
PSRAM を有効にする必要があります。そうしないと malloc() が失敗します。
:::

:::note
このスケッチでは、XVF3800 ファームウェアを使用して I2S マスターとして動作させています。アップロードする前に、次の DFU ファームウェアを XVF3800 に書き込んでください：

`respeaker_xvf3800_i2s_master_dfu_firmware_v1.0.x_48k_test5.bin`
:::

アップロードする前に、次のフィールドを更新してください：
...
## Python サーバーコード（WebSocket）

**websockets ライブラリをインストールする必要があります：** `pip install websockets`
