...
[The Things Network (TTN)](https://www.thethingsnetwork.org/) は、LoRaWAN® をベースとしたグローバルなオープンソース IoT プラットフォームで、低消費電力かつ長距離の無線接続を提供します。これは [The Things Stack (TTS)](https://www.thethingsindustries.com/stack/) によってサポートされており、安全なデバイス管理とデータルーティングを提供する LoRaWAN® ネットワークサーバーです。この章では、[SenseCAP T2000 Tracker](https://www.seeedstudio.com/SenseCAP-Asset-Tracker-T2000-C-p-6582.html) を [The Things Network](https://www.thethingsnetwork.org/) に接続する方法をユーザーにガイドします。


## デバイス設定

TTS に接続する前に、SenseCraft APP でデバイスの基本パラメータを設定する必要があります。詳細については [クイックスタート](https://wiki.seeedstudio.com/ja/Get_Started_with_SenseCAP_T2000_tracker/) を確認してください。

プラットフォームを The Things Network に設定し、Device EUI / AppEUI / AppKey をコピーします。その後、The Things Stack でデバイスを手動で追加します。
...
The Things Stack(TTS) は、オープンソースコアをベースに構築されたエンタープライズグレードの LoRaWAN ネットワークサーバーです。The Things Stack を使用すると、独自のハードウェアまたはクラウド上で LoRaWAN ネットワークを構築・管理できます。

まず、[The Things Network](https://www.thethingsnetwork.org/) でアカウントを登録してください。
...
Applications ページに移動し、`Add application` をクリックします。
...
Application ID、Application name を入力し、`Create application` をクリックして変更を保存します。
...
アプリケーションが作成されたら、`Register end device` をクリックします。
...
アプリケーション概要ページでデバイスのアクティビティを確認できます。
