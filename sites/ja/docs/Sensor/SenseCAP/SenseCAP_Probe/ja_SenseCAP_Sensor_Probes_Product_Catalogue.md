...
url: https://wiki.seeedstudio.com/ja/SenseCAP_Sensor_Probes_Product_Catalogue/
---

## SenseCAP について

Seeed IIoT 製品シリーズの最初のローンチの中で、SenseCAP は無線環境センシングアプリケーションに焦点を当てています：スマート農業、精密農業、スマートシティなどです。ハードウェア製品（センサー、データロガー、ゲートウェイなど）、ソフトウェアサービス（SenseCAP Mate アプリ、SenseCAP ウェブポータル、オープンダッシュボード）、およびデバイスとデータ管理のための API で構成されています。

## センサープローブについて

当社の SenseCAP 産業用センサープローブシリーズは、産業用アプリケーションで一般的な振動や化学物質・ガスへの暴露などの過酷な環境に耐えるよう設計されており、当社の産業用センサーシリーズは IP66 から IP68 までの本格的な IP 等級を備えており、それぞれの過酷な環境で製品が確実に動作することを保証しています。
SenseCAP センサープローブは、測定対象媒体への確実な取り付けを保証するプロセス接続と、産業用計装システムに適合するよう信号を変換するセンサーデータトランスミッターも特徴としています。
一部の MODBUS RS485 産業用センサーには 2 つのバージョンがあります。主な違いはケーブルコネクターです。お客様のニーズに最適なバージョンをお選びください：
（1）フックアップワイヤー付きのものは、[SenseCAP LoRaWAN® データロガー](https://www.seeedstudio.com/SenseCAP-S2100-LoRaWAN-Data-Logger-p-5361.html?queryID=82d7de6ca64b8316686bdfe2c05664d2&objectID=5361&indexName=bazaar_retailer_products)と組み合わせて使用し、LoRaWAN® ネットワークに接続して、低消費電力・長距離通信の利点を享受することをお勧めします。
（2）航空コネクター付きのものは、すでに事前設定された [SenseCAP センサーハブ 4G データロガー](https://www.seeedstudio.com/SenseCAP-Sensor-Hub-4G-Data-Logger-with-builtin-battery-p-4852.html?queryID=b8ef1fa7e141765c30033fc3d8eddf10&objectID=4852&indexName=bazaar_retailer_products)でプラグアンドプレイ体験により、3G/4G LTE 経由で様々なセンサーデータを収集できます。
...
# 産業グレード MODBUS RS485 NH3 センサー - 防水航空コネクター付き
...
[1] センサーの電源投入からデータが読み取れるまでの時間。センサーの電源投入時のパラメータに注意してください。

[2] きれいな空気の環境で開始した場合、より短い時間で済みます

[3] 測定データの更新間隔。電源投入ウォームアップ時間後、電源供給が継続される場合、センサーはこの間隔で定期的に読み取り値を更新します。

[4] T90＜30s

## 産業グレード MODBUS RS485 H2S センサー - 防水航空コネクタ付き
...
[1] センサーの電源投入からデータ読み取りまでの時間。センサー電源投入時のパラメータに注意してください。

[2] きれいな空気環境で開始した場合、より短い時間で済みます

[3] 測定データ更新間隔。電源投入ウォームアップ時間後、電源供給が継続される場合、センサーはこの間隔で定期的に読み取り値を更新します。

[4] T90＜30s

## SenseCAP CO2、温度・湿度センサー RS485 & SDI-12 対応、防水航空コネクタ付き
...
[1] センサーの電源投入からModbus命令による測定値読み取り準備完了までに必要な準備時間。読み取りごとにセンサーの電源をオン・オフする際に注意すべきパラメータです。

[2] ウォームアップ時間 = (レジスタ0x0021の値 + 3)；ウォームアップ時間前にレジスタ0x0000を読み取ると0になり、ウォームアップ時間後は毎秒読み取り値が更新されます；センサーのT90時間は300秒、T60時間は120秒です。

[3] 測定データ更新間隔。電源投入ウォームアップ時間後、継続的に電源供給される場合、センサーはこの間隔で定期的に読み取り値を更新します。

[4] Modbusマスタークエリ頻度。

[5] 読み取り命令受信後にセンサーが応答するまでに必要な時間。応答遅延レジスタ0x0020が0に設定された後の時間です。

## 産業用 MODBUS RS485 大気温度・湿度センサー
...
## 産業用土壌水分・温度センサー MODBUS-RTU RS485、0-2Vアナログ電圧 (S-Soil MT-02A)

フックアップワイヤ付き [101990668](https://www.seeedstudio.com/RS485-Soil-Moisture-Temperature-Sensor-S-Soil-MT-02-p-4634.html?queryID=d56c42134f064eaf1203b9292aaba7c8&objectID=4634&indexName=bazaar_retailer_products) 
...
## 産業用土壌水分・温度・EC センサー MODBUS-RTU RS485 (S-Soil MTEC-02A)

フックアップワイヤー付き [101990667](https://www.seeedstudio.com/RS485-Soil-Moisture-Temperature-EC-Sensor-S-Soil-MTEC-02-p-4633.html) 
...
## 産業用光強度センサー、MODBUS-RTU RS485 &0-2V (S-Light-01)

フックアップワイヤー付き [314990739](https://www.seeedstudio.com/light-intensity-sensor-p-a-4863.html?queryID=f94dd76cedb45ec9d57e70d6e82af574&objectID=4863&indexName=bazaar_retailer_products)
...
## 産業用 PAR センサー (S-PAR-02A)、MODBUS-RTU RS485

フックアップワイヤー付き [314990733](https://www.seeedstudio.com/RS485-S-PAR-02A-p-4829.html)
...
## 産業用 PAR センサー (PAR-2.5V)、アナログ 0~2.5V 出力対応
...
## 産業用全天日射センサー (S-ZFS-02A)、MODBUS-RTU RS485
...
## 産業用UV放射センサー (S-ZW-01A), MODBUS-RTU RS485
...
## 産業用葉面湿度・温度センサー (A), MODBUS-RTU RS485

フックアップワイヤー付き [314990737](https://www.seeedstudio.com/leaf-wetness-sensor-p-a-4861.html)
...
## 産業用液体レベルセンサー MODBUS RS485 航空コネクタ
...
## 産業用pHセンサー MODBUS-RTU RS485 & 0~2Vアナログ電圧 (S-pH-01A) V2.0

フックアップワイヤー付き  [101990666](https://www.seeedstudio.com/RS485-pH-Sensor-S-pH-01A-p-4632.html)
