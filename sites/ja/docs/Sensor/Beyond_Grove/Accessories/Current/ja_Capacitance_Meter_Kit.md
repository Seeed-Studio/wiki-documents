---
title: 静電容量計キット
nointro: null
keywords:
  - ドキュメント
  - ドキュサウルス
image: https://wiki.seeedstudio.com/ja/Capacitance_Meter_Kit/
slug: /Capacitance_Meter_Kit
last_update:
  date: 05/15/2025
  author: gunengyu
createdAt: '2025-05-27'
updatedAt: '2025-09-17'
url: https://wiki.seeedstudio.com/ja/Capacitance_Meter_Kit/
---


![https://www.seeedstudio.com/depot/images/product/cmk1.jpg](http://bz.seeedstudio.com/depot/images/product/cmk1.jpg)

これはシンプルながら非常に便利な静電容量計です。新バージョンでは以前のバージョンの設計上のエラーを修正し、いくつかの改善が加えられました：

* 改良されたPCBレイアウト。
* 電源接続を簡単にするための一般的なDCジャックを追加。
* スイッチを追加。バッテリーで動作する場合に便利です。
* 取り付け穴を追加。
* 極性を明示。
* より小型化。

[![](https://files.seeedstudio.com/wiki/Seeed-WiKi/docs/images/300px-Get_One_Now_Banner-ragular.png)](https://www.seeedstudio.com/Capacitance-Meter-Kit-p-268.html)

## 特徴

* 約1%の精度
* 測定範囲：1pF - 500uF
* 自動範囲切り替え
* ゼロ調整可能
* タイムスタンプ付きの測定結果をリアルタイムでシリアル出力
* 低コストで簡単に組み立て可能

## 部品表（BOM）/ パーツリスト

**キット内容：**

* 電源を除くすべての部品
* 回路図
* マニュアル

## リソース

* [静電容量計使用例の写真](http://www.jyetech.com/Products/CapMeter/eCapMeter_Examples.html)

* [静電容量計マニュアル](http://www.jyetech.com/Products/CapMeter/DN060-02v02.pdf)

## 技術サポート & 製品ディスカッション
技術的な問題がある場合は、[フォーラム](http://forum.seeedstudio.com/)に問題を投稿してください。  
弊社製品をお選びいただきありがとうございます！製品の使用体験がスムーズになるよう、さまざまなサポートを提供しています。異なる好みやニーズに対応するため、複数のコミュニケーションチャネルを用意しています。

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>