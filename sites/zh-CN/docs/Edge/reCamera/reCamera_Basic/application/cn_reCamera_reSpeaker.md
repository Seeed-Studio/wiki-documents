...
url: https://wiki.seeedstudio.com/cn/reCamera/reCamera_Basic/application/sound_source_localization_of_recamera_gimbal_bak/
---
...
我们决定来点有趣的实验，把音频和视觉边缘 AI 结合起来玩一玩！🤖
...
首先，不要连接 reCamera Gimbal 的电源，直接用 Type-C 连接电脑，访问 192.168.42.1，进入后点击左下角的 "Login to SenseCraft"，登录后给它连接 WiFi，确保连接的 WiFi 与你的电脑相同，然后查看 reCamera Gimbal 的 IP 地址，复制该 IP 地址并访问，确认可以正常访问。接着拔掉 Type-C 线，插上 12V 电源，待 reCamera Gimbal 自动校准完成后，在浏览器中重新访问刚才的 IP 地址，进入 reCamera Gimbal 的界面，确保 Node-RED 已经正常启动运行。
...
访问 [GitHub 项目](https://github.com/xr686/reCamera-Gimbal-reSpeaker-Tracker.git)，下载压缩包并解压。

为了通过 USB 与 reSpeaker 通信，在你的电脑上安装依赖：

\# 从提供的 requirements 文件安装所需的 Python 包

```bash
pip install -r Sound-Tracking/requirements.txt
```

\# 通过 Conda 安装 libusb（后端驱动支持所必需）
...
-**智能安防**：结合 Home Assistant，在房间出现异常噪音时自动跟踪并记录。
...
同时，reCamera 会快速转向声音的物理角度。

3. **物理朝向校准**：reSpeaker 麦克风阵列的“0 度”朝向可能与 reCamera 默认正前方的“0 度”并不完全一致。如果你发现云台每次转动后总是固定偏离一个角度（例如总是偏差 90 度），可以直接在 Node-RED 中双击新建的 `Calculate & Throttle` function 节点，取消注释相关代码并修改偏移计算：`targetYaw = (targetYaw +90)% 360;`。

4. - - 防抖设计 **：由于 Python 代码每 0.1 秒就会刷新并发送一次数据，如果将这些高频信号全部推送给电机控制器（CAN 总线），很容易导致设备总线过载卡顿。因此，我在 Node-RED 的 function 节点中写入了防抖逻辑（只有当角度变化“>5 °”或时间间隔“>1 second”时才会触发下发）。
