...
url: https://wiki.seeedstudio.com/cn/reCamera/reCamera_Basic/application/use_cpenclaw_to_control_the_recamera_gimbal_bak/
---
...
安装完成后，你可以直接进入网页尝试与它聊天。如果此时可以正常获得反馈，则说明你已经成功安装并配置好 OpenClaw，请继续按步骤往下操作。如果安装 OpenClaw 出现问题，请自行排查原因并解决。
...
首先，不要接入 reCamera Gimbal 的电源，直接用 Type-C 连接电脑并访问 192.168.42.1。进入后，点击左下角的 "Login to SenseCraft"。登录后，为其连接 WiFi，确保连接的 WiFi 与电脑相同。然后查看 reCamera Gimbal 的 IP 地址，复制该 IP 地址并访问，确认是否可以正常访问。
...
在 SenseCraft AI 平台上，依次进入 **`Applications`** > **`Application Square`**（或[直接打开](https://sensecraft.seeed.cc/ai/application)），找到名为 "OpenClaw & reCamera Gimbal" 的项目，点击 **`Deploy Application`** > **`Visit Workspace Via USB`**，部署后你看到的工作流应如下图所示：
...
请确保你的模型配置正确。点击左侧的 "Agent" 后，你可以在 "Primary model" 下看到你已经配置好的模型。这里我使用的是 GLM4.7 模型。
...
### 5.3 使用 OpenClaw Gimbal 开始控制你的 reCamera

请直接在聊天窗口中对 OpenClaw 说“打开相机的 LED 灯”、“环顾四周看看有没有人”、“录制一段 5s 的音频并播放”等任务，或者你也可以尝试更复杂的任务，例如：“打开 reCamera 的哨兵模式（快速环顾四周，一次扫描周期，即一圈扫描控制在 6s 内），如果画面中出现穿白色 T 恤的人，就打开相机补光灯并播放 test.wav 音频，分析画面内容并将图片保存到本地”，然后稍等片刻，观察 reCamera Gimbal 的反应。
