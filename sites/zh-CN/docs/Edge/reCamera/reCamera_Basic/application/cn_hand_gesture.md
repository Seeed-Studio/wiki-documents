...
:::note
在构建本方案前，请确保已按照[主项目文档](https://wiki.seeedstudio.com/cn/recamera_develop_with_c_cpp/)配置好 **ReCamera-OS** 环境（版本 0.2.1 或更高），包括 SDK 路径和交叉编译工具链。
:::
...
克隆[仓库](https://github.com/RobotXTeam/sscma-example-sg200x/tree/main)，进入对应方案目录进行构建：
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `palm_model` | 手掌检测模型（必填） | - |
| `landmark_model` | 关键点检测模型（必填） | - |
| `embedder_model` | 手势特征嵌入模型（必填） | - |
| `classifier_model` | 手势分类模型（必填） | - |
