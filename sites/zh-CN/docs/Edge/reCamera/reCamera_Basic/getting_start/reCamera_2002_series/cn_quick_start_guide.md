...
url: https://wiki.seeedstudio.com/cn/reCamera/reCamera_Basic/getting_start/recamera_2002_series_quick_start_guide
---

# reCamera 2002 系列快速入门指南

<div align="center"><img width={1000} src="https://files.seeedstudio.com/wiki/reCamera/recamera_banner.png" /></div>

欢迎使用 reCamera！本指南将帮助你快速完成设备的设置，并开始使用它来解锁强大的 AI 视觉功能。无论你是初学者还是有经验的用户，本分步教程都会引导你完成安装、配置以及首次使用。
...
将 USB 线缆从 reCamera 连接到你的电脑，然后在浏览器中访问 ```192.168.42.1```，即可打开 reCamera 的加载页面。
...
1. **确保处于同一网络**：请确保 reCamera 和你的电脑连接到**同一个 WiFi 网络**。这可以保证后续的开发和调试能够正常进行。
...
如果你想使用以太网接口，可以使用盒子内附带的网线连接到你的路由器。注意，这**不是 POE（以太网供电）**。你仍然需要使用 Type-C 为设备供电。你可以**拧开相机背面**，通过接口进行连接。
...
开机后，reCamera 会开启热点（AP），方便你修改网络偏好设置。打开手机或笔记本电脑的 Wi-Fi 列表，你应该能看到一个名为 `reCamera_******` 的 Wi-Fi。
该命名规则为 ``"recamera_" + 设备 MAC 地址的后六位``。
默认密码为 `12345678`。
...
如果你有多台 reCamera，可以参考[设备管理](https://wiki.seeedstudio.com/cn/recamera_getting_started/#多设备管理)来区分每一台设备。

## 基础网页访问

当你已将 reCamera 连接到网络后，可以在浏览器中访问对应 IP 地址，进入 reCamera 的网页界面。
...
**请检查你的设备 OS 版本**。在 0.1.3 与 0.1.4 版本之间有一次重大升级。为获得更流畅的使用体验，我们建议升级到 0.1.5 及以上版本。请前往 ```Sidebar -> System -> Software Update``` 查看软件版本，并应用和安装最新的 OS 版本。更新说明请参考 [OTA 升级指南](https://wiki.seeedstudio.com/cn/recamera_getting_started/#ota-从-013-升级到最新版本)。
...
**步骤 5**：你将进入加载页面以修改密码。现在，你可以按照[此下载说明](https://wiki.seeedstudio.com/cn/recamera_getting_started/#访问-recamera-预览仪表盘)体验带有 Node-RED 仪表盘的最新版本 reCamera。

<div align="center"><img width={600} src="https://files.seeedstudio.com/wiki/reCamera/OSupgrade5.png" /></div>

### 访问 reCamera 预览仪表盘

通过将设备与 Node-RED 集成，我们提供了一个内置的仪表盘 UI 示例，你可以在其中预览视频流、切换所需模型。你也可以自由调整并与自己的应用集成。
...
通过修改流程中的 `function nodes`，你可以添加更多需要统计的物体，或者将此功能完全更改为其他计算机视觉应用。
...
你可以随意以任何方式调整和玩转流程。只是要注意 **不要** 在模型节点后放置太多 `debug nodes` 或 `preview nodes`，因为从 AI 模型中打印出多个结果会消耗 CPU 并导致设备崩溃。

### 计算机视觉模型

对于这些提供的工作流，默认选择了 `YOLO11n detection model`，这是最新的实时目标检测计算机视觉模型。你也可以在节点中将其替换为其他模型。如果你想查看还有哪些可用模型，可以参考这个 [wiki](https://wiki.seeedstudio.com/cn/recamera_on_device_models/)。
...
**Confidence**：在 YOLO 模型中，Confidence 表示预测的边界框中包含目标的概率以及该预测的准确程度。它是一个介于 0 和 100 之间的数值。
...
这里我们以在仪表盘中添加补光灯功能为例，简单地添加一个节点并控制 reCamera。
...
**步骤 3**. 进入 dashboard 2.0，并将控制按钮框放置在模型选择下方。
...
**步骤 5**：不要忘记导出该流程的 json 文件并分享至我们的 GitHub。
...
默认的相机视角方向如下图所示，此时 type-c **朝下**。请注意，其他朝向可能会影响模型的准确性，这取决于你是如何训练检测模型的。
