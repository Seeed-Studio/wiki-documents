...
#### 偏航轴处理 (Process Yaw)
* **节点类型**: `http in` $\rightarrow$ `function` $\rightarrow$ `set-motor-angle`
* **逻辑摘要**: 
  * 默认角度为 `180°`。
...
#### 数字变焦处理 (Process Zoom)
* **节点类型**: `http in` $\rightarrow$ `function` $\rightarrow$ `ui-template`
* **逻辑摘要**: 
  * 默认缩放比例为 `1.0`。
...
| 按键功能 | 节点类型链路 | 逻辑摘要说明 |
| :--- | :--- | :--- |
| **LED 切换 (Toggle LED)** | `http in` $\rightarrow$ `function` $\rightarrow$ `exec` | 内部维护一个 `ledState` 变量进行 **0 和 1 的状态翻转**。状态为 1 时通过 Linux 命令行向系统写入 `brightness` 开启白色 LED，状态为 0 时关闭。 |
| **音频录制 (Record Audio)** | `http in` $\rightarrow$ `exec` | 调用系统底层 `arecord` 工具，以 16000Hz 采样率、单声道格式，**录制 5 秒钟音频**并保存为本地的 `/home/recamera/test.wav`。 |
| **音频播放 (Play Audio)** | `http in` $\rightarrow$ `exec` | 调用系统底层 `aplay` 工具，直接**播放**刚刚录制好的 `test.wav` 音频文件。 |

---

## Stream Deck 配置
为了让 Stream Deck 的操作顺滑且不会每次触发都弹出一个浏览器窗口，我们将使用 Stream Deck 系统自带的**“网站” (Website)** 动作，并开启后台静默运行。
