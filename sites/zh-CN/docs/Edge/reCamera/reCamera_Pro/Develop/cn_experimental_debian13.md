...
reCamera Pro 由 RV1126B 芯片驱动，提供 2 GB 或 4 GB 内存版本。其出厂预装 Buildroot 固件，便于快速开始进行 AI 推理。本页面为需要更高开发与部署灵活性的用户提供 Debian 13 镜像。
...
:::warning
该固件目前为实验性版本。Seeed 目前不对其进行维护，仅作为额外的开发选项提供。
:::
...
4. 在 SocToolKit 中右键单击并在右键菜单中选择最后一项。
...
7. 选中 `rootfs` 条目并点击其右侧的省略号（`...`）。将其镜像文件替换为解压固件目录中的 `rootfs_debian_clean.img`。
...
:::caution
烧录会覆盖设备上的系统数据。继续之前，请备份重要数据，并确保设备处于 Loader 模式。
:::
...
然后克隆 `Linux_Upgrade_Tool` 并使 `upgrade_tool` 具有可执行权限：
...
如果你需要通过 HTTP 代理访问网络（例如使用 `apt` 时），可以临时设置以下环境变量。如果不使用代理，请跳过此步骤。将示例地址和端口替换为你的代理服务器地址和端口。
