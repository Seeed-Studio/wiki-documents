...
description: 学习如何在 reCamera Pro 上部署一个由凝视触发的、完全本地运行的双语流式语音识别系统，并通过终端、网页和 RTSP 查看状态。
keywords:
  - reCamera Pro
  - RV1126B
  - RKNN
...
它适用于语音助手、展览展台以及隐私优先的边缘语音入口。视觉、姿态评估、语音特征提取和 Zipformer 推理都在本地运行。Web 仪表盘不依赖任何云服务、Node.js 进程、CDN 或外部字体。

源代码和后续发布版本可在 [recamera_pro_face_stt GitHub 仓库](https://github.com/yyling0101-a11y/recamera_pro_face_stt)中获取。该仓库会在推送本目录后包含完整项目。
...
默认状态包括 `IDLE`、`FACE_DETECTED`、`ATTENTION_PENDING`、`LISTENING` 和 `COOLDOWN`。当绝对偏航角不超过 18° 且绝对俯仰角不超过 13° 时，视为一次有效注视。保持该姿态 700 ms 会触发 `LISTENING`。只有在监听状态下才会开始采集音频，因此应用不会持续录音。
...
:::note
默认音频输入为 `ai_asr`：16 kHz、S16_LE、4 通道，选择通道 0。这样可以避免与占用 `hw:0,0` 的厂商进程发生冲突。
:::
...
`scripts/build_recamera.sh` 还会验证 `librknnrt.so`。如果你的 SDK 或 runtime 使用了其他路径，请在脚本中将 `qualified_rknnrt` 更新为一个已验证的 RKNN 2.3.2 runtime，同时保留校验和验证步骤。
...
将部署目录的**内容**复制到 reCamera Pro 上的某个目录中。对于 IP 为 `192.168.42.1` 的设备：
...
:::tip
请保持 `models/` 和 `web/` 目录处于所示的相对路径。应用会使用这些默认相对路径加载其资源。
:::
...
| `--attention-dropout-ms N` | `250` | 从开始注视起，允许短暂姿态无效的宽限时间（毫秒）。设为 `0` 可禁用。 |
...
| `--stt-vocab FILE` | `models/stt/vocab.txt` | 词表路径。它必须与所选模型集匹配。 |
...
| `--stt-min-speech-ms N` | `160` | 一次识别会话中可接受的最短语音时长（毫秒）。 |
| `--stt-end-silence-ms N` | `2500` | 语音开始后，用于结束识别的静音时长（毫秒）。 |
| `--stt-start-timeout-ms N` | `4000` | 视觉唤醒后若语音未开始的超时时间（毫秒）。 |
...
终端至少需要 80×18 个字符。左侧面板显示 STT 状态、部分文本和最终结果；右侧面板显示人脸数量、是否面向摄像头、姿态角度、RTSP 客户端状态以及视觉延迟。按下 `Ctrl-C` 可恢复普通终端。`--dashboard` 不能与 `--debug` 同时使用。
...
该页面会自动连接到 `ws://DEVICE_IP:8080/ws`，并在中断后重新连接。它会显示部分和最终转写文本、视觉状态、人脸数量、偏航/俯仰/翻滚角、RTSP 状态以及延迟图表。健康检查端点为 `http://DEVICE_IP:8080/health`。使用 `--web-port 8081` 可更换端口，或使用 `--web-page PATH` 指定自定义页面。
...
叠加层会显示人脸框、全部 98 个关键点、原始偏航/俯仰、`facing=YES/NO`，以及当前状态。绿色表示姿态在进入阈值范围内，橙色表示姿态有效但在阈值范围外，红色表示姿态无效。

## 调优

摄像头位置、镜头特性和环境噪声都会影响体验。请按以下顺序调优：

1. 使用 `--debug` 在正对摄像头时观察偏航/俯仰，然后调整进入阈值。
2. 调整 `--attention-dropout-ms` 以适应短暂遮挡；使用 `0` 可禁用宽限期。
3. 在目标环境中观察音频 RMS，并调整 `--speech-rms-threshold`。
4. 使用 `--stt-end-silence-ms`、`--stt-start-timeout-ms` 和 `--stt-max-ms` 在响应速度与支持更长指令之间取得平衡。

控制器使用 EMA 平滑和进入/离开滞后机制。单次无效姿态不会触发监听，也不会立即清除正在计时的注意力定时器，而人脸跟踪丢失会立即重置状态。

## 故障排查

| 问题 | 可能原因 | 解决方案 |
| --- | --- | --- |
| 构建无法找到 OpenCV 或 RKNN | SDK 环境缺失或运行时不兼容 | 确认 `RECAMERA_PRO_SDK`，加载 SDK 的 `env.sh`，并使用 RKNN 2.3.2。 |
| 找不到模型文件 | 部署目录结构未被保留 | 确认执行目录中存在 `models/` 和 `web/dashboard.html`。 |
| 视觉唤醒从未触发 | 摄像头错误、阈值过严或人脸过小 | 使用 `--debug` 和 RTSP 叠加层检查 `/dev/video13`、关键点以及偏航/俯仰。 |
| STT 报告音频错误 | `ai_asr` 不可用或其格式不同 | 在设备上测试 `arecord -D ai_asr -f S16_LE -r 16000 -c 4 -d 5 /tmp/test.wav`。 |
| 网页不可访问 | 端口不可达或 Web 服务被禁用 | 不要使用 `--no-web`；测试 `http://DEVICE_IP:8080/health`。 |
| 无法播放 RTSP | 网络、端口或 GStreamer 插件问题 | 使用 TCP 播放并确认固件中所需的 GStreamer 元素。 |
