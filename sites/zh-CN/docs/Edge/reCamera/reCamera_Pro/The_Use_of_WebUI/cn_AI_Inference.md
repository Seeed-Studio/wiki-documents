...
url: https://wiki.seeedstudio.com/cn/recamera_pro_ai_inference_legacy/
---
<!-- 旧版页面（reCamera Pro wiki 重构，第 2 阶段）：此页面已被 Build_Your_App/configure_detection.md（https://wiki.seeedstudio.com/cn/recamera_pro_ai_inference/）取代，该页面现在使用原始 slug /recamera_pro_ai_inference。此文件作为草稿（slug /recamera_pro_ai_inference_legacy）保留以供历史记录，并从正式构建中排除。请不要链接到此处。 -->

## 内容迁移索引（旧版页面）

| 原始章节 | 新位置 |
| --- | --- |
| 功能概览、模型配置、推理配置、实时监控 | [配置检测](https://wiki.seeedstudio.com/cn/recamera_pro_ai_inference/)（此 slug） |
| 模型管理、上传模型 | [上传并配置 RKNN 模型](https://wiki.seeedstudio.com/cn/recamera_pro_rknn_upload/) |
| SenseCraft 模型转换 | [SenseCraft ONNX-to-RKNN](https://wiki.seeedstudio.com/cn/recamera_pro_sensecraft/) |
| 推理输出配置 | [HTTP/UART 结果](https://wiki.seeedstudio.com/cn/recamera_pro_http_uart/)、[MQTT](https://wiki.seeedstudio.com/cn/recamera_pro_mqtt/) |
| 模型格式/兼容性汇总 | [模型兼容性与输出格式](https://wiki.seeedstudio.com/cn/recamera_pro_model_compatibility/) |
| Web UI 字段参考 | [Web UI 参考](https://wiki.seeedstudio.com/cn/recamera_pro_webui_reference/) |
...
通过此页面，用户无需使用命令行即可完成模型部署和推理配置。

## 模型管理

模型管理区域用于显示当前设备中已上传或内置的 AI 模型。列表会显示模型文件、模型名称、推理框架、算法类型、任务类型、版本、文件大小以及可用操作。

常见字段说明如下：

| 字段        | 描述                               |
| ------------ | ----------------------------------------- |
| Model File   | 设备上当前模型的文件名 |
| Model Name   | 模型的显示名称               |
...
reCamera Pro 可以使用内置检测模型，也支持用户上传自定义模型。对于目标检测场景，用户可以根据应用需求选择不同的模型，例如行人检测、安全帽检测、施工安全检测、车辆检测等。
...
在模型转换界面中，用户可以创建转换任务、上传 ONNX 模型文件，并可选地上传量化数据集。如果未提供量化数据集，系统可以使用默认数据集完成转换。
...
该功能降低了用户部署模型的门槛，使其能够快速将自训练的 ONNX 模型转换为设备可执行的模型。
...
对于用户自定义的检测模型，可以在配置页面手动添加检测类别，或通过 TXT 文件批量导入类别列表。配置完成后，推理结果将按照用户自定义的类别名称进行显示和输出。
...
页面右侧会显示当前推理状态，例如 **Running**，并显示当前实际推理帧率，方便用户判断模型是否正常运行。

## 实时推理监控

AI 推理页面提供了实时推理输出监控区域，用于查看模型推理结果。推理结果以日志形式实时显示，方便开发者调试模型输出、查看边界框坐标、类别 ID、置信度以及时间戳。
...
该页面提供了实时输出控制功能，例如禁用监控、暂停实时显示、清除日志等，方便用户在调试过程中观察和管理输出内容。

## 推理输出配置

除了可以在 Web UI 中查看实时推理日志外，reCamera Pro 还支持将推理结果输出到外部系统。用户可以在 **Inference Output Configuration** 中选择输出模板、任务类型和输出方式。
