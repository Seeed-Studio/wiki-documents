...
绿色 LED 表示系统状态，红色 LED 为电源指示灯。下表描述了绿色 LED 各种状态所对应的设备状态。

1. ***breathing*** ：此时表示设备正在开机过程中。
2. ***Solid on*** ：设备运行正常。
3. ***off*** ：异常状态，核心进程未运行。
4. ***100 ms blink*** ：当前处于 OTA 过程。
5. ***Short green blink*** ：检测到文件系统不干净的 SD 卡。
...
如果连接 Type-C 线缆后仍无法打开 Web 界面，通常是因为 Windows 系统的虚拟网卡未正确获取 IP 地址。请按以下步骤进行排查：
