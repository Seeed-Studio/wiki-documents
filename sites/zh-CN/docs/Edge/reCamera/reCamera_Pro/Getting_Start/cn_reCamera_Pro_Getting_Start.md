---
description: 本指南将协助您完成硬件连接、登录设备 Web 界面、配置无线网络以及排查常见连接故障。
...
欢迎使用 **reCamera Pro**。本指南将协助您完成硬件连接、登录设备 Web 界面、配置无线网络以及排查常见连接故障。
...
* **电源适配器**：一根 DC 电源线（规格要求：**12V, 1A**，出厂标配电源为：12V, 2A）。

### 连接步骤

1. 如下图所示，将 12V DC 电源线插入设备的电源接口，将 USB Type-C 线缆一端连接到 reCamera Pro，另一端连接到您的 Windows 主机（Host PC）。
...
2. 在地址栏中输入默认 IP 地址：`192.168.42.1` 并按回车键。
...
3. 开启 Wi-Fi 功能（如果默认处于关闭状态），系统会自动扫描附近的无线网络。
...
成功连接 Wi-Fi 后，您可以点击已连接的 Wi-Fi 名称查看其详细网络信息（例如分配的 IP 地址、MAC 地址等）。
...
6. 确认配置中已勾选**自动获取 IP 地址**和**自动获取 DNS 服务器地址**。
...
### Q2：通过 USB 线连接 reCamera 到电脑后，电脑无法上网

如果您的 MAC 电脑在通过 USB 线连接 reCamera 后无法访问互联网，这是因为虚拟网卡会接管 MAC 的 WIFI，导致 MAC 无法连接互联网。请按照以下步骤解决问题：

1. 打开 ***MAC 的系统设置 -> 网络***。
2. 在右下角（或右下侧）找到一个***内部有三个点“...”的圆形图标***（或一个向下的箭头，具体取决于您的 MAC 版本）。
3. 点击 ***设置服务顺序...(Set Service Order...)***。
4. 在弹出的列表中，将 WIFI 拖动到最上方，确保其位于 USB 虚拟网卡（如 USB_Gadget）之前。
5. 点击“OK”保存。之后，在使用 MAC 上网时会优先使用 WIFI 连接，只有在访问 reCamera 时才会使用 USB 连接。
