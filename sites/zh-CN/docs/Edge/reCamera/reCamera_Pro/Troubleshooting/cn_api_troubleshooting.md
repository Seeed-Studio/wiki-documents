---
description: 诊断并修复常见的 reCamera Pro Web API 故障——登录错误、Token 过期、网络变更、HTTPS 证书以及 API 路径问题。
title: 排查 API 调用问题
keywords:
  - Edge AI
  - reCamera Pro
...
Token 默认有效期为 24 小时（`Max-Age=86400`）。过期后，请重新登录以获取新的 Token。
...
设备使用的是自签名证书。对于本地测试，你可以忽略警告继续访问。对于生产部署，请安装受信任的证书。
...
大多数端点需要使用 CGI 基础路径 `/cgi-bin/entry.cgi`。串口和文件相关端点是绕过 CGI 层的直连路径，通过 `/api/v1/...` 访问。

### 为什么某个端点会返回 404？

当前固件版本可能尚未实现该端点，或者路径构造不正确。请确认该路径是否需要 `/cgi-bin/entry.cgi`，并检查固件版本。

有关响应状态码的完整列表，请参见 [API 错误码](/cn/recamera_pro_api_error_codes/)。有关 API 的通用用法，请从 [API 简介](/cn/recamera_pro_api_introduction/) 开始。
