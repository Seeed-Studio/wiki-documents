---
description: 本维基将简要介绍如何使用设备上的串口进行调试以及触发拍照/视频拍摄/录制。
title: reCamera Pro UART 使用
keywords:
  - reCamera
  - reCamera Pro
  - UART
...
1. 首先准备一个 USB 转串口工具，将转换器的 TX 接到设备的 RX 引脚，将转换器的 RX 接到设备的 TX 引脚，同时记得连接 GND。
...
该 UART 接口作为 Web UI 触发的 UART 接口使用，可用于接收 AI 推理数据，也可以用于发送命令以触发事件。

### 串口触发
1. 首先准备一根 10 针 MX1.25 线缆，并插入扩展接口。

2. 然后根据上方扩展接口图中的引脚顺序，将其连接到你的 USB 转 TTL 工具。将转换器的 TX 接到设备的 RX 引脚，将转换器的 RX 接到设备的 TX 引脚，同时记得连接 GND。
...
感谢您选择我们的产品！我们将为您提供多种支持，确保您在使用我们产品的过程中体验顺畅。我们提供多种交流渠道，以满足不同的偏好和需求。
