...
<div align="center"><img width={1000} src="https://files.seeedstudio.com/wiki/reCamera-Pro/Secondary_Development/cpp_skill/recamera_skill.png" alt="reCamera Pro Development Skill 工作流" /></div>

## 介绍

[reCamera Pro Development Skill](https://github.com/Seeed-Projects/recamera-pro-development-skill) 为 AI 编码代理打包了 reCamera Pro 专用说明、脚本、技术参考以及原生 C++ 应用模板。安装后，你可以用自然语言描述一个应用，让代理为你准备模型转换和原生开发工作流。
...
该仓库为 OpenAI Codex、Claude Code、GitHub Copilot、Cursor 和 Gemini CLI 提供了不同版本。每个版本都使用相同的 reCamera Pro 开发知识，只是安装和调用方式根据代理不同而有所区别。
...
如果你的代理支持从 Git 仓库安装 skill，向它发送如下请求：
...
若要使用其他受支持的代理，请将 `main` 替换为上表中对应的分支名。每个分支的安装脚本会将该 skill 复制到该代理的用户级 skill 目录中。

如果在当前会话中没有看到该 skill，请在安装后重启代理。

:::tip
请保留已克隆的仓库。若要在之后更新 skill，只需在同一代理分支上拉取最新更改，并按照该分支的 README 和安装说明操作。
:::

## 使用 Skill

你可以在提示词中显式写出该 skill 的名称：
...
安装 skill 之后，你也可以直接用自然语言描述任务。代理会自动加载相关的模型转换、交叉编译、相机、音频或流媒体指导。
...
代理会检查模型的输入输出约定，为 `rv1126b` 配置 RKNN-Toolkit2 2.3.2，并将预处理和转换元数据与生成的模型一同保存。
...
6. 确认生成的可执行文件为 aarch64 架构，并且其 RKNN、GStreamer、libc、C++ ABI 和运行时路径与设备兼容。
7. 为部署准备可执行文件、RKNN 模型、运行命令以及预期的输入/输出约定。

默认情况下，skill 会在本地构建和检查构建产物。只有在你明确请求并授权时，才会在 reCamera Pro 上执行文件传输和运行操作。
...
- 模型成功转换并不保证推理结果正确。请将数值输出和预处理与源框架进行对比。
- 应用会链接到与开发板兼容的 RKNN Runtime，并使用设备上已有的运行时；它不应替换 reCamera Pro 上已安装的 RKNN Runtime。

## 故障排查

| 问题 | 可能原因 | 解决方案 |
| --- | --- | --- |
| 未列出该 skill | 代理尚未重新加载其用户级 skill 目录 | 重启代理，并确认该 skill 已安装到上文所示目录 |
| 选择了错误的平台 | 混入了针对 SG2002/riscv64 reCamera 的说明 | 明确说明目标是 reCamera Pro、RV1126B 和 aarch64 |
| RKNN 转换失败 | Toolkit 版本、ONNX 算子支持或模型输入约定不兼容 | 先让代理检查 ONNX 模型，并使用 RKNN-Toolkit2 2.3.2 |
...
- [reCamera Pro 开发技能仓库](https://github.com/Seeed-Projects/recamera-pro-development-skill)
