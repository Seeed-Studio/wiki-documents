...
通过短接 TPM 模块的 MISO 和 MOSI 引脚，你实际上创建了一个回环场景，即 MOSI 发送的数据会在 MISO 上被接收。该设置允许你在没有连接实际设备的情况下测试 SPI 通信。
...
该命令以指定串口（***/dev/ttyUSB2***）和 115200 的波特率打开 minicom。
...
reComputer Industrial R21xx 包含 **2 路 RS485 接口**。它们对应的 **COM 口** 和 **设备文件** 如下：  
...
- 再按 ***O*** 打开配置，选择 Serial port setup，并按 ***Enter***；打开所有 RS485 相关选项，依次按 ***H/I/J/K/L*** 将其打开；
...
以 ACM2 和 ACM3 为例：
如果想从 ACM2 发送到 ACM3，需要重新设置 ACM2：***ctrl+A***，然后按 ***Z*** 再按 ***E***，然后开始串口写命令。此时可以在 ACM2 中任意打印字符串，同时可以在 ACM3 中看到 ACM2 的内容；
反之，如果想从 ACM3 发送到 ACM2，需要重新设置 ACM3：***ctrl+A***，然后按 ***Z*** 再按 ***E***，然后开始串口写命令。此时可以在 ACM3 中任意打印字符串，同时可以在 ACM2 中看到 ACM3 的内容，如图所示。
:::
...
你需要打开两个终端，连接了扩展板时为 ACM1，未连接扩展板时为 ACM2：
**终端 1：***
...
1. 使用两根杜邦线将 can0 和 can1 的 H-H、L-L 短接，形成最小回路测试。
...
8. 如果希望后续开机操作更简单，可以将启动命令写入脚本，只需在开机时执行一次即可自动拉起接口。
...
如果 USB Hub 工作正常，你应该能在 lsusb 命令的输出中看到它的详细信息。如果没有列出，可能是 Hub 本身或其与系统的连接存在问题。在这种情况下，你可能需要对 USB Hub 或其连接进行排查。
...
5. 断开 RTC 的电源，等待几分钟后重新接通电源，再次查看 RTC 时间，以确认是否保持了正确时间。
...
与蜂鸣器对应的 GPIO 为 gpio627。输入以下脚本即可打开/关闭蜂鸣器：
...
该命令会显示系统中连接的所有磁盘列表，如果 SSD 被正确检测到，也会包含在内。查找代表你 SSD 的条目，它们通常以 ***/dev/sd*** 开头，后面跟一个字母（例如 ***/dev/sda, /dev/sdb,*** 等）。
一旦确定了与你的 SSD 对应的条目，就可以根据需要继续对其进行分区或格式化。

## UPS 安全关机

CPU 与直流电源输入之间的 GPIO6 用于在电源掉电时向 CPU 报警。然后 CPU 应该在超级电容能量耗尽之前，通过脚本执行一些紧急操作并运行 "$ shutdown"。
使用此功能的另一种方式是：当 GPIO 引脚状态变化时发起关机。指定的 GPIO 引脚被配置为输入按键，用于产生 KEY_POWER 事件。systemd-logind 会处理该事件并发起关机。
...
保存并退出编辑器（按 ***`Ctrl+O`*** 保存，***`Enter`*** 确认，按 ***`Ctrl+X`*** 退出）。
...
如果你购买的是 R21xx-10 系列产品，则需要另外购买 Hailo 的 NPU 模块以启用 AI 功能。
