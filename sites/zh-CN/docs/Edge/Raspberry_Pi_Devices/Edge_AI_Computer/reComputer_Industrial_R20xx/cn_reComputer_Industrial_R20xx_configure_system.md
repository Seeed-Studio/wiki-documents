...
通过短接 TPM 模块的 MISO 和 MOSI 引脚，你实际上创建了一个回环场景，其中 MOSI 发送的数据会在 MISO 上被接收。此设置允许你在没有实际外接设备的情况下测试 SPI 通信。
...
10-15 秒后（模块上电并通过 USB 枚举需要一段时间），检查设备节点是否出现：
...
2. 若要使用 4G 模块拨打电话号码，可以使用 ATD 命令并在后面加上电话号码：
...
执行以下命令后，模块会自动重启。如果你不退出 minicom，可以看到相应的配置信息。
ECM 拨号上网：
...
- 先按 ***Ctrl+A***，再按 ***Z***，会出现 Minicom Command Summary 界面：
...
reComputer Industrial R20xx 包含 1 个 RS232 接口，其对应的 COM 端口和设备文件如下：
...
DI 接口的输入类型为 PNP。其支持的输入电压为 5VDC~24VDC，电流 - 1000mA。
...
rtc0 无法保持时间数据。因此，要使用 RTC 功能时，需要在设置时间时手动指定设备号 rtc1。
:::
...
如果你在输出中看到 ***tpm0*** 和 ***tpmrm0***，则表示系统已检测到 TPM（可信平台模块）设备并可用。这表明 TPM 硬件已被识别且可访问，这是一个良好信号。你可以在确认这些设备存在且可访问的前提下，继续使用与 TPM 相关的功能或应用程序。
...
在识别出对应于 SSD 的条目后，你就可以根据需要对其进行分区或格式化操作。
...
reComputer Industrial R20xx 上的 M.2 M-KEY 2280 插槽用于安装 PCIE M.2 AI 加速器。而 R20xx-12 系列已预装 Hailo-8 M.2 AI 加速模块，性能最高可达 26TOPS。
