...
reComputer Industrial R20xx 支持多种物联网无线通信，包括 4G、5G、LoRa®、Wi-Fi/BLE 和 Zigbee。它支持通过 M.2 插槽扩展 SSD 存储，或通过 NPU 加速器扩展 AI 能力。如果你在购买时直接选择包含 AI 加速器，你将获得额外 26TOPS 的计算能力。此外，我们的硬件工程服务还包括 Logo 定制、包装品牌化、标签、固件烧录和镜像服务，为你的特定需求提供全面支持。

本 Wiki 将向你展示如何装配和拆卸设备以安装外设组件，以及安装方式的选项。
...
reComputer Industrial R20xx 提供多种安装方式。包装内已包含导轨卡扣和安装螺丝。请按照示意图，将导轨卡扣正确固定到设备侧面的安装孔上。螺丝拧紧后，即可将设备安装到导轨上。
...
reComputer Industrial R20xx 也适用于垂直壁挂安装，但安装支架不包含在包装内，需要另外购买。
