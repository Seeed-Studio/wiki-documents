...
该设备专为 **7×24 小时可靠运行**而设计，具备**宽电压输入（9-36V）**、**硬件看门狗**和**高效散热**，可在 **-20°C 至 65°C** 环境下稳定工作。非常适合用于**智能工厂**、**安防监控**和 **AIoT**，为**边缘侧**带来**强大的 AI 计算能力**。
...
- **灵活存储选项**：PCIe3.0 双 M.2 插槽同时支持 AI 加速器和 SSD 存储。
...
[Hailo](https://hailo.ai/) 提供专为边缘设备高性能深度学习应用打造的前沿 AI 处理器。其解决方案专注于在边缘侧实现新一代生成式 AI，同时支持感知与视频增强，这些能力由先进的 AI 加速器和视觉处理器提供。搭载 Hailo-8 NPU 加速器、具备 26 TOPS AI 性能的 reComputer_R2000，可在 YOLOv8s 上实现超过 200 FPS 的推理速度。
...
Hailo AI 软件套件提供强大的工具，可在硬件加速器上高效运行 AI 模型。它被设计为可与现有深度学习框架无缝集成，为开发者提供顺畅的工作流。流程包括在模型构建环境中从 ONNX 文件生成 HEF（Hailo 可执行二进制文件）。生成后，将 HEF 文件传输到推理设备（运行环境），并通过 HailoRT API 执行推理。提供的脚本可在模型构建环境中将 ONNX 文件转换为 HEF 文件。
...
reComputer AI Industrial R2135 通过 9~36V 直流端子电压供电。电源通过 2 针电源端子座连接。要对 reComputer AI Industrial R2135 接地，可将地线固定在电源端子左上角的螺丝上。

#### 功耗

reComputer AI Industrial R2135 的功耗测试数据来自 Seeed Studio 实验室，请参考下表。请注意，该数值仅供参考，由于测试方法和环境不同，结果可能会有所差异。
...
|**满载**|12V|2.08A|24.2W| 使用 "stress -c 4" 命令让 CPU 以满载运行。USB 端口外接 1A 负载。|

#### 开关机

reComputer AI Industrial R2135 默认不带电源按键，接通电源后系统会自动启动。关机时，请在操作系统中选择关机选项，并等待系统完全关机后再切断电源。要重启系统，只需重新接通电源即可。
...
reComputer AI Industrial R2135 配备了 3 个 LED 指示灯，用于指示设备的运行状态。各 LED 的具体功能和状态请参考下表：

| 名称  | 颜色            | 状态 | 说明                                                                                                                                              |
|-------|-----------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| **PWR**  | 绿色           | On     | 设备已接通电源。                                                                                                                |
|        |                 | Off    | 设备未接通电源。                                                                                                                    |
| **ACT**  | 橙色          |        | 在 Linux 下，该引脚会通过闪烁来表示 eMMC 访问。如果在启动过程中发生任何错误，该 LED 会以错误模式闪烁（参见 Raspberry Pi 文档）。 |
| **USER** | 绿/红/蓝  |        | 需要由用户自定义。                                                                                                                            |
| **LTE**  | 绿色           | On     | 拨号成功且连接正常。                                                                                                |
|        |                 | Off    | LTE 信号未连接或设备未上电。                                                                                             |
...
为了控制用户 LED，我们建议使用 sysfs，这是 Linux 内核提供的一个伪文件系统，用于暴露各种内核子系统、硬件设备及其相关驱动的信息。在 ReComputer R2000 上，我们将用户 LED 接口抽象为三个设备文件（led-red、led-blue 和 led-green），用户只需与这些文件交互即可控制 LED 灯。示例如下：
...
如果 USB 集线器工作正常，你应当能在 **lsusb** 命令的输出中看到它的详细信息。如果没有列出，可能是集线器本身或其与系统连接存在问题。在这种情况下，你可能需要对 USB 集线器或其连接进行故障排查。
...
reComputer AI Industrial R2135 配备两个 M.2 Key-M 插槽（NVMe1 和 NVMe2），用于 NVMe M.2 2280 SSD 和 AI 加速，支持高速存储扩展，使用户能够提升系统的性能和容量。
...
>  
> 市面上的 SSD 卡并非都支持启动功能。  
> 如果你计划将 SSD 用作启动盘且不确定兼容性，我们推荐 **1TB SSD（SKU 112990267）**。该型号已经过测试验证支持启动，可帮助避免兼容性问题并减少试错成本。
...
reComputer AI Industrial R2135 配备一个标准 RJ45 千兆以太网端口（GbE），使用带集成变压器的 MagJack，以获得更好的信号质量和 EMI 防护。
...
reComputer AI Industrial R2135 支持丰富的扩展模块和配件，适用于多种场景和需求。如果你对定制 reComputer AI Industrial R2135 感兴趣，请联系 odm@seeed.cc 了解更多信息。
以下是配件和可选模块列表：
...
<div class="table-center">
  <table border="1" cellspacing="0" cellpadding="6">
    ...
    <tbody>
      ...
      <tr>
        <td rowspan="7">4G 功能需要搭配 4G 模块使用 4G 天线，GPS 功能需要搭配 4G 模块使用 GPS 天线</td>
        <td>4G 模块</td>
...
TPM（可信平台模块）是一种专门为增强计算机安全性而设计的硬件芯片，通过提供基于硬件的加密功能来实现。它可以安全地存储加密密钥、证书和密码等敏感数据，常用于安全启动、磁盘加密（例如 BitLocker）和身份验证等场景。

在 reComputer AI Industrial R2135 上使用如下命令检查 TPM：
...
> 2. **带镜像的启动盘：** SSD 卡既可以作为大容量存储设备，也可以通过存储系统镜像作为启动盘，从而实现从卡中直接启动系统。  
>
> 需要注意的是，市面上的 SSD 卡并非都支持启动功能。如果你计划将 SSD 用作启动盘，但不确定选择哪一款型号，我们推荐我们已经测试过的 1 TB SSD（SKU 112990267）。该型号已验证支持启动功能，可降低兼容性风险并减少反复试错成本。  
...
Frigate 是一个开源 NVR（网络视频录像机），专为使用 AI 进行实时目标检测而设计。它可以与现有摄像头集成，并使用 TensorFlow、Coral 等机器学习模型对视频流进行目标检测。Frigate 针对低延迟和高性能视频处理进行了优化，提供运动检测、实时视频流和自动告警等功能。
...
YOLO（You Only Look Once）系列模型是一类为速度和精度而设计的实时目标检测模型。与传统的先生成候选区域再分别进行分类的目标检测方法不同，YOLO 在一次神经网络前向传播中同时完成区域预测和分类，因此速度更快。YOLO 模型将图像划分为网格，并为每个网格单元预测边界框和类别概率。多年来，YOLO 经过多个版本的演进，在精度、速度以及检测小目标的能力方面不断提升。YOLOv4、YOLOv5 以及最新的 YOLOv7 和 YOLOv8 广泛应用于监控、自动驾驶和机器人等场景。
...
CLIP（对比语言-图像预训练，Contrastive Language-Image Pretraining）是由 OpenAI 开发的一个机器学习模型，能够同时理解图像和文本。它通过训练将图像与相应的文本描述关联起来，从而可以执行涉及这两种模态的任务。CLIP 具备零样本学习能力，这意味着它无需针对特定类别进行专门训练，也能识别图像中的物体和概念。它在多种任务中表现出色，例如图像分类、目标检测，甚至为图像生成文本描述。
