...
2. 当设备处于关机或睡眠状态时，无法正确烧录固件。如果你选择了设备的正确端口，但始终看不到固件烧录进度，则可能需要按下设备顶部的绿色按键唤醒设备，然后重试。
...
**步骤 2.** 连接成功后，你的手机应会自动跳转到 Wi-Fi 配置页面。如果没有自动跳转，请在浏览器中访问 `192.168.4.1`。
...
通过 Gallery 功能，将你的 reTerminal E1001 打造成数字相框。上传你喜欢的图片，平台会为电子纸显示屏进行优化。你可以创建带有自定义切换时间的幻灯片放映。
...
按照上一节所述，将设备与 SenseCraft 平台配对后，你就可以开始创建第一个仪表盘了。关于每个功能的详细使用说明，请参考对应的 Wiki 页面：
...
reTerminal E1001 配备一个 8 针扩展排针（J2），可用于连接外部传感器、模块或其他硬件，以扩展设备功能。该扩展排针引出了多个 ESP32-S3 GPIO 引脚和通信接口，使其成为 DIY 项目中非常灵活的连接点。
...
**步骤 4.** 固定好之后，将 reTerminal E1001 放置在平坦的表面上，使支架能够支撑其保持竖直状态。
