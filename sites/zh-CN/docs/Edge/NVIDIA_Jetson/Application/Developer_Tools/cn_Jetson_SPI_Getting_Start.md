...
在本指南中，使用 ST7789 SPI LCD 显示屏作为示例。对于其他 SPI 显示模块，例如 ST7735 或 ILI9341，整体流程类似，但连线细节、显示分辨率、初始化序列和驱动参数可能会有所不同。
...
| SPI 显示模块                 | 本指南以 ST7789 SPI LCD 显示屏为例                   |
...
示例程序使用 sysfs GPIO 控制 `DC` 和 `RES` 引脚。
...
示例程序使用 RGB565 格式填充整个屏幕。示例程序中使用的显示分辨率为：
...
整屏缓冲区大于一次小的 SPI 传输。因此，示例程序分块发送显示数据：
...
如果 `/sys/class/gpio/PQ.05` 或 `/sys/class/gpio/PQ.06` 不存在，则可能是你的系统镜像中 GPIO 命名或导出方式不同。
...
如果背光亮起但没有图像显示，则可能是电源接线正确，但 SPI 通信或显示初始化不正确。
...
4. 将当前的颜色填充测试仅作为基础硬件验证示例程序使用。

## 总结

在本篇 wiki 中，我们介绍了如何通过 40-pin 排针将 SPI 显示屏连接到 Seeed reComputer J4012 Classic。通用的 SPI 显示屏工作流程包括连接显示屏、启用 SPI 接口、检查 SPI 设备节点、安装依赖、编译 C++ 示例程序以及运行显示测试。
