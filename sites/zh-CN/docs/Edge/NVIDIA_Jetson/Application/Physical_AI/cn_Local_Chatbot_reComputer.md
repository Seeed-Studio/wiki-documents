...
url: https://wiki.seeedstudio.com/cn/local_chatbot_recomputer_bk/
---
...
所有组件都是自包含的并在本地运行，让您完全控制您的数据——无云依赖、无延迟、无隐私担忧。
...
我们将首先安装 [Jetson Containers](https://github.com/dusty-nv/jetson-containers) 来轻松管理和部署我们在 Docker 内的服务。
...
💡 **提示：**从较小的模型（如 `llama3.2:1b`）开始测试您的配置，然后扩展。
...
直接在终端中运行 Ollama 或通过提供的 Python 脚本 `ollama_run.py`。
...
为了实现真正的语音交互，我们使用**NVIDIA Riva**，一个 GPU 加速的工具包，提供：

- **自动语音识别（ASR）**  
- **文本转语音（TTS）**  
- **神经机器翻译（NMT）**（可选）

它可以部署在任何地方——从云端到像**Jetson 系列**这样的嵌入式设备。

### 步骤 1：获取您的 NGC API 密钥

1. 登录到 [NVIDIA NGC](https://catalog.ngc.nvidia.com/)。
2. 创建一个**API 密钥**并在本地保存——您需要它来访问 Riva 资源。
...
您现在可以**与您的助手对话**，它将**聆听、思考并回话**——全部在本地，由您的 Jetson 提供支持。
