...
- **打造最强大的边缘计算 AI 计算机：** 设计用于集成 Jetson AGX Orin 模块，最高可达 275 TOPS AI 性能，相比 Jetson AGX Xavier 提升 8 倍 AI 性能。功耗可在 15W 到 60W 之间配置。
- **为多种传感器提供高速接口支持：** 22 条 PCIe Gen4 通道、1x 10GbE、1 个 Display Port、16 条 MIPI CSI-2 通道、USB 3.2 接口以及 40 针排针。
- **为多种 IO 提供低速接口支持：** 4x DI、4x DO、3x GND_DI、2x GND_DO、1x GND_ISO、1x CAN、1x RS232/RS422/RS485。
...
1x RS232/RS422/RS485 </td>
    ...
    <tr>
      <td rowSpan={9}>扩展</td>
      <td> Mini PCIE </td>
      <td> 1x Mini PCIe，用于 LoRaWAN®/4G/系列无线（不含模块） </td>
    </tr>
    <tr>
      <td> M.2 Key B </td>
      <td> 1x M.2 Key B (3042/3052)，支持 4G/5G（不含模块） </td>
    </tr>
    <tr>
      <td> M.2 Key E </td>
      <td> 1x M.2 Key E </td>
    ...
    <tr>
      <td> TPM </td>
      <td> 1x TPM 2.0 接口（不含模块） </td>
    </tr>
    <tr>
      <td> RTC </td>
      <td> 1x RTC 插座 (含 CR1220), <br />1x RTC 2 针 </td>
    </tr>
    <tr>
      <td> 摄像头 </td>
...
## 将 JetPack OS 烧录到 J501 载板

在这里，我们将向你展示如何将 [Jetpack](https://developer.nvidia.com/embedded/jetson-linux-archive) 烧录到连接在 reServer J501 上的 NVMe SSD 中。
...
在这里，我们需要根据所使用的 Jetson 模块，在 Ubuntu 电脑上下载对应的系统镜像：
...
<table style={{textAlign: 'center'}}>
  ...
  <tbody>
    ...
    <tr>
      <td>AGX Orin 64GB</td>
      <td>❌</td>
      <td><a href="https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/IQClLB_mdGMPQpqEw1jxTRuFAYqxZRQJZIAtiYt7-clcocI" target="_blank" rel="noopener noreferrer">下载</a></td>
    ...
    <tr>
      <td>AGX Orin 64GB</td>
      <td>❌</td>
      <td><a href="https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/IQCcNYUtYA0RTL2GQwXFv35rAdWnduxknLXrUtQNxklpIZo" target="_blank" rel="noopener noreferrer">下载</a></td>
      <td> - </td>
...
**步骤 4.** 使用 USB Type-C 数据线将开发板连接到 Ubuntu 主机电脑。

**步骤 5.** 在 Linux 主机电脑上打开终端窗口并输入命令 `lsusb`。如果返回的内容中根据您使用的 Jetson SoM 出现以下任一输出，则说明开发板已进入强制恢复模式。
