---
description: 本文介绍如何在 J401-Mini 载板上烧录 JetPack 6.0，涵盖设备概览、准备工作、进入强制恢复模式以及使用一键脚本烧录，帮助用户快速部署 NVIDIA Jetson Orin 设备。
title: 烧录 Jetpack
keywords:
  - reComputer Mini
...
# 将 JetPack 烧录到 J401-Mini 载板

reComputer Mini 是一款由 NVIDIA Jetson Orin Nano/Orin NX 模块驱动的小型 AI 计算机，可提供高达 100 TOPS 的 AI 性能。其底部配备 PCIe 接口，提供丰富的扩展能力，并且可以灵活定制。整个系统专为嵌入无人机、巡逻机器人、配送机器人等自主机器而设计。它可以直接接入 54V 直流输入，能够广泛应用于电池供电系统。
...
- **面向量产的卓越 AI 性能：** 基于 NVIDIA Orin SoC，可在低功耗和低延迟下实现高达 **100 TOPS** 的 AI 性能，结合 NVIDIA Ampere™ GPU 架构与 64 位操作能力，集成先进的多功能视频与图像处理以及 NVIDIA 深度学习加速器。
- **掌上尺寸的边缘 AI 设备：** 体积紧凑，仅 **63mmx95mmx42mm**，集成 NVIDIA Jetson Orin NX 16GB 模块、Mini J401 载板、风扇和外壳。支持桌面和壁挂安装。
- **丰富 I/O 可扩展：** 包含多达 7 路 USB、1 路 DP 2.1、2 路 CSI、1 路用于 GbE 的 RJ45、M.2 Key E、M.2 Key M、双路 CAN，以及通过扩展板提供的 GPIO。
- **加速方案落地：** 在 128GB NVMe SSD 上预装 **JetPack 6.0**、Linux OS BSP，支持 Jetson 软件和主流 AI 框架。
- **规模化部署：** 支持 OTA，由 Allxon 和 Balena 提供远程管理服务。
- **灵活定制：** 包括更换配件模块、Logo 以及基于 reComputer Mini J4012 原始设计的硬件接口修改。
...
<table style={{textAlign: 'center'}}>
  <tbody>
    ...
    <tr>
      <td>UART</td>
      <td colSpan={3}>1x UART；1x 调试用 UART</td>
    </tr>
    <tr>
      <td>显示</td>
    ...
    <tr>
      ...
      <td >尺寸（宽 x 深 x 高）</td>
      <td colSpan={3}>63mm * 95mm * 42mm（无扩展板）<br />63mm * 95mm * 66.7mm（带扩展板）</td>
    </tr>
    <tr>
      <td>重量</td>
      <td colSpan={3}>345g（无扩展板）<br />462g（带扩展板）</td>
    </tr>
    <tr>
      <td>安装方式</td>
      <td colSpan={3}>桌面、壁挂</td>
    </tr>
    <tr>
      <td>工作温度</td>
...
在这里，我们将向你展示如何将 [Jetpack 6.0](https://developer.nvidia.com/embedded/jetson-linux-archive) 烧录到连接在 reComputer Mini 上的 NVMe SSD 中。
...
- Ubuntu 主机电脑
- 搭载 Jetson Orin 模块的 reComputer J401-Mini 载板
- USB Micro-B 数据传输线
...
- **步骤 2.**  使用一根针插入 RECOVERY 孔按下恢复按键，并保持按住。
- **步骤 3.**  接通电源。
- **步骤 4.**  松开恢复按键。
