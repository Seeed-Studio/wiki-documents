---
description: 本文提供了一个详细指南，介绍如何将 JetPack 操作系统烧录到支持 NVIDIA Jetson Orin NX/Nano 模组的 A608 载板上。内容涵盖从前置准备和进入强制恢复模式，到下载系统镜像和驱动程序，最后将操作系统烧录到 NVMe SSD、USB 闪存盘或 SD 卡的完整流程，确保用户能够顺利完成系统安装与启动。
title: A608 载板
keywords:
  - Edge
  - reComputer
...
# 将 JetPack OS 烧录到 A608 载板（支持 NVIDIA Jetson Orin NX/Nano）
...
在本 Wiki 中，我们将向你展示如何将 JetPack 烧录到连接在 A608 载板上的 NVMe SSD 和 USB 闪存盘上，该载板同时支持 NVIDIA Jetson Orin NX 模组和 NVIDIA Jetson Orin Nano 模组。

## 前置准备

- Ubuntu 主机电脑
- 搭载 Jetson Orin NX 或 Jetson Orin Nano 模组的 A608 载板
- USB Type-C 数据传输线
...
**步骤 1.** 关闭系统电源，请确保是断电关机，而不是进入待机模式。

**步骤 2.** 使用 Type C 转 USB Type A 线缆连接载板和主机。

**步骤 3.** 使用 GH1.25MM 锁扣端子线短接 Recovery 处的 pin1 和 pin2，使其进入恢复模式。

<div align="center"><img width={800} src="https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/hardware_connection.png" /></div>

**步骤 4.** 给设备上电。

**步骤 5.** 在 Linux 主机电脑上打开终端窗口并输入命令 `lsusb`。如果返回的内容中根据你使用的 Jetson SoM 出现以下任一输出，则说明载板已进入强制恢复模式。
...
这里我们将使用 NVIDIA L4T 35.3.1 在搭载 Jetson Orin NX 模组的 A608 载板上安装 Jetpack 5.1.1。

**步骤 1.** 在主机电脑上[下载](https://developer.nvidia.com/embedded/jetson-linux-r3531) NVIDIA 驱动程序。所需驱动如下所示：

<div align="center"><img width="{800}" src="https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/nvidia_driver.png" /></div>

**步骤 2.** [下载](https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/EZ5nv2iWBQlIvPq7_aTQiucBp_HUwDmFNgkBMR04SI_teg?e=wseTuy)外设驱动，并将所有驱动放在同一文件夹中。
...
烧录完成后，再次给 Jetson 设备上电并登录系统。
</TabItem>
<TabItem value="JP5.1.2" label="JP5.1.2">

这里我们将使用 NVIDIA L4T 35.4.1 在搭载 Jetson Orin NX 模组的 A608 载板上安装 Jetpack 5.1.2。

**步骤 1.** 在主机电脑上[下载](https://developer.nvidia.com/embedded/jetson-linux-r3541) NVIDIA 驱动程序。所需驱动如下所示：
<div align="center"><img width="{800}" src="https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/5.1.2_P1.png" /></div>

**步骤 2.** [下载](https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/EZcvwwGTgLBBq_M_pAa2tmEB-pZmFQraF9v9JcdiqcRbLA?e=Px98MO)外设驱动，并将所有驱动放在同一文件夹中。
...
这里我们将使用 NVIDIA L4T 36.3 在搭载 Jetson Orin NX 模组的 A608 载板上安装 Jetpack 6.0。

**步骤 1.** 在主机电脑上[下载](https://developer.nvidia.com/embedded/jetson-linux-r363) NVIDIA 驱动程序。所需驱动如下所示：
<div align="center"><img width="{800}" src="https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/5.1.2_P1.png" /></div>

**步骤 2.** [下载](https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/EZdUUKln2yBKhPS8yegaLzMBWZm2MtIaFnHbFYkwazArzA?e=blzKtI)外设驱动，并将所有驱动放在同一文件夹中。
...
在这里，我们将使用 NVIDIA L4T 36.4 在搭载 Jetson Orin NX 模组的 A608 载板上安装 Jetpack 6.1。

**步骤 1.** 在主机 PC 上[下载](https://developer.nvidia.com/embedded/jetson-linux-r3640) NVIDIA 驱动程序。所需驱动程序如下所示：
<div align="center"><img width="{800}" src="https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/5.1.2_P1.png" /></div>

**步骤 2.** [下载](https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/EVrGntfS1wxHhrgnwGeHQmQBtQ0gvHj4udkREIDIACvFDw?e=5B07Za)外设驱动，并将所有驱动放在同一个文件夹中。
...
在这里，我们将使用 NVIDIA L4T 36.4.3 在搭载 Jetson Orin NX 模组的 A608 载板上安装 Jetpack 6.2。

**步骤 1.** 在主机 PC 上[下载](https://developer.nvidia.com/embedded/jetson-linux-r3643) NVIDIA 驱动程序。所需驱动程序如下所示：
<div align="center"><img width="{800}" src="https://files.seeedstudio.com/wiki/reComputer-Jetson/A608/jp6.2.png" /></div>

**步骤 2.** [下载](https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/EYGdRLSx_oxDjagkG2J6GTYBB9TDLvTKagnRfQcbz6gplA?e=sswKna)外设驱动，并将所有驱动放在同一个文件夹中。
...
在这里，我们将使用 NVIDIA L4T R39.2.0 在搭载 Jetson Orin NX 模组的 A608 载板上安装 Jetpack 7.2。

**步骤 1.** 在主机 PC 上[下载](https://developer.nvidia.com/downloads/embedded/L4T/r39_Release_v2.0/release/Jetson_Linux_R39.2.0_aarch64.tbz2) NVIDIA 驱动程序和[root filesystem](https://developer.nvidia.com/downloads/embedded/L4T/r39_Release_v2.0/release/Tegra_Linux_Sample-Root-Filesystem_R39.2.0_aarch64.tbz2)：

- `Jetson_Linux_R39.2.0_aarch64.tbz2`
- `Tegra_Linux_Sample-Root-Filesystem_R39.2.0_aarch64.tbz2`

**步骤 2.** [下载](https://seeedstudio88-my.sharepoint.com/:u:/g/personal/youjiang_yu_seeedstudio88_onmicrosoft_com/IQBsOH7dA-WtQafi5M86XzZGATQW5JJObMxPNJvT_n_ESj0?e=AfZVk4) A608 Jetpack 7.2 外设驱动，并将所有驱动放在同一个文件夹中。
...
**步骤 5.** 系统备份与恢复（首先进入 Force Recovery Mode）。
