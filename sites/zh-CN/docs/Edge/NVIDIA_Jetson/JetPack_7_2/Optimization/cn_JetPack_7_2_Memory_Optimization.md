...
本指南整合了本合集里已有的 JetPack 7.2 资料：

- [JetPack 7.2 深度解析](/cn/jetpack72_deep_dive_bk/)，包括加载 27B 模型后测得的内存下降；
- [NVIDIA Skills 工作流](/cn/rapid_prototyping_on_jetson_with_nvidia_skills_bk/)，用于设备诊断、内存审计和无头部署；
- [TensorRT Edge-LLM 指南](/cn/deploy_tensorrt_edge_llm_on_jetpack7.2/)，用于在 JetPack 7.2 上进行 FP16、INT8 和 INT4 推理。
...
| BSP 内存回收 | 禁用未使用的显示或相机固件和保留内存 | 高 | 重建并重刷 |
| SWIOTLB 调优 | 在测量实际使用后缩小 DMA 跳转池 | 高 | 重建并重刷 |
...
不要只比较 `free` 中的 `used` 值。要结合使用 `MemAvailable`、进程 RSS 列表以及 `tegrastats` 报告的峰值。

## 2. 在编辑 BSP 之前使用 Skills 进行审计

基于技能驱动的工作流应从观察开始，而不是立即修改配置。
...
审计应在推荐更改之前给出证据。不要仅因为某个服务出现在进程列表顶部就将其禁用。
...
| 场景 | 预期部署 | 回收的平台区域 |
| --- | --- | --- |
| `headless` | 无本地显示输出 | DCE/显示固件、早期帧缓冲区以及匹配的内核节点 |
| `no-camera` | 无 CSI、GMSL 或其他相机管线 | RCE、VI、ISP、NVCSI 以及匹配的固件 carveout |
| `swiotlb` | 实测 DMA 跳转池使用远低于保留池 | 更小但非零的 SWIOTLB 分配 |
...
对于 carveout 更改，MB1 BCT、MB2 加载控制、MB2 AST 引用和内核设备树节点必须保持一致。仅将某个 carveout 条目清零并不是有效的优化。对于 SWIOTLB，切勿配置大小为零的池，并且如果 `io_tlb_used` 接近 `io_tlb_nslabs`，要立即回退。
...
不要假设仅更改引擎标志就能正确量化 FP16 checkpoint。请使用模型支持的 checkpoint 和导出路径，然后在 JetPack 7.2 上重建 TensorRT 引擎。

### 控制上下文、KV 缓存和并发度

LLM 内存并不只由模型权重决定。KV 缓存会随着上下文长度、批大小、生成的 token 数量以及并发请求数而增长。
...
当多个应用需要同一个模型时，使用一个长时间运行的模型服务器。独立的 Python 脚本、notebook、测试服务器和生产服务都可能各自再加载一份权重或引擎副本。

在启动推理前，检查是否已有模型进程在运行：
...
TensorRT Edge-LLM 使用 x86 GPU 主机进行 checkpoint 导出，并在 Jetson 上构建目标引擎。导出可能需要数倍于 checkpoint 大小的 RAM 和 VRAM，因此将导出保留在主机上，可以为 Jetson 留出内存用于验证和推理。

在构建引擎期间，关闭无关的模型服务器，并将峰值内存与运行时内存分开记录。构建时的内存压力不一定代表稳态部署需求。

![TensorRT Edge-LLM engine build](https://files.seeedstudio.com/wiki/tensorRT-edge-llm_imgs/6.png)

### 将 Swap 视为恢复工具，而不是免费的 DRAM

Swap 可以帮助一次性的模型转换或引擎构建完成，但持续的交换会降低延迟并可能增加存储磨损。对于实时推理，在依赖 swap 之前，更应优先选择更小或量化的模型、更短的上下文、更低的并发度以及更少的重复服务。
...
| 解码吞吐量 | 确认更低内存没有让推理变得不可用地缓慢 |
| 温度和板级功耗 | 确认结果是稳定的，而不是短暂的突发 |

[JetPack 7.2 深度解析](/cn/jetpack72_deep_dive_bk/) 记录了在 Seeed 的对比中，加载 27B 模型后内存从 JetPack 6.2 上的大约 **24.6 GB** 降至 JetPack 7.2 上的大约 **14.7 GB**。将该结果视为特定工作负载的参考，而不是对每个模型都保证的降幅。
...
1. 测量空闲、引擎负载、预填充和解码内存。
2. 移除重复的模型进程和不必要的应用服务。
3. 减少上下文、生成长度、批大小和并发数。
4. 评估由 TensorRT Edge-LLM 支持的 INT8 或 INT4 检查点。
5. 对于无显示的设备部署，使用 `jetson-headless-mode`。
6. 仅在硬件场景完全匹配时使用 `jetson-optimize-memory headless` 或 `no-camera`。
7. 仅在测量实际 DMA 跳转池使用情况后再考虑减少 SWIOTLB。
8. 每次更改后重新运行正确性、延迟、吞吐量、散热和稳定性测试。

## 回滚

- 如果再次需要图形桌面，请恢复原始服务目标。
- 如果 carveout 或设备树更改导致启动或外设故障，请恢复干净的 BSP 源码并重新烧录。
