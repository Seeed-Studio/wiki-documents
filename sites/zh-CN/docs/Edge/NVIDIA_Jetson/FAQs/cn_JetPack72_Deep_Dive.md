...
JetPack 7.2 通过更新的 Jetson Linux 基础、Ubuntu 24.04、CUDA 13、更新的 AI 运行时组件，以及在内存效率和 Agentic AI 工作流方面的平台级改进，将 Jetson 软件栈向前推进。
...
Seeed 使用了同一块 Jetson AGX Orin 32GB 模组，对比了在大语言模型推理负载下 JetPack 6.2 与 JetPack 7.2 的软件环境。

| 项目 | JetPack 6.2 测试 | JetPack 7.2 测试 |
| --- | --- | --- |
| 模组 | Jetson AGX Orin 32GB | Jetson AGX Orin 32GB |
| 载板 | AGX Orin H01 | reComputer Classic J5011 |
...
#### Q3: Seeed 观察到哪些内存和性能提升？

在 Seeed 的对比中，JetPack 7.2 在模型加载后降低了内存占用，并提升了提示词处理速度和 Token 生成速度。

| 指标 | JetPack 6.2 | JetPack 7.2 | 观察到的变化 |
| --- | --- | --- | --- |
| 模型加载后内存占用 | 24.6 GB / 30 GB | 14.7 GB / 30 GB | 降低约 40% |
| 推理期间 GPU 频率 | 930 MHz | 1.36 GHz | 更高的加速频率 |
| 提示词处理 | 18.2 tokens/s | 25.8 tokens/s | 约快 41.8% |
| Token 生成 | 4.3 tokens/s | 5.5 tokens/s | 约快 27.9% |

最具实用价值的结果是内存余量。在 JetPack 6.2 的运行中，27B 模型在加载后占用了大部分可用内存。在 JetPack 7.2 的运行中，系统大约多保留了 10 GB 可用内存，这在同时运行视觉预处理、机器人中间件或其他与 LLM 负载并行的服务时非常有用。
...
| 场景 | 建议 |
| --- | --- |
| 新的 Jetson AGX Orin 项目 | 如果所需的 BSP、驱动和应用栈已经就绪，可考虑直接从 JetPack 7.2 起步。 |
...
| 使用自定义载板的量产系统 | 不要仅通过运行 `apt upgrade` 来升级。请使用经过验证的完整镜像，或 Seeed 官方支持的 OTA 升级路径。 |
...
有关该工具的更多细节，请参考 [Seeed Jetson DevelopTool](https://github.com/Seeed-Projects/Seeed-Jetson-DevelopTool)。

#### Q7: 升级后我应该验证哪些内容？

在刷机或升级完成后，在运行生产负载之前请验证以下项目：
