...
EasySkyMesh 增加了**省电和传感器数据采集**功能。MeshCore 节点可以采集传感器数据，并通过现有的 MeshCore 网络进行传输。它特别适用于：
...
* Wio Tracker L1。[点击此处](https://www.seeedstudio.com/Wio-Tracker-L1-Pro-for-Meshcore-p-6717.html)获取。
* USB Type-C 线缆
* 电脑
...
EasySkyMesh 的发布版本可能会针对不同硬件和不同升级场景提供不同的固件文件。对于全新安装，项目通常会提供全新安装固件镜像。对于已有安装，如果你希望保留现有配置，请使用升级固件。
...
该项目提供针对不同 MeshCore 开发板进行优化的 PowerSaving 固件发布版本。EasySkyMesh 发布版本已经为基于 ESP32 和 nRF52 的设备引入了省电特性，并为不同 MeshCore 角色增加了额外的电源管理功能。
...
[点击此处](https://github.com/IoTThinks/EasySkyMesh/wiki/PowerSaving#11-repeaters-and-room-servers)以使用更多命令。
...
如果问题仍然存在，请查看 [EasySkyMesh GitHub 仓库](https://github.com/IoTThinks/EasySkyMesh)中的最新文档和 issue。
...
* [EasySkyMesh 传感器使用说明](https://github.com/IoTThinks/EasySkyMesh/blob/main/firmware/Add-sensors-to-repeaters.md)
