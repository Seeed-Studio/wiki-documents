...
[Meshtastic®](https://meshtastic.org/) 是一个开源、离网、去中心化的 Mesh 网络，构建于经济实惠、低功耗的设备之上，它使你能够在没有现有或可靠通信基础设施的区域，使用廉价的 LoRa 电台作为远距离离网通信平台。
...
Meshtastic 使用 LoRa，这是一种远距离无线电协议，在大多数地区无需额外的许可证或认证即可广泛使用，这一点不同于 HAM 业余无线电操作。

此外，Meshtastic 电台可以与一部手机配对，使朋友和家人能够直接向你的特定电台发送消息。需要注意的是，每个设备在同一时间只能支持一个用户的连接。"
...
Meshtastic 节点地图是一个在线界面，用于在地图上可视化 Meshtastic 设备的位置。它帮助用户监控 Mesh 网络中设备的实时位置，从而了解网络覆盖范围和连接情况。
...
[Meshtastic](https://meshtastic.org/docs/introduction/) 网络监控工具可以让你在 Meshtastic Mesh 网络中对节点进行可视化、管理和监控。它提供关于设备状态、路由和通信的实时信息。该监控工具是 Meshtastic 分析器，通过连接的网关或 MQTT 服务器收集数据，可视化网络拓扑，并显示电池状态、信号强度和 GPS 位置等节点信息。它也可以作为 Meshtastic 节点地图使用。
...
将 MQTT 与 Meshtastic 集成，可以通过桥接远距离网络来扩展离网通信。这使得全球消息传递成为可能，在 Mesh 信号较弱时提供备份，并通过互联网确保更快速的消息传递，非常适合偏远地区和紧急情况。
