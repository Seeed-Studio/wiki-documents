...
url: https://wiki.seeedstudio.com/cn/wio_sx1262_with_xiao_esp32s3_kit/
---
...
这款拇指大小的 LoRa 开发套件由 XIAO ESP32S3 和 Wio-SX1262 LoRa 模组组成，是 Meshtastic 和 LoRa/LoRaWAN 项目的理想入门套件。它支持 WiFi、BLE 和 LoRa，内置电源管理芯片，可通过 IIC、UART 和其他 GPIO 接口进行扩展，并兼容 Arduino 开发。
XIAO ESP32S3 & Wio-SX1262 套件是一款单通道 LoRaWAN LoRa 模组，价格低廉且性价比高。适用于远距离、低功耗、小数据量的应用场景。它支持 30 个以上的节点设备（取决于上传间隔、数据载荷等）。

Seeed 为这款 XIAO ESP32S3 & Wio-SX1262 套件提供了 3 种应用方式。
...
- **Meshtastic 开箱即用**：预先烧录 Meshtastic 固件，上电即可工作。
- **出色的射频性能**：支持 LoRa（862-930MHz）、2.4GHz Wi-Fi 和 BLE 5.0 双无线通信，连接 U.FL 天线时可实现 2~5km（LoRa）和 100m+（Wi-Fi/BLE）的远程通信。
- **拇指大小的紧凑设计**：21 x 18mm，采用经典 XIAO 外形尺寸，适用于可穿戴设备等空间受限项目。
- **强大的 MCU 开发板**：集成 ESP32S3 32 位双核 Xtensa 处理器，最高运行频率 240MHz，板载多种开发接口，支持 Arduino / MicroPython。
- **精心设计的电源方案**：集成 Type-C USB 接口和锂电池充电管理。

## 规格参数

| 特性 | 详情 |
| ---- | ---- |
| ESP32S3 | ESP32-S3R8，Xtensa LX7 双核 32 位处理器，最高运行频率 240 MHz |
| LoRa | 基于 Semtech SX1262，支持 862-930MHz |
| BLE | Bluetooth 5.0，Bluetooth mesh |
| Wi-Fi | 完整的 2.4GHz Wi-Fi 子系统 |
| 接口（ESP32S3） | 1x UART，1x IIC，1x IIS，1x SPI（连接到 SX1262），11x GPIO（PWM），9x ADC，1x 用户 LED，1x 充电 LED，1x 复位按钮，1x Boot 按钮 |
| 接口（Wio-SX1262） | 1x 用户按钮，1x SPI（连接到 ESP32S3） |
| 电源 | 输入电压（Type-C）：5V <br/> 输入电压（BAT）：4.2V <br/> 充电电流：100mA |
| 工作温度 | -40°C ~ 65°C |
| 尺寸 | 21 x 17.8mm |
...
兼容 XIAO 扩展板，可用于增加屏幕和传感器，并可通过 Grove 扩展板接入 Grove 传感器。
...
:::note
原装 FPC 天线：仅用于**测试**。如有可靠性要求，请另外购买天线 2。
:::
...
<table align="center">
 ...
 <tr>
     <th>XIAO ESP32S3 正面标注示意图</th>
 </tr>
 <tr>
     <td><div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/img/front-indication.png" style={{width:700, height:'auto'}}/></div></td>
 </tr>
    <tr>
     <th>XIAO ESP32S3 背面标注示意图</th>
 </tr>
    <tr>
...
:::note
请注意，`Wio-SX1262 for XIAO ESP32S3` 采用 B2B 连接，而 `Wio-SX1262 for XIAO nRF52840` 采用通孔排针连接，这两款扩展板都必须与对应的 XIAO 开发板配套使用。
:::
...
- [Wio-SX1262 模组数据手册](https://files.seeedstudio.com/products/SenseCAP/Wio_SX1262/Wio-SX1262_Module_Datasheet.pdf)
