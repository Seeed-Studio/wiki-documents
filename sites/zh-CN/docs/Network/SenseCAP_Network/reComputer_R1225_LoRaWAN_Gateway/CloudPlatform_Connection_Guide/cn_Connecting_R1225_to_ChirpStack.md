...
ChirpStack 为 LoRaWAN 网络提供开源组件。它们共同组成了一个开箱即用的解决方案，包括用于设备管理的用户友好型 Web 界面以及用于集成的 API。

### 添加网关

开始之前，请先[登录](https://www.chirpstack.io/application-server/use/login/)到 [ChirpStack Application Server](https://www.chirpstack.io/application-server/)。

默认凭据为：用户名：admin；密码：admin

:::tip Note
如果你尚未将你的 [ChirpStack Application Server](https://www.chirpstack.io/project/application-server/) 实例与 [ChirpStack Network Server](https://www.chirpstack.io/project/network-server/) 实例连接，则需要先完成此操作。请参阅 [Network servers](https://www.chirpstack.io/application-server/use/network-servers/)。此外，你还需要通过创建一个 [Service profile](https://www.chirpstack.io/application-server/use/service-profiles/) 将组织与 network-server 连接起来。
:::
...
**Gateway EUI**：网关 EUI 可以在设备标签或本地控制台中找到
...
在你将设备添加到 ChirpStack 之前，如果尚未创建 [device-profile](https://www.chirpstack.io/application-server/use/device-profiles/)，则必须先创建一个。
...
:::note
请根据你的设备选择 MAC 版本/区域参数修订版本/ADR 算法。详情请参考：https://lora-alliance.org/resource_hub/
:::
...
**Device EUI**：设备 EUI 可以在设备标签或 SenseCAP Mate APP 中找到

**Device profile**：选择我们创建的设备配置文件。
...
在将 LoRaWAN 设备添加到 ChirpStack 后，验证你的设备是否能够激活（如果是 OTAA）并发送数据。
