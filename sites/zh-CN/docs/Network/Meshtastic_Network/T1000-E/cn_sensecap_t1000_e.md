...
:::danger note
当设备处于以下状态时，请不要手动重启或关机。否则设备可能会损坏报废。
1. 尚未完成消息传输过程
2. 正在被配置
:::
## 视频教程
...
:::caution note
在刷写固件前，请确保你购买的是 `T1000-E for Meshtastic`。请不要给不支持 Meshtastic 的其他 tracker 型号刷写该固件。请`不要使用 NRF-OTA` 来更新固件，否则可能会导致设备完全损坏。
:::
...
按一次按键即可打开设备电源，会听到上升音阶的提示音，LED 指示灯会持续亮约 1 秒。

:::tip
如果按下按钮后设备没有反应，请先为其充电。请不要使用快充充电器。
:::
...
如果你希望在网页端发送文本消息并与其他节点通信，现在可以将设备连接到 [Meshtastic 网站](https://client.meshtastic.org/messages/broadcast/0)。

  第 1 步：打开网站

[点击这里](https://client.meshtastic.org/messages/broadcast/0) 访问网站。
...
为了在 mesh 网络中开始通信，你必须设置你的区域。该设置决定设备使用的频率范围，应根据你所在的地区进行设置。
...
:::info
**EU_868** 必须遵守每小时 10% 的占空比限制，以滚动 1 小时为周期，每分钟计算一次。如果达到该限制，设备将停止发射，直到再次被允许。
:::
...
依次进入 `Settings` -> `Telemetry(Sensors)` -> 启用传感器。
...
依次进入 `Settings` -> `External Notification` -> 启用 `GPIO` -> 设置 `Output Pin GPIO`。
...
:::tip
在你更新设备配置之后，设备会重启，这可能需要一些时间。
:::

### 配置铃声

依次进入 `Settings` -> `Ringtone Config`，然后输入你希望用于外部通知的 RTTTL 铃声字符串。
...
请将 GPS 设置为启用状态。你可以调整更新间隔和广播间隔，以获得更加实时的位置数据。
...
依次进入 `Settings` -> `Firmware Updates`，查看当前固件版本。
...
:::caution note
在烧录固件之前，请确保你购买的是 `T1000-E for Meshtastic`。请不要将该固件烧录到不支持 Meshtastic 的其他追踪器型号上。请`不要使用 NRF-OTA` 更新固件，否则可能导致设备彻底损坏。
:::
#### 步骤 1：进入 DFU 模式
...
### 如何查看设备名称
 ...
 点击 `Open Serial Monitor`，将设备连接到电脑，在串口日志中查看关键字 `using nodenum`。
...
- 拔下 USB 线，然后按住按键不放，再在按住按键的同时重新插上 USB 线并保持按住 5 秒。
...
- 有时看起来像是关机了，但实际上是因为 LED 和蜂鸣器没有被激活。请检查以下参数：
 ...
 - 如果仍然没有效果，请按住设备按键，然后连接充电线，查看电脑中是否弹出一个磁盘。如果有，请[点击这里](https://wiki.seeedstudio.com/cn/sensecap_t1000_e/#device-bricked)重新安装 bootloader
...
- 步骤 1：尝试手动进入 DFU 模式：按住设备按键，然后**快速**连接充电线两次，绿色 LED 将保持常亮。

:::note
要成功进入 DFU 模式，你需要快速完成该操作，可能需要多次尝试。
:::
...
如果你在使用 PyPi 安装时遇到问题或想要修改工具，请使用此方法。首先克隆此仓库并进入其文件夹。
...
注意：以下命令使用 `python3`，但是如果你使用的是 Windows，可能需要改为 `python`，因为 Windows 上的 Python 3.x 安装仍然使用 python.exe 这个名称。
...
你可以在 `Adafruit_nRF52_nrfutil\nordicsemi\dist\adafruit-nrfutil` 中找到 .exe（如果你在 Windows 上则带有 `.exe`）。
为方便使用，可以将其复制或移动到其他位置，例如你的 %PATH% 中的某个目录。
...
对于 Windows 用户，打开设备管理器，进入“端口”选项，新出现的端口号就是设备的端口号。
...
在终端或命令提示符中，导航到你下载引导程序 zip 包的目录，并执行以下命令，将端口替换为你设备的正确端口：
...
请将 COMXX 改为你的串口号。例如，如果你的设备在 com6 上，将命令修改为：
...
完成以上步骤之后，你可以按照此[步骤](https://wiki.seeedstudio.com/cn/sensecap_t1000_e/#flash-the-application-firmware)烧录应用固件。
...
- 连接设备。
   当你连接设备时，指示灯会短暂闪烁。持续尝试，直到指示灯常亮，意味着设备可以回到 DFU 模式，然后依次执行 [烧录引导程序](https://wiki.seeedstudio.com/cn/sensecap_t1000_e/#flash-the-bootloader) -> [擦除 Flash](https://wiki.seeedstudio.com/cn/sensecap_t1000_e/#step-2-flash-erase) -> [烧录固件](https://wiki.seeedstudio.com/cn/sensecap_t1000_e/#step-3-flash-firmware)。
...
- 按住设备按键不放，然后连接充电线。当电脑中弹出一个磁盘后，你可能就能看到串口了。

- 如果仍然不行，请断开充电线，将设备放置几天直到电池完全耗尽，然后再接上充电线并尝试重新配对。

**4) 如果以上步骤都无法解决问题，请联系技术支持：support@sensecapmx.com**
...
- **无法打开串口**
 ...
 这可能是因为在设备处于以下状态时，手动强制重启或关闭了设备：尚未完成消息发送过程、正在配置中......
...
- 重置前，请确认问题确实与异常的节点列表有关。
- 如果问题只是节点显示延迟，建议先等待一段时间，看看是否会自动恢复。
- 如果在重置 NodeDB 后问题仍然存在，请继续从设备配置或其他可能原因进行排查。
...
### 信号质量
  ...
  - **RSSI** 由设备及其周围环境共同决定。正常设备通常在 -110 dBm 以上工作。RSSI 低于 -115 dBm 的设备被认为性能较差。
      ...
      为获得最佳信号效果，请在开阔、无遮挡且干扰尽可能少的环境中使用设备。
