---
description: 本指南说明如何使用 Meshtastic Node Map 显示 Meshtastic 设备在地图上的位置。
...
url: https://wiki.seeedstudio.com/cn/meshtastic_node_map_x1/
---
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

本指南说明如何使用 [MeshTracker X1](https://www.seeedstudio.com/Wio-Tracker-L1-Pro-p-6454.html) 在 Meshtastic Node Map 上显示 [Meshtastic](https://meshtastic.org/docs/introduction/) 设备的位置，前提是这些设备已经刷入 Meshtastic 固件。
...
Meshtastic 设备会通过 MQTT 定期向 Meshtastic 服务器上报其位置。Node Map 获取这些上报数据，并显示每个节点的位置、路径和信号范围。设备可以通过以下方式上报位置：

- **直接通过 MapReport：** 设备直接向服务器发送位置信息。
- **通过其他节点间接上报：** 位置信息通过邻近节点中继。
...
| [Liam Cottle 的 Meshtastic Map](https://meshtastic.liamcottle.net/) | 实时跟踪、历史路径查看、信号范围显示 | 高级用户、数据分析 | 使用自定义 MQTT 服务器 |
...
你将在地图上看到所有节点。通过 **via MapReport** 上报的节点是设备直接更新的。通过 **via other nodes** 上报的节点是中继更新的。
...
## Liam Cottle 的 Meshtastic Map 入门
...
在浏览器中打开 **Liam Cottle 的 Meshtastic Map**： [Liam Cottle 的 Map 链接](https://meshtastic.liamcottle.net/)。你将在地图上看到所有节点。
