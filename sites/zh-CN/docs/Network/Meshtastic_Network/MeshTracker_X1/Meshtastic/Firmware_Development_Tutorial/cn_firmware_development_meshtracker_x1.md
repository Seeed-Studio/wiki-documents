---
description: 一篇关于搭建 Meshtastic 固件源代码环境、编译 MeshTracker X1 并烧录固件的实用教程。
title: Meshtastic 开发教程
keywords:
  - Meshtastic
  - Source Code
  - PlatformIO
...
在这个阶段，先不要急着修改代码。请先确保项目可以顺利跑通完整的构建流程。
...
如果界面与上图类似，说明构建流程已经正确启动。第一次构建通常会花费较长时间，请耐心等待。
