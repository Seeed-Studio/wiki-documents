---
description: 搭建 MeshCore 固件源代码环境、编译 Tracker L1 并烧录固件的实用教程。
...
找到你目标板卡对应的 environment。以 L1 Pro companion 为例：
...
然后双击 RST 按钮进入 DFU 模式。将 uf2 文件拖入弹出的磁盘中。uf2 文件应位于 `.pio\build\WioTrackerL1_companion_radio_ble` 中。

### 示例

#### 用户显示控制

本示例展示如何修改 L1 Pro 的 UI 显示。它会删除屏幕上的消息显示，持续显示“Test”。
...
**2. 建议改进 / 新功能**
- 新建一个 issue，并在标题前加上前缀 [Feature request]
- 解释使用场景 → 这个改进能解决什么问题？
...
- 使用有描述性的提交信息
  ...
  不好：update
- 关联任何相关的 issue（Fixes #123、Closes #89 等）
...
- 类名使用 UpperCamelCase / PascalCase 命名
