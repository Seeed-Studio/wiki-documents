---
description: 通过 Web USB 或拖放方式在 Wio Tracker L1 Pro 上烧录 MeshCore 固件的指南，并介绍 LoRa 区域、GPS 与消息发送等应用配置。
...
在 `Community Firmware` 组中选择 `Seeed Studio Wio Tracker L1 Pro`。
...
如果你已点击 `Erase Flash` 但没有响应，请再次点击 `Enter DFU`，然后再点击 `Erase Flash`，以确保已成功进入 DFU。
...
如果你已点击 `Flash` 但没有响应，请再次点击 `Enter DFU`，然后再点击 `Flash`，以确保已成功进入 DFU。
...
双击 RST 按钮手动进入 DFU 模式。10~15 秒后你会看到一个名为 `TRACKER L1` 的磁盘弹出。
...
固件成功烧录后，该磁盘会消失。此时设备中还没有固件，所以设备`不会`自动重启。
...
固件成功烧录后，该磁盘会消失。此时设备中还没有固件，所以设备不会自动重启。
...
:::tip
如果按下按钮后设备没有反应，请先为其充电。不要使用快充充电器。
:::
...
输入设备屏幕上显示的 pin 码，然后点击 `Pair` 来连接设备。

:::tip
如果连接失败，请打开手机的蓝牙列表，并对该设备执行 `forget` 或 `Unpair` 操作。然后再尝试重新连接。
...
4. 出现提示时，输入设备屏幕上显示的蓝牙密码，然后点击 **OK**。此密码不是 `123456`。
...
5. 等待状态变为已连接，并且你的节点出现在 MeshCore 界面中。
...
为了开始通过 Mesh 通信，你必须先设置所在区域。此设置会控制设备使用的频率范围，并应根据你所在的地区进行设置。
...
现在你已经在设备上设置好了 LoRa 区域，可以继续配置任意适合你需求的 [LoRa 参数](https://github.com/meshcore-dev/MeshCore/blob/main/docs/faq.md#5-general)。

### 配置 GPS

如果你想使用 GPS，请将其设置为启用。

你可以在 APP 的 `Position Settings` 中启用 GPS。也可以进入设备的 GPS 页面，按下四向摇杆来打开或关闭 GPS。
...
如果你有两台受支持的设备，而你附近的 MeshCore 用户不多，可以将两台设备都烧录为 BLE Companion 固件，这样你就可以使用这些设备与附近的朋友和家人进行通信。
...
MeshCore 允许你手动广播你的名称、位置和公共加密密钥，这些信息也会被签名以防止伪造。当你点击广播按钮时，它会通过 LoRa 发送这些数据。MeshCore 将此称为一次广播（Advert）。有两种广播方式，“零跳”（zero hop）和“泛洪”（flood）。

- **Zero hop** 意味着你的广播会发送给所有能听到它的设备，仅此而已。
...
在你配置好 LoRa 之后，你可以在主频道中发送消息。同一频段附近的所有人都会听到。

### 私人消息

只有当两个设备都接收到对方的广播时，它们之间才会建立连接。
...
:::danger note
当你在烧录 bootloader 时，请确保线缆连接稳定，并且在烧录过程中 **不要** 拔掉线缆。
:::
...
- 步骤 4：烧录固件
  ...
  当你完成以上步骤后，就可以按照此[步骤](https://wiki.seeedstudio.com/cn/get_started_with_other_mesh_firmware/#flash-firmware-1)烧录应用固件。你可能需要[手动进入 DFU 模式](https://wiki.seeedstudio.com/cn/get_started_with_meshtastic_wio_tracker_l1/#unable-to-enter-dfu--entering-dfu-mode-manually)

### 无法进入 DFU & 手动进入 DFU 模式

将设备连接到你的电脑，双击按下 `Reset` 按钮。黄色 LED 将保持常亮，你的电脑上会出现一个名为 `Tracker L1` 的新 USB 驱动器。
...
- 串口日志运行一段时间后停止。
 ...
 这可能是因为在设备处于以下状态时，被人为强制重启或关机所导致：尚未完成消息发送过程、正在被配置中……
...
如果你想恢复到默认设置，可以执行恢复出厂设置。你有两种方式来进行恢复出厂设置。

- [Flash Erase](https://wiki.seeedstudio.com/cn/get_started_with_other_mesh_firmware/#flash-erase-1) 固件

- 点击应用中的 `Factory Reset` 按钮。设备会自动重启并载入出厂配置。
...
- **SNR** 反映通信链路的质量。正常设备通常在 -7 dB 以上运行。SNR 低于 -10 dB 的设备表明性能较差。

- **RSSI** 由设备本身及其周围环境共同决定。正常设备通常在 -110 dBm 以上运行。RSSI 低于 -115 dBm 的设备被认为性能较差。
