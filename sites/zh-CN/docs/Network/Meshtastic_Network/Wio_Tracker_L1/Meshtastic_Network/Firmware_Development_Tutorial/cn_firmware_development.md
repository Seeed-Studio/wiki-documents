---
description: 一篇实用教程，介绍如何搭建 Meshtastic 固件源代码环境、编译 Wio Tracker L1 目标、做一个简单的 UI 修改并烧录固件。
...
本教程介绍在 Windows 和 macOS 上进行 Meshtastic 固件基础开发的流程：克隆仓库、构建 `seeed_wio_tracker_L1`、做一个小的 UI 修改，并将结果烧录到设备。

如果你已经安装好了 Git、Python 和 PlatformIO，可以直接跳到动手实践部分。

:::tip
文中提供了 Windows 和 macOS 的命令示例。大部分截图基于 Windows，但在 macOS 上的流程是相同的。
:::
...
在 macOS 上，安装 Git 的方式有多种，但使用 Homebrew 通常是最简单的选择：
...
在 VS Code 的扩展市场中搜索 `PlatformIO` 并进行安装。
...
这里我们用一个简单示例：在主屏幕的标题栏中添加一个自定义标签。
...
- 记录电池文本的右边界。
- 在电池区域和右侧图标之间预留空间。
- 仅在 `SEEED_WIO_TRACKER_L1` 上且标题为空时绘制 `made by AE`。
...
至此，Meshtastic 源码实战练习已经完成。你已经走完了完整的工作流：环境搭建、仓库克隆、板级配置探索、固件编译、显示逻辑修改以及最终烧录验证。
...
- 在 macOS 上，先检查是否已经存在 `python3` / `pip3`，仅在需要时再使用 Homebrew 安装 Python。
