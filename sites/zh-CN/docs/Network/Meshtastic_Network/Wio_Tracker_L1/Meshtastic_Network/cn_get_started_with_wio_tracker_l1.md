...
:::caution note
请`不要使用 NRF-OTA`来更新固件，否则可能导致设备完全损坏。
:::
...
目前 L1 E-Ink 的`最新 Alpha 固件`已经支持四向摇杆。如果你想使用摇杆，请[点击这里](https://wiki.seeedstudio.com/cn/get_started_with_meshtastic_wio_tracker_l1/#第-2-部分-固件烧录)升级固件。
...
打开设备电源，点击 `Enter DFU Mode`，此时会出现一个名为 `Tracker L1` 的串口，点击并连接它，然后应该会显示一个名为 `Tracker L1` 的驱动器。
...
设备没有响应，没有 LED 指示灯，无法与你的 App 配对。如果你在烧录自定义固件后发现设备完全无法启动，你也可以尝试重新安装 bootloader。
...
如果 ZIP 不在终端当前目录，请将其完整路径传给 `--package`，例如 `"C:\path\to\wio_tracker_l1_bootloader-0.10.0_s140_7.3.0.zip"`。
...
`--touch 1200` 会将设备重新启动到 DFU 模式，因此串口通常会发生变化，第一个命令可能会以类似下面的回溯信息结束：
...
这并不是失败 —— 设备已经在一个新的串口上进入 DFU 模式。例如，在测试的 Windows 电脑上，它从 `COM43` 变成了 `COM45`：
...
**保持 USB 线缆连接。** 找到新的串口（刷新 **设备管理器 → 端口 (COM & LPT)**，或重新运行 `ls /dev/cu.*` / `ls /dev/ttyACM*`），然后在新的串口上再次烧录，这一次不要使用 `--touch 1200`：
...
之后，你的电脑上会出现一个 UF2 驱动器（截图来自测试的 Windows 电脑）。它的卷标会随 bootloader 构建版本而变化 —— 测试设备上显示为 `XIAO-BOOT` —— 这并不是通过/失败的判断标准；不要根据驱动器名称或 `INFO_UF2.TXT` 来判断结果：
...
- 安装后无法识别 `adafruit-nrfutil`：Python 用户脚本目录没有加入 PATH（Windows 下为 `Scripts`，macOS/Linux 下为主目录下的 `bin`）—— 将其加入 PATH，或在安装时不要使用 `--user`。
- 端口忙 / 访问被拒绝：关闭串口监视器、网页烧录标签页、Arduino IDE 或其他占用该端口的程序。
- 在 `Touched serial port ...` 之后出现 `FileNotFoundError: could not open port ...`：设备已经在新的串口上进入 DFU 模式 —— 请参考步骤 5。
...
如果 `--touch 1200` 无法让设备进入 DFU 模式（例如固件完全无响应），可以手动进入：连接设备，双击按下 `Reset`（黄色 LED 常亮），按照步骤 3 找到 DFU 串口，然后在该串口上运行步骤 5 中的命令，但不要带 `--touch 1200`。成功的表现与步骤 6 相同。

### 无法进入 DFU & 手动进入 DFU 模式

将设备连接到电脑并双击 `Reset` 按钮 —— 黄色 LED 会保持常亮，你的电脑上会出现一个 DFU 串口。

要在此模式下烧录 bootloader，请参考 [Device Bricked & Bootloader Recovery](https://wiki.seeedstudio.com/cn/get_started_with_meshtastic_wio_tracker_l1/#device-bricked--bootloader-installation)，但跳过 `--touch 1200` —— 设备已经处于 DFU 模式。
...
如果设备无法与附近节点通信或发送消息，首先检查 LoRa 区域和调制解调器预设是否与周围节点匹配。你还应检查默认 **PSK** 是否被更改。主信道上使用不同的 PSK 会导致设备无法与该信道上的其他节点通信。
...
如果你不确定哪些设置被更改过，可以按照 [Factory Reset](https://wiki.seeedstudio.com/cn/get_started_with_meshtastic_wio_tracker_l1/#factory-reset) 指南将设备恢复到默认设置。如果只是 PSK 被更改，将其重新设置为 `AQ==`。
...
如果你想恢复到默认设置，可以执行出厂重置。你可以通过两种方式进行出厂重置。
