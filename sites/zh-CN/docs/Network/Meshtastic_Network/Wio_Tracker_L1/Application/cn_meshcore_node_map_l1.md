...
节点地图可以通过绘制节点位置，为 [MeshCore](https://meshcore.io/#hardware) 网络提供实时可视化，使用户能够了解网络拓扑结构和节点分布。 
...
要让一个 companion 节点出现在 MeshCore 节点地图上，设备必须正确配置以上报其位置。按如下步骤进行配置：
...
启用 GPS 并获取位置（部分中继器固件不支持更新实时位置，你可能需要手动选择位置）。
...
你可以看到设备名称、角色和配置信息。因此，通过使用此地图，你可以轻松找出那些在线节点所在的位置。

## （进阶）构建你自己的节点地图前端

对于高级用户，可以构建自定义的节点地图前端。建议使用以下工作流程： 
