---
description: 基于 XIAO ESP32S3 和 Wio-SX1262 套件的 Meshtastic MQTT 网关设置教程。支持全球消息通信，在网状网络信号较弱时提供备份，并通过互联网实现更快速的消息传递。 
title: Meshtastic 网络监视器
keywords:
  - Meshtastic
  - Source Code
  - PlatformIO
...
[Meshtastic](https://meshtastic.org/docs/introduction/) 网络监视器是一款用于可视化、管理和监控 Meshtastic 网状网络中节点的工具。它提供有关设备状态、路由和通信的实时信息。该监视器作为 Meshtastic 分析器，通过连接的网关或 MQTT 服务器收集数据，可视化网络拓扑，并显示节点信息，例如电池状态、信号强度和 GPS 位置。它也可以作为 Meshtastic 节点地图使用。
...
* Meshtastic 分析器。用于监控网状网络中的通信性能和路由情况。
* Meshtatsic 监视器。用于调试和测试 Meshtastic 设备的网络连接性。

**常见 Meshtastic 监视器**

* Meshsense：[点击此处](https://affirmatech.com/meshsense)了解更多
* Meshmonitor：[点击此处](https://meshmonitor.org/)了解更多

**如何连接到 Meshtastic 监视器**

1. 设置你的 Meshtastic 节点，并确保它们组成一个可正常工作的网状网络。
...
为你的平台下载并安装 **Meshsense** 应用：[Meshsense 应用下载](https://affirmatech.com/meshsense)
...
你需要一个配置为 MQTT 网关的 **Seeed Xiao ESP32S3 & SX1262 Meshtastic 套件**。这可以实现 Meshtastic 网络与 Meshsense 之间的实时通信。如果你还想监控和显示位置，请添加一个 GPS 模块。
...
按照官方配置教程来设置 mqtt：[Xiao ESP32S3 & SX1262 MQTT 网关设置](https://wiki.seeedstudio.com/cn/xiao_esp32s3_&_wio_sx1262_kit_mqtt/)

:::tip
若要显示实时 GPS 位置，请确保在节点上启用了 GPS。在配置 MQTT 时，请确认已允许位置上报。
...
打开串口终端（例如 Arduino 串口监视器或 PuTTY）以查找 Xiao 设备的 IP 地址。你也可以使用 [Meshtastic web serial monitor](https://flasher.meshtastic.org/)。
...
若要显示节点位置，请将 **L76K GPS 模块** 连接到 Xiao ESP32S3 套件。[点击此处](https://www.seeedstudio.com/L76K-GNSS-Module-for-Seeed-Studio-XIAO-p-5864.html)立即获取。

#### 软件设置

* 在 Meshsense 应用中，确保已启用 GPS 上报。
* 连接成功后，应用会在地图上显示每个节点的实时位置。你可以在 APP 中直接查看节点位置。
