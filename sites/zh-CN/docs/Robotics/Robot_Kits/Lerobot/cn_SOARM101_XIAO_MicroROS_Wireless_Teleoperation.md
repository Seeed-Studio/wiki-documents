...
当前启动器同时支持双无线路径和有线主臂回退。2026 年 9 月 20 日完成了一次无线耐久验证，实际运行 783 秒（约 13 分钟）。该次运行记录到命令超时 0 次、命令拒绝 0 次、主臂恢复 0 次；反馈平均频率为 19.87 Hz，指令确认延迟 P50 为 66.0 ms，观测到的最低 RSSI 为 -65 dBm。这些数据仅代表一次参考运行，不保证适用于所有 Wi-Fi 环境。
