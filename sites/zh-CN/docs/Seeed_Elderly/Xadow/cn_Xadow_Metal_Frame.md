---
description: Xadow - 金属框架
title: Xadow - 金属框架
keywords:
  - Seeed_Elderly
image: https://files.seeedstudio.com/wiki/wiki-platform/S-tempor.png
slug: /Xadow_Metal_Frame
last_update:
  date: 1/13/2023
  author: shuxu hu
createdAt: '2025-08-07'
updatedAt: '2026-03-03'
url: https://wiki.seeedstudio.com/cn/Xadow_Metal_Frame/
---
![](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/img/XFrame_01.jpg)

Xadow 金属框架是一款制作精良、可靠的外骨骼，可以保护并为您的 Xadow 设备提供稳定性。它可穿戴，并能够承受跑步、步行、徒步旅行或静止不动。通过添加各种 Xadow 传感器，定制您的使用体验。

![](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/img/XFrame.jpg)

[![](https://files.seeedstudio.com/wiki/Seeed-WiKi/docs/images/300px-Get_One_Now_Banner-ragular.png)](https://www.seeedstudio.com/Xadow-Metal-Frame-p-1554.html)

## 特性
---
*   使用硬质航空铝合金，金属质感丰富
*   采用精湛的 CNC 工艺技术制造
*   方便灵活的佩戴形式，采用朋克风格设计
*   内部 PCB 的灵活搭配，可实现特定功能

## 装配清单
---
如果您已经拥有我们的 Xadow 模块，那么您可以使用金属框架组装一个可穿戴设备。下图是 BOM（物料清单）。

![](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/img/Xadow_Enclosure1.jpg)

关于组装指南，请点击 [这里](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/res/Xdow_Metal_Frame_Guide.pdf) 查看具体步骤。

通过自己组装，您将得到以下效果：

|![](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/img/IMG_7085.JPG)|![](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/img/XadowDiaplay.JPG)|
|---|---|

它可以作为配件佩戴，如下图所示：

![](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/img/Xadow_metel_frame.jpg)

嘿！朋友们，展示您为 Xadow 开发的任何创新应用吧。

## 资源
---
[Xdow 金属框架组装指南](https://files.seeedstudio.com/wiki/Xadow_Metal_Frame/res/Xdow_Metal_Frame_Guide.pdf)

## 技术支持与产品讨论

感谢您选择我们的产品！我们致力于为您提供不同的支持，以确保您使用我们的产品时体验顺畅。我们提供多种沟通渠道，以满足不同的偏好和需求。

<div class="button_tech_support_container">
<a href="https://forum.seeedstudio.com/" class="button_forum"></a> 
<a href="https://www.seeedstudio.com/contacts" class="button_email"></a>
</div>

<div class="button_tech_support_container">
<a href="https://discord.gg/eWkprNDMU7" class="button_discord"></a> 
<a href="https://github.com/Seeed-Studio/wiki-documents/discussions/69" class="button_discussion"></a>
</div>