...
Arduino Uno 是目前最受欢迎的 Arduino 开发板，但是当你的项目需要很多传感器或 LED 时，杂乱的杜邦线会让人很头疼。Base Shield 的设计初衷就是帮助你摆脱面包板和杜邦线。借助底板上丰富的 Grove 接口，你可以非常方便地将所有 Grove 模块连接到 Arduino Uno！Base Shield V2 的引脚排列与 Arduino Uno R3 完全相同。
...
<table align="center">
  ...
  <tr>
    ...
    <td><h4>初始版本</h4></td>
    <td><h4>2011 年 10 月</h4></td>
  </tr>
  <tr>
    <td><h4>Base Shield V1.3</h4></td>
    <td><h4>更改 Grove 接口的布局和数量</h4></td>
    <td><h4>2012 年 8 月</h4></td>
  </tr>  
  <tr>
    <td><h4>Base Shield V2.0</h4></td>
    <td><h4>更改 Grove 接口的布局和数量，并增加电源开关以同时支持 3.3V 和 5V。</h4></td>
    <td><h4>2014 年 3 月</h4></td>
  </tr>
  </tbody></table>
...
:::caution
Grove Base Shield 的默认 I2C 引脚是 D70/D71，而 Arduino DUE 的默认 I2C `Wire` 定义为 D20/D21，因此当两者一起使用时，如果你使用 `Wire` 定义，将无法正常使用扩展板上的 I2C 引脚，你需要使用 `Wire1`。
:::
...
- **1-模拟端口**：包含 4 个模拟端口，A0、A1、A2 和 A3。
- **2-数字端口**：包含 7 个数字端口，D2、D3、D4、D5、D6、D7 和 D8。
- **3-UART 端口**：1 个 UART 端口。
- **4-I2C 端口**：4 个 I2C 端口。
- **5-电源开关**：当 Arduino UNO 与 Base Shield v2 搭配使用时，请将开关拨到 5V 档；当 Seeeduino Arch 与 Base Shield v2 搭配使用时，请将开关拨到 3.3V 档。
- **6-复位按钮**：复位 Arduino 开发板。
- **7-PWR LED**：上电时绿色 LED 亮起。
- **8-P1、P2**：如果将 Base Shield v2 与 Seeeduino V3 搭配使用，请焊接焊盘 P1 和 P2。
- **尺寸**：2.1 × 2.7 英寸  
...
<table align="center">
  ...
  <tr>
    <td><h4><a href="https://www.seeedstudio.com/Seeeduino-V4.2-p-2517.html" target="_blank"><span>立即获取</span></a></h4></td>
    <td><h4><a href="https://www.seeedstudio.com/Base-Shield-V2-p-1378.html" target="_blank"><span>立即获取</span></a></h4></td>
    <td><h4><a href="https://www.seeedstudio.com/Grove-Buzzer-p-768.html" target="_blank"><span>立即获取</span></a></h4></td>
    <td><h4><a href="https://www.seeedstudio.com/category/Grove-Button-p-766.html" target="_blank"><span>立即获取</span></a></h4></td>
  </tr>  
  </tbody></table>

- 步骤 2. 将 Grove - Buzzer 连接到 Base Shield 的 D3 端口。
- 步骤 3. 将 Grove - Button 连接到 Base Shield 的 D2 端口。
- 步骤 4. 将 Base Shield 插到 Seeeduino 上。
- 步骤 5. 通过 USB 线将 Seeeduino 连接到电脑。
...
- 步骤 1. 将下面的代码复制到 Arduino IDE 中并上传到 Seeeduino。
