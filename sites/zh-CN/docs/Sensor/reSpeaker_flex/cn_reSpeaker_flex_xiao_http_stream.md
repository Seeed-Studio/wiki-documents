...
title: 使用 Xiao ESP32S3 对 reSpeaker Flex 进行 HTTP 流式传输
keywords:
  - reSpeaker flex
  - xvf3800
  - ESP32S3
...
url: https://wiki.seeedstudio.com/cn/respeaker_flex_xiao_http_stream/
---

## 介绍 

本指南介绍如何使用 ESP32-S3（XIAO ESP32S3）从 reSpeaker Flex XVF3800 麦克风阵列采集音频，录制 5 秒 16 kHz 立体声 32 位 PCM 音频，并将数据存储在 PSRAM 中以处理大容量音频缓冲区。随后通过 Wi-Fi 将录制的音频上传到基于 Python 的 Flask HTTP 服务器，在服务器端接收并保存为标准 WAV 文件。该方案非常适合语音采集、远程音频日志记录，以及包括语音转文本（STT）和基于机器学习的音频分析在内的语音处理流水线等应用。
...
**你需要在你的环境中安装并运行 flask** `pip install flask` 
