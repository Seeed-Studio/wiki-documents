...
reSpeaker Flex 是一款基于 XMOS XVF3800 处理器构建的模块化分体架构语音处理系统，专为机器人和智能交互终端设计。与传统的一体化麦克风阵列不同，Flex 将核心处理板与麦克风阵列板分离，两者通过柔性 FPC 软排线连接，使阵列可以嵌入产品外壳内部的任意物理位置，而将处理电子部分放置在其他位置。
该系统支持两种可互换的麦克风阵列配置：用于全向 360° 采集的**环形 4 麦阵列**，以及用于前向拾音并抑制后向声音的**线性 4 麦阵列**。两块阵列板均通过标准化的 24 针 FPC 接口连接到同一核心板，系统可在 USB（UAC 2.0 即插即用）或 I2S 模式下运行，以便嵌入式集成。可选的 XIAO ESP32S3 模块可焊接到核心板上，以实现无线连接和扩展控制功能。
...
FPC 麦克风阵列接口为 24 针、0.5mm 间距带锁扣连接器，包装盒中附带的 20cm FPC 软排线与该连接器配套。
...
通过 USB 线缆将 reSpeaker Flex 连接到你的电脑。请注意，你需要使用 XMOS 的 USB-C 接口（靠近 RST 按钮）来烧录 XMOS 的固件。
...
打开开始菜单并输入 Device manager。找到相关的 reSpeaker Flex 设备，右键点击并选择 Uninstall devices。之后，重启设备（USB 拔插），Windows 会为其重新安装正确的声卡驱动。
