---
description: reSpeaker Flex 是一款强大的模块化语音系统，基于 XMOS XVF3800，配备可互换的环形与线性 4 麦克风阵列，可实现精确的 360° 或定向音频采集——非常适合机器人和智能设备。
...
reSpeaker Flex 是一套基于 XMOS XVF3800 处理器构建的模块化、分体式语音处理系统，专为机器人和智能交互终端设计。与传统的一体化麦克风阵列不同，Flex 将核心处理板与麦克风阵列板分离，两者通过柔性 FPC 软排线连接，使得麦克风阵列可以嵌入产品外壳内的任意物理位置，同时将处理电子部分安置在其他位置。
系统支持两种可互换的麦克风阵列配置：用于全向 360° 拾音的**环形 4 麦阵列**，以及具有后向抑制的、面向前方定向拾音的**线性 4 麦阵列**。两块阵列板均通过标准化的 24 针 FPC 接口连接到同一核心板，系统可在 USB（UAC 2.0 即插即用）或 I2S 模式下运行以适配嵌入式集成。核心板上可选焊接 XIAO ESP32S3 模组，以实现无线连接和扩展控制。
...
- **分体模块化设计**：核心板与麦克风阵列板物理分离，通过 FPC 软排线连接，可在任意产品机壳中灵活放置

- **麦克风配置可选**：在使用同一核心板的前提下，可在环形 4 麦阵列（360° 拾音，44mm 间距）与线性 4 麦阵列（180° 前向指向性，33mm 间距）之间切换

- **XMOS XVF3800 音频处理**：完整集成 AEC、AGC、DoA、多通道波束成形、VAD、噪声抑制和混响消除

- **双工作模式**：通过 USB UAC 2.0 与 PC 和 SBC 即插即用；通过 I2S 模式实现直接嵌入式集成

- **双 USB 连接方式**：USB-C 接口和 PH2.0 锁扣接口均支持 UAC 2.0 音频与 DFU 固件升级

- **板载扬声器功放**：通过 JST 连接器驱动 10W 4 欧姆扬声器，并提供 3.5mm AUX 耳机输出

- **支持 XIAO ESP32S3（可选）**：焊接模组后可增加 Wi-Fi/Bluetooth，I2S 和 I2C 总线直接连接 XVF3800，实现无线控制与音频转发

- **固件灵活性**：双 USB 固件版本（2 通道与 6 通道）、I2S 固件、通过 dfu-util 进行 DFU 升级，并可在断电后保持配置

- **Python SDK**：可通过 Python 调整录音格式、音量、算法参数以及通道路由
...
| **PH2.0 接口** | 锁扣式连接器；提供与 USB-C 接口相同的 UAC 2.0 音频和 DFU 支持 |
| **3.5 mm AUX 插孔** | 侧出式耳机输出，由板载 DAC 驱动 |
| **扬声器 JST 连接器** | 立式安装连接器，可驱动最高 10W、4Ω 扬声器；保留焊盘以支持扬声器硬连线 |
| **外部电源端子** | 为整个系统提供 12V 供电，并支持 10W 扬声器负载（P1 优先） |
| **RST 按键** | XVF3800 的硬件复位按键 |
| **SafeMode（Boot）按键** | 上电时按住可进入安全模式以进行固件恢复 |
| **PWR 指示灯** | 绿色电源指示灯 |
| **调试焊盘** | 预留 XMOS XTAG4 焊盘（出厂未焊接） |
| **XMOS 3800 IO 排针** | 引出 I2C、I2S、5V/GND、扬声器引脚以及剩余 XVF3800 IO 的排针/焊盘 |
| **安装孔** | 4 × M3 安装孔 |
| **XIAO ESP32S3** | 支持在作为主控设备时焊接 XIAO ESP32S3 |
...
reSpeaker Flex 核心板预留了焊接 Seeed Studio XIAO ESP32S3 模组的专用焊盘，在 XVF3800 音频处理链路的基础上增加 Wi-Fi 和 Bluetooth 连接能力。包含 XIAO 的 SKU（-C4-1 和 -L4-1）在出厂时已预装该模组；基础 SKU（-C4-0 和 -L4-0）则保留焊盘不焊接。
当焊接 XIAO 后，XIAO 与 XVF3800 之间会建立如下连接：

- **I2S 总线** — 双总线（播放与录音），共享 BCLK、MCLK 和 LRCLK，用于双向音频流传输
- **I2C 总线** — 供 XIAO 读写 XVF3800 的配置参数
- **RST 线** — XIAO 可通过专用 IO 引脚对 XVF3800 进行硬件复位
- **剩余 XIAO IO** — 引出至带标签的排针/焊盘供用户扩展

所有配备 XIAO 的版本均附带一根 FPC Wi-Fi 天线（适用于不带板载天线的 XIAO ESP32S3）。
...
FPC 麦克风阵列接口为 24 针、0.5mm 间距的带锁扣连接器，盒内附带的 20cm FPC 软排线已按此连接器做了钥槽定位。
...
| 2 | MIC_VDD | Power | 麦克风电源 | 双引脚以增强电源稳定性 |
| 3 | GND | GND | 电源回流地 | 电源地 |
...
当按下复位（RST）按钮时，为 reSpeaker Flex 提供硬件复位，它会重新启动芯片，并从最初开始重新初始化系统，就像一次完整的断电重启。
...
在这里，你可以看到如何根据自己的音频输出偏好，选择使用 3.5mm AUX 耳机插孔或板载 JST 扬声器接口来连接扬声器。

#### 安全模式 

**Safe Mode** 是 **reSpeaker Flex** 上的一种特殊恢复模式，它允许你通过 **USB DFU** 或 I2C（如 Raspberry Pi 和 **ESP32** 等设备）烧录固件。

如果你之前已经烧录了 **I2S 固件**，并希望切换回 **USB 固件**，可以进入 **Safe Mode**，并使用 **USB DFU** 重新烧录 USB 固件。
...
现在设备正在运行存储在 Factory 分区中的 **Safe Mode 固件。**


### 更新固件 

在官方 GitHub 仓库中提供了三个固件版本。你可以根据应用需求选择并烧录合适的固件。更多详情和下载信息请参考 Github [链接](https://github.com/respeaker/reSpeaker_Flex) 
...
USB 固件用于在通过 USB 硬件接口通信时，搭配 **Windows、Linux 和 macOS** 等宿主操作系统使用。
提供了两个固件变体：**respeaker_xvf3800_usb_dfu_firmware_v2.0.x.bin**，提供 2 通道音频，以及 **respeaker_xvf3800_usb_dfu_firmware_6chl_v2.0.x.bin**，提供 6 通道音频。两个固件版本都以 16 kHz 采样率、32 位深度运行。
...
通过 USB 线将 reSpeaker Flex 连接到你的电脑。请注意，需要使用 XMOS 的 USB-C 接口（靠近 RST 按钮）来烧录 XMOS 固件。

</TabItem>

 <TabItem value="I2S" label="I2S">

I2S 固件适用于当设备连接到如 XIAO ESP32S3 等微控制器主机时使用。在这种配置下，语音数据通过 I2S 协议传输。固件文件 **respeaker_xvf3800_i2s_dfu_firmware_v1.0.x.bin** 可在此处获取。该固件支持 2 通道音频，采样率为 16 kHz，位深度为 32 位。
...
- 将 `dfu-util.exe` 的路径添加到系统的 `Path` 变量：  
...
1. 打开 **Audacity**
2. 前往 **Audio Setup > Audio Settings**
...
3. 设置：
   ...
   - **Channels**：`2 (Stereo)`
   - **Sample Rate**：`16000 Hz`（对 **Project** 和 **Default Sample Rate** 都设为此值）
...
在 alsamixer 中，使用左右方向键切换到正确的声音设备。使用向上方向键增加音量。
...
- 在 Audio Settings 窗口中：
  ...
  - 选择合适的播放设备（例如 reSpeaker XVF3800 Flex）。
  - 确保 Host 设置为 ALSA，以获得在 Raspberry Pi 上的最佳兼容性。
- 点击 OK 以应用设置。
