---
description: reSpeaker Flex 是一款强大的模块化语音系统，搭载 XMOS XVF3800，配备可互换的环形与线性 4 麦克风阵列，可实现精确的 360° 或定向音频采集——非常适合用于机器人和智能设备。
...
reSpeaker Flex 是一个基于 XMOS XVF3800 处理器构建的模块化分体式语音处理系统，专为机器人和智能交互终端设计。与传统的一体化麦克风阵列不同，Flex 将核心处理板与麦克风阵列板分离，两者通过柔性 FPC 软排线连接，这样就可以将麦克风阵列嵌入到产品外壳内部的任意物理位置，同时将处理电子部分放置在其他位置。
该系统支持两种可互换的麦克风阵列配置：用于全向 360° 收音的**环形 4 麦阵列**，以及用于前向定向拾音并抑制后方声音的**线性 4 麦阵列**。两块板都通过标准化的 24 针 FPC 接口连接到同一核心板，系统可以在 USB（UAC 2.0 即插即用）或 I2S 模式下运行，以便进行嵌入式集成。还可以在核心板上焊接可选的 XIAO ESP32S3 模块，用于实现无线连接和扩展控制能力。
...
- **分体模块化设计**：核心板和麦克风阵列板物理分离，通过 FPC 软排线连接，可在任意产品机壳内灵活放置

- **多种麦克风配置选择**：可在同一核心板上互换环形 4 麦阵列（360° 拾音，44mm 间距）或线性 4 麦阵列（180° 前向指向性，33mm 间距）

- **XMOS XVF3800 音频处理**：集成完整的 AEC、AGC、DoA、多通道波束成形、VAD、噪声抑制和混响消除算法

- **双工作模式**：USB UAC 2.0，可与 PC 和 SBC 即插即用；I2S 模式，用于直接嵌入式集成

- **双 USB 接口**：USB-C 端口和 PH2.0 锁扣端口均支持 UAC 2.0 音频和 DFU 固件升级

- **板载扬声器功放**：通过 JST 连接器可驱动 10W、4 欧姆扬声器，并提供 3.5mm AUX 耳机输出

- **XIAO ESP32S3 支持（可选）**：焊接模块可提供 Wi-Fi/Bluetooth 功能，且 I2S 与 I2C 总线直接连到 XVF3800，实现无线控制和音频转发

- **固件灵活性**：双 USB 固件版本（2 通道和 6 通道）、I2S 固件，支持通过 dfu-util 进行 DFU 升级，并可在断电后保持配置

- **Python SDK**：通过 Python 调整录音格式、音量、算法参数以及通道路由
...
| **FPC 麦克风阵列接口** | 24 针带锁扣的 FPC 连接器，支持最多 8 麦阵列和 2 路 GPIO 线 |
| **USB-C 端口** | UAC 2.0 音频、DFU 固件升级以及供电 |
| **PH2.0 端口** | 带锁扣连接器；提供与 USB-C 端口相同的 UAC 2.0 音频和 DFU 支持 |
| **3.5 mm AUX 插孔** | 侧向出线耳机输出，由板载 DAC 驱动 |
| **扬声器 JST 连接器** | 直立安装连接器，可驱动最高 10W、4Ω 扬声器；预留焊盘以支持扬声器硬接线 |
| **外部电源端子** | 为整个系统提供 12V 供电并支持 10W 扬声器负载（P1 优先） |
| **RST 按钮** | 用于 XVF3800 的硬件复位 |
...
| 组件 / 特性 | 描述 |
|--------------------|------------|
| **麦克风** | 4× PDM MEMS 麦克风，44 mm 间距，以环形布局排列 |
| **拾音模式** | 360° 全向音频采集 |
| **接口** | 通过 FPC 连接器连接到核心板 |
| **安装** | 3 × M3 安装孔，用于固定到外壳或机壳上 |
...
| 组件 / 特性 | 描述 |
|--------------------|------------|
| **麦克风** | 4× PDM MEMS 麦克风，33 mm 间距，以线性布局排列 |
...
reSpeaker Flex 核心板预留了专用焊盘，可焊接 Seeed Studio XIAO ESP32S3 模块，在 XVF3800 的音频处理流水线之上增加 Wi-Fi 和 Bluetooth 连接能力。包含 XIAO 的 SKU（-C4-1 和 -L4-1）在出厂时已预装该模块；基础 SKU（-C4-0 和 -L4-0）则保留焊盘未焊接。
当 XIAO 安装到位后，它与 XVF3800 之间会建立如下连接：

- **I2S 总线** — 双总线（播放和录制），共享 BCLK、MCLK 和 LRCLK，用于双向音频流传输
- **I2C 总线** — 用于在 XIAO 上读取和写入 XVF3800 的配置参数
...
FPC 麦克风阵列接口为 24 针、0.5mm 间距带锁扣连接器，盒内附带的 20cm FPC 软排线与该连接器配套。
...
| 引脚 | 信号名称 | 类型 | 功能描述 | 备注 |
|-----|------------|------|----------------------|-------|
| 1 | MIC_VDD | Power | 麦克风电源 | 3.3V 电源 |
| 2 | MIC_VDD | Power | 麦克风电源 | 双引脚以增强供电稳定性 |
| 3 | GND | GND | 电源回流地 | 电源 GND |
...
当按下复位（RST）按钮时，将对 reSpeaker Flex 进行硬件复位，它会从最开始重新启动芯片并重新初始化系统，就像一次完整的断电重启一样。
...
在这里你可以看到如何根据自己的音频输出偏好，使用 3.5mm AUX 耳机插孔或板载 JST 喇叭接口来连接扬声器。

#### 安全模式 

**Safe Mode** 是 **reSpeaker Flex** 上的一种特殊恢复模式，它允许你通过 **USB DFU** 或 I2C（例如与 Raspberry Pi 和 **ESP32** 等设备配合）来烧录固件。
...
- 你的固件**运行不正常**（例如，**USB 无法被检测到**）。
- 你需要**重新烧录新的固件**，但当前固件**没有响应**。
- 你**不小心烧错了固件**，希望恢复设备。
...
- **红色 LED 将开始闪烁**，表示设备已进入 **Safe Mode。**
此时设备正在运行存储在 Factory 分区中的 **Safe Mode 固件。**


### 更新固件 

官方 GitHub 仓库中提供了三个固件版本。你可以根据应用需求选择并烧录合适的固件。更多详情和下载，请参考 Github [链接](https://github.com/respeaker/reSpeaker_Flex) 
...
通过 USB 线将 reSpeaker Flex 连接到你的电脑。注意，你需要使用 XMOS USB-C 接口（靠近 RST 按钮）来烧录 XMOS 固件。

</TabItem>

 <TabItem value="I2S" label="I2S">

I2S 固件适用于当设备连接到 XIAO ESP32S3 等微控制器主机时使用。在这种配置下，语音数据通过 I2S 协议进行传输。固件文件 **respeaker_xvf3800_i2s_dfu_firmware_v1.0.x.bin** 可在此处获取。该固件支持 16 kHz 采样率、32 位深度的 2 通道音频。
...
从 GitHub 在此处下载完整的固件仓库 reSpeaker Flex XVF 3800

- 运行以下命令来烧录固件

    - dfu-util -R -e -a 1 -D /path/to/dfu_firmware.bin

- 在 Linux 上，需要使用 sudo 运行
...
3. 设置：
   ...
   - **Channels**: `2 (Stereo)`
   - **Sample Rate**: `16000 Hz`（对于 **Project** 和 **Default Sample Rate** 均为此值）
...
- **Recording Device（录音设备）**: `reSpeaker 3800 Flex`
- **Channels（声道）**: `2 (Stereo)`
- **Sample Rate（采样率）**: `16000 Hz`（用于 **Project** 和 **Default Sample Rate**）
- **Sample Format（采样格式）**: `16-bit`
...
3. **在 ALSA 上调节 reSpeaker XVF3800 的音量**

```bash
alsamixer
```

在 alsamixer 中，使用左右方向键切换到正确的音频设备，使用向上方向键增大音量。
...
- 在 Audio Settings 窗口中：
  - 选择正确的 Recording Device（录音设备）（例如：reSpeaker XVF3800 Flex）。
  - 选择合适的 Playback Device（播放设备）（例如：reSpeaker XVF3800 Flex）。
...
感谢您选择我们的产品！我们为您提供多种支持方式，以确保您在使用我们产品时尽可能顺畅。我们提供多种沟通渠道，以满足不同的偏好和需求。
