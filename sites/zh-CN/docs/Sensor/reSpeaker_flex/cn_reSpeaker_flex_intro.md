...
reSpeaker Flex 是一款围绕 XMOS XVF3800 处理器构建的模块化、分体式架构语音处理系统，专为机器人和智能交互终端设计。与传统的一体化麦克风阵列不同，Flex 将核心处理板与麦克风阵列板分离，两者通过柔性的 FPC 排线连接，使阵列可以嵌入产品外壳内的任意物理位置，而将处理电子部分放置在其他位置。
该系统支持两种可互换的麦克风阵列配置：用于全向 360° 采集的**环形 4 麦克风阵列**，以及具有后向抑制、面向前方拾音的**线性 4 麦克风阵列**。两块阵列板均通过标准化的 24 针 FPC 接口连接到同一核心板，系统可在 USB（UAC 2.0 即插即用）或 I2S 模式下运行，以便嵌入式集成。可选的 XIAO ESP32S3 模块可焊接在核心板上，实现无线连接和扩展控制功能。
...
- **分体模块化设计**：核心板和麦克风阵列板物理分离，通过 FPC 排线连接，可在任意产品机壳内灵活布置

- **多种麦克风配置选择**：在同一核心板上可切换环形 4 麦克风阵列（360° 拾音，44mm 间距）或线性 4 麦克风阵列（180° 前向指向性，33mm 间距）

- **XMOS XVF3800 音频处理**：集成完整的 AEC、AGC、DoA、多通道波束成形、VAD、噪声抑制和去混响功能

- **双工作模式**：通过 USB UAC 2.0 与 PC 和 SBC 即插即用；通过 I2S 模式实现直接嵌入式集成

- **双 USB 连接方式**：USB-C 端口和 PH2.0 带锁端口均支持 UAC 2.0 音频和 DFU 固件升级
...
- **灵活固件方案**：双 USB 固件版本（2 通道和 6 通道）、I2S 固件，支持通过 dfu-util 进行 DFU 升级，并可在断电后保持配置
...
| **音频 Codec** | TLV320AIC3104，负责音频转换和 DAC 输出 |
| **FPC 麦克风阵列接口** | 带锁扣的 24 针 FPC 连接器，支持最多 8 麦克风阵列和 2 路 GPIO 线 |
| **USB-C 端口** | 提供 UAC 2.0 音频、DFU 固件升级及供电 |
| **PH2.0 端口** | 带锁扣连接器；提供与 USB-C 端口相同的 UAC 2.0 音频和 DFU 支持 |
| **3.5 mm AUX 插孔** | 侧向出线耳机输出，由板载 DAC 驱动 |
| **扬声器 JST 接口** | 直立安装接口，可驱动最高 10W、4Ω 扬声器；预留焊盘以支持扬声器硬接线 |
| **外部电源端子** | 为整个系统提供 12V 供电并支持 10W 扬声器负载（P1 优先级） |
| **RST 按钮** | 用于 XVF3800 的硬件复位 |
| **SafeMode（启动）按钮** | 上电时按住可进入安全模式，用于固件恢复 |
| **PWR 指示灯** | 绿色上电指示灯 |
...
| **拾音模式** | 约 180° 的前向拾音模式，并对后向声源进行抑制 |
| **接口** | FPC 接口；排线可从侧面或背面走线 |
| **安装** | 2 × M3 安装孔 |
...
reSpeaker Flex 核心板预留了专用焊盘，可焊接 Seeed Studio XIAO ESP32S3 模块，在 XVF3800 音频处理链路之上增加 Wi-Fi 和 Bluetooth 连接能力。包含 XIAO 的 SKU（-C4-1 和 -L4-1）出厂时已预装该模块；基础 SKU（-C4-0 和 -L4-0）则保留焊盘未焊接。
当 XIAO 安装就位后，它与 XVF3800 之间会建立以下连接：

- **I2S 总线** — 双总线（播放和录音），共用 BCLK、MCLK 和 LRCLK，实现双向音频流
- **I2C 总线** — 供 XIAO 读取和写入 XVF3800 的配置参数
- **RST 线** — XIAO 可通过专用 IO 引脚对 XVF3800 进行硬件复位
- **其余 XIAO IO** — 引出到带标签的排针/焊盘，供用户扩展

对于不带板载天线的 XIAO ESP32S3，所有配备 XIAO 的版本都会随附一根 FPC Wi-Fi 天线作为附件。
...
FPC 麦克风阵列接口为 24 针、0.5mm 间距并带锁扣的连接器，盒内附带的 20cm FPC 排线与该连接器的键位匹配。
...
| 3 | GND | GND | 电源回流地 | 电源地 |
| 4 | MIC_CLK | Out | 全局时钟信号 | 核心时钟信号，2–4 MHz |
...
当按下复位（RST）按键时，会对 reSpeaker Flex 执行硬件复位，它会重新启动芯片，并从最开始重新初始化系统，就像执行了一次完整的断电重启。
...
在这里，你可以看到如何根据自己的音频输出偏好，使用 3.5mm AUX 耳机插孔或板载 JST 扬声器接口来连接扬声器。

#### 安全模式 

**Safe Mode** 是 **reSpeaker Flex** 上的一种特殊恢复模式，它允许你通过 **USB DFU** 或 I2C（适用于 Raspberry Pi 和 **ESP32** 等设备）来烧录固件。

如果你之前已经烧录了 **I2S 固件** 并希望切换回 **USB 固件**，可以进入 **Safe Mode**，然后通过 **USB DFU** 重新烧录 USB 固件。
...
在官方 GitHub 仓库中提供了三个固件版本。你可以根据自己的应用需求选择并烧录合适的固件。有关更多详情和下载，请参考 Github [链接](https://github.com/respeaker/reSpeaker_Flex) 
...
USB 固件专为在通过 USB 硬件接口进行通信时，与 **Windows、Linux 和 macOS** 等主机操作系统配合使用而设计。
有两种固件可用：**respeaker_xvf3800_usb_dfu_firmware_v2.0.x.bin** 提供 2 通道音频，**respeaker_xvf3800_usb_dfu_firmware_6chl_v2.0.x.bin** 提供 6 通道音频。两种固件版本均在 16 kHz 采样率下以 32 位深度运行。
...
通过 USB 线将 reSpeaker Flex 连接到你的电脑。注意，需要使用 XMOS 的 USB-C 端口（靠近 RST 按钮）来烧录 XMOS 固件。

</TabItem>

 <TabItem value="I2S" label="I2S">

I2S 固件适用于设备连接到如 XIAO ESP32S3 等微控制器主机时使用。在这种配置中，语音数据通过 I2S 协议进行传输。固件文件 **respeaker_xvf3800_i2s_dfu_firmware_v1.0.x.bin** 可在此处获取。该固件支持 2 通道音频，在 16 kHz 采样率下具有 32 位深度。
...
在 alsamixer 中，使用左右方向键切换到正确的声卡设备。使用上方向键增大音量。
...
感谢你选择我们的产品！我们将通过多种支持方式，确保你在使用我们的产品时拥有尽可能顺畅的体验。我们提供多个交流渠道，以满足不同的偏好和需求。
