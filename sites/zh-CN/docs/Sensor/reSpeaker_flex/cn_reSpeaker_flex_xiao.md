---
description: reSpeaker Flex 是一款强大的模块化语音系统，采用 XMOS XVF3800，配备可互换的圆形与线性 4 麦克风阵列，可实现精确的 360° 或定向音频采集——非常适合机器人与智能设备。
title: 使用 XIAO ESP32S3 的 reSpeaker Flex 入门指南
keywords:
  - reSpeaker flex
  - xvf3800
  - ESP32S3
...
<table align="center">
  <tr>
    ...
    <th>reSpeaker Flex XVF3800 圆形阵列，搭配 XIAO ESP32S3</th>
...
reSpeaker Flex 核心板包含专用焊盘，可焊接 Seeed Studio XIAO ESP32S3 模块，在 XVF3800 音频处理链路之上增加 Wi-Fi 与 Bluetooth 连接能力。包含 XIAO 的 SKU（-C4-1 和 -L4-1）在出厂时已预装此模块；基础 SKU（-C4-0 和 -L4-0）则保留焊盘但不焊接模块。
当安装 XIAO 后，它与 XVF3800 之间具备以下连接：
...
要将 reSpeaker Flex 与 XIAO ESP32S3 搭配使用，请确保 reSpeaker Flex 固件为 `I2S` 版本。请参考固件烧录以刷入最新的 `I2S` 固件。请访问此[章节](https://wiki.seeedstudio.com/cn/respeaker_flex_introduction/#update-firmware)
...
- 安装当前上游版本、版本号为 1.8 或更高的 Arduino IDE。当前版本可在 [arduino.cc](https://www.arduino.cc/en/Main/Software) 网站获取。
