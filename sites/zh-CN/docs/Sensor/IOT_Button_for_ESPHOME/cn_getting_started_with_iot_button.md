---
description: Seeed Studio IoT Button 入门指南
title: Seeed Studio IoT Button 入门指南
keywords:
  - IoT Button
  - ESP32-C6
...
IoT Button 是一款多功能智能开关，通过 ESPHome（Wi-Fi）和 Zigbee 提供双重 Home Assistant 集成。由 ESP32-C6 驱动，具有可定制的 RGB LED 指示器、多种按压模式的可编程事件触发器、USB-C 充电和灵活的安装选项 - 为直观的智能家居控制提供所需的一切。

本指南将引导您设置 IoT Button，无论您拥有的是 **V1 硬件（出厂预装 ESPHome 固件）** 还是 **V2 硬件（出厂预装 Zigbee 固件）**。

### 特性

- **Home Assistant 就绪**：通过 ESPHome 或 Zigbee Home Automation (ZHA) 集成实现无缝集成。
...
在开始之前，了解产品的一些基本参数是非常重要的。下表提供了 IoT Button V1 和 V2 特性的信息。
...
当您首次收到 IoT Button 时，在使用前执行初始激活步骤是必要的。这适用于 **V1 和 V2** 硬件。

1. 使用标准 5V USB Type-C 数据线将 IoT Button 连接到电源（如 USB 充电器或计算机）。
2. **这个初始电源连接至关重要，因为它激活了内置电池的保护电路。**
3. **没有这个激活步骤，设备在电池供电时将无法正常工作。**
...
如果您是高级用户，可以在任一硬件版本上刷写任一固件。更多详情请参见"高级用法"部分。

---

### 方法 1：ESPHome（Wi-Fi）集成

此方法是 **V1 硬件** 的默认方法，适用于希望通过 Wi-Fi 将 IoT Button 连接到 Home Assistant 的用户。
...
Home Assistant 运行后，确保从插件商店安装并启动 **ESPHome 插件**。
...
您的设备已预刷固件。只有在您想要更新到[最新固件版本](https://github.com/Seeed-Studio/xiao-esphome-projects/releases)或现有固件损坏时才需要执行此步骤。

<Tabs>
<TabItem value="Web Tool" label="网页工具">

最简单的方法是使用我们的 [**XIAO ESPHome Projects 固件刷写器**](https://gadgets.seeed.cc/)。

1. 通过 USB 线缆将 IoT Button 连接到计算机。
2. 在刷写器页面上，找到 "Seeed Studio IoT Button" 并点击 **INSTALL**。
3. 从弹出对话框中选择正确的 COM 端口。
4. 浏览器将自动下载并刷写最新的 ESPHome 固件到您的设备。

</TabItem>
<TabItem value="ESPHome Web" label="ESPHome 网页">
...
3. 强制门户应该会自动打开。如果没有，请在浏览器中导航到 `192.168.4.1`。
...
2. 在 "Discovered" 部分下，您应该看到 `Seeed Studio IoT Button`。
3. 点击 **CONFIGURE**，然后 **SUBMIT**，并将设备分配到一个区域（例如，客厅）。
...
2. 对于 **Trigger**，选择 `Device` 并选择您的 IoT Button。
3. 从列表中选择触发器类型，例如：
    - `Single-press action`
    - `Double-press action`
    - `Long-press action`
4. 对于 **Action**，选择您想要控制的设备或服务（例如，`light.toggle`）。
5. 保存自动化。
...
此方法是 **V2 硬件** 的默认方法，适用于希望将 IoT Button 连接到 Zigbee 网状网络的用户。
...
2. **设置 Zigbee Home Automation (ZHA)**：
    ...
    - 点击 **Add Integration** 并搜索 **Zigbee Home Automation**。
    - 按照提示使用您的协调器设置 ZHA。
...
3. 按一次 IoT Button 上的按钮来唤醒它并启动配对。
    - 如果它没有配对，设备可能已进入睡眠状态。再次按下它。
    - 要强制配对，按住按钮 **超过 5 秒**。这会触发恢复出厂设置并将设备直接置于配对模式。RGB LED 将闪烁以确认。
...
- （不推荐）**切换到 ESPHome**：如果您的按钮正在运行 Zigbee 固件而您想使用 Wi-Fi，只需按照**方法 1，步骤 2** 中的刷写说明安装 ESPHome 固件。

- **切换到 Zigbee**：如果您的按钮正在运行 ESPHome 而您想加入 Zigbee 网络，您需要编译并刷写 [使用 Arduino IDE 的 Zigbee 固件](https://wiki.seeedstudio.com/cn/seeed_iot_button_with_zigbee/)。刷写后，按照**方法 2** 中的配对步骤进行操作。

## 资源

- **[GITHUB]** [Seeed Studio IoT Button V2 Github 仓库](https://github.com/Seeed-Studio/xiao-esphome-projects/tree/main/projects/seeedstudio-iot-button)
- **[PCB 设计文件]** [Seeed Studio IoT Button V2 KiCad 项目](https://files.seeedstudio.com/wiki/IoT_Botton_ESPHOME/Seeed%20IoT%20Button-C6%20v2.0_SCH&PCB_250721.zip)
- **[原理图]** [Seeed Studio IoT Button V2 原理图](https://files.seeedstudio.com/wiki/IoT_Botton_ESPHOME/Seeed%20IoT%20Button-C6%20v2.0_SCH_250721.pdf)
