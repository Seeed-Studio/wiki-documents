---
description: SenseCraft AI 是 Seeed Studio 推出的零代码/低代码 AIoT 平台，可将预训练 AI 模型部署到诸如 reSpeaker XVF3800 等边缘设备。通过唤醒词 "Lumio"，本方案可实现语音控制的 AI 应用——无需复杂编码。
title: 搭配 reSpeaker XVF3800 的 SenseCraft AI
keywords:
  - SenseCraft
  - reSpeaker XVF3800
  - 关键词检测
...
SenseCraft AI 是 Seeed Studio 推出的零代码/低代码 AI 平台，可简化将预训练人工智能模型部署到诸如 reSpeaker XVF3800 等边缘设备的过程。本指南重点介绍如何将 reSpeaker XVF3800 配置为使用自定义触发词 "Lumio" 进行唤醒词检测，从而在无需编写复杂代码的情况下，为你的项目实现语音激活控制。借助 SenseCraft AI，你可以快速测试、预览并将唤醒词事件集成到硬件工作流中。该平台还允许你创建并上传自己的自定义模型，用于检测特定声音事件和自定义唤醒词，为你按需定制语音交互提供充分的灵活性。
...
按照设备的标准 DFU（设备固件更新）[流程](https://wiki.seeedstudio.com/cn/respeaker_xvf3800_introduction/#update-firmware)将此文件烧录到 reSpeaker XVF3800。
...
你正在部署的模型名为 **Lumos**。它是一个轻量级语音识别模型，旨在为边缘设备提供高效、低延迟的语音交互能力。通过分析音频的频谱特征，该模型即使在复杂的环境背景噪声中，也能准确检测到特定唤醒词 **"Lumos"**。
:::
...
3. 在模型烧录到 reSpeaker XVF3800 的过程中稍作等待
...
在训练自定义声音之前，你必须先建立正常环境噪声的基线。

1. 按下 **Collect Training Data for Background Noise**
...
为了获得可靠的检测效果，请重复数据采集过程，直到为 Grassbreaking 类别至少收集到 **8 个样本**。
...
在界面中进入 **Step 3: Deploy**。

**6.2 部署模型**

1. 点击 **Deploy** 按钮，将模型推送到你的设备
...
2. 将弹出一个确认对话框
3. **Confirm** 部署，以将 TFLM（TensorFlow Lite Micro）模型推送到 reSpeaker 的 XIAO ESP32-S3 上
...
1. 界面将为每个检测到的类别显示 **动态置信度条**
2. 发出或制造草折断的声音以测试检测效果
