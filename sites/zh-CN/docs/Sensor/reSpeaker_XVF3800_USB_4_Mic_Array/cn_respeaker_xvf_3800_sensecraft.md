...
title: 搭配 reSpeaker 的 SenseCraft AI
keywords:
  - SenseCraft
  - reSpeaker XVF3800
  - 关键词唤醒
...
SenseCraft AI 是 Seeed Studio 推出的零代码/低代码 AI 平台，可简化将预训练人工智能模型部署到诸如 reSpeaker 等边缘设备的流程。本指南重点介绍如何配置 reSpeaker 以使用自定义触发词 "Lumio" 进行唤醒词检测，从而在无需编写复杂代码的情况下，为你的项目启用语音激活控制。借助 SenseCraft AI，你可以快速测试、预览并将唤醒词事件集成到硬件工作流中。该平台还允许你创建并上传自己的自定义模型，用于检测特定声音事件和自定义唤醒词，为你量身定制语音交互提供充分的灵活性，以满足独特的应用需求。
...
按照设备的标准 DFU（设备固件更新）[流程](https://wiki.seeedstudio.com/cn/respeaker_xvf3800_introduction/#更新固件)将此文件烧录到 reSpeaker XVF3800。

如果你使用的是 reSpeaker Lite，请确保其正在运行正确的固件版本。

所需的固件文件为：
> `respeaker_lite_i2s_dfu_firmware_v1.0.9.bin`

按照设备的标准 DFU（设备固件更新）[流程](https://wiki.seeedstudio.com/cn/reSpeaker_usb_v3/#firmware-下载)将此文件烧录到 reSpeaker Lite。
...
2. 确认工作区已将 **reSpeaker** 设置为当前设备类型
3. 在设备列表中选择 **reSpeaker**
4. 点击 **Connect** 按钮与设备建立连接
...
2. 将弹出一个确认对话框
3. **Confirm** 部署，将 TFLM（TensorFlow Lite Micro）模型推送到 reSpeaker 的 XIAO ESP32-S3 上
