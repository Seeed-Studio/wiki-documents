---
description: XIAO 1.14'' IPS Display（ESP32-S3）板载各外设的独立功能级示例。涵盖屏幕、IMU、PDM 麦克风和 I2S 音频（语音条 + 闪存录音机）、Grove I2C、按键和电池电压检测。
title: 板载外设使用
keywords:
  - XIAO
  - ESP32-S3
...
本页汇总了 1.14'' IPS Display 上每个板载外设的独立功能级示例。每个小节都是自包含的——你可以直接选择符合自己使用场景的示例，而无需阅读其他部分。
...
:::note
本页所有示例都需要按照[快速上手](/cn/getting_started_1.14_inch_display_esp32s3)中所述安装 **esp32 Boards by Espressif (3.3.11)**，并按下文说明手动安装 **Seeed_GFX2** 库。
:::
...
:::tip
- **Seeed_GFX2** 是 Seeed Studio 基于分层 `Board` + `Panel Config` 架构构建的图形库。每个示例都通过一次 `display.begin<Board_..., Config_...>()` 调用来初始化显示屏——**Board** 模板负责引脚映射（CS/DC/SCK/MOSI/RST/BL），而 **Panel Config** 预设了 135×240 分辨率、颜色顺序和反色设置。无需 `driver.h` 或手动引脚配置。
- 在这块板子上，示例使用 `Board_XIAO_1inch14_LCD<13, 12>`（RST=13，BL=12）配合 `Config_Seeed_1inch14_LCD_ST7789`（135×240）。少数示例会定义一个草图本地的 `Config_XIAO_1inch14_LCD_ST7789_BGR` 覆盖配置，以使用 BGR 颜色顺序。
...
- 1.14'' IPS Display **没有触摸控制器，也没有 SD 卡槽**，因此不需要任何触摸或 SD 库。
:::

## 获取示例代码

本页所有示例都位于 [Display-Gadgets](https://github.com/Seeed-Projects/Display-Gadgets) 仓库的 `code_GFX2/Function/` 目录下。每个示例都是一个包含单个 `.ino` 草图的文件夹。**务必下载完整文件夹**，而不是从 GitHub 网页视图中复制 `.ino` 源码。

**方案 A —— 将仓库下载为 ZIP（推荐）：**

1. 打开 [github.com/Seeed-Projects/Display-Gadgets](https://github.com/Seeed-Projects/Display-Gadgets)，点击 **Code > Download ZIP**，然后在任意方便的位置解压。
2. 进入 `code_GFX2/Function/`，打开每个示例 **Code location** 行中给出的文件夹。例如，本板的 GraphicTest 示例位于 `code_GFX2/Function/114_ESP32/xiao_esp32s3_114_graphictest/`。
...
然后从克隆得到的 `code_GFX2/Function/...` 文件夹中打开示例的 `.ino` 文件。

## 屏幕显示 — GraphicTest

此示例在 1.14 英寸 ST7789 IPS 面板（135×240）上运行完整的图形基准测试，涵盖色条、直线、矩形、圆形、三角形、圆角矩形、文本以及像素渐变。可用来验证屏幕连线是否正确，以及所有绘图调用是否按预期工作。
...
**Board** 模板负责引脚映射——CS=D2、DC=D3、SCK=D8、MOSI=D10——其 `<RST, BL>` 模板参数使用裸 GPIO 号，因此 `<13, 12>` 会将 RST 设为 GPIO13（D17），BL 设为 GPIO12（D18）。**Panel Config** 预设了 135×240 分辨率、颜色顺序以及反色设置（`invert = true`），因此无需 `driver.h` 或手动调用 `invertDisplay()`。
...
在屏幕上，你会看到每个测试图案显示约一秒钟，然后切换到下一个。当所有测试完成后，会出现一个带蓝色圆角矩形边框的 “Graphic / Finished” 结束画面。
...
当草图运行完所有图案后，屏幕会显示 “Graphic / Finished” 提示，并在下方显示 “Reset to rerun”。重置开发板即可再次运行测试。
...
:::note
板载 IMU 为 **LSM6DS3**（已从板卡原理图确认，I2C 地址为 `0x6A`）。Electronic Quicksand 示例会探测 QMI8658 兼容传感器，作为防御性回退方案。Raise to Wake 示例则针对板载 LSM6DS3 的唤醒寄存器。
:::

下列示例通过 I2C（`Wire`）直接读取 IMU——不需要外部 IMU 库。

<a id="imu-quicksand"></a>

### 示例 1：电子流沙

此示例将屏幕变成一个交互式流体模拟——金色沙粒会根据重力方向流动和沉降，而重力方向由板载六轴 IMU 测量。倾斜开发板，沙粒就会实时改变流动方向。
...
该仿真在 135×240 屏幕上叠加了一个 **22×40 的占用网格**，其中每个单元为 6×6 像素。大约有 **150 个粒子** 被放置在网格中，每个粒子具有位置、速度以及金色的颜色渐变。
...
3. **单元占用** —— 在流动中更深（相对于重力更接近“底部”）的粒子流动性降低，从而产生逼真的堆积效果。
...
该演示使用 LSM6 兼容 IMU 的**嵌入式唤醒事件检测器**——这是一项硬件特性，它在内部监测加速度计数据，当运动超过可配置阈值时拉高 INT1 引脚（在本板上连接到 D14）。这意味着 MCU 不需要持续轮询加速度计。
...
2. **自动休眠** —— 在 8 秒无活动后，示例程序关闭背光，显示“Sleeping... Pick up device to wake”消息，通过 `esp_sleep_enable_gpio_wakeup()` 将 D14 配置为唤醒源，并进入 ESP32 轻睡眠。
...
**步骤 2.** 屏幕会显示一个包含电源状态、运动数据和倒计时计时器的仪表盘。让开发板静止 8 秒——它会自动进入休眠。
...
1.14'' IPS Display 板载一个用于音频输入的 **PDM（脉冲密度调制）数字麦克风**，以及用于驱动外部扬声器/功放的 I2S 输出焊盘。本节展示两个演示：一个是实时的麦克风输入**语音柱状图（Voice Bar）**可视化（无需额外硬件），另一个是**Flash 录音机**，可将音频录制到板载 Flash，并通过外部 I2S 功放回放。
...
该演示将 PDM 麦克风的实时音频输入可视化为动态均衡器风格的波形和分段音量条。对着板载麦克风说话、鼓掌或吹气，观察柱状条如何即时响应——无需任何外部硬件。
...
麦克风以 **16 kHz 单声道**采样到 256 样本的 DMA 缓冲区（4 个描述符）。在 `loop()` 中，`i2s_channel_read()` 获取一个缓冲区，去除直流偏移，计算峰值幅度，并将信号下采样到 27 个区间用于波形可视化。还通过 `gpio_set_drive_capability()` 降低 PDM 时钟驱动能力，以减少 EMI/耦合噪声。
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td><strong>波形</strong></td><td>顶部 (y=30–95)</td><td>27 柱均衡器可视化。原始采样被降采样，并以围绕中心基线对称的柱状图绘制。波形颜色跟随平滑后的音量变化——绿色 (&lt;50%)、黄色 (50–90%)、红色 (&gt;90%)。</td></tr>
    <tr><td><strong>百分比</strong></td><td>中部</td><td>大的数字音量百分比 (0–100%)，按颜色编码：绿色 (&lt;50%)、黄色 (50–90%)、红色 (&gt;90%)。</td></tr>
    <tr><td><strong>音量条</strong></td><td>底部 (y=130–225)</td><td>10 段条形（绿/黄/红渐变）。根据来自 PDM 峰值的平滑音量进行更新。</td></tr>
  </table>
</div>
...
**步骤 5.** 对着麦克风说话、鼓掌或吹气。波形和音量条会实时响应。随着音量增加，百分比标签的颜色会发生变化。
...
此演示会将来自板载 PDM 麦克风的 5 秒音频录制到板载 Flash 存储中，然后通过连接到 I2S 输出的外部扬声器进行回放。按下一个按键进行录音，另一个按键进行播放。
...
**回放** — 按下 **USR2** 会从 Flash 中读回 WAV 文件，并通过 I2S 外设以标准（Philips）立体声模式在 D11/D12/D13 上进行流式输出。单声道采样会复制到两个声道，并施加 `0.75×` 增益以避免削波。功放驱动一个小扬声器，你就可以听到录音内容。
...
:::note
录音存储在板载 Flash（`LittleFS`）中，因此在断电后仍然保留——你可以录制一次，稍后再回放。再次录音会覆盖之前的文件。
:::
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td>SDA</td><td>D4</td><td>I2C 数据线 — 与板载 IMU 共享</td></tr>
    <tr><td>SCL</td><td>D5</td><td>I2C 时钟线 — 与板载 IMU 共享</td></tr>
  </table>
</div>
...
2. **单次测量** — 发送高重复性单次测量指令（`0x24 0x00`，无时钟拉伸），等待 20 ms，然后读取 6 个字节：温度高/低字节 + CRC，湿度高/低字节 + CRC。
...
屏幕会以每秒一次的频率更新，显示 “SHT31 OK” 以及温度和湿度。如果传感器断开连接或 CRC 校验失败，屏幕会显示 “SHT31 ERROR” 和错误代码。
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td><strong>USR2</strong></td><td>D7</td><td>切换关闭屏幕 / 恢复到上次亮度</td></tr>
...
示例通过 `Board_XIAO_1inch14_LCD<13, 12>` 和示例本地的 `Config_XIAO_1inch14_LCD_ST7789_BGR`（135×240，BGR，invert = true）初始化显示屏，然后以 700 µs 间隔对 **D16** 采样 12 次，使用 `analogReadMilliVolts()`，12 位分辨率、11 dB 衰减。它将采样结果求平均得到原始分压电压，再乘以分压比得到电池电压（`Calc = D16 × 2.975`），并将这两个数值以两行居中的黄色文字绘制出来。只有当任一数值发生显著变化时（D16 ≥ 0.02 V 或 Calc ≥ 0.05 V），屏幕才会刷新。
...
**步骤 4.** 观察屏幕——它会显示两行黄色文字：原始 D16 分压电压和计算得到的电池电压。连接或断开 LiPo 电池（或 USB-C 线缆），即可观察数值的变化。
...
屏幕顶部行显示原始 D16 分压电压，底部行显示计算得到的电池电压（`Calc`）。当连接 LiPo 电池时，`Calc` 近似等于电池端电压。在仅使用 USB 供电时也可能出现读数，因此仅凭 `Calc` 无法确认是否连接了电池。
...
- **🖨️[3D 打印外壳]** [XIAO 1.14'' IPS Display Enclosure (by gokul)](https://www.printables.com/model/1843003-enclosure-for-xiao-114-ips-display-esp32nrf52840)
