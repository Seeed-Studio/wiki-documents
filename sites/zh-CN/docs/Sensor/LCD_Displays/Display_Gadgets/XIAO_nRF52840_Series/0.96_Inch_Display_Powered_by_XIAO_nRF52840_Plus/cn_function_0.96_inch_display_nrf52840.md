...
本页汇总了 0.96'' IPS Display 上每个板载外设的独立函数级演示。每个小节都是自包含的——你可以直接选择符合自己使用场景的部分，而无需通读其他内容。
...
:::note
本页中的所有演示都需要已按 [入门指南](/cn/getting_started_0.96_inch_display_nrf52840) 中说明安装好的 **Seeed nRF52 Boards (1.1.13)**，以及按下文说明手动安装的 **Seeed_GFX2** 库。
:::
...
<div class="table-center">
  <table align="center">
    <tr><th>库</th><th>搜索关键字</th><th>作者</th><th>被以下示例使用</th></tr>
...
**步骤 2.** 在 Arduino IDE 中，进入 **Sketch > Include Library > Add .ZIP Library...**，选择刚刚下载的 ZIP。IDE 会读取 `library.properties` 并自动将其安装到正确的 `Seeed_GFX2` 文件夹中——你无需重命名解压后的文件夹。（如果要改为手动安装，请先解压压缩包，将解压后的文件夹重命名为 `Seeed_GFX2`，再放入 `Documents/Arduino/libraries/`。）

**步骤 3.** 重启 Arduino IDE，使其检测到新库。

:::tip
- **Seeed_GFX2** 是 Seeed Studio 基于分层 `Board` + `Panel Config` 架构构建的图形库。每个示例都通过一次 `display.begin<Board_XIAO_0inch96_LCD<38, 37>, Config_Seeed_0inch96_LCD_ST7789>()` 调用来初始化显示屏——其中 **Board** 模板负责引脚映射（CS/DC/SCK/MOSI/RST/BL），而 **Panel Config** 预设了 80×160 分辨率、BGR 颜色顺序和旋转角度 2。Seeed_GFX2 使用保守的 10 MHz 硬件 SPI 驱动该面板，从而避免了旧示例中需要使用软件 SPI 的信号裕度问题。
- 0.96'' IPS Display **没有触摸控制器和 SD 卡槽**，因此不需要任何触摸或 SD 库。
:::

## 获取示例代码

本页中的每个示例都位于 [Display-Gadgets](https://github.com/Seeed-Projects/Display-Gadgets) 仓库的 `code_GFX2/Function/` 目录下。每个示例都是一个包含单个 `.ino` 草图的文件夹。**务必下载完整文件夹**，而不是从 GitHub 网页视图中复制 `.ino` 源码。

**方案 A — 将仓库下载为 ZIP（推荐）：**

1. 打开 [github.com/Seeed-Projects/Display-Gadgets](https://github.com/Seeed-Projects/Display-Gadgets)，点击 **Code > Download ZIP**，然后将压缩包解压到任意方便的位置。
2. 进入 `code_GFX2/Function/`，打开每个示例 **Code location** 行中给出的文件夹。例如，本板的 GraphicTest 示例位于 `code_GFX2/Function/096_nRF52840/xiao_nrf52840_096_graphictest/`。
...
然后从克隆得到的 `code_GFX2/Function/...` 文件夹中打开该示例的 `.ino` 文件。

## 屏幕显示 — GraphicTest

此示例在 0.96 英寸 ST7789 IPS 面板（80×160）上运行完整的图形基准测试，涵盖色条、直线、矩形、圆形、三角形、圆角矩形、文本以及像素渐变。可用来验证屏幕连线是否正确，以及所有绘图调用是否按预期工作。
...
**Board** 模板负责引脚映射——CS=D2、DC=D3、SCK=D8、MOSI=D10——其 `<RST, BL>` 模板参数直接使用裸 GPIO 号，因此 `<38, 37>` 表示 RST=GPIO38、BL=GPIO37。**Panel Config** 预设了 80×160 分辨率、BGR 颜色顺序和旋转角度 2——无需 `driver.h` 或手动调用 `invertDisplay()`。Seeed_GFX2 使用保守的 10 MHz 硬件 SPI 驱动面板，从而避免了旧示例中被迫使用软件 SPI 的信号裕度问题。

:::note
**颜色顺序（BGR 面板）。** 这块 0.96 英寸面板在物理上交换了红色和蓝色通道。示例中对颜色做了相应别名处理（例如，线级的红色 `0xF800` 在屏幕上会显示为蓝色）。如果你编写自己的绘图代码，请使用示例中的颜色别名，或自行考虑 BGR 顺序——否则红色和蓝色会互换显示。
:::
...
在屏幕上，你会看到每个测试图案显示约一秒钟，然后切换到下一个。当所有测试完成后，会出现 “Done! All tests OK” 屏幕。
...
0.96'' IPS Display 集成了一颗板载 6 轴 IMU（**LSM6DS3**），通过 I2C 连接在 D4/D5 上。**D14** 上的运动中断引脚支持硬件唤醒和手势检测。

下方两个示例都将 LSM6DS3 配置在 I2C 地址 **0x6A**。

<a id="imu-quicksand"></a>

### 示例 1：电子流沙

此示例将屏幕变成一个交互式流体模拟——金色沙粒会根据重力方向流动和沉降，而重力方向由板载 6 轴 IMU 测量。倾斜开发板，沙粒就会实时改变流动方向。
...
该仿真在 80×160 屏幕上叠加了一个 **13×26 占用网格**，每个单元为 6×6 像素。大约 **65 个粒子** 被放置在网格中，每个粒子具有位置、速度以及一个金色的颜色渐变。

IMU 通过 I2C（D4/D5）读取，使用 Seeed Arduino LSM6DS3 库，地址为 `0x6A`。原始加速度值经过低通滤波，用于推导重力向量。当你倾斜开发板时：

1. **重力向量更新** — 加速度计数据通过指数移动平均进行平滑，以避免抖动。
2. **粒子速度** — 每个粒子沿重力向量方向加速，并带有阻尼以及基于其在流体中深度的每粒子流动性因子。
3. **单元占用** — 在流体中更深（相对于重力更接近“底部”）的粒子流动性降低，从而产生逼真的堆积效果。
4. **差分渲染** — 只重绘粒子进出过的单元，最大限度减少 SPI 传输，使小屏幕上的动画保持流畅。
...
该演示实现了一个由 IMU 内置运动中断驱动的 **屏幕休眠/唤醒系统**，中断引脚为 **D14**。屏幕在可配置的空闲时间后会自动关闭（背光关闭 + nRF52 System ON 休眠），当你拿起或移动设备时会立即唤醒。
...
3. **唤醒** — 当用户拿起开发板时，IMU 检测到运动并将 D14 拉高。nRF52840 退出 System ON 休眠，重新点亮背光并重绘 UI——由于 System ON 休眠保留 RAM，LCD 和 IMU 会保持其状态。
...
在唤醒状态下，屏幕显示实时运动数据。静止 8 秒后，屏幕熄灭，nRF52840 进入低功耗休眠。拿起设备后，屏幕会立即恢复，并且唤醒计数器会递增。
...
回放端需要一个外部 I2S 功放。该演示针对 **MAX98357A** 模块编写，通过 0.96'' IPS Display 的 I2S 测试焊盘连接：
...
:::note
0.96'' IPS 显示屏**没有** SD 卡槽，因此本示例会录制到 nRF52840 的**内部闪存文件系统**（InternalFS）中。InternalFS 大约为 **28 KB**——本教程以 16 kHz 录制 11,200 个单声道采样，生成约 22 KB 的 PCM 音频，录音时长约为 0.7 秒。
:::
...
1. 草图以**16 kHz、单通道**启动 PDM 外设，并通过 ISR（`onPdmData`）将原始采样捕获到一个 11200 采样的缓冲区中（≈ 0.7 秒，≈ 22 KB PCM）。
2. 进度屏幕实时显示录音百分比和已用时间。
3. 当缓冲区填满时，草图在前面添加一个 44 字节的 WAV 头，并将文件 `/REC_RAW.WAV` 写入 InternalFS。

**回放（USR2）：**

1. 从 InternalFS 中重新加载 WAV 文件。
2. 草图以**主模式**、32× 比率（≈ 16 kHz LRCK）驱动 nRF52840 的 I2S 外设，将 16 位立体声帧以 **0.75× 增益**发送到 MAX98357A（每个单声道采样被复制到两个声道）。
3. 双缓冲（乒乓）方案保持音频流不中断，直到片段结束。
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td>USR1</td><td>D6</td><td>录制新片段（覆盖之前的片段）</td></tr>
    <tr><td>USR2</td><td>D7</td><td>回放已保存的片段</td></tr>
  </table>
</div>

:::note
**PDM**、**Adafruit TinyUSB**、**Adafruit LittleFS** 和 **InternalFileSystem** 库已随 **Seeed nRF52 Boards 1.1.13** 一起打包——不要从 Library Manager 单独安装其它版本。本教程不使用 SdFat。
:::
...
**步骤 4.** 屏幕显示“Recorder”空闲界面。按下 **USR1**——红色 “REC” 进度条在录音时填充。完成后，它会保存 WAV 并返回空闲界面。

**步骤 5.** 按下 **USR2**——片段会通过连接的 MAX98357A 扬声器播放，屏幕显示 “Playing...”。
...
按下 USR1 后，红色进度条填充到 100%，并保存片段。按下 USR2 会通过 MAX98357A 回放录音。WAV 文件存储在 InternalFS 中，在复位或断电重启后仍然可用。再次按下 USR1 录音会覆盖之前的文件。

:::note
本教程已在 XIAO nRF52840 Plus、0.96'' IPS 显示屏、MAX98357A 功放和外接扬声器上完成编译、烧录和硬件测试。
:::
...
0.96'' IPS 显示屏上有**两个物理按键**连接到 XIAO nRF52840 Plus：
...
若要在不阻塞主循环的情况下实现响应迅速且带消抖的按键处理，可以使用引脚变化中断：
...
当屏幕关闭（通过 USR2 切换）时，再次按下 USR2 会将其恢复到之前的非零亮度级别。

---

## 电池状态

本示例会在 0.96'' IPS 显示屏上显示电池状态——带电量等级和充电状态的电池图标。它会检测是否实际连接了 LiPo 电池，并显示三种状态之一：**USB PWR**（无电池）、**百分比**（仅电池）或 **charging**（USB + 电池）。

0.96'' IPS 显示屏**没有专用电池 ADC 引脚**（D16 悬空 NC）——电池电压通过 XIAO nRF52840 Plus 模组的内部分压器测量。
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td><code>READ_BAT</code></td><td><strong>P0.14</strong></td><td>电池分压器使能。低电平有效——置为 LOW 以使能分压器，然后释放为 HIGH（高阻态）以节省功耗。</td></tr>
...
在 USB-C 供电下，静态 VBAT 电压无法判断电池是否存在——即使没有接电池，充电器的 BAT 节点也可能看起来像一个真实的锂电池。因此示例程序首先学习一个**仅 USB 的基线**，然后只有在 VBAT 持续向下偏移后才确认插入电池，并在出现带噪声/跳变读数且 `~CHG` 变为 HIGH 时确认移除电池。这与出厂 Dashboard 的检测逻辑一致。

**图标状态：**

- **无电池**——灰色轮廓电池，带红色叉号，标注为 **USB PWR**。
- **有电池**——白色轮廓电池，内部为彩色填充（按百分比显示为绿色 / 黄色 / 红色），标注**百分比**和**电压**。
- **充电中**——青色填充并带闪电图标，标注百分比和电压。

:::note
`~CHG` 引脚是通过 nRF52840 的**原始 GPIO 寄存器**（`nrf_gpio_cfg_input()` 和 `NRF_P0->IN`）读取的，而不是通过 `digitalRead()`。在 Arduino API 中，引脚编号遵循开发板包的映射，其中 `digitalRead(17)` 实际读取的是 **P0.07**（6D IMU 的 I2C 数据线），而不是 P0.17。这里的常量 `14` 和 `17` 是**原始 Nordic P0.x 引脚编号**（P0.14 和 P0.17），这正是寄存器调用所期望的。
:::

:::note
该演示使用的是工厂校准的 **499 kΩ** 低端电阻（分压比约为 3.004），而不是 510 kΩ 的标称值。该分压器集成在 XIAO nRF52840 Plus 模组本体中，而不是在显示板上。P0.14 使能引脚为**低电平有效**：将其驱动为低电平以启用分压器，然后将其释放为高阻态（输入）以在未测量电池时将静态电流消耗降到最低。
:::
...
在没有电池的情况下，屏幕会显示一个带红色叉号的灰色电池，并带有 **USB PWR** 标签。插入 LiPo 电池后，图标会切换为带有百分比和电压的彩色填充。电池存在时插入 USB-C，填充会变为青色并带有闪电符号，表示正在充电。
