...
1.47'' IPS Display 是一款为 XIAO 系列设计的扩展板，由 XIAO nRF52840 Plus 提供驱动。它配备 172×320 彩色 LCD 电容触摸屏、板载 PDM 麦克风、6 轴 IMU（LSM6DS3）、MicroSD 卡槽以及电池电源管理——所有功能都集成在一个紧凑的外形中。
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td>内存</td><td>256 KB RAM + 1 MB 内部 Flash + 2 MB 板载 Flash</td></tr>
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td>D17</td><td>LCD_RST</td><td>屏幕复位信号</td><td>独立 GPIO 控制，确保可靠启动</td></tr>
...
**步骤 2.** 在 Arduino IDE 中，依次点击 **Sketch > Include Library > Add .ZIP Library...**，选择下载的 ZIP 文件。IDE 会读取 `library.properties` 并自动将其安装到正确的 `Seeed_GFX2` 文件夹中——你无需重命名解压后的文件夹。（如果要改为手动安装，请先解压压缩包，并将解压后的文件夹重命名为 `Seeed_GFX2`，再放入 `Documents/Arduino/libraries/`。）
...
进入 `code_GFX2/getting_started_code/xiao_nrf52840_147_hello/` 并在 Arduino IDE 中打开 `xiao_nrf52840_147_hello.ino`。**请下载完整文件夹**，而不是从 GitHub 网页视图中复制 `.ino` 源码。
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td>SD 卡</td><td>[Image Reader](/cn/function_1.47_inch_touch_display_nrf52840#sd-card--image-reader) — 从 MicroSD 卡上显示一张 BMP 图片</td></tr>
    ...
    <tr><td>按键</td><td>[User Button](/cn/function_1.47_inch_touch_display_nrf52840#user-button) — 读取按键并使用中断进行去抖</td></tr>
