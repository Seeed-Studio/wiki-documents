...
这种组合使其成为可穿戴设备、紧凑型传感器节点、便携式仪器以及对空间要求严格的 IoT 原型开发的理想平台。
...
**步骤 2.** 在 Arduino IDE 中，依次点击 **Sketch > Include Library > Add .ZIP Library...**，并选择下载的 ZIP 文件。IDE 会读取 `library.properties` 并自动将其安装到正确的 `Seeed_GFX2` 文件夹中——你无需重命名解压后的文件夹。（如果要改为手动安装，请先解压压缩包，并在放入 `Documents/Arduino/libraries/` 之前将解压后的文件夹重命名为 `Seeed_GFX2`。）
...
:::note
如果遇到上传问题，请双击复位按钮进入 bootloader 模式。USR LED 会呈红色呼吸效果，并且电脑上会出现一个名为 **NRF52BOOT** 的磁盘，这表明开发板已进入 bootloader 模式。
:::
...
该显示板集成了多个板载外设。[Function](/cn/function_1.14_inch_display_nrf52840) 页面为每个外设都提供了独立的演示：
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td>麦克风和扬声器</td><td>[Voice Bar + Flash Recorder](/cn/function_1.14_inch_display_nrf52840#microphone--speaker) — 实时 PDM 电平条和带 I2S 回放的录音</td></tr>
...
双击 XIAO nRF52840 Plus 上的复位按钮。USR LED 会呈红色呼吸效果，并且电脑上会出现一个名为 **NRF52BOOT** 的磁盘。将编译生成的 `.uf2` 文件拖拽到 **NRF52BOOT** 磁盘中。开发板会自动完成烧录并自动复位。

### [关于出厂固件 - DashBoard]

#### 在仪表盘运行时，我可以热插拔 I2C 设备吗？

不能——我们强烈**不建议热插拔**在仪表盘运行时连接在 I2C 接口上的设备。请务必在断电状态下，再连接或断开 Grove I2C 接口或 SDA/SCL 引出焊盘上的任何设备。热插拔可能会导致 I2C 总线挂起。
