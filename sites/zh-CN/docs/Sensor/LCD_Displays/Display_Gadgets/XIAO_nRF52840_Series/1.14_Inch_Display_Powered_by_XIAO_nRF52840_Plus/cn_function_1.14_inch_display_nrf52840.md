---
description: 针对 XIAO 1.14'' IPS Display（nRF52840）上每个板载外设的独立函数级演示。涵盖屏幕、IMU、PDM 麦克风、内部 Flash 录音与 I2S 音频回放、按键、电池以及 Grove I2C。
title: 板载外设使用
sidebar_label: 功能
keywords:
  - XIAO
...
本页汇总了 1.14'' IPS Display 上每个板载外设的独立函数级演示。每个小节都是自包含的——你可以直接选择符合自己使用场景的部分，而无需通读其他内容。
...
:::note
本页所有演示都需要已按 [入门指南](/cn/getting_started_1.14_inch_display_nrf52840) 中说明安装好的 **Seeed nRF52 Boards (1.1.13)**，以及按下文说明手动安装的 **Seeed_GFX2** 库。
:::
...
**步骤 2.** 在 Arduino IDE 中，进入 **Sketch > Include Library > Add .ZIP Library...**，选择刚下载的 ZIP。IDE 会读取 `library.properties` 并自动将其安装到正确的 `Seeed_GFX2` 文件夹中——你无需重命名解压后的文件夹。（如果改为手动安装，请先解压压缩包，并在放入 `Documents/Arduino/libraries/` 之前将解压后的文件夹重命名为 `Seeed_GFX2`。）

**步骤 3.** 重启 Arduino IDE，使其检测到新库。

:::tip
- **Seeed_GFX2** 是 Seeed Studio 基于分层 `Board` + `Panel Config` 架构构建的图形库。每个演示都通过一次 `display.begin<Board_..., Config_...>()` 调用来初始化显示屏——**Board** 模板负责引脚映射（CS/DC/SCK/MOSI/RST/BL），而 **Panel Config** 预设了 135×240 分辨率、颜色顺序和反色设置。无需 `driver.h` 或手动配置引脚。
- 在这块板子上，演示使用 `Board_XIAO_1inch14_LCD<38, 37>`（RST=38，BL=37）配合 `Config_Seeed_1inch14_LCD_ST7789`（135×240）。少数演示会定义一个草图本地的 `Config_XIAO_1inch14_LCD_ST7789_BGR` 覆盖配置，以使用 BGR 颜色顺序。
- **IMU** 演示使用上文安装的 **Seeed Arduino LSM6DS3** 库。
- 1.14'' IPS Display **没有触摸控制器，也没有 SD 卡槽**，因此不需要任何触摸或 SD 相关库。
:::

:::note
**Flash Recorder** 教程使用的 **PDM**、**Adafruit TinyUSB**、**Adafruit LittleFS** 和 **InternalFileSystem** 库都已随 **Seeed nRF52 Boards 1.1.13** 一同打包——不要再从 Library Manager 单独安装其它版本。
...
本页的每个演示都位于 [Display-Gadgets](https://github.com/Seeed-Projects/Display-Gadgets) 仓库的 `code_GFX2/Function/` 目录下。每个演示都是一个包含单个 `.ino` 草图的文件夹。**务必下载完整文件夹**，而不是从 GitHub 网页视图中复制 `.ino` 源码。
...
2. 进入 `code_GFX2/Function/`，打开每个演示中 **Code location** 行所示的文件夹。例如，本板子的 GraphicTest 演示位于 `code_GFX2/Function/114_nRF52840/xiao_nrf52840_114_graphictest/`。
...
此演示会在 1.14 英寸 ST7789 IPS 面板（135×240）上运行完整的图形基准测试，涵盖色条、直线、矩形、圆形、三角形、圆角矩形、文本以及像素渐变。可用来验证屏幕连线是否正确，以及所有绘图调用是否按预期工作。
...
**Board** 模板负责引脚映射——CS=D2、DC=D3、SCK=D8、MOSI=D10——其 `<RST, BL>` 模板参数直接使用裸 GPIO 号，因此 `<38, 37>` 即设置 RST=GPIO38、BL=GPIO37。**Panel Config** 预设了 135×240 分辨率、颜色顺序以及反色设置（`invert = true`），因此无需 `driver.h` 或手动调用 `invertDisplay()`。
...
在屏幕上，你会看到每个测试图案显示约一秒钟，然后切换到下一个。当所有测试完成后，会出现一个 “Finished” 结束画面。
...
当草图跑完所有图案后，屏幕会显示 “Finished” 提示。重置开发板即可再次运行测试。

---

## IMU

1.14'' IPS Display 板载一个通过 D4/D5 上的 I2C 连接的 6 轴 IMU（LSM6DS3）。**D14** 上的运动中断引脚支持硬件唤醒和手势检测。
...
此演示将屏幕变成一个交互式流体模拟——金色沙粒会根据板载 6 轴 IMU 测得的重力方向流动和沉降。倾斜开发板，沙粒会实时改变流动方向。
...
该仿真在 135×240 屏幕上叠加了一个 **22×40 占用网格**，其中每个单元为 6×6 像素。大约有 **150 个粒子** 被放置在网格中，每个粒子具有位置、速度以及金色的颜色渐变。
...
当你倾斜开发板时，金色沙粒会平滑流动。保持开发板水平时，沙子会沉积在屏幕底部。将开发板旋转 90 度，沙子会在一秒内流向新的“底部”。

---

### 演示 2：抬起唤醒

该演示实现了一个由 IMU 内置运动中断驱动的 **屏幕休眠/唤醒系统**，中断引脚为 **D14**。屏幕会在可配置的空闲时间后自动关闭（背光关闭 + nRF52 System ON 休眠），当你拿起或移动设备时会立即唤醒。
...
1. **活动状态** — 屏幕点亮，背光为全亮度，UI 每 250 ms 刷新一次实时 IMU 数据。倒计时计时器显示距离自动休眠剩余的秒数。
...
3. **唤醒** — 当用户拿起开发板时，IMU 检测到运动并将 D14 拉高。nRF52840 退出 System ON 休眠，恢复背光并重绘 UI。LCD 和 IMU **不会**被重新初始化 —— System ON 休眠会保留 RAM 和外设配置，因此二者保持启动时应用的设置。
...
**步骤 4.** 打开 **Tools > Serial Monitor**（115200 波特率）以观察休眠/唤醒的状态切换：
...
该演示将 PDM 麦克风的实时音频输入可视化为动态均衡器风格的波形和分段音量条。对板载麦克风说话、鼓掌或吹气，观察音量条如何即时响应。
...
该示例通过 `PDM` 库（随 Seeed nRF52 Boards 一同提供）以 16 kHz、单通道的方式使用 nRF52840 的 PDM 外设。ISR（`onPDMdata`）将原始 PDM 采样捕获到一个 256 采样的环形缓冲区中，并计算峰值幅度。
...
<div class="table-center">
  <table align="center">
    ...
    <tr><td><strong>波形</strong></td><td>顶部（y=30–95）</td><td>27 柱均衡器可视化。原始采样被降采样，并作为围绕中心基线对称的柱状条绘制。波形颜色由与音量条和百分比标签相同的平滑音量驱动 —— 绿色（&lt;50%）、黄色（50–90%）、红色（&gt;90%）。</td></tr>
...
此演示将板载 PDM 麦克风录制的一小段音频剪辑保存到 nRF52840 的**内部 Flash 文件系统**中，然后通过外部 I2S 功放和扬声器回放：

- **USR1** 从板载 PDM 麦克风录音。
- 录音为 **16 kHz、16 位、单声道**。
- 每段剪辑约 **0.7 秒**——11,200 个采样（22,400 字节 PCM）。
- 剪辑以 **`/REC_RAW.WAV`** 的形式保存在内部 Flash 文件系统中。
...
将扬声器连接到 MAX98357A 的 **SPK+** 和 **SPK-** 端子。**不要**将扬声器的一根线接到 GND——MAX98357A 是桥接负载（BTL）功放，因此两个扬声器端子都必须连接到 SPK 输出。
...
**步骤 5.** 按下 **USR1**，并立即对着板载麦克风说话，大约 **0.7 秒**。

:::tip
录音在你按下 **USR1** 的那一刻就开始——不要等红色进度条出现。0.7 秒的时间窗口是从按下 **USR1** 的那一刻开始计时的，所以请立刻说话，否则会错过剪辑的开头。
:::
...
此演示从插在 Grove I2C 接口上的 **Grove SHT31** 传感器读取温度和湿度，并在屏幕上显示读数。该示例通过 I2C 使用 `Wire.h` 直接与传感器通信——不需要 SHT31 库——并使用传感器的 CRC 校验每次读数。
...
将一个 **Grove SHT31** 温湿度传感器插入 Grove I2C 接口。传感器以 3.3V 供电，并在 I2C 地址 `0x44` 上通信：
...
3. **CRC 校验** — 每个 16 位数值都会与其 CRC 字节进行校验；不匹配会报告为错误（接线问题或模块损坏/噪声）。
...
由于板上已经有外部 1 KΩ 上拉电阻，你可以通过简单的直接读取来读取按键状态：
...
在 USB-C 供电下，静态 VBAT 电压无法判断电池是否存在——即使没有连接电池，充电器的 BAT 节点看起来也像一个真实的锂离子电池。因此示例程序首先学习一个 **仅 USB 的基线值**，然后只有在 VBAT 持续下降后才确认电池插入，并在出现带噪声/跳变的读数且 `~CHG` 变为 HIGH 时确认电池移除。这与出厂 Dashboard 的检测逻辑一致。

**图标状态：**

- **无电池** —— 灰色轮廓电池图标带红色叉号，标签为 **USB PWR**。
- **电池存在** —— 白色轮廓电池图标，内部填充颜色（根据百分比为绿色 / 黄色 / 红色），标签为 **percentage** 和 **voltage**。
...
:::note
该示例使用出厂校准的 **499 kΩ** 低端电阻（分压比约为 3.004），而不是标称的 510 kΩ。分压器集成在 XIAO nRF52840 Plus 模组本身，而不是显示板上。P0.14 使能引脚为 **低电平有效**：将其驱动为 LOW 以使能分压器，然后释放为高阻态（INPUT），以在不测量电池时将静态电流消耗降到最低。
:::
...
在没有电池的情况下，屏幕会显示一个带红色叉号的灰色电池图标，并标有 **USB PWR** 标签。插入一块 LiPo 电池后，图标会切换为彩色填充，并显示百分比和电压。当电池存在时插入 USB-C，填充会变为青色并带有闪电符号，表示正在充电。
