---
description: 将 TRMNL 与任意兼容的 Seeed 电子纸产品一起使用 - reTerminal E1001、E1002、E1003、E1004、TRMNL 7.5" (OG) DIY Kit，或 XIAO 7.5" 电子纸面板。
title: 搭配 TRMNL 使用
keywords:
  - 电子纸显示屏
  - TRMNL
...
本指南是将 **TRMNL** 与任意兼容的 Seeed 电子纸产品配合使用的唯一权威参考。只需在每个需要选择硬件的步骤中选定一次硬件，其余工作流程完全相同。

:::caution 硬件支持
TRMNL 官方支持 **reTerminal E1001**、**reTerminal E1002**、**reTerminal E1003**、**reTerminal E1004**、**TRMNL 7.5" (OG) DIY Kit** 和 **XIAO 7.5" ePaper Panel**。**E1002** 和 **E1004** 采用全彩 Spectra™ 6 面板，但 TRMNL 固件会以**单色模式**渲染其内容。**E1003** 的电容触摸面板仅在 SenseCraft Seeedash 固件下可用；TRMNL 固件则使用设备上的物理按键。
:::
...
- **低功耗**：TRMNL 的高效软件与 E Ink® 极低的功耗需求相结合，构成一个极其节能的解决方案。
...
reTerminal E 系列和 XIAO 7.5" 面板已预先组装——你只需为其供电即可。TRMNL DIY Kit 则需要自行组装。
...
如果设备处于休眠状态且没有响应，请在连接 USB 或进入烧录模式前，先按下机身顶部的 **Refresh** 按钮将其唤醒。
:::
...
:::tip
请注意，屏幕的软性排线非常脆弱，操作时务必小心。如果损坏，整块屏幕将无法工作。
:::

从 [Resources](#resources) 部分获取开源外壳部件并打印，然后将组件装入其中。
...
:::tip
如果你的 TRMNL 套件离路由器较远，可以将天线引到外壳外，以获得更好的信号性能。
:::

</TabItem>
<TabItem value="xiao-075-panel" label="XIAO 7.5&quot; Panel">

XIAO 7.5" 电子纸面板在出厂时已将 XIAO ESP32-C3 直接与面板连接。只需通过 USB-C 插入即可——无需进一步组装。
...
1. 访问 [reTerminal E-Series Firmware Hub](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/)。如需直接进入 E1004 TRMNL 条目，也可以打开 [E1004 TRMNL 1.8.10 Hub 链接](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/?platform=trmnl&device=E1004&firmware=TRMNL_reTerminal_E1004&version=1.8.10)。

2. 在 **Select platform** 中，从官方平台区域选择 **TRMNL** 卡片。
...
reTerminal E1002 使用全彩电子纸屏幕，但 TRMNL 固件当前会以单色模式渲染 E1002 的内容。在 Wi-Fi 配网期间，E1002 的 AP 界面会显示一个紧凑布局，并在右下角显示 TRMNL 标志。当你看到此界面时，请搜索并连接到 E1002 热点，然后按照与其他设备相同的 Wi-Fi 设置流程继续操作。
...
Wi-Fi 配网流程对所有支持的硬件都是相同的。

> 💡 **连接 Wi-Fi 有问题？** 请查看 [TRMNL Device Wi-Fi Troubleshooting Guide](https://help.usetrmnl.com/en/articles/10193157-device-wifi-troubleshooting)。

### 上电并进入配网模式

在烧录 TRMNL 固件并为设备上电后，如果设备尚未连接到 Wi-Fi，它会自动进入配网模式。
...
连接成功后，打开浏览器。设备应自动跳转到 TRMNL 配置页面；如果没有自动跳转，请手动访问 [http://4.3.2.1](http://4.3.2.1)。
...
设备会断开 TRMNL Wi-Fi 并加入你的家庭网络。强制门户窗口会显示设备的 MAC 地址。**记录下这个 MAC 地址** —— 下一步会用到。
...
当设备已连接到 Wi-Fi 并且你已经获得它的 MAC 地址后，在 TRMNL Web 界面中注册该设备：
...
4. **输入你在购买 BYOD 访问权限时收到的 Device ID**（这 **不是** MAC 地址 —— Device ID 可在购买确认邮件或 TRMNL 控制台中找到）。点击 **Add new device**。
...
> 💡 **提示：** 如果你有多台设备，请为每台设备使用其唯一的 Device ID 重复上述步骤。
...
在 TRMNL 固件下，reTerminal E1004 使用前面板的物理按键。使用这些按键唤醒或刷新屏幕，并将电源开关滑到 **OFF** 再滑回 **ON** 以重启设备。若需重新配置 Wi-Fi，请按照设备在出厂烧录或网络重置后屏幕上显示的长按/配网流程操作。
...
:::note
电容触摸仅在 SenseCraft Seeedash 固件下受支持。TRMNL 固件不会使用触摸面板 —— 所有交互都通过上述物理按键完成。
:::
...
- **按下 Reset → 松开 → 按住 BOOT 5 秒**：重新配置网络（返回 TRMNL 配网门户）。
- **RESET 单击**：重启设备。
...
- **Add a Group / Add a Plugin**：将播放列表组织为多个分组或添加新的内容插件。
...
在 Plugins 页面顶部，你可以看到所有已连接到你 TRMNL 账户的插件。例如 Weather、Days Left This Year、Stock Price、RSS Feed、Reddit、Hacker News、Language Learning、Motivational Quote、Custom Text 等。

### 插件市场

在已连接插件下方，你可以看到插件市场。插件按类别和标签（#productivity、#news、#ecommerce 等）进行组织。你可以浏览、搜索，并点击任意插件将其连接到你的账户。
...
长按 **Green Button** 5 秒。设备将返回到初始 TRMNL 界面并激活 AP。
...
1. **电池电压确实偏低。** 这是最常见的情况——设备确实需要充电。请接上电源或更换/给电池充电，一旦电压恢复，警告就会消失。

2. **TRMNL 服务器端问题。** 电池电量是在 TRMNL 的服务器上评估的，而不仅仅是在设备上。服务器 Bug 或最近的平台更新可能会暂时错误报告电池状态，即使硬件正常也会持续显示警告。

**如果电池确实电量不足**，请先给设备充电——在确认电池组状态良好之前，不要隐藏该警告。

**如果你确定电池没有问题**，但警告仍然无法消除，则**不需要**修改固件。TRMNL 允许你在 Web 仪表盘中屏蔽屏幕上的提示：

**步骤 1.** 打开 [TRMNL Web 界面](https://trmnl.app) 并进入 **Devices**。

**步骤 2.** 选择受影响的设备并打开其 **Battery** 设置。

**步骤 3.** 打开 **Hide Low Battery Screen**。这将阻止在 TRMNL 服务器端电量检测恢复期间，或当你需要显示保持在播放列表上时，在设备上显示电量低警告屏幕。
...
- **步骤 2**：在仍按住 `BOOT` 的同时，通过 USB-C 将设备连接到电脑。设备连接后松开 `BOOT`。
...
他们的专业知识使本次集成成为可能，并极大提升了 reTerminal E 系列（E1001、E1002、E1003 和 E1004）、TRMNL DIY Kit 和 XIAO 7.5" ePaper Panel 社区的使用体验。
...
- **[STP]** [3D 外壳模型](https://files.seeedstudio.com/wiki/xiao_075inch_epaper_panel/3D_model.zip)
