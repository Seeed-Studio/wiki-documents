...
如果你想要一个**从“烧录 → 连接 → 第一个仪表盘”的端到端演练**，可以直接跳到下面与你硬件对应的 cookbook；这些页面复用本参考中的样板内容，并添加产品特定示例。

## 兼容硬件

在 [主汇总页面](/cn/seeed_epaper_displays) 上，任何在 **ESPHome** 列被勾选的 Seeed ePaper 产品都可以按本流程运行：
...
:::tip
在找 **TRMNL 云端仪表盘**（无需 YAML、基于插件）吗？请查看 [使用 TRMNL 工作](/cn/reterminal_e10xx_trmnl)。在找 **Seeed 的零代码可视化 HMI** 吗？请查看 [使用 SenseCraft HMI 工作](/cn/EE04_with_hmi)。
:::

## 为什么在 ePaper 上使用 ESPHome？

- **能效高**——ePaper 只有在屏幕更新时才耗电，因此组合（ESP32 + ESPHome 深度睡眠 + ePaper）可以在电池供电下运行数周/数月。
- **日光可读**——与 LCD 不同，显示屏在阳光下也可读；非常适合作为壁挂式 Home Assistant 面板。
- **原生 HA 公民**——一旦设备出现在 Home Assistant 中，每个实体（空调、日历、传感器、人员、天气）都只差一个 Jinja 模板就能上墙显示。
- **本地优先**——无云端、无厂商锁定。一切都在你的局域网中运行。

## 步骤 1：选择烧录路径

ESPHome 固件可以通过两种方式加载到设备上。正确的选择取决于你是想从零编写 YAML，还是只是想快速获得一个可用的仪表盘。

<Tabs groupId="esphome-flash-path">
<TabItem value="web-installer" label="路径 A：ESPHome Web Installer（推荐首次使用者）" default>

Seeed 托管了一个预构建的固件 ZIP，并通过浏览器中的 WebSerial 将其烧录到你的设备上。
...
此路径**不需要编辑 YAML**。如果你之后想开始自定义，可以在 ESPHome 仪表盘中“接管”该设备。
...
- Home Assistant 的一个 **插件（Add-on）**（如果你已经运行 HA OS / HA Supervised，推荐此方式），或
...
2. 仪表盘会生成一个初始的 `<device-name>.yaml`。将其主体替换为 cookbook 中与你产品对应的 YAML（见下方步骤 2 了解结构）。
3. 点击 **Install** → 首次烧录时选择 **Plug into the computer running ESPHome**；之后的烧录通过 OTA 无线完成。
...
每个 Seeed ePaper 的 ESPHome 配置都遵循相同的大纲。与你产品对应的 cookbook 会填充**产品特定部分**（替换项、引脚映射、屏幕型号）——但整体结构始终是：
...
- 任何板载外设（按键 / 蜂鸣器 / 电池 / SHT4x）——在相关 cookbook 的 **进阶** 章节中介绍。
...
现在你可以将这些实体拖入一个 Lovelace 仪表盘，或者——在 ePaper 上更有趣的做法——使用 `display.lambda` 块，通过 [`homeassistant`](https://esphome.io/components/homeassistant.html) 组件将任意 HA 实体直接渲染到屏幕上。
...
- 对于彩色 ePaper（`spectra6`、`7-color`、`bwr`），第一次刷新可能需要 25–45 秒；在进一步调试前请耐心等待。

### 设备没有出现在 Home Assistant 中

- 验证设备是否已加入 Wi-Fi（查看 ESPHome 仪表盘日志）。
