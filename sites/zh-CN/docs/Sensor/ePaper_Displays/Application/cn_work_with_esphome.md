---
description: 使用 ESPHome 和 Home Assistant 驱动任意兼容的 Seeed ePaper 产品的参考指南——YAML 工作流、通用骨架，以及在哪里找到每个产品的 cookbook。
title: 使用 ESPHome 工作
keywords:
  - ePaper 显示屏
  - ESPHome
...
最快的入门方式是使用 **[reTerminal E 系列 Firmware Hub](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/)**。选择 **ESPHome** 卡片，选中你的设备，勾选你想要启用的板载功能（显示屏、按键、电池、传感器、RTC、SD 卡、麦克风、触摸、深度睡眠等），然后将生成的 YAML 复制或下载到你的 ESPHome 控制面板中。这个 Hub 也可以直接在浏览器中烧录演示固件（桌面版 Chrome 或 Edge）。
...
本页是通过 [ESPHome](https://esphome.io/) 驱动任意兼容的 Seeed ePaper 产品并将其集成到 [Home Assistant](https://www.home-assistant.io/) 中的**参考手册**。它涵盖了所有硬件之间相同的部分：

1. 为什么你会选择用 ESPHome 来驱动 ePaper 显示屏。
2. 如何使用 cookbook YAML 示例，将其调整为你的需求，并从 ESPHome 控制面板中烧录。
3. 通用 YAML 骨架——`wifi`、`api`、`ota`、`display`——每个产品都在此基础上用自己的引脚映射进行专门化。
4. 每个产品的 cookbook 在哪里（外设、硬件相关的 lambda、控制面板配方）。

如果你想要一个**从“烧录 → 连接 → 第一个控制面板”的端到端演练**，可以直接跳到下面与你硬件对应的 cookbook；这些页面会复用本参考中的样板内容，并添加产品特定的示例。

## 兼容硬件

在 [主 Hub 页面](/cn/seeed_epaper_displays) 上，每个在 **ESPHome** 列被勾选的 Seeed ePaper 产品都可以使用本工作流：
  ...
  <table align="center">
    ...
    <tr>
      ...
      <td>
        ...
        <a href="https://wiki.seeedstudio.com/cn/reterminal_e10xx_with_esphome_advanced/">按键、蜂鸣器、LED、电池、触摸（E1003）、SHT4x 与深度睡眠</a><br/>
...
- **能效高** —— ePaper 只有在刷新屏幕时才会耗电，因此（ESP32 + ESPHome 深度睡眠 + ePaper）的组合可以在电池供电下运行数周甚至数月。
...
最简单的入门方式是使用 **[reTerminal E 系列 Firmware Hub](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/)**。该 Hub 可以根据你的设备和功能选择生成 ESPHome YAML 文件，因此你不需要手动拼装第一份配置。
...
1. 在桌面版 Chrome 或 Edge 中打开 [reTerminal E 系列 Firmware Hub](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/)。
2. 选择 **ESPHome** 作为平台。
3. 选择你的设备。
4. 在设置步骤中，选择你想启用的功能，例如显示屏、按键、电池、传感器、RTC、SD 卡、麦克风，或者（如果你的设备支持）深度睡眠。
5. 让页面生成匹配的 ESPHome YAML。
6. 使用 **Copy to clipboard** 或 **Download file** 导出生成的 YAML。
7. 将 YAML 粘贴或导入到你的 ESPHome 控制面板中。
8. 将手动编辑的精力集中在对你的项目最重要的部分：ePaper 显示内容、Home Assistant 实体、布局、字体和刷新行为。

:::tip
Firmware Hub 是新手推荐的起点，因为它处理了大量与设备相关的 YAML 结构。当你想要理解生成的配置、组合高级功能，或从更小的示例构建自定义布局时，再使用各个 cookbook。
:::

运行 ESPHome 控制面板的方式可以是：
...
2. 在 ESPHome 控制面板中点击 + New device，输入名称，并选择 cookbook 中列出的 ESP 变体（ESP32-S3、ESP32-C3 等）。
3. 用你的配置替换生成的起始文件。只有在你需要在同一设备上启用多个功能时，才组合多个 cookbook 片段。
4. 点击 Install → Plug into this computer 进行第一次 USB 烧录。在 `wifi`、`api` 和 `ota` 设置完成后，后续更新可以通过 Wi-Fi 进行。
5. 当设备在线后，它会通过 ESPHome 集成出现在 Home Assistant 中。

## 步骤 2：理解生成的 YAML 结构

每个 Seeed ePaper 的 ESPHome 配置都遵循相同的基本结构，但硬件数值并非通用。请将 Firmware Hub 或你产品的 cookbook 作为关于板卡类型、总线引脚、电源使能引脚、显示平台、显示型号以及板载外设的权威来源。

下面的代码块是一个**结构指南**，而不是可以直接烧录的配置。它展示了在你生成或复制了某个产品特定的 YAML 文件之后，各类设置通常会出现在什么位置：
...
请从 Firmware Hub 或 cookbook 中保留以下数值：
...
- `wifi`、`api` 和 `ota` 的密钥。
- `font` 的选择和字号。
- Home Assistant 中提供你想要绘制数据的 `sensor`、`binary_sensor`、`text_sensor` 或 `time` 实体。
- `display.lambda` 代码块，在这里你设计实际的电子纸屏幕布局。
- 刷新行为，例如 `update_interval`、按键触发刷新或深度睡眠的时间安排。
...
4. 设备及其所有实体（传感器、binary_sensors、显示屏）现在都可以在 HA 中使用了。
...
- **[reTerminal E Series — ESPHome Display](/cn/reterminal_e10xx_with_esphome)** — 为 E1001/E1002/E1003/E1004 提供第一个仪表盘、Wi-Fi 设置和电子纸绘图示例。
- **[reTerminal E Series — ESPHome I/O, Battery, Touch & Power](/cn/reterminal_e10xx_with_esphome_advanced)** — 按钮、蜂鸣器、板载 LED、电池监控、SHT4x 传感器、电容触摸（E1003）、深度睡眠和多页面仪表盘。
- **[reTerminal E1001 / E1002 — ESPHome RTC, SD & Microphone](/cn/reterminal_e10xx_with_esphome_rtc_sd_microphone)** — PCF8563 RTC 时间同步、microSD 卡电源/检测引脚以及板载 PDM 麦克风配置。
- **[EE04 driver board — ESPHome](/cn/EE04_with_esphome_advanced)** — 在 XIAO ESP32-S3 + EE04 + 你选择的电子纸屏幕上实现完整的 Home Assistant 集成。
- **[XIAO 7.5" ePaper Panel — ESPHome](/cn/xiao_075inch_epaper_panel_esphome)** — 精简的 ESP32-C3 仪表盘。
- **[TRMNL 7.5" DIY Kit — ESPHome](/cn/ogdiy_kit_works_with_esphome)** — 使用该套件硬件搭配 ESPHome，而不是 TRMNL 云平台。

当有新的电子纸产品发售时，相应的 cookbook 会被添加到每个产品的文件夹下；本主页面也会更新链接到它。
