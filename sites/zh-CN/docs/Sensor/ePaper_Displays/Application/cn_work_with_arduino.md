---
description: 使用 Arduino IDE 驱动任意兼容的 Seeed ePaper 产品的参考指南——Arduino IDE 设置、Seeed_GFX 库安装、driver.h 生成。与硬件相关的深入内容位于各产品的 cookbook 页面中。
title: 使用 Arduino
keywords:
  - ePaper 显示屏
  - Arduino
...
如果你需要的是**从“开箱、烧录 Hello World、点亮传感器和按键”的端到端演练**，请跳转到你所用硬件的 **cookbook**——这些页面会复用本参考中的样板内容，并在此基础上添加产品特定示例（外设、代码示例、故障排查）：

- [reTerminal E 系列 — ePaper 显示屏 cookbook](/cn/reterminal_e10xx_with_arduino)——涵盖 reTerminal E1001 / E1002 / E1003 / E1004，并提供同时使用 `Seeed_GFX` 和 `GxEPD2` 的 Hello World 示例。
- [reTerminal E 系列 — 板载外设 cookbook](/cn/reterminal_e10xx_with_arduino_peripherals)——为同一批型号提供完整外设示例：LED、蜂鸣器、三个用户按键、SHT4x 传感器、电池监测、microSD 卡以及从 SD 渲染 BMP 图片。
...
在 [主汇总页面](/cn/seeed_epaper_displays) 上，任何在 **Arduino** 列被勾选的 Seeed ePaper 产品都可以按本流程操作。概览如下：
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      ...
      <td>你插入的任意 XIAO</td>
      <td>根据你的 XIAO + 屏幕组合使用 Configuration Tool</td>
    </tr>
  </table>
</div>
...
在 Arduino IDE 中选择 **Sketch → Include Library → Add .ZIP Library**，然后选择下载的 ZIP。各产品的 cookbook 会在适用时包含具体的 `GxEPD2` 示例（例如 [reTerminal E 系列 cookbook](/cn/reterminal_e10xx_with_arduino) 展示了在 E1002 上的全彩渲染）。

## 接下来去哪儿——Cookbook

本页有意只停留在样板层面。与硬件相关的代码示例和端到端演练都在各产品的 cookbook 中：

- **[reTerminal E 系列 — ePaper 显示屏 cookbook](/cn/reterminal_e10xx_with_arduino)**——在 E1001/E1002/E1003/E1004 上使用 `Seeed_GFX` 和 `GxEPD2` 运行 Hello World。
- **[reTerminal E 系列 — 板载外设 cookbook](/cn/reterminal_e10xx_with_arduino_peripherals)**——为同一批型号提供完整外设套件示例：LED 控制、蜂鸣器（无源 PWM 蜂鸣 + 旋律播放）、3 按键输入、SHT4x 温湿度、电池管理、microSD 卡、从 SD 渲染 BMP 图片。
- **[XIAO 7.5" ePaper Panel + Arduino](/cn/xiao_075inch_epaper_panel_arduino)**——基于 `Seeed_GFX` 的 XIAO ESP32-C3 全流程演练。
- **[TRMNL 7.5" DIY Kit + Arduino](/cn/ogdiy_kit_works_with_arduino)**——在自定义 Arduino 草图中使用该套件的硬件（而不是 TRMNL 云平台）。
