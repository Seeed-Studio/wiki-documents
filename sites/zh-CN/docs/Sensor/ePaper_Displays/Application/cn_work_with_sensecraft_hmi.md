---
description: 使用 SenseCraft Seeedash 为兼容的 Seeed 电子纸显示设备设计并部署零代码仪表盘。
title: 使用 SenseCraft Seeedash
keywords:
  - 电子纸显示屏
  - SenseCraft Seeedash
  - 零代码仪表盘
...
# 使用 SenseCraft Seeedash

[SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 是 Seeed Studio 面向电子纸显示设备的零代码界面设计平台。你可以在浏览器中设计仪表盘、图片库、日历、RSS 页面、网页内容页以及其他常亮信息屏，然后通过 Wi-Fi 将它们部署到兼容设备上。

本 Wiki 是一个电子纸显示应用指南。它说明了从 Seeed 电子纸设备到可用 SenseCraft Seeedash 页面之间的最短路径，并使用一块电子纸显示板加一块电子纸屏幕作为示例硬件配置。
...
<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="https://sensecraft.seeed.cc/hmi" target="_blank" rel="noopener noreferrer">
            <strong><span><font color={'FFFFFF'} size={"4"}> 打开 SenseCraft Seeedash</font></span></strong>
    </a>
</div><br />

## 何时使用本指南

当你希望将 SenseCraft Seeedash 与兼容的 Seeed 电子纸产品配合使用，并快速完成以下工作流程时，请使用本指南：

1. 确认或烧录 SenseCraft Seeedash 固件。
2. 将设备连接到 Wi-Fi。
3. 将设备添加到你的 SenseCraft Seeedash 工作区。
4. 将你的第一个页面部署到电子纸显示屏。

如需完整的平台手册，包括账号设置、编辑器细节、模板、数据组件和发行说明，请参阅[官方 SenseCraft Seeedash 文档](https://sensecraft-hmi-docs.seeed.cc/en/overview/)。

## 开始之前

在使用 SenseCraft Seeedash 之前，请准备以下物品：

- 一款兼容的 Seeed 电子纸显示设备。
- 一个 SenseCraft 账号。你可以在 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 登录页面创建账号。
- 一个 2.4 GHz Wi-Fi 网络。
- 如果你的设备需要烧录固件，则需要一根 USB-C 数据线。
- 一台电脑，运行支持通过 SenseCraft Seeedash 网页工具进行串口烧录的浏览器。

:::tip
不同的电子纸产品使用不同的 Seeedash 固件构建版本。务必选择与你的具体设备和屏幕尺寸完全匹配的固件。
:::

## 官方 SenseCraft Seeedash 文档

SenseCraft Seeedash 拥有专门的文档网站。使用本 Wiki 完成电子纸显示的快速入门流程，使用官方文档获取完整的平台使用说明。
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      <td>概览</td>
      <td>了解 SenseCraft Seeedash 是什么以及它能做什么。</td>
...
下面的步骤以 **XIAO ePaper Display Board (ESP32-S3) - EE04** 搭配 **7.3" Spectra 6 电子纸显示屏** 为例。其他兼容的 Seeed 电子纸设备遵循相同的平台流程，但固件构建版本、屏幕尺寸和硬件组装可能不同。
...
:::note
本示例并非兼容性列表。关于当前支持的硬件和固件矩阵，请始终参考 [SenseCraft Seeedash 支持的硬件页面](https://sensecraft-hmi-docs.seeed.cc/en/supported_hardware/)。
:::

## 步骤 1：确认或烧录 Seeedash 固件

第一步是确保你的电子纸设备正在运行 SenseCraft Seeedash 固件。

<Tabs groupId="hmi-hardware">
<TabItem value="reterminal" label="reTerminal E 系列" default>

reTerminal E 系列开箱即可与 SenseCraft Seeedash 协同工作。给设备上电，然后继续前往[步骤 2：将设备连接到 Wi-Fi](#步骤-2：将设备连接到-wi-fi)。

如果你之前烧录过其他固件，请打开 [SenseCraft Seeedash 设备页面](https://sensecraft.seeed.cc/hmi/device)，选择你的设备型号，并按照屏幕上的固件指引操作。
...
对于显示板加独立电子纸屏幕的组合，请烧录与你的板卡和面板匹配的 Seeedash 固件。

**步骤 1.** 登录 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi)，然后从顶部菜单中打开 **Tools**。
...
**步骤 2.** 选择与你的显示屏匹配的固件条目。本示例中，选择 **7.3" Full-Color Display 800 x 480** 条目。
...
:::tip
如果没有出现任何端口，请按下板上的 **RESET** 按钮后重试。同时确认你的 USB-C 线缆支持数据传输。
:::
...
在设备运行 Seeedash 固件后，将其连接到你的本地 Wi-Fi 网络。
...
**步骤 2.** 扫描电子纸屏幕上的二维码。如果配置页面没有自动打开，请打开浏览器并访问 `192.168.4.1`。
...
**步骤 3.** 选择本地 Wi-Fi 网络，输入密码，然后点击 **Connect**。
...
## 步骤 3：将设备添加到 SenseCraft Seeedash

完成 Wi-Fi 设置后，将设备添加到你的 SenseCraft Seeedash 工作区。
...
**步骤 2.** 输入设备名称和电子纸屏幕上显示的配对码，然后点击 **Create**。
...
设备配对完成后，你就可以从 SenseCraft Seeedash 向电子纸屏幕部署内容。

最快的入门方式是使用现有模板或内置的创建工具之一。有关详细的编辑器流程，请参考[官方入门指南](https://sensecraft-hmi-docs.seeed.cc/en/guides/getting_started/)。
...
<div class="table-center">
  <table align="center">
    ...
    <tr>
      <td>Web Content</td>
      <td>在电子纸显示屏上渲染网页或基于 Web 的信息面板。</td>
      <td><a href="https://sensecraft-hmi-docs.seeed.cc/en/overview/" target="_blank" rel="noopener noreferrer">打开文档</a></td>
    </tr>
  </table>
</div>

下面的示例展示了部署到 7.3 英寸电子纸显示屏的 SenseCraft Seeedash 页面。
...
<div style={{textAlign:'center'}}>
    ...
    <p style={{fontSize:'0.9em', color:'#555', marginTop:10}}><em>7.3 英寸电子纸显示屏上的示例效果</em></p>
</div>

## 故障排查

### 问题 1：为什么在烧录过程中我的电脑检测不到设备？
...
### 问题 3：为什么烧录后屏幕没有显示预期的设置二维码？

固件可能与所连接的屏幕不匹配，或者屏幕连接不正确。

- 在 SenseCraft Seeedash Tools 页面重新检查设备型号、屏幕尺寸和固件条目。
- 在重新连接电子纸软排线前先断电。
- 确认软排线方向和连接器类型与你的硬件相匹配。
- 重新烧录匹配的固件，并等待电子纸刷新完成。

### 问题 4：为什么部署后的页面与编辑器预览效果不同？

不同的电子纸面板在分辨率、色彩能力、灰度表现和刷新特性方面各不相同。
...
- [SenseCraft Seeedash 平台](https://sensecraft.seeed.cc/hmi)
- [SenseCraft Seeedash 官方文档](https://sensecraft-hmi-docs.seeed.cc/en/overview/)
- [SenseCraft Seeedash 支持的硬件](https://sensecraft-hmi-docs.seeed.cc/en/supported_hardware/)
- [SenseCraft Seeedash 入门指南](https://sensecraft-hmi-docs.seeed.cc/en/guides/getting_started/)
- [SenseCraft Seeedash 发布说明](https://sensecraft-hmi-docs.seeed.cc/en/release_note/)
