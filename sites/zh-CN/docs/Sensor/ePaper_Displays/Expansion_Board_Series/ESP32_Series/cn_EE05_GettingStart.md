...
由 **XIAO ESP32-S3 Plus** 提供算力，显示板 EE05 是一款紧凑型电子纸驱动板，支持 24 针电子纸显示屏。它配备带电源开关的 JST 2.0 mm 电池连接器、内置电池电量检测电路，并带有一个复位按钮和三个用户可编程按钮。非常适合用于低功耗电子纸项目，例如数字标牌、电子标签和便携式信息板。

### 特性

- **由 XIAO ESP32-S3 Plus 供电：** 连接兼容的电子纸显示屏后即可立即工作。
- **多样化显示支持：** 兼容多种电子纸屏幕，原生支持 24 针 SPI 接口。
- **带开关的 BAT 接口：** 提供简单的电池连接，并集成硬件拨动开关，实现高效的电源管理和节能。增加了用于 XIAO ESP32 电池电量检测的附加电路。
- **用户友好按键：** 包含 1 个侧边安装的复位按钮和 3 个用户可编程按钮，为自定义功能（例如唤醒 / 上一页 / 下一页）提供灵活性。
- **低功耗设计：** 在 MCU 休眠期间有效控制或关闭外设的功耗。
...
:::tip
该库与 TFT 库功能相同，但不兼容。如果你已经安装了 TFT 库或其他类似的显示库，请先卸载它们。
:::
...
这里我们以 7.5 英寸黑白显示屏为例。其他受支持屏幕的步骤完全相同；你只需在驱动中选择合适的屏幕尺寸标识符。
...
之后，依次进入 **Tools** -> **Board** -> **XIAO ESP32S3** 和 **Tools** -> **Port** -> **Select the port your board is connected to**。
...
然后点击 **Upload** 上传代码。
现在你就能在电子纸屏幕上看到反馈了！下面是 Bitmap 示例的显示效果。
...
**Q1: 上传代码后屏幕没有任何显示。**
- **检查 FPC 线缆：** 确保电子纸 FPC 线缆插入方向正确，并牢固锁紧在连接器中。
- **确认驱动配置：** 再次检查 `driver.h`，确保为你的具体屏幕尺寸选择了正确的 `BOARD_SCREEN_COMBO`。

**Q2: 颜色反转或显示异常图案。**
- **选择完全匹配的型号：** 不同的屏幕尺寸或 IC 版本需要特定的驱动配置。确保组合编号与屏幕型号完全匹配。
