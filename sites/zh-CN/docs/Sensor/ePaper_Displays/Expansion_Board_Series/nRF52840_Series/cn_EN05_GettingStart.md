...
由 **XIAO nRF52840 Plus** 提供算力，显示板 EN05 支持多种 24 针 ePaper 显示屏。它配备用于连接额外传感器的扩展 IO 端口、带电源开关的 JST 2.0 mm 电池接口、内置充电 IC，并集成了三个用户按键。非常适合用于超低功耗 ePaper 项目，例如智能姓名牌、电子价签、会议室标牌以及便携式信息看板等。
...
| **无线** | Bluetooth 5.0 |
| **ePaper 接口** | SPI FPC 24 针 0.5mm |
| **电池接口** | JST 2.0mm（3.7V 锂电池） |
| **开关** | 硬件电源滑动开关 |
| **按键** | 1 个复位按键（侧边安装）<br/>3 个用户按键（侧边安装） |
| **PCB 厚度** | 1.2mm |
...
完成后，依次进入 **Tools** -> **Board** -> **Seeed nRF52 Boards** -> **Seeed XIAO nRF52840 Plus**，以及 **Tools** -> **Port** -> **选择你的开发板所连接的端口**。
