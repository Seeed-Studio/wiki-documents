---
description: 适用于 reTerminal E1001 / E1002 / E1003 / E1004 的 Arduino 菜谱——包含板载硬件外设示例，如 LED、蜂鸣器、三个用户按键、SHT4x 温湿度传感器、电池监测、microSD 卡以及从 SD 渲染 BMP 图像。
...
:::tip 在找显示相关内容？
本页重点介绍如何使用 Arduino **驱动 reTerminal E 系列的板载硬件外设**。如果你想在 ePaper 屏幕上渲染文本、图形或图像，请前往 **[Arduino 菜谱：ePaper 显示](https://wiki.seeedstudio.com/cn/reterminal_e10xx_with_arduino)**。
:::

## 介绍

reTerminal E 系列不仅仅是一块 ePaper 屏幕——每一款型号还提供一个板载 LED、一个蜂鸣器、三个用户按键、一个 SHT4x 温湿度传感器、电池电压监测电路以及一个 microSD 卡槽。本菜谱收集了针对这些外设的可直接烧录的 Arduino 示例，并包含一个从 SD 卡加载 BMP 图像并渲染到 ePaper 屏幕的真实案例。

本菜谱涵盖内容：

- **LED 控制**：通过 GPIO6（反向逻辑）。
- **蜂鸣器**：在 GPIO45 上实现提示音和音乐音调。
- **三个用户按键**（KEY0 / KEY1 / KEY2），带消抖状态检测。
- 通过 I²C（GPIO19 SDA / GPIO20 SCL）访问 **SHT4x 传感器**，使用 Sensirion 库。
- 通过 ADC + 使能引脚电路实现 **电池电压监测**。
- 在共享 SPI 总线上进行 **microSD 卡** 挂载 / 检测 / 文件列出。
- **综合示例**：从 SD 卡渲染 BMP 图像到 ePaper 屏幕。

### 所需材料

本菜谱适用于所有四款 reTerminal E 系列型号。请选择你手头的设备：
...
- 准备好一根可用的 **USB-C 数据线**，并在 IDE 中选择正确的串口。
- 已确认可以向设备烧录一个基础示例草图——如果你还没有完成，请参考 [Arduino 菜谱：ePaper 显示](https://wiki.seeedstudio.com/cn/reterminal_e10xx_with_arduino#环境准备) 中的环境准备部分。
...
reTerminal E 系列在 GPIO45 上集成了一个蜂鸣器，可以发出不同的音调和提示音。
...
1. **串口初始化**：使用 `Serial1`，并将引脚 44（RX）和 43（TX）配置为 reTerminal E 系列专用
...
:::note
reTerminal E 系列仅支持容量最高为 64GB 且使用 **Fat32** 文件系统格式化的 MicroSD 卡。
:::
...
所有其他 SD 卡引脚（DET、CS、MOSI、MISO、SCK）在各型号之间都相同。请选择与你的设备对应的选项卡，并将代码复制到 Arduino IDE 草图中。

<Tabs>
<TabItem value="e1001_e1002_e1004" label="E1001 / E1002 / E1004" default>

<details>
<summary>点击展开完整 SD 卡示例代码</summary>
...
这个综合示例结合了前面章节的功能。我们将编写一个程序，从 MicroSD 卡中读取一个 Bitmap（`.bmp`）图像文件，并将其显示在 reTerminal 的电子纸屏幕上。这展示了该设备的一个实用的、真实世界的应用场景。
...
准备一张 MicroSD 卡（建议 64GB 或更小），并使用 **FAT32** 文件系统对其进行格式化。
...
黑白屏只能显示黑白像素。虽然我们的代码可以在实时处理中将彩色图像转换为灰度图像，但如果你**在电脑上预先将图像转换为高质量的灰度图像**，将会获得更好的对比度和细节效果。

1. **调整图像尺寸：** 将你的图片调整为 **800x480 像素**。
...
彩色屏可以显示 6 种颜色：黑、白、红、黄、蓝和绿。所提供的代码包含一个“最近颜色”算法，可以智能地将源图像中的任意颜色映射为屏幕上最合适的可用颜色。为了获得最佳效果，请按照以下步骤操作：

1. **调整图像尺寸：** 使用任意图像编辑器，将你的图片调整为 **800x480 像素**。

2. **保存为标准 BMP：** 代码被设计为读取**未压缩的** 24 位或 32 位 BMP 文件。使用专业的图像编辑器是确保格式正确的最佳方式。我们推荐免费开源软件 **GIMP**：
    - 在 GIMP 中打开你已调整尺寸的图像。
    - 打开菜单 **File > Export As...**。
...
下面是最终验证过的代码。它包含所有必要的检查以及高级颜色匹配算法。只需将 `EPD_SELECT` 宏设置为 `0`（适用于 E1001（黑白））或 `1`（适用于 E1002（彩色））。
...
- **颜色处理**：这里的逻辑会根据 `EPD_SELECT` 宏进行分支：
  - **彩色模式（E1002）**：调用 `findNearestColor(r, g, b)`。该函数计算像素颜色与屏幕调色板中 6 种颜色之间的“距离”。它返回距离最小的调色板颜色，以确保尽可能准确的颜色呈现。
  - **黑白模式（E1001）**：使用标准亮度公式（`r * 0.299 + g * 0.587 + b * 0.114`）将 RGB 颜色转换为单一亮度值。如果该值低于阈值（128），则将像素绘制为黑色；否则绘制为白色。
...
如果你在 Arduino IDE 配置、USB 驱动、程序上传失败，或“电子纸显示屏不刷新”等方面遇到问题，请参阅 [Arduino Cookbook: ePaper Display](/cn/reterminal_e10xx_with_arduino#故障排查) 中的 **Troubleshooting** 部分。
