...
:::tip 首先阅读 ESPHome 主指南
本页是**针对 reTerminal E 系列的 ESPHome 显示菜谱**。通用的样板内容——选择烧录路径、通用 YAML 骨架、连接 Home Assistant——都在 **[使用 ESPHome](/cn/epaper_work_with_esphome)** 中。如果你是第一次在 Seeed 电子纸上使用 ESPHome，请先快速浏览那一页。关于按键、蜂鸣器、LED、电池、SHT4x 和深度睡眠，请参阅 [I/O、电池和低功耗菜谱](/cn/reterminal_e10xx_with_esphome_advanced)。关于 RTC、microSD 卡检测和麦克风设置，请参阅 [RTC、SD 卡和麦克风菜谱](/cn/reterminal_e10xx_with_esphome_rtc_sd_microphone)。
:::
...
这些优势使 reTerminal E 系列电子纸显示屏成为为 Home Assistant 搭建节能、常亮信息显示屏的理想选择。
...
首先，你需要点击 **Manual download** 下载编译好的固件。
...
:::tip
这是最简单的方法，但前提是首次安装程序时，应先使用左侧的方法将程序上传到 ePaper 面板。之后，你就可以通过 wifi 上传了。同时，请确保你的 YAML 配置中包含正确配置的 `ota` 和 `api` 部分，并带有有效的加密密钥，否则此方法无法工作。
:::
...
要在 reTerminal E 系列 ePaper 显示设备上显示天气数据，你需要从 Home Assistant 获取温度、天气状况和风速数据。Open-Meteo 集成提供了可靠的天气数据，可以通过开发者工具访问。
...
步骤 2. 在过滤框中输入 `weather` 来找到主要的天气实体。
...
要在 reTerminal E 系列的 ESPHome 配置中使用这些天气数据，你需要在 ESPHome 的 YAML 配置中设置 Home Assistant API 连接：
...
此配置会在你的 ESPHome 设备中创建传感器实体，从 Home Assistant 的天气集成中获取数据。然后你可以使用这些传感器来更新 reTerminal E 系列 ePaper Display 显示屏上的当前天气信息。
...
在编译并将上述代码上传到你的设备后，你可能会先在屏幕上看到 **NaN** 显示，请不要担心，这是正常现象。这是因为设备尚未添加到 Home Assistant 环境中，所以 reTerminal 还无法获取 Home Assistant 的数据。我们只需要按照下面的步骤添加设备即可。

#### 将 reTerminal E 系列 ePaper Display 添加到 Home Assistant

步骤 1. 烧录设备完成后，返回 Home Assistant 并导航到 **Settings → Devices & Services**。

步骤 3. Home Assistant 应该会通过 mDNS 自动发现你的 reTerminal E 系列 ePaper Display 设备。如果它出现在已发现设备部分，点击 Configure 将其添加。
...
步骤 3. 找到喜欢的图标后，点击它查看详细信息。查找 Unicode 值，其格式类似 `F0595`。
...
:::tip
对于天气仪表板，可以考虑使用 `F0590`（晴天）、`F0591`（多云）、`F0593`（雨天）和 `F059E`（有风）等图标。
:::

通过将这些图标与我们之前设置的来自 Home Assistant 的天气数据结合起来，你可以创建一个动态天气显示，用合适的图标展示当前天气状况。

### 显示自定义图片

本示例演示如何在 reTerminal E 系列 ePaper Display 电子纸显示屏上显示自定义图片。你可以使用此功能显示 Logo、图标或任何能增强仪表板体验的图形。
...
步骤 5. 在 ESPHome 配置文件的 `captive_portal` 部分之后添加以下代码。此代码定义图片资源并配置显示以显示该图片。
...
通过将图片与文本以及前面示例中介绍的其他显示元素结合起来，你可以在 reTerminal E 系列上创建丰富且信息量大的仪表板。

## 继续阅读

本文重点介绍连接显示屏并在 ePaper 屏幕上绘制内容。当你想使用其余板载硬件时，请继续阅读以下 ESPHome 使用示例：

- **[ESPHome 使用示例：按键、蜂鸣器、LED、电池与低功耗](/cn/reterminal_e10xx_with_esphome_advanced)** - 用户按键、蜂鸣器反馈、板载 LED、电池监测、SHT4x 传感器、深度睡眠和多页面仪表板。
...
在这种情况下，你应该前往 Settings -> Devices & Services -> Integrations 来 **RECONGFIGURE** 设备。还没有找到你的 reTerminal？尝试重启 Home Assistant。
...
reTerminal E 系列在 UART0 上使用 CH340K USB 转 UART 桥接芯片。请在你的 YAML 中保持以下日志记录器设置：
