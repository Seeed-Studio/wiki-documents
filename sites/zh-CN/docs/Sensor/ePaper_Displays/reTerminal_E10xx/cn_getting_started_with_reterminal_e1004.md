...
找到包装中附带的金属支架。该支架通过 reTerminal E1004 背面的螺纹孔进行固定——你可以将其垂直安装以获得纵向方向，或旋转 90° 安装到另一组螺纹孔以获得横向方向。使用附带的螺丝刀和螺丝固定支架。安装完成后，将 reTerminal E1004 放置在平整的表面上，支架即可支撑其保持所选方向。
...
reTerminal E1004 随机附带预装的 16GB microSD 卡，因此在大多数情况下你无需自行插卡。如果你想更换为其他容量的卡或取出该卡，请参考下方视频获取操作指引。
...
2. 当设备处于关机或睡眠状态时，无法正确烧录固件。如果你选择了设备的正确端口，但始终看不到固件烧录进度，可能需要按下设备上的 **Refresh** 按钮唤醒设备，然后重试。
...
SenseCraft HMI 拥有独立的 Wiki，提供完整的教程和功能文档。有关更深入的指南（账户设置、固件烧录、高级编辑器使用、部署等），请参考：
...
刷新按钮要么位于设备正面下方最左侧，要么位于设备背面如上图所示的位置。它具有以下几个功能：

- **单击**：手动刷新显示并从 SenseCraft 平台检查新内容。蜂鸣器会响一声以确认操作。该按钮也常用于唤醒设备。当设备进入睡眠状态且通常无法立即接收到仪表盘刷新的指令时，你可以使用此按钮唤醒设备。
