---
description: reTerminal E1004 是一款 13.3 英寸开源全彩电子纸显示屏，由 ESP32-S3 驱动，采用 E Ink® Spectra™ 6 技术，并支持 SenseCraft Seeedash。
...
reTerminal E1004 是一款 13.3 英寸开源全彩电子纸显示屏，电池续航最长可达 6 个月。其由 ESP32-S3 驱动，原生支持我们的 SenseCraft Seeedash 零代码 UI 平台，可轻松创建仪表盘，同时也支持通过 [ESPHome](/cn/reterminal_e10xx_with_esphome)、Arduino 和 ESP-IDF 进行进一步开发。凭借 E Ink® Spectra™ 6 全彩电子纸显示屏，它非常适合作为数码相框、多彩仪表盘可视化使用。E1004 显示驱动需要 ESPHome **2026.7.0 或更高版本**。

### 特性

- 高分辨率、美观且开箱即用的 E Ink® Spectra™ 6 彩色电子纸显示屏
- 基于 SenseCraft Seeedash 的零代码 UI 设计与部署
- 支持图片幻灯片播放与实时数据同步，电池可续航数月
- 适应不同场景下的装饰与信息展示需求
...
2. **触摸按键：** 位于前面框的电容式触摸按键，用于屏幕刷新和页面导航（上一页 / 下一页）。
...
10. **扩展接口：** 2 × 4 针排针，提供 VDD、GND、UART、I2C、GPIO 和 ADC 连接，用于外接传感器、模块或外设。
11. **支架安装螺纹孔：** 背面的螺纹孔，用于将设备固定在支架上。

## 快速上手

reTerminal E1004 预装了支持 **SenseCraft Seeedash** 的固件，这是一个零代码平台，可让你轻松设计并部署个性化仪表盘。
...
找到包装中附带的金属支架。该支架通过 reTerminal E1004 背面的螺纹孔进行固定——你可以将其垂直安装以获得纵向放置，或旋转 90° 安装到另一组螺纹孔以获得横向放置。使用随附的螺丝刀和螺丝固定支架。安装完成后，将 reTerminal E1004 放置在平整的表面上，支架即可支撑其保持所选方向。
...
reTerminal E1004 内置了一张 16GB microSD 卡，因此在大多数情况下你无需自行插卡。如果你想更换为其他容量的卡，或取出该卡，请参考下方视频中的操作说明。
...
:::note
reTerminal E 系列电子纸显示屏仅支持容量不超过 32GB 且格式为 Fat32 的 MicroSD 卡。  
32GB SD 卡默认格式为 exFAT。将其格式化为 FAT32 后，即可成功挂载并用于存储图片。
:::

**步骤 4.**（可选）如有需要，安装 USB 驱动：

根据你的操作系统，你可能需要安装 USB 驱动，以确保与 reTerminal E1004 正常通信：
...
**步骤 3.** 绿色用户指示灯会点亮约 3 秒，表示设备已上电并正在初始化。若设备 30 秒内无操作，为了节省电量，设备会自动进入睡眠模式，指示灯也会自动熄灭。
...
<Tabs>
<TabItem value="Network Setup via PC" label="通过 PC 进行网络配置" default>

**步骤 1.** 使用手机或电脑连接设备的 Wi-Fi 热点。AP 名称会显示在屏幕上（无需密码）。网络凭据格式为 `reTerminal E1004-{MAC Address}`。

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/5.png" style={{width:400, height:'auto'}}/></div><br />

**步骤 2.** 连接成功后，手机应会自动跳转到 Wi-Fi 配置页面。如果没有，请打开浏览器并访问 `192.168.4.1`。

<div style={{textAlign:'center'}}><img src="https://files.seeedstudio.com/wiki/reterminal_e10xx/img/6.png" style={{width:700, height:'auto'}}/></div><br />

**步骤 3.** 选择本地 Wi-Fi 网络并输入密码，然后点击 "Connect"。
...
**步骤 1.** 在浏览器中访问 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi)，并创建账号或登录。
...
<TabItem value="Network Setup via SenseCraft APP" label="通过 SenseCraft APP 进行网络配置">

此方法使用 SenseCraft 手机应用，通过蓝牙为设备配置网络并将其添加到你的 SenseCraft 账号中。

首先下载 SenseCraft App。你可以在 Google Play 商店或 Apple App Store 中搜索 "SenseCraft" 获取。或者，也可以从以下网站下载。
...
**步骤 1.** 打开 SenseCraft App，登录你的账号，进入 **User** 选项卡。点击 **Device Bluetooth Configuration** 开始配置流程。
...
**步骤 2.** 在 "Please select the device type" 界面，选择 **reTerminal E1004**。
...
**步骤 3.** 按照屏幕提示将设备置于蓝牙广播模式，并确保手机蓝牙已开启。点击 **Scan**，应用会搜索附近设备。从列表中选择你的设备进行连接。
...
**步骤 5.** 应用会将 Wi-Fi 凭据发送到设备，并将其添加到你的 SenseCraft 账号中。完成后会显示 "Device added successfully" 消息。现在你可以点击 **Start exploring!** 继续。
...
**步骤 7.** 你可以点击列表中的设备进入其 **Device Detail** 页面。在这里可以管理设备并上传照片。对于更复杂的仪表盘和画布设计，系统会提示你使用 SenseCraft 的网页版。
...
1. **选择你的设备** — 通过 USB-C 线将 reTerminal E1004 连接到电脑，确保电源开关为 **ON**，然后点击 **Select** 并在列表中选择你的设备。
...
2. 当设备处于关机或睡眠状态时，无法正确烧录固件。如果你选择了正确的设备端口却始终看不到固件烧录进度，可能需要按下设备上的 **Refresh** 按钮唤醒设备后再重试。
...
设备配对完成后，你就可以开始用它来显示你想要的内容。我们推荐使用 **SenseCraft Seeedash** —— 一个零代码云平台，只需几次点击即可为 reTerminal E1004 设计并部署内容。

SenseCraft Seeedash 拥有独立的 Wiki，提供完整的教程和功能文档。有关详细指南（账号设置、固件烧录、高级编辑器使用、部署等），请参考：

- **[SenseCraft Seeedash Wiki](https://sensecraft-hmi-docs.seeed.cc/en/)**

由于本文是产品**入门指南**，我们不会在此重复相同内容——下面仅演示如何在 reTerminal E1004 上快速显示一些内容。

### Home —— 从社区模板开始

最快在屏幕上显示内容的方法是从现成模板开始。在 SenseCraft Seeedash 的 **Home** 页面，你可以找到一个庞大且持续增长的模板集合，由社区贡献——仪表盘、时钟、天气面板、语录墙、日历、相框等。浏览图库，选择你喜欢的设计，一键部署到 reTerminal E1004 上。
...
设计保存后，点击 **Deploy to Device**，选择已配对的 reTerminal E1004，内容就会通过无线方式传输到设备上，电子纸屏幕会更新以显示你的设计。

:::note
由于 E Ink 技术的特性，屏幕刷新可能需要几秒钟才能完全更新图像并清除残影。
:::

关于其他内容——编辑器的详细操作、技巧和版本说明——请参考 [SenseCraft Seeedash Wiki](https://sensecraft-hmi-docs.seeed.cc/en/)。
...
刷新按钮位于设备前面下方最左侧的按钮位置，或者位于设备背面上图所示的位置。它具有以下几个功能：

- **单击**：手动刷新显示屏并从 SenseCraft 平台检查新内容。蜂鸣器会鸣叫一次以确认操作。此按钮也常用于唤醒设备。当设备进入睡眠状态且通常无法立即接收到仪表盘刷新命令时，你可以使用此按钮唤醒设备。


### 导航按钮

如果你的仪表盘包含多个页面，左右按钮可以让你在多个页面之间导航：
...
- 电池寿命取决于刷新频率（在低功耗模式下，刷新间隔为 6 小时时可持续使用长达 6 个月）
...
reTerminal E1004 配备了一个 8 针扩展排针（J2），可用于连接外部传感器、模块或其他硬件，以扩展设备的功能。该扩展排针引出了多个 ESP32-S3 GPIO 引脚和通信接口，使其成为 DIY 项目中非常灵活的连接点。
...
扩展连接器（J2）是一个 2×4 排针（料号 **ST-FH-254-0148-2×4P**），提供两列共 8 个引脚。下面的引脚位置标记使用左列 `Lx` 和右列 `Rx`（从上到下编号，如上图所示）：
...
- 如果问题仍然存在，尝试重启设备
- 如果设备在重启后仍无响应，请在 SenseCraft Seeedash 平台上重新烧录相应固件，并检查设备是否可以正常刷新

### Q4：网络连接丢失

- 设备会自动尝试重新连接到已知网络
- 重新连接后，Wi-Fi 断线图标会消失
- 如果仍无法重新连接，请按照上面的网络重置步骤操作
