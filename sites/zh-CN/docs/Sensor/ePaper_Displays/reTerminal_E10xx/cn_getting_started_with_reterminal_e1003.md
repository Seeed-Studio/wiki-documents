...
reTerminal E1003 是一款支持触控的 10.3 英寸开源黑白电子纸显示屏，具备 16 级灰度和 1404×1872 像素的高分辨率，在每天刷新一次的情况下，电池续航最长可达 6 个月。其由 ESP32-S3 提供算力，原生支持我们的 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 零代码 UI 平台，可轻松创建仪表盘。无论是智能家居仪表盘可视化与控制、办公信息显示，还是教育项目，这款开箱即用、支持触控的设备都能提供出色的显示效果和灵活的定制能力，满足你的各种需求。

:::tip
reTerminal E1003 的触控交互自 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 固件 v1.1.2 起开始支持。目前最新版本为 v1.1.4.3 —— 我们建议升级到该版本以获得最佳体验。
与触控相关的库支持计划在未来开源发布，以提供更高的面板定制灵活性。更多更新将很快与大家分享。
...
2. **按键：** 位于设备顶部 —— **绿色按键** 用于手动刷新屏幕；**两个白色按键** 用于上下翻页，在页面之间导航
...
找到包装中附带的 3D 打印支撑支架，将支架对准 reTerminal E1003 背面带有嵌入螺母的指定安装区域。使用螺丝刀将螺丝拧入背面的嵌入螺母中，将支架牢固固定在设备上。安装完成后，将 reTerminal E1003 放置在平整的桌面上，支架即可将其支撑为竖直状态。
...
:::tip
如果 macOS 阻止安装，请前往 **System Settings → Privacy & Security**，向下滚动找到被拦截的 WCH 软件，并点击 **Allow**。
:::
...
**步骤 3. 连接并验证**
确保设备的**电源开关处于 ON 状态**，并使用**数据 USB-C 线缆**（非仅充电线）。打开终端并运行：
```bash
ls /dev/tty.wch*
...
**步骤 3.** 绿色状态指示灯会亮起约 3 秒钟然后熄灭，这表示设备已上电并正在初始化。如果在 30 秒内没有进行任何操作，设备会自动进入睡眠模式以节省电量。
...
**步骤 2.** 连接成功后，你的手机应会自动跳转到 Wi-Fi 配置页面。如果没有自动跳转，你可以使用手机扫描 reTerminal E1003 屏幕上显示的二维码，或手动在浏览器中访问 `192.168.4.1`。
...
**步骤 4.** 通过蓝牙连接成功后，应用会提示你进行 Wi-Fi 配置。从下拉列表中选择本地 2.4GHz Wi-Fi 网络，输入密码，然后点击 **Next**。
...
当设备成功添加到你的 SenseCraft 账号后，平台会自动检查固件版本。如果有更新的固件可用，你将会收到更新提示。
...
1. **选择你的设备** — 使用 USB-C 线缆将 reTerminal E1003 连接到电脑，确保电源开关处于 **ON** 状态，然后点击 **Select** 并在列表中选择你的设备。

2. **选择固件** — 在下拉菜单中选择最新的固件版本。

3. **烧录** — 点击 **Flash** 并等待更新完成。仅当你希望将设备恢复到出厂状态时才勾选 **Full Flash**（所有数据、设置和设计将被清除）。

:::note

1. 更新固件可以确保最佳性能并获取最新功能。reTerminal E1003 的触控交互需要固件版本 **v1.1.2 或更高**。当前最新版本为 v1.1.4.3 —— 我们建议更新到该版本以获得最佳体验。

2. 当设备处于关机或睡眠状态时，无法正常烧录固件。如果你选择了正确的设备端口却始终看不到固件烧录进度，可能需要按下机身顶部的 **Refresh** 按钮唤醒设备，然后重试。
...
[SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 拥有独立的 Wiki，提供完整的教程和功能文档。关于更深入的指南（账号设置、固件刷写、高级编辑器使用、部署等），请参考：

- **[SenseCraft HMI Wiki](https://sensecraft-hmi-docs.seeed.cc/en/)**

由于本文是产品的**入门指南（Getting Started）**，我们不会在这里重复相同的内容——下面只展示如何在你的 reTerminal E1003 上快速显示一些内容。

### Home —— 从社区模板开始

在屏幕上显示内容的最快方式是从一个现成的模板开始。在 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 的 **Home** 页面，你会发现一个由社区贡献的大型且持续增长的模板集合——仪表盘、时钟、天气面板、语录墙、日历、相框等。浏览图库，选择你喜欢的任意设计，一键部署到你的 reTerminal E1003 上。
...
设计保存后，点击 **Deploy to Device**，选择与你配对的 reTerminal E1003，内容就会通过无线方式传输到设备上。电子墨水屏会更新以显示你的设计。触控交互需要固件 **v1.1.2 或更高版本**——当前最新版本为 **v1.1.4.3**，我们建议更新到该版本以获得最佳体验。
...
设备顶部的刷新按钮（绿色）具有以下几个功能：

- **单击**：手动刷新显示并从 SenseCraft 平台检查新内容。蜂鸣器会响一声以确认操作。当设备进入休眠且仪表盘无法立即刷新时，这个按钮也常用于唤醒设备。

- **长按**（未来功能）：将激活语音输入模式。

### 导航按钮

设备顶部的两个白色翻页按钮可让你在仪表盘包含多个页面时在页面之间进行导航：
...
reTerminal E1003 配备了一个 6 针扩展排针（J2），为添加外部传感器、模块或其他硬件提供连接选项，从而扩展设备的功能。该扩展排针引出了多个 ESP32-S3 GPIO 引脚和通信接口，使其成为你的 DIY 项目中一个多功能的连接点。
...
- 如果问题仍然存在，尝试重启设备
- 如果设备在重启后仍无响应，请在 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 平台上重新刷写相应固件，并检查是否恢复正常刷新
