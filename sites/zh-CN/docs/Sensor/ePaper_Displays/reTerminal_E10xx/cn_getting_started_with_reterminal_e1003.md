...
11. **3D 打印支架：** 随机附带的支架配件，用于将设备竖立放置在平面上。
...
找到包装中附带的 3D 打印支架，将支架对准 reTerminal E1003 背面带有嵌入螺母的指定安装区域。使用螺丝刀将螺丝拧入 reTerminal E1003 背面的嵌入螺母中，将支架牢固固定在设备上。安装完成后，将 reTerminal E1003 放置在平整的表面上，支架即可支撑其保持竖直状态。
...
**步骤 1.** 将电源开关滑动到 **ON** 位置为设备上电。电源开关位于机身背面。
...
**步骤 2.** 连接成功后，你的手机应会自动跳转到 Wi-Fi 配置页面。如果没有自动跳转，你可以使用手机扫描 reTerminal E1003 屏幕上显示的二维码，或手动打开浏览器并访问 `192.168.4.1`。
...
**步骤 5.** 应用会将 Wi-Fi 凭证发送到设备，并将其添加到你的 SenseCraft 账号中。完成后会出现 “Device added successfully” 提示信息。现在你可以点击 **Start exploring!** 继续操作。
...
**步骤 7.** 你可以点击列表中的设备进入其 **Device Detail** 页面。在这里，你可以管理设备并上传图片。对于更复杂的仪表盘和画布设计，系统会提示你使用 SenseCraft 的网页版。
...
设备配对完成后，你就可以开始使用它来显示你想要的内容。我们推荐使用 **[SenseCraft HMI](https://sensecraft.seeed.cc/hmi)** —— 一个零代码云平台，只需几次点击即可为你的 reTerminal E1003 设计并部署内容。
...
在屏幕上快速显示内容的最快方式，是从一个现成的模板开始。在 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 的 **Home** 页面，你可以找到由社区贡献的大量且持续增长的模板集合——仪表盘、时钟、天气面板、语录墙、日历、相框等。浏览图库，选择你喜欢的任意设计，一键部署到你的 reTerminal E1003 上。
...
设计保存后，点击 **Deploy to Device**，选择与你配对的 reTerminal E1003，内容就会通过无线方式传输到设备上。电子纸显示屏会更新以显示你的设计。触摸交互需要固件版本 **v1.1.2 或更高**——当前最新版本为 **v1.1.4.3**，我们建议更新到该版本以获得最佳体验。
...
- **单击**：手动刷新显示并从 SenseCraft 平台检查新内容。蜂鸣器会鸣叫一次以确认操作。当设备进入休眠且仪表盘无法立即刷新时，此按钮也常用于唤醒设备。
