---
description: reTerminal E1003 是一款 10.3 英寸开源黑白电子纸显示屏，具备 16 级灰度和 1404×1872 像素的高分辨率，并在 Seeedash 固件 v1.1.2 上支持触控交互。
...
reTerminal E1003 是一款支持触控的 10.3 英寸开源黑白电子纸显示屏，具备 16 级灰度和 1404×1872 像素的高分辨率，在每天刷新一次的情况下，电池续航最长可达 6 个月。其由 ESP32-S3 驱动，原生支持我们的 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 零代码 UI 平台，可轻松创建仪表盘。无论是智能家居仪表盘可视化与控制、办公信息展示，还是教育项目，这款开箱即用的触控设备都能提供出色的显示效果和灵活的定制能力，满足你的各种需求。

:::tip
reTerminal E1003 的触控交互自 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 固件 v1.1.2 起开始支持。目前最新版本为 v1.1.4.3 —— 我们建议升级到该版本以获得最佳体验。
与触控相关的库支持计划在未来开源发布，以便更灵活地自定义面板。更多更新将很快与大家分享。

reTerminal E1003 支持搭配 Home Assistant 使用 [ESPHome](/cn/reterminal_e10xx_with_esphome)。你可以先从 [显示 cookbook](/cn/reterminal_e10xx_with_esphome) 开始，然后继续阅读 [I/O、电池、触控和低功耗](/cn/reterminal_e10xx_with_esphome_advanced) 以及 [RTC、SD 卡和麦克风](/cn/reterminal_e10xx_with_esphome_rtc_sd_microphone)。E1003 显示驱动需要 **ESPHome 2026.7.0 或更高版本**。
:::

### 特性

- 3 秒全屏刷新，实现响应更快的电子纸仪表盘
- 借助 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 和触控支持，轻松构建交互界面
...
- **低功耗黑白电子相框：** 借助 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 或 App，你可以轻松远程上传相册中的一系列照片。配合大尺寸、高分辨率的黑白电子墨水屏，实现超低功耗的图像显示，让你不再担心电池续航。
- **智能办公显示屏：** 以低功耗、类“永远在线”的电子纸显示会议室占用情况、办公日程或指引信息。
...
3. **麦克风：** 载板 PDM 数字麦克风，用于语音交互应用
4. **MicroSD 卡 / TF 卡槽：** 用于扩展存储（FAT32，最大 64 GB）
5. **电源开关：** 用于打开或关闭设备。
6. **红色电源指示灯：** 指示电池充电状态。
7. **绿色状态指示灯：** 指示当前运行或系统状态。
8. **USB-C 数据与充电接口：** 用于给设备充电和进行数据传输。
9. **扩展接口：** 排针提供 VDD、GND、I2C 和 GPIO 连接，可接入外部传感器、模块或外设。
10. **支架安装螺丝孔：** 位于设备背面的螺纹孔，用于固定支撑支架。

## 快速上手

reTerminal E1003 预装了支持 **[SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi)** 的固件，这是一个零代码平台，可让你轻松设计和部署个性化仪表盘。
...
**步骤 2.**（可选）安装 3D 打印支撑支架，使设备竖直放置：

找到包装中附带的 3D 打印支撑支架，将其对准 reTerminal E1003 背面带有嵌入螺母的指定安装区域。使用螺丝刀将螺丝拧入背面的嵌入螺母中，将支架牢固固定在设备上。安装完成后，将 reTerminal E1003 放置在平整的表面上，支架即可支撑其保持竖直姿态。

:::note
该支架提供固定的观看角度，无法调节。此固定角度旨在在大多数使用场景下提供最佳可视性。
:::

**步骤 3.**（可选）如果你计划将设备用作电子相框或需要额外存储空间，请插入 microSD 卡。

:::note
reTerminal E1003 支持容量最高为 **64 GB**、格式为 FAT32 的 microSD 卡。  
大于 32 GB 的卡通常默认被格式化为 exFAT。将其重新格式化为 FAT32 后，即可挂载并用于存储图片。
:::
...
**步骤 3. 连接并验证**
确保设备的**电源开关为 ON**，并使用**数据 USB-C 线缆**（非仅充电线）。打开 Terminal 并运行：
```bash
ls /dev/tty.wch*
...
**步骤 2.** 连接成功后，你的手机应会自动跳转到 Wi-Fi 配置页面。如果没有自动跳转，你可以用手机扫描 reTerminal E1003 屏幕上显示的二维码，或在浏览器中手动访问 `192.168.4.1`。
...
**步骤 1.** 在浏览器中访问 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi)，创建账号或登录。
...
**步骤 4.** 配对成功后，设备会显示一条提示信息，引导你创建第一个仪表盘。
...
此方法通过 SenseCraft 手机应用使用蓝牙为设备配置网络，并将其添加到你的 SenseCraft 账号中。

首先下载 SenseCraft App。你可以在 Google Play Store 或 Apple App Store 中搜索 “SenseCraft” 获取。或者，也可以从以下网站下载。
...
**步骤 4.** 通过蓝牙连接成功后，应用会提示你进行 Wi-Fi 配置。从下拉列表中选择本地的 2.4GHz Wi-Fi 网络，输入密码，然后点击 **Next**。
...
1. **选择你的设备** — 通过 USB-C 线缆将 reTerminal E1003 连接到电脑，确保电源开关为 **ON**，然后点击 **Select** 并在列表中选择你的设备。

2. **选择固件** — 在下拉菜单中选择最新的固件版本。

3. **烧录** — 点击 **Flash** 并等待更新完成。仅当你希望将设备恢复到出厂状态时才勾选 **Full Flash**（所有数据、设置和设计都会被清除）。
...
3. **执行标准 Flash** 以保留你的设置：Wi-Fi、已部署的设计和图片都会被保留。**执行完整 Flash 以全新开始**：所有数据、设置和设计都会被清除（恢复出厂状态）。

:::

## 在你的 reTerminal E1003 上显示内容

设备配对完成后，你就可以开始用它来显示你想要的任何内容。我们推荐使用 **[SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi)** —— 一个零代码云平台，只需几次点击即可为你的 reTerminal E1003 设计并部署内容。

[SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 拥有独立的 Wiki，提供完整的教程和功能文档。关于更深入的指南（账号设置、固件烧录、高级编辑器使用、部署等），请参考：

- **[SenseCraft Seeedash Wiki](https://sensecraft-hmi-docs.seeed.cc/en/)**

由于本文是产品的**入门指南**，我们不会在此重复相同的内容——下面仅展示如何在你的 reTerminal E1003 上快速显示一些内容。

### Home — 从社区模板开始

在屏幕上显示内容的最快方式是从一个现成的模板开始。在 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 的 **Home** 页面，你会发现一个由社区贡献的大量且持续增长的模板集合——仪表盘、时钟、天气面板、语录墙、日历、相框等。浏览图库，选择你喜欢的任意设计，一键部署到你的 reTerminal E1003 上。
...
设计保存后，点击 **Deploy to Device**，选择与你配对的 reTerminal E1003，内容就会通过无线方式传输到设备上。电子纸显示屏会更新以显示你的设计。触控交互需要固件版本 **v1.1.2 或更高**——当前最新版本为 **v1.1.4.3**，我们建议更新到该版本以获得最佳体验。

关于其他所有内容——编辑器的详细操作、技巧和发行说明——请参考 [SenseCraft Seeedash Wiki](https://sensecraft-hmi-docs.seeed.cc/en/)。
...
- **单击**：手动刷新显示并从 SenseCraft 平台检查新内容。蜂鸣器会响一声以确认操作。当设备进入睡眠且仪表盘无法立即刷新时，此按钮也常用于唤醒设备。

- **长按**（未来功能）：将激活语音输入模式。

### 导航按钮

设备顶部的两个白色翻页按钮可让你在仪表盘包含多个页面时在页面之间导航：
...
:::tip
如果你想自己写一些代码来读取电池电压，在 `analogRead()` 函数前增加 10ms 延时会更精确。
:::
...
reTerminal E1003 配备了一个 6 针扩展排针（J2），为添加外部传感器、模块或其他硬件以扩展设备功能提供了连接选项。该扩展排针引出了多个 ESP32-S3 GPIO 引脚和通信接口，使其成为你的 DIY 项目中一个多用途的连接点。
...
- 如果问题仍然存在，尝试重启设备
- 如果设备在重启后仍无响应，请在 [SenseCraft Seeedash](https://sensecraft.seeed.cc/hmi) 平台上重新烧录相应固件，并检查是否恢复正常刷新
...
- [reTerminal E1003 外壳 3D 模型 (STP)](https://files.seeedstudio.com/wiki/reterminal_e10xx/res/reTerminal_E1003_3D.stp)
