...
reTerminal E1003 是一款 10.3 英寸支持触控的开源黑白电子纸显示屏，具备 16 级灰度和 1404×1872 像素的高分辨率，在每天刷新一次的情况下，电池续航最长可达 6 个月。其由 ESP32-S3 提供算力，原生支持我们的零代码 UI 平台 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi)，可轻松创建仪表盘。无论是智能家居仪表盘可视化与控制、办公信息看板，还是教育项目，这款开箱即用的触控设备都能提供出色的显示效果和灵活的定制能力，满足你的各种需求。

:::tip
reTerminal E1003 的触控交互自 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 固件 v1.1.2 起开始支持。目前最新版本为 v1.1.4.3 —— 我们建议升级到该版本以获得最佳体验。
触控相关的库支持计划在未来开源发布，以提供更高的面板定制灵活性。更多更新将很快与大家分享。

reTerminal E1003 支持搭配 Home Assistant 使用的 [ESPHome](/cn/reterminal_e10xx_with_esphome)。你可以先从 [display cookbook](/cn/reterminal_e10xx_with_esphome) 入手，然后继续阅读 [I/O、电池、触控和低功耗](/cn/reterminal_e10xx_with_esphome_advanced) 以及 [RTC、SD 卡和麦克风](/cn/reterminal_e10xx_with_esphome_rtc_sd_microphone)。E1003 显示驱动需要 **ESPHome 2026.7.0 或更高版本**。
:::
...
找到包装中附带的 3D 打印支架，将支架对准 reTerminal E1003 背面带有嵌入螺母的指定安装区域。使用螺丝刀将螺丝拧入 reTerminal E1003 背面的嵌入螺母中，将支架牢固固定在设备上。安装完成后，将 reTerminal E1003 放置在平整的表面上，支架即可支撑其保持竖直姿态。
...
**步骤 3.** 绿色状态指示灯会亮起约 3 秒后熄灭，这表示设备已通电并正在初始化。如果在 30 秒内没有进行任何操作，设备会自动进入睡眠模式以节省电量。
...
**步骤 2.** 连接成功后，你的手机应会自动跳转到 Wi-Fi 配置页面。如果没有跳转，你可以使用手机扫描 reTerminal E1003 屏幕上显示的二维码，或在浏览器中手动访问 `192.168.4.1`。
...
**步骤 4.** 配对成功后，设备会显示提示信息，引导你创建第一个仪表盘。
...
此方法使用 SenseCraft 手机应用，通过 Bluetooth 配置设备网络并将其添加到你的 SenseCraft 账号中。

首先下载 SenseCraft App。你可以在 Google Play Store 或 Apple App Store 中搜索 “SenseCraft” 获取。或者，也可以从本网站下载。
...
**步骤 3.** 按照屏幕提示将设备置于 Bluetooth 广播模式。确保手机的 Bluetooth 已开启。点击 **Scan**，应用会搜索附近的设备。从列表中选择你的设备进行连接。
...
**步骤 4.** 通过 Bluetooth 连接成功后，应用会提示你进行 Wi-Fi 设置。从下拉列表中选择本地 2.4GHz Wi-Fi 网络，输入密码，然后点击 **Next**。
...
当设备成功添加到你的 SenseCraft 账号后，平台会自动检查固件版本。如果有更新的固件可用，你将收到更新提示。

你也可以随时手动检查固件版本：

**步骤 1.** 在 **Device** 页面找到你的 reTerminal E1003 查看其详细信息，并记下**当前固件版本**。
...
1. **选择你的设备** — 使用 USB-C 线缆将 reTerminal E1003 连接到电脑，确保电源开关为 **ON**，然后点击 **Select** 并在列表中选择你的设备。

2. **选择固件** — 在下拉菜单中选择最新的固件版本。

3. **烧录** — 点击 **Flash** 并等待更新完成。仅当你希望将设备恢复到出厂状态时才启用 **Full Flash**（所有数据、设置和设计将被清除）。

:::note

1. 更新固件可确保最佳性能并获取最新功能。reTerminal E1003 的触控交互需要固件 **v1.1.2 或更高版本**。当前最新版本为 v1.1.4.3 —— 我们建议更新到该版本以获得最佳体验。

2. 当设备处于关机或睡眠状态时，无法正确烧录固件。如果你为设备选择了正确的端口，但始终看不到固件烧录进度，你可能需要按下机身顶部的 **Refresh** 按钮唤醒设备，然后重试。

3. **执行标准 Flash** 可保留你的设置：Wi-Fi、已部署的设计和图片都会被保留。**执行完整 Flash 以全新开始**：所有数据、设置和设计都会被清除（恢复出厂状态）。

:::

## 在你的 reTerminal E1003 上显示内容

设备配对完成后，你就可以开始用它来显示你想要的任何内容。我们推荐 **[SenseCraft HMI](https://sensecraft.seeed.cc/hmi)** —— 一个零代码云平台，只需几次点击就能为你的 reTerminal E1003 设计并部署内容。
...
由于本文是产品的**入门指南**，我们不会在这里重复相同的内容——下面只展示如何在你的 reTerminal E1003 上快速显示一些内容。

### Home — 从社区模板开始

在屏幕上显示内容的最快方式是从一个现成的模板开始。在 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 的 **Home** 页面，你会发现一个由社区贡献的大量且持续增长的模板集合——仪表盘、时钟、天气面板、语录墙、日历、相框等。浏览图库，选择你喜欢的任意设计，一键部署到你的 reTerminal E1003 上。
...
设计保存后，点击 **Deploy to Device**，选择已配对的 reTerminal E1003，内容就会通过无线方式传输到设备上。电子纸显示屏会更新以显示你的设计。触控交互需要固件版本 **v1.1.2 或更高**——当前最新版本为 **v1.1.4.3**，我们建议更新到该版本以获得最佳体验。

关于其他所有内容——详细的编辑器操作、技巧和版本说明——请参考 [SenseCraft HMI Wiki](https://sensecraft-hmi-docs.seeed.cc/en/)。
...
- **单击**：手动刷新显示并从 SenseCraft 平台检查新内容。蜂鸣器会鸣叫一次以确认操作。当设备进入睡眠且仪表盘无法立即刷新时，这个按钮也常用于唤醒设备。

- **长按**（未来功能）：将激活语音输入模式。

### 导航按钮

设备顶部的两个白色翻页按钮可用于在多个页面之间导航，如果你的仪表盘包含多于一页：
...
reTerminal E1003 配备了一个 6 针扩展排针（J2），为添加外部传感器、模块或其他硬件提供连接选项，以扩展设备功能。该扩展排针引出了多个 ESP32-S3 GPIO 引脚和通信接口，使其成为你的 DIY 项目中一个多用途的连接点。
...
- 如果问题仍然存在，尝试重启设备
- 如果设备在重启后仍无响应，请在 [SenseCraft HMI](https://sensecraft.seeed.cc/hmi) 平台上重新烧录对应固件，并检查是否恢复正常刷新
