...
# 在 Arduino 中开始使用 reTerminal E 系列电子纸显示屏
...
reTerminal E 系列代表了 Seeed Studio 在工业 HMI 解决方案方面的最新进展，以 ESP32-S3 作为主控制器并集成电子纸显示屏。本指南将带你通过 Arduino IDE 对 reTerminal E 系列设备上的电子纸显示屏进行编程，使你能够创建具有出色可视性和超低功耗的自定义界面和应用。
...
要使用 Arduino 对 reTerminal E 系列电子纸显示屏进行编程，你需要在 Arduino IDE 中配置 ESP32 支持。
...
**reTerminal E1001 配备 7.5 英寸黑白电子纸显示屏**，而 **reTerminal E1002 则配备 7.3 英寸全彩电子纸显示屏**。这两款显示屏在各种光照条件下都具有出色的可视性和超低功耗，非常适合需要始终显示且功耗极低的工业应用。
...
**步骤 2.** 在 Arduino IDE 中通过添加 ZIP 文件来安装该库。进入 **Sketch > Include Library > Add .ZIP Library**，然后选择下载好的 ZIP 文件。
...
**步骤 4.** 访问 [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/)，并在设备列表中选择 **reTerminal E1001**。
...
**步骤 5.** 复制生成的配置代码并将其粘贴到 `driver.h` 文件中。代码应类似如下所示：

```cpp
#define BOARD_SCREEN_COMBO 520 // reTerminal E1001 (UC8179)
```

**步骤 6.** 将草图上传到 reTerminal E1001。你应该会看到显示屏展示包括线条、文本和形状在内的各种图形，以演示基本绘图功能。
...
全彩电子纸显示屏支持红色、黑色和白色三种颜色，可实现更加丰富的可视化界面。
...
**步骤 3.** 在与你的草图相同的文件夹中创建一个名为 `driver.h` 的新文件，操作方式与之前相同。
...
**步骤 4.** 访问 [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/)，并在设备列表中选择 **reTerminal E1002**。
...
**步骤 5.** 复制生成的配置代码，并将其粘贴到 `driver.h` 文件中。代码应如下所示：

```cpp
#define BOARD_SCREEN_COMBO 521 // reTerminal E1002 (UC8179C)
```

**步骤 6.** 将示例程序上传到你的 reTerminal E1002。显示屏将展示丰富多彩的图形，以演示电子墨水屏的全彩显示能力。
...
#### 编程 reTerminal E1003（10.3 英寸电子墨水屏）

按照相同的工作流程，使用 Seeed_GFX 库来配置并驱动 reTerminal E1003 上的电子墨水屏。
...
**步骤 4.** 访问 [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/)，并在设备列表中选择 **reTerminal E1003**。
...
**步骤 5.** 复制生成的配置代码，并将其粘贴到 E1003 的 `driver.h` 文件中。

```cpp
#define BOARD_SCREEN_COMBO 522 // reTerminal E1003 (ED103TC2)
```

**步骤 6.** 将示例程序上传到你的 reTerminal E1003，以验证图形基本元素绘制、文本渲染以及全屏刷新行为。
...
#### 编程 reTerminal E1004（13.3 英寸全彩电子墨水屏）

使用 Seeed_GFX 库来配置并驱动 reTerminal E1004 上的 E Ink® Spectra™ 6 全彩电子墨水显示屏。
...
**步骤 4.** 访问 [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/)，并在设备列表中选择 **reTerminal E1004**。
...
**步骤 5.** 复制生成的配置代码，并将其粘贴到 E1004 的 `driver.h` 文件中。

```cpp
#define BOARD_SCREEN_COMBO 523 // reTerminal E1004 (T133A01)
```

**步骤 6.** 将示例程序上传到你的 reTerminal E1004，以验证颜色渲染、图形基本元素绘制、文本渲染以及全屏刷新行为。
...
除了 Seeed_GFX，你也可以使用 `GxEPD2` 库来驱动 reTerminal 的电子墨水屏显示。`GxEPD2` 是一个功能强大且广泛使用的库，支持多种电子墨水屏。
...
**步骤 3.** `GxEPD2` 库需要 `Adafruit GFX Library` 才能运行，你也必须安装它。最简单的方式是通过库管理器安装：进入 **Tools > Manage Libraries...**，搜索 "Adafruit GFX Library"，然后点击 "Install"。

:::note
虽然为了方便，`GxEPD2` 也可以通过 Arduino Library Manager 获取，但其中的版本往往比较陈旧。GitHub 仓库才是最新版本的权威来源，其中包含最新功能、错误修复以及对最新电子墨水屏的支持。因此，直接从 GitHub 下载该库是推荐的方式，以确保你使用的是最新的代码。
:::
...
下面是一个示例代码，使用 `GxEPD2` 库在 reTerminal E1001 的黑白电子墨水屏上显示 "Hello World!"。将 `EPD_SELECT` 设置为 `0` 以选择 E1001 的驱动。
...
对于 reTerminal E1002，你只需要将 `EPD_SELECT` 的值改为 `1`。这将选择适用于 7.3 英寸全彩电子墨水屏的驱动程序。其余代码保持不变。
...
reTerminal E 系列带有一个板载 LED，可通过 GPIO6 控制。请注意，该 LED 的逻辑是反向的（LOW = 亮，HIGH = 灭）。
...
reTerminal E 系列配备了三个用户可编程按键，可用于多种控制用途。本节演示如何读取按键状态，并使用 Arduino 对按键按下做出响应。
...
4. **去抖动**：在每次按键后加入简单的 200ms 延时，可防止由于机械抖动导致一次按键被多次检测。

5. **串口输出**：每次按键都会向串口监视器发送一条消息，用于调试和验证。
...
这个综合示例结合了前面章节的功能。我们将编写一个程序，从 MicroSD 卡中读取一个 Bitmap（`.bmp`）图像文件，并将其显示在 reTerminal 的电子纸屏幕上。这展示了该设备的一个实用的真实应用场景。

程序会在 SD 卡根目录中查找名为 `test.bmp` 的文件。
...
黑白屏只能显示黑白像素。虽然我们的代码可以实时将彩色图像转换为灰度图像，但如果你在电脑上**预先将图像转换为高质量灰度图像**，将会获得更好的对比度和细节。
...
下面是最终验证过的代码。它包含所有必要的检查以及高级颜色匹配算法。只需将 `EPD_SELECT` 宏设置为 `0`（适用于 E1001 黑白屏）或 `1`（适用于 E1002 彩色屏）。
...
### Q1：为什么在运行上述代码时，reTerminal 的电子纸显示屏没有任何显示或不刷新？

如果你在 reTerminal 中插入了 MicroSD 卡，可能会出现此问题。原因是 MicroSD 卡和电子纸显示屏在 reTerminal 上共享同一条 SPI 总线。如果插入了 MicroSD 卡，但其使能（片选）引脚未被正确管理，就会在 SPI 总线上产生冲突。具体来说，MicroSD 卡可能会将 BUSY 线保持为高电平，从而阻止电子纸显示屏正常工作——导致没有显示更新或刷新。
...
为了解决这个问题，你必须确保使用上面提供的代码正确启用 MicroSD 卡。该代码通过设置正确的引脚状态来初始化并启用 MicroSD 卡，从而防止 SPI 总线冲突，并使 SD 卡和 ePaper 显示屏能够同时工作。在 reTerminal 上使用 MicroSD 卡时，请务必使用推荐的初始化代码，以避免此类问题。
