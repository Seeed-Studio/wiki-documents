---
description: 本文介绍如何在 ESPHome 上利用 reTerminal E 系列电子墨水屏的硬件功能。
title: ESPHome 高级用法
...
# 在 Home Assistant 中高级使用 reTerminal E 系列电子墨水屏的 ESPHome 配置
...
本文基于我们在[ESPHome 基础用法指南](https://wiki.seeedstudio.com/cn/reterminal_e10xx_with_esphome)中介绍的基础概念，进一步探索适用于 reTerminal E 系列电子墨水屏设备的高级 ESPHome 配置。如果你是 ESPHome 或 reTerminal E 系列的新手，建议先阅读基础指南，再来尝试这些高级应用。

## 硬件功能

reTerminal E 系列电子墨水屏包含多个硬件组件，可以通过 ESPHome 在 Home Assistant 中加以利用：
...
下面我们来探索如何在 Home Assistant 中使用 reTerminal E 系列电子墨水屏上的各个硬件组件。

### 按键和 LED

本示例演示如何使用 reTerminal E 系列电子墨水屏上的三个按键来控制功能，并通过板载 LED 提供视觉反馈。

你可以将下面的代码复制并粘贴到 Yaml 文件中 `captive_portal` 代码行之后来使用此示例。
...
- 将三个按键全部设置为带上拉电阻的输入
- 配置按键 1 使其短暂闪烁 LED
- 配置按键 2 使其以双闪模式闪烁 LED
- 配置按键 3 使其切换 LED 的开/关状态
...
reTerminal E 系列电子墨水屏可以通过 GPIO1 上的模拟输入监测电池电量。以下是设置方法：
...
步骤 2. 在插件页面中，点击 "INSTALL" 按钮并等待安装完成。
...
步骤 5. 创建一个具有描述性名称的新令牌，例如 "Puppet Screenshot"，并复制生成的令牌。
...
步骤 6. 返回 Puppet 插件配置，并将令牌粘贴到 "Long-Lived Access Token" 字段中。
...
Puppet 插件在端口 10000 上启动一个服务器，用于生成任意 Home Assistant 页面截图。使用方法如下：
...
在网页浏览器中输入你的截图 URL 进行测试。你应该会看到所选 Home Assistant 页面的截图。
...
:::caution
如果在你的网络中本地域名解析不起作用，请将 `homeassistant.local` 替换为你的 Home Assistant 实际 IP 地址。
:::
...
当你的配置成功上传并运行后，你的 reTerminal E 系列电子纸显示屏将显示一个包含环境数据、时间和电池状态的综合仪表板：
