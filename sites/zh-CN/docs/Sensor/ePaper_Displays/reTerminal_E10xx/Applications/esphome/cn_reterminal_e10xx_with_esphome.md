---
description: 本文介绍如何让 reTerminal E 系列电子纸显示屏与 Home Assistant 和 ESPHome 协同工作。
title: ESPHome 基本用法
...
# reTerminal E 系列电子纸显示屏在 Home Assistant 中使用 ESPHome
...
### 为什么在 Home Assistant 中使用 reTerminal E 系列电子纸显示屏？

reTerminal E 系列电子纸显示屏是 Home Assistant 的绝佳搭档，原因如下：

1. **能效高**：电子纸显示屏只在更新内容时消耗电能，非常适合显示天气预报、日历事件或系统状态等持久信息。

2. **清晰易读**：与 LCD 屏不同，电子纸显示屏在任何光照条件下都易于阅读，包括直射阳光，非常适合作为壁挂式家庭控制面板。
...
ESPHome 是一个专为 ESP8266/ESP32 设备设计的开源固件生成工具。它允许你使用简单的 YAML 配置文件创建自定义固件，然后将其烧录到设备上。对于 reTerminal E 系列而言，ESPHome 是实现设备与 Home Assistant 通信的关键中间件。
...
:::tip
如果你在附加组件商店中找不到 ESPHome，请确保你使用的是支持附加组件的 Home Assistant 安装方式（例如 Home Assistant OS 或 supervised 安装）。对于其他安装类型（如 Home Assistant Container），你可能需要使用 Docker 独立运行 ESPHome Device Builder。更多详情请参阅 [ESPHome 官方文档](https://esphome.io/guides/getting_started_hassio)。
:::
...
为设备起一个你喜欢的名字，并将芯片类型选择为 **ESP32-S3**，然后点击 **SKIP**。
...
:::tip
如果你的 Home Assistant 主机（Raspberry PI/Green/Yellow 等）离你比较远，推荐使用这种方式。你可以使用手边的电脑进行安装。
:::
...
按照下图所示选项点击，将代码安装到设备上。 [设备处于深度睡眠模式时没有找到端口？](#port)
...
:::tip
这是最简单的方式，但前提是首次安装程序时，需要先使用左侧的方法将程序上传到 ePaper 面板。之后就可以通过 Wi-Fi 上传。同时，请确保你的 YAML 配置中包含正确配置并带有有效加密密钥的 `ota` 和 `api` 部分，以保证此方法可以正常工作。
:::
...
由于篇幅所限，我们不会对其他图案的绘制方法和原理做过多展开，如有需要，建议读者阅读 [ESPHome 在这一部分的详细示例](https://esphome.io/components/display/)。
...
步骤 1. 打开 Home Assistant 仪表盘并导航到 **Settings** → **Devices & Services**。
...
要在 reTerminal E 系列的 ESPHome 配置中使用这些天气数据，你需要在 ESPHome 的 YAML 配置中设置一个 Home Assistant API 连接：
...
此配置会在你的 ESPHome 设备中创建传感器实体，从 Home Assistant 的天气集成中获取数据。然后你可以使用这些传感器来更新 reTerminal E 系列电子墨水屏的当前天气信息显示。
...
#### 将 reTerminal E 系列电子墨水屏添加到 Home Assistant

步骤 1. 在烧录设备完成后，返回 Home Assistant，导航到 **Settings → Devices & Services**。

步骤 3. Home Assistant 应该会通过 mDNS 自动发现你的 reTerminal E 系列电子墨水屏设备。如果它出现在已发现设备部分，点击 Configure 将其添加。
...
步骤 5. 输入 reTerminal E 系列电子墨水屏设备的 IP 地址，以及如果你设置了的话，输入 API 加密密钥。

步骤 6. 连接成功后，你的 reTerminal E 系列电子墨水屏将作为一个设备出现在 Home Assistant 中，其所有传感器和组件都可以使用。
...
本示例演示如何在 reTerminal E 系列电子墨水屏上使用 TrueType 字体显示自定义图标。Material Design Icons 提供了大量可缩放的符号，非常适合电子墨水屏显示。

#### 安装所需工具

步骤 1. 首先，我们需要安装 Studio Code Server 插件来管理文件。进入 Home Assistant 的 Add-ons 商店，搜索 **Studio Code Server** 并点击它。
...
步骤 6. 在 ESPHome 配置文件的 `captive_portal` 部分之后添加以下代码。此代码定义了两种图标字体大小，并配置显示屏以显示天气图标。
...
步骤 4. 通过以下方式将 Unicode 值添加到你的 ESPHome 配置中：
...
通过将这些图标与我们之前在 Home Assistant 中设置的天气数据结合，你可以创建一个动态天气显示，用合适的图标展示当前天气状况。
...
:::tip
如果你想显示复杂且精美的风景图片，最好提前在[我们的网站](https://sensecraft.seeed.cc/hmi/tools/dither)上进行图像抖动处理，以获得更好的效果。JPG 和 PNG 格式都支持。
:::

#### 为图片显示配置 ESPHome

步骤 5. 将以下代码添加到 ESPHome 配置文件中 `captive_portal` 部分之后。此代码定义图片资源并配置显示以展示该图片。
...
步骤 6. 保存你的配置并将其上传到 reTerminal E 系列。当更新完成后，你的电子纸显示屏将显示该图片。
...
由于篇幅限制，本文只涵盖了该设备的一些基础用例和绘图示例。我们将在 [Advanced ESPHome Usage of reTerminal E Series ePaper Display in Home Assistant](https://wiki.seeedstudio.com/cn/reterminal_e10xx_with_esphome_advanced) 的 Wiki 中更详细地介绍在 ESPHome 上使用 reTerminal 硬件的内容，你可以继续阅读。
...
### Q3：reTerminal E Series ePaper Display 无法连接到你的电脑？
