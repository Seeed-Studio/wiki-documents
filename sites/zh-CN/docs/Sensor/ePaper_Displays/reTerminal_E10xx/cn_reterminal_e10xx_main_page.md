...
打开 **[reTerminal E-Series Firmware Hub](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/)** 即可快速上手。选择 **ESPHome** 卡片，挑选你的设备（E1001 / E1002 / E1003 / E1004），勾选所需的板载功能，生成匹配的 ESPHome YAML —— 或者直接在浏览器中烧录演示固件（桌面版 Chrome 或 Edge）。
...
reTerminal E 系列是 Seeed Studio 基于 ESP32-S3 的电子纸显示终端产品家族，专为**始终在线、低功耗信息显示**而打造。该系列包含四款型号 —— **E1001、E1002、E1003、E1004** —— 覆盖 7.3" 到 13.3" 的黑白和全彩面板，可选触控交互和长达数月的电池续航。它们共享同一套软件栈（SenseCraft Seeedash、Home Assistant / ESPHome、Arduino 等），你只需学习一次，即可按场景选择合适的外形尺寸。
...
四款型号，一个生态系统。可按以下维度选择：**屏幕尺寸**、**彩色或黑白**、**触控**以及**电池续航**：
...
<div class="table-center">
	<table align="center">
		...
		<tr>
			...
			<td align="center">—</td>
			<td align="center">—</td>
			<td align="center">✅ 电容触控<br/>(SenseCraft Seeedash v1.1.2+)</td>
			<td align="center">—</td>
		</tr>
		<tr>
...
使用下面的快速指南来缩小到一个型号。如果有多行都符合，你通常可以选择**最后一行匹配的那款**。
...
<div class="table-center">
	<table align="center">
		...
		<tr>
			...
			<td>7.5" 单色屏，支持 4 级灰度，局部刷新，2–5 秒刷新率。</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>10.3" 16 级灰度，支持电容触控（Seeedash v1.1.2+），电池续航可达 6 个月。</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td align="center"><strong>E1001 / E1002 / E1003 / E1004</strong></td>
			<td>四款型号均已支持。E1002 和 E1004 配备彩色面板，但目前在 TRMNL 下以单色模式运行。</td>
		</tr>
		<tr>
			<td>电池续航是唯一最重要的因素</td>
...
reTerminal E1002 采用 7.3 英寸高亮全彩电子纸显示屏，搭载 ACeP（Advanced Color ePaper）技术。这块彩色屏幕在保留电子纸技术低功耗优势的同时，支持多种颜色，能够呈现更具吸引力的视觉内容，非常适合需要颜色区分和美观呈现的应用场景。
...
reTerminal E1003 是一款 10.3 英寸开源单色电子纸显示屏，具备 16 级灰度和 1404×1872 像素高分辨率，电池续航最长可达 6 个月。目前在 SenseCraft Seeedash 固件 v1.1.2 上已支持触控交互。其由 ESP32-S3 驱动，原生支持 SenseCraft Seeedash 零代码 UI 设计，同时也支持 Arduino、PlatformIO 和 ESPHome（Home Assistant）进行进一步开发。
...
- 10.3 英寸单色电子纸显示屏，在 Seeedash 固件 v1.1.2 上支持触控交互
- 16 级灰度，1404×1872 高分辨率
- 超低功耗运行，电池续航最长可达 6 个月
- 原生支持 SenseCraft Seeedash，并兼容 Arduino、PlatformIO 和 ESPHome（Home Assistant）
...
reTerminal E1004 配备 13.3 英寸全彩电子纸显示屏，电池续航最长可达 6 个月。由 ESP32-S3 驱动，它支持使用 SenseCraft Seeedash 进行零代码仪表盘创建和图像上传，同时还通过 ESPHome 支持 Home Assistant，并兼容 Arduino 和 ESP-IDF 等高级开发流程。E1004 显示驱动需要 ESPHome **2026.7.0 或更高版本**——请参阅 [ESPHome cookbooks](/cn/reterminal_e10xx_with_esphome)。

### 主要特性

- 13.3 英寸全彩电子纸显示屏
- 在始终在线场景下电池续航最长可达 6 个月
- ESP32-S3 平台，支持 SenseCraft Seeedash 零代码开发
- 兼容 Home Assistant / ESPHome、Arduino 和 ESP-IDF

## SenseCraft Seeedash — 默认零代码平台
...
每一台 reTerminal E 系列设备出厂时都预装了 **SenseCraft Seeedash** 固件，因此你无需编写任何代码即可快速上手。Web 应用负责登录、仪表盘设计以及将内容空中部署到你的设备上：
...
- **Touch interaction** —— 从 Seeedash 固件 v1.1.2 起在 E1003 上得到完整支持。

如果你需要比零代码流程更高的控制能力，每台设备也都开放了底层 ESP32-S3，你可以随时切换到 ESPHome / Home Assistant、Arduino，以及下文的应用教程。若需要现成的 ESPHome YAML，可使用 [Firmware Hub](https://seeed-projects.github.io/OSHW-reTerminal-Series-E-D/) 中的 ESPHome 卡片，并勾选你所需的功能。

## 应用与教程

除了 SenseCraft Seeedash 之外，reTerminal E 系列还集成了多种行业标准平台和可视化 UI 设计工具。下表将告诉你每个教程当前支持哪些型号。
...
<div class="table-center">
	<table align="center">
		...
		<tr>
			...
			<td>按钮、蜂鸣器、板载 LED、电池监控、深度睡眠、多页面仪表盘以及电容触摸（E1003）。</td>
			<td align="center">✅</td>
			<td align="center">✅</td>
			<td align="center">✅</td>
			<td align="center">✅</td>
...
<div class="table-center">
	<table align="center">
		...
		<tr>
			...
			<td>RTC 时间管理、深度睡眠 / 轻度睡眠、I2S 麦克风录音以及电容触摸绘图（麦克风和触摸仅 E1003 支持）。</td>
			<td align="center">✅</td>
			<td align="center">✅</td>
			<td align="center">✅</td>
			<td align="center">✅</td>
...
:::tip
在寻找针对每款设备的入门说明（开箱、Wi-Fi 设置、固件更新）？请跳转到上表中链接的产品 Wiki：
...
- [SenseCraft Seeedash 文档](https://wiki.seeedstudio.com/cn/sensecraft_hmi_overview/)
...
注意：本设备已经过测试，符合 FCC 规则第 15 部分中针对 B 类数字设备的限制。这些限制旨在为住宅安装环境中防止有害干扰提供合理保护。本设备会产生、使用并能辐射射频能量，如果未按说明书安装和使用，可能会对无线电通信造成有害干扰。然而，不能保证在某一特定安装中不会发生干扰。如果本设备确实对无线电或电视接收造成有害干扰（可通过关闭和开启设备来判断），建议用户尝试通过以下一种或多种措施纠正干扰：
