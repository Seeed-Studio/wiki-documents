---
description: 本文介绍如何使用 TRMNL 7.5inch(OG) DIY Kit 搭配 Arduino。
title: 适配 Arduino 的使用方法
keywords:
  - ePaper 显示屏
  - TRMNL
...
# 在 Arduino 中开始使用 TRMNL 7.5" (OG) DIY Kit

## 介绍

TRMNL 7.5" (OG) DIY Kit 是一款多功能开发平台，其核心是强大的 XIAO ESP32-S3 Plus。该 DIY Kit 将 ESP32-S3 的处理能力与精美的 7.5 英寸 ePaper 显示屏相结合，为低功耗信息显示项目提供了完美基础。本文档重点介绍如何使用 Arduino 框架对 TRMNL DIY Kit 进行编程，为你提供使用其各类硬件功能所需的基础知识。

## 入门

在深入了解具体功能之前，我们先为 TRMNL 7.5" (OG) DIY Kit 搭建开发环境。

### 设备安装

**步骤 1. 将显示屏连接到驱动板**  
将 FPC 线缆与 XIAO ePaper Display Board 上的连接器对齐，然后扣上卡扣以确保连接牢固。  
...
:::
从[资源部分](#resources)打印开源外壳部件，并将组件装入其中。  
...
要使用 Arduino 对 TRMNL DIY Kit 进行编程，你需要在 Arduino IDE 中添加 ESP32 支持。
...
**步骤 5.** 使用 USB-C 线将 TRMNL DIY Kit 连接到电脑。
...
TRMNL DIY Kit 提供了三个用户可编程按键（D1、D2、D4）以及一个复位按键。我们先创建一个简单示例来检测按键按下。

#### 按键测试示例

本示例将读取三个用户按键的状态，并将其状态打印到串口监视器。
...
1. 我们为三个用户按键（D1、D2、D4）定义引脚常量。

2. 在 `setup()` 函数中，我们初始化串口通信，并将按键引脚配置为带内部上拉电阻的输入模式。

3. 在 `loop()` 函数中，我们读取每个按键的状态。由于使用了上拉电阻，当按键被按下时，引脚读取为 LOW。

4. 我们对读数进行取反（使用 `!` 运算符），这样 `true` 表示“按下”，`false` 表示“松开”。

5. 如果任意按键被按下，我们就将所有按键的状态打印到串口监视器。

6. 一个小的延时可以防止在按键长按时出现过于频繁的重复读取。
...
TRMNL 7.5" (OG) DIY Kit 集成了一路用于监测电池电压的电路，该电路连接到 D0（GPIO1）引脚。此设计的一个重要特性是还包含一个控制引脚（GPIO6），用于启用/禁用 ADC 供电，从而在不进行测量时帮助节省电量。
...
- `ADC_EN_PIN` (GPIO6/A5): 控制 ADC 测量电路的供电
...
- 通过将 ADC_EN_PIN 设为 HIGH 来启用 ADC 电路
...
此实现的一个关键特性是在不使用时可以禁用电池测量电路。TRMNL DIY Kit 旨在实现低功耗，这种方法通过仅在需要实际测量时才为分压电路供电，从而有助于延长电池寿命。

校准系数（0.968）用于补偿分压电路中元器件公差，帮助确保读数准确。如果你发现与万用表测量相比读数总是有偏差，可能需要针对你的特定板子对该数值进行微调。

进行多次读数并取平均值有助于降低噪声，提供更稳定的电压测量结果，这对电池监测尤为重要，因为微小的电压变化在判断剩余容量时可能具有重要意义。

## 电子纸显示屏

TRMNL 7.5" (OG) DIY Kit 配备了一块精美的 7.5 英寸黑白电子纸显示屏，在各种光照条件下都具有良好的可视性，并且功耗极低。本节中，我们将介绍如何使用 Arduino 来设置和控制电子纸显示屏。
...
**步骤 2.** 在 Arduino IDE 中通过添加 ZIP 文件来安装该库。依次进入 **Sketch > Include Library > Add .ZIP Library**，然后选择下载的 ZIP 文件。

:::note
如果你之前安装过 TFT_eSPI 库，可能需要暂时从 Arduino 库文件夹中移除或重命名它以避免冲突，因为 Seeed_GFX 是在 TFT_eSPI 基础上增加功能的分支。
:::

**步骤 3.** 从 Seeed_GFX 库中打开示例程序：**File > Seeed_GFX > Examples > ePaper > Basic > Clock**

**步骤 4.** 在与 Arduino 示例草图相同的文件夹中创建一个名为 `driver.h` 的新文件，方法是点击示例标签旁边的箭头并选择 "New Tab"。
...
**步骤 6.** 访问 [Seeed GFX Configuration Tool](https://seeed-studio.github.io/Seeed_GFX/)，并按照图示选择 "TRMNL 7.5" (OG) DIY Kit"。
...
**步骤 7.** 复制生成的代码并将其粘贴到你创建的 `driver.h` 文件中。代码应类似如下：
...
Step 8. 现在你可以将该示例上传到 TRMNL DIY Kit，并查看电子纸显示屏的实际效果！
...
下面我们来看看一个简单的模拟时钟示例，演示如何在电子纸显示屏上绘制图形。[该示例](https://github.com/Seeed-Studio/Seeed_GFX/blob/master/examples/ePaper/Basic/Clock/Clock.ino) 会创建一个带有时针、分针和秒针的经典表盘。
...
3. **`loop` 函数：**
   ...
   - 我们根据当前时间计算每个指针的角度。
   - 使用三角函数计算指针的位置：
     - 时针：每小时 30 度（再加上对分钟的调整）
     - 分针：每分钟 6 度（再加上对秒的调整）
     - 秒针：每秒 6 度
   - 为了减少不必要的更新并延长显示屏的使用寿命，我们只在每分钟一次（当秒 = 0 时）擦除并重绘时针和分针。
   - 秒针每秒更新一次，通过擦除其先前位置并在新位置重新绘制。
...
在绘图操作之后，请记得调用 `display.update()` 来更新物理显示屏。
