...
**EE02** 是一款专为 **13.3 英寸** 电子纸显示屏设计的高性能电子墨水屏驱动板。其搭载 ESP32-S3 芯片，支持 Wi-Fi 和 Bluetooth 连接，非常适合无线显示类应用场景。
...
- **多样化接口**：包含用于供电/烧录的 Type-C 接口以及标准电子墨水屏连接器。
...
- **复位按钮**：用于重置开发板。
- **按键**：分别为 "refresh the page"、"switch previous page" 和 "switch next page"。
- **拨动开关**：控制电池电源的开/关。
...
**步骤 5. 获取配对码**
网络配置成功后，屏幕会再次刷新并显示一个唯一的 **Pairing Code**（或 Device ID）。你将在下一步中用到此代码。
...
完成后，依次点击 **Tools** -> **Board** -> **XIAO ESP32S3 Plus** 和 **Tools** -> **Port** -> **Select the port your board is connected to**。 
...
然后点击 **Upload** 上传代码。
现在你将在电子墨水屏上看到反馈！下面是 Bitmap 示例的显示效果。
...
- **[PDF]** [Seeed Studio XIAO ePaper Display EE02 原理图](https://files.seeedstudio.com/wiki/Epaper/EE02/202000224_XIAO_ePaper_Display_Board_EE02_V1.pdf)
