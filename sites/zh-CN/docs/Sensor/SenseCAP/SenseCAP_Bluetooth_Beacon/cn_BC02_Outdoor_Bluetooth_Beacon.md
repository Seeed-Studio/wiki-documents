...
- **Bluetooth® LE 5.0**
- **电池寿命长**：可更换锂电池，在默认配置下可使用超过 7 年。
- **远距离传输**：在空旷区域最远可达 75 米。
- **坚固耐用**：IP68 防水等级和 IK08 抗冲击等级，可应对恶劣环境

## 部署指南

- 蓝牙 Beacon 通常安装在距离地面 2.5–3m 的高度，水平间距为 5–10m。根据环境不同，这些 Beacon 可以按列部署，或均匀分布成三角形阵列。
...
- 不同场景的部署策略有所差异。例如，室内环境可以采用居中（密集）布置或三角网格分布（稀疏）。在室内走廊中，约 3m 宽的走廊可采用中线布置，更宽的走廊可采用双列布置。高层建筑中（层高 > 4m）应避免安装在天花板上，改为安装在墙面或较低位置，间距约 5m。室外开阔区域则建议采用三角网格布置。
...
- 安装产品时应与金属、玻璃或其他遮挡物保持一定距离，不应靠近拐角位置。

:::tip
上述部署指南仅供参考。实际部署应根据客户的定位算法、产品安装环境以及测试条件进行定制化设计。
:::
...
:::tip
由于带磁吸的产品体积和重量通常大于普通产品（无磁吸），建议不要将此类产品安装在天花板等表面，以避免掉落风险。
:::
...
- 产品安装的平台材质表面应为平整、干燥的表面，如陶瓷、玻璃/环氧树脂、亚克力、PBT、ABS、PC 和硬质 PVC 等。由于不同型号双面胶的粘性和可安装性存在差异，不建议在表面有灰纹、未完全干燥、老化、潮湿的墙面（如水泥墙、石膏板等）上使用默认双面胶，否则存在脱落风险；
- 推荐安装温度范围：20~40℃；
...
:::tip
如果需要在低温环境或恶劣环境中安装产品，建议采用双面胶 + 强力免钉胶的方式进行安装，以获得最佳效果。关于免钉胶的更多信息，请联系我司销售人员。
:::
