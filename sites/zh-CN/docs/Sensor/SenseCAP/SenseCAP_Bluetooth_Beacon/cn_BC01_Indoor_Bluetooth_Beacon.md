...
BLE（低功耗蓝牙）定位 Beacon 是一种小型无线设备，会以固定时间间隔发射蓝牙信号。这些信号可以被支持蓝牙的设备检测到，例如 [**SenseCAP T1000 Tracker**](https://www.seeedstudio.com/SenseCAP-Card-Tracker-T1000-A-p-5697.html)、[**SenseCAP T2000 
...
GPS 已经证明了其在室外定位方面的能力。现在，我们也倾向于转向室内定位，而蓝牙 Beacon 让室内精确定位成为可能。与 SenseCAP T1000 Tracker 结合即可构建室内定位解决方案。

在目标区域内部署蓝牙 Beacon，Tracker 接收 Beacon 发出的信号，定位精度可达米级（2-3 米）。可以实现资产追踪、路线规划、反向寻车等功能，并可集成到小程序和 APP 中。
...
- **Bluetooth® LE 5.0**
- **电池寿命长**：可更换锂电池，默认配置下（0dBm/500ms）续航超过 5 年。
...
|开机|关机|
| :-: | :-: |
|打开产品后盖，长按按键 3 秒，蓝色 LED 常亮 5 秒，则表示开机成功。|在开机状态下，打开产品后盖，长按按键 3 秒，蓝色 LED 闪烁 5 次，表示关机成功。|

## 部署指南

- 蓝牙 Beacon 通常安装在距离地面 2.5-3m 的高度，水平间距为 5-10m。根据环境不同，可以采用列式部署或在空间内均匀分布成三角形网格。
...
- 不同场景的部署策略有所差异。例如，室内环境可以采用中心（密集）布置或三角网格（稀疏）分布。在室内走廊中，约 3m 宽的走廊可采用中线布置，更宽的走廊可采用双列布置。高层建筑中（层高 > 4m）应避免安装在天花板上，而应选择墙面或较低位置安装，间距约 5m。室外开阔区域则建议采用三角网格布置。
...
- 安装产品时应与金属、玻璃或其他遮挡物保持一定距离，不应靠近拐角。

:::tip
部署指南仅供参考。实际部署应根据客户的定位算法、产品安装环境以及测试条件进行定制化设计。
:::
...
- 推荐安装温度范围：20~40°C；
- 安装产品时，尽量远离拐角、其他周围磁场以及大型障碍物。

#### 安装步骤

将产品的磁性接触面直接吸附在需要安装的支架或设备表面即可。

:::tip
由于带磁产品的体积和重量通常大于普通产品（无磁吸），建议不要将此类产品安装在天花板等表面，以避免掉落风险。
:::
...
- 产品安装的平台材质表面应为平整、干燥的表面，如陶瓷、玻璃/环氧树脂、亚克力、PBT、ABS、PC 和硬质 PVC 等。由于不同型号双面胶的粘性和可安装性存在差异，不建议在有灰质纹理、未完全干燥、老化、潮湿的墙面（如水泥墙、石膏板等）上使用默认双面胶，否则存在脱落风险；
- 推荐安装温度范围：20~40℃；
- 产品应远离金属、玻璃屏蔽或其他遮挡物安装；产品不应部署在靠近拐角的位置。

#### 安装步骤

- 安装前：清洁需要粘贴的应用平台或工具表面，确保粘贴表面干燥且无灰尘；
- 粘贴时，用手或工具将双面胶贴在设备上并按压 1-2 秒，多次重复按压，使产品的双面胶或免钉胶与应用粘贴表面更好地粘合。

:::tip
如果需要在低温环境或恶劣环境中安装产品，建议使用双面胶 + 强力免钉胶的方式安装，以获得最佳效果。关于免钉胶的更多信息，请联系我司销售人员。
:::
...
- **步骤 2**：连接 Beacon，可以通过 MAC 地址进行区分，MAC 地址也贴在 Beacon 标签上。
...
根据需要修改参数并点击 `Save`。出现提示时，输入密码 `seeed123` 进行确认。
