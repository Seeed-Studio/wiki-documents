...
在本地运行 TinyML AI 模型，仅通过 LoRaWAN®（863-928MHz）传输推理结果，从而提升隐私保护并提高能效。其工作分辨率为 480×480、帧率为 10 FPS，内置三种预部署模型，用于人体检测、人数统计和仪表读数；同时还可通过 [SenseCraft AI](https://sensecraft.seeed.cc/ai/#/home) 平台以零代码方式访问 300+ 预训练模型或进行自定义模型训练。它还支持从 [TensorFlow Lite](https://www.tensorflow.org/) 和 [PyTorch](https://pytorch.org/) 框架部署模型。用户可以通过 [SenseCraft App](https://sensecap-mate-download.seeed.cn/) 配置设备和频段，并在恶劣的室内或室外环境中部署具备 IP66 防护等级的传感器。其内置电池，非常适合需要远距离通信的各类 AI 应用场景。

## 特性

- 高级边缘 AI 相机：配备强大的基于 MCU 的视觉相机处理器，可在 480×480 分辨率下以 10 FPS 进行本地推理。通过仅传输推理结果，确保数据隐私并最大限度减少网络带宽占用。

- 零代码 AI 模型部署与训练：预置三种本地 AI 模型（人体检测、人数统计和仪表读数），开箱即用实现视觉应用。原生集成基于 Web 的 SenseCraft AI 平台，提供 300+ 预训练模型可立即部署，并为分类和检测模型提供简单的 3 步训练流程。这使得无需编程经验即可快速完成模型训练与部署。此外，设备还支持将 TensorFlow 和 PyTorch 框架转换而来的自定义 AI 模型进行边缘部署。
...
- SenseCraft Cloud 集成：配套 SenseCraft Mate 移动应用和 Web 门户，便于设备配置、设备管理和快速数据可视化。具备开放 API 支持和 3 个月免费试用权限，并兼容第三方 IoT 平台。
...
LoRaWAN®（Long Range Wide Area Network，长距离广域网）是一种无线通信协议，用于在 IoT（物联网）设备与网关之间实现低功耗、长距离通信。它使用工业、科学和医疗（ISM）频段中的免授权无线频谱，通常在欧洲为 868 MHz，在美国为 915 MHz。LoRaWAN® 为远距离连接 IoT 设备提供了一种低成本、高能效的解决方案。该技术支持设备与网关之间的双向通信，并支持多种数据速率，以适配不同类型的应用。
...
由于这里我们主要关注模型训练流程，因此不会详细展开云平台数据展示的内容。但如果你感兴趣，随时可以访问 SenseCAP 云平台尝试添加设备并查看数据，这是更好了解平台能力的绝佳方式！
...
- 对于 500 万像素相机，建议在 1m 至 5m 的范围内采集图像，以获得最佳图像质量。拍摄过程中，请确保相机正对被摄体，光线充足（无眩光或过曝），并避免明显的运动或振动。

**SenseCAP Vision AI 相机在多远的距离内可以检测目标并获得良好效果？**

- 根据我们使用人体检测模型的测试结果，在 1m 至 5m 的范围内，相机可以达到 70% 的结果置信度。
...
- 可以，如果你有大批量需求，请通过 sensecap@seeed.cc 联系我们。
