...
Seeed Studio XIAO RP2040 和 Seeed Studio XIAO SAMD21 一样小巧，但性能更强。一方面，它搭载了强大的双核 RP2040 处理器，时钟频率最高可灵活运行至 133 MHz，是一款低功耗微控制器。同时，Seeed Studio XIAO RP2040 还集成了 264KB SRAM 和 2MB 片上 Flash 存储，可提供更多空间用于程序存储和运行。另一方面，这块小板在处理性能方面表现出色，却只需要更低的功耗。  
...
| **工作温度** | -20°C-70°C |
| **尺寸** | 21x17.8mm |
| **产品变体** | [Seeed Studio XIAO RP2040 3 片装套件 \| 为微型 Mic 节省 10%](https://www.seeedstudio.com/Seeed-Studio-XIAO-RP2040-3PCS-p-5942.html)<br/>[Seeed Studio XIAO RP2040 预焊接版 \| 即插即用迷你 ...](https://www.seeedstudio.com/Seeed-Studio-XIAO-RP2040-Pre-Soldered-p-6333.html) |
...
对于电源引脚：
板载 DC-DC 转换电路可以将 5V 电压转换为 3.3V，允许通过 VIN-PIN 和 5V-PIN 使用 5V 电源为设备供电。

XIAO RP2040 目前仅支持电池供电，当连接电池时不能再连接 Type-C 接口，否则可能存在安全风险。
...
有时在用户烧录程序失败后，Seeed Studio XIAO RP2040 的端口可能会消失。我们可以通过以下操作来解决这个问题：

- 长按 “B” 按钮。
- 将 Seeed Studio XIAO PR2040 连接到电脑。
- 电脑上会出现一个磁盘驱动器。

此时，芯片进入 Bootloader 模式，烧录端口会重新出现。由于 RP2040 芯片有两个分区，一个是 Bootloader，另一个是用户程序。产品在出厂时会在系统存储器中烧录一段 Bootloader 代码。我们可以通过执行上述步骤在两种模式之间切换。

### **复位**

如果你想复位 Seeed Studio XIAO RP2040，请按以下步骤操作：
...
**硬件设计**
- **📄[数据手册]** [Raspberry Pi RP2040 数据手册](https://files.seeedstudio.com/wiki/XIAO-RP2040/res/rp2040_datasheet.pdf )
- **📄[原理图]** [XIAO RP2040 原理图](https://files.seeedstudio.com/wiki/XIAO-RP2040/res/Seeed-Studio-XIAO-RP2040-v1.3.pdf )
- **🗃️[PCB 设计文件]** 
...
**机械设计**
- **📄[2D 尺寸]** [XIAO RP2040 DXF 尺寸图](https://files.seeedstudio.com/wiki/XIAO-RP2040/res/XIAO-RP2040-DXF.zip )
