...
此外，该板还有一个进阶版本 **Seeed  Studio XIAO nRF52840 Sense**。它集成了两个额外的板载传感器。其中之一是 **脉冲密度调制（PDM）数字麦克风**，可以实时接收音频数据，从而用于语音/音频识别。另一个是 **6 轴惯性测量单元（IMU）**，在诸如手势识别等 TinyML 项目中非常有用。这些板载传感器在保证开发板超小尺寸的同时，为用户提供了极大的便利。

新升级的 **XIAO nRF52840 Plus 和 XIAO nRF52840 Sense Plus** 在功能性和易用性方面都有显著提升。多功能引脚数量增加到 **20 个**，新增了 **I2S** 和 **SPI** 资源以支持更复杂的项目，**NFC 引脚** 被引出，便于集成到物联网和智能卡应用中，同时 **BAT 引脚** 重新布局以提升焊接便利性，从而带来更友好的硬件使用体验。

与 Seeed Studio XIAO RP2040 相比，Seeed Studio XIAO nRF52840 拥有**更丰富的接口**。首先需要注意的是，板载的**近场通信（NFC）接口**是可用的。其次，在 Type-C 接口一侧配有一个小型**复位按钮**。在另一侧，有一个**三合一 LED（用户 LED）**以及一个**充电指示 LED**，在连接电池时用于指示充电状态。它提供 **11 个数字 I/O** 可用作 **PWM 引脚**，以及 **6 个模拟 I/O** 可用作 **ADC 引脚**。它支持 **UART、I2C 和 SPI** 等三种常见串行接口。与 Seeed Studio XIAO RP2040 相同，它配备了**板载 2 MB Flash**，这意味着它也可以使用 **Arduino、MicroPython、CircuitPython 或其他编程语言**进行编程。
...
| Reset                   |                  | P0.18         | 复位                                                 |              |
| ADC_BAT                 | READ_BAT_ENABLE  | P0.14         | 电池电压读取使能控制引脚                             |              |
| 6 DOF IMU_PWR           |                  | P1.08         | 6D 模组的电源开关                                    |              |
| 6 DOF IMU__INT1         |                  | P0.11         | 6D 模组的中断信号引脚                                |              |
...
这两个库所支持的引脚定义可能会有些许不同，Seeed 会持续更新本 wiki，直到说明清楚为止。
...
- **步骤 3.** 向 Arduino IDE 中添加 Seeed Studio XIAO nRF52840（Sense）板卡包

依次点击 **File > Preferences**，在 **"Additional Boards Manager URLs"** 中填入以下链接：
...
依次点击 **Tools > Board > Boards Manager...**，在搜索框中输入关键字“**seeed nrf52**”，选择你想要的最新版本板卡并安装。你也可以两个都安装。
...
安装好板卡包后，依次点击 **Tools > Board** 并选择你想要的开发板，继续选择“**Seeed XIAO nRF52840 Sense**”。现在我们已经完成了在 Arduino IDE 中对 Seeed Studio XIAO nRF52840（Sense）的设置。
...
依次点击 **Tools > Port**，选择已连接的 Seeed Studio XIAO nRF52840（Sense）的串口名称。它很可能是 COM3 或更高（**COM1** 和 **COM2** 通常保留给硬件串口）。已连接的 Seeed Studio XIAO nRF52840（Sense）的串口名称通常会带有括号，其中写着 **Seeed Studio XIAO nRF52840**（对应 **Seeed Studio XIAO nRF52840**）或 **Seeed Studio XIAO nRF52840 Sense**（对应 **Seeed Studio XIAO nRF52840 Sense**）。

<p style={{textAlign: 'center'}}><img src="https://files.seeedstudio.com/wiki/XIAO-BLE/port.png" alt="pir" width={550} height="auto" /></p>

- **步骤 5.** 依次点击 **File > Examples > 01.Basics > Blink** 打开 **Blink** 示例
...
首先你需要了解，这个 LED 在代码控制下的行为与平常不同。**当我们输出 LOW 信号时，LED 会点亮（ON）**，而**当我们输出 HIGH 信号时，LED 会熄灭（OFF）**。这是因为该 LED 由共阳极控制，只会在低电平信号下点亮。
...
在这里，即使使用了 **HIGH**，LED 也会是**熄灭（OFF）**状态。你需要将 **HIGH** 替换为 **LOW** 才能**点亮（ON）** LED。
...
可以使用 P0.13 将电池充电电流设置为大约 50 mA 或 100 mA。将 P0.13 配置为无上拉或下拉的高阻输入可选择 50 mA，将其配置为输出并驱动为 LOW 可选择 100 mA。这些数值适用于恒流充电阶段；当电池接近充满时，实际充电电流会减小。
...
当 P0.14 设为 HIGH 时，电池电压读取通路被禁用，P0.31 可能会达到 3.6V 的输入电压上限，从而有损坏 P0.31 引脚的风险。为了安全地读取电池电压，应将 P0.14 设为 LOW（以启用读取通路），然后在 P0.31 上读取 ADC 值。
...
- **📄[Schematic]** [XIAO nRF52840 原理图](https://files.seeedstudio.com/wiki/XIAO-BLE/Res/260828_XIAO_nRF52840.pdf)
- **🗃️[PCB Design Files]** 
