...
:::tip
本教程中关于 microSD 卡的部分**仅**适用于 **XIAO ESP32S3 Sense**，其余内容适用于 ESP32-S3 芯片，因此除了 microSD 卡部分外，你可以在 XIAO ESP32S3 上运行这些示例。
:::
...
安装扩展板非常简单，你只需要将扩展板上的连接器与 XIAO ESP32S3 上的 B2B 连接器对齐，用力按压并听到“咔哒”一声，即表示安装完成。
...
这也意味着，如果你选择使用扩展板的 microSD 卡功能，就不能同时使用 XIAO ESP32S3 的 SPI 功能。你可以通过连接/切断 J3 焊盘来开启或关闭 microSD 卡功能。
...
在 Arduino IDE 中有多个示例展示了如何使用 XIAO ESP32S3 操作 microSD 卡上的文件。在 Arduino IDE 中，依次进入 **File > Examples > SD > SD_Test**，或者复制以下代码。
...
:::caution
请注意，您不能直接使用示例程序，您需要对程序进行一些小的修改以确保其正常工作。修改内容是在 `Setup()` 函数中将原始代码从 `SD.begin()` 更改为 `SD.begin(21)`。指定用于初始化的引脚编号。
:::

将程序上传到 XIAO ESP32S3 Sense，打开串口监视器，您将看到文件创建过程和写入过程。您也可以通过 microSD 转 USB 的方式，在电脑上访问新创建的文件及其内容。
...
首先，您需要包含以下库：用于处理文件的 **FS.h**，用于与 microSDcard 接口的 **SD.h**，以及用于使用 SPI 通信协议的 **SPI.h**。

1. 在 `setup()` 中，以下代码行使用 `SD.begin(21)` 初始化 microSDcard。在这里，我们需要为 `SD.begin()` 传入一个参数，即 CS 引脚。对于扩展板的 microSD 卡设计，CS 引脚连接到 **GPIO 21**。如果您使用的是 Round Display for XIAO，那么传入的参数应为 **D2**。
...
3. 您可以通过调用 `cardSize()` 方法获取 microSDcard 的容量：
...
**该示例提供了多个函数来处理 microSDcard 上的文件。**

4. `listDir()` 函数用于列出 SD 卡上的目录。该函数的参数包括文件系统（SD）、主目录名称以及要进入目录的层级数。

   以下是调用此函数的示例。`/` 对应 microSDcard 的根目录。

```c
listDir(SD, "/", 0);
```

5. `createDir()` 函数用于创建新目录。将 `SD` 文件系统和目录路径作为参数传入。例如，以下命令会在根目录下创建一个名为 `mydir` 的新目录。

```c
createDir(SD, "/mydir");
```

6. 要从 microSDcard 中删除目录，请使用 `removeDir()` 函数，并将 SD 文件系统和目录路径作为参数传入。

```c
removeDir(SD, "/mydir");
```

7. `readFile()` 函数会读取文件内容，并在串口监视器中打印出来。与之前的函数一样，将 `SD` 文件系统和文件路径作为参数传入。例如，以下代码行会读取 `hello.txt` 文件的内容。

```c
readFile(SD, "/hello.txt")
```

8. 要向文件写入内容，可以使用 `writeFile()` 函数。将 `SD` 文件系统、文件路径和要写入的信息作为参数传入。以下代码行会在 `hello.txt` 文件中写入 `Hello`。

```c
writeFile(SD, "/hello.txt", "Hello ");
```

9. 同样，您可以使用 `appendFile()` 函数向文件追加内容（而不是覆盖之前的内容）。以下代码行会在 `hello.txt` 文件中追加消息 `World!\n`。其中 `\n` 表示下次向文件写入内容时，将会写入到新的一行。

```c
appendFile(SD, "/hello.txt", "World!\n");
```

10. 您可以使用 `renameFile()` 函数重命名文件。将 SD 文件系统、原始文件名和新文件名作为参数传入。以下代码行会将 `hello.txt` 文件重命名为 `foo.txt`。

```c
renameFile(SD, "/hello.txt", "/foo.txt");
```

11. 使用 `deleteFile()` 函数删除文件。将 SD 文件系统和要删除文件的文件路径作为参数传入。以下代码行会从 microSDcard 中删除 `foo.txt` 文件。

```c
deleteFile(SD, "/foo.txt");
```

12. `testFileIO()` 函数用于显示读取文件内容所需的时间。以下函数会测试 test.txt 文件。
...
本项目展示了如何使用 XIAO ESP32S3 Sense 将带有时间戳的数据记录到 TF 卡中。作为示例，我们将每 10 分钟记录一次 Multichannel Gas Sensor 的温度读数。XIAO ESP32S3 在每次读数之间将处于深度睡眠模式，并通过网络时间协议（NTP）请求日期和时间。
...
在软件方面，您需要提前在 Arduino IDE 中安装以下库。
...
下面是完整的示例程序。在程序中我们需要使用网络对时，因此您需要将程序中的 WiFi 名称和密码修改为您自己的。
...
在本示例中，XIAO ESP32S3 在每次读取之间都处于深度睡眠模式。在深度睡眠模式下，你的所有代码都应该写在 `setup()` 函数中，因为 XIAO ESP32S3 永远不会运行到 `loop()`。

本示例使用了从微秒到秒的转换因子，这样你就可以在 `TIME_TO_SLEEP` 变量中以秒为单位设置睡眠时间。在本例中，我们将 XIAO ESP32S3 设置为睡眠 10 分钟（600 秒）。如果你希望 XIAO ESP32S3 睡眠不同的时间，只需在 `TIME_TO_SLEEP` 变量中输入深度睡眠的秒数即可。
...
创建一个名为 `readingID` 的变量来保存读取 ID。这是一种整理读取数据的方式。要在深度睡眠期间保存变量值，我们可以将其保存在 RTC 内存中。要在 RTC 内存中保存数据，只需在变量定义前添加 `RTC_DATA_ATTR`。
...
接下来，在 `Setup()` 函数中初始化 NTP 客户端，以从 NTP 服务器获取日期和时间。你可以使用 `setTimeOffset(<time>)` 方法来根据你的时区调整时间。
...
我们建议你将这两个函数一起使用。请确保在设置好唤醒时间后，让 XIAO 尽快进入深度睡眠模式。
...
ESP32 内置了一个串行外设接口闪存文件系统（SPIFFS）。SPIFFS 是为带有通过 SPI 总线连接的闪存芯片（如 ESP32 闪存）的微控制器创建的轻量级文件系统。在本文中，我们将演示如何使用 Arduino IDE 的一个插件，轻松地将文件上传到 ESP32 的文件系统。
...
**步骤 1.** 进入[发布页面](https://github.com/me-no-dev/arduino-esp32fs-plugin/releases/)，点击 [ESP32FS-1.1.zip](https://github.com/me-no-dev/arduino-esp32fs-plugin/releases/download/1.1/ESP32FS-1.1.zip) 文件进行下载。

**步骤 2.** 找到你的 Sketchbook 位置。在 Arduino IDE 中，依次点击 **File > Preferences**，查看你的 Sketchbook 位置。以我的情况为例，它在以下路径：`C:\Users\mengd\Documents\Arduino`。
...
**步骤 7.** 然后，打开草图文件夹。你可以依次点击 **Sketch > Show Sketch Folder**。将会打开保存草图的文件夹。
...
**步骤 9.** 你想要保存到 ESP32 文件系统中的文件，都应放在 data 文件夹内。举例来说，创建一个带有一些文本内容的 _.txt_ 文件，并命名为 **test_example**。
...
**步骤 10.** 然后，在 Arduino IDE 中上传这些文件时，只需依次点击 **Tools > ESP32 Sketch Data Upload**。
...
上传完成后，以 115200 的波特率打开串口监视器。它应当在串口监视器上打印出你的 _.txt_ 文件内容。
...
在使用开发板时，很多人都希望能够使用芯片上的 Flash 存储器来保存一些重要数据。这就需要一种存储方式，即使在开发板异常的情况下也能确保数据不丢失。

本教程将从以下两种不同的存储方式，介绍如何在 XIAO ESP32 的 Flash 存储器上存储重要数据：

1. 第一个指南展示如何使用 Preferences.h 库在 ESP32 的 Flash 存储器中永久保存数据。存储在 Flash 中的数据在复位或断电后仍然存在。使用 Preferences.h 库非常适合保存网络凭据、API 密钥、阈值，甚至 GPIO 的最后状态等数据。你将学习如何在 Flash 存储器中保存和读取数据。

2. 第二个指南解释什么是 XIAO ESP32C3 的 EEPROM，以及它的用途。我们还将向你展示如何对 EEPROM 进行读写，并构建一个项目示例，将学到的概念付诸实践。

本节是为 XIAO ESP32C3 编写的，并且与新的 XIAO ESP32S3 完全兼容，因此你可以直接使用此处的示例程序，这里不再赘述。
...
以下是原文的参考链接，欢迎你通过下面的原文链接进一步了解 ESP32 文件系统。
...
- [在 Arduino IDE 中安装 ESP32 Filesystem Uploader](https://randomnerdtutorials.com/install-esp32-filesystem-uploader-arduino-ide/)

关于使用 ESP32 开发板的更多信息，请访问 Random Nerd Tutorials 官方网站。
