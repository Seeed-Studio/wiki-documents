...
:::warning
**深度睡眠期间 USB 外设已禁用：** 在深度睡眠期间，包括内部 USB 外设（USB-Serial-JTAG）在内的所有数字外设都会被断电。设备处于深度睡眠状态时，**无法通过 USB 输出串口信息**。如果需要调试，请使用连接到硬件 UART 引脚的外部 USB-UART 芯片。
:::
...
:::warning
**轻度睡眠期间 USB 外设已禁用：** 为了节省功耗，内部 USB 外设（USB-Serial-JTAG）会在轻度睡眠期间被禁用。这意味着设备处于轻度睡眠状态时，**无法通过 USB 输出串口信息**。如果你使用 USB 端口查看串口日志，在睡眠期间将看不到任何输出。若要调试，可考虑使用连接到硬件 UART 引脚的外部 USB-UART 芯片，或使用 GPIO 唤醒，以便在设备唤醒后监视输出。
:::
