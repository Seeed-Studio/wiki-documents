---
description: Seeed Studio XIAO ESP32-S3 系列入门指南。
title: Seeed Studio XIAO ESP32-S3 系列入门指南
keywords:
  - esp32s3
  - xiao
...
# Seeed Studio XIAO ESP32-S3 系列入门指南
...
Seeed Studio XIAO 系列是袖珍型开发板，具有相似的硬件结构，尺寸真正做到了拇指大小。这里的代号“XIAO”代表其一半特性“微小”，而另一半则是“强大”。

Seeed Studio XIAO ESP32-S3 Sense 集成了摄像头传感器、数字麦克风和 SD 卡支持。结合嵌入式机器学习算力和成像能力，这块开发板可以成为你入门智能语音与视觉 AI 的得力工具。

:::tip
**OV2640 camera** 已经停产，后续的 XIAO ESP32-S3 Sense 使用 **OV3660 camera** 型号。不过，Wiki 中的摄像头示例代码仍然适用。
:::
...
<table>
  ...
  <tr>
    ...
    <td>1x UART<br/>1x IIC<br/>1x IIS<br/>1x SPI<br/>11x GPIOs (PWM)<br/>9x ADC<br/>1x 用户 LED<br/>1x 充电 LED<br/>1x B2B 连接器（带 2 个额外 GPIO）<br/>1x 复位按钮<br/>1x Boot 按钮</td>
    <td>2x UART<br/>1x IIC<br/>1x IIS<br/>2x SPI<br/>18x GPIOs (PWM)<br/>9x ADC<br/>1x 用户 LED<br/>1x 充电 LED<br/>1x B2B 连接器<br/>1x 复位按钮<br/>1x Boot 按钮</td>
  </tr>
  <tr>
    <th>尺寸</th>
    <td>21 x 17.8mm</td>
...
<table align="center">
 <tr>
  <th>XIAO ESP32-S3 Plus 正面示意图</th>
 </tr>
 <tr>
 ...
 <tr>
  <th>XIAO ESP32-S3 Plus 背面示意图</th>
 </tr>
 <tr>
...
- XIAO ESP32-S3 Plus 的 B2B 连接器与 [Wio-SX1262 扩展板](https://www.seeedstudio.com/Wio-SX1262-with-XIAO-ESP32S3-p-5982.html) 兼容，但与插接式摄像头传感器板不兼容。
:::
...
要改变位值，应将启动配置引脚连接到外部下拉/上拉电阻。如果将 ESP32-S3 作为主 MCU 的从设备使用，启动配置引脚的电压电平也可以由主 MCU 控制。

所有启动配置引脚都带有锁存器。在系统复位时，锁存器会采样各自启动配置引脚的位值，并将其保存，直到芯片断电或关闭。锁存器状态不能通过其他方式改变。这样可以在整个芯片运行期间提供启动配置引脚的值，并在复位后释放这些引脚，用作普通 IO 引脚。
...
为了让你更快上手 XIAO ESP32-S3，请阅读下面的硬件和软件准备内容来完成对 XIAO 的准备。
...
XIAO ESP32-S3 Sense 出厂时预装了 WebCam 示例程序。你只需为 XIAO 安装好天线并给其上电即可使用该程序。详细信息可以阅读关于此程序的 Wiki。
...
XIAO ESP32-S3 默认出厂不带排针，你需要自备排针并将其焊接到 XIAO 对应的引脚上，这样才能连接扩展板或传感器。
...
如果你购买的是 XIAO ESP32-S3 Sense，那么你还会得到一块扩展板。这块扩展板上集成了 1600x1200 OV2640 / 2048x1536 OV3660 摄像头传感器、板载 SD 卡槽以及数字麦克风。
...
:::tip
Wiki 中所有与摄像头相关的程序同时兼容 OV5640、OV2640 和 OV3660 摄像头。
:::

#### 安装升级版散热片

通过安装我们升级版的散热片，为你的 XIAO ESP32-S3 Sense 提供最佳散热效果。这个全新设计专门用来解决旧款在高强度运行（例如使用摄像头）时散热不足的问题。用户反馈指出原有散热片的散热效果不佳，因此我们开发了更高效的解决方案。

<details>
<summary>点击查看性能对比</summary>

我们的测试结果表明，升级版散热片相较于原始方案具有明显优势：
...
***安装提示：*** 安装时请优先让散热片覆盖 Thermal PAD，因为它正好位于 ESP32S3 芯片上方，是主要的发热源。正确对齐可以确保最佳散热效果，同时请尽量保持 BAT 引脚不被遮挡。
:::
...
:::note
如果你想使用 XIAO ESP32-S3 Plus 的 BAT 引脚，那么这种双散热片方案并不适合。
:::
...
:::caution
XIAO ESP32-S3 的板载支持包要求版本为 **2.0.8** 及以上才可用。
:::
...
在 Arduino IDE 顶部你可以直接选择端口。通常端口名称中会包含 "usbmodem" 或 "usbserial"。如果你不确定，可以先拔掉再插上，查看是哪个端口消失和出现。

</TabItem>
</Tabs>

<!-- :::tip
我们已经向 ESP32 提交了合并请求，当 ESP32 发布下一版板载包更新后，就可以在 Arduino IDE 中搜索并使用 XIAO ESP32-S3。
...
- **步骤 2**. 保持按住 `BOOT` 按钮，然后通过数据线连接电脑，连接完成后再松开 `BOOT` 按钮。
...
到现在为止，相信你已经对 XIAO ESP32-S3 的特性和硬件有了比较深入的了解。接下来，我们以最简单的 Blink 程序为例，为你的 XIAO ESP32-S3 完成第一次闪烁！
...
XIAO ESP32-S3 系列内置电源管理芯片，使 XIAO ESP32-S3 可以通过电池独立供电，或通过 XIAO ESP32-S3 的 USB 端口为电池充电。
...
1. 请确保你的程序已经正确编译并能正常运行，否则 UF2 文件可能无法按预期执行。
2. 在 **xiaos3-bin2uf2** 文件夹中提供了一个用于 Blink 的示例 UF2 文件。上传后，XIAO ESP32-S3 上的橙色 LED 会闪烁。你可以使用此 UF2 文件进行测试。
...
如果你遇到上述问题，建议首先尝试按下 XIAO ESP32-S3 上的复位按钮，尝试让程序重新运行。如果问题仍然存在，请重新检查你的程序，并阅读 **[BootLoader 模式](#bootloader-模式)** 中提供的方法来恢复设备。
...
- **📄[原理图]** [XIAO ESP32-S3 原理图](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/new-res/202003751_XIAO%20ESP32S3_v1.4_SCH_260226.pdf.pdf)
- **🗃️[PCB 设计文件]** 
  - [XIAO ESP32-S3 KiCad 工程](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/new-res/202003751_XIAO%20ESP32S3_v1.4_SCH&PCB_260226.zip)
- **🗃️[PCB 设计库]** 
...
**结构设计**
- **📄[二维尺寸]** [XIAO ESP32-S3 DXF 尺寸图](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/XIAO_ESP32S3_v1.1_Dimensioning.dxf )
...
**结构设计**
- **📄[二维尺寸]** [XIAO ESP32-S3 Sense DXF 尺寸图（顶层）](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/XIAO_ESP32S3_ExpBoard_v1.0_top.dxf )
- **📄[二维尺寸]** [XIAO ESP32-S3 Sense DXF 尺寸图（底层）](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/XIAO_ESP32S3_ExpBoard_v1.0_bot.dxf )
...
**结构设计**
- **📄[二维尺寸]** [XIAO ESP32-S3 Plus DXF 尺寸图（顶层）](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/TOP.dxf )
- **📄[二维尺寸]** [XIAO ESP32-S3 Plus DXF 尺寸图（底层）](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/BOTTOM.dxf )
...
- **[电子书]** [XIAO：强大性能，小巧主板——精通 Arduino 和 TinyML](https://mjrovai.github.io/XIAO_Big_Power_Small_Board-ebook/)
