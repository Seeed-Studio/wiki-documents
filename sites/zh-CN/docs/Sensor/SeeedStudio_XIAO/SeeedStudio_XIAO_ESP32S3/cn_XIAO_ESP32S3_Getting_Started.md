...
<table>
  ...
  <tr>
    <th>存储</th>
    <td>片上 8MB PSRAM &amp; 8MB Flash</td>
    <td>片上 8MB PSRAM &amp; 8MB Flash<br/>板载 SD 卡槽，支持 32GB FAT</td>
    <td>片上 8MB PSRAM &amp; 16MB Flash</td>
  </tr>
  <tr>
    <th>接口</th>
...
在开始之前，了解一些产品的基础参数非常重要。下表提供了 Seeed Studio XIAO ESP32-S3 的特性信息。
...
:::caution
- 尽管 XIAO ESP32-S3 将 GPIO41 和 GPIO42 分配给 A11 和 A12 引脚，但由于 ESP32-S3 芯片本身的特性，A11 和 A12 引脚不支持 ADC 功能。请务必加以区分并正确区分它们。
...
在每次启动或复位时，芯片都需要一些初始配置参数，例如以哪种启动模式加载芯片、闪存电压等。这些参数是通过启动配置引脚传递的。复位之后，启动配置引脚会作为普通 IO 引脚工作。
...
要改变位值，应将启动配置引脚连接到外部下拉/上拉电阻。如果 ESP32-S3 作为主控 MCU 的从设备使用，启动配置引脚的电平也可以由主控 MCU 控制。

所有启动配置引脚都带有锁存器。在系统复位时，锁存器会采样各自启动配置引脚的位值，并将其保存，直到芯片断电或关机。锁存器的状态不能通过其他方式改变。这样可以在整个芯片运行期间提供启动配置引脚的值，并在复位后释放这些引脚，用作普通 IO 引脚。
...
XIAO ESP32-S3 Sense 出厂时预装了 WebCam 示例程序。你只需为 XIAO 安装好天线并给其上电即可使用该程序。详情可阅读关于此程序的 Wiki。

- [视频流传输](https://wiki.seeedstudio.com/cn/xiao_esp32s3_camera_usage#project-ii-video-streaming)

:::note
从 **2025 年 6 月** 起，XIAO ESP32-S3 Sense 的出厂固件会启用一个默认的 AP Wi‑Fi，凭据如下：
...
由于 XIAO ESP32-S3 体积非常小，在焊接排针时请务必小心，不要把不同的引脚焊连在一起，也不要把焊锡粘到屏蔽罩或其他元件上。否则可能导致 XIAO 短路或无法正常工作，由此造成的后果需由用户自行承担。
...
天线的安装有一个小技巧，如果直接用力向下按，你会发现非常难按下去，而且手指会很疼！正确的安装方式是先将天线连接器的一侧卡入连接器座中，然后再轻轻按下另一侧，天线就会安装到位。
...
如果你购买的是 XIAO ESP32-S3 Sense，那么你还会配有一块扩展板。该扩展板上集成了 1600x1200 OV2640 / 2048x1536 OV3660 摄像头传感器、板载 SD 卡槽以及数字麦克风。

通过将扩展板与 XIAO ESP32-S3 Sense 安装在一起，你就可以使用扩展板上的各项功能。
...
通过安装我们升级版的散热片，为你的 XIAO ESP32-S3 Sense 提供最佳散热效果。这个全新设计专门用来解决旧款在高强度运行（尤其是摄像头使用）时散热不足的问题。反馈表明原有散热片的散热效果不够理想，因此我们开发了更高效的解决方案。
...
- 配备散热片的设备在运行超过一小时后仍保持稳定，在达到峰值温度时也没有出现性能下降。
...
在开始之前，请确保你的设备已断电并从所有电源上拔下。

:::tip notice

***购买须知：*** 在购买 XIAO ESP32-S3 Sense 时，需要注意只有**带摄像头的型号**才会附带散热片。如果你购买的是不带摄像头的 ESP32S3 版本，则需要单独购买散热片。

***安装提示：*** 安装时请优先确保用散热片覆盖 Thermal PAD，因为它正好位于 ESP32S3 芯片上方，是主要热源。正确对齐可确保最佳散热效果，同时请尽量保持 BAT 引脚不被遮挡。
:::

现在，让我们开始安装过程：

***步骤 1. 准备散热片：***
首先取下散热片上的保护盖，露出导热胶。这样就可以为散热片与 ESP32S3 芯片的牢固贴合做好准备。
...
:::note
如果你想使用 XIAO ESP32-S3 Plus 的 BAT 引脚，那么这种双散热片外壳并不适用。
:::
...
推荐用于 XIAO ESP32-S3 的编程工具是 Arduino IDE，因此在软件准备部分，你需要完成 Arduino 的安装。
...
在 Arduino IDE 顶部，你可以直接选择端口。它很可能是 COM3 或更高（**COM1** 和 **COM2** 通常保留给硬件串口）。
...
在 Arduino IDE 顶部你可以直接选择端口。它通常是名称中带有 "usbmodem" 或 "usbserial" 的那个。如果你不确定，可以先拔掉再插上，查看哪个端口消失又出现。
...
到现在为止，相信你已经对 XIAO ESP32-S3 的特性和硬件有了较好的了解。接下来，我们以最简单的 Blink 程序为例，为你的 XIAO ESP32-S3 完成第一次闪烁！
...
恭喜，你已经学会了如何为 XIAO ESP32-S3 编写和上传程序！
...
XIAO ESP32-S3 系列内置电源管理芯片，可以让 XIAO ESP32-S3 通过电池独立供电，或者通过 XIAO ESP32-S3 的 USB 接口为电池充电。

如果你想为 XIAO 连接电池，我们建议你购买合格的可充电 3.7V 锂电池。焊接电池时，请注意区分正负极。电源负极应为靠近 USB 接口的一侧，电源正极为远离 USB 接口的一侧。
...
1. 当 XIAO ESP32-S3 未连接电池时，连接 Type-C 线缆后红灯会亮起，并在 30 秒后熄灭。
2. 当连接电池并插入 Type-C 线缆进行充电时，红灯会闪烁。
3. 当通过 Type-C 为电池充满电时，红灯会熄灭。
...
将 `BIN` 文件复制到之前解压的 **xiaos3-bin2uf2** 目录中。然后运行 **convert_uf2.bat** 脚本来生成 UF2 文件，此时会需要你输入 `bin` 文件的名称。

**步骤 3**：进入 UF2 BootLoader 模式

将 XIAO 连接到电脑并运行 **boot_uf2.bat** 脚本。XIAO 会在你的电脑上显示为一个 USB 驱动器，这表明它已成功进入 UF2 BootLoader 模式。
...
该项目由从 IDF 自定义的第二阶段 bootloader 和作为第三阶段 bootloader 的 UF2 出厂应用组成。
...
### Q2：为什么我的 XIAO 在圆角处有不对齐的问题？这是质量问题吗？
...
由于集成度很高，XIAO ESP32-S3 是所有 XIAO 中最复杂的一款，PCB 在工厂生产时需要拼板。由于集成度高，拼板连接只能放在四个圆角处，这就会导致如图所示的圆角不平整问题。我们会尽力改进工艺，确保在后续生产中解决这一问题。
...
要烧录固件，只需运行相应的 `.bat` 文件。如果烧录过程失败，请从提示中复制命令行，并在文件所在的终端中手动运行。
...
**结构设计**
- **📄[2D 尺寸]** [DXF 格式的 XIAO ESP32-S3 尺寸图](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/XIAO_ESP32S3_v1.1_Dimensioning.dxf )
...
**硬件设计**
- **📄[Datasheet]** [Espressif ESP32-S3 数据手册](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/esp32-s3_datasheet.pdf )
- **📄[Schematic]** [XIAO ESP32-S3 Sense 原理图](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/new-res/202003753_XIAO%20ESP32S3%20Sense_v1.5_SCH_260226.pdf.pdf)
- **📄[Schematic]** [XIAO ESP32-S3 扩展板原理图](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/XIAO_ESP32S3_ExpBoard_v1.0_SCH.pdf)
- **🗃️[PCB Design Files]** 
  - [XIAO ESP32-S3 Sense KiCad 项目](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/new-res/202003753_XIAO%20ESP32S3%20Sense_v1.5_SCH&PCB_260226.zip)
- **🗃️[PCB Design Libraries]** 
  - [XIAO 系列 KiCad 封装库](https://files.seeedstudio.com/wiki/XIAO-KiCad-Library/New_XIAO_Series_Footprints.zip )
...
- **🗃️[PCB 设计文件]** 
  ...
  - [XIAO Plus 底板（无底部焊盘引出）KiCad 工程](https://files.seeedstudio.com/wiki/SeeedStudio-XIAO-ESP32S3/res/XIAO_Plus_Base_without_botton_pad_lead_out_V1.0.zip )
- **🗃️[PCB 设计库]** 
  - [XIAO 系列 KiCad 封装库](https://files.seeedstudio.com/wiki/XIAO-KiCad-Library/New_XIAO_Series_Footprints.zip )
  - [XIAO 系列 KiCad 原理图符号](https://files.seeedstudio.com/wiki/XIAO-KiCad-Library/XIAO_Series_SCH_Symbols.zip )
