---
description: 使用通俗语言概述 reSpeaker Clip AI 聊天系统，涵盖路由流程、LangGraph 架构、工具使用、记忆召回、向量搜索、语音处理流水线以及已存对话上下文。
...
一份用通俗语言编写的指南，介绍 reSpeaker Clip AI Agent 聊天系统是如何工作的。这里的所有内容都来自 README 和 `backend/` 中的实际代码。
...
该系统是一个以语音为优先的 AI 助手，构建在 Flask + LangGraph + Groq 之上。你可以通过说话或输入消息与其交互，智能体会决定如何处理请求，可选地调用工具，生成答案，并将回复语音播放给你。
...
- 一个路由器会将每个请求分类到三条路径之一。
- 这些路径上的某个智能体可以调用工具（网页搜索、计算器、Notion、历史对话搜索）。
- 向量存储 + 记忆层让智能体可以回忆你的历史对话和长期事实。
...
- LangGraph 是状态机，用来路由请求并运行选定的节点。
- Groq 提供 LLM、语音转文本（Whisper）和文本转语音（Orpheus）。
- Mem0 提供长期记忆；Pinecone + 本地向量嵌入提供历史对话搜索。
...
- 该节点使用 `create_agent` 构建一个 LangChain 智能体，并允许模型在循环中调用工具。
- 节点：`agentic_node`（`backend/graph/nodes/agentic.py`），LLM = 专用工具调用模型（`gpt-oss-20b`）。

### `persona` — 风格化

- 用户明确要求某种风格、人格、角色或教学方式。
- 与 `simple` 机制相同，但使用不同的 system prompt，指示模型调整语气。
...
路由在 `router_node`（`backend/graph/router.py`）中完成，先进行关键词预检查，然后再进行 LLM 分类。
...
2. LLM 分类。否则，请求会连同 `ROUTER_PROMPT` 一起发送到主 Groq 模型，这是一个分类器提示词，只返回一个单词：

   | 路由 | 触发输入 |
   | --- | --- |
   | `simple` | 一般问题、解释说明、普通对话，不需要工具/人设 |
   | `context` | 可能需要外部信息/工具、数据管理（Notion/日历）、关于用户设备/文件/已存信息的问题 |
   | `persona` | 用户明确请求某种风格、人设、教学方式、角色或行为 |

3. 回退逻辑。输出会被转为小写并去除空白。任何不在 `{simple, context, persona}` 中的结果都会回退到 `simple`。
...
- 智能体会被缓存，只有在工具签名发生变化时才会重建（`_get_agent`）。
- 重试逻辑：最多 `MAX_RETRIES = 3` 次尝试；短暂的 `tool_use_failed` 错误会在休眠 1 秒后重试。
- 保护栏：递归深度上限为 `MAX_AGENT_ITERATIONS = 10`。如果达到上限，智能体会返回一条固定的“我达到上限……”消息，而不是崩溃。
- system prompt 会告诉模型尽量减少工具调用次数，一旦信息足够就尽快回答。
- 在 SSE 流式传输期间，每次工具调用都会作为 `thinking` 事件（`{"tool": "web_search"}`）暴露给 UI。
...
| 工具 | 后端服务 | 作用 | 配置键 |
| --- | --- | --- | --- |
| `calculator` | 本地、安全 AST 求值 | 通过白名单 `+ - * / ** %` 求值进行数学计算 | —（始终可用） |
| `search_conversations` | Pinecone + 本地嵌入 | 按相关性查找用户自己的历史对话 | `PINECONE_API_KEY` |
| `composio_search` | Composio | 根据自然语言请求搜索匹配的 Composio 应用工具 | `COMPOSIO_API_KEY` |
| `composio_execute` | Composio | 使用 JSON 参数按 slug 运行匹配到的工具 | 同上 |
| `composio_connect` | Composio | 通过 Connect Link 授权一个工具集（例如 `github`） | 同上 |
| `web_search` | Tavily | 实时/当前网页信息（新闻、固件、产品详情） | `TAVILY_API_KEY` |
| `add_todo` | Notion | 向待办列表添加任务 | `NOTION_API_KEY` / `NOTION_DATABASE_ID` |
| `list_todos` | Notion | 列出任务及其状态 | 同上 |
| `complete_todo` | Notion | 将任务标记为完成（按名称/关键词匹配） | 同上 |
| `delete_todo` | Notion | 删除任务（按名称/关键词匹配） | 同上 |

- 只有在配置了 Notion 时，Notion 工具才会被添加。
- 未配置的工具会返回友好的提示信息（例如，“Web search is unavailable…”），因此系统可以优雅降级。
...
在启动时，如果 Pinecone 索引不存在，`init_index()` 会自动创建该索引（维度 `384`，度量方式 `cosine`，无服务器）。

### 哪些内容被嵌入，哪些作为元数据存储

该设计让向量保持精简，并将其他内容放入元数据：

- 嵌入（向量）：`"<title>\n\n<overview>"` —— 会话的简短语义摘要。
- 元数据：`user_id`、`conversation_id`、`title`、`created_at`。
- 完全不进入向量存储：完整转录文本。实际的会话轮次保存在 Supabase/SQLite 中，并在匹配后通过 id 拉取。
...
- 标题/概览由主 Groq LLM 使用两行提示词（`Title:` / `Overview:`）生成。
- 向量嵌入会被归一化（适用于 cosine）。
- 以上全部为异步操作（守护线程），因此用户的响应永远不会因为索引而被阻塞。
...
- 查询会使用同一个本地模型进行嵌入，然后在搜索时加上 `user_id` 过滤条件，这样用户只能看到自己的会话。
- 匹配结果返回 id + 分数 + 元数据；完整摘要会通过 id 从关系型存储中拉取。
...
代码中没有硬编码的记忆桶；相反，`MEM0_CUSTOM_INSTRUCTIONS` 会告诉 Mem0 要提取哪些持久事实，并按优先级排序：

1. 健康限制与过敏——尤其是任何医生给出的建议（被视为适用于用户本人）。
2. 日程安排——会议、预约、提醒。
3. 偏好和个人细节。
...
主动召回会在每次请求时发生，在路由之前（`backend/routes/chat.py` 中的 `recall(text)`）：

1. 将传入消息发送到 Mem0 的语义搜索（`top_k = 5`）。
...
4. 在该消息到达任何节点之前，这个块会被预先添加到用户消息前面——因此 LLM 会将其视为上下文，但被告知只有在与当前主题直接相关时才使用它。

写入发生在每次对话交换之后（`save_exchange`）：用户/助手这一对会连同自定义指令一起发送到 Mem0。若缺少 Mem0 密钥，记忆保存和召回都会优雅失败（记录日志并忽略）。
...
4. 在生成回复之后：保存这两轮消息，将该交换保存到 Mem0，并启动异步向量索引。
...
| 组件 | 来源 | 大小 |
| --- | --- | --- |
| 系统提示 | 每条路径的常量（`simple`/`persona`/agent `SYSTEM_PROMPT`） | 固定 |
...
- 历史来自关系型存储，而不是向量存储（向量存储保存的是摘要，而不是每轮对话）。
- 记忆以内联方式注入到用户消息中，因此模型会将它们视为“你过去对话中的相关上下文”。
- agent 路径将消息构建为 `[history..., ("user", memories + transcript)]`，并允许 agent 使用工具进行循环。
...
simple、persona 和 agentic 提示共享一个通用的“house style”结尾：

- 回复最多 2–3 句简短句子。
- 仅使用纯文本——不使用 markdown、不使用星号、不使用表情符号。
- 召回的记忆仅在与当前主题直接相关时使用（对日程与饮食的区分有明确约束）。
- 当出现冲突时，相信最新创建的记忆。
