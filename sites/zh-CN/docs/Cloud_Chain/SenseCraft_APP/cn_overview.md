...
:::tip note
SenseCAP Mate App 正式更名为 `SenseCraft` App！
:::

# 介绍和使用

SenseCraft APP 是一个用于数据可视化和设备管理的强大应用程序。

## 应用亮点

- 节省时间 - 提供扫描即用体验，让用户能够在 4 个步骤内完成整个配置
- 适合技术背景较少或没有技术背景的人员轻松开始使用传感器数据
- 在简洁的用户界面上获取和查看数据
- 设置时间间隔：提高电池效率
...
SenseCraft App 提供 iOS 和 Android 版本。
...
:::tip Note
请选择服务器位置的 `Global`。您也可以通过 <a href="http://sensecap.seeed.cc">SenseCAP Portal</a> 创建账户
:::
...
1. 点击添加图标或 Add Event 按钮创建事件警报，Add Event 页面 Conditioins 添加条件选项，点击 Add 按钮选择设备。
...
2. 选择设备，选择测量类型，设置警报条件，选择大于或小于等条件，拖动进度条设置数值，点击下一步。
...
3. Add Event 页面 Back to normal actions 设备恢复正常，选择是否发送通知，点击 Save，输入 Event 名称，点击 Submit 成功添加事件警报返回 Event 列表。
...
4. Device 页面，点击 Message Center 查看警报消息，显示 Device warning 设备触发警报推送消息，System notification 系统消息。
...
5. 点击警报消息，设备触发条件项，推送警报消息，点击查看警报详情。返回警报列表状态变为已读，点击编辑按钮选择消息，可以进行已读、删除等操作。
...
SenseCraft 应用提供了一套模板功能，包括**添加新模板、导入模板、下载模板和应用预设模板**，以简化和加速设备配置。这些功能有效帮助用户减少设置时间，并简化大规模安装的部署。
...
7. 返回模板页面，您可以看到 `Save successful`。<br />
...
### 下载模板
1. 导航到 `Template` 页面，选择您要下载的模板。<br />
2. 点击右上角的三点菜单。<br />
3. 选择 `Download Template`。<br />
4. 将文件保存到您的移动设备存储中。<br />
...
4. 在预设模板页面，选择所需的模板。<br />
5. 返回设置页面并点击 `Send` 应用配置。<br />
