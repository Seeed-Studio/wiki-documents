...
- 适用于技术背景较少或没有技术背景的用户，轻松开始获取传感器数据
- 在简洁的用户界面上获取和查看数据
- 设置时间间隔：提升电池效率
...
3. 在 Add Event 页面中设置恢复正常时的动作，当设备恢复正常时，选择是否发送通知，点击保存，输入事件名称，点击提交，即可成功添加事件告警并返回事件列表。
...
4. 在设备页面，点击消息中心查看告警消息，显示设备告警（设备触发的告警推送消息）和系统通知（系统消息）。
...
5. 点击告警消息，可查看设备触发的条件项和推送的告警消息，点击进入查看告警详情。返回告警列表后状态变为已读，点击编辑按钮选择消息，可以进行标记已读、删除等操作。
...
SenseCraft app 提供了一套模板功能，包括 **Add New Template、Import Template、Download Template 和 Apply Preset Template**，用于简化并加速设备配置。这些功能可以高效地帮助用户减少设置时间，并简化大规模部署。
