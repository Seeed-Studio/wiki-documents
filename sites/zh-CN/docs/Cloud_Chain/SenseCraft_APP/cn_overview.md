...
SenseCraft APP 是一款用于数据可视化和设备管理的强大应用。

## 应用亮点

- 节省时间 - 提供扫码即用的体验，使用户能够在 4 个步骤内完成整个配置
- 适用于技术背景较少或没有技术背景的人，轻松开始使用传感器数据
- 在简洁的用户界面上获取和查看数据
- 设置时间间隔：提升电池效率
- 设置阈值：专注于你需要采取行动的数据 - 即将推出
- 当数据超出安全范围时发送告警 - 即将推出

## 下载

SenseCraft App 提供 iOS 和 Android 双版本。
...
SenseCraft 支持设备配置和远程管理。要使用 SenseCAP Portal 平台和其他功能，请先注册账号。

:::tip Note
请选择 Server Location 中的 `Global`。你也可以通过 <a href="http://sensecap.seeed.cc">SenseCAP Portal</a> 创建账号
:::
...
1. 点击添加图标或 Add Event 按钮创建事件告警，在 Add Event 页面 Conditions 中添加条件选项，并点击 Add 按钮选择设备。
...
3. 在 Add Event 页面中，设备恢复正常时的 Back to normal actions，选择是否发送通知，点击 Save，输入事件名称，点击 Submit 即可成功添加事件告警并返回事件列表。
...
4. 在设备页面，点击消息中心查看告警消息，显示 Device warning 设备触发告警推送消息，System notification 系统消息。
...
5. 点击告警消息，设备触发条件项时会推送告警消息，点击可查看告警详情。返回告警列表后状态变为已读，点击编辑按钮选择消息，可以进行已读、删除等操作。
...
SenseCraft app 提供了一套模板功能，包括 **Add New Template、Import Template、Download Template 和 Apply Preset Template**，用于简化和加速设备配置。这些功能可以高效地帮助用户减少设置时间，并简化大规模部署。
