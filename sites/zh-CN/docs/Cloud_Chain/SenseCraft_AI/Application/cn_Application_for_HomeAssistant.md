...
为实现这一集成，可通过 HACS 商店获取的 SenseCraft-HomeAssistant 插件，通过 SenseCraft 数据平台将 Grove Vision AI V2 连接到 Home Assistant。整个流程简洁且用户友好，你只需登录 SenseCraft 账号，即可将设备和传感器数据引入 Home Assistant 生态系统，方便你进行自定义和控制。
...
4. [在 Home Assistant 中构建 MQTT 服务](#在-home-assistant-中构建-mqtt-服务)：利用本地 MQTT 服务将 Grove Vision AI V2 和 XIAO 的数据传输到 Home Assistant。
...
为了释放 Home Assistant 的全部潜力并访问高级功能，你可以在用户界面中启用“Advanced Mode”。

导航到你的 [Home Assistant Web 界面](http://homeassistant.local:8123)。点击 Home Assistant 侧边栏左下角的个人资料图标。在个人资料页面向下滚动，找到 **Advanced Mode** 开关。将该开关切换到打开状态。
...
接下来，系统会提示你使用 GitHub 账号登录。这是必要步骤，因为 HACS 需要与 GitHub 集成，以管理社区创建的集成和插件的安装。
...
当你授权 Home Assistant 使用你的 GitHub 账号后，HACS 将完成安装。
...
会弹出一个对话框，要求你输入仓库 URL。在这里，你需要输入 SenseCraft 集成的自定义仓库 URL。输入 URL 后，选择类别（对于 SenseCraft 集成，应选择 **Integration**）。
...
:::caution
请注意，尽管此处选择的是 XIAO ESP32-S3，我们实际使用的仍然是 XIAO ESP32-C3！
:::
...
<div class="get_one_now_container" style={{textAlign: 'center'}}>
    <a class="get_one_now_item" href="https://files.seeedstudio.com/wiki/visionai-v2-ha/res/SSCMA_XIAO_ESP32C3_adapter_sensecraft_v1.1.8.bin" target="_blank" rel="noopener noreferrer">
            <strong><span><font color={'FFFFFF'} size={"4"}>下载该文件</font></span></strong>
    </a>
</div>
...
找到 **EMQX** 后，点击它查看详细信息。在插件页面，你会看到概览、文档和配置选项。要安装该插件，请点击 **Install** 按钮。
...
点击数据库旁边的开关以启用它。数据库状态应变为 "Enabled"。
...
:::caution
当显示 IP Address 时，说明 WIFI 没有问题，并且 Service status 必须与图中所示文本一致才表示正常。注意，如果 WIFI 不正常，MQTT 一定也不正常；刚切换过来时可能会显示未连接，需要等待大约 10 秒左右，你不需要刷新页面即可看到状态，它会在状态变化后自动显示最新状态。
:::
...
在 **Settings->Devices & services** 中，你可以看到设备的详细信息以及显示支持的标签页，你可以添加任意数量的标签页以在概览中显示。
...
- **Port**：`1883`。
- **clientId**：对于设备识别号，请在前面加上 `grove_vision_ai_v2_`，后面再跟上自定义的 ID 编号。
