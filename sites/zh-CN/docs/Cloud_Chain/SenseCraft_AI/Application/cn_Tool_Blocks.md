---
description: 在编写 SenseCraft AI 应用时可用的工具模块参考。
title: 工具模块
keywords:
  - tool blocks
  - sensecraft ai
...
在部署流程中，你可以一键插入 SenseCraft AI 集成功能。它们既简化创作体验，也简化终端用户的部署过程。
...
该工具模块目前支持**检测**和**分类**。分割和姿态估计计划在后续版本中提供。
...
- **Upload model：** 上传在第三方平台训练的模型文件。你需要提供模型名称、任务类型（Task）、模型类别（Class ID 和 Class Name）、附加模型文件，并选择支持的设备。上传完成后，模型会显示在 **My model** 下。
- **Train model：** 若要训练新模型或基于现有模型再训练，打开训练入口并按照向导操作。完成后，打开 **Historical models**，选择 **Send to SenseCraft**，完成上传流程，并将结果保存到 **My model**。

:::caution
当你将包含私有模型的应用发布到社区时，该模型会自动变为公开。
:::

### 选择设备

在下拉列表中，从该模型支持的设备列表中选择你实际使用的硬件。

### 推荐参数

根据你的场景微调推理和设备设置。这些值会在部署时作为默认值写入设备。

- **Confidence threshold：** 判断模型是否输出最终预测的标准。提高阈值通常可减少误报；降低阈值通常可减少漏报。
- **IOU threshold：** 在检测任务中，IoU 用于比较预测框与真实框。较高的阈值有助于区分重叠目标，但若设置过高，可能在同一目标上产生多个重复框。较低的阈值有助于合并重复框，但若设置过低，可能会把两个相近的目标合并为一个。

#### 输出模式

- **None：** 上电后不会立即开始推理，直到有其他触发条件运行。适用于仅在需要时才运行推理的低功耗场景。
- **MQTT：** 上电后立即开始推理，并通过 Wi-Fi 使用 MQTT 发送结果。
  - **SenseCraft official service（推荐）：** 为你的应用选择或创建一个 Dev Kit，MQTT 将自动完成配置。打开 [SenseCraft Data 平台](https://sensecap.seeed.cc) 以查看和处理接收的数据。
  - **Third-party service：** 使用你自己的或第三方的 MQTT broker，并手动配置参数。
- **GPIO：** 使用电平信号驱动 GPIO 引脚。
  - **Trigger conditions：** 使用对象、条件和置信度阈值来定义规则。
  - **Trigger actions：** 当条件满足时，设备可以将引脚拉高/拉低、将抓拍保存到 SD 卡、点亮 LED 等。
- **Serial：** 使用 [SSCMACore library](https://github.com/Seeed-Studio/Seeed_Arduino_SSCMA?tab=readme-ov-file#seeed_arduino_sscma-library) 通过 UART（GPIO）、I2C 或 SPI 导出推理数据。
...
- **SenseCraft AI model：** 从库中选择社区共享的模型。
- **My model：** 从你的私有列表中选择。
- **Upload model：** 上传在第三方平台训练的模型文件。提供模型名称和类别（Class ID 和 Class Name）。上传完成后，模型会显示在 **My model** 下。
...
- **Confidence threshold：** 与视觉模型中的作用相同；用于调节漏检与误检之间的权衡。
- **Output mode：** 音频模型的默认输出路径。
...
容器应用在 [**SenseCraft Fleet**](https://seeed-fleet.com?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_fleet_home) 上进行管理，并通过 [**SenseCraft AI**](https://sensecraft.seeed.cc/ai/home?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_ai_home) 部署（位于 **`Applications`** 下）：

- **SenseCraft Fleet** 是容器镜像所在的位置——你在这里上传镜像、定义其配置模式，并绑定你的设备。
...
**选择应用：** 从 SenseCraft Fleet 上的应用库（内置目录）中选择。如果你希望上传自定义容器应用，需要通过 [**SenseCraft Fleet**](https://seeed-fleet.com?utm_source=seeedstudio_wiki&utm_medium=referral&utm_campaign=wiki_to_sensecraft&utm_content=sensecraft_fleet_home) 添加。

**配置应用：** 每个应用都会暴露其作者在 SenseCraft Fleet 上定义的配置项。通常你不需要配置任何内容，除非你有特殊需求。

### 部署工作流

**步骤 1 — 选择应用：** 点击 **Deploy Container App**。SenseCraft AI 会从 SenseCraft Fleet 加载你的完整应用库（内置目录加上你上传的任何自定义应用），并以名称、图标和描述的形式展示每个应用。

**步骤 2 – 绑定设备：** 如果你已经有已绑定的设备，请跳到步骤 3。否则，请给设备上电，将其连接到网络，然后根据你的硬件类型进行绑定：
...
**步骤 4 — 配置应用：** 填写应用作者暴露的配置项。有关控件类型和默认值，请参见上面的[配置工作流](#configuration-workflow)部分。

**步骤 5 — 一键部署：** 点击 **Deploy**。SenseCraft AI 会显示实时进度指示器，并在每个阶段流式显示状态。如果任何步骤失败，弹窗中会显示具体错误，方便你修复后重试。使用新参数重新部署即可更新现有部署——之前的部署会被自动替换。
...
**一个合并后的 `.bin` 文件。** 如果你有单独的 bootloader、分区和应用二进制文件，请在上传前将它们合并。

- **Arduino IDE 2.x：** 只有在运行 **Sketch → Export Compiled Binary** 时才会生成合并后的二进制文件。输出的 `*.ino.merged.bin` 会写入到与你的 `.ino` 文件同级的 `build/` 文件夹中。
- **旧版 Arduino IDE / 手动合并：** **Export Compiled Binary** 会生成单独的 bootloader、分区和应用 `.bin` 文件——在上传前使用 `esptool.py merge_bin` 将它们合并。请参阅 Seeed Studio 论坛指南：[How to export a single merged `.bin` file from Arduino IDE](https://forum.seeedstudio.com/t/how-to-export-compiled-binary-create-a-single-bin-file-for-production-and-test/274990)。
...
JSON 流程不会嵌入模型二进制文件。请指定此流程应使用的模型：

- **设备内置：** 使用预装在 reCamera 上的模型，包括用于分类、检测、分割和姿态的 YOLO11n 变体。
...
<img src="https://github.com/ailyProject.png" alt="" style={{width:'1.25em',height:'1.25em',verticalAlign:'-0.3em',marginRight:'0.35em',borderRadius:'4px'}}/>[Aily Blockly](https://aily.pro) 是一个可视化编程环境。使用此模块，你可以一键将你在 Aily Blockly 中构建的程序烧录到边缘设备上。
