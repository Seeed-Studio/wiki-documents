---
title: Driving E-Paper and MicroSD Card with XIAO nRF54LM20A
description: ''
keywords:
  - xiao
  - nrf54lm20a
...
url: https://wiki.seeedstudio.com/xiao_nrf54lm20a_with_epaper_and_sdcard/
---
...
1. Modify the device tree file **app.overlay** and write the hardware configuration for the target display driver.
...
2. Modify **prj.conf** to enable software configurations.
...
3. Modify the **main.c** file and implement the display logic and content.
...
1. Modify the device tree file **app.overlay** and add SPI protocol configurations for SD card read and write operations.
...
2. Enable the SPI software configuration in **prj.conf**.
...
3. Add the logic for writing data to the SD card in **main.c**. Since the code is quite lengthy, it is recommended to refer to the sample code in the repository.
...
1. Insert the MicroSD card into the card slot of the Expansion Board for XIAO. Power on the device and open the serial monitor. The system will read the size of the SD card, test the mounting function, and then write files.

- Create a new folder named `some`
- Create a new file named `some.dat`
- Create a new file named `test.txt` and write content into it
...
2. Use a card reader to access the SD card, and you will find the three created files with content inside.
...
3. Open the TXT file. Its content is `XIAO nRF54LM20A SD Card Test`, which verifies successful writing to the SD card.
