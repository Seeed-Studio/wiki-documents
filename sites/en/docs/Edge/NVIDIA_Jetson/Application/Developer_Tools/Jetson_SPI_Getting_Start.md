...
JetPack 6 uses the Linux character device GPIO interface. The `DC` and `RES` pins are controlled through `libgpiod`, so no GPIO export step is required.
...
1. The selected GPIO chip is incorrect.
2. The selected line offset is incorrect.
3. The GPIO line is already used by another process or driver.
4. The program does not have permission to access `/dev/gpiochip*`.
